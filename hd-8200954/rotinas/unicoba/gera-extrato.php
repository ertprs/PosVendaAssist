...
try {
	...
	if (pg_num_rows($res) > 0 && strlen($msg_erro) == 0) {
		...
		for ($i = 0; $i < pg_num_rows($res); $i++) {
			...
			$sqlLGR = "UPDATE tbl_faturamento_item SET
					...
					AND (tbl_os_item.peca_obrigatoria OR tbl_peca.produto_acabado) 
	...
	if (strlen($bug) > 0) {
		...
		Log::envia_email($vet, 'Log - Extrato Unicoba', $bug);
	...
	Log::envia_email($data,Date('d/m/Y H:i:s')." - Unicoba - Erro na geração de extrato(gera-extrato.php)", $e->getMessage());
