	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			...
			if (!empty($linha)) {
				...
				$sql_produto = "SELECT produto FROM tbl_produto
								WHERE tbl_produto.referencia = UPPER(TRIM('$referencia_produto'))
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = utf8_decode('Wurth: Erros na importação de lista basica ') . date('d/m/Y');
