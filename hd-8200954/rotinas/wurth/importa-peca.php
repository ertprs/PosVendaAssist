 ...
 * importa-peca.php
 *
 * Importação de peças Vonder/DWT
 *
...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				list ($referencia, $descricao, $unidade, $origem_arq, $ipi) = explode ("\t",$linha);
				$original = array($referencia, $descricao, $unidade, $origem_arq, $ipi);
				...
				echo $unidade = strtim($unidade);
				echo $origem_arq = strtim($origem_arq);
				echo $ipi = 0;
				...
				echo $sql_peca = "SELECT tbl_peca.peca FROM   tbl_peca
