...
try {
	...
	echo $diretorio_origem = '/www/cgi-bin/' . $fabrica_nome . '/entrada';
	echo $arquivo_origem = 'telecontrol-produto.txt';

	$ftp = '/tmp/wurth/telecontrol-' . $fabrica_nome;
	...
	echo $arquivo = $diretorio_origem . '/' . $arquivo_origem;
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				echo $referencia = strtim($referencia);
                                echo $descricao = strtim($descricao);
                                echo $codigo_linha = strtim($codigo_linha);
                                echo $codigo_familia = strtim($codigo_familia);
                                echo $linha_aux = strtim($linha_aux);
                                echo $origem = strtim($origem);
                                echo $voltagem = strtim($voltagem);
                                echo $garantia = strtim($garantia);
                                echo $mao_de_obra = strtim($mao_de_obra);
