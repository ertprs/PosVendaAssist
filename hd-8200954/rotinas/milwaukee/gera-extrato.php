...
try {
	...
	if (pg_num_rows($res) > 0 && strlen($msg_erro) == 0) {
		...
		for ($i = 0; $i < pg_num_rows($res); $i++) {
			...
			$sql7 = "UPDATE tbl_extrato SET
					aprovado = current_date,
	...
	if (strlen($bug) > 0) {
		...
		Log::envia_email($vet, 'Log - Extrato milwaukee', $bug);
	...
	Log::envia_email($data,Date('d/m/Y H:i:s')." - milwaukee - Erro na geração de extrato(gera-extrato.php)", $e->getMessage());
