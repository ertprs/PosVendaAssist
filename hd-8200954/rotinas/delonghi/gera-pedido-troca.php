...
try {
    ...
    /* Log */
    $log = new Log2();
    $log->adicionaLog(array("titulo" => "Log erro Geração de Pedidos Delonghi")); // Titulo

    if (ENV == 'producao' ) {
    $log->adicionaEmail("helpdesk@telecontrol.com.br");
    } else {
//         $log->adicionaEmail("ronald.santos@telecontrol.com.br");
        $log->adicionaEmail("william.brandino@telecontrol.com.br");
    }

    $arquivo_err = "/tmp/delonghi/gera-pedido-troca-{$data_sistema}.err";
    $arquivo_log = "/tmp/delonghi/gera-pedido-troca-{$data_sistema}.log";
    system ("mkdir /tmp/delonghi/ 2> /dev/null ; chmod 777 /tmp/delonghi/" );
    ...
    #Tipo_pedido
    $sql = "select tipo_pedido from tbl_tipo_pedido where fabrica = ".$fabrica." and lower(descricao) = 'troca'";
    $resultP = pg_query($con, $sql);
    if(pg_last_error($con)){
        $logs_erro[] = "Erro por falta de tipo de pedido 'TROCA'";
        $log->adicionaLog("Erro por falta de tipo de pedido 'TROCA'");
        $log->adicionaLog("linha");
        throw new Exception ($msg_erro);
    }else{
                ...
                for($x = 0; $x < pg_num_rows($result); $x++){
                    ...
                    if(pg_last_error($con)){
                        ...
                        if (pg_last_error($con)) {
                            ...
                            $resultX = pg_query($con, $sql);
                            if(pg_last_error($con)){
                                $logs_erro[] = "Erro ao finalizar o pedido $pedido para o Posto: $codigo_posto - $nome_posto | ".pg_last_error($con);
                                $log->adicionaLog("Erro ao finalizar o pedido $pedido para o Posto: $codigo_posto - $nome_posto | ".pg_last_error($con));
                                $log->adicionaLog("linha");
                                $erro = "*";
                            }
                        }
