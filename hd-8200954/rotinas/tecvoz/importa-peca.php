...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				list ($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $ativo, $garantia, $multiplo) = explode ("\t",$linha);
				$original = array($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $ativo, $garantia, $multiplo);
