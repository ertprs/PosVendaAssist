	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				$descricao = cortaStr($descricao,80);
				...
				$sql_produto = "SELECT tbl_produto.produto FROM tbl_produto 
								WHERE upper(tbl_produto.referencia) = upper('$referencia') AND tbl_produto.fabrica_i = $fabrica";
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = utf8_decode('Elgin: Erros na importação de produtos ') . date('d/m/Y');
