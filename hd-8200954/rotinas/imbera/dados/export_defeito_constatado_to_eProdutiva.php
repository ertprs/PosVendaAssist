	...
	if ($value['tipo'] == 1) {
		...
		$codigo_array = explode('_',$value['codigo']);
		 ...
		 $sql = "SELECT defeito_constatado
			from tbl_defeito_constatado c
			join tbl_diagnostico d using(defeito_constatado) 
			join tbl_familia f on f.familia = d.familia 
			where c.fabrica = 158 
					and c.codigo = '" .$codigo_array[0]. "'
					and substring(f.descricao from 1 for 3) = '".$codigo_array[2]. "'
					and d.ativo
					";
//		 echo $sql;
		$res = pg_query($sql);
//		die;
		if (pg_num_rows($res)==0) {
			...
			$curl = curl_init();
			$array_put = array("statusModel"=>0,"tipo"=>1,"titulo"=>$value['titulo']);
...
WHERE d.fabrica = 158 and d.ativo";
...
while($row = pg_fetch_assoc($res)){
    ...
    $dados = array('titulo'=>trim(utf8_encode($row['titulo'])),'codigo'=>trim($row['codigo']),'tipo'=>'1');
