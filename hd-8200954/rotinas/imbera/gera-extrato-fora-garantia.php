...
#$dia_mes     = "05";
#$dia_extrato = "2017-01-05 23:59:00";
...
$sql = "SELECT 
        ...
        o.posto,
        CASE
        WHEN json_field('unidadeNegocio',oce.campos_adicionais::text) IN('6107','6101','6108','6103','6102','6106','6104','6105') THEN
        '6101'
        WHEN json_field('unidadeNegocio',oce.campos_adicionais::text) IN('6200') THEN
        '6200'
        ELSE json_field('unidadeNegocio',oce.campos_adicionais::text)
        ...
        AND json_field('unidadeNegocio',oce.campos_adicionais::text) NOT IN ('6300','6700')
        ) AS os ON os.posto = tbl_posto_fabrica.posto
        WHERE tbl_posto_fabrica.fabrica = {$fabrica} 
        AND os.unidade_negocio IS NOT NULL
  ...
  $unidade_negocio = $os_posto[$i]["unidade_negocio"];
    ...
    */
    ...
    if ($fora_garantia == "t") {
      ...
      $sql = "
        SELECT tbl_os_item.peca, SUM(tbl_os_item.qtde) AS qtde, tbl_os_item.peca_obrigatoria
        FROM tbl_os_item
        INNER JOIN tbl_servico_realizado ON tbl_servico_realizado.servico_realizado = tbl_os_item.servico_realizado AND tbl_servico_realizado.fabrica = {$fabrica}
        INNER JOIN tbl_os_produto ON tbl_os_produto.os_produto = tbl_os_item.os_produto
        INNER JOIN tbl_os ON tbl_os.os = tbl_os_produto.os AND tbl_os.fabrica = {$fabrica}
        ...
        GROUP BY tbl_os_item.peca, tbl_os_item.peca_obrigatoria
      ";
      $res = $pdo->query($sql);
      ...
      if ($res->rowCount() > 0) {
        ...
        foreach ($pecas as $peca) {
          ...
          while ($total_peca > 0) {
            $sqlMovimento = "
              SELECT qtde_entrada, qtde_usada_estoque, faturamento
              FROM tbl_estoque_posto_movimento
              WHERE fabrica = {$fabrica}
              AND posto = {$posto}
              AND peca = {$peca['peca']}
              AND qtde_entrada IS NOT NULL
              AND (qtde_usada_estoque IS NULL OR qtde_usada_estoque < qtde_entrada)
            ...
            $insert = "
              ...
              (extrato, posto, peca, qtde, devolucao_obrigatoria, faturamento_item)
              VALUES
              ({$extrato}, {$posto}, {$peca['peca']}, {$qtde_update}, {$peca_obrigatoria}, {$faturamento_item['faturamento_item']})
            ";
    ...
    * Calcula o Extrato
    */
    $total_extrato = $classExtratoImbera->calcula($extrato,$posto, $unidade_negocio);
