...
$sql = "
        SELECT DISTINCT exportado FROM tbl_os WHERE fabrica = {$login_fabrica} AND exportado::date = '".date("Y-m-d")."'
";
...
function saveIntoFile($fileData) {
	...
	ftp_chdir($conexao_ftp, "FTP_facelect");
	ftp_chdir($conexao_ftp, "410");
	ftp_chdir($conexao_ftp, "Inbound");
	ftp_chdir($conexao_ftp, "InbImbera");

        ftp_put($conexao_ftp, $filename_ftp, $filename, FTP_ASCII);

        $arquivos = ftp_nlist($conexao_ftp, ".");

        if (!array_search($filename_ftp, $arquivos)) {
            unlink($filename);
