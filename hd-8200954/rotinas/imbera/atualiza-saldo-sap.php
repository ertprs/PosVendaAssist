...
while ($e = pg_fetch_object($estoque)) {
    try {
	    ...
	    $sql = "
                SELECT peca_id, peca_referencia, posto_id, posto_codigo, nota_fiscal, SUM(qtde) AS qtde, SUM(qtde_usada) AS qtde_usada, SUM(saldo) AS saldo
                FROM tmp_imbera_saldo_sap
                WHERE posto_id = {$e->posto_id}
                AND peca_id = {$e->peca_id}
		AND nota_fiscal::integer >= {$nf_antiga_saldo}
		GROUP BY peca_id, peca_referencia, posto_id, posto_codigo, nota_fiscal
		...
		$sql = "
		    ...
		    AND epm.qtde_entrada >= {$nf->saldo}
		    AND epm.nf::int = {$nf->nota_fiscal};
		";
	...
	echo $sql."\n";
		...
		while ($qtde > 0) {
		    ...
		    $sql = "
                        SELECT qtde_entrada AS qtde, COALESCE(qtde_usada, 0) AS qtde_usada, faturamento, nf::integer
