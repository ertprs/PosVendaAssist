...
try {
    ...
    /**
     * OS - vw_os_cadence > os_cadence
     */
    $errors = array();

    echo "Table: os_cadence\n";

    $sql = "SELECT * FROM vw_os_cadence";
    $res = pg_query($con, $sql);
    ...
    if (pg_num_rows($res) > 0) {
        echo $columns = getColumns("vw_os_cadence");
        ...
        while ($row = pg_fetch_array($res)) {
            ...
            $sqlOsExists = "SELECT os FROM os_cadence WHERE os = {$row['os']}";
    ...
    echo "======================================\n\n";
    /**
     * OS Item - vw_os_item_cadence > os_item_cadence
     */
    $errors = array();

    echo "Table: os_item_cadence\n";

    $sql = "SELECT * FROM vw_os_item_cadence";
    $res = pg_query($con, $sql);
    ...
    if (pg_num_rows($res) > 0) {
        $columns = getColumns("vw_os_item_cadence");
	    ...
	    $sqlItemExists = "SELECT os_item FROM os_item_cadence WHERE os_item = {$row['os_item']} ";
 		...
 		$sqlItem = "
                ...
                ";
                pg_query($dbcadence, $sqlItem);

                $updatedITens++;
                echo "atualizado - OSItem\n";
            } else {
                $sqlItem = "
                    INSERT INTO os_item_cadence 
                    (".implode(", ", array_keys($data)).")
                    VALUES 
                    (".implode(", ", $data).")
                ";
    ...
    /**
     * Historico - vw_os_historico_cadence > os_historico_cadence
     */
    $errors = array();

    echo "Table: os_historico_cadence\n";

    $sql = "SELECT * FROM vw_os_historico_cadence";
    $res = pg_query($con, $sql);
    ...
    if (pg_num_rows($res) > 0) {
        $columns = getColumns("vw_os_historico_cadence");
        ...
        while ($row = pg_fetch_array($res)) {
            ...
            $sqlHdExists = "SELECT os FROM os_historico_cadence WHERE os = {$row['os']}";
            $qryHdExists = pg_query($dbfabrica, $sqlHdExists);

            if (pg_num_rows($qryHdExists) > 0) {
                $update = array();

		$sql_his = "SELECT status_checkpoint,data FROM os_historico_cadence WHERE os = {$row['os']} order by 2 DESC LIMIT 1 ";
		...
		if ((($row['status_checkpoint']) <> '9') or ($status_checkpoint <>  ($row['status_checkpoint'])) ){
			$sqlHd = "
			    INSERT INTO os_historico_cadence
				    (".implode(", ", array_keys($data)).")
			    VALUES
				    (".implode(", ", $data).")
			";
		...
		echo "E R R O: os_historico_cadence";
		die;
            }
        }
...
#
    ...
    mail("marisa.silvana@telecontrol.com.br, marisa.silvana@telecontrol.com.br", "Cadence - Exportação de Dados", $log);
} catch(Exception $e) {
