<?php
    ...
    */
    $logClass = new Log2();
    $logClass->adicionaLog(array("titulo" => "Log erro Abastecimento de Estoque - Cadence")); // Titulo
    ...
    if ($postos_controlam_estoque != false) {
        foreach ($postos_controlam_estoque as $value) {
            ...
            try {
                ...
                foreach ($estoque as $key => $value) {
                    if (in_array($value["peca"],$pecasPedido)) { 
    ...
    */
    if (!empty($msg_erro)) {
        ...
        $fp = fopen("/tmp/cadence/log-erro.txt", "a");
