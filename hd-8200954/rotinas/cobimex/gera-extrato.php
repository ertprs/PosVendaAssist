...
try {
    ...
    if (pg_num_rows($res) > 0 && strlen($msg_erro) == 0) {
        ...
        for ($i = 0; $i < pg_num_rows($res); $i++) {
            ...
            $res_extrato = pg_query($con,$sql_extrato);
            echo $msg_erro = pg_errormessage($con);
            ...
            if(empty($msg_erro)){
				...
				$sqle = "UPDATE tbl_extrato SET  aprovado = current_timestamp,
    ...
    if (strlen($bug) > 0) {
        ...
        Log::envia_email($vet, 'Log - Extrato Cobimex', $bug);

    }

} catch (Exception $e) {

    Log::envia_email($data,Date('d/m/Y H:i:s')." - cobimex - Erro na geração de extrato(gera-extrato.php)", $e->getMessage());
