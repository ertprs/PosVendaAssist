<?php
	define('APP','Importa Pedido Cancelado - Tectoy'); // Nome da rotina, para ser enviado por e-mail
	...
	try {
		...
		if ( ENV == 'testes' ) {
			...
			$dir 	= '/home/' . $vet['fabrica'] . '/' . $vet['fabrica'] . '_telecontrol';
		}
		...
		$sql = "UPDATE ".$vet['fabrica']."_cancelado SET os = tbl_os_produto.os
				...
				WHERE tbl_os_item.pedido_item = ".$vet['fabrica']."_cancelado.pedido_item
		...
		$sql = "INSERT INTO tbl_pedido_cancelado (fabrica, data, posto, pedido, pedido_item, os, peca, qtde, motivo) 
				(SELECT $fabrica,data, posto, pedido, pedido_item, os, peca, qtde, motivo FROM ".$vet['fabrica']."_cancelado )";
