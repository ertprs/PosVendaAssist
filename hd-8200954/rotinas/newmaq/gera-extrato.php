...
try {
	...
	$sql9 = " select os into temp newmaq_extrato from tbl_os_status join tbl_os_extra using(os) where tbl_os_extra.extrato isnull and tbl_os_status.status_os = 104 and fabrica_status = $fabrica;
    ...
    if (pg_num_rows($res) > 0 && strlen($msg_erro) == 0) {
		...
		for ($i = 0; $i < pg_num_rows($res); $i++) {
			...
			$sql4 = "UPDATE tbl_os_extra SET extrato = $extrato
						...
						AND   tbl_os.finalizada    <= '$dia_extrato' 
						AND   tbl_os.finalizada::date <= current_date
						AND   tbl_os.os not in (select os from newmaq_extrato)";
			$res4      = pg_query($con, $sql4);
			$msg_erro .= pg_last_error($con);
	...
	if (strlen($bug) > 0) {
		Log::envia_email($vet, 'Log - Extrato newmaq', $bug);
	...
	Log::envia_email($data,Date('d/m/Y H:i:s')." - NEWMAQ - Erro na geração de extrato(gera-extrato.php)", $e->getMessage());
