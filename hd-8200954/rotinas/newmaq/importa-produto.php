 ...
 * importa-produto.php
 *
 * Importação de produtos newmaq/DWT
 *
...
try {
	...
	if (ENV == 'teste') {
		$arquivo = "entrada/telecontrol-produto.txt";
	}
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				Referência	Código do Produto - tamanho máximo de 20 caracteres - sem caracteres especiais
				Descrição	Nome do Produto - tamanho máximo de 80 caracteres - sem caracteres especiais
				Linha		Nome da Linha (Serve para diferenciar os postos e filtros para relatórios. Ex: Linha Branca, Refrigeração, Eletrônicos...)
				Família		Nome da Família (Produtos com os mesmo defeitos. Ex.DVD Palyer, Bebedouro Eletrônico, Bebedouro Convencional...)
				Origem		NAC ou IMP
				Voltagem	12v, 110v, 127v, 220v, Bivolt, Bateria ou Pilha
				Garantia	Meses de garantia do produto
				Mão de Obra Posto	Valor de mão de obra do produto.
				Número de Série Obrigatório	t ou f (indica se a digitação do número de série será obrigatória na abertura da OS)
				NCM	Numérico
				...
				*/
				...
				// print "referencia: $referencia    descrição: $descricao    linha: $cod_linha    familia: $codigo_familia   garantia: $garantia";
				...
				if(ENV == "teste"){
					printf("$referencia - $descricao - $codigo_linha - $codigo_familia - $origem - $voltagem - $garantia - $mao_de_obra - $numero_serie_obrigatorio - $ncm");
				}
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = utf8_decode('newmaq: Erros na importação de produtos ') . date('d/m/Y');
