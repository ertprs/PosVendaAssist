<?php
	define('APP','Importa Pedido Cancelado - Arge'); // Nome da rotina, para ser enviado por e-mail
	...
	try {
		...
		$vet['dest']    = ENV == 'testes' ? 'ronald.santos@arge.com.br' : 'helpdesk@arge.com.br';
		$vet['log']     = 1;

		/* Log */
    	$log = new Log2();

// 		$file = 'pedido_cancelado.txt';

		if ( ENV == 'testes' ) {
			$dir 	= '/home/ronald/perl/'.$vet['fabrica'].'/entrada';
		} else {
			$dir 	= '/home/' . $vet['fabrica'] . '/' . $vet['fabrica'] . '-telecontrol';
		}

			$array_arquivos = scandir($dir);
		    sort($array_arquivos);
		    $array_arquivos = array_slice($array_arquivos,2);

		foreach($array_arquivos as $chave=>$valor){
			$file = null;
			if(preg_match("/pedido-cancelado_[0-9-_\.]+\.txt/",$valor)){
				$file = $valor;
			}else{
				continue;
			}
				...
				$sql = "INSERT INTO tbl_pedido_cancelado (fabrica, data, posto, pedido, peca, qtde, motivo)
						(SELECT $fabrica,data, posto, pedido, peca, qtde, motivo FROM ".$vet['fabrica']."_cancelado )";
				...
				if ( !empty($msg_erro) ) {
					...
					$log->adicionaLog("Erro ao Cancelar os Pedidos");

					/* Log de Erro */
					$log->adicionaTituloEmail("Logs de Erro da Rotina de Importação de Pedidos Cancelados - Arge");
				...
				system ("mv $dir/$file /tmp/".$vet['fabrica']."/arge-pedido-cancelado-$data.txt");
