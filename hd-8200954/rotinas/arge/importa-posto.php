 ...
 * importa-posto.php
 *
 * Importação de postos Thermosystem
 *
 */
...
//	$diretorio_origem = '/mnt/home/fabiano/public_html/assist/rotinas/'.$fabrica_nome; //  teste local
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("CNPJ Inválido: $cnpj");
				...
				if (pg_num_rows($query_posto) == 0) {
					...
					if (pg_last_error()) {
						...
						$log->adicionaLog("Erro ao inserir o Posto com o CNPJ: $cnpj");
					...
					#$sql = "UPDATE tbl_posto SET CEP='$cep' WHERE tbl_posto.posto = $posto";
					#$query = pg_query($con, $sql);
 					#if (pg_last_error()) {
                                        #	array_push($original, 'erro');
	                                #        $log = implode(";", $original);
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Posto não encontrado com o CNPJ: $cnpj");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao Inserir / Atualzar o Posto: $nome_fantasia");
		...
		if (count($dadosDivergentes["Base"]) > 0) {
			...
			$assunto =  ucfirst($fabrica_nome) . utf8_decode("  - Importação de postos - Razão Social e/ou IE Divergente");
		...
		if ( !empty($erro) ) {
			...
			$log->adicionaLog("Erro ao Importar Postos");

			/* Log de Erro */
            $log->adicionaTituloEmail("Logs de Erro da Rotina de Importação de Postos - Arge");
