...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao selecionar a Linha $codigo_linha");
				...
				if (pg_num_rows($query_linha) == 1) {
					...
					$log->adicionaLog("O código $codigo_linha tem varias Linhas cadastradas ");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao selecionar a familia com o código $codigo_familia");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao selecionar o produto com o código $referencia");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao Inserir / Arualizar o produto: $referencia");
		...
		if ( !empty($erro) ) {
			...
			$log->adicionaLog("Erro ao Importar Produtp");

			/* Log de Erro */
            $log->adicionaTituloEmail("Logs de Erro da Rotina de Importação de Produtos - Arge");
