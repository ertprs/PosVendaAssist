...
try {
    ...
    $sql = "SELECT
				...
				AND tbl_os_status.status_os IN (13,19,62,64,70,139)
				)
				LEFT JOIN tbl_os_status st2 ON tbl_os.os=st2.os
				AND st2.os_status=(
				SELECT MAX(os_status)
				FROM tbl_os_status
				WHERE tbl_os_status.os=tbl_os.os
				AND tbl_os_status.status_os IN (98,99,100,101)
				)
				LEFT JOIN tbl_os_status st3 ON tbl_os.os=st3.os
				AND st3.os_status=(
				SELECT MAX(os_status)
				FROM tbl_os_status
				WHERE tbl_os_status.os=tbl_os.os
				AND tbl_os_status.status_os IN (118,187,185)
				)
				...
				AND (tbl_os_status.status_os IN(19,64,139) OR tbl_os_status.status_os IS NULL)
				AND (st2.status_os IN(99,100) OR st2.status_os IS NULL)
				AND (st3.status_os IN(187) OR st3.status_os IS NULL);

				SELECT DISTINCT posto from tmp_pedido_arge ; 				";
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - Arge - Erro na geração de pedido(gera-pedido.php)", $msg);
}?>
