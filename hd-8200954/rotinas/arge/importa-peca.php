 ...
 * importa-peca.php
 *
 * Importação de peças Thermosystem
 */
...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao Atualizar / Inserir a Peça $referencia");
		...
		if(!empty($erro)){
			...
			/* Log de Erro */
            $log->adicionaTituloEmail("Logs de Erro da Rotina de Importação de Peças - Arge");
