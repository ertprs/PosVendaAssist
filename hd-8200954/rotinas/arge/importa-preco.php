...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if ($preco_peca == 0 ){
					...
					$log->adicionaLog("Peça $referencia_peca sem preço");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Peça $referencia_peca sem Tabela");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Peça $referencia_peca sem cadastro");
				...
				if (pg_num_rows($query_peca) == 1) {
					...
					$log->adicionaLog("Peça $referencia_peca esta cadastrada mais de uma vez no sistema");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Tabela $tabela_id sem itens");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao inserir item na tabela $tabela_id, com a peça $peca_id");
		...
		if ( !empty($erro) ) {
			...
			$log->adicionaLog("Erro ao Importar Preços");

			/* Log de Erro */
            $log->adicionaTituloEmail("Logs de Erro da Rotina de Importação de Preços - Arge");
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = ucfirst($fabrica_nome) . utf8_decode(': Erros na importação de preço ') . date('d/m/Y');
