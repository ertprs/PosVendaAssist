...
try {
	...
	$ftp = '/tmp/arge/telecontrol-' . $fabrica_nome;
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_last_error()) {
	  				...
	  				$log->adicionaLog("Erro ao Importar a Lista Básica, Produto: $referencia_produto");
				...
				if (pg_num_rows($query_produto) == 1) {
					...
					$log->adicionaLog("Erro ao Importar a Lista Básica, Foram encontrados mais de um Produto com a referencia $referencia_produto");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao Importar a Lista Básica, Não foi possivel encontrar a peça $referencia_peca");
				...
				if (pg_num_rows($query_peca) == 1) {
					...
					$log->adicionaLog("Erro ao Importar a Lista Básica, Foi encotrada mais de uma peça com a referencia $referencia_peca");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao encontrar o Produto: $produto_id e Peça: $peca_id na Lista Básica");
				...
				if (pg_last_error()) {
					...
					$log->adicionaLog("Erro ao Inserir / Atualizar a Lista Básica, Produto: $referencia_produto e Peça: $peca_id");
		...
		if(!empty($erro)){
			...
			/* Log de Erro */
            $log->adicionaTituloEmail("Logs de Erro da Rotina de Importação de Lista Básica - Arge");
