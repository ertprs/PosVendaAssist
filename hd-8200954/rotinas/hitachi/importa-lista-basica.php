...
try {
	...
	$ftp = '/tmp/hitachi/telecontrol-' . $fabrica_nome;

	if (ENV == 'teste') {
		$ftp = "/home/william/hitachi/entrada";
	}
	...
	if (ENV == 'teste') {
		$arquivo = '/home/william/hitachi/entrada/' . $arquivo_origem;
	}
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_num_rows($query_produto) == 1) {
					...
					array_push($original, 'erro produto não encontrado');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_last_error()) {
					array_push($original, 'erro na consulta de peça');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_num_rows($query_peca) == 1) {
					...
					array_push($original, 'erro peça não encontrada ');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
		...
		system("mv $arquivo /tmp/$fabrica_nome/lista-basica/telecontrol-lista-basica-$data_arq_process.txt");
