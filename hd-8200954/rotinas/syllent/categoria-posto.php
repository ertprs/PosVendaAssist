...
$sqlOrcamentosAprovados = "
                                SELECT tbl_os.os
                                FROM tbl_os
                                JOIN tbl_status_os ON tbl_status_os.status_os = tbl_os.status_os_ultimo
                                WHERE   tbl_os.fabrica = $fabrica
                                AND     tbl_os.posto            = ".$postos->posto."
                                AND     tbl_os.excluida         IS NOT TRUE
                                AND     tbl_status_os.descricao NOT IN('Aguardando Orçamento','Aguardando Aprovação','Orçamento Reprovado')
                                AND     tbl_os.data_abertura BETWEEN '".date('Y-m-01')."' AND '".date('Y-m-t')."'
                            ";

$resOrcamentosAprovados = pg_query($con,$sqlOrcamentosAprovados);
echo "<pre>".print_r(pg_num_rows($resOrcamentosAprovados ),1)."</pre>";exit;
