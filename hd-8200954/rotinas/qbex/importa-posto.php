	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				$tipo_posto = strtim($tipo_posto);
				$linha = strtim($linha);
				$ponto_saida = strtim($ponto_saida);
				echo $inicio_horario = strtim($inicio_horario);
				echo $final_horario = strtim($final_horario);
				$horario = strtim($horario);
				...
				if (pg_num_rows($query_posto) == 0) {
					echo $sql = "INSERT INTO tbl_posto (
											nome,
											nome_fantasia,
											cnpj,
											ie,
				...
				if (pg_num_rows($query) == 0) {
					echo $sql = "INSERT INTO tbl_posto_fabrica (
												posto,
												fabrica,
												senha,
												tipo_posto,
					...
					echo $sql = "UPDATE tbl_posto_fabrica SET
				...
				echo "passei aqui....";
				echo $sql_tecnico = "SELECT tbl_tecnico.tecnico FROM tbl_tecnico WHERE tbl_tecnico.cpf = '$cnpj'";
                                $query_tecnico = pg_query($con, $sql_tecnico);

				echo $telefone = cortaStr($telefone, 20);

                                if (pg_num_rows($query_tecnico) == 0) {
					  echo $sql = "INSERT INTO tbl_tecnico (
                                                                                        posto,
                                                                                        fabrica,
											nome,
                                                                                        cpf,
						 ...
						 echo $sql = "UPDATE tbl_tecnico SET
										...
										complemento = '$complemento',
                                                                                ...
                                                                                cidade = (E'$cidade'),
                                                                                estado = '$estado',
										inicio_trabalho = '$inicio_horario',
										fim_trabalho = '$final_horario'
                                                                WHERE tbl_tecnico.cpf = '$cnpj'
                                                                AND tbl_tecnico.fabrica = $fabrica";
				...
				echo "ESTOU AQUI...............................";
   				...
   				echo $sql_posto_linha = "SELECT posto FROM tbl_posto_linha WHERE posto = $posto AND tabela = 858 and linha = $linha";
                                $query_posto_linha = pg_query($con, $sql_posto_linha);

                                if (pg_num_rows($query_posto_linha) == 0) {
                                          echo $sql = "INSERT INTO tbl_posto_linha(
                                                                                        posto,
                                                                                        tabela,
                                                                                        linha,
                                                                                        tabela_posto
                                                 ...
                                                 echo "nao tem UPDATE TBL-POSTO_lINHA";
                                }
                                $query = pg_query($con, $sql);
                                if (pg_last_error()) {
				          array_push($original, 'erro insert');
