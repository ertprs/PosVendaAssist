<?php
	...
	if (ENV == "prod") {
		try {
			$soap = new SoapClient("https://cws.correios.com.br/logisticaReversaWS/logisticaReversaService/logisticaReversaWS?wsdl", array("trace" => 1, "exception" => 0,'authorization' => 'Basic', 'login'   => 'qbex.computador', 'password' => 'qbex2016'));
        } catch (Exception $e) {
            ...
            $mail->Body = "ERRO AO CONECTAR SERVIDOR DOS CORREIOS...<br/> Fabrica: Qbex <br/> Data: ".date('d/m/Y H:i:s')." <br/>";
            $mail->Send();
    ...
    $sql_rastreio = "SELECT conhecimento, numero_postagem FROM tbl_faturamento_correio 
                        WHERE fabrica = $login_fabrica  
						AND conhecimento  notnull
					   and length(trim(numero_postagem)) > 3
                        AND data >= CURRENT_DATE - interval '3 months'";
    ...
    for($i=0; $i<pg_num_rows($res_rastreio); $i++){
        ...
        foreach($conhecimento as $linhaConhecimento){
            ...
            $metodo         = "buscaEventos";
            $buscaEventos   = (object) array("usuario" => $dados_acesso['usuario'], "senha" => 'qbex2016', "tipo" => "L", "resultado" => "T", "lingua" => 101,"objetos" => "$cod_conhecimento");
