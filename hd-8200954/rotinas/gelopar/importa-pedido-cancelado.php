<?php
    define('APP','Importa Pedido Cancelado - Gelopar'); // Nome da rotina, para ser enviado por e-mail
    ...
    try {
        ...
        $vet['dest']    = ENV == 'testes' ? 'lucas.carlos@telecontrol.com.br' : array('wally.jarek@gelopar.com.br','sidney.sanches@gelopar.com.br','fernanda.carlota@gelopar.com.br','felipe.barbosa@gelopar.com.br','sergio.jash@gelopar.com.br');
        $vet['log']     = 1;
		...
		if (!file_exists("{$dir}/{$file}")) {
			$phpCron->termino();
			die("Não existia arquivo: {$dir}/{$file}");
		}

		if (filesize("{$dir}/{$file}") == 0) {
			$to = "sidney.sanches@gelopar.com.br";
			$subj = "Telecontrol: Arquivo de integração de pedido cancelado vazio";
			$mssg = "O arquivo telecontrol-pedido-cancelado.txt estava vazio.";
			mail($to, $subj, $mssg);
