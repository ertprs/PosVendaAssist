...
$fabrica_nome = "brotherInternational";
...
try {
    ...
    $login_fabrica = 203;
    $vet['fabrica'] = 'brotherInternational';
    ...
    if ($numrows) {
        ...
        for ($i = 0; $i < $numrows; $i++) {
            ...
            $tipo_trans                 = trim(pg_fetch_result($res, $i,'tipo_trans'));
            ...
            for ($x = 0; $x < $tot_pecas; $x++) {
                ...
                fwrite($fp,'32'              . ";"); #c-cod-rep           - codigo representante, pode ser sempre ZERO
                ...
                if ($tipo_pedido) {                   #c-tab-finan         - tabela financiamento
                  ...
                  if ($tipo_pedido == "Garantia") {
                    if($devolucao_obrigatoria == "YES"){
                      $tipo_pedido = "VENDA"; # combinado com o Alessandor Baccin para imprimir NF com outro CFOP difenten de garantia sem devolução obrigatória (enviado no email do Samuel)
	...
	if (file_exists($file) and (filesize($file) > 0)) {
		...
		$destino = "pedido-assist-$data.sdf";
		copy($file, '/home/brotherInternational/telecontrol-brotherInternational/'.$destino);
