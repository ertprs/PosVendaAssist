...
try {
    ...
    if(file_exists($origem.$file)){
		$sql = "DROP TABLE IF EXISTS brotherInternational_recibo;";
		$res = pg_query($con,$sql);
		$msg_erro .= pg_errormessage($con);


		$sql = "CREATE TABLE brotherInternational_recibo (
				data 		text,
				cnpj		text,
				tipo 		text,
				txt_pedido  text,
		...
		foreach($linhas AS $linha){
			...
			if(!empty($txt_pedido)){
				...
				$sql = "INSERT INTO brotherInternational_recibo(data,cnpj,tipo,txt_pedido,txt_qtde) VALUES('$data','$cnpj', '$tipo','$txt_pedido','$txt_qtde')";
				$res = pg_query($con,$sql);
				$msg_erro .= pg_errormessage($con);
		...
		$sql = "UPDATE brotherInternational_recibo SET 
					cnpj 			 = trim(cnpj),
					txt_pedido       = trim(txt_pedido)";
		$res = pg_query($con,$sql);

		$sql = "ALTER TABLE brotherInternational_recibo ADD COLUMN posto INT4";
		$res = pg_query($con,$sql);
		$msg_erro .= pg_errormessage($con);
		
		$sql = "ALTER TABLE brotherInternational_recibo ADD COLUMN pedido INT4";
		$res = pg_query($con,$sql);
		$msg_erro .= pg_errormessage($con);

		$sql = "UPDATE brotherInternational_recibo SET posto = tbl_posto_fabrica.posto
				FROM tbl_posto_fabrica, tbl_posto
				WHERE tbl_posto.cnpj = brotherInternational_recibo.cnpj
				AND tbl_posto.posto = tbl_posto_fabrica.posto
				AND tbl_posto_fabrica.fabrica = $login_fabrica";
		$res = pg_query($con,$sql);
		$msg_erro .= pg_errormessage($con);

		$sql = "UPDATE brotherInternational_recibo SET pedido = tbl_pedido.pedido
				FROM tbl_pedido
				WHERE tbl_pedido.pedido = brotherInternational_recibo.txt_pedido::numeric
				AND tbl_pedido.fabrica = $login_fabrica";
		$res = pg_query($con,$sql);
		$msg_erro .= pg_errormessage($con);

		$sql = "DROP TABLE brotherInternational_recibo_sem_posto";
        $res = pg_query($con,$sql);
        $msg_erro .= pg_errormessage($con);

        $sql = "SELECT * INTO brotherInternational_recibo_sem_posto 
                        FROM brotherInternational_recibo 
                        WHERE posto IS NULL";
        $res = pg_query($con,$sql);
        $msg_erro .= pg_errormessage($con);

        $sql = "DROP TABLE brotherInternational_recibo_sem_pedido";
        $res = pg_query($con,$sql);
        $msg_erro .= pg_errormessage($con);

        $sql = "SELECT * INTO brotherInternational_recibo_sem_pedido 
                        FROM brotherInternational_recibo 
                        WHERE pedido IS NULL";
        $res = pg_query($con,$sql);
        $msg_erro .= pg_errormessage($con);

        $sql = "DELETE FROM brotherInternational_recibo 
        ...
        $sql = "SELECT fn_atualiza_pedido_recebido_fabrica (tbl_pedido.pedido,$login_fabrica, current_date )
                FROM   brotherInternational_recibo, tbl_pedido
                WHERE tbl_pedido.pedido = brotherInternational_recibo.pedido
                AND tbl_pedido.posto = brotherInternational_recibo.posto
        ...
        if (!empty($msg_erro)) {
			$msg_erro .= "\n\n".$log_erro;
			$fp = fopen("/tmp/brotherInternational/pedido-recebidos.err","w");
			fwrite($fp,$msg_erro);
			fclose($fp);
			...
			system("mv $origem$file /tmp/brotherInternational/pedido-recebidos".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - brotherInternational - Importa pedidos recebidos (importa-pedido-recebido.php)", $msg);
}?>
