...
try {
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS brother_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);


	  $sql = "CREATE TABLE brother_nf (
				  txt_cnpj           text,
	...
	foreach($linhas AS $linha){
		...
		if(!empty($txt_cnpj)){
				...
				$sql = "INSERT INTO brother_nf ( txt_cnpj                 ,
								    txt_nota_fiscal         ,
								    txt_serie               ,
								    txt_emissao             ,
								    txt_cfop                ,
	...
	$sql = "ALTER TABLE brother_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf ADD COLUMN saida DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE brother_nf SET posto = tbl_posto.posto
			FROM tbl_posto,tbl_posto_fabrica
			WHERE trim(brother_nf.txt_cnpj) = tbl_posto.cnpj
	...
	$sql = "DROP TABLE IF EXISTS brother_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT * INTO brother_nf_sem_posto FROM brother_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM brother_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DROP TABLE IF EXISTS brother_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "CREATE TABLE brother_nf_item (
				txt_cnpj        text,
				txt_nota_fiscal text,
				txt_serie       text,
				txt_referencia  text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(strlen(trim($txt_cnpj)) > 0 AND trim($txt_pedido_item) != "?"){
			...
			$txt_base_icms = str_replace("\r","",$txt_base_icms);
            		...
            		$res = pg_query($con,"BEGIN");
	                $sql = "INSERT INTO brother_nf_item    (
                                                        txt_cnpj        ,
														txt_nota_fiscal ,
														txt_serie       ,
														txt_referencia  ,
	...
	$sql = "UPDATE brother_nf_item SET
				txt_cnpj 		= trim(txt_cnpj), 
				txt_nota_fiscal = trim(txt_nota_fiscal) , 
				txt_serie 		= trim(txt_serie) , 
				txt_referencia 	= trim(txt_referencia) , 
	...
	$sql = "ALTER TABLE brother_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN aliq_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN base_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN valor_subs_trib FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN base_subs_trib FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN os_item INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE brother_nf_item ADD COLUMN devolucao boolean";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE brother_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE brother_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE brother_nf_item
			SET pedido = tbl_pedido_item.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE brother_nf_item.txt_pedido::integer = tbl_pedido_item.pedido
			AND   brother_nf_item.txt_pedido_item::integer = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE brother_nf_item
                    ...
                    FROM tbl_os_item
                    WHERE brother_nf_item.pedido = tbl_os_item.pedido
                    AND   brother_nf_item.pedido_item = tbl_os_item.pedido_item
	...
	$sql = "UPDATE brother_nf_item
			...
			FROM  tbl_peca
			WHERE brother_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE brother_nf
				SET pedido = brother_nf_item.pedido
			FROM brother_nf_item
			WHERE trim(brother_nf.txt_nota_fiscal) = trim(brother_nf_item.txt_nota_fiscal)
			AND  brother_nf.posto = brother_nf_item.posto";
	$res = pg_query($con,$sql);
$msg_erro .= pg_errormessage($con);

	#------------ Desconsidera Notas ja Importadas ------------------
	$sql = "DELETE FROM brother_nf_item
	...
	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS brother_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT brother_nf.*
			INTO brother_nf_sem_itens
			FROM brother_nf
			LEFT JOIN brother_nf_item ON trim(brother_nf.txt_nota_fiscal) = trim(brother_nf_item.txt_nota_fiscal)
			WHERE brother_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM brother_nf
			USING brother_nf_sem_itens
			WHERE brother_nf.txt_nota_fiscal = brother_nf_sem_itens.txt_nota_fiscal
			AND   brother_nf.txt_serie       = brother_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						brother_nf.emissao         ,
						brother_nf.saida         ,
						substring(brother_nf.txt_transp, 1,30),
						brother_nf.posto           ,
						brother_nf.total           ,
						brother_nf.txt_cfop        ,
						brother_nf.txt_nota_fiscal ,
						brother_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = brother_nf.pedido),
						brother_nf.txt_natureza
				FROM brother_nf
				LEFT JOIN tbl_faturamento ON  brother_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  brother_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE brother_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE brother_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE brother_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = brother_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = brother_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE IF EXISTS brother_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);


	$sql = "SELECT * INTO brother_nf_item_sem_peca
			FROM brother_nf_item
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		$msg_erro .= "\n\n".$log_erro;
		$fp = fopen("/tmp/brother/faturamento.err","w");
		fwrite($fp,$msg_erro);
		fclose($fp);
		...
		system("mv $origem$file /tmp/brother/posvenda/faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/brother/posvenda/faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - BROTHER - Importa faturamento (importa-faturamento.php)", $msg);
}?>
