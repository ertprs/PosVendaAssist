...
try {
    ...
    */
    ...
    $param = "marca"; /* posto | os */

    /* 
    * Log 
    */
    $logClass = new Log2();
    $logClass->adicionaLog(array("titulo" => "Log erro - Geração de Pedidos Ferragens Negrão")); // Titulo
    ...
    */
    ...
    for ($i = 0; $i < count($os_garantia); $i++) {
        try {
            ...
            $os_pedido_posto = $osClass->getOsPosto($posto, $marca);

            if (empty($os_pedido_posto)) {
                continue;
            }
    ...
    if(!empty($msg_erro)){
        ...
        $fp = fopen("tmp/{$fabrica_nome}/pedidos/".date("d-m-Y").".txt", "a");
