...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				list ($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $ativo, $garantia_diferenciada, $multiplo, $peso,$devolucao_obrigatoria,$bloqueada_garantia,$peca_critica,$aguarda_inspecao) = explode ("\t",$linha);
				$original = array($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $ativo, $garantia_diferenciada, $multiplo, $peso,$devolucao_obrigatoria,$bloqueada_garantia,$peca_critica,$aguarda_inspecao);
				...
				foreach ($not_null as $value) {
					if (!$value) {
						array_push($original, 'erro vazio');
				...
				$ncm = strtim($ncm);
				$ativo =  strtim($ativo);
				$garantia_diferenciada=strtim($garantia_diferenciada);
 				$multiplo=strtim($multiplo);
				$peso=str_replace(",", ".", $peso);
				$devolucao_obrigatoria= strtim($devolucao_obrigatoria);
				$bloqueada_garantia=strtim($bloqueada_garantia);
				$peca_critica=strtim($peca_critica);
				$aguarda_inspecao=strtim($aguarda_inspecao);
