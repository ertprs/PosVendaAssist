...
try{
	...
	$sql_condicao = "SELECT condicao FROM tbl_condicao WHERE fabrica = $login_fabrica AND lower(descricao) = 'garantia'";
	...
	$sql_tipo_pedido = "SELECT tipo_pedido FROM tbl_tipo_pedido WHERE fabrica = $login_fabrica AND lower(codigo) = 'bnf'";
    ...
    if(pg_last_error($con)){
	    ...
	    if(pg_num_rows($res_postos) > 0){
	    	...
	    	while($posto = pg_fetch_object($res_postos)){
			    ...
			    $sql_pecas = "SELECT tbl_estoque_posto.peca, (tbl_estoque_posto.estoque_maximo - case when tbl_estoque_posto.qtde isnull then 0 else tbl_estoque_posto.qtde end ) AS qtde_pedido FROM tbl_estoque_posto 
			    			...
			    			AND tbl_estoque_posto.estoque_maximo > tbl_estoque_posto.qtde
				    ...
				    if(pg_num_rows($res_pecas) > 0){
						...
						if(pg_last_error($con)){
							...
							for($i = 0; $i < pg_num_rows($res_pecas); $i++){
								...
								if(pg_last_error($con)){
									$logs[] = $msg_erro = Date("Y-m-d H:i:s")." - Erro SQL: 'Inserir Pedido Item'";
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - ATLAS FOGOES - Erro na geração de pedido Pulmão(gera-pedido-pulmao.php)", $msg);
}
