 ...
 * importa-produto.php
 *
 * Importação de produtos dleletronicos
 */
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_num_rows($query_produto) == 1) {
					...
					array_push($original, 'erroa', $sql_produto);
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = utf8_decode('DL Eletronicos: Erros na importação de lista basica ') . date('d/m/Y');
