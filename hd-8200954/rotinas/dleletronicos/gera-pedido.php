...
try {
    ...
    $data['login_fabrica'] 	= 127;
    $data['fabrica_nome'] 	= 'dleletronicos';
    ...
    if (ENV == 'producao' ) {
		$data['dest'] 		= 'edson.tavares@dleletronicos.com.br';
    } else {
    ...
    $sql = "SELECT
						...
						tbl_os_item.qtde    ,
						tbl_os.sua_os
						INTO TEMP tmp_pedido_dleletronicos
				FROM    tbl_os_item
        ...
        AND     tbl_os.validada    IS NOT NULL
				...
				AND (tbl_os_status.status_os IN(19,64,103,149,151,166) OR tbl_os_status.status_os IS NULL);

				SELECT DISTINCT posto,linha from tmp_pedido_dleletronicos ;
    ...
    if($erro){
		...
		$mail->AddAddress("edson.tavares@dleletronicos.com.br");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - DL Eletronicos - Erro na geração de pedido(gera-pedido.php)", $msg);
}?>
