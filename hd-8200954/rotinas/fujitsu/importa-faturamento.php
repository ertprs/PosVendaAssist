...
try {
    ...
    $data['login_fabrica'] 		= 138;
    $data['fabrica'] 			= 'fujitsu';
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']			= "/home/fujitsu/fujitsu-telecontrol/";
    } else {
	    $data['dest'] 			= 'waldir@telecontrol.com.br';
	    $data['dest_cliente'] 	= 'waldir@telecontrol.com.br';
	    $data['origem']			= '/www/cgi-bin/fujitsu/entrada/';
    }
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS fujitsu_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);
	

	  $sql = "CREATE TABLE fujitsu_nf (
				  txt_cnpj           text,
	...
	$sql = "ALTER TABLE fujitsu_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE fujitsu_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE fujitsu_nf.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	#------------ IDENTIFICAR POSTOS NAO ENCONTRADOS PELO CNPJ --------------#
	$sql = "DROP TABLE IF EXISTS fujitsu_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "SELECT * INTO fujitsu_nf_sem_posto FROM fujitsu_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DELETE FROM fujitsu_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DROP TABLE IF EXISTS fujitsu_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "CREATE TABLE fujitsu_nf_item (
				txt_cnpj            text,
				txt_nota_fiscal     text,
				txt_serie           text,
				txt_referencia      text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(!empty($txt_cnpj)){
		      ...
		      $res = pg_query($con,"BEGIN");
		      $sql = "INSERT INTO fujitsu_nf_item (
                                txt_cnpj,
                                txt_nota_fiscal,
                                txt_serie,
                                txt_referencia,
	...
	$sql = "UPDATE fujitsu_nf_item SET
				txt_cnpj        = TRIM(txt_cnpj)                     ,
				txt_nota_fiscal = LPAD(TRIM(txt_nota_fiscal) ,9, '0'),
				txt_serie       = TRIM(txt_serie)                    ,
	...
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN aliq_icms FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN valor_subs_trib FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN base_ipi FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
    
    	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN base_subs_trib FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);

    	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN os_item INT4";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);

    	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN devolucao boolean";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE fujitsu_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE fujitsu_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE fujitsu_nf_item
			SET pedido = tbl_pedido.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE fujitsu_nf_item.txt_pedido::integer = tbl_pedido.pedido
			AND   fujitsu_nf_item.txt_pedido_item::integer = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE fujitsu_nf_item
                    ...
                    FROM tbl_os_item
                    WHERE fujitsu_nf_item.txt_pedido::numeric = tbl_os_item.pedido
	...
	$sql = "UPDATE fujitsu_nf_item
			...
			FROM  tbl_peca
			WHERE fujitsu_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "UPDATE fujitsu_nf
				SET pedido = fujitsu_nf_item.pedido
			FROM fujitsu_nf_item
			WHERE trim(fujitsu_nf.txt_nota_fiscal) = trim(fujitsu_nf_item.txt_nota_fiscal)
			AND  fujitsu_nf.posto = fujitsu_nf_item.posto";
#	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM fujitsu_nf_item
			WHERE pedido is null;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS fujitsu_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "SELECT fujitsu_nf.*
			INTO fujitsu_nf_sem_itens
			FROM fujitsu_nf
			LEFT JOIN fujitsu_nf_item ON fujitsu_nf.txt_nota_fiscal = fujitsu_nf_item.txt_nota_fiscal
			WHERE fujitsu_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "DELETE FROM fujitsu_nf 
			USING fujitsu_nf_sem_itens
			WHERE fujitsu_nf.txt_nota_fiscal = fujitsu_nf_sem_itens.txt_nota_fiscal
			AND   fujitsu_nf.txt_serie       = fujitsu_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						fujitsu_nf.emissao         ,
                        fujitsu_nf.emissao,
						substring(fujitsu_nf.txt_transp, 1,30),
						fujitsu_nf.posto           ,
						fujitsu_nf.total           ,
						fujitsu_nf.txt_cfop        ,
						fujitsu_nf.txt_nota_fiscal ,
						fujitsu_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = fujitsu_nf.pedido),
						fujitsu_nf.txt_natureza
				FROM fujitsu_nf
				LEFT JOIN tbl_faturamento ON  fujitsu_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  fujitsu_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "ALTER TABLE fujitsu_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	$sql = "UPDATE fujitsu_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = fujitsu_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = fujitsu_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE IF EXISTS fujitsu_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	

	$sql = "SELECT * INTO fujitsu_nf_item_sem_peca 
			FROM fujitsu_nf_item 
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		$msg_erro .= "\n\n".$log_erro;
		$fp = fopen("/tmp/fujitsu/faturamento.err","w");
		fwrite($fp,$msg_erro);
		fclose($fp);
		...
		system("mv $origem$file /tmp/fujitsu/faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/fujitsu/faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - fujitsu - Importa faturamento (importa-faturamento.php)", $msg);
}
