...
try {
    ...
    if (ENV == 'producao' ) {
        ...
        $data['origem']     = "/home/unilever/unilever-telecontrol";
    ...
    if(file_exists($origem.$file)){
      ...
      $sql = "DROP TABLE IF EXISTS unilever_nf;";
      $res = pg_query($con,$sql);
      $msg_erro .= pg_errormessage($con);


      $sql = "CREATE TABLE unilever_nf (
                  txt_cnpj           text,
      ...
      foreach($linhas AS $linha){
        ...
        $msg_erro = "";
            ...
            if(!empty($txt_cnpj)){
                ...
                $sql = "INSERT INTO unilever_nf ( txt_cnpj                 ,
                                    txt_nota_fiscal         ,
                                    txt_serie               ,
                                    txt_emissao             ,
                                    txt_cfop                ,
    ...
    $sql = "ALTER TABLE unilever_nf ADD COLUMN total FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf ADD COLUMN emissao DATE";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf ADD COLUMN saida DATE";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf ADD COLUMN posto INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf ADD COLUMN pedido INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf ADD COLUMN valor_frete FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "UPDATE unilever_nf SET posto = (
                SELECT tbl_posto.posto
                FROM tbl_posto
                JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
                                    AND   tbl_posto_fabrica.fabrica = $login_fabrica
                WHERE unilever_nf.txt_cnpj = tbl_posto.cnpj
            )";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    #------------ IDENTIFICAR POSTOS NAO ENCONTRADOS PELO CNPJ --------------#
    $sql = "DROP TABLE IF EXISTS unilever_nf_sem_posto";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "SELECT * INTO unilever_nf_sem_posto FROM unilever_nf WHERE posto IS NULL";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "DELETE FROM unilever_nf
            WHERE posto IS NULL";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "DROP TABLE IF EXISTS unilever_nf_item;";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "CREATE TABLE unilever_nf_item (
                txt_cnpj        text,
                txt_nota_fiscal text,
                txt_serie       text,
                txt_referencia  text,
    ...
    foreach($linhas_item AS $linha_item){
          ...
          if(!empty($txt_cnpj)){
              ...
              $res = pg_query($con,"BEGIN");
              $sql = "INSERT INTO unilever_nf_item ( txt_cnpj         ,
                              txt_nota_fiscal ,
                              txt_serie       ,
                              txt_referencia  ,
                              txt_pedido      ,
    ...
    $sql = "UPDATE unilever_nf_item SET
                txt_cnpj        = TRIM(txt_cnpj)                     ,
                txt_nota_fiscal = LPAD(TRIM(txt_nota_fiscal) ,9, '0'),
                txt_serie       = TRIM(txt_serie)                    ,
    ...
    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN posto INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN peca INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN qtde FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN pedido INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN pedido_item INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN unitario FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN aliq_ipi FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN valor_ipi FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN valor_icms FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN base_icms FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "UPDATE unilever_nf_item SET posto = (
                SELECT tbl_posto.posto
                FROM tbl_posto
                JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
                                    AND   tbl_posto_fabrica.fabrica = $login_fabrica
                WHERE unilever_nf_item.txt_cnpj = tbl_posto.cnpj
            )";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "UPDATE unilever_nf_item
            SET pedido = tbl_pedido_item.pedido,
                pedido_item = tbl_pedido_item.pedido_item
            FROM tbl_pedido_item, tbl_pedido
            WHERE unilever_nf_item.txt_pedido::numeric = tbl_pedido_item.pedido
            AND   unilever_nf_item.txt_pedido_item::numeric = tbl_pedido_item.pedido_item
    ...
    $sql = "UPDATE unilever_nf_item
            ...
            FROM  tbl_peca
            WHERE unilever_nf_item.txt_referencia = tbl_peca.referencia
            AND tbl_peca.fabrica = $login_fabrica";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "UPDATE unilever_nf
                SET pedido = unilever_nf_item.pedido
            FROM unilever_nf_item
            WHERE trim(unilever_nf.txt_nota_fiscal) = trim(unilever_nf_item.txt_nota_fiscal)
            AND  unilever_nf.posto = unilever_nf_item.posto";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "DELETE FROM unilever_nf_item
            WHERE pedido is null;";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    #------------ NFs sem Itens --------------#
    $sql = "DROP TABLE IF EXISTS unilever_nf_sem_itens";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "SELECT unilever_nf.*
            INTO unilever_nf_sem_itens
            FROM unilever_nf
            LEFT JOIN unilever_nf_item ON unilever_nf.txt_nota_fiscal = unilever_nf_item.txt_nota_fiscal
            WHERE unilever_nf_item.txt_nota_fiscal IS NULL";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "DELETE FROM unilever_nf
            USING unilever_nf_sem_itens
            WHERE unilever_nf.txt_nota_fiscal = unilever_nf_sem_itens.txt_nota_fiscal
            AND   unilever_nf.txt_serie       = unilever_nf_sem_itens.txt_serie";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "INSERT INTO tbl_faturamento (
                ...
                SELECT  $login_fabrica,
                        unilever_nf.emissao         ,
                        unilever_nf.saida         ,
                        substring(unilever_nf.txt_transp2, 1,30),
                        unilever_nf.posto           ,
                        unilever_nf.total           ,
                        unilever_nf.txt_cfop        ,
                        unilever_nf.txt_nota_fiscal ,
                        unilever_nf.txt_serie       ,
                        (select tipo_pedido from tbl_pedido where pedido = unilever_nf.pedido),
                        case when unilever_nf.valor_frete is null then 0 else unilever_nf.valor_frete end as valor_frete
                FROM unilever_nf
                LEFT JOIN tbl_faturamento ON  unilever_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
                                         AND  unilever_nf.txt_serie         = tbl_faturamento.serie
                                         AND  tbl_faturamento.fabrica      = $login_fabrica
    ...
    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN faturamento INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE unilever_nf_item ADD COLUMN cfop text";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "UPDATE unilever_nf_item
            ...
            AND   tbl_faturamento.nota_fiscal = unilever_nf_item.txt_nota_fiscal
            AND   tbl_faturamento.serie       = unilever_nf_item.txt_serie";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "DROP TABLE unilever_nf_item_sem_peca ";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);


    $sql = "SELECT * INTO unilever_nf_item_sem_peca
            FROM unilever_nf_item
                WHERE peca IS NULL" ;
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    $sql = "UPDATE tbl_pedido
            ...
            FROM  unilever_nf_item
            WHERE tbl_pedido.pedido = unilever_nf_item.pedido";
    $res3 = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
    ...
    if (!empty($msg_erro)) {
        ...
        $fp = fopen("/tmp/unilever/faturamento.err","w");
        fwrite($fp,$log_erro);
        fclose($fp);

        /*system("mv $origem$file /tmp/unilever/Faturamento".date('Y-m-d-H-i').".txt");
        system("mv $origem$file2 /tmp/unilever/Faturamento_item".date('Y-m-d-H-i').".txt");*/
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - UNILEVER - Importa faturamento (importa-faturamento.php)", $msg);
}?>