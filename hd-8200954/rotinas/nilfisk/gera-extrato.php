...
try {
    ...
    */
    $fabrica        = 190;
    $dia_mes        = date('d');
    $dia_extrato    = '2020-02-13 23:59:59';
    ...
    */
    $logClass = new Log2();
    $logClass->adicionaLog(array("titulo" => "Log erro Geração de Extrato Nilfisk")); // Titulo
   ...
   // $query_cond = "AND tbl_os.tipo_atendimento = {$tipo_atendimento}";

    /*
    * Resgata a quantidade de OS por Posto
    */
    $os_posto = $classExtrato->getOsPosto($dia_extrato, $fabrica, false, '','',false);


//echo "<pre>".print_r($os_posto,1)."</pre>";exit;
    ...
    */
    ...
    for ($i = 0; $i < count($os_posto); $i++) {
        ...
        $tipo_atendimento  = [$os_posto[$i]["tipo_atendimento"]];
        ...
        try {
            ...
            */
            $classExtrato->relacionaExtratoOS($fabrica, $posto, $extrato, $dia_extrato, null,null, $tipo_atendimento);
    ...
    */
    if(!empty($msg_erro)){
        ...
        $fp = fopen("/tmp/nilfik/extrato/gera-extrato-".date("dmYH").".txt", "a");
