...
try {
    ...
    if(pg_last_error($con)){
    	$logs_erro[] = $sql;
    	$logs[] = "Erro ao gerar o pedido do tipo 'CONSIGNAÇÃO'";
    	$erro   = true;
    }
      ...
      $posto = pg_result($res,$i,'posto');   
	    ...
	    if(pg_num_rows($res_peca) > 0){
		  ...
		  $erro_pedido = false;
			...
			if(pg_last_error($con)){
				$logs[] 		= $msg_erro = Date("Y-m-d H:i:s")." - Erro ao consultar os pedidos do tipo 'CONSIGNAÇÃO'";
				$logs_erro[] 		= $sql;
				$erro   		= true;
				$erro_pedido	= true;
			}else{
			...
			for ($x=0; $x < pg_num_rows($res_peca); $x++) { 
				...
				if(pg_last_error($con)){
					...
					$logs[] 		= $msg_erro = Date("Y-m-d H:i:s")." - Erro ao salvar os itens do pedido <br> OS: {$os} - Posto: {$nome_posto} - Peça: {$referencia} - Qtd: {$qtde}";
					$logs_erro[] 		= $sql;
					$erro   		= true;
					$erro_pedido	= true;
					break;
				...
				if(pg_last_error($con)){
					...
					$logs[] 		= $msg_erro = Date("Y-m-d H:i:s")." - Erro ao atualizar os itens do pedido <br> OS: {$os} - Posto: {$nome_posto} - Peça: {$referencia} - Qtd: {$qtde}";
					$logs_erro[] 	= $sql;
					$erro   		= true;
					$erro_pedido	= true;
				}
				...
				if(pg_last_error($con)){
					...
					$logs[] 		= $msg_erro = Date("Y-m-d H:i:s")." - Erro ao atualizar o pedido <br> OS: {$os} - Posto: {$nome_posto} - Peça: {$referencia} - Qtd: {$qtde}";
					$logs_erro[] 	= $sql;
					$erro   		= true;
					$erro_pedido	= true;
				}
			...
			if(!$erro_pedido) {
				...
				if(pg_last_error($con)){
					...
					$logs[] 		= $msg_erro = Date("Y-m-d H:i:s")." - Erro ao finalizar o pedido <br> OS: {$os} - Posto: {$nome_posto} - Peça: {$referencia} - Qtd: {$qtde}";
					$logs_erro[] 	= $sql;
					$logs[] 		= pg_last_error($con);
					$logs_cliente[] 	= Array(
											  'posto'	=> $nome_posto, 
											  'peca'	=> $referencia,
											  'erro'	=> "Erro ao finalizar o pedido <br> OS: {$os} - Posto: {$nome_posto} - Peça: {$referencia} - Qtd: {$qtde}"
									  );
					$erro   		= true;
					$erro_pedido	= true;
				}
