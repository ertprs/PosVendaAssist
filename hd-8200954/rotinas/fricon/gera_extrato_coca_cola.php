...
try {
	...
	if ($dia_mes == '07') {
		$cond = " AND tbl_posto_fabrica.contato_estado in ('BA','MG','SE','DF','MS','MT','AM','MA','RO','AP','RR','TO','AC') ";
	}

	if ($dia_mes == '14') {
		$cond = " AND tbl_posto_fabrica.contato_estado in ('PE','RN','PI') ";
	}

	if ($dia_mes == '21') {
		$cond = " AND tbl_posto_fabrica.contato_estado in ('SP','RJ','PR','RS','GO','PA','SC','AL') ";
	}

	if ($dia_mes == '28') {
		$cond = " AND tbl_posto_fabrica.contato_estado in ('CE','ES','PB') ";
	}
	...
	if (strlen($bug) > 0) {
		...
		Log::envia_email($vet, 'Log - Extrato Fricon (Linha Coca-Cola)', $msg_erro);
