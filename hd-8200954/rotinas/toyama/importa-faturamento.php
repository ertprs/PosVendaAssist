...
try {
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']		= "/home/toyama/toyama-telecontrol";
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS toyama_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);
	

	  $sql = "CREATE TABLE toyama_nf (
				  txt_cnpj           text,
	  ...
	  foreach($linhas AS $linha){
		...
		$msg_erro = "";
			...
			if(!empty($txt_cnpj)){
				...
				$sql = "INSERT INTO toyama_nf ( txt_cnpj                 ,
								    txt_nota_fiscal         ,
								    txt_serie               ,
								    txt_emissao             ,
								    txt_cfop                ,
	...
	$sql = "ALTER TABLE toyama_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf ADD COLUMN saida DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf ADD COLUMN valor_frete FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE toyama_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE toyama_nf.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	#------------ IDENTIFICAR POSTOS NAO ENCONTRADOS PELO CNPJ --------------#
	$sql = "DROP TABLE IF EXISTS toyama_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "SELECT * INTO toyama_nf_sem_posto FROM toyama_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DELETE FROM toyama_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DROP TABLE IF EXISTS toyama_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "CREATE TABLE toyama_nf_item (
				txt_cnpj        text,
				txt_nota_fiscal text,
				txt_serie       text,
				txt_referencia  text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(!empty($txt_cnpj)){
		      ...
		      $res = pg_query($con,"BEGIN");
		      $sql = "INSERT INTO toyama_nf_item ( txt_cnpj         ,
							  txt_nota_fiscal ,
							  txt_serie       ,
							  txt_referencia  ,
							  txt_pedido      ,
	...
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE toyama_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE toyama_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE toyama_nf_item
			SET pedido = tbl_pedido.pedido
			FROM tbl_pedido
			WHERE toyama_nf_item.txt_pedido::numeric = tbl_pedido.pedido
	...
	$sql = "UPDATE toyama_nf_item
			...
			FROM  tbl_peca
			WHERE toyama_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "UPDATE toyama_nf
				SET pedido = toyama_nf_item.pedido
			FROM toyama_nf_item
			WHERE trim(toyama_nf.txt_nota_fiscal) = trim(toyama_nf_item.txt_nota_fiscal)
			AND  toyama_nf.posto = toyama_nf_item.posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM toyama_nf_item
			WHERE pedido is null;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS toyama_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "SELECT toyama_nf.*
			INTO toyama_nf_sem_itens
			FROM toyama_nf
			LEFT JOIN toyama_nf_item ON toyama_nf.txt_nota_fiscal = toyama_nf_item.txt_nota_fiscal
			WHERE toyama_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "DELETE FROM toyama_nf 
			USING toyama_nf_sem_itens
			WHERE toyama_nf.txt_nota_fiscal = toyama_nf_sem_itens.txt_nota_fiscal
			AND   toyama_nf.txt_serie       = toyama_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						toyama_nf.emissao         ,
						toyama_nf.saida         ,
						substring(toyama_nf.txt_transp2, 1,30),
						toyama_nf.posto           ,
						toyama_nf.total           ,
						toyama_nf.txt_cfop        ,
						toyama_nf.txt_nota_fiscal ,
						toyama_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = toyama_nf.pedido),
						case when toyama_nf.valor_frete is null then 0 else toyama_nf.valor_frete end as valor_frete
				FROM toyama_nf
				LEFT JOIN tbl_faturamento ON  toyama_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  toyama_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "ALTER TABLE toyama_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	$sql = "UPDATE toyama_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = toyama_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = toyama_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE toyama_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	

	$sql = "SELECT * INTO toyama_nf_item_sem_peca 
			FROM toyama_nf_item 
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE tbl_pedido
			...
			FROM  toyama_nf_item
			WHERE tbl_pedido.pedido = toyama_nf_item.pedido";
	$res3 = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		...
		$fp = fopen("/tmp/toyama/faturamento.err","w");
		fwrite($fp,$log_erro);
		fclose($fp);

		system("mv $origem$file /tmp/toyama/Faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/toyama/Faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - TOYAMA - Importa faturamento (importa-faturamento.php)", $msg);
}?>