...
try {
    ...
    $data['login_fabrica'] 	= 140;
    $data['fabrica_nome'] 	= 'lavor';
    ...
    /* Log */
    $log = new Log2();
    $log->adicionaLog(array("titulo" => "Log erro Geração de Pedidos Lavor")); // Titulo
...
$sql = "SELECT interv_reinc.os
			...
			WHERE interv_reinc.ultimo_reinc_status IN (62,65);
	";
	$res = pg_query($con,$sql);
...
$sql = "SELECT interv_reinc.os
			...
			FROM (
				...
				(
					...
					WHERE tbl_os_status.os = ultima_reinc.os 
					AND   tbl_os_status.fabrica_status= $login_fabrica 
					AND   status_os IN (19, 67, 68, 70, 90 ,139)
					ORDER BY os_status DESC LIMIT 1
				) AS ultimo_reinc_status
			...
			WHERE interv_reinc.ultimo_reinc_status IN ( 67, 68, 70);
	";
	$res = pg_query($con,$sql);
...
$res = pg_query($con,$sql);
    ...
    $sql = "SELECT  
				...
				AND		tbl_os.os NOT IN ( SELECT os from tmp_interv_fab )
	...
	for ($i=0; $i < pg_num_rows($resP); $i++) { 
		...
		$posto  		= pg_result($resP,$i,'posto');
		$posto_nome  	= pg_result($resP,$i,'posto_nome');
