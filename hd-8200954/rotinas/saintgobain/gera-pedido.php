...
try {
    ...
    $sql = "SELECT
						...
						tbl_os.sua_os      ,
					       tbl_os.os
						INTO TEMP tmp_pedido_saintgobain
				FROM    tbl_os_item
				...
				SELECT DISTINCT posto,os,linha from tmp_pedido_saintgobain ;
    ...
    if($erro){
	    if(count($logs_erro) > 0){
			...
			$mail->IsSMTP();
			$mail->IsHTML();
			$mail->AddReplyTo("paulo@telecontrol.com.br", "Suporte Telecontrol");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - saintgobain - Erro na geração de pedido(gera-pedido.php)", $msg);
}?>
