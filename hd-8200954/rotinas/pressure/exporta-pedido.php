...
try {
    ...
    if ($numrows) {
        ...
        for ($i = 0; $i < $numrows; $i++) {
			...
			$sql_marca = "SELECT DISTINCT tbl_marca.codigo_marca AS marca
				FROM tbl_marca
				JOIN tbl_produto ON tbl_produto.marca = tbl_marca.marca
				JOIN tbl_os_produto ON tbl_os_produto.produto = tbl_produto.produto
			...
			if (pg_num_rows($res_marca) > 0) {
				$marca = pg_fetch_result($res_marca, 0, 'marca');
			}
             ...
             *  Layout:
             *
             *  Emissao | codigo_posto | Tipo Pedido | Pedido | Condicao | Tipo Frete
             *
             */
            ...
            for ($x = 0; $x < $tot_pecas; $x++) {
                ...
                $preco           = trim(number_format(pg_fetch_result($res_pecas, $x, 'preco'), 2, '.', ''));
        ...
        if (file_exists($file_pedido) and (filesize($file_pedido) > 0)) {
			...
			$destino = '/home/pressure/telecontrol-'  . $fabrica_nome . '/pedido.txt';
			$destino2 = '/home/pressure/telecontrol-' . $fabrica_nome . '/pedido-item.txt';

			copy($file_pedido, $dir . '/pedido-' . $data_arquivo . '.txt');
			copy($file_pedido, '/home/pressure/telecontrol-pressure/bkp/pedido-' . $data_arquivo . '.txt');
			system("mv $file_pedido $destino");
			copy($file_pedido_item, $dir . '/pedido-item-' . $data_arquivo . '.txt');
			copy($file_pedido_item, '/home/pressure/telecontrol-pressure/bkp/pedido-item-' . $data_arquivo . '.txt');
