...
try {
    ...
    $sql = "SELECT
				...
				LEFT Join tbl_pedido on tbl_pedido.pedido = tbl_os_troca.pedido
				WHERE   tbl_os_item.pedido IS NULL
				AND     tbl_os.validada    IS NOT NULL
				AND     tbl_os.excluida    IS NOT TRUE
				AND     tbl_os.fabrica    = $login_fabrica
				AND     ((tbl_os_troca.os isnull or tbl_pedido.status_pedido = 14) )
				...
				/* intervenção de peças */
				AND     (
					CASE
						WHEN (
							SELECT os_status FROM tbl_os_status
							WHERE status_os = 205
				...
				/* intervenção de peças excedentes */
				AND     (
					CASE
						WHEN (
							SELECT os_status FROM tbl_os_status
				...
				/* intervenção de valores adicionais */
				AND     (
					CASE
						WHEN (
							SELECT os_status FROM tbl_os_status
				...
				/* auditoria de reincidencia */
				AND     (
					CASE
						WHEN (
							SELECT os_status FROM tbl_os_status
							WHERE status_os IN (70,68)
							AND tbl_os_status.os = tbl_os.os
							LIMIT 1
						) > 0
						THEN (
							SELECT os_status FROM tbl_os_status
							WHERE status_os IN (19,139,155)
							AND tbl_os_status.os = tbl_os.os
							LIMIT 1
						)
						ELSE 1
				...
				SELECT DISTINCT os, posto, linha from tmp_pedido_pressure ;
	...
	for ($i=0; $i < pg_num_rows($resP); $i++) {
		...
		$sql_auditoria = "SELECT auditoria_os
				...
				WHERE os = $os
				AND (bloqueio_pedido IS TRUE OR ((cancelada IS NOT NULL OR reprovada IS NOT NULL) and admin notnull))
				";
		$res_auditoria = pg_query($con, $sql_auditoria);

		if (pg_num_rows($res_auditoria) > 0) {
			$logs_erro[] = "OS $os não gerou pedido, pois encontra-se em auditoria";
			continue;
		}
		...
		if(pg_last_error($con)){
			$logs[] = $msg_erro = Date("Y-m-d H:i:s")." - Erro SQL: 'Pedido' - OS: $os";
			$logs_erro[] = $sql;
			$logs[] = pg_last_error($con);
			$erro = "*";
		}else{
			...
			for ($x=0; $x < pg_num_rows($result2); $x++) {
				...
				if(pg_last_error($con)){
					$logs[] = $msg_erro = Date("Y-m-d H:i:s")." - Erro SQL: 'Pedido Item' - OS: $os";
					$logs_erro[] = $sql;
					$logs[] = pg_last_error($con);
					$erro = "*";
				}else{
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - Pressure - Erro na geração de pedido(gera-pedido.php)", $msg);
}?>
