...
try {
	...
	$diretorio_origem = '/home/pressure/pressure-telecontrol';
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_num_rows($query_produto) == 0) {
					echo $sql = "INSERT INTO tbl_produto (
											linha,
											familia,
											referencia,
											descricao,
					...
					echo $sql = "UPDATE tbl_produto SET
		...
		if (filesize($arq_log) > 0) {
			...
			$joga_ftp = "cd $log_dir && cp produto$data_arq_enviar.txt $ftp/$fabrica_nome-produtos$data_arq_enviar.txt";
			system("$joga_ftp");
