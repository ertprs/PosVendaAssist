...
try {
    ...
    if (ENV == 'producao' ) {
	    $data['dest']  	        = array('helpdesk@telecontrol.com.br','jean.toffoli@pressure.com.br','srp04@pressure.com.br');
	    $data['origem']		= "/home/pressure/pressure-telecontrol/";
	    $data['file']		= 'faturamento.txt';
	    $data['file2']		= 'faturamento-item.txt';
    } else {
	    $data['dest'] 		= 'ronald.santostelecontrol.com.br';
	   # $data['origem']		=  "/home/anderson/public_html/rotinateste/pressure/entrada/";
	    $data['origem']		= "/var/www/cgi-bin/pressure/entrada/";
	    $data['file']		= 'faturamento.txt';
	    $data['file2']		= 'faturamento-item.txt';
    }
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS pressure_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);


	  $sql = "CREATE TABLE pressure_nf (
				  txt_cnpj           text,
	  ...
	  foreach($linhas AS $linha){
		...
		$msg_erro = "";
			...
			if(!empty($txt_cnpj)){
				...
				$sql = "INSERT INTO pressure_nf ( txt_cnpj                 ,
								    txt_nota_fiscal         ,
								    txt_serie               ,
								    txt_emissao             ,
								    txt_cfop                ,
	...
	$sql = "ALTER TABLE pressure_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf ADD COLUMN saida DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE pressure_nf SET posto = tbl_posto.posto
			FROM tbl_posto,tbl_posto_fabrica
			WHERE trim(pressure_nf.txt_cnpj) = tbl_posto.cnpj
	...
	$sql = "DROP TABLE IF EXISTS pressure_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT * INTO pressure_nf_sem_posto FROM pressure_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DELETE FROM pressure_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DROP TABLE IF EXISTS pressure_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "CREATE TABLE pressure_nf_item (
				txt_cnpj        text,
				txt_nota_fiscal text,
				txt_serie       text,
				txt_referencia  text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      list(
	      	...
	      	$txt_base_ipi, 
	      	$txt_base_icms,
	      	$txt_base_subs_trib) = explode (';',$linha_item);
	      if(strlen(trim($txt_cnpj)) > 0){
            ...
            $res = pg_query($con,"BEGIN");
            $sql = "INSERT INTO pressure_nf_item    (
            	   	txt_cnpj,
            	   	txt_nota_fiscal ,
            	   	txt_serie       ,
					txt_referencia  ,
	...
	$sql = "UPDATE pressure_nf_item SET
				txt_cnpj 		= trim(txt_cnpj), 
				txt_nota_fiscal = trim(txt_nota_fiscal) , 
				txt_serie 		= trim(txt_serie) , 
	...
	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN aliq_icms FLOAT";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN base_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN valor_subs_trib FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN base_subs_trib FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN os_item INT4";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);

    $sql = "ALTER TABLE pressure_nf_item ADD COLUMN devolucao boolean";
    $res = pg_query($con,$sql);
    $msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE pressure_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE pressure_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "UPDATE pressure_nf_item
			SET pedido = tbl_pedido_item.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE pressure_nf_item.txt_pedido::numeric = tbl_pedido_item.pedido
			AND   pressure_nf_item.txt_pedido_item::numeric = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE pressure_nf_item
                    ...
                    FROM tbl_os_item
                    WHERE pressure_nf_item.txt_pedido::numeric = tbl_os_item.pedido
                    AND   pressure_nf_item.txt_pedido_item::numeric = tbl_os_item.pedido_item
	...
	$sql = "UPDATE pressure_nf_item
			...
			FROM  tbl_peca
			WHERE pressure_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	

	$sql = "UPDATE pressure_nf
				SET pedido = pressure_nf_item.pedido
			FROM pressure_nf_item
			WHERE trim(pressure_nf.txt_nota_fiscal) = trim(pressure_nf_item.txt_nota_fiscal)
			AND  pressure_nf.posto = pressure_nf_item.posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM pressure_nf_item
	...
	$sql = "DELETE FROM pressure_nf_item
			...
			WHERE txt_nota_fiscal         = tbl_faturamento.nota_fiscal
			AND   txt_serie               = tbl_faturamento.serie
			AND   pressure_nf_item.peca   = tbl_faturamento_item.peca
			AND   tbl_faturamento.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS pressure_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT pressure_nf.*
			INTO pressure_nf_sem_itens
			FROM pressure_nf
			LEFT JOIN pressure_nf_item ON trim(pressure_nf.txt_nota_fiscal) = trim(pressure_nf_item.txt_nota_fiscal)
			WHERE pressure_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DELETE FROM pressure_nf
			USING pressure_nf_sem_itens
			WHERE pressure_nf.txt_nota_fiscal = pressure_nf_sem_itens.txt_nota_fiscal
			AND   pressure_nf.txt_serie       = pressure_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						pressure_nf.emissao         ,
						pressure_nf.saida         ,
						substring(pressure_nf.txt_transp, 1,30),
						pressure_nf.posto           ,
						pressure_nf.total           ,
						pressure_nf.txt_cfop        ,
						pressure_nf.txt_nota_fiscal ,
						pressure_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = pressure_nf.pedido),
						pressure_nf.txt_natureza
				FROM pressure_nf
				LEFT JOIN tbl_faturamento ON  pressure_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  pressure_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE pressure_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE pressure_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = pressure_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = pressure_nf_item.txt_serie";
	...
	$sql = "DROP TABLE IF EXISTS pressure_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);


	$sql = "SELECT * INTO pressure_nf_item_sem_peca
			FROM pressure_nf_item
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		$msg_erro .= "\n\n".$log_erro;
		$fp = fopen("/tmp/pressure/faturamento.err","w");
		fwrite($fp,$msg_erro);
		fclose($fp);
        ...
        system("mv $origem$file /tmp/pressure/faturamento".date('Y-m-d-H-i').".txt");
        system("mv $origem$file2 /tmp/pressure/faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - pressure - Importa faturamento (importa-faturamento.php)", $msg);
}?>
