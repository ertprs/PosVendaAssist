...
try {
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']		= "/home/newup/newup-telecontrol/";
    } else {
   ...
   if(file_exists($origem.$file)){
     ...
     $sql = "DROP TABLE IF EXISTS newup_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);


	  $sql = "CREATE TABLE newup_nf (
				  txt_cnpj           text,
	...
	$sql = "ALTER TABLE newup_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newup_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE newup_nf.txt_cnpj = tbl_posto.cnpj
			)";
	...
	$sql = "DROP TABLE IF EXISTS newup_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT * INTO newup_nf_sem_posto FROM newup_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM newup_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DROP TABLE IF EXISTS newup_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "CREATE TABLE newup_nf_item (
				txt_cnpj            text,
				txt_nota_fiscal     text,
				txt_serie           text,
				txt_referencia      text,
	...
	$sql = "ALTER TABLE newup_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN aliq_icms FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN valor_subs_trib FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN base_ipi FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

 	$sql = "ALTER TABLE newup_nf_item ADD COLUMN base_subs_trib FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

 	$sql = "ALTER TABLE newup_nf_item ADD COLUMN os_item INT4";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

 	$sql = "ALTER TABLE newup_nf_item ADD COLUMN devolucao boolean";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newup_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE newup_nf_item.txt_cnpj = tbl_posto.cnpj
			)";

	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE newup_nf_item
			SET pedido = tbl_pedido.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE newup_nf_item.txt_pedido::integer = tbl_pedido.pedido
			AND   newup_nf_item.txt_pedido_item::integer = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE newup_nf_item
                    ...
                    FROM tbl_os_item
                    WHERE newup_nf_item.pedido_item = tbl_os_item.pedido_item
   ...
   $msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newup_nf_item
			...
			FROM  tbl_peca
			WHERE newup_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE newup_nf
				SET pedido = newup_nf_item.pedido
			FROM newup_nf_item
			WHERE trim(newup_nf.txt_nota_fiscal) = trim(newup_nf_item.txt_nota_fiscal)
			AND  newup_nf.posto = newup_nf_item.posto";
		#$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM newup_nf_item
			...
			WHERE txt_nota_fiscal         = tbl_faturamento.nota_fiscal
			AND   txt_serie               = tbl_faturamento.serie
			AND   tbl_faturamento_item.pedido_item = newup_nf_item.pedido_item
			AND   tbl_faturamento.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS newup_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT newup_nf.*
			INTO newup_nf_sem_itens
			FROM newup_nf
			LEFT JOIN newup_nf_item ON newup_nf.txt_nota_fiscal = newup_nf_item.txt_nota_fiscal
			WHERE newup_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM newup_nf
			USING newup_nf_sem_itens
			WHERE newup_nf.txt_nota_fiscal = newup_nf_sem_itens.txt_nota_fiscal
			AND   newup_nf.txt_serie       = newup_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						newup_nf.emissao         ,
                        newup_nf.emissao,
						substring(newup_nf.txt_transp, 1,30),
						newup_nf.posto           ,
						newup_nf.total           ,
						newup_nf.txt_cfop        ,
						newup_nf.txt_nota_fiscal ,
						newup_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = newup_nf.pedido),
						newup_nf.txt_natureza
				FROM newup_nf
				LEFT JOIN tbl_faturamento ON  newup_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  newup_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE newup_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newup_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newup_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = newup_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = newup_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE IF EXISTS newup_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);


	$sql = "SELECT * INTO newup_nf_item_sem_peca
			FROM newup_nf_item
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		$msg_erro .= "\n\n".$log_erro;
		$fp = fopen("/tmp/newup/faturamento.err","w");
		fwrite($fp,$msg_erro);
		fclose($fp);
		...
		system("mv $origem$file /tmp/newup/faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/newup/faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - newup - Importa faturamento (importa-faturamento.php)", $msg);
}
