...
try {
	...
	$arquivos = "/tmp/newup";
	$origem   = "/home/newup/newup-telecontrol/";
	...
	if (file_exists($arquivo)) {
		$sql = "CREATE TEMP TABLE tmp_newup_numero_serie (
					serie text,
					cnpj text,
					referencia text,
					venda date,
				...
				CREATE TEMP TABLE tmp_newup_numero_serie_duplicado (
					serie text,
					produto int
				)";
		$res = pg_query($con, $sql); echo pg_last_error();
		...
		foreach ($arquivo_conteudo as $linha) {
			if ($linha == "\n") {
				...
				$sql = "INSERT INTO tmp_newup_numero_serie
		...
		$sql = "UPDATE tmp_newup_numero_serie
				SET produto = tbl_produto.produto
				FROM tbl_produto
				JOIN tbl_linha USING (linha)
				WHERE tmp_newup_numero_serie.referencia = tbl_produto.referencia
				AND tbl_produto.fabrica_i = {$fabrica}";
		$res = pg_query($con, $sql); echo pg_last_error();

		$sql = "SELECT serie, referencia FROM tmp_newup_numero_serie WHERE produto IS NULL";
		$res = pg_query($con, $sql); echo pg_last_error();
		...
		$sql = "DELETE FROM tmp_newup_numero_serie WHERE produto IS NULL";
		$res = pg_query($con, $sql); echo pg_last_error();

		$sql = "SELECT serie, referencia FROM tmp_newup_numero_serie WHERE fabricacao IS NULL";
		$res = pg_query($con, $sql); echo pg_last_error();
		$rows = pg_num_rows($res);
		...
		$sql = "DELETE FROM tmp_newup_numero_serie WHERE fabricacao IS NULL";
		$res = pg_query($con, $sql); echo pg_last_error();

		$sql = "INSERT INTO tmp_newup_numero_serie_duplicado
                       (serie, produto)
                       SELECT serie, produto
                       FROM tmp_newup_numero_serie
                       GROUP BY serie, produto
                       HAVING COUNT(*) > 1";
	        $res = pg_query($con, $sql); echo pg_last_error();

		$sql = "DELETE FROM tmp_newup_numero_serie
                                  USING tmp_newup_numero_serie_duplicado
			          WHERE tmp_newup_numero_serie_duplicado.serie = tmp_newup_numero_serie.serie
			          AND tmp_newup_numero_serie_duplicado.produto = tmp_newup_numero_serie.produto";
		$res = pg_query($con, $sql); echo pg_last_error();

		$sql = "UPDATE tmp_newup_numero_serie
				SET numero_serie = tbl_numero_serie.numero_serie
				FROM tbl_numero_serie
				WHERE tmp_newup_numero_serie.produto = tbl_numero_serie.produto
				AND tmp_newup_numero_serie.serie = tbl_numero_serie.serie
		...
		$sql = "SELECT COUNT(*) AS total_insert FROM tmp_newup_numero_serie WHERE insert IS TRUE";
		$res = pg_query($con, $sql); echo pg_last_error();
		$rows = pg_num_rows($res);
		...
		$log[] = "$total_insert NÚMEROS DE SÉRIE INSERIDOS";
                ...
                system("mv {$arquivo} /tmp/newup/telecontrol-numero-serie_{$data_bkp}.txt");
	}else{
	...
	mail("helpdesk@telecontrol.com.br", "TELECONTROL / newup ({$data}) - IMPORTA NÚMERO DE SÉRIE", implode("<br />", $log), $header);
	...
	system("mv {$arquivo} /tmp/newup/num_serie_{$data_bkp}.txt");
}
