...
try {
    ...
    $data['fabrica_nome'] 	= 'thermosystem';
   ...
   $inter_auditoria_peca = "SELECT tbl_auditoria_os.os
                  INTO TEMP tmp_peca_auditada_hydra
                  FROM tbl_auditoria_os
                  JOIN tbl_os USING(os)
                  WHERE tbl_auditoria_os.auditoria_status = 4
                  AND tbl_auditoria_os.liberada IS NULL
                  AND tbl_os.fabrica = $login_fabrica";
  	$res_inter_auditoria_peca = pg_query($con, $inter_auditoria_peca);


  	$sql_aprovadas = "SELECT os,
                       (
                         SELECT liberada
                           FROM tbl_auditoria_os
                     ...
                     AND liberada notnull";
   $res_aprovadas = pg_query($con, $sql_aprovadas);
   ...
   $sql = "SELECT
						...
						tbl_os.sua_os      ,
					       tbl_os.os
						INTO TEMP tmp_pedido_thermosystem
				FROM    tbl_os_item
				...
				AND tbl_os.os IN ( SELECT os FROM liberada)
				AND tbl_os.os NOT IN ( SELECT os FROM tmp_peca_auditada_hydra)
				AND (tbl_os_status.status_os IN(19,64,118) OR tbl_os_status.status_os IS NULL );

				SELECT DISTINCT posto, linha from tmp_pedido_thermosystem ;
				";	
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - ThermoSystem - Erro na geração de pedido(gera-pedido.php)", $msg);
}?>
