...
try {
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']		= "/home/newmaq/newmaq-telecontrol/";
    } else {
   ...
   if(file_exists($origem.$file)){
     ...
     $sql = "DROP TABLE IF EXISTS newmaq_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);


	  $sql = "CREATE TABLE newmaq_nf (
				  txt_cnpj           text,
	...
	$sql = "ALTER TABLE newmaq_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newmaq_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE newmaq_nf.txt_cnpj = tbl_posto.cnpj
			)";
	...
	$sql = "DROP TABLE IF EXISTS newmaq_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT * INTO newmaq_nf_sem_posto FROM newmaq_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM newmaq_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DROP TABLE IF EXISTS newmaq_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "CREATE TABLE newmaq_nf_item (
				txt_cnpj            text,
				txt_nota_fiscal     text,
				txt_serie           text,
				txt_referencia      text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(!empty($txt_cnpj)){
		      ...
		      $res = pg_query($con,"BEGIN");
		      $sql = "INSERT INTO newmaq_nf_item (
                                txt_cnpj,
                                txt_nota_fiscal,
                                txt_serie,
                                txt_referencia,
	...
	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN aliq_icms FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN valor_subs_trib FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN base_ipi FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

 	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN base_subs_trib FLOAT";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

 	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN os_item INT4";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);

 	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN devolucao boolean";
 	$res = pg_query($con,$sql);
 	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newmaq_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE newmaq_nf_item.txt_cnpj = tbl_posto.cnpj
			)";

	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE newmaq_nf_item
			SET pedido = tbl_pedido.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE newmaq_nf_item.txt_pedido::integer = tbl_pedido.pedido
			AND   newmaq_nf_item.txt_pedido_item::integer = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE newmaq_nf_item
                    ...
                    FROM tbl_os_item
                    WHERE newmaq_nf_item.pedido_item = tbl_os_item.pedido_item
   ...
   $msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newmaq_nf_item
			...
			FROM  tbl_peca
			WHERE newmaq_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE newmaq_nf
				SET pedido = newmaq_nf_item.pedido
			FROM newmaq_nf_item
			WHERE trim(newmaq_nf.txt_nota_fiscal) = trim(newmaq_nf_item.txt_nota_fiscal)
			AND  newmaq_nf.posto = newmaq_nf_item.posto";
		#$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM newmaq_nf_item
			WHERE pedido is null;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM newmaq_nf_item
			...
			WHERE txt_nota_fiscal         = tbl_faturamento.nota_fiscal
			AND   txt_serie               = tbl_faturamento.serie
			AND   tbl_faturamento_item.pedido_item = newmaq_nf_item.pedido_item
			AND   tbl_faturamento.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS newmaq_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT newmaq_nf.*
			INTO newmaq_nf_sem_itens
			FROM newmaq_nf
			LEFT JOIN newmaq_nf_item ON newmaq_nf.txt_nota_fiscal = newmaq_nf_item.txt_nota_fiscal
			WHERE newmaq_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM newmaq_nf
			USING newmaq_nf_sem_itens
			WHERE newmaq_nf.txt_nota_fiscal = newmaq_nf_sem_itens.txt_nota_fiscal
			AND   newmaq_nf.txt_serie       = newmaq_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						newmaq_nf.emissao         ,
                        newmaq_nf.emissao,
						substring(newmaq_nf.txt_transp, 1,30),
						newmaq_nf.posto           ,
						newmaq_nf.total           ,
						newmaq_nf.txt_cfop        ,
						newmaq_nf.txt_nota_fiscal ,
						newmaq_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = newmaq_nf.pedido),
						newmaq_nf.txt_natureza
				FROM newmaq_nf
				LEFT JOIN tbl_faturamento ON  newmaq_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  newmaq_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE newmaq_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE newmaq_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = newmaq_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = newmaq_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE IF EXISTS newmaq_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);


	$sql = "SELECT * INTO newmaq_nf_item_sem_peca
			FROM newmaq_nf_item
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	for($x = 0; $x < pg_numrows($res); $x++){
		...
		$devolucao_obrig = (pg_fetch_result($res,$x,'devolucao') != 't') ? 'f' : 't';
		...
		if(pg_numrows($res2) > 0){
			...
			$sql = "SELECT fn_atualiza_pedido_item 
					(peca,pedido,pedido_item,$qtde_fat)
	...
	if (!empty($msg_erro)) {
		$msg_erro .= "\n\n".$log_erro;
		$fp = fopen("/tmp/newmaq/faturamento.err","w");
		fwrite($fp,$msg_erro);
		fclose($fp);
		...
		system("mv $origem$file /tmp/newmaq/faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/newmaq/faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - newmaq - Importa faturamento (importa-faturamento.php)", $msg);
}
