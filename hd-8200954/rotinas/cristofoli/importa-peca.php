...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				list ($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $multiplo, $bloqueado_garantia, $bloqueado_venda) = explode ("\t",$linha);
				$original = array($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $multiplo, $bloqueado_garantia, $bloqueado_venda);
				...
				$bloqueada_venda = strtim($bloqueada_venda);
				$bloqueada_garantia = ($bloqueada_garantia <> 't') ? 'f' : $bloqueada_garantia;
				$bloqueada_venda    = ($bloqueada_venda <> 't') ? 'f' : $bloqueada_venda;
