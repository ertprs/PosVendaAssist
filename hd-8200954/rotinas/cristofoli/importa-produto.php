 ...
 * importa-produto.php
 *
 * Importação de produtos Imbera
 *
 * @author  ronald S. S. Andrade
 * @version 2014.07.30
 *
 */
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				$not_null = array($referencia, $descricao, $codigo_linha, $codigo_familia);
				...
				$nome_comercial  = strtim($nome_comercial);
				...
				if (empty($garantia)){
					...
					$garantia = substr($garantia,0,2);
				}

				if (!empty($codigo_marca)) { // Marcas.. HD 806096

					echo $sql = "SELECT marca
							FROM tbl_marca
				...
				$sql_familia = "SELECT familia FROM tbl_familia
								WHERE TRIM(tbl_familia.descricao) = TRIM('$codigo_familia')
