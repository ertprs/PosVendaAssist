...
//	$diretorio_origem = '/mnt/home/fabiano/public_html/assist/rotinas/'.$fabrica_nome; //  teste local
	...
	$ftp = '/tmp/rowa/telecontrol-' . $fabrica_nome;
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				$desc_tipo_posto = strtim($desc_tipo_posto);
				...
				echo $sql_tipo_posto = "SELECT tipo_posto from tbl_tipo_posto where trim(descricao)='$desc_tipo_posto'";
				$query_tipo_posto = pg_query($con,$sql_tipo_posto);
				if (pg_num_rows($query_tipo_posto) == 0){
					echo "TIPO POSTO...................";
					exit;
				}
				else
				{	
					$tipo_posto = pg_fetch_result($query_tipo_posto, 0, 'tipo_posto');
				}
