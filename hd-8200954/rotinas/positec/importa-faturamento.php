 ...
 *
 * ImportaÁ„o de pedidos de pecas
 *
...
define('ENV','producao');  // producao Alterar para produÁ„o ou algo assim
...
try {
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']		= "/home/positec/positec-telecontrol";
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS positec_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);


	  $sql = "CREATE TABLE positec_nf (
				  txt_cnpj           text,
	  ...
	  foreach($linhas AS $linha){
		...
		$msg_erro = "";
			...
			if(!empty($txt_cnpj)){
				...
				$sql = "INSERT INTO positec_nf ( txt_cnpj                 ,
								    txt_nota_fiscal         ,
								    txt_serie               ,
								    txt_emissao             ,
								    txt_cfop                ,
	...
	$sql = "ALTER TABLE positec_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf ADD COLUMN saida DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf ADD COLUMN valor_frete FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE positec_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE positec_nf.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	#------------ IDENTIFICAR POSTOS NAO ENCONTRADOS PELO CNPJ --------------#
	$sql = "DROP TABLE IF EXISTS positec_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT * INTO positec_nf_sem_posto FROM positec_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM positec_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DROP TABLE IF EXISTS positec_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "CREATE TABLE positec_nf_item (
				txt_cnpj        text,
				txt_nota_fiscal text,
				txt_serie       text,
				txt_referencia  text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(!empty($txt_cnpj)){
		      ...
		      $res = pg_query($con,"BEGIN");
		      $sql = "INSERT INTO positec_nf_item ( txt_cnpj         ,
							  txt_nota_fiscal ,
							  txt_serie       ,
							  txt_referencia  ,
							  txt_pedido      ,
	...
	$sql = "UPDATE positec_nf_item SET
				txt_cnpj        = TRIM(txt_cnpj)                     ,
				txt_nota_fiscal = LPAD(TRIM(txt_nota_fiscal) ,9, '0'),
				txt_serie       = TRIM(txt_serie)                    ,
	...
	$sql = "ALTER TABLE positec_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE positec_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE positec_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE positec_nf_item
			SET pedido = tbl_pedido_item.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE positec_nf_item.txt_pedido::numeric = tbl_pedido_item.pedido
			AND   positec_nf_item.txt_pedido_item::numeric = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE positec_nf_item
			...
			FROM  tbl_peca
			WHERE positec_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE positec_nf
				SET pedido = positec_nf_item.pedido
			FROM positec_nf_item
			WHERE trim(positec_nf.txt_nota_fiscal) = trim(positec_nf_item.txt_nota_fiscal)
			AND  positec_nf.posto = positec_nf_item.posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM positec_nf_item
			WHERE pedido is null;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS positec_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "SELECT positec_nf.*
			INTO positec_nf_sem_itens
			FROM positec_nf
			LEFT JOIN positec_nf_item ON positec_nf.txt_nota_fiscal = positec_nf_item.txt_nota_fiscal
			WHERE positec_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "DELETE FROM positec_nf
			USING positec_nf_sem_itens
			WHERE positec_nf.txt_nota_fiscal = positec_nf_sem_itens.txt_nota_fiscal
			AND   positec_nf.txt_serie       = positec_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						positec_nf.emissao         ,
						positec_nf.saida         ,
						substring(positec_nf.txt_transp2, 1,30),
						positec_nf.posto           ,
						positec_nf.total           ,
						positec_nf.txt_cfop        ,
						positec_nf.txt_nota_fiscal ,
						positec_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = positec_nf.pedido),
						case when positec_nf.valor_frete is null then 0 else positec_nf.valor_frete end as valor_frete
				FROM positec_nf
				LEFT JOIN tbl_faturamento ON  positec_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  positec_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE positec_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE positec_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE positec_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = positec_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = positec_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE positec_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);


	$sql = "SELECT * INTO positec_nf_item_sem_peca
			FROM positec_nf_item
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	for($x = 0; $x < pg_numrows($res); $x++){
		...
		if(pg_numrows($res2) > 0){
			...
			$msg_erro .= " Pedido: $pedido - Pe√ßa: $txt_referencia - Qtd.Fat: $qtde_fat \n";
			$msg_erro .= "\n\n\n";
		}
	...
	$sql = "UPDATE tbl_pedido
			...
			FROM  positec_nf_item
			WHERE tbl_pedido.pedido = positec_nf_item.pedido";
	$res3 = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		...
		$fp = fopen("/tmp/positec/faturamento.err","w");
		fwrite($fp,$log_erro);
		fclose($fp);

		system("mv $origem$file /tmp/positec/Faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/positec/Faturamento_item".date('Y-m-d-H-i').".txt");
	...
	$e->getMessage();
    $msg = "Arquivo: ".__FILE__."\r\n<br />Linha: " . $e->getLine() . "\r\n<br />Descri√ß√£o do erro: " . $e->getMessage() ."<hr /><br /><br />". implode("<br /><br />", $logs);

    Log::envia_email($data,Date('d/m/Y H:i:s')." - POSITEC - Importa faturamento (importa-faturamento.php)", $msg);
}?>