...
try {
    ...
    $arquivo_err = "/tmp/positec/gera-pedido-troca-{$data_sistema}.err";
    $arquivo_log = "/tmp/positec/gera-pedido-troca-{$data_sistema}.log";
    system ("mkdir /tmp/positec/ 2> /dev/null ; chmod 777 /tmp/positec/" );
    ...
    $sql = "SELECT  DISTINCT
			...
			AND tbl_servico_realizado.troca_produto
					...
					if(pg_last_error($con)){
						...
						if(pg_last_error($con)){
							...
							$sql = "SELECT fn_atualiza_os_item_pedido_item (os_item,$pedido,$pedido_item,$fabrica)
									FROM tbl_os_item
									WHERE peca = $peca
									AND os_produto IN (SELECT os_produto FROM tbl_os_produto join tbl_os_item using(os_produto) join tbl_servico_realizado using(servico_realizado)  WHERE os = $os and tbl_servico_realizado.troca_produto)";
							$resultX = pg_query($con, $sql);
	...
	if ($logs_erro) {
		...
		Log::envia_email($vet, "Log de ERROS - Geração de Pedido de Troca de OS positec", $logs_erro);
