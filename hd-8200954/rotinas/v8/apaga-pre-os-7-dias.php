...
try{
    ...
    */
    $sql = "SELECT  tbl_hd_chamado_extra.hd_chamado
            INTO    TEMP tmp_hd_chamado_7_dias
            ...
            AND     (tbl_hd_chamado.data::date + INTERVAL '7 days') < CURRENT_DATE
    ";
    ...
    */
    $sqlOS = "  SELECT  tbl_os.os           ,
                        tbl_os.hd_chamado
                FROM    tbl_os
                JOIN    tmp_hd_chamado_7_dias USING (hd_chamado)
    ...
    if(pg_num_rows($resOS) > 0){
        for($i=0;$i < pg_num_rows($resOS); $i++){
            ...
            */
            $sqlDelTemp = " DELETE FROM tmp_hd_chamado_7_dias
                            WHERE hd_chamado = $hd_atualizar
            ";
    ...
    */
    ...
    $sqlUpHD = "UPDATE  tbl_hd_chamado_extra
                SET     abre_os = false
                WHERE   hd_chamado IN   (
                                            SELECT  tmp_hd_chamado_7_dias.hd_chamado
                                            FROM    tmp_hd_chamado_7_dias
                                        )
    ";
    ...
    $sqlFinal = "   SELECT  COUNT(tmp_hd_chamado_7_dias.hd_chamado) AS qtde_hd
                    FROM    tmp_hd_chamado_7_dias
    ";
    ...
    echo 'Rotina rodada com sucesso!<br />Total de '.$qtde_hd.' registros!';
