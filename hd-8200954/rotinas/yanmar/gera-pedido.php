...
try {
    ...
    */
    $logClass = new Log2();
    $logClass->adicionaLog(array("titulo" => "Log erro - Geração de Pedidos Yanmar")); // Titulo
    if ($env == 'producao' ) {
    ...
    */
        ...
        try {
			...
			$preco = $pedidoClass->getPrecoPecaGarantia($peca["peca"], $os);
                    ...
                    $dadosItens[] = array(
                        ...
                        "qtde_cancelada"    => 0,
                        "preco"             => $preco,
			"total_item"        => $preco * $dadosItens["qtde"]
                    );
    ...
    if(!empty($msg_erro)){
        ...
        $fp = fopen("tmp/{$fabrica_nome}/pedidos/log-erro".date('d-m-Y').".txt", "a");
