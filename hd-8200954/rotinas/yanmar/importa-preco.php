...
try {
	...
	$arquivo_origem = 'telecontrol-tabela.txt';
	...
	if (!is_dir($log_dir)) {
		if (!mkdir($log_dir, 0777, true)) {
			throw new Exception("ERRO: Não foi possí­vel criar logs. Falha ao criar diretório: $log_dir");
		}
	}
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				if (pg_num_rows($query_tabela) == 1) {
					...
					$sql_tabela = "INSERT INTO tbl_tabela(sigla_tabela,fabrica,descricao,tabela_garantia) VALUES(TRIM('$sigla_tabela'),$fabrica,TRIM('$sigla_tabela'),'t') RETURNING tabela";
					$query_tabela = pg_query($con, $sql_tabela);

					if (pg_last_error()) {
						array_push($original, 'erro ao gravar tabela');
						$log = implode(";", $original);
						fwrite($nlog, $log . "\n");
