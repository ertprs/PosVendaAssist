...
try {
	...
	if (ENV == 'producao' ) {
    	...
    	$data['dest'] 			= 'guilherme.curcio@telecontrol.com.br';
    }
    ...
    $sql = "SELECT
				...
				AND tbl_os_status.status_os IN (13,19,62,64,68,118,139)
				)
				...
				AND (tbl_os_status.status_os IN(19,64,118,139) OR tbl_os_status.status_os IS NULL);

				SELECT DISTINCT posto, linha from tmp_pedido_rinnai ;
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - rinnai - Erro na geração de pedido(gera-pedido.php)", $msg);
}?>
