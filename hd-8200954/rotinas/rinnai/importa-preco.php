...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				$sql_peca = "SELECT peca FROM tbl_peca
								WHERE tbl_peca.referencia = UPPER(TRIM('$referencia_peca'))
		...
		system("mv $arquivo /tmp/$fabrica_nome/posvenda/telecontrol-preco-$data_arq_process.txt");
