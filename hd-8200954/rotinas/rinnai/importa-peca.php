...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				list ($referencia, $descricao, $unidade, $origem_arq, $ipi,$x, $y) = explode ("\t",$linha);
				$original = array($referencia, $descricao, $unidade, $origem_arq, $ipi, $x, $y);
				...
				$sql_peca = "SELECT tbl_peca.peca FROM   tbl_peca
								WHERE  upper(tbl_peca.referencia) = upper('$referencia')
