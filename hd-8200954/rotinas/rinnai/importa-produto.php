...
try {
	...
	$diretorio_origem = '/home/' . $fabrica_nome . '/posvenda/' . $fabrica_nome . '-telecontrol';
	$arquivo_origem = 'telecontrol-produto.txt';

	$ftp = '/tmp/rinnai/posvenda/telecontrol-' . $fabrica_nome;
	...
	$log_dir = '/tmp/' . $fabrica_nome . '/posvenda/logs';
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				foreach ($not_null as $value) {
					if (!$value) {
						array_push($original, 'Campos sem conteúdo');
						$log = implode("\t", $original);
						fwrite($nlog, $log . "\n");

						$log_erro = implode(";", $original);
				...
				if (pg_last_error()) {
					array_push($original, 'Erro cadastro linha');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_num_rows($query_linha) == 1) {
					...
					array_push($original, 'Código linha não cadastrado.');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_last_error()) {
					array_push($original, 'Erro no cadastro familia.');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_num_rows($query_familia) == 1) {
					...
					array_push($original, utf8_decode('Código familia não cadastrado.'));
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_last_error()) {
					array_push($original, 'Erro cadastro produto');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
				...
				if (pg_last_error()) {
					array_push($original, 'Erro no insert do produto');
					$log = implode(";", $original);
					fwrite($nlog, $log . "\n");
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				if (ENV == 'producao') {
					$mail->AddAddress('marisa.silvana@telecontrol.com.br');
					$mail->AddAddress('ronaldo@telecontrol.com.br');
					$mail->AddAddress('luis@rinnai.com.br');
				} else {
					$mail->AddAddress(DEV_EMAIL);
				}
		...
		system("mv $arquivo /tmp/$fabrica_nome/posvenda/telecontrol-produto-$data_arq_process.txt");
