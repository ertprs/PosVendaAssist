...
try {
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']		= "/home/nordtech/nordtech-telecontrol";
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS nordtech_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);
	

	  $sql = "CREATE TABLE nordtech_nf (
				  txt_cnpj           text,
	  ...
	  foreach($linhas AS $linha){
		...
		$msg_erro = "";
			...
			if(!empty($txt_cnpj)){
				...
				$sql = "INSERT INTO nordtech_nf ( txt_cnpj                 ,
								    txt_nota_fiscal         ,
								    txt_serie               ,
								    txt_emissao             ,
								    txt_cfop                ,
	...
	$sql = "ALTER TABLE nordtech_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf ADD COLUMN saida DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf ADD COLUMN valor_frete FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE nordtech_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE nordtech_nf.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	#------------ IDENTIFICAR POSTOS NAO ENCONTRADOS PELO CNPJ --------------#
	$sql = "DROP TABLE IF EXISTS nordtech_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "SELECT * INTO nordtech_nf_sem_posto FROM nordtech_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DELETE FROM nordtech_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DROP TABLE IF EXISTS nordtech_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "CREATE TABLE nordtech_nf_item (
				txt_cnpj        text,
				txt_nota_fiscal text,
				txt_serie       text,
				txt_referencia  text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(!empty($txt_cnpj)){
		      ...
		      $res = pg_query($con,"BEGIN");
		      $sql = "INSERT INTO nordtech_nf_item ( txt_cnpj         ,
							  txt_nota_fiscal ,
							  txt_serie       ,
							  txt_referencia  ,
							  txt_pedido      ,
	...
	$sql = "UPDATE nordtech_nf_item SET
				txt_cnpj        = TRIM(txt_cnpj)                     ,
				txt_nota_fiscal = LPAD(TRIM(txt_nota_fiscal) ,9, '0'),
				txt_serie       = TRIM(txt_serie)                    ,
	...
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE nordtech_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE nordtech_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE nordtech_nf_item
			SET pedido = tbl_pedido.pedido
			FROM tbl_pedido
			WHERE nordtech_nf_item.txt_pedido::numeric = tbl_pedido.pedido
	...
	$sql = "UPDATE nordtech_nf_item
			...
			FROM  tbl_peca
			WHERE nordtech_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "UPDATE nordtech_nf
				SET pedido = nordtech_nf_item.pedido
			FROM nordtech_nf_item
			WHERE trim(nordtech_nf.txt_nota_fiscal) = trim(nordtech_nf_item.txt_nota_fiscal)
			AND  nordtech_nf.posto = nordtech_nf_item.posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM nordtech_nf_item
			WHERE pedido is null;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS nordtech_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "SELECT nordtech_nf.*
			INTO nordtech_nf_sem_itens
			FROM nordtech_nf
			LEFT JOIN nordtech_nf_item ON nordtech_nf.txt_nota_fiscal = nordtech_nf_item.txt_nota_fiscal
			WHERE nordtech_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "DELETE FROM nordtech_nf 
			USING nordtech_nf_sem_itens
			WHERE nordtech_nf.txt_nota_fiscal = nordtech_nf_sem_itens.txt_nota_fiscal
			AND   nordtech_nf.txt_serie       = nordtech_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						nordtech_nf.emissao         ,
						nordtech_nf.saida         ,
						substring(nordtech_nf.txt_transp2, 1,30),
						nordtech_nf.posto           ,
						nordtech_nf.total           ,
						nordtech_nf.txt_cfop        ,
						nordtech_nf.txt_nota_fiscal ,
						nordtech_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = nordtech_nf.pedido),
						case when nordtech_nf.valor_frete is null then 0 else nordtech_nf.valor_frete end as valor_frete
				FROM nordtech_nf
				LEFT JOIN tbl_faturamento ON  nordtech_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  nordtech_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "ALTER TABLE nordtech_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	$sql = "UPDATE nordtech_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = nordtech_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = nordtech_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE nordtech_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	

	$sql = "SELECT * INTO nordtech_nf_item_sem_peca 
			FROM nordtech_nf_item 
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE tbl_pedido
			...
			FROM  nordtech_nf_item
			WHERE tbl_pedido.pedido = nordtech_nf_item.pedido";
	$res3 = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		...
		$fp = fopen("/tmp/nordtech/faturamento.err","w");
		fwrite($fp,$log_erro);
		fclose($fp);

		system("mv $origem$file /tmp/nordtech/Faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/nordtech/Faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - NORDTECH - Importa faturamento (importa-faturamento.php)", $msg);
}?>