	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		if (filesize($arq_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = ucfirst($fabrica_nome) . utf8_decode(': Importacao de postos ') . date('d/m/Y');
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = ucfirst($fabrica_nome) . utf8_decode(': Erros na importacao de postos ') . date('d/m/Y');
