...
try {
    ...
    $log->adicionaLog(array("titulo" => "Log erro Exportação Pedidos Master Áudio")); // Titulo
    ...
    if ($numrows) {
        ...
        if (!is_dir($dir)) {
            ...
            if (!mkdir($dir)) {
                $log->adicionaLog("Erro ao criar diretório do fabricante.");
                $log->adicionaLog("linha");
                throw new Exception('Erro ao criar diretório do fabricante.'."\n");
            }

            if (!chmod($dir, 0777)) {
                $log->adicionaLog("Erro ao dar permissão ao diretório.");
                $log->adicionaLog("linha");
        ...
        if (!is_resource($fp)) {
            $log->adicionaLog("Erro ao criar arquivo de exportação.");
            $log->adicionaLog("linha");
