	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				...
				echo $sql_peca = "SELECT peca FROM tbl_peca
								WHERE ( tbl_peca.referencia = TRIM('$referencia_peca') or tbl_peca.referencia_pesquisa = TRIM('$referencia_peca'))
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = utf8_decode('MasterAudio: Erros na importação de preço ') . date('d/m/Y');
