...
try {
    ...
    $data['login_fabrica'] 		= 126;
    $data['fabrica'] 	= 'masteraudio';
    ...
    if (ENV == 'producao' ) {
	    ...
	    $data['origem']		= "/home/masteraudio/posvenda/masteraudio-telecontrol/";
    } else {
	    $data['dest'] 		= 'ronald.santos@telecontrol.com.br';
	    $data['dest_cliente'] 	= 'ronald.santos@telecontrol.com.br';
	    $data['origem']		= '/home/ronald/perl/masteraudio/entrada/';
    }
    ...
    if(file_exists($origem.$file)){
	  ...
	  $sql = "DROP TABLE IF EXISTS masteraudio_nf;";
	  $res = pg_query($con,$sql);
	  $msg_erro .= pg_errormessage($con);
	

	  $sql = "CREATE TABLE masteraudio_nf (
				  txt_cnpj           text,
	  ...
	  foreach($linhas AS $linha){
		...
		$msg_erro = "";
			...
			if(!empty($txt_cnpj)){
				...
				$sql = "INSERT INTO masteraudio_nf ( 
                                    txt_cnpj,
								    txt_nota_fiscal,
								    txt_serie,
								    txt_emissao,
	...
	$sql = "ALTER TABLE masteraudio_nf ADD COLUMN total FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf ADD COLUMN emissao DATE";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE masteraudio_nf SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE masteraudio_nf.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	#------------ IDENTIFICAR POSTOS NAO ENCONTRADOS PELO CNPJ --------------#
	$sql = "DROP TABLE IF EXISTS masteraudio_nf_sem_posto";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "SELECT * INTO masteraudio_nf_sem_posto FROM masteraudio_nf WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DELETE FROM masteraudio_nf
			WHERE posto IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "DROP TABLE IF EXISTS masteraudio_nf_item;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "CREATE TABLE masteraudio_nf_item (
				txt_cnpj            text,
				txt_nota_fiscal     text,
				txt_serie           text,
				txt_referencia      text,
	...
	foreach($linhas_item AS $linha_item){
	      ...
	      if(!empty($txt_cnpj)){
		      ...
		      $res = pg_query($con,"BEGIN");
		      $sql = "INSERT INTO masteraudio_nf_item (
                                txt_cnpj,
                                txt_nota_fiscal,
                                txt_serie,
                                txt_referencia,
	...
	$sql = "UPDATE masteraudio_nf_item SET
				txt_cnpj        = TRIM(txt_cnpj)                     ,
				txt_nota_fiscal = LPAD(TRIM(txt_nota_fiscal) ,9, '0'),
				txt_serie       = TRIM(txt_serie)                    ,
	...
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN posto INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN peca INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN qtde FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN pedido INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN pedido_item INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN unitario FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN aliq_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN aliq_icms FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN valor_ipi FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN valor_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN valor_subs_trib FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN base_icms FLOAT";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN base_ipi FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
    
    	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN base_subs_trib FLOAT";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);

    	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN os_item INT4";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);

    	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN devolucao boolean";
    	$res = pg_query($con,$sql);
    	$msg_erro .= pg_errormessage($con);
	...
	$sql = "UPDATE masteraudio_nf_item SET posto = (
				SELECT tbl_posto.posto
				FROM tbl_posto
				JOIN tbl_posto_fabrica ON tbl_posto.posto = tbl_posto_fabrica.posto
									AND   tbl_posto_fabrica.fabrica = $login_fabrica
				WHERE masteraudio_nf_item.txt_cnpj = tbl_posto.cnpj
			)";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);

	$sql = "UPDATE masteraudio_nf_item
			SET pedido = tbl_pedido.pedido,
				pedido_item = tbl_pedido_item.pedido_item
			FROM tbl_pedido_item, tbl_pedido
			WHERE masteraudio_nf_item.txt_pedido::integer = tbl_pedido.pedido
			AND   masteraudio_nf_item.txt_pedido_item::integer = tbl_pedido_item.pedido_item
	...
	$sql = "UPDATE masteraudio_nf_item
                    ...
                    FROM tbl_os_item
                    WHERE masteraudio_nf_item.txt_pedido::numeric = tbl_os_item.pedido
		    AND masteraudio_nf_item.txt_pedido_item::numeric = tbl_os_item.pedido_item
	...
	$sql = "UPDATE masteraudio_nf_item
			...
			FROM  tbl_peca
			WHERE masteraudio_nf_item.txt_referencia = tbl_peca.referencia
			AND tbl_peca.fabrica = $login_fabrica";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	
	$sql = "UPDATE masteraudio_nf
				SET pedido = masteraudio_nf_item.pedido
			FROM masteraudio_nf_item
			WHERE trim(masteraudio_nf.txt_nota_fiscal) = trim(masteraudio_nf_item.txt_nota_fiscal)
			AND  masteraudio_nf.posto = masteraudio_nf_item.posto";
#	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DELETE FROM masteraudio_nf_item
			WHERE pedido is null;";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	#------------ NFs sem Itens --------------#
	$sql = "DROP TABLE IF EXISTS masteraudio_nf_sem_itens";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "SELECT masteraudio_nf.*
			INTO masteraudio_nf_sem_itens
			FROM masteraudio_nf
			LEFT JOIN masteraudio_nf_item ON masteraudio_nf.txt_nota_fiscal = masteraudio_nf_item.txt_nota_fiscal
			WHERE masteraudio_nf_item.txt_nota_fiscal IS NULL";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "DELETE FROM masteraudio_nf 
			USING masteraudio_nf_sem_itens
			WHERE masteraudio_nf.txt_nota_fiscal = masteraudio_nf_sem_itens.txt_nota_fiscal
			AND   masteraudio_nf.txt_serie       = masteraudio_nf_sem_itens.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "INSERT INTO tbl_faturamento (
				...
				SELECT  $login_fabrica,
						masteraudio_nf.emissao         ,
                        masteraudio_nf.emissao,
						substring(masteraudio_nf.txt_transp, 1,30),
						masteraudio_nf.posto           ,
						masteraudio_nf.total           ,
						masteraudio_nf.txt_cfop        ,
						masteraudio_nf.txt_nota_fiscal ,
						masteraudio_nf.txt_serie       ,
						(select tipo_pedido from tbl_pedido where pedido = masteraudio_nf.pedido),
						masteraudio_nf.txt_natureza
				FROM masteraudio_nf
				LEFT JOIN tbl_faturamento ON  masteraudio_nf.txt_nota_fiscal   = tbl_faturamento.nota_fiscal
										 AND  masteraudio_nf.txt_serie         = tbl_faturamento.serie
										 AND  tbl_faturamento.fabrica      = $login_fabrica
	...
	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN faturamento INT4";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	

	$sql = "ALTER TABLE masteraudio_nf_item ADD COLUMN cfop text";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);	
	...
	$sql = "UPDATE masteraudio_nf_item
			...
			AND   tbl_faturamento.nota_fiscal = masteraudio_nf_item.txt_nota_fiscal
			AND   tbl_faturamento.serie       = masteraudio_nf_item.txt_serie";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	$sql = "DROP TABLE IF EXISTS masteraudio_nf_item_sem_peca ";
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	

	$sql = "SELECT * INTO masteraudio_nf_item_sem_peca 
			FROM masteraudio_nf_item 
				WHERE peca IS NULL" ;
	$res = pg_query($con,$sql);
	$msg_erro .= pg_errormessage($con);
	...
	if (!empty($msg_erro)) {
		$msg_erro .= "\n\n".$log_erro;
		$fp = fopen("/tmp/masteraudio/faturamento.err","w");
		fwrite($fp,$msg_erro);
		fclose($fp);
		...
		system("mv $origem$file /tmp/masteraudio/faturamento".date('Y-m-d-H-i').".txt");
		system("mv $origem$file2 /tmp/masteraudio/faturamento_item".date('Y-m-d-H-i').".txt");
    ...
    Log::envia_email($data,Date('d/m/Y H:i:s')." - Master Audio - Importa faturamento (importa-faturamento.php)", $msg);
}
