...
/**
 *
 * igera-pedido-troca.php
 *
 * Geração de pedidos de troca com base na OS
 *
 * @author  Felipe Marttos
 * @version 2016.09.29
 *
*/

error_reporting(E_ALL ^ E_NOTICE);
define('ENV','producao');  // producao-teste Alterar para produção ou algo assim
...
try {
	...
	if (ENV != 'teste' ) {
    	...
    	$vet['dest'] = 'felipe.marttos@telecontrol.com.br';
    }

    $arquivo_err = "/tmp/arge/gera-pedido-troca-{$data_sistema}.err";
    $arquivo_log = "/tmp/arge/gera-pedido-troca-{$data_sistema}.log";
    system ("mkdir /tmp/arge/ 2> /dev/null ; chmod 777 /tmp/arge/" );
				...
				for($x = 0; $x < pg_num_rows($result); $x++){
					...
					if(pg_last_error($con)){
						...
						if(pg_last_error($con)){
							...
							$sql = "UPDATE tbl_pedido SET status_pedido=2 WHERE pedido=$pedido AND fabrica=$fabrica";
							$resultX = pg_query($con, $sql);
	...
	if ($logs_erro) {
		...
		Log::envia_email($vet, "Log de ERROS - Geração de Pedido de Troca de OS Masterfrio", $logs_erro);
