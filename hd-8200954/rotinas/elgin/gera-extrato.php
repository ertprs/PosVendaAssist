...
try {
    ...
    */
    $sql = "SELECT  posto           ,
            ...
            JOIN tbl_macro_linha_fabrica ON tbl_macro_linha_fabrica.linha = tbl_produto.linha 
      ...
      GROUP BY      posto, tbl_macro_linha_fabrica.macro_linha
      ORDER BY      posto ";
	...
	if (pg_num_rows($res) > 0 && strlen($msg_erro) == 0) {
		...
		for ($i = 0; $i < pg_num_rows($res); $i++) {
			...
			$sql4 = "   UPDATE  tbl_os_extra
                ...
                $sql4 .= "
                        AND     tbl_macro_linha_fabrica.macro_linha = $linha 
                ";
            $sql4 .= "  AND     tbl_os.os               = tbl_os_extra.os
                        AND     tbl_os_extra.extrato    IS NULL
                        AND     tbl_os.excluida         IS NOT TRUE
                        AND     tbl_os.finalizada       < '$dia_extrato'
                        AND     tbl_os.finalizada::date < current_date";
	...
	if (strlen($bug) > 0) {
		...
		Log::envia_email($vet, 'Log - Extrato Elgin', $bug);
	...
	Log::envia_email($data,Date('d/m/Y H:i:s')." - ELGIN - Erro na geração de extrato(gera-extrato.php)", $e->getMessage());
