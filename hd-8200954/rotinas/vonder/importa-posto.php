  ...
  //$ftp = '/mnt/home/fernando/vonder/telecontrol-' . $fabrica_nome;
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			...
			if (!empty($linha)) {
				...
				if(count(explode(';', $linha)) != 17) {
					$log_erro = logErro("Erro, formato do arquivo inválido (Linha $cont_linha)");
					fwrite($elog, $log_erro);
					$log_erro = logErro("Erro, formato do arquivo inválido (Linha $cont_linha)") . pg_last_error();
				...
				if (pg_last_error()) {
					...
					$log_erro = logErro("Erro, cnpj inválido (Linha $cont_linha)");
					fwrite($elog, $log_erro);
					$log_erro = logErro("Erro, cnpj inválido (Linha $cont_linha)") . pg_last_error();
					fwrite($elog_admin, $log_erro);
					continue;
				}
				...
				if (pg_num_rows($query_posto) == 0) {
					...
					if (pg_last_error()) {
						...
						$log_erro = logErro("Erro, não foi possível inserir o posto.");
						fwrite($elog, $logErro);
						$log_erro = logErro("Erro, não foi possível inserir o posto." . pg_last_error());
				...
				if (pg_last_error()) {
					...
					$log_erro = logErro("Erro ao buscar vínculo do posto com a fábrica.");
					fwrite($elog, $logErro);
					$log_erro = logErro("Erro ao buscar vínculo do posto com a fábrica." . pg_last_error());
					fwrite($elog_admin, $logErro);
					continue;
				}
		...
		if (count($dadosDivergentes["Base"]) > 0) {
			...
			$assunto =  ucfirst($fabrica_nome) . ("  - Importação de postos - Razão Social e/ou IE Divergente");
		...
		if (filesize($err_log) > 0) {
			...
			if ($retorno == 0) {
				...
				$assunto = ucfirst($fabrica_nome) . ': Erros na importação de postos ' . date('d/m/Y');
				...
				$mail->Body     = $mail_admin->Body     = "Segue anexo log de erro na importação de postos.<br/><br/>";
			...
			if ($retorno == 0) {
				...
				$assunto = ucfirst($fabrica_nome) . ': Importação de postos ' . date('d/m/Y');
