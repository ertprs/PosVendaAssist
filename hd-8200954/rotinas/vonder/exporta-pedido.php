    ...
    if ($numrows) {
        ...
        for ($i = 0; $i < $numrows; $i++) {
            ...
            for ($x = 0; $x < $tot_pecas; $x++) {
                ...
                #alterado status_pedido de 2 para 9
        ...
        if (file_exists($file) and (filesize($file) > 0)) {
			...
			$destino =  '/home/vonder/telecontrol-vonder/'.$ovd_dwt . '-pedidos.csv';
			copy($file, '/home/vonder/telecontrol-vonder/bkp/'.$ovd_dwt. "_" . $data_arquivo . '.txt');
			copy($file, $destino);
			rename($file, $destino);
