<?php
	define('APP','Importa Pedido Cancelado - VONDER'); // Nome da rotina, para ser enviado por e-mail
	...
	try {
		...
		$vet['dest']    = ENV == 'testes' ? 'gabriel.silveira@telecontrol.com.br' : 'helpdesk@telecontrol.com.br';
		$vet['log']     = 1;

		$file = 'ovd-pedido-cancelado.csv';

		if ( ENV == 'testes' ) {
			$dir 	= '/www/cgi-bin/vonder/entrada';
		} else {
		...
		$sql = "DROP TABLE IF EXISTS ".$vet['fabrica']."_cancelado;
				...
				(cnpj text, txt_pedido text, txt_pedido_item text, txt_peca text, txt_data text, motivo text, txt_qtde text, branco text);

				COPY ".$vet['fabrica']. "_cancelado FROM stdin WITH DELIMITER ';'";
		...
		$sql = "INSERT INTO tbl_pedido_cancelado (fabrica, data, posto, pedido, peca, qtde, motivo, pedido_item) 
				(SELECT $fabrica,data, posto, pedido, peca, qtde, motivo, pedido_item FROM ".$vet['fabrica']."_cancelado )";
