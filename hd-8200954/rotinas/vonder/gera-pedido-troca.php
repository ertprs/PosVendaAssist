...
try {
    ...
    $arquivo_err = "/tmp/vonder/gera-pedido-troca-{$data_sistema}.err";
    $arquivo_log = "/tmp/vonder/gera-pedido-troca-{$data_sistema}.log";
    system ("mkdir /tmp/vonder/ 2> /dev/null ; chmod 777 /tmp/vonder/" );
	...
	if(pg_num_rows($res) > 0 AND count($logs_erro) == 0){
		...
		for($i = 0; $i < pg_num_rows($res); $i++){
			...
			if (count($logs_erro)>0){
				$logs_erro2 = array_merge($logs_erro2, $logs_erro);
	...
	if ($logs_erro) {
		...
		$logs_erro = implode("<br>", $logs_erro2);
	...
	if ($logs_erro) {
		...
		Log::envia_email($vet, "Log de ERROS - Geração de Pedido de Troca de OS vonder", $logs_erro);
