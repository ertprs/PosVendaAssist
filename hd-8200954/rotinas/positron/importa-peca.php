...
try {
	...
	if (file_exists($arquivo) and (filesize($arquivo) > 0)) {
		...
		foreach ($conteudo as $linha) {
			if (!empty($linha)) {
				list ($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $ativo, $garantia_diferenciada, $multiplo, $critica, $bloquea_venda, $bloquea_garantia) = explode ("\t",$linha);
				$original = array($referencia, $descricao, $unidade, $origem_arq, $ipi, $ncm, $ativo, $garantia_diferenciada, $multiplo, $critica, $bloquea_venda, $bloquea_garantia);
				...
				$bloquea_venda = strtim($bloquea_venda);
				$bloquea_garantia = strtim($bloquea_garantia);
				...
				if (pg_num_rows($query_peca) == 0) {
					...
					$sql = "UPDATE tbl_peca SET
									...
									bloqueada_venda = '$bloquea_venda',
									bloqueada_garantia = '$bloquea_garantia'
