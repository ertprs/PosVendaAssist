	...
	for($i=0;$i<$numero_de_notas;$i++){
			...
			if (strlen($msg_erro)==0){
				...
				if (pg_numrows($res)>0){
					...
					$msg_erro = "Foi constatado que a $nota_fiscal já foi emitida por seu posto em $xxemissao. Não pode haver duplicidade.";
				}
			}
			...
			if (strlen($msg_erro)==0){
				$sql = "INSERT INTO tbl_faturamento		  
						...
						VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',13996,$login_posto,$total_nota,'$nota_fiscal','2','Devolução de Garantia', $base_icms, $valor_icms, $base_ipi, $valor_ipi, $extrato, 'Devolução de peças do posto para à  Fábrica',$cfop,'$movimento')";
				$res = pg_exec ($con,$sql);
...
<TR>
	<TD colspan='8' class="table_line" style='padding:10px'>
		As peças ou produtos não devolvidos neste extrato serão apresentadas na tela de consulta de pendências. 
		Caso não sejam efetivadas as devoluções, os itens serão cobrados do posto autorizado.
	<br><br>
	<? //HD 15408 ?>
...
//						echo "<input type='hidden' name='id_nota_$numero_nota-linha' value='$linha'>\n";
						...
						if (strlen ($nota_fiscal) >0){
							echo "<table border='1' cellspacing='0' cellpadding='3' border-color='#000' style='border-collapse:collapse;font-size:12px' width='650' >\n";
							echo "<tr>\n";
							echo "<td>aqui3<h1><center>Nota de Devolução $nota_fiscal</center></h1></td>\n";
							echo "</tr>";
							echo "</table>";
						}
					}
