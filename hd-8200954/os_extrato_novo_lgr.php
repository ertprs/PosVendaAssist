...
if ($ajax == "true") {
    ...
    if (strlen($extrato) > 0) {
		...
		if(!in_array($login_fabrica,[177])){
			...
			fecho("clique.aqui.para.preencher.a.nota.fiscal.de.devolucao",$con,$cook_idioma); 	
		}
                echo "'>";
...
?>
        $(document).ready(function(){
            setTimeout(function() {
                Shadowbox.open({
                    content:    'comunicado_extrato_positron.php',
                    player:     "iframe",
                    title:      "Comunicado",
                    width:      800,
                    height:     1000
...
?>
        $(document).ready(function(){
            setTimeout(function() {
                Shadowbox.open({
                    content:    'comunicado_extrato_positec.php',
                    player:     "iframe",
                    title:      "Comunicado",
                    width:      800,
                    height:     1000
...
<?php
    ...
    if ($usaNotaFiscalServico) { ?>
        $(document).ready(function(){
            setTimeout(function() {
                Shadowbox.open({
                    content:    'comunicado_extrato_nfe.php',
                    player:     "iframe",
                    title:      "Comunicado",
                    width:      800,
                    height:     1000
...
function gravaNota(nota,data,valor,mostrar,extrato){
    ...
    if(nota.length == 0 || data.length==0 || valor.length==0 ) {
        ...
        if(confirm('<?= traduz("VocÍ tem certeza das informaÁıes digitadas? Elas n„o poder„o ser alteradas, somente a F·brica poder· alterar?") ?>') == true){
            $.post(
                '<?=$PHP_SELF?>',
                {
                    nota_fiscal: nota,
...
<?  }
    ...
    if (strlen($periodo) > 0 or $login_fabrica==19) {
        ...
        if ($exibir == 'acumulado') {
            ...
            if (pg_num_rows($res) > 0) {
                ...
                if(!in_array($login_fabrica,array(121,123,124,125,126,127,128,129,131,134,136,137,138,139,140,141,144,145,146,147,150,152,153,157,160,161,180,181,182)) and !$replica_einhell){
                    echo "<td align=\"center\">";
                ...
                for ($i = 0 ; $i < pg_num_rows ($res) ; $i++){
                    ...
                    if(!in_array($login_fabrica,array(121,123,124,125,126,127,128,129,131,134,136,137,138,139,140,141,142,144,145,146,147,152,153,156,157,160,161,180,181,182)) and !$replica_einhell){
    ...
    /*      LEFT JOIN   tbl_os_extra         ON tbl_os_extra.extrato      = tbl_extrato.extrato
        ...
        if($login_fabrica == 24){
		$sql_data = " AND (
			(tbl_extrato.data_geracao > CURRENT_DATE - INTERVAL '2 months'
			OR tbl_extrato.extrato IN (SELECT extrato FROM tbl_extrato_status WHERE data::DATE > CURRENT_DATE - INTERVAL '2 months'))
			OR (tbl_extrato_pagamento.data_pagamento IS NOT NULL AND tbl_extrato.data_geracao <= CURRENT_DATE - INTERVAL '2 months')
			)
		";
        }
    ...
    if ($login_fabrica == 91) {                                                                                                                                            
        if (is_null($meses_extrato)) {
            $qtde_extrato = 4; 
        } else {
            $sql_data = " AND ((tbl_extrato.data_geracao > current_date-interval '$meses_extrato month' and tbl_extrato_pagamento.data_pagamento isnull) or tbl_extrato_pagamento.data_pagamento notnull) ";
        }
    }

        if(in_array($login_fabrica, array(81,114,122,123,125,147,160)) or $replica_einhell) {
            $coluna_positec = " tbl_extrato.nf_recebida,  ";
        }
        ...
        if ( in_array($login_fabrica,array(15,158))) {
            ...
            if(pg_num_rows($res_extrato_3_meses) > 0){
                ...
                if(strlen($parametros_adicionais) > 0){
                    ...
                    if(isset($parametros_adicionais["extrato_mais_3_meses"])){
                        ...
                        if($parametros_adicionais["extrato_mais_3_meses"] == "f"){
    ...
    $limit_extrato = ($login_fabrica == 91) ? 4 : 3;
    $limit_extrato = ($login_fabrica == 91 AND !empty($meses_extrato)) ? intval($meses_extrato) : $limit_extrato;
    
    if(in_array($login_fabrica,[15])){
        
        
                $sql .= " limit $limit_extrato";
        }
    }
    $res = pg_query ($con,$sql);
//exit($sql);
    ...
    /* hd 39502 */
    if (in_array($login_fabrica,array(85,91,125,131,138,140))) {
    ...
    if ($login_fabrica == 156) {
        $sql = "SELECT * FROM tmp_os_extrato WHERE extrato NOT IN (
...
//echo $sql;exit;
    ...
    if ($login_fabrica == 158) {
        echo "
            <td align='center' width='16' bgcolor='#fff159'>&nbsp;</td>
            <td align='left'><font size=1><b>&nbsp; ".traduz('PendÍncia de Nota de LGR')."</b></font></td>
            ";
    }

    if($telecontrol_distrib || $usaNotaFiscalServico){
        echo "<td align='center' width='16' bgcolor='#FFFF99'>&nbsp;</td>";
        echo "<td align='left'><font size=1><b>&nbsp;";
        echo traduz("Envio de NFe pendente");
        echo "</b></font></td>";

        echo "<td align='center' width='16' bgcolor='#33CCFF'>&nbsp;</td>";
        echo "<td align='left'><font size=1><b>&nbsp;";
        echo traduz("NFe Enviada");
        echo "</b></font></td>";

        echo "<td align='center' width='16' bgcolor='#99FF99'>&nbsp;</td>";
        echo "<td align='left'><font size=1><b>&nbsp;";
        echo traduz("Extrato Pago");
        echo "</b></font></td>";
    }
    if ($login_fabrica == 161) {
        echo "<td align='center' width='16' bgcolor='red'>&nbsp;</td>";
        echo "<td align='left'><font size=1><b>&nbsp;";
        echo traduz("PeÁas ObrigatÛria acima de 90 dias");
        echo "</b></font></td>";
    }

    if(in_array($login_fabrica,array(94,104,105,106,115,116,117,123,125,127,128,129,134,141,142,144,149,151,157,160,161,163,164)) or $replica_einhell){
        echo "<td align='center' width='16'><input type ='button' value='".traduz('PeÁas para InspeÁ„o')."' onclick=\"window.open('lgr_vistoria_itens.php')\"></td>";
    }
    ...
    if ($login_fabrica == 158) {
        ...
        if (pg_num_rows($resPostoUnidade) > 0) {
            $posto_unidade_negocio = true;
        } else {
            $posto_unidade_negocio = false;
        }
    }
    ...
    if (pg_num_rows($res) > 0) {
        ...
        if ($login_fabrica == 158) {
            ...
            echo "<td align='center'>Unidade de negÛcio</td>";
        }
        ...
        if((!in_array($login_fabrica,array(94,99,106,121,123,124,125,126,127,128,129,131,134,136,137,140,139)) && !isset($novaTelaOs)) || (isset($novaTelaOs) && !$nao_calcula_peca) && (!in_array($login_fabrica, [193]))){
            echo "<td align='center'>";
            fecho("pecas",$con,$cook_idioma);
            echo "</td>\n";
        }
        ...
        if(in_array($login_fabrica, array(35,90))){
            echo "<td align='center'>";
        ...
        if($login_fabrica <> 51 AND $login_fabrica <> 128){
            ...
            if (in_array($login_fabrica, array(120,201,129))) {
                ...
                if(!isset($novaTelaOs)){
                    ...
                    echo (in_array($login_fabrica,array(50,30,85,91,94,120,201,125,129,131,138,140))) ? "<td align='center' nowrap>+ KM</td>\n" : "";
                } else {
                    if (isset($fabrica_usa_valor_adicional) || $login_fabrica == 145) {
                ...
                if (!in_array($login_fabrica,[11,20,45]) && !isset($novaTelaOs)) { 
                    echo "<td align='center'>TOTAL</td>\n";
                }
        ...
        if (!in_array($login_fabrica, array(99,142,145,164,186))) {

            echo "<td align='center'";

            echo (!in_array($login_fabrica,array(11,15,45,50,81,114,115,116,117,120,201,121,123,124,125,126,127,128,129,131,134,136,137,138,139,140,141,142,144,151,152,153,157,160,161,180,181,182)) and !$replica_einhell) ? (($login_fabrica == 146) ? "nowrap>" : "nowrap>(*)") : ">".(in_array($login_fabrica, array(157)) ? "Previs„o de Pagamento" : "Data de Pagamento");
            if ($login_fabrica == 20 or $login_fabrica == 5) {
                ...
                if(!in_array($login_fabrica,array(11,15,45,50,81,114,115,116,117,120,201,121,123,124,125,126,127,128,129,131,134,136,137,138,139,140,141,142,144,151,152,153,157,160,161,180,181,182)) and !$replica_einhell){

                   if(in_array($login_fabrica, array(104,146))){
                        echo "Data Baixa";
                    }elseif ($login_fabrica == 91) {
                        fecho("Data de<br> Pagamento",$con,$cook_idioma);
                    }else{
                        fecho("previsao",$con,$cook_idioma);
                    }
                }
        ...
        if (in_array($login_fabrica, array(101))) echo "<td align='center' nowrap>NF de ServiÁo</td>\n";
        ...
        if ($login_fabrica == 51) {
             if ($extrato > 640885) {
                ...
                $join_extrato = " JOIN tbl_extrato ON fi.extrato_devolucao = tbl_extrato.extrato AND tbl_extrato.fabrica = $login_fabrica and liberado notnull";
             } else {
                $campo_extrato_dev = "fi.extrato_devolucao ";
                $join_extrato = " JOIN tbl_extrato ON f.extrato_devolucao = tbl_extrato.extrato AND tbl_extrato.fabrica = $login_fabrica and liberado notnull";
             }
        } else {
            $campo_extrato_dev = "fi.extrato_devolucao ";
            $join_extrato = " JOIN tbl_extrato ON fi.extrato_devolucao = tbl_extrato.extrato AND tbl_extrato.fabrica = $login_fabrica and liberado notnull";
        ...
        if (in_array($login_fabrica,array(6,24,35,50,81,85,90,91,94,99,106,114,115,116,117,120,201,121,123,124,125,126,127,128,129,131,134,136,138,139,140,141,142,144,145,146,147,151,152,153,157,160,161,180,181,182)) or ($login_fabrica  == 51 and $extrato < '839297') || isset($usaLGR) or $replica_einhell) {
			...
			if($login_fabrica == 6){
					...
					$peca_obri_cond = " AND (tbl_peca.produto_acabado IS TRUE  OR (tbl_peca.devolucao_obrigatoria = 't' AND (tbl_peca.parametros_adicionais is null or tbl_peca.parametros_adicionais !~* '$devolucaoEstoqueFabrica')) OR (tbl_peca.parametros_adicionais like '%".$devolucaoEstoqueFabrica."%' AND tbl_peca.devolucao_obrigatoria = 'f'))";
			} else if (in_array($login_fabrica, array(114,126,129,131,134,138,141,144,145,147,151,152,157,180,181,182))  || isset($usaLGR)) {
				$peca_obri_cond = " AND (fi.devolucao_obrig is true or tbl_peca.produto_acabado or tbl_os_item.peca_obrigatoria)";
			} elseif($login_fabrica == 35)  {
				$peca_obri_cond = " AND ((tbl_os_item.peca_obrigatoria or (tbl_peca.devolucao_obrigatoria and tbl_os_item.peca_obrigatoria isnull)) and tbl_peca.produto_acabado is not true)";
			} else {
				$peca_obri_cond = " AND (tbl_os_item.peca_obrigatoria or (tbl_peca.devolucao_obrigatoria and tbl_os_item.peca_obrigatoria isnull))";
			}
		}            
            ...
            if ($login_fabrica == 158 && $posto_unidade_negocio == true) {
                $sqlDev = "
                    SELECT DISTINCT tbl_extrato_lgr.extrato AS extrato_devolucao
                    FROM tbl_extrato_lgr
                    INNER JOIN tbl_extrato ON tbl_extrato.extrato = tbl_extrato_lgr.extrato AND tbl_extrato.fabrica = {$login_fabrica}
                    LEFT JOIN tbl_faturamento_item AS fdi ON fdi.extrato_devolucao = tbl_extrato_lgr.extrato AND fdi.peca = tbl_extrato_lgr.peca AND fdi.faturamento = tbl_extrato_lgr.faturamento
                    LEFT JOIN tbl_faturamento AS fd ON fd.faturamento = fdi.faturamento AND fd.fabrica = {$login_fabrica} AND fd.distribuidor = {$login_posto}
                    WHERE tbl_extrato_lgr.posto = {$login_posto}
                    AND (
                        tbl_extrato_lgr.faturamento IS NULL
                        OR (
                            tbl_extrato.admin_lgr IS NULL
                ...
                $sqlDev = "SELECT DISTINCT $campo_extrato_dev,
                    ...
                    LEFT JOIN tbl_os_item ON fi.pedido = tbl_os_item.pedido and fi.pedido_item = tbl_os_item.pedido_item
                      $join_extrato
					  ...
					  AND (f.natureza !~*'Devolu' or f.natureza isnull)
                      AND (
                            f.cfop LIKE '59%'
                            OR f.cfop LIKE '69%'
                            OR fi.cfop ILIKE '59%'
        ...
        for ($i = 0 ; $i < pg_num_rows ($res); $i++){
            ...
            $gerado = trim(pg_fetch_result($res,$i,data_extrato));                
            if ($login_fabrica == 91){ 
                $data_limite = date('Y-m-d', strtotime("-{$limit_extrato} month", strtotime(date('Y-m-d'))));
                if ($data_pagamento == '' && strtotime($data_limite) > strtotime($gerado)) {
                    continue;
                }
            }   
            ...
            if ($login_fabrica == 160 or $replica_einhell) {
                $sqlAuditoriaBonificacao = "SELECT auditoria_os 
                                            FROM tbl_extrato_lancamento
                                            JOIN tbl_auditoria_os ON tbl_auditoria_os.os = tbl_extrato_lancamento.os
                                            ...
                                            ";
                $resAuditoriaBonificacao = pg_query($con, $sqlAuditoriaBonificacao);

                //caso o extrato tenha alguma OS com auditoria de bonificacao pendente, n„o exibe
                if (pg_num_rows($resAuditoriaBonificacao) > 0) {
                    continue;
                }
            }
            ...
            if ($aux_anexo == 't') {
                ...
                if(count($nota_fiscal_servico) > 0 && $nf_recebida != 't') {
                    $nf_recebida = 't';
                }
                unset($nota_fiscal_servico);
            }

            $data_geracao            = trim(pg_fetch_result($res,$i,data_geracao));

            $mostra_extrato_bloqueado = false;
            if($login_fabrica == 24 ){
                $data_array = explode("/", $data_geracao);
                $data_geracao_lgr = $data_array[2]."-".$data_array[1]."-".$data_array[0];
                $data_corte_lgr = '2018-07-01'; 
                $ano_geracao = substr($data_geracao, 6, 4);
                //hd-3872999 -> Liberar para o Posto 23465 - ELETROSUL COM. DE COMP os valores dos extratos de 2017, n„o dever· mostrar o link "EXTRATO BLOQUEADO"
                if($posto == 22161 AND $ano_geracao == '2017'){
                    $mostra_extrato = true;
                }else{
                    $mostra_extrato = false;
                }

                if(strtotime($data_geracao_lgr) >= strtotime($data_corte_lgr)){
                    $mostra_extrato = true;
                }

                $sqlVerificaExtratoLiberado = "SELECT extrato
                                               FROM tbl_extrato_status
                                               WHERE extrato = {$extrato}
                                               AND obs = 'extrato desbloqueado'
                                             ";
                $resVerificaExtratoLiberado = pg_query($con, $sqlVerificaExtratoLiberado);

                if (pg_num_rows($resVerificaExtratoLiberado) > 0) {
                    $mostra_extrato_bloqueado = true;
                }
            ...
            if(in_array($login_fabrica,array(30,35,50,90,94,120,201,128,131,140)) || isset($novaTelaOs)){
                $deslocamento_km         = trim(pg_fetch_result($res,$i,deslocamento_km));
            }

            if(in_array($login_fabrica,array(85,91,125))){
            ...
            //aqui comeca a o lgr
            // retirada a fabrica 120 do if, porem manteva a regra interna, caso a fabrica volte a utilizar
            if ((in_array($login_fabrica,array(6,24,35,50,81,85,90,91,94,99,106,114,115,116,117,120,201,121,123,124,125,126,127,128,129,131,134,136,138,139,140,141,142,144,145,146,147,151,152,153,157,160,161,180,181,182)) || ($login_fabrica  == 51 and $extrato < '839297') || $replica_einhell || isset($usaLGR)) && (($extrato >= $extrato_dev and !empty($extrato_dev)) || ($login_fabrica == 158 || $login_fabrica == 177))) {
                $msg_notas = "";
                $msg_mes_anterior = "";
                ...
                if (pg_num_rows($resConf)>0){
                  ...
                  $sqlConf = "SELECT  DISTINCT faturamento,
                    ...
                    if (count($notas_array)>0 OR pg_num_rows($resConf)==0){
                        ...
                        if (count($notas_array)>0 AND !in_array($login_fabrica,[24,99])){
                            ...
                            if ($verificacao=='1' OR in_array($login_fabrica,array(6,24,35,50,81,85,91,94,99,106,114,115,116,117,120,201,121,123,124,125,126,127,128,129,131,134,136,138,140,141,142,144,145,139,146,147,151,152,153,157,160,161,180,181,182)) OR ($login_fabrica == 90 AND $data_corte_extrato) || isset($usaLGR) || $replica_einhell) {
                                if($login_fabrica == 6){
                                         $devolucaoEstoqueFabrica = "\"devolucao_estoque_fabrica\":\"t\"";
                                $sqlConf .= " AND (tbl_peca.produto_acabado IS TRUE  OR (tbl_peca.devolucao_obrigatoria = 't' AND (tbl_peca.parametros_adicionais is null or tbl_peca.parametros_adicionais !~* '$devolucaoEstoqueFabrica')) OR (tbl_peca.parametros_adicionais like '%$devolucaoEstoqueFabrica%' AND tbl_peca.devolucao_obrigatoria = 'f'))";

                                } else if (in_array($login_fabrica, array(114,125,129,131,134,136,138,141,144,145,147,151,152,157,180,181,182)) || isset($usaLGR) ) {
                                        $sqlConf .= "AND    (tbl_faturamento_item.devolucao_obrig IS TRUE OR tbl_peca.produto_acabado IS TRUE or tbl_os_item.peca_obrigatoria) ";
								} elseif($login_fabrica == 35)  {
                                        $sqlConf .= "AND    ((tbl_faturamento_item.devolucao_obrig IS TRUE or tbl_os_item.peca_obrigatoria) and tbl_peca.produto_acabado is not true) ";
                                } else {
                                    $sqlConf .= " AND (tbl_os_item.peca_obrigatoria or (tbl_peca.devolucao_obrigatoria and tbl_os_item.peca_obrigatoria isnull) or tbl_peca.produto_acabado)";
                                }
                            }                                   

                            $resConf = pg_query ($con,$sqlConf);                            
                            if (pg_num_rows($resConf)> 0 AND !in_array($login_fabrica, array(99,120,201)) AND $mostra_extrato == false) {
            ...
            #Regra nova do LGR pegando no faturamento_item inicialmente para suggar Waldir 24/12/2009
                    if (in_array($login_fabrica,array(6,35,50,81,51,85,90,91,24,94,99,106,114,115,116,117,120,201,121,123,124,125,126,128,129,131,134,136,138,139,140,141,142,144,145,146,147,151,152,153,157,160,161,180,181,182)) || isset($usaLGR) || $replica_einhell) {
                        if ($login_fabrica == 51) {
                             if ($extrato > 640885) {
                        ...
                        if($login_fabrica == 90){
                            ...
                            $sqlConf .= " AND (tbl_peca.produto_acabado IS TRUE  OR (tbl_peca.devolucao_obrigatoria = 't' AND (tbl_peca.parametros_adicionais is null or tbl_peca.parametros_adicionais !~* '$devolucaoEstoqueFabrica')) OR (tbl_peca.parametros_adicionais like '%$devolucaoEstoqueFabrica%' AND tbl_peca.devolucao_obrigatoria = 'f'))";
                         }else if (in_array($login_fabrica, array(35,114,129,131,134,136,138,141,144,145,147,151,152,157,180,181,182)) || isset($usaLGR) ) {

                            $peca_obri_cond = " AND (tbl_faturamento_item.devolucao_obrig is true or tbl_peca.produto_acabado or tbl_os_item.peca_obrigatoria) ";
                        } elseif($login_fabrica == 35) {
                            $peca_obri_cond = " AND ((tbl_faturamento_item.devolucao_obrig or tbl_os_item.peca_obrigatoria) and tbl_peca.produto_acabado is not true) ";
                        } else {
                            $peca_obri_cond = " AND (tbl_os_item.peca_obrigatoria or (tbl_peca.devolucao_obrigatoria and tbl_os_item.peca_obrigatoria isnull))";
                        }
                        ...
                        if (in_array($login_fabrica, array(158,177))) {
                            $sqlLgr = "
                                ...
                                FROM tbl_extrato_lgr
                                INNER JOIN tbl_extrato ON tbl_extrato.extrato = tbl_extrato_lgr.extrato AND tbl_extrato.fabrica = {$login_fabrica}
                                WHERE tbl_extrato_lgr.posto = $login_posto AND tbl_extrato_lgr.extrato = $extrato
";
			    if($login_fabrica == 177){
				$sqlLgr .= " AND (qtde > qtde_nf or qtde_nf isnull)";
			    }
                        } else {
                            $sqlLgr = "SELECT count(*)
                                FROM tbl_faturamento_item
                                JOIN tbl_faturamento USING(faturamento)
                                JOIN tbl_peca             USING(peca)
                                LEFT JOIN tbl_os_item ON tbl_faturamento_item.pedido = tbl_os_item.pedido and tbl_faturamento_item.pedido_item = tbl_os_item.pedido_item
                ...
                if (in_array($login_fabrica, array(158,177))) {
                    $sqlLgr = "
                        ...
                        WHERE tbl_faturamento.posto = {$posto_da_fabrica}
                        AND tbl_faturamento_item.extrato_devolucao = {$extrato}
                ...
                if (pg_num_rows($resLGR)>0){
                    ...
                    if (in_array($login_fabrica, array(161))) {
                        $sqlr = "SELECT tbl_faturamento_item.faturamento_item
                                FROM tbl_faturamento_item
                                ...
                                WHERE tbl_faturamento.fabrica = {$login_fabrica} AND
                                        tbl_faturamento_item.extrato_devolucao = {$extrato} AND
                                        tbl_faturamento.posto = {$login_posto} AND
                                        (tbl_extrato_lgr.qtde - CASE WHEN tbl_extrato_lgr.qtde_nf IS NULL THEN 0 ELSE tbl_extrato_lgr.qtde_nf END) > 0 AND
                                        (tbl_faturamento.cfop ilike '59%' or tbl_faturamento.cfop ilike '69%' or tbl_faturamento_item.cfop ilike '59%' or tbl_faturamento_item.cfop ilike '69%') AND
                                        (tbl_faturamento.distribuidor IS NULL or tbl_faturamento.distribuidor = 4311 ) AND
                                        (tbl_peca.produto_acabado IS TRUE OR tbl_peca.devolucao_obrigatoria = 't') AND
                                        tbl_faturamento.emissao > '2005-10-01'";
                        ...
                        if (pg_num_rows($resr)){
                            ...
                            /*HD-4135980*/
                            $aux_sql = "SELECT qtde, qtde_nf FROM tbl_extrato_lgr WHERE extrato = $extrato AND posto = $login_posto";
                            $aux_res = pg_query($con, $aux_sql);
                            $total   = pg_num_rows($aux_res);

                            for ($yz = 0; $yz < $total; $yz++) { 
                                $aux_qtde    = (int) pg_fetch_result($aux_res, $yz, 'qtde');
                                $aux_qtde_nf = (int) pg_fetch_result($aux_res, $yz, 'qtde_nf');

                                if ($aux_qtde != $aux_qtde_nf) {
                                    $devolveu_pecas = "nao";
                                    break;
                                } else {
                                    continue;
           ...
           if (in_array($login_fabrica,array(94,128,120,201,129))) {
            ...
            if($telecontrol_distrib){
                ...
                if(strlen(trim($data_pagamento))>0){
                    $cor = "#99FF99";
                }
            ...
            if ($usaNotaFiscalServico) {
                ...
                $sqlStatusNfeServico = "SELECT obs, data
                                        FROM tbl_extrato_status 
                                        WHERE fabrica = {$login_fabrica}
                                        AND extrato = {$extrato}
                                        ORDER BY data DESC
                                        LIMIT 1";
                $resStatusNfeServico = pg_query($con, $sqlStatusNfeServico);

                $obsStatusNfe = pg_fetch_result($resStatusNfeServico, 0, 'obs');
                ...
                if (in_array('nfe_servico', $tiposInseridos) || in_array('boleto', $tiposInseridos)) {
                    $cor = "#33CCFF";
                } else if ($obsStatusNfe == 'Aguardando Envio da Nota Fiscal' || $obsStatusNfe == 'Aguardando Nota Fiscal do Posto') {
                    $cor = "#FFFF99";
                }
            ...
            if ($login_fabrica == 158) {
                if ((isset($usaLGR) AND $qtde_devolucao > 0 AND $devolveu_pecas == "nao" AND strlen($msg_mes_anterior) == 0) || (isset($fabrica_usa_lgr_ressarcimento) && $lgrRessarcimento === true) || true === $extrato_troca_produto and  ($mostra_extrato == false)) {
                    $cor = "#fff159";
                } else if( isset($usaLGR) AND strlen($msg_mes_anterior) > 0 ) {
                    $cor = "#fff159";
                }
            }
            echo "<tr class='table_line' style='background-color: $cor;'>\n";
            ...
            if ($login_fabrica == 19 || $login_fabrica == 158) {
                echo "<td align='left' style='padding-left:7px;padding-right:7px;'>";
                echo $protocolo;
                echo "</td>\n";
            }
            ...
            if($login_fabrica == 158){
                $sql_unidade = "SELECT tbl_unidade_negocio.nome AS unidade_negocio, tbl_unidade_negocio.codigo AS codigo_negocio
                                FROM tbl_extrato_agrupado 
                                JOIN tbl_unidade_negocio on (tbl_unidade_negocio.codigo = tbl_extrato_agrupado.codigo) 
                                WHERE tbl_extrato_agrupado.extrato = $extrato";

                $res_unidade = pg_query($con, $sql_unidade);

                if(pg_num_rows($res_unidade) > 0){
                    $unidade_negocio = pg_fetch_result($res_unidade,unidade_negocio);
                    $codigo_negocio = pg_fetch_result($res_unidade,codigo_negocio);
                    echo "<td align='left' nowrap>$codigo_negocio - $unidade_negocio</td>\n";
                }
            }
            if ($login_fabrica <> 11) {
            ...
            if ($login_fabrica == 153) {
                $qryDev = pg_query($con, "SELECT * FROM tbl_faturamento_item WHERE extrato_devolucao = $extrato");

                if (pg_num_rows($qryDev) == 0) {
                    $sqlExtratos = "
                        ...
                        FROM tbl_os
                            ...
                            INNER JOIN tbl_peca ON tbl_peca.referencia = tbl_produto.referencia AND tbl_peca.ativo IS TRUE
                        WHERE tbl_os.fabrica        = $login_fabrica
                            AND  tbl_os.posto          = $login_posto
                            AND tbl_os_troca.ressarcimento IS NOT TRUE
                            AND tbl_extrato.extrato <> $extrato
                            AND data_fechamento >= '2016-07-18'
                            AND data_geracao < '$data_extrato 23:59:59'
                            ";
                    $qryExtratos = pg_query($con, $sqlExtratos);

                    $extrato_bloqueado = false;

                    if (pg_num_rows($qryExtratos) > 0) {
                        $extratos_anteriores = implode(', ', pg_fetch_assoc($qryExtratos));
                        $sqlPend = "SELECT * FROM tbl_faturamento_item WHERE extrato_devolucao IN ($extratos_anteriores)";
                        $qryPend = pg_query($con, $sqlPend);

                        if (pg_num_rows($qryPend) == 0) {
                            $extrato_bloqueado = true;
                            $nao_conta_total = 'sim';
                            $msg_mes_anterior = "<a href=\"$PHP_SELF?ajax=true&extrato=$lgr_extrato&status=anterior&height=240&width=320\"  title=\"NF n„o confirmada\" class=\"thickbox\" id='bloqueado_$i'>".traduz("extrato.bloqueado",$con,$cook_idioma)."</a>";
                        }
                    }

                    if (false === $extrato_bloqueado) {
                        $sql = "SELECT DISTINCT
                                tbl_os.os,
                                tbl_os.sua_os,
                                ...
                                tbl_os_item.preco AS peca_preco,
                                tbl_admin.login
                            FROM tbl_os
                                JOIN tbl_os_extra     ON tbl_os_extra.os = tbl_os.os
                                JOIN tbl_os_troca     ON tbl_os_troca.os = tbl_os.os
                        ...
                        if (pg_num_rows($resTrocaProduto) > 0) {
            ...
            if($login_fabrica == 50 && strlen($msg_mes_anterior) > 0){
                ...
                $sql_lgr_provisorio = "SELECT extrato FROM tbl_extrato 
                                       WHERE extrato = {$extrato} 
                                       AND admin_libera_pendencia IS NOT NULL 
                                       AND data_libera_pendencia IS NOT NULL";
            ...
            if ($login_fabrica == 161) {
                $sinaliza_peca_obrigatoria = '';
                $datageracao_aux = implode('-', array_reverse(explode('/', $data_geracao)));
                $datageracao_aux = date("Y-m-d",strtotime(date("Y-m-d", strtotime($datageracao_aux)) . " +6 month"));
                if ($datageracao_aux < date('Y-m-d') ) {
                    $sinaliza_peca_obrigatoria = "class='fora_do_prazo'";
                }
            }

            if (((in_array($login_fabrica,array(6,24,35,50,81,85,90,91,94,99,106,114,115,116,117,121,123,124,125,126,127,128,129,131,134,136,138,139,140,142,141,144,145,146,147,151,152,153,157,160,161,180,181,182)) or ($login_fabrica == 51 and $extrato < '839297') or ( $mostra_extrato ) || isset($usaLGR) || $replica_einhell) AND $qtde_devolucao > 0 AND $devolveu_pecas == "nao" AND strlen($msg_mes_anterior) == 0) || (isset($fabrica_usa_lgr_ressarcimento) && $lgrRessarcimento === true) || true === $extrato_troca_produto and  ($mostra_extrato == false))   {
                ...
                if ($login_fabrica == 158) {
                    ...
                    echo "<td align='right'>".number_format($xvrmao_obra,2,",",".")."</td>";
                    echo "<td align='right'>".number_format($valor_adicional,2,",",".")."</td>";
                    echo "<td align='right'>".number_format($deslocamento_km,2,",",".")."</td>";
                    echo "<td align='right'>".number_format($avulso,2,",",".")."</td>";
                ...
                if(($login_fabrica != 158 && $inf_valores_adicionais) || $login_fabrica == 145){
                    echo "<td></td>";
                }
                ...
                if ($login_fabrica == 158) {
                    ...
                    $xcolspan = " colspan='4'";
                    $texto_nf = '<img src="imagens/btn_pecasretornaveis_amarelo.gif">';
                ...
                $colspan = (in_array($login_fabrica, array(24,142,145,146,157))) ? "colspan='4'" : "";
                if($login_fabrica == 158){
                    echo "<td><a href='os_extrato_detalhe.php?extrato=$extrato&posto=$posto'><img src='imagens/btn_detalhar_amarelo.gif'></a></td>\n";
                }

		if(!in_array($login_fabrica,[177])){
			$texto_title = "Clique aqui para preencher a nota fiscal de devoluÁ„o. ApÛs a devoluÁ„o da NF, poder„o ser visualizado a M„o de Obra";
		}else{
			$texto_title = "Clique aqui para preencher a nota fiscal de devoluÁ„o.";
		}
                echo "<td align='center' $colspan $xcolspan $sinaliza_peca_obrigatoria>
                <a href='extrato_posto_devolucao_lgr_novo_lgr.php?extrato=$extrato' title='$texto_title' id='preencha' rel='$i'>$texto_nf</a></td>\n";
                if ($login_fabrica == 161) {
                    echo "<td align='center' $sinaliza_peca_obrigatoria colspan='3'><a href='extrato_posto_devolucao_lgr_novo_lgr.php?extrato={$extrato}&listagem=true'><img src='imagens/btn_pecasretornaveis_azul.gif'></a></td>";
                }
            } else if ((in_array($login_fabrica,array(2,6,7,11,25,50,81,85,90,91,94,99,106,114,115,116,117,121,125,153,160,161)) or $replica_einhell or ($login_fabrica == 51 and $extrato < '839297') or ($login_fabrica == 24 and $novo_processo_lgr == 1)) AND count($notas_array) > 0 AND strlen($admin_lgr) == 0) {
            ...
            } else if ((in_array($login_fabrica,array(2,6,7,11,25,35,43,50,51,81,85,90,91,94,99,106,114,115,116,117,120,201,121,125,131,138,139,141,142,144,145,146,147,151,152,153,157,160,161,180,181,182)) || isset($usaLGR)  || $replica_einhell or ($login_fabrica == 24 and $novo_processo_lgr == 1)) AND strlen($msg_mes_anterior)>0){
                if ($login_fabrica == 161) {
                    echo "<td align='center' colspan='7'";
                    echo (!empty($sinaliza_peca_obrigatoria)) ? '' : "bgcolor='#FF9E5E'";
                    echo ">\n";
                    echo $msg_mes_anterior;
                    echo "</td>";
                    echo "<td align='center' $sinaliza_peca_obrigatoria colspan='3'><a href='extrato_posto_devolucao_lgr_novo_lgr.php?extrato={$extrato}&listagem=true'><img src='imagens/btn_pecasretornaveis_azul.gif'></a></td>";
                }elseif (in_array($login_fabrica, [158])) {
                    $nao_calcula_km = \Posvenda\Regras::get("nao_calcula_km", "mao_de_obra", $login_fabrica);
                    ...
                    echo "<td align='center' colspan='4' $sinaliza_peca_obrigatoria>
                    <a href='os_extrato_detalhe.php?extrato=$extrato&posto=$posto'><img src='imagens/btn_detalhar_amarelo.gif'></a>
                    ...
                    $xxcolSpan = (in_array($login_fabrica, [193])) ? 8 : 7;
                    echo "<td align='center' colspan='$xxcolSpan' bgcolor='#FF9E5E'>\n";
                    echo $msg_mes_anterior;
                    echo "</td>";
                }
            } else {
                ...
                if ($aguardando_nf == 't') {
                    ...
                    if ((!in_array($login_fabrica,array(94,99,106,121,125,126,128,129,131,134,136,137,140,139)) || isset($usaLGR)) && $login_fabrica != 193) {
                        if (isset($novaTelaOs)) {
                            $nao_calcula_peca = \Posvenda\Regras::get("nao_calcula_peca", "mao_de_obra", $login_fabrica);
                        }

                        // Fabricas que n„o pagam a peÁa
                        if(!isset($novaTelaOs) || (isset($novaTelaOs) && !$nao_calcula_peca)){
                            echo "<td align='right'  style='padding-right:3px;' nowrap> ";
                                if ($login_fabrica == 6) {
                                    if (strlen($liberado) > 0) {
                    ...
                    if(in_array($login_fabrica, array(140, 145)) or isset($fabrica_usa_valor_adicional)){
                    ...
                    if(isset($novaTelaOs) && !$nao_calcula_km){
                        $deslocamento_km = ($deslocamento_km) ? $deslocamento_km : 0;
			$textoDeslocamento = ($login_fabrica == 183) ? "<a href='gerar_nota_debito.php?extrato={$extrato}' target='_blank'>".number_format($deslocamento_km,2,",",".")."</a>" : number_format($deslocamento_km,2,",","."); 
                        echo "<td align='right'  style='padding-right:3px;' nowrap>";
                            echo $textoDeslocamento;
                        echo "</td>\n";
                        $total += $deslocamento_km;
                    }
                    ...
                    if($inf_valores_adicionais and !isset($novaTelaOs)){
                        $valor_adicional = ($valor_adicional) ? $valor_adicional : 0;
                        $total += $valor_adicional;
                        echo "<td align='right'  style='padding-right:3px;' nowrap>";
                            echo number_format($valor_adicional,2,",",".");
                    ...
                    if (!in_array($login_fabrica,array(51,90,128))) {
                        if (in_array($login_fabrica,[81,122,125,160]) or $replica_einhell) {
                        ...
                        if (in_array($login_fabrica, array(120,201,129))) {
                            ...
                            echo "<td><strong>".number_format($total + $avulso,2,",",".")."</strong></td>";
                             ...
                             if ($login_fabrica <> 45 && !isset($novaTelaOs) && (!in_array($login_fabrica,array(50,30,85,91,94,120,201,125,129,131,138,140))) ) {
                                ...
                                if ($login_fabrica == 6) {
                                     ...
                                     echo ($login_fabrica==11)
                                        ? number_format($avulso,2,",",".")
                                        : ($login_fabrica == 85)
                                            ? number_format($total_extrato,2,',','.')
                                            : ($login_fabrica == 117)
                                                ?   number_format($avulso,2,",",".")
                                                    :
                            ...
                            if ( ($login_fabrica != 151) && (!in_array($login_fabrica,array(11,15,45,50,81,114,115,116,117,120,201,121,122,123,124,125,126,127,128,129,131,134,136,137,139,140,141,142,144,151,153,157,160,161,177)) and !$replica_einhell) ){
                                ...
                                if ($login_fabrica == 6) {
                                    ...
                                    if ($login_fabrica == 85) {
                                        $sqlAvulso = "
                                            SELECT  SUM (tbl_extrato_lancamento.valor) AS total_avulso
                                            FROM    tbl_extrato_lancamento
                                            WHERE   extrato = $extrato
                                            AND     fabrica = $login_fabrica
                                        ...
                                        $total_avulso = pg_fetch_result($resAvulso,0,total_avulso);
                                    }
                                    echo ($login_fabrica==11)
                                        ? number_format($total+$avulso,2,",",".")
                                            : ($login_fabrica == 85)
                                                ? number_format($total_avulso,2,",",".")
                                                : number_format($avulso,2,",",".");
                                }
                            ...
                            if ($login_fabrica == 117) {
                                echo "<td align='right' style='padding-right:3px;' nowrap>";
                                echo number_format($total+$avulso,2,",",".");
                                echo "</td>\n";
                            }
                    ...
                    echo "</td>\n";
                     ...
                     if ($login_fabrica == 153) { //HD-2922574
                        echo "<td align='right'  style='padding-right:3px;' nowrap> ";
                        echo number_format($total_extrato, 2, ",", ".");
                        echo "</td>\n";
                    }    

                    if (in_array($login_fabrica, array(30,50,85,91,94,125,131,140)) && !isset($novaTelaOs)){
                        echo "<td align='right'  style='padding-right:3px;' nowrap> ";
                        if(in_array($login_fabrica,array(85,91,125))){
                    ...
                    if (!in_array($login_fabrica,array(51,128))) { 
                        if (!in_array($login_fabrica, array(120,201,129))) {
                            if ( $login_fabrica <> 45 && !isset($novaTelaOs) && in_array($login_fabrica,array(50,30,85,91,94,120,201,125,129,131,138,140)) ) {
                                echo "<td align='right'  style='padding-right:3px;' nowrap> ";
                                if ($login_fabrica == 6) {
                    ...
                    if (isset($novaTelaOs) AND $login_fabrica <> 153 AND $login_fabrica != 177) {
        ...
        if(in_array($login_fabrica, array(190))){
            ...
            $total_extrato = ($total_extrato+$valor_mao_obra_fixa);
                    ...
                    if ($login_fabrica != 99) {//HD 416354
                        ...
                        if(in_array($login_fabrica,array(11,15,24,45,50,81,91,104,114,115,116,117,120,201,121,123,124,125,126,127,128,129,131,134,136,137,138,139,140,141,144,145,146,151,152,153,157,160,180,181,182)) || $replica_einhell) {
                            echo $data_pagamento;
                        } elseif ($login_fabrica==20) {
                            echo $previsao_pagamento;
                        } elseif ($login_fabrica==101) {
                            echo $previsao_pagamento_extrato;
                        } else {
                            if($login_fabrica<>11){
                                    echo $previsao;
                            }
               ...
               if(in_array($login_fabrica, array(101))){
                    ...
                    if(strlen($nf_autorizacao) == 0){
                        $nf_autorizacao = "<a href='nota_servico_extrato.php?extrato=$extrato' rel='shadowbox; width= 550; height= 350;'>Informar Nota de ServiÁo</a>";
                    }else{
                        ...
                        if ($sistema_lingua == 'ES') {
                            ...
                            if ($login_fabrica <> 15) {
                                ...
                                if ($login_fabrica == 11) {
                                    ...
                                    if (in_array($login_fabrica,array(6,35,50,51,85,90,91,94,99,106,114,115,116,117,120,201,121,123,125,126,127,128,129,131,134,136,137,138,139,140,141,142,144,145,146,147,151,152,153,157,160,161,180,181,182)) or ($login_fabrica == 24 AND $novo_processo_lgr == 1) || isset($usaLGR) || $replica_einhell) {
                                        ...
                                        if ($login_fabrica == 51) {
                                            ...
                                            if($login_fabrica == 35 && $data_extrato < '2014-05-01'){
                                                ...
                                                if (((in_array($login_fabrica,array(120,201)) || isset($usaLGR)) AND $qtde_devolucao > 0 AND $devolveu_pecas == "nao" AND strlen($msg_mes_anterior) == 0) || (isset($fabrica_usa_lgr_ressarcimento) && $lgrRessarcimento === true) || true === $extrato_troca_produto) {

                                                    $link_extrato = "<a href='extrato_posto_devolucao_lgr_novo_lgr.php?extrato=$extrato' title='Clique aqui para preencher a nota fiscal de devoluÁ„o. ApÛs a devoluÁ„o da NF, poder„o ser visualizado a M„o de Obra' id='preencha' rel='$i'>Preencha a NF<br> do LGR<br> Clique Aqui</a>";
                                                } else{
                                                    $link_extrato = "<a href='extrato_posto_lgr_itens_novo.php?extrato=$extrato'><img src='imagens/btn_pecasretornaveis_".$btn.".gif'></a>";
                                                }
                                            }elseif($login_fabrica == 50){
                                                if($data_extrato < '2017-04-01'){
                                                    $sql_d = "SELECT *
                                                                FROM tbl_faturamento f
                                                                    INNER JOIN tbl_faturamento_item tfi ON f.faturamento = tfi.faturamento
                                                                    LEFT JOIN tbl_faturamento tf ON f.extrato_devolucao = tf.extrato_devolucao AND f.posto = tf.distribuidor
                                                                WHERE f.extrato_devolucao =  {$extrato}
                                                                    AND tfi.extrato_devolucao IS NULL
                                                                    AND tf.faturamento IS NULL;";
