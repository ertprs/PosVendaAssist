...
$data_string = "fabrica=14&cnpj=11111111000122&dv=1&sua_os=72779&sequencial=&consumidor_revenda=C&data_abertura=2010-09-27&data_fechamento=&produto_referencia=4390519&serie=PI10033000085&cpf_cnpj=28361566000168&consumidor_nome=FONTEL T ELEFONIA E TELECOMUNICAÇÕES LTDA&consumidor_fone=21 2122-7878&consumidor_cidade=RIO DE JANEIRO&consumidor_estado=RJ&revenda_cnpj=03488058000114&revenda_nome=MAXCOM &revenda_fone=21 2524-2461&nota_fiscal=5901&data_nf=16/07/2010&defeito_reclamado=3745&defeito_constatado=11050&causa_defeito=161&peca_referencia=1024535&qtde=1&defeito=164&servico_realizado=82&voltagem=&codigo_fabricacao=&type=&tipo_atendimento=&segmento_atuacao=&promotor_treinamento=&sua_os_offline=72779&satisfacao=&laudo=&subproduto=168885&posicao=CI3&solucao_os=82&btn_acao=gravar";

$teste = explode("&", $data_string);
$array_post = array();

foreach($teste as $indice => $variavel) {
	$variavel = explode("=", $variavel);
	$array_post[$variavel[0]] = $variavel[1];
}
...
function fnc_pesquisa_peca_lista_intel (produto_referencia,	peca_referencia, peca_descricao, peca_posicao, tipo) {
	...
	if (tipo ==	"posicao") {
		url	= "peca_pesquisa_lista.php?produto=" + produto_referencia +	"&posicao="	+ peca_posicao.value + "&tipo="	+ tipo;
	}
	if (peca_referencia.value.length >=	4 || peca_descricao.value.length >=	4 || peca_posicao.value.length >=0)	{
		janela = window.open(url, "janela",	"toolbar=no, location=yes, status=yes, scrollbars=yes, directories=no, width=501, height=400, top=18, left=0");
		janela.produto		= produto_referencia;
		janela.referencia	= peca_referencia;
		janela.descricao	= peca_descricao;
...
function montaCombo3(campo,result){
	...
	} else { campo.innerHTML = "Selecione o	subconjunto";//caso	o XML volte	vazio, printa a	mensagem abaixo
	}
}
...
function verificaPedido	(servico) {
	var	servico	= servico;
	if (servico	!= 82 && servico !=	83)	{
		document.getElementById('nao_pedido').checked =	true;
	}
	else {
		document.getElementById('nao_pedido').checked =	false;
	}
}

function verificaPedido2 (check,linha) {
	if (check.checked){
		document.getElementById('76').selected = true;
	}
}
	...
	<td	valign="top" align="left">
		...
		<tr>
			...
			<td>
				...
				&nbsp;
				<img src='imagens/btn_lupa_novo.gif' border='0' align='absmiddle'	style="cursor:pointer" onclick="javascript:	fnc_pesquisa_produto2 (document.frm_os.produto_referencia,document.frm_os.produto_descricao,'referencia',document.frm_os.produto_voltagem)"	/>
				&nbsp;<?php
...
<?			} else {?>
				...
				<td	id='nf_table_item'>
					...
					<img src='imagens/delete_2.gif'	id='excluir_nota' style='cursor:pointer'
						file='<?=$os?>'	title='Excluir imagem anexada'>
				</td>
<?			}?>
			</td>
		</tr>
...
for	($i	= 0	; $i < $loop ; $i++) {
	...
	if(strlen($os)==0){
		echo "<select name='produto_$i'	id='produto_$i'	class='frm'	style='width:220px;' onFocus='listaSubconjunto(this,document.frm_os.produto_referencia.value);'	onChange='javascript: document.frm_os.posicao_$i.focus(); fnc_pesquisa_peca_lista_sub (document.frm_os.produto_$i.value, document.frm_os.posicao_$i, document.frm_os.peca_$i, document.frm_os.descricao_$i)'>";
		echo "<option id='opcoes3' value=''></option>";
		...
		echo "<select class='frm' size='1' name='produto_$i' onChange='javascript: document.frm_os.posicao_$i.focus(); fnc_pesquisa_peca_lista_sub (document.frm_os.produto_$i.value, document.frm_os.posicao_$i, document.frm_os.peca_$i, document.frm_os.descricao_$i)'>";
		$sql = "SELECT	tbl_produto.produto	  ,
						tbl_produto.referencia,
						tbl_produto.descricao
				FROM	tbl_subproduto
	...
	if ($os_item_subconjunto ==	'f') {
		...
		if ($os_item_serie == 't') {
			echo "<td align='center'><input	class='frm'	type='text'	name='serie_$' size='9'	value='$serie[$i]' maxlength='20'></td>\n";
		}
	}
	echo "<td align='center' nowrap><input class='frm' type='text' name='posicao_$i' size='6' maxlength='6'	value='$posicao[$i]'>";
	echo "<img src='imagens/btn_lupa_novo.gif' border='0' align='absmiddle'	onclick='javascript: fnc_pesquisa_peca_lista_intel (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i ,	document.frm_os.posicao_$i , \"posicao\")' alt='Clique para	efetuar	a pesquisa'	style='cursor:pointer;'>";
	echo "</td>\n";
	echo "<td align='center' nowrap><input class='frm' type='text' name='peca_$i' size='15'	value='$peca[$i]'>&nbsp;";
	echo "<img src='imagens/btn_lupa_novo.gif' border='0' align='absmiddle'	onclick='javascript: fnc_pesquisa_peca_lista_intel (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i ,	document.frm_os.posicao_$i , \"referencia\")' alt='Clique para efetuar a pesquisa' style='cursor:pointer;'>";
	echo "</td>";

	echo "<td align='center' nowrap><input class='frm' type='text' name='descricao_$i' size='25' value='$descricao[$i]'>&nbsp;";
	echo "<img src='imagens/btn_lupa_novo.gif' border='0' align='absmiddle'	onclick='javascript: fnc_pesquisa_peca_lista_intel (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i ,	document.frm_os.posicao_$i , \"descricao\")' alt='Clique para efetuar a	pesquisa' style='cursor:pointer;'>";
	echo "</td>";
	...
	//--===== Serviço Realizado	=======================================================================
	echo "<td align='center'>";
	echo "<select class='frm' size='1' name='servico_$i' style='width:150px' onchange='verificaPedido(this.value);'>";
	echo "<option selected></option>";
	...
	echo "<td align='center'> <input type='checkbox' value='t' name='nao_pedido' id='nao_pedido' class='frm' onchange='verificaPedido2(this,$i);'>";
	echo "</td>";
