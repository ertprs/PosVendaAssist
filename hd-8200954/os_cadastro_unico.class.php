...
class grupo {
	...
	private $jQuery = array(); // Todo o código jQuery deveria estar num só $.ready()
	...
	function add_field($tabela, $campo, $id="") {
		...
		if(extract($this->dados_campos[$tabela][$campo]))// or die (print_r($this->dados_campos[$tabela][$campo]));
    		$tipo = $tipo == "" ? $tipo = "texto" : $tipo = $tipo;
		else{
			//print_r($this->dados_campos[$tabela][$campo]);    
			echo "Ausência do INPUT: ".$id;
		}
		...
		switch($tipo_dados) {
			...
			case 'float':
				$this->jQuery[] = "$('#{$id}').numeric({ decimal: ',', negative: false });";
			break;
		}
		
		if (strlen($mascara) > 0) {
			$this->jQuery[] = "$( '#{$id}' ).maskedinput('$mascara');";
		}
...
class input_button extends input {
	...
	function draw() {
		...
		echo "<div id='div_{$this->id}' class='div_campo $required $div_classes'>";
			echo "<input type='button' id='{$this->id}' name='{$this->name}' class='$input_classes' $maxlength $size {$this->attr} value='{$this->value}' />";
		echo "</div>";
	}
}

class img extends input {
	function __construct($id, $label='', $value='', $tipo='') {
		parent::__construct($id, $label);
		$this->value = $value;
		$this->tipo = $tipo;
	}
	...
	function draw() {
		...
		echo "<div id='div_{$this->id}' class='div_campo $required $div_classes'>";
			echo "<img src='{$this->tipo_imagem($this->tipo)}' alt='{$this->value}' class='cursor_pointer $input_classes' {$this->attr} />";
		echo "</div>";
	}
	...
	function tipo_imagem($tipo){
		...
		switch($tipo) {
			case 'lista_basica': $src = "imagens/btn_lista.gif"; break;
			default : $src = $tipo;
		}
...
class input_text extends input {
	...
	function draw() {
		...
		$div_classes   = $this->get_classes_string("div");
		$input_classes = $this->get_classes_string("input");
		$label         = strlen($this->label) > 0 ? $label = "<label class='$required' for='{$this->id}'>{$this->label}</label>" : "";
		$read_only     = $this->read_only ? "readonly='readonly'" : "";
		...
		echo "{$label}<input type='text' id='{$this->id}' name='{$this->name}' value='{$this->value}' class='$input_classes' $maxlength $size $read_only {$this->attr} /><label class='ajuda'>{$this->ajuda}</label>";
		echo $this->html_sufix;
		echo $autocomplete;
		echo "</div>";
	}
...
function valida_campo_autocomplete($campo, $label='') {
	...
	if(in_array($login_fabrica, array(20))){
		if ((strlen($valor) > 0 && (strlen($valor_id) == 0 || strlen($valor_last) == 0)))
			return "No campo {$label} ($valor) ($valor_id) ($valor_last) você deve começar a digitar um valor e selecionar um item da lista";
	}else{
