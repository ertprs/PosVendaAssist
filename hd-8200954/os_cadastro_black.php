...
// fabio 19/11/2007 - Verifica se está em Intervenção
...
/* MLG HD 175044    */
/*  11/01/2011 - Adicionando validação de nº de série */
...
if ($btn_acao == "continuar") {
	...
	if ($login_fabrica == 1) {
		...
		if($login_posto == 5089){ // liberados pela Fabiola em 20/03/2006
			...
			if ($sdata_abertura < 20050901){
				$msg_erro = "Erro. Data de abertura inferior a 01/09/2005.<br>OS deve ser lançada no sistema antigo até 30/09. <br>";
	...
	if (strlen($consumidor_cpf)>0) {
		...
		if (!is_numeric($validaCPF)) {
			$msg_erro .= ($xfisica_juridica == "'F'") ? 'CPF inválido' : 'CNPJ inválido';
			$consumidor_cpf = 'null';
			$validaCPF = false;
		} else {
			$xconsumidor_cpf = "'$validaCPF'"; // A função devolve o CPF ou CNPJ 'limpo'
			$validaCPF = true;
		}
	} else {
		$validaCPF = null;
	...
	if (!empty($xconsumidor_celular)) {
        ...
        if(filter_input(INPUT_POST,"consumidor_email",FILTER_VALIDATE_EMAIL,FILTER_FLAG_EMAIL_UNICODE)){
            ...
            $msg_erro .= "<br>E-mail de contato obrigatório.<br>Caso não possuir endereço eletrônico, deverá ser informado o e-mail: 'nt@nt.com.br'";
	...
	$consumidor_profissao = retira_acentos($consumidor_profissao);
		...
		if ($login_fabrica == 1 AND $xconsumidor_revenda<>"'R'") {
			...
			if (strlen($xconsumidor_numero) == 0) {
				$msg_erro .= "<br> Digite o número do consumidor (endereço). <br>";
...
//=====================revenda
    ...
    if ($garantia_pecas && !empty($data_abertura) && !empty($data_nf)) {
        ...
        if ($dataHoje > $dataAberturaTeste) {
			...
			$msg_erro .= "A data de abertura dessa ordem de serviço poderá ser no máximo 7 dias anterior à data de digitação.<br>";
            $campos_erro[] = "Data Abertura";
        }
        if ($dataHoje2 > $dataNfTeste) {
			$msg_erro .= "Peças com mais de 90 dias de compra não serão aceitas.<br>";
            $campos_erro[] = "Data Abertura";
        }
...
//pedido por Leandrot tectoy, feito por takashi 04/08
	...
	if($xdata_nf > $xdata_abertura and $xdata_nf <> 'null' and $xtroca_faturada <> 't') $msg_erro .= "A data da nota não pode ser maior que a data de abertura";
...
/*A satisfação 30 dias é gerada somente para consumidores. Em nenhuma hipótese poderá ser permitido a digitação do laudo técnico para OS's em que foi selecionada a opção revenda.*/
	...
	if($xconsumidor_revenda=="'R'" and $xsatisfacao=="'t'"){
		$msg_erro .= "Ordem de Serviço de Revenda não pode ser 30 dias Satisfação DeWALT/Porter Cable.<Br>";
	}
		...
		if (strlen($produto_referencia) > 0 AND (strlen($xproduto_serie) == 0 OR $xproduto_serie == 'null')) {
			...
			if (pg_num_rows($res) > 0) {
				...
				if ($numero_serie_obrigatorio == 't') {
					$msg_erro .= "<br>Nº de Série $produto_referencia é obrigatório.<br />";
				}
			}
		}
	}
		...
		if (strlen($produto_serie) > 0) {
			...
			if (pg_num_rows($res) > 0) {
				...
				if (strlen($xxxextrato) == 0) {
					...
					Favor reabrir a ordem de serviço $xxxsua_os e acrescentar itens.<br />";
				}else{
					$os_reincidente = "'t'";
				}
			}
...
#	if ($login_fabrica == 1 AND strlen($consumidor_cpf) == 0) $msg_erro .= 'Tamanho do CPF/CNPJ do cliente inválido';
	...
	if (@pg_num_rows ($res) == 0) {
		$msg_erro .= " Produto ".($produto_referencia=='null' ? '' : $produto_referencia )." não cadastrado ou voltagem incorreta.<br />";
	}else{
...
//===============================REVENDAAAA
                    ...
                    if (!empty($file["size"])) {
                        ...
                        if ($obriga_anexo) {
                        	if (!temNF($os,'bool') and ($filesByImageUploader == 0)) {
	                            $tmp_erro = 'Anexo de NF obrigatório X';
	                            $campos_erro[] = "anexo";
	                        }
                        }
                    }
			...
			/*if ( strlen($msg_erro) == 0 && $_POST['satisfacao'] == 't') {
				...
				if ($_FILES['foto_laudo_tecnico']['size'] == 0 && $obriga_anexo) {
					...
					$msg_erro = 'Para OS em satisfação de 90 dias é necessário anexar o comprovante de Laudo Técnico';
					excluirNF($os);
...
<script type="text/javascript">
	...
	/* Mensagem de atenção para a Black&Decker !! */
	var text_atencao = '<p class=atencao><b>Atenção:</b> A revenda informada nessa O.S. é a própria B&D e nesse caso é necessário seguir as orientações abaixo:</p>'+
		'<p><b>1</b> - Se o produto informado for de estoque de revenda deverá ser digitado na OS de revenda. Portanto, não conclua esse cadastro e comece um novo cadastro no link Abertura de OS de revenda</p>'+
		'<p><b>2</b> - Se o produto for de locadora verificar se está no prazo de 6 meses de garantia, pois não cobrimos a garantia após esse prazo. As OS´s de locadora digitadas com produtos com mais de 6 meses de compra serão canceladas. Lembrando que, os produtos de locadoras são carimbados na fábrica com a especificação locadora e, portanto precisam estar com esse carimbo.</p>'+
		'<p><b>3</b> - Se houver uma outra situação diferente das mencionadas no item 1 e 2 gentileza informar o que houve na observação da ordem de serviço.</p>';
	...
	/* Função que verifica a Revenda para mostrar uma mensagem - HD 7849 - Fabio Nowaki*/
	...
	/* Função para gravar a OS - Faz a verificação da Revenda para mostrar uma mensagem - HD 7849 - Fabio Nowaki*/
	function gravarOS(){
...
?>
        <tr>
            <td colspan="100%" align="center">
                ...
                <span style="font-family:Geneva, Arial, Helvetica, san-serif;">Devolução de Peças (90 dias de garantia)</span>
            </td>
        </tr>
        <tr>
            <td colspan="100%" align="center">
                <span style="font-family:Geneva, Arial, Helvetica, san-serif;font-size:14px;color:#F00;font-weight:bold;">
                    Selecione essa opção apenas para realizar a devolução das peças <br />que apresentaram problema de fabricação dentro dos 90 dias após a venda.
                </span>
            </td>
        </tr>
<?php
...
onfocus="this.className='frm';" onblur="this.className='frm'; displayText('&nbsp;'); checarComunicadoProduto();" onfocus="this.className='frm-on'; displayText('&nbsp;Entre com a Data da Abertura da OS.'); " tabindex="0" ><br><font face='arial' size='1'>Ex.: <? echo date("d/m/Y"); ?></font>
			...
			<? if ($login_fabrica <> 6){ ?>
				...
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">N. Série</font>
				<br>
				<input class="frm" type="text" name="produto_serie" size="8" maxlength="20" value="<? echo $produto_serie ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Digite aqui o número de série do aparelho.'); "><br><font face='arial' size='1'><? if ($login_fabrica == 1) echo "(somente p/ linha DeWalt/Porter Cable)"; ?></font>
			</td>
			<? } ?>
		</tr>
		</table>
...
<? if ($login_fabrica == 1) { ?>
		...
		<tr valign='top'>
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif" class='label_obrigatoria'>Código Fabricação</font>
				<br>
...
?>
			...
			<!-- <td nowrap><?// HD15589?>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">90 dias Satisfação DeWALT/Porter Cable</font>
				<br>
		...
		<br>
		<input class="<?php echo (in_array('Nota Fiscal', $campos_erro))? 'frm_obrigatorio' : "frm-on" ?>" type="text" name="nota_fiscal"  size="10"  id="nota_fiscal" maxlength="20"  value="<? echo $nota_fiscal ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Entre com o número da Nota Fiscal.');" <? if (strlen($locacao) > 0) echo "readonly"; ?>>
		</td>
	...
	//	$sql = "SELECT * FROM tbl_tipo_atendimento WHERE fabrica = 19 ORDER BY tipo_atendimento";
		...
		<tr>
			...
			<td>
				...
				<select class='<?php echo (in_array('consumidor_tipo', $campos_erro))? 'frm_obrigatorio' : "frm" ?>' onfocus="this.className='frm'"; name="fisica_juridica" id='tipo_consumidor'>
					<option></option>
					<OPTION VALUE="F" <?echo ($fisica_juridica == 'F')? " SELECTED ": "";?>>Pessoa Física</OPTION>
					<OPTION VALUE="J" <?echo ($fisica_juridica == 'J')? " SELECTED ": "";?>>Pessoa Jurídica</OPTION>
				</select>
			</td>
			<td align="center">
		...
		<td align='left' nowrap>
			<font size="1" face="Geneva, Arial, Helvetica, san-serif" class='label_obrigatoria'>Endereço</font>
			<BR>
			<input class="<?php echo (in_array('endereco_consumidor', $campos_erro))? 'frm_obrigatorio' : "frm" ?> address" type="text" name="consumidor_endereco"   size="25" maxlength="60" value="<? echo $consumidor_endereco ?>" onblur="this.className='frm address'; displayText('&nbsp;');" onfocus="this.className='address frm-on'; displayText('&nbsp;Digite o endereço do consumidor.');">
		</td>

		<td nowrap>
			<font size="1" face="Geneva, Arial, Helvetica, san-serif" class='label_obrigatoria'>Número</font>
			<BR>
			<input class="<?php echo (in_array('numero_consumidor', $campos_erro))? 'frm_obrigatorio' : "frm" ?>" type="text" name="consumidor_numero"   size="5" maxlength="10" value="<? echo $consumidor_numero ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Digite o número do endereço do consumidor.');">
		</td>
		</tr>
		<tr>
			<td nowrap>
				...
				<BR>
				<input class="frm" type="text" name="consumidor_complemento"   size="10" maxlength="20" value="<? echo $consumidor_complemento ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Digite o complemento do endereço do consumidor.');">
			</td>
		...
		<td>
			...
			<br>
			<input class="frm address_rev" type="text" name="revenda_endereco" size="30" maxlength="60" value="<? echo $revenda_endereco ?>" onblur="this.className='frm address_rev'; displayText('&nbsp;');" onfocus="this.className='frm-on address_rev'; displayText('&nbsp;Digite o endereço da Revenda.');">
		</td>
		<td>
...
#			echo " -->";
		...
		<tr>
			...
			<td>
				...
				<? if ($login_fabrica == 20) {
					...
					if($login_fabrica==11){
						...
						echo "<input class='$class_aparencia' type='text' name='aparencia_produto' size='30' value='$aparencia_produto' onblur=\"this.className='frm'; displayText('&nbsp;');\" onfocus=\"this.className='frm-on'; displayText('&nbsp;Texto livre com a aparência externa do aparelho deixado no balcão.');\">";
					}
				}
				?>
			...
			<td id="anexaLaudo" style="display:none;">
				<font size='1' face='Geneva, Arial, Helvetica, san-serif'>Anexar Laudo Técnico</font><br />
...
<tr class="top">
	<td  class="top"><font size='1'>Informações sobre a Troca Faturada</font></td>
</tr>
<tr class="top">
...
<tr>
	...
	<td height="27" valign="middle" align="center" bgcolor="#FFFFFF">
		...
		if ($login_fabrica == 1) { ?>
			<img src='imagens/btn_continuar.gif' name="sem_submit" onclick="javascript: gravarOS()" class="verifica_servidor"  alt="Continuar com Ordem de Serviço" border='0' style='cursor:pointer;'>
		<? }else { ?>
