...
$peca			= $cod_produto;
	...
	if ($num_fotos>0){
		...
		for ($i=0; $i<$num_fotos; $i++){
			...
			?>
			<li>
			<a href="javascript:abrir('lv_detalhe_popup.php?caminho_final=<?echo $caminho_dir."/".$caminho; ?>&peca=<? echo $peca; ?>&caminho=<? echo $caminho_popup; ?>&descricao=<? echo $descricao; ?>&referencia=<? echo $referencia; ?>');" title="<?=$descricao;?>" style='text-decoration: none;' >
			<img src="<?echo $caminho_popup . "\" " . $mlg_medidasimg; ?> alt="" />
			...
			while (false !== ($filename = readdir($dh))) {
				...
				if (strpos($filename,$Xreferencia) !== false){
					...
					if(substr($filename, 0,$po)==$Xreferencia){
						$file_final = $filename;
						echo "<a href=\"javascript:abrir('lv_detalhe_popup.php?caminho_final=$diretorio/media/$file_final&peca=$peca&caminho=$diretorio/media/$file_final&descricao=$descricao&referencia=$referencia');\" title=''><img src='$diretorio/pequena/$file_final'/></a>";
...
echo "<td valign='top' width='80%'><BR>\n";
	...
	if($login_fabrica==3 AND strlen($produto_acabado)>0 AND $preco_anterior>0){
		$preco = $preco_anterior;
	}

	echo "<input type='text' name='valor'       value='$preco'>\n";
...
if (strlen($qtde_max_site)==0){
	...
	if($login_fabrica==3 AND strlen($produto_acabado)>0){ //HD 98211
...
/*echo "<BR>".pg_numrows($res);*/
		for ($x = 0 ; $x < pg_numrows($res); $x++){
			...
			echo "\n<div rel='box_content' class=\"content_box\"><form action='lv_carrinho.php?acao=adicionar' method='post' name='frmcarrinho_$x'>\n";
				if( $preco > 0){
				...
				if(strlen($filename_final)>0){
					list($width, $height) = getimagesize("$diretorio/pequena/$filename_final");
					...
					echo "<center><div style='height: ".($tamanho + 10)."px;
						  ...
						  <img src='$diretorio/pequena/$filename_final' border='0' $limita >\n";
