...
if(empty($referencia)) $referencia = $_POST['codigo'];
...
if (strlen($xreferencia) > 0) {
    ...
    if ($item_aparencia == 'f') {
        $cond_item_aparencia = 'AND tbl_peca.item_aparencia IS FALSE';
    }
    ...
    $sql = "
        ...
        AND UPPER(TRIM(tbl_peca.referencia_pesquisa)) = UPPER(TRIM('{$xreferencia}')) 
        $cond_item_aparencia
        ";
    $res = pg_exec ($con,$sql);
        ...
        for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
            ...
            if(pg_num_rows($res) == 1){
                if (!empty($peca_fora_linha)) {
                    ...
                    echo "alert('Peça $peca_referencia é obsoleta, não é mais fornecida.'); window.parent.Shadowbox.close();";
                    echo "</script>";
                    exit;
                }
                echo "<script type='text/javascript'>";
