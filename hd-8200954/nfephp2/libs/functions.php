...
/**
 * calculaChaveAcesso
 * Cria uma dígito verificador para Nota Fiscal 2.0
 *
 * @exemple
	...
	(02) Código da UF     = 43
	(04) AAMM da emissão  = 1005
	(14) CNPJ do Emitente = 06880590000170
	(02) Modelo           = 55
	(03) Série            = 000
	(09) Número da NF-e   = 000000184
	(09) Tipo de emissão  = 1
	(09) Código Numérico  = 787277221
	(01) DV               = 5
	calculaDV: 5
...
function calculaChaveAcesso($cUF, $dEmi, $CNPJ, $mod, $serie, $nNF, $tipo, $cNF) {
	...
	$chave  = sprintf("%02d", $cUF);   // 02 - cUF   - código da UF do emitente do Documento Fiscal
	$chave .= sprintf("%04d", $dEmi);  // 04 - AAMM  - Ano e Mes de emissão da NF-e
	$chave .= sprintf("%014s", $CNPJ); // 14 - CNPJ  - CNPJ do emitente
	$chave .= sprintf("%02d", $mod);   // 02 - mod   - Modelo do Documento Fiscal
	$chave .= sprintf("%03d", $serie); // 03 - serie - Série do Documento Fiscal
	$chave .= sprintf("%09d", $nNF);   // 09 - nNF   - Número do Documento Fiscal
	$chave .= sprintf("%01d", $tipo);  // 01 - tipo  - Forma de emissÃ£o da NF-e [01]
	$chave .= sprintf("%08d", $cNF);   // 09 - cNF   - Código Numérico que compõe a Chave de Acesso
	$chave .= calculaDV($chave);       // 01 - cDV   - Dígito Verificador da Chave de Acesso
...
function getStatus($i) {
	...
	'302' => 'Denegado: Irregularidade fiscal do destinatário',
