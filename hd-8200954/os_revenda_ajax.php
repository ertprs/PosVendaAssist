		...
		for ($y = 0;$y < $qtde_item;$y++) {
			...
			if (!empty($referencia)) {
				...
				if (pg_num_rows($resVerificaProduto) == 0) {
					...
					$msg_erro .= "Produto {$referencia} não encontrado para o fabricante <br />";
				}
...
if (in_array($login_fabrica, [80]) && empty($os_revenda)) { 
	...
	<form name="frm_xml_nf" method="POST" action="<?= $_SERVER['php_self'] ?>" enctype="multipart/form-data" style="width: 400px;">
		<table width="400" border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="white" class="table">
			<tr>
				<td colspan="2">Insira o XML da nota fiscal, e clique no botão "Carregar NF" para lançar os itens</td>
			</tr>
			<tr>
...
if(($login_fabrica == 15 or $login_fabrica == 80) AND $os_revenda == 0 AND ($btn_acao <> 'gravar' || $login_fabrica == 80)){
	...
	echo "<thead>";
		echo "<tr>";
			echo "<td class='menu_top'>Ação</td>";
	...
	for ($i=0; $i<$qtde_item; $i++) {
		...
		if ($i % 20 == 0 && ($login_fabrica != 80 || ($login_fabrica == 80 && !$exibiuCabecalho))) {
			...
			if ($login_fabrica != 80) {
				...
				echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif'>Ação</font></td>\n";
				echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif'>Descrição</font></td>\n";
				echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif'>Referência</font></td>\n";
				echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif'>Série</font></td>\n";
				echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif'>Nota Fiscal</font></td>\n";
			}
