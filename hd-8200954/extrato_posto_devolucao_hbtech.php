...
$sql = "SELECT  CASE WHEN data_geracao > '2008-06-01'::date THEN '1' ELSE '0' END
		FROM tbl_extrato
		WHERE extrato = $extrato ";
...
if ($login_posto==6359 and ($login_fabrica==11 or $login_fabrica==25)){#Somente para teste do PA TESTE
	$verificacao = "1";
}
...
<? if($login_fabrica==25) $class = "menu_top4";
   else                   $class = "menu_top";?>
<TR>
...
<? if($login_fabrica==25) $class = "table_line3";
	...
	if ($numero_linhas==5000){
		...
		if (pg_numrows($resConf)>0){
			...
			$sqlLgr = " SELECT count(*)
						...
						AND   (cfop LIKE '59%' OR cfop LIKE '69%') 
			...
			if (pg_numrows($resConf)>0 OR $qtde_devolucao == 0){
				...
				if (count($notas_array)==1){
					$msg_notas = "O Fabricante não acusou o recebimento da nota N° ".implode(",",$notas_array).". Favor entrar em contato c/ o Sr. Alberto TEL: 071 3379-1997, P/ posterior liberação da M.O.";
