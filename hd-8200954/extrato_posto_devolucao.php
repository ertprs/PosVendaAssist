...
if ($login_fabrica==11){
	...
	header("Location: extrato_posto_devolucao_lenoxx.php?extrato=$extrato");
	exit;
}

if ($login_fabrica==7 or $login_fabrica == 43 or $login_fabrica== 80){ // HD 112647
	$extrato = $_POST['extrato'];
	if (strlen($extrato)==0){
		$extrato = $_GET['extrato'];
	}
...
if ($login_fabrica==25){
	...
	header("Location: extrato_posto_devolucao_hbtech.php?extrato=$extrato");
	exit;
}
...
if ($login_fabrica==51){
	...
	header("Location: extrato_posto_devolucao_gama.php?extrato=$extrato");
	exit;
}
...
if (strlen($btn_acao) > 0) {
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
		...
		if (strlen($nota_fiscal) == 0) {
			...
			$assunto       = "URGENTE - $extrato EXTRATO POSTO DEVOLUCAO $extrato";
...
<?
	#echo "<br><br>";
	#echo "<h1 style='color:black'>Programa em manutenção. Previsão de retorno às 12h00.</h1>";
	#exit;
?>
...
<?
	...
	echo "<br>";
	$tipo_nota = ($login_fabrica == 50) ? "RETORNO DE REMESSA" : "Devolução";
	echo "O valor da mão-de-obra será exibido somente após confirmação da Nota Fiscal de $tipo_nota.";
	if($login_fabrica != 50){
	echo "<br>";
