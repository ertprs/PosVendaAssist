...
//    exit;
//}
// if ($login_fabrica == 19) { // 12/8/2009 09:22:30 Nathanael pediu para bloquear a tela de cadastro por causa do defeito reclamado
...
$calcula_km				= (($login_fabrica  == 1 AND $login_posto == 6359) OR
			               ...
			               in_array($login_fabrica, array(30,46,50,56,57,72,74,85,88,90,91,92)));

$combo_tipo_atendimento = (($login_fabrica == 15 AND ($login_tipo_posto == 174 OR $login_tipo_posto == 175)) OR
                           ($login_fabrica == 1  AND $login_posto == 6359) OR
                           ($login_fabrica == 24 AND $login_tipo_posto == 256) OR
                           in_array($login_fabrica, array(19, 20, 56, 58, 30, 50, 46, 7, 74, 40, 85, 72, 88,90, 91,92,96)));
...
if (strlen($_GET["os_externa"])) {
	...
	*
	...
	if (pg_num_rows($res) > 0) {
		...
		$msg_erro = "Falha ao importar OS $os_externa";
		$os_externa = "";
	}
}
...
#HD 38369
if($_GET["ajax"]=="true" AND $_GET["buscaPreOS"]=="true"){
    $serie      = trim($_GET["serie"]);
    $hd_chamado = trim($_GET["hd_chamado"]);
    ...
    if(strlen($serie)>0 or strlen($hd_chamado) > 0){
        ...
        if (pg_num_rows($res)==0) {
            ...
            $sql = "SELECT    ''                     AS hd_chamado,
                    ...
                    JOIN tbl_produto      ON tbl_produto.referencia = tbl_numero_serie.referencia_produto
        ...
        if (pg_num_rows($res)>0){
            ...
            */
            ...
            if (strlen($os)==0 or strlen($data_fechamento)>0){
                ...
                if ($login_fabrica == 52 or $login_fabrica == 30) {
    ...
    echo "nao|Nenhum número de série encontrado neste chamado";
    exit;
}
...
if ($btn_acao == "continuar") {
	...
	if (strlen($sua_os_offline) == 0 && strlen($os_externa) > 0) {
		$sql = "SELECT sua_os_offline FROM tbl_os_externa WHERE os_externa=$os_externa";
		$res = pg_query($con, $sql);

		if (pg_num_rows($res) == 1) {
			$sua_os_offline = pg_result($res, 0, "sua_os_offline");
		}
	}
    ...
    if (strlen (trim ($sua_os)) == 0) {
        $sua_os = 'null';
        //hd 4617
        if ($pedir_sua_os == 't' AND $login_fabrica<>5 AND $login_fabrica <> 86) {
...
#  fazer rotina para contar 6 caracteres antes do "-"
	...
	if($login_fabrica == 88 || $login_fabrica > 96){
    ...
    if ($login_fabrica == 6 or $login_fabrica == 7 or $login_fabrica == 51) {
    ...
    if (in_array($login_fabrica, array(7, 14, 45, 50, 51, 80)) and $xconsumidor_fone=='null') {
        $msg_erro .= " Erro: Digite o telefone do consumidor.<br />";
    }

    if (($login_fabrica == 7 or $login_fabrica == 14 OR $login_fabrica == 45) AND $xconsumidor_cidade == 'null') {
        $msg_erro .= " Digite a cidade do consumidor.<br />";
    }

    if (($login_fabrica == 7 or $login_fabrica == 14 OR $login_fabrica == 45) AND $xconsumidor_estado == 'null') {
        $msg_erro .= " Digite o estado do consumidor.<br />";
    }

    if ($login_fabrica == 19) {
        if (strlen($xconsumidor_fone) <> 16 AND strlen($xconsumidor_fone) <> 17) {
            $msg_erro .= "Telefone do consumidor em formato inválido. Formato válido: Ex. 011-91234-5678";
        }
    }
    ...
    if ($login_fabrica == 2 || $login_fabrica == 51 || $login_fabrica == 1 || $login_fabrica == 7 or $login_fabrica == 45 or $login_fabrica == 80) {
        if (strlen($xconsumidor_endereco) == 0) $msg_erro .= " Erro: Digite o endereço do consumidor. <br />";
    }

    if ($login_fabrica == 1 or $login_fabrica == 7 or $login_fabrica == 45 or $login_fabrica == 51) {
        if (strlen($xconsumidor_numero) == 0) $msg_erro .= " Erro: Digite o número do consumidor. <br />";
    ...
    if (pg_num_rows($res) > 0) {
        ...
        if ($km_google == 't') {
            ...
            if ($distancia_km_maps <> 'maps' AND ($qtde_km <> $qtde_km2 AND $qtde_km > 0)) {
                ...
                if ($login_fabrica == 50 AND $qtde_km > 50) {//HD: 24813 - PARA
                    $km_auditoria = "TRUE";
                }
				...
				if($login_fabrica == 90 AND $qtde_km > 120){ // fabrica 90 HD 310122
                    $km_auditoria = "TRUE";
                    $obs_km=" KM maior que 120km. ";
                }
    ...
    if($login_fabrica==7 or $login_fabrica==45){
        if (strlen(trim($xconsumidor_cep)) == 0) $msg_erro .= " Digite o cep do consumidor. <br />";
    ...
    /*
                    if (strlen($msg_erro) == 0) {
                        ...
                        $xcliente      = pg_fetch_result ($res,0,0);
                        $msg_erro      = pg_errormessage($con);
                    }
    */
                }
    ...
    if (strlen($revenda_cnpj) == 0 and in_array($login_fabrica, array(3, 51, 72, 81))) {
...
//=====================revenda
    ...
    if (strlen(trim($_POST['revenda_estado'])) == 0){
        if ($login_fabrica==7 or $login_fabrica==14
            ...
            $msg_erro .= " Erro: Digite o estado da revenda. <br />";
        }
    }else{
...
//=====================revenda
    ...
    // HD 15835
    //HD 206869: Exigir nota fiscal para a Salton
    if (($login_fabrica == 14) or ($login_fabrica == 6) or ($login_fabrica == 24) or ($login_fabrica == 11 and $login_posto != 20321) or ($login_fabrica == 81)){
...
//pedido por Leandro Tectoy, feito por takashi 04/08
    if(($login_fabrica==6) or ($login_fabrica == 24) or ($login_fabrica == 5)or ($login_fabrica == 11 and $login_posto != 20321 )){
...
//pedido por Leandrot tectoy, feito por takashi 04/08
    ...
    // HD 56479
    //if ( ((strpos (strtoupper($revenda_nome),"CASA BAHIA") === false) and (strpos (strtoupper($revenda_nome),"CASAS BAHIA") === false)) and $login_posto <> 653){ // HD 57109
    ...
    if($xproduto_serie=='null' and ($login_fabrica == 11 or $login_fabrica == 56 or $login_fabrica == 79))  $msg_erro .= " Digite o Número de Série.<br />";
...
//    $os = $_POST['os'];
	...
	} else if ($defeito_reclamado == 'null' and $login_fabrica==86){
...
//
    ...
    //HD 206869: Exigir número de telefone do consumidor para a Salton
     if ($login_fabrica == 81 or $login_fabrica == 3) {
...
//CARTÃO CLUBE - LATINATEC
    ...
    if (strlen($msg_erro) == 0){
        ...
        if (strlen($os) == 0) {
            ...
            if ($login_fabrica == 52 or ($login_fabrica == 30 and strlen($admin > 0)) ) {
                $sql .= ($admin) ? ", admin " : '';
...
// echo "<pre>$sql</pre>";
        ...
        if (strlen ($msg_erro) == 0) {
            if (strlen($os) == 0) {
				...
				if (strlen($os_externa)) {
					$sql = "UPDATE tbl_os_externa SET os=$os WHERE os_externa=$os_externa";
					$res = pg_query($con, $sql);
				}
            }
        }
    ...
    if (strlen ($msg_erro) == 0) {
		...
		if( $login_fabrica == 74 and empty( $_POST['produto_serie'] ) ) {
		...
		if($login_fabrica == 96 AND !empty($hd_chamado)) { # HD 390996
			$origem_anexo  = '/var/www/assist/www/admin_cliente/anexos';
        ...
        if(($login_fabrica == 3 and $linhainf == 't') or in_array($login_fabrica, array(2, 30, 40, 43, 52, 59, 80, 81, 85,96))) {
			...
			if(strlen($sua_os)>0 and strlen($hd_chamado)>0){
                if ($login_fabrica <> 52 and $login_fabrica <> 30 and $login_fabrica !=96) {
                    ...
                    $sqlinf     = "UPDATE tbl_hd_chamado_item
                                    SET os = $os
                                    WHERE tbl_hd_chamado_item.hd_chamado_item = $hd_chamado_item;";
                }
        ...
        if (strlen ($msg_erro) == 0) {
            ...
            if (strlen($msg_erro) == 0 AND strlen ($revenda_cnpj) > 0 and strlen ($xrevenda_cidade) > 0 AND $xrevenda_cidade<>'null' and strlen ($xrevenda_estado) > 0 AND $xrevenda_estado<>'null') {
				...
				if ($usa_revenda_fabrica) {
					...
					}elseif($revenda_fabrica_status == "cadastrado"){
						$sql = "
						UPDATE tbl_revenda_fabrica SET 
...
//===============================REVENDAAAA
            ...
            /* Se o cliente tiver DESCONTO, atribui-lo. Caso contrario, somente o ADMIN pode alterar */
            if ($login_fabrica == 7 AND strlen($consumidor_cpf) > 0){
                $sql = "SELECT  tbl_posto_consumidor.posto,
                ...
                if (pg_num_rows ($res2) > 0 ) {
                    ...
                    $xdesconto_peca= trim(pg_fetch_result($res2,0,desconto_peca));

                    if (strlen($xdesconto_peca)==0){
                        $xdesconto_peca = " 0 ";
                    }
                }
            }
           ...
           // echo nl2br($sql);
			...
			if ($anexaNotaFiscal) {
				if (is_array($_FILES)) {
					...
					if ($anexou != 0) $msg_erro.= $msgs_erro[$anexou]; // '0' é que executou OK
			}
		}
		...
		//  FIM Anexa imagem NF
            ...
            if (strlen ($msg_erro) == 0) {
                ...
                //hd chamado 3371 e 12881
                if ($login_fabrica == 3 and $pedir_sua_os == 'f') {
                    $sua_os_repetiu = 't';
                    while ($sua_os_repetiu == 't') {
                        ...
                        if (pg_num_rows($res_sua_os) > 0) {
                            ...
                            //HD 52457 - Da um sleep se a OS for repetida. Entra neste caso somente quando duas OS estao sendo gravadas no mesmo momento. Entao da um tempo para outra OS passar no processo sem duplicadas a numeracao
                            ...
                            if ($login_fabrica == 1) {
                                $sql_sua_os = " UPDATE tbl_os set sua_os = lpad ('$sua_os_atual',6,'0')
                                                WHERE  tbl_os.os      = $os
                                                and    tbl_os.fabrica = $login_fabrica";
                            }
...
/* HD 7998 - TAKASHI 12/12/2007 - VERIFICA SE EXISTE ALGUM CHAMADO DE CALLCENTER PARA ESSE PRODUTO, SE TIVER REABRE O CHAMADO INSERE UM CHAMADO ITEM, MANDA EMAIL PARA O SUPERVISOR E PARA QUEM ABRIU O CHAMADO*/
...
/*        }
        if(strlen($msg_erro)==0){*/
            $sql = "UPDATE tbl_hd_chamado set status = 'Aberto'
...
/* HD 7998 - TAKASHI 12/12/2007 - VERIFICA SE EXISTE ALGUM CHAMADO DE CALLCENTER PARA ESSE PRODUTO, SE TIVER REABRE O CHAMADO INSERE UM CHAMADO ITEM, MANDA EMAIL PARA O SUPERVISOR E PARA QUEM ABRIU O CHAMADO*/
                ...
                // Lenoxx HD 13826
                if ($login_fabrica == 3 or $login_fabrica == 11 or $login_fabrica==25 or $login_fabrica==35 or $login_fabrica==51) {
                                                        ...
                                                        if($login_fabrica == 51){#HD52537 alterado apenas para a Gama pois não sei se as outras fábrica atualiza o pedido_item
                                                            ...
                                                            if (@pg_num_rows($res_dis) > 0) {
                                                                for($x=0;$x<@pg_num_rows($res_dis);$x++){
                                                                    ...
                                                                    $sql2 = "SELECT fn_pedido_cancela_garantia(4311,$login_fabrica,$pedido_pedido,$pedido_peca,$pedido_os_item,'Troca de Produto'); ";
...
<?    }   ?>
    ...
    $(document).ready(function(){
		...
		<?
		if (strlen($os_externa) && strlen($btn_acao) == 0) {
			echo "abreOSExterna();";
		}
		?>
	    });
...
<?    if ($login_fabrica == 43 or $login_fabrica == 24 or $login_fabrica == 88 or $login_fabrica == 89 or $login_fabrica == 74) {  // +89 HD 320189  ?>
...
function verificaPreOS(){
    ...
    if (numero_serie.length > 0 || chamado.length>0) {
        ...
        url = "<?=$PHP_SELF?>?ajax=true&buscaPreOS=true&serie="+numero_serie+'&hd_chamado=' +chamado+'&hd_chamado_item='+chamado_item+'&data='+curDateTime;
...
function abreOSExterna(){
	os_externa = "<?=$os_externa?>";

    if (os_externa.length > 0) {
        var curDateTime = new Date();
        http6[curDateTime] = createRequestObject();
        url = "os_cadastro_tudo_ajax_externa.php?acao=busca&os_externa=<?=$os_externa?>&data="+curDateTime;
        http6[curDateTime].open('get',url);
...
<?
    ...
    if($login_fabrica == 88 OR $login_fabrica > 96){
...
if($login_fabrica==30){ // HD 56479
    ...
    echo "<br />";
    echo "OS CAMPOS EM VERMELHO SÃO DE PREENCHIMENTO OBRIGATÓTIO";
    echo "</td>";
    echo "</tr>";
    echo "</table>";
...
?>
        ...
        <form style="margin: 0px;" name="frm_os"  method='post' enctype="multipart/form-data" action="<? echo $PHP_SELF ?>">
        <input class="frm" type="hidden" name="os_externa" value="<? echo $os_externa; ?>">
...
?>
        ...
        <tr>
        <? if ($pedir_sua_os == 't' && $login_fabrica != 86) { ?>
            <td nowrap  valign='top'>

                <font size="1" face="Geneva, Arial, Helvetica, san-serif">OS Fabricante</font>
                <br />
                <input  name ="sua_os" class ="frm" type ="text" size ="10" <?if ($login_fabrica==5){?>maxlength="6" ReadOnly onclick="alert('Mantenha esse campo em branco para geração automática de número de ordem de serviço.\nCaso tenha alguma dúvida, entrar em contato com a Mondial através do 0800-7707810 ou ata@mondialline.com.br');" <?}else{?>maxlength="20"<?}?> value ="<? echo $sua_os ?>" onblur = "this.className='frm'; displayText('&nbsp;');" onfocus ="this.className='frm-on';displayText('&nbsp;Digite aqui o número da OS do Fabricante.');">
                <?
                } else {
                    echo "&nbsp;";
            ...
            <? if (in_array($login_fabrica,array(6,50,56,43,74,95)) ){ ?>
            <td nowrap  valign='top'>
                ...
                <input class="frm" type="text" name="produto_serie"  id="produto_serie" size="12" maxlength="20"
                     ...
                     onfocus="this.className='frm-on'; displayText('&nbsp;Digite aqui o número de série do aparelho.');"
                      onblur="this.className='frm'; displayText('&nbsp;');<?
                        if ($login_fabrica == 50) echo 'fnc_pesquisa_numero_serie(document.frm_os.produto_serie, \'produto_serie\');';
                        ?>">
                ...
                <? if($login_fabrica==6 or $login_fabrica==50 or $login_fabrica == 43) { ?>
                    ...
                    <?} else {?>
                        onclick="javascript: fnc_pesquisa_numero_serie (document.frm_os.produto_serie, 'produto_serie');">
                    <?}
                }
            ...
            <td nowrap  valign='top'>
                ...
                <input class="frm" type="text" name="produto_serie" id="produto_serie"  size="8"
                    maxlength="<?=($login_fabrica==35) ? '12' : '20' ?>" value="<? echo $produto_serie ?>" <?
            ...
            if($login_fabrica <> 15){?>
                <td nowrap  valign='top'>
                    ...
                    <input class="frm" type="text" name="produto_referencia"  id="produto_referencia" size="15" maxlength="20" value="<? echo $produto_referencia ?>" readonly>
                    <? }else{ ?>
...
<?}?>
        <?
            ...
            <? if (!in_array($login_fabrica,array(6,24,19,50,43,56,59,2,30,15,85,74,95)) AND $linhainf <> 't'){ ?>
                ...
                <?}?>
                <input class="frm" type="text" name="produto_serie" id="produto_serie"  size="14" <? if($login_fabrica == 40) { ?> maxlength="7" <? }elseif($login_fabrica==35){ ?> maxlength="12" <?}else{?> maxlength="20" <?}?> value="<? echo $produto_serie ?>" <? if($login_fabrica==50 or $login_fabrica == 43){?>onChange="javascript: this.value=this.value.toUpperCase();"<?}?> onblur="this.className='frm'; displayText('&nbsp;');<? if($login_fabrica==3 and $login_posto==6359){ echo " MostraEsconde('dados_1');";} if ($login_fabrica==7 /*and 1==2*/){echo "verificaProduto(document.frm_os.produto_referencia,this)";} ?>" onfocus="this.className='frm-on'; displayText('&nbsp;Digite aqui o número de série do aparelho.'); <? if($login_fabrica==3 and $login_posto==6359){ echo " MostraEsconde('dados_1');";} ?> "><? if($login_fabrica == 25){ ?>&nbsp;<INPUT TYPE="button" onClick='javascritp:fn_verifica_garantia();' name='Verificar' value='Verificar' <? if($login_fabrica ==3) echo "rel=serie";?>>
                <? } ?>
<br /><font face='arial' size='1'><? if ($login_fabrica == 1) echo "(somente p/ linha DeWalt)"; ?></font>
                <div id='dados_1' style='position:absolute; display:none; border: 1px solid #949494;background-color: #f4f4f4;'>
                </div>
    ...
    <? } ?>
        ...
        <? if ($login_fabrica <> 95){ ?>
            <? if ($login_fabrica == 30) { ?>
                ...
                <td valign='top' align='left'>
                    ...
                    <br />
                    <?
					  if( !in_array( $login_fabrica, array(42,74,86,96) ) ) //HD314245
                        if($pedir_defeito_reclamado_descricao == 't'){
                            //HD 204082: Recuperar defeito reclamado da pré-os
                            echo "<input type='text' name='defeito_reclamado_descricao' id='defeito_reclamado_descricao' class='frm' ".
                         "value='$defeito_reclamado_descricao' size='40' onKeyUp='somenteMaiusculaSemAcento(this);'>";
                        }else{
                            if(strlen($defeito_reclamado) >0) {
                                $sql = " SELECT descricao
                                    FROM tbl_defeito_reclamado
                            ...
                            echo "<select name='defeito_reclamado' id='defeito_reclamado' style='width: 220px;' onfocus='listaDefeitos(document.frm_os.produto_referencia.value);' class='frm' $defeito_reclamado_onchange>";
                            if(strlen($defeito_reclamado) > 0) {
                                echo "<option id='opcoes' value='$defeito_reclamado'>$reclamado_descricao</option>";
                            }else{
                                echo "<option id='opcoes' value='0'></option>";
							...
							if ($login_fabrica<>86){
								...
								echo '<td valign="top" align="left" style="padding:0 5px 0 5px;"> <font size="1" face="Geneva, Arial, Helvetica, san-serif">Descrição do Defeito Reclamado</font>
								<br />';
								echo "<input type='text' name='defeito_reclamado_descricao' id='defeito_reclamado_descricao' class='frm' ".
        ...
        <td align='left'>
        <font size="1" face="Geneva, Arial, Helvetica, san-serif">
        <?=($login_fabrica==7)?"Natureza":"Tipo Atendimento"?>
        </font><br />
        <select name="tipo_atendimento" id='tipo_atendimento' class='frm' style='width:180px;'
        <? if ($login_fabrica==50) { // HD 54668 para Colormaq ?>
        onFocus="javascript:tipoatendimento();"
        <?}?>
...
//  Solicitação do Nathanael, 12/05/2010, para fazer testes no posto de Testes. Finalizados os testes,
//  acrescentar '14' ao 'codigo IN...' no SELECT e tirar este trecho
//         if ($login_fabrica==19 and $login_posto == 6359) {
//             echo "\t\t\t<option value='81'>14 - Solicitação de Peças</option>\n";
//         }
        ?>
        </select>
        </td>
...
<?
        if($login_fabrica==6 or $login_fabrica == 11) {
        ...
        }elseif($login_fabrica == 30){ // HD 65178
            ...
            echo "<input type='text' name='os_posto' class='frm' id='os_posto' size='12' maxlength='10' value='$os_posto'>";
            echo "</td>";
        }elseif($login_fabrica == 50){ // HD 79844
		...
		if($login_fabrica<>7 and $login_fabrica <> 11) {
            echo "<td nowrap  valign='top'><font size='1' face='Geneva, Arial, Helvetica, san-serif'>&nbsp;Box/Prateleira</font><br />";
            echo "&nbsp;<INPUT TYPE='text' name='prateleira_box' class='frm' value='$prateleira_box' size='8' maxlength='10'>";
            echo "</td>";
        }
?>
        </tr>
...
<table width="750" align='center' border="0" cellspacing="5" cellpadding="0">
    <tr>
        ...
        <td>
            ...
            <br />
            <input class="frm" type="text" name="consumidor_cep" id="consumidor_cep"  size="12" maxlength="10" value="<? echo $consumidor_cep ?>" onkeypress="mascara(this,soNumeros)" onblur="this.className='frm'; displayText('&nbsp;'); buscaCEP(this.value, document.frm_os.consumidor_endereco, document.frm_os.consumidor_bairro, document.frm_os.consumidor_cidade, document.frm_os.consumidor_estado) ;" onfocus="this.className='frm-on'; displayText('&nbsp;Digite o CEP do consumidor.');">
        </td>
    </tr>
</table>
...
<table width='750' align='center' border='0' cellspacing='5' cellpadding='2'>
    ...
    <tr>
        <td valign='top' align='left'>
            <font size="1" face="Geneva, Arial, Helvetica, san-serif">Email</font>
            <br />
            <INPUT TYPE='text' name='consumidor_email' id='consumidor_email' class='frm' value='<? echo $consumidor_email ?>' size='30' maxlength='50' <? if($login_fabrica==50){?>onChange="javascript: this.value=this.value.toUpperCase();"<?}?> onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Digite o endereço do consumidor.');">
            <? if($login_fabrica== 79) echo "<font color='#FF0000'>*</fotn>"; // HD 78055?>
        </td>
        <? if ($login_fabrica == 43 or $login_fabrica == 52) { ?>
            <td>
...
//  14/07/2010 MLG - HD 264024 - Retirei o bairro do endereço (confunde o GoogleMaps) e adicionei, se tem, a latitude e longitude.
    if (pg_num_rows($res_posto) > 0) {
        ...
        $endereco_posto = $info_posto[endereco].', '.$info_posto[numero].' '.
                          $info_posto[cidade].' '.$info_posto[estado];
...
<div id='div_mapa' style='background:#efefef;border:#999999 1px solid;font-size:10px;padding:5px;<?if($km_google<>'t' or $login_fabrica <> 52) echo "visibility:hidden;position:absolute;";?>' >
...
#            echo " -->";
            ...
            if ($login_fabrica <> 1) {
                if ($login_fabrica == 11) {
                    ...
                    <td>
                        ...
                        <br />
                        <input class="frm" type="text" id="acessorios" name="acessorios" size="30" value="<? echo $acessorios ?>" <? if($login_fabrica==50){?>onChange="javascript: this.value=this.value.toUpperCase();"<?}?> onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Texto livre com os acessórios deixados junto ao produto.');">
                    </td><?php
                }
            }
        ...
        <?
        if ($login_fabrica == 30 OR $login_fabrica == 43 ) {
            ?>
            <table width="750" align='center' border="0" cellspacing="5" cellpadding="0">
            <tr>
...
if (strlen($os)==0 and
	...
	($linhainf == 't' or in_array($login_fabrica, array(2, 24, 40, 46, 52, 59,30, 74,80, 81, 85, 90, 91,96)))) { //89, ?>
