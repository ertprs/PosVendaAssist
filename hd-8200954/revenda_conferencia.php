...
//					$teste .= "<br>$sql";
	...
	$resposta .= "\n <br><FONT COLOR='#FF0000'>LOTE JÁ CONFERIDO, NÃO É POSSÍVEL LANÇAR PRODUTOS</font><br>\n";
...
if($_GET['ajax']=='sim' AND $_GET['acao']=='ver') {
	$conferencia = $_GET['conferencia'];
		...
		*/
		...
		$resposta  .= "<a href='javascript:mostrar_treinamento(\"dados\",\"&conferencia=nao_explodido\");'style='font-size:10px;'>Ver notas fiscais não explodida em OS</a>&nbsp;&nbsp;&nbsp;";
	...
	if (pg_numrows($res) > 0 ) {
		...
		$resposta  .=  "<TD background='admin/imagens_admin/azul.gif'><b>Código Lote</b></TD>";
...
if($_GET['ajax']=='sim' AND $_GET['acao']=='formulario') {
	...
	if ($qtde > 0) {
		...
		if($conferencia<>'t' OR strlen($conferencia)==0) {
			...
			$resposta .= "<br><FONT COLOR='#FF0000'>LOTE JÁ CONFERIDO, NÃO É POSSÍVEL LANÇAR PRODUTOS</font><br>";
			$resposta .= "<P></P><div id='saida' style='display:inline;'></div><div id='erro' style='display:inline;'></div>";

			if(strlen($data_explodido)==0){
				$resposta .= "<input type='button' name='btn_explodir'  value='Explodir' onClick=\"if (this.value!='Explodir'){ alert('Aguarde');}else {this.value='Executando...'; window.location = '$PHP_SELF?explodir=$lote_revenda';}\" style=\"width: 150px;\" class='Explodir'>";
			}else{
				$resposta .= "<FONT COLOR='#FF0000'>LOTE JÁ EXPLODIDO</font><br>";
				$resposta  .= "<br><center><a href='javascript:mostrar_treinamento(\"dados\",\"&conferencia=recebido\");'>VER OUTROS LOTES</a></center><br>";
				$sql = "SELECT DISTINCT
				...
				if (pg_numrows($res) > 0) {
					...
					$resposta .=  "<td height='20' colspan='3'><b>Não geraram OS</b></td>\n";
					$resposta .=  "</tr>\n";
...
*/
				...
				if (@pg_numrows($res) > 0) {
					...
					$resposta .=  "<tr align='center' class='Conteudo'>\n";
					$resposta .=  "<td height='20' colspan='6'><b>Devolução</b></td>\n";
					$resposta .=  "</tr>\n";
