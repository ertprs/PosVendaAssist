  ...
  function(value, element) {
	var val_exp=value.split('/');
      ...
      if((ano>=1000)&&(ano<10000)){
		//var da = new Date();
		var de = value.replace(/(\d{2})\D(\d{2})\D(\d{4})/, '$2/$1/$3');
		var data_atual = da.valueOf(); // Timestamp da data atual
		var data_elem  = new Date(de).valueOf();

		if (data_elem > data_atual) return false; // False se a data é > que a data atual
