...
if (strlen($acao) > 0 && $acao == "PESQUISAR") {
	...
	if (strlen($aux_data_inicial) < 8){
		if($sistema_lingua == "ES") $erro .= "Fecha inicial en formato invalido";
		else                       $erro .= "Data inicial em formato inválido";
	}
	...
	if (strlen($erro) == 0){
		...
		if($sistema_lingua == "ES") $erro .= "Informe la fecha inicial para realizar la busca ";
		else                        $erro .= " Informe a Data Inicial para realizar a pesquisa. ";
		}
		...
		if (strlen($x_data_final) > 0 && $x_data_final != "null") {
			...
			if (strlen($aux_data_final) < 8) {
				if($sistema_lingua == "ES") $erro .= "Fecha cierre en formato invalido";
				else                        $erro .= "Data final em formato inválido";
			}
			...
			if($sistema_lingua == "ES") $erro .= "Informe la fecha cierre para realizar la busca ";
			else                        $erro .= " Informe a Data Fechamento para realizar a pesquisa. ";
		}
...
//if($ip=='200.246.168.219')echo nl2br($sql);
		...
		if (pg_numrows($res) > 0) {
			...
			for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
				...
				if(strlen($os_admin) >0) $cor = "#FF0000";
				...
				if($login_fabrica==20){
					echo "<td nowrap align='center' title='Extrato/Lote que a OS foi paga'>" . $extrato . "</td>";
				}
...
*/
	...
	if ($achou == "nao") {
		...
		echo "Não foram encontrados registros com os parâmetros informados/digitados!!!";
