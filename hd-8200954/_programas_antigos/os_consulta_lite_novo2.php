...
function retornaFechamentoOS (http , sinal) {
		...
		if (http.status == 200) {
			...
			if (typeof (results[0]) != 'undefined') {
				if (results[0] == 'ok') {
					...
					alert ('Erro no Fechamento da OS ' + results[1]);
				}
			}else{
...
function fechaOS (os , sinal ) {
	url = "<?= $PHP_SELF ?>?fechar=" + escape(os) ;
	http.open("GET", url , true);
	http.onreadystatechange = function () { retornaFechamentoOS (http , sinal) ; } ;
	http.send(null);
}
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			if (strlen($fechamento) == 0) {
				if ($excluida == "f" || strlen($excluida) == 0) {
					echo "<a href=\"javascript: if (confirm('Confirma fechamento da OS $sua_os ?') == true) { fechaOS ($os,sinal_$i) ; }\"><img id='sinal_$i' border='0' src='/assist/admin/imagens_admin/status_verde.gif'>fechar</a>";
				}
			}else{
				echo "&nbsp;";
			}
