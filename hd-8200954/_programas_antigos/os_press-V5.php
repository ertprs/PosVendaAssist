...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0">
<TR>
	<TD colspan="<? if ($login_fabrica == 1) { echo "8"; }else{ echo "7"; } ?>"><img src="imagens/top_diagnostico.gif"></TD>
</TR>
<TR>
...
<!-- 	<TD class="conteudo" style="text-align:left;"><? echo pg_result ($res,$i,referencia) . " - " . pg_result ($res,$i,descricao); ?></TD> -->
	...
	/*
	if ($login_fabrica == 1) {
		$sqlX =	"SELECT tbl_tipo_posto.descricao                ,
		...
		if (pg_numrows($resX) > 0) {
			$tipo_posto_descricao  = trim(pg_result($resX,0,descricao));
			$icms                  = trim(pg_result($resX,0,icms));
			$acrescimo_tabela_base = trim(pg_result($resX,0,acrescimo_tabela_base));
		}
		
		if (strlen($tipo_posto_descricao) > 0 && strlen($icms) > 0) {
			echo "<TD class='conteudo' style='text-align: center;'>";
			
			$sqlX = "SELECT tbl_peca.peca";
			switch (substr($tipo_posto_descricao,0,3)) {
				case "Dis" :
					$sqlX .= " , (tbl_tabela_item.preco / $icms) AS preco";
				break;
				case "Vip" :
					$sqlX .= " , (tbl_tabela_item.preco * $acrescimo_tabela_base / $icms) AS preco";
				break;
				case "Loc" :
					$sqlX .= " , (tbl_tabela_item.preco * $acrescimo_tabela_base / $icms) AS preco";
				break;
			...
			$sqlX .=	" FROM tbl_peca
						...
						AND   tbl_tabela_item.tabela = 108;";
			$resX = pg_exec ($con,$sqlX);
			
			if (pg_numrows($resX) == 1) {
				echo "R$ " . number_format(trim(pg_result($resX,0,preco)),2,",",".");
			}else{
				echo "&nbsp;";
			}
		}else{
	...
	*/
	...
	if (strtolower($nf) <> 'pendente'){
		if ($link == 1) {
			...
			$sqlX = "SELECT tbl_embarque.embarque, TO_CHAR (tbl_embarque.faturar,'DD/MM') AS faturar FROM tbl_embarque JOIN tbl_embarque_item USING (embarque) WHERE tbl_embarque_item.os_item = $os_item AND tbl_embarque.faturar IS NOT NULL";
			echo "$nf";
			//echo "<a href='nota_fiscal_detalhe.php?nota_fiscal=$nf&peca=$peca' target='_blank'>$nf</a>";
	}else{
		echo "$nf &nbsp;";
