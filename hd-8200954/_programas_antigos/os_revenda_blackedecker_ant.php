					...
					if (strlen ($referencia) > 0) {
						...
						if (strlen($serie) == 0) {
							...
							if ($linha == 199 OR $linha == 200) {
								$msg_erro .= " Número de série do produto $referencia não pode ser preenchido. ";
								$linha_erro = $i;
							}else{
								$serie = "'". $serie ."'";
							}
						...
						if (strlen($codigo_fabricacao) == 0) {
							$msg_erro = "Digite o Código de fabricação";
						}else{
						...
						if (strlen ($msg_erro) == 0) {
							...
							if (strlen ($msg_erro) == 0) {
								$sql = "SELECT fn_valida_os_item_revenda_black($os_revenda,$login_fabrica,$produto)";
								$res = @pg_exec ($con,$sql);
								$msg_erro = pg_errormessage($con);
							}
