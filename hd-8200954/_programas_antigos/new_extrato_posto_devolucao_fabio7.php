...
if (strlen($_GET['fechar_nota'])>0 AND strlen($_GET['extrato'])>0 AND strlen($_GET['extrato_devolucao'])>0){
	...
	$sql = "SELECT  
		...
		GROUP BY tbl_faturamento_item.faturamento_item, tbl_peca.peca, tbl_peca.referencia, tbl_peca.descricao, tbl_peca.ipi, tbl_faturamento_item.aliq_icms, tbl_faturamento_item.aliq_ipi";
	$res = pg_exec ($con,$sql);
...
if (strlen($btn_acao) > 0) {
	...
	$sql = "SELECT  
			...
			AND   tbl_faturamento.serie = '$serie'
			AND   tbl_faturamento_item.extrato_devolucao IS NULL
			GROUP BY  tbl_peca.peca, tbl_peca.referencia, tbl_peca.descricao, tbl_peca.ipi, tbl_faturamento_item.aliq_icms, tbl_faturamento.nota_fiscal,tbl_faturamento_item.aliq_ipi
			ORDER BY tbl_peca.referencia";
...
//			nf = cat.getElementsByTagName('nf')[0].firstChild.nodeValue;
			...
			if (gravou=='sim'){
				g_gravar='nao';
				g_next_qtde='';
				adicionaLinha(referencia, descricao, qtde, preco, total, icms, ipi);
			}
			else{
				var msg1="";
				var msg2="";
				...
				if (confirm("Item a ser incluido nesta nota\n\nReferencia: "+referencia+"\nDescrição: "+descricao+"\nQuantidade: "+qtde+"\n\n"+msg1+"\n\nOK: Sim\nCancelar: Não\n\n")){
					...
					if (qtde>1) {
						var qtdetem = prompt("Peça: "+referencia+" - "+descricao+"\nDas "+qtde+" peças, quantas você possui?\n\n\n0 = Não tenho nenhuma\n\nQuantidade:","");
					...
					if (erros==0){
						g_next_qtde=pecas_para_devolver
						proximo_item('sim');
					}
				}
			}
...
function proximo_item(gravar){

g_gravar = gravar
document.getElementById('next').disabled=true;
id_tabela=g_tbl_peca;

Ajax.Request('new_extrato_posto_devolucao_fabio7.php?proximo=item&extrato='+g_extrato+'&extrato_devolucao='+g_extrato_devolucao+'&pa='+g_produto_acabado+'&linha='+g_linha+'&distrib='+g_distribuidor+'&serie='+g_serie+'&gravar='+g_gravar+'&qtde='+g_next_qtde, Ajax.Response);
}
...
function adicionaLinha(referencia, descricao, qtde, preco, total, icms, ipi) {
...
function concluir_nota(extrato,extrato_devolucao,id_campo_nota,id_total){
	...
	Ajax.Request('new_extrato_posto_devolucao_fabio7.php?fechar_nota=nota&extrato='+extrato+'&extrato_devolucao='+extrato_devolucao, Ajax.FecharNota);
}
...
//			echo "<input type='hidden' name='next_qtde'   id='next_qtde'   value=''>\n";
			echo "<input type='button' name='next' id='next' value='+ Incluir mais um item' onClick=\"proximo_item('nao');return true;\">\n";
			echo "</td>\n";
			echo "</tr>\n";
			echo "</tfoot>\n";
		}
		...
		if ($nota_fiscal==0){
			...
			echo " var g_extrato='$extrato';
				   var g_extrato_devolucao='$extrato_devolucao'
				   var g_produto_acabado='$produto_acabado';
				   var g_linha='$linha';
				   var g_distribuidor='$distribuidor';
				   var g_serie='$serie';
				   var g_tbl_peca='tbl_pecas_$i';
				   var g_gravar='nao';
				   var g_next_qtde='';
				   ";
			echo "</script>";
...
// 		$digitou_nota = pg_result ($resQ,0,0);
		...
		if ($nota_fiscal==0){
			echo "<br><input type='button' id='fechar' value='Finalizar Esta Nota' name='fechar' onclick=\"javascript:concluir_nota('$extrato','$extrato_devolucao','rodape_$i','total_nota_$i');this.style.display='none';return false;\"><br>";
		}
