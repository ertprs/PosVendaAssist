...
if ($btn_acao == "gravar") {
	...
	if (strlen ($msg_erro) == 0) {
		...
		for ($i = 0 ; $i < $qtde_item ; $i++) {
			...
			$xcausa_defeito = $_POST['causa_defeito_' . $i];
		...
		if(strlen($os) > 0 AND strlen ($msg_erro) == 0){
			...
			if (pg_numrows($res) > 0) {
				for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
					...
					$posicao[$i]                 = pg_result($res,$i,posicao);
					$descricao[$i]               = pg_result($res,$i,descricao);
					$defeito[$i]                 = pg_result($res,$i,defeito);
					$causa_defeito[$i]           = pg_result($res,$i,causa_defeito);
					$causa_defeito_descricao[$i] = pg_result($res,$i,causa_defeito_descricao);
				...
				for ($i = 0 ; $i < $qtde_item ; $i++) {
					...
					$causa_defeito[$i] = $_POST["causa_defeito_"     . $i];
...
#		if (strlen($faturado) > 0) $loop = $qtde_item - $faturado;
		...
		for ($i = 0 ; $i < $loop ; $i++) {
			...
			if ( 1==1 AND $os_item_aparencia == 't' AND $posto_item_aparencia == 't' and $os_item_subconjunto == 'f') {
...
//					echo "<td align='center'><input class='frm' type='text' name='descricao_$i' size='25' value='$xdescricao'>&nbsp;<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_peca_lista (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.preco , \"descricao\" )' alt='Clique para efetuar a pesquisa' style='cursor:pointer;'></td>";
			...
			if ($pedir_causa_defeito_os_item == 't') {
				...
				echo "<select class='frm' size='1' name='causa_defeito_$i'>";
				...
				for ($x = 0 ; $x < pg_numrows ($res) ; $x++ ) {
					echo "<option ";
					if ($causa_defeito[$i] == pg_result ($res,$x,causa_defeito)) echo " selected ";
