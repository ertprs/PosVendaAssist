				...
				if ($ativo == 't'){
					...
					if (strlen ($erro) == 0) {
						$sql = "UPDATE  tbl_os SET
								...
								WHERE   tbl_os.os         = $os
								AND     (tbl_os.posto = $login_posto OR tbl_os.posto IN (SELECT posto FROM tbl_posto_linha WHERE distribuidor = $login_posto))
...
<?
	if ( (strlen($sua_os) > 0 AND $btn_acao_pesquisa == 'continuar') OR strlen($listar) > 0 OR strlen ($codigo_posto) > 0){
		...
		$sql .= "ORDER BY lpad(substr(tbl_os.sua_os,0,strpos(tbl_os.sua_os,'-')),20,0) DESC, lpad(os,20,0) DESC;";
...
?>
		...
		<tr  bgcolor="<? echo $cor ?>" <? if ($linha_erro == $i and strlen ($msg_erro) > 0 )?>>
			...
			<td align="center"><? if (strlen($flag_bloqueio) == 0) { ?><input type="checkbox" class="frm" name="ativo_<?echo $i?>" value="t" ><? } ?></td>
			<td><font size="2" face="Geneva, Arial, Helvetica, san-serif"><a href='os_press.php?os=<? echo $os ?>' target='_blank'><? if ($login_fabrica == 1) echo $login_codigo_posto; echo $sua_os; ?></a></td>
