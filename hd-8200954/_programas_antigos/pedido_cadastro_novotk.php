...
if($login_fabrica==24 and $tipo_pedido==104){
	...
	if(strlen($mes)>0){
		...
		if($qtde >= 2){
		$msg_erro = "Seu PA já fez 2 (dois) pedidos de garantia este mês, por favor entre em contato com o fabricante";
		}
	}
...
if(strlen($mes)>0){
	...
	if($qtde < 2){
		echo "<tr>";
		echo "<td nowrap align='center'><font size='2' face='Geneva, Arial, Helvetica, san-serif'>Seu PA fez <B>$qtde</B> pedido(s) em garantia este mês</font>";
		echo "</td>";
		echo "</tr>";
	}else{
		echo "<tr>";
		echo "<td nowrap align='center'><font size='2' face='Geneva, Arial, Helvetica, san-serif' color='#ff0000'>Seu PA fez <B>$qtde</B> pedido(s) em garantia este mês, caso necessite de outro pedido em garantia<BR> por favor entre em contato com o fabricante.</font>";
		echo "</td>";
		echo "</tr>";
	}
}
