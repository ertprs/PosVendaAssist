...
#--------- SELECT para ver as OS abertas do proprio posto ----------#
...
/*
if ($login_posto <> 1053) {
	header("Location:os_consulta_em_aberto.php");
	exit;
}
*/
...
if (strlen($acao) > 0 && $acao == "PESQUISAR") {
	...
	if (strlen($opcao1) == 0 && strlen($opcao2) == 0 && strlen($opcao3) == 0 && strlen($opcao4) == 0 && strlen($opcao5) == 0 && strlen($opcao6) == 0 && strlen($opcao7) == 0) {
	...
	if (strlen($erro) == 0 && strlen($opcao1) > 0) {
		...
		if (strlen($mes) == 0) $erro .= " Selecione o mês para realizar a pesquisa. ";
	...
	if (strlen($erro) == 0 && strlen($opcao5) > 0) {
		...
		if (strlen($numero_os) < 3) {
			$erro .= " O campo Número da OS deve ser preenchido com mais de 4 caracteres. ";
		}
	}else{
		$numero_os = "";
		$tipo_os   = "";
	...
	if (strlen($erro) == 0 && strlen($opcao7) > 0) {
...
if (strlen($acao) > 0 && strlen($erro) == 0) {
		...
		$sql =	"SELECT distinct
				...
				JOIN      (
							SELECT os 
							FROM tbl_os 
							WHERE tbl_os.posto = $login_posto AND tbl_os.fabrica = $login_fabrica
							AND   tbl_os.excluida IS NOT TRUE
							UNION
							SELECT tbl_os.os 
							FROM tbl_os 
							...
							WHERE tbl_posto_linha.distribuidor = $login_posto AND tbl_os.fabrica = $login_fabrica
							AND   tbl_os.excluida IS NOT TRUE
							) oss ON tbl_os.os = oss.os
				JOIN      tbl_posto          ON tbl_posto.posto           = tbl_os.posto
		...
		if ($login_fabrica == 14) $sql .= " LEFT JOIN tbl_os_status      ON tbl_os_status.os          = tbl_os.os";
		$sql .= " WHERE tbl_os.fabrica = $login_fabrica
				AND   (tbl_os.posto = $login_posto OR tbl_os_extra.distribuidor = $login_posto OR tbl_posto_linha.distribuidor = $login_posto)
				AND   tbl_os.excluida IS NOT TRUE";

		if ($login_fabrica == 14) $sql .= " AND (tbl_os_status.status_os NOT IN (15) OR tbl_os_status.status_os IS NULL)";
		...
		if (strlen($numero_os) > 0) {
			if ($login_fabrica == 1) {
#				$sql .= " AND tbl_os.sua_os NOT ILIKE '%-%'";
				$numero_os = substr($numero_os, strlen($numero_os)-5, strlen($numero_os));
			}
			$sql .= " AND (tbl_os.sua_os ILIKE '$numero_os%' OR tbl_os.sua_os ILIKE '0$numero_os%' OR tbl_os.sua_os ILIKE '00$numero_os%')";
#			if ($tipo_os == "C")     $sql .= " AND tbl_os.sua_os = '$numero_os'";
#			elseif ($tipo_os == "R") $sql .= " AND tbl_os.sua_os ILIKE '$numero_os%'";
		}
		...
		if (strlen($produto) > 0) {
			$sql .= " AND tbl_os.produto ILIKE '$produto'";
		}
...
//if ($ip == "201.0.9.216") echo nl2br($sql);
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			$importacao_fabrica = trim(pg_result($res,$i,importacao_fabrica));
...
<form name="frm_consulta" method="get" action="<?echo $PHP_SELF?>">

<input type="hidden" name="acao">

<table width="400" align="center" border="0" cellspacing="0" cellpadding="2">
	<tr class="Titulo">
		<td colspan="6" align="center">SELECIONE OS PARÂMETROS PARA A PESQUISA</td>
	</tr>
	<tr bgcolor="#D9E2EF">
		<td colspan="6">&nbsp;</td>
	</tr>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td colspan="2"><input type="checkbox" name="opcao2" value="2" <? if (strlen($opcao2) > 0) echo "checked"; ?>> Nome do Consumidor</td>
		<td colspan="2"><input type="text" name="consumidor_nome" size="17" value="<?echo $consumidor_nome?>"></td>
		<td>&nbsp;</td>
	</tr>
	<tr bgcolor="#D9E2EF">
		<td colspan="6">&nbsp;</td>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td colspan="2"><input type="checkbox" name="opcao3" value="3" <? if (strlen($opcao3) > 0) echo "checked"; ?>> CPF/CNPJ do Consumidor</td>
		<td colspan="2"><input type="text" name="consumidor_cpf" size="17" value="<?echo $consumidor_cpf?>"></td>
		<td>&nbsp;</td>
	</tr>
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td colspan="6">&nbsp;</td>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td><input type="checkbox" name="opcao4" value="4" <? if (strlen($opcao4) > 0) echo "checked"; ?>> Produto</td>
		<td>Referência</td>
		<td>Descrição</td>
		<td>Voltagem</td>
		<td>&nbsp;</td>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td nowrap><input type="text" name="produto_referencia" size="8" value="<?echo $produto_referencia?>"> <img src="imagens/btn_lupa.gif" width='20' height='18'  width='20' height='18' style="cursor: hand;" align='absmiddle' alt="Clique aqui para pesquisar postos pelo código" onclick="javascript: fnc_pesquisa_produto (document.frm_consulta.produto_referencia, document.frm_consulta.produto_descricao, 'referencia', document.frm_consulta.produto_voltagem)"></td>
		<td nowrap><input type="text" name="produto_descricao" size="15" value="<?echo $produto_descricao?>"> <img src="imagens/btn_lupa.gif" width='20' height='18'  style="cursor: hand;" align='absmiddle' alt="Clique aqui para pesquisas pela referência do aparelho." onclick="javascript: fnc_pesquisa_produto (document.frm_consulta.produto_referencia, document.frm_consulta.produto_descricao, 'descricao', document.frm_consulta.produto_voltagem)"></td>
		<td nowrap><input type='text' name='produto_voltagem' size='5' value="<?echo $produto_voltagem?>"></td>
		<td>&nbsp;</td>
	</tr>
	<tr bgcolor="#D9E2EF">
		<td colspan="6">&nbsp;</td>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td colspan="2"><input type="checkbox" name="opcao5" value="5" <? if (strlen($opcao5) > 0) echo "checked"; ?>> Número da OS</td>
		<td colspan="2"><input type="text" name="numero_os" size="17" value="<?echo $numero_os?>"></td>
		<td>&nbsp;</td>
	</tr>
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td colspan="2">&nbsp;</td>
		<td nowrap><input type="radio" name="tipo_os" value="C" <? if (strlen($tipo_os) == 0 || $tipo_os == "C") echo "checked"; ?>> OS Consumidor</td>
		<td nowrap colspan="2">
<? if ($login_fabrica <> 1) { ?> 
...
<? } ?>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td colspan="2"><input type="checkbox" name="opcao6" value="6" <? if (strlen($opcao6) > 0) echo "checked"; ?>> Número de Série</td>
		<td colspan="2"><input type="text" name="numero_serie" size="17" value="<?echo $numero_serie?>"></td>
		<td>&nbsp;</td>
	</tr>
	<tr bgcolor="#D9E2EF">
		<td colspan="6">&nbsp;</td>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td>&nbsp;</td>
		<td colspan="2"><input type="checkbox" name="opcao7" value="7" <? if (strlen($opcao7) > 0) echo "checked"; ?>> Número da NF</td>
		<td colspan="2"><input type="text" name="numero_nf" size="17" value="<?echo $numero_nf?>"></td>
		<td>&nbsp;</td>
	</tr>
	<tr bgcolor="#D9E2EF">
		<td colspan="6">&nbsp;</td>
	...
	<tr bgcolor="#D9E2EF">
		<td colspan="6"><img src="imagens/btn_pesquisar_400.gif" onClick="document.frm_consulta.acao.value='PESQUISAR'; document.frm_consulta.submit();" style="cursor: hand;" alt="Preencha as opções e clique aqui para pesquisar"></td>
	</tr>
</table>
