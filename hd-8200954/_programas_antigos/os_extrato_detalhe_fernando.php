...
$sql .= "ORDER BY    lpad(substr(tbl_os.sua_os,0,strpos(tbl_os.sua_os,'-')),20,0)               ASC,
					replace(lpad(substr(tbl_os.sua_os,strpos(tbl_os.sua_os,'-')),20,0),'-','') ASC LIMIT 60";
//echo $sql;
$res = pg_exec ($con,$sql);
...
if ($btn_acao == 'filtrar' AND strlen($servico_realizado) > 0){
	$sql = "SELECT	 tbl_peca.referencia          ,
			...
			WHERE	 tbl_os_item.servico_realizado = $servico_realizado ";
