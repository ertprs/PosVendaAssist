...
#  fazer rotina para contar 6 caracteres antes do "-"
	...
	if($login_fabrica==6){
		if (strlen(trim($_POST['consumidor_nome'])) == 0) {$msg_erro .= " Digite o nome do consumidor. <br>";
...
//pedido por Leandrot tectoy, feito por takashi 04/08
	...
	if(($xproduto_serie=='null') and ($login_fabrica==11))  $msg_erro .= " Digite o Número de Série.<BR>";
...
//	$os = $_POST['os'];
	...
	else if (strlen($defeito_reclamado) == 0 and $login_fabrica <> 95) 
...
if($posto == 669 and 1==2){
			...
			$corpo.="<br>sql :$sql_OS <BR>erro: $msg_erro <BR>_______________________________________________\n";
	...
	if (abertura.length==10 && xnota_fiscal.length==10){
		...
		http3[curDateTime].onreadystatechange = function(){
			if(http3[curDateTime].readyState == 1) {
				atencao.innerHTML = "Calculando validade..";
			}
...
?>
		...
		if ($login_fabrica==11 and pg_numrows($res)==0) {
			echo "<table width=750><tr><td>";
			echo "<font face='Verdana' size='2' color='#0000FF'><left><b>Conforme comunicado de 14/12/2006, a partir de <font color='#FF0000'>04/01/2007<font color='#0000FF'> o número da OS Fabricante não será digitado pelo posto autorizado, este número será gerado automaticamente pelo sistema quando o botão <i><u>Continuar</i></u> for clicado. A via de papel não será mais enviada pelo fabricante, ela deve ser impressa pelo site. Em caso de dúvidas entrar em contato com o fabricante.</b></left></font>";
			echo "</td></tr></table>";
		}
		?>
		...
		<? if ($pedir_sua_os == 't') { ?>	
			...
			<td nowrap  valign='top'>
				...
				<? }else{ ?>
				<input class="frm" type="text" name="produto_referencia" size="15" maxlength="20" value="<? echo $produto_referencia ?>" onblur="this.className='frm'; displayText('&nbsp;');checarComunicado(<? echo $login_fabrica ?>);<?if($login_fabrica==24) {?> fnc_pesquisa_produto (document.frm_os.produto_referencia,document.frm_os.produto_descricao,'referencia',document.frm_os.produto_voltagem);<? } ?> " onfocus="this.className='frm-on'; displayText('&nbsp;Entre com a referência do produto e clique na lupa para efetuar a pesquisa.');" <? if (strlen($locacao) > 0) echo "readonly"; ?>>&nbsp;<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick="javascript: fnc_pesquisa_produto (document.frm_os.produto_referencia,document.frm_os.produto_descricao,'referencia',document.frm_os.produto_voltagem) " style='cursor: hand'>
				<? } ?>
...
?>
				<input name="data_abertura"  onKeyUp="formata_data(this.value,'frm_os', 'data_abertura')" size="12" maxlength="10" value="<? echo $data_abertura; ?>" type="text" class="frm" onblur="this.className='frm'; displayText('&nbsp;');<? if($login_fabrica==24){echo "MostraAtencao('atencao');"; } ?>" onfocus="this.className='frm-on'; displayText('&nbsp;Entre com a Data da Abertura da OS.');<? if($login_fabrica==24){echo "MostraAtencao('atencao');"; } ?>" tabindex="0"><br><font face='arial' size='1'>Ex.: <? echo date("d/m/Y"); ?></font>
			</td>
		...
		<br>
		<input class="frm" type="text" name="data_nf" onKeyUp="formata_data(this.value,'frm_os', 'data_nf')"  size="12" maxlength="10" value="<? echo $data_nf ?>" onblur="this.className='frm'; displayText('&nbsp;');<? if($login_fabrica==24){echo "MostraAtencao('atencao');"; } ?>" onfocus="this.className='frm-on'; displayText('&nbsp;Entre com a data da compra. Verifique se o produto está dentro do PRAZO DE GARANTIA.');<? if($login_fabrica==24){echo "MostraAtencao('atencao');"; } ?>" tabindex="0" <? if (strlen($locacao) > 0) echo "readonly"; ?>><br><font face='arial' size='1'>Ex.: <? echo date("d/m/Y"); ?></font>
<div id='atencao' style='position:absolute; display:none; border: 1px solid #949494;background-color: #f4f4f4;'>
		...
		<? if($login_fabrica == 15){ ?>
			<td width='100' valign='top' nowrap>
			<font size="1" face="Geneva, Arial, Helvetica, san-serif">Cartão Clube do Lar</font>
			<br>
			<input class="frm" type="text" name="nota_fiscal"  size="10"  maxlength="8"  value="<? echo $cartao_clube ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Entre com o número da Nota Fiscal.');" <? if (strlen($locacao) > 0) echo "readonly"; ?>>
			</td>
		<? } ?>
