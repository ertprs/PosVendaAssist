...
//A pedido da Latinatec  - Modificado por Fernando
	if($login_fabrica == 15 AND $xconsumidor_cpf == 'null'){
		$msg_erro = 'Entre com o CPF do consumidor';
	}
...
function recuperardados() {
	var produto_referencia = document.frm_os.produto_referencia.value;
	produto_referencia = trim(produto_referencia);
	var fabrica = "<? echo "$login_fabrica"; ?>" ;

	if (produto_referencia.length>3){
		var ajax = new AJAX();
		ajax.Updater("ajax_produto_referencia.php?produto_referencia="+ produto_referencia +"&fabrica="+fabrica,"conteudo","get","carregando os dados...");
	}
}
...
?>
		...
		<form style="MARGIN: 0px; WORD-SPACING: 0px" name="frm_os" method="post" action="<? echo $PHP_SELF ?>" onSubmit="return false">
...
?>
		...
		<? if ($pedir_sua_os == 't') { ?>	
			...
			<td nowrap  valign='top'>
				...
				<? }else{ ?>
				<input class="frm" type="text" name="produto_referencia" size="15" maxlength="20" value="<? echo $produto_referencia ?>" onKeyUp="recuperardados()" onblur="this.className='frm'; displayText('&nbsp;');checarComunicado(<? echo $login_fabrica ?>);" onfocus="this.className='frm-on'; displayText('&nbsp;Entre com a referência do produto e clique na lupa para efetuar a pesquisa.');" <? if (strlen($locacao) > 0) echo "readonly"; ?>>&nbsp;<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick="javascript: fnc_pesquisa_produto (document.frm_os.produto_referencia,document.frm_os.produto_descricao,'referencia',document.frm_os.produto_voltagem) "  style='cursor: hand'>
				<? } ?>
				...
				<BR>
				<div id="conteudo" style="position:absolute; border: 1px solid #949494;background-color: #f4f4f4;">
				</div>
				<!--border: 1px solid #949494;background-color: #f4f4f4; -->
			</td>
						<td nowrap  valign='top'>
				<?
				if ($login_fabrica == 3) {
...
?>
			...
			<? if ($login_fabrica <> 6){ ?>
				...
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">N. Série</font>
				<br>
				<input class="frm" type="text" name="produto_serie" size="8" maxlength="20" value="<? echo $produto_serie ?>" onblur="this.className='frm'; displayText('&nbsp;'); MostraEsconde('dados_1');" onfocus="this.className='frm-on'; MostraEsconde('dados_1'); displayText('&nbsp;Digite aqui o número de série do aparelho.'); "><br><font face='arial' size='1'><? if ($login_fabrica == 1) echo "(somente p/ linha DeWalt)"; ?></font>
				<div id='dados_1' style='position:absolute; display:none; border: 1px solid #949494;background-color: #f4f4f4;'>
				Mascara: LLNNNNNNLLNNL<br>
				L: Letra<BR>
				N: Número
				</div> 
