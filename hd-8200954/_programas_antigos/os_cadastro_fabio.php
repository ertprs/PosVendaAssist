...
/*if($ip == "201.26.21.165"){
$teste1 = $_POST["consumidor_cidade"] ;
$teste2 = $_POST['consumidor_cidade'] ;
...
if(($login_posto=='1550') and ($login_fabrica==2)){ // liberado 03/05 9:32 hd 1454
include 'os_cadastro_tudo.php';
exit;
}
...
if($login_fabrica==1){
include 'os_cadastro_black_antiga.php';
exit;
}
/*
...
/*takashi HD 931  21-12*/
	...
	if (strlen($revenda_cnpj) <> 0 AND strlen($revenda_cnpj) <> 14) {
		if($sistema_lingua <> "ES") $msg_erro .= " Tamanho do CNPJ da revenda inválido.";
	}
...
//pedido por Leandro Tectoy, feito por takashi 02/08 Alterado por Raphael para a bosch tb
	if($login_fabrica==6 OR $login_fabrica==20){
		if (strlen ($_POST['data_nf']) == 0) {
			if($sistema_lingua == "ES") $msg_erro = "Digite la fecha de compra";
...
//pedido por Leandro tectoy, feito por takashi 04/08
	...
	if (strlen($_POST['consumidor_revenda']) == 0){
		...
		if (strlen($produto_referencia) > 0 AND (strlen($xproduto_serie) == 0 OR $xproduto_serie == 'null')) {
			...
			if (pg_numrows($res) > 0) {
				...
				if ($numero_serie_obrigatorio == 't') {
					if($sistema_lingua == "ES") echo "Nº de série $produto_referencia es obligatório";
					else $msg_erro .= "<br>Nº de Série $produto_referencia é obrigatório.";
				}
			}
		}
	}
...
//BOSCH - REGRAS DE VALICAÇÃO - RAPHAEL GIOVANINI
	if($login_fabrica==20 AND ($tipo_atendimento==14)){
		...
		if($bosch_data_abertura >$bosch_nf_data)
			if($sistema_lingua == "ES") $msg_erro =  "Plazo de garantía de reparo caducado";
...
#	if ($login_fabrica == 1 AND strlen($consumidor_cpf) == 0) $msg_erro .= 'Tamanho do CPF/CNPJ do cliente inválido';
	...
	if (@pg_numrows ($res) == 0) {
if($sistema_lingua == "ES") $msg_erro =  "Producto (producto) no catastrado";
...
$title = "Cadastro de Ordem de Serviço '$login_pais";
...
if ($digita_os == 'f') {
if($sistema_lingua == "ES") echo "<h4>Sin permisión de acceso</h4>";
			...
			if (http[curDateTime].readyState == 4) 
			{
				if (http[curDateTime].status == 200 || http[curDateTime].status == 304) 
				{
					...
					if (response=="ok"){
						...
						document.frm_os.link_comunicado.value="<? if($sistema_lingua == "ES"){ 

echo "EXISTE COMUNICADOS PARA ESTE PRODUCTO. CLICK AQUÍ PARA LEER ";
}else{
...
?>
		...
		<? if($sistema_lingua) echo "Tipo de atendimiento";else echo "Tipo de Atendimento";?>
...
//BOSCH 
		...
		<tr valign='top'>
			<td nowrap>
				...
				<br>
				<? if ($login_fabrica == 5) {?>
					<input  name ="sua_os" class ="frm" type ="text" size ="10" maxlength="6" value ="<? echo $sua_os ?>" onblur = "this.className='frm'; displayText('&nbsp;');" onfocus ="this.className='frm-on';displayText('&nbsp;Digite aqui o número da OS do Fabricante.');">
				<?} else {?>
...
/*visibility:<? if ($arquivo_comunicado) echo "visible;";else echo "hidden;"; ?>*/
				...
				<input class="frm" type="text" name="produto_referencia" size="15" maxlength="20"
					...
					onfocus="this.className='frm-on'; displayText('&nbsp;<? if($sistema_lingua == "ES") echo "Informe la referencia del producto y haga um click em la lupa para pesquisar";else echo "Entre com a referência do produto e clique na lupa para efetuar a pesquisa.";?>');" <? if (strlen($locacao) > 0) echo "readonly"; ?> <?if($desabilita)echo " readonly";?>>&nbsp;
...
//				if (strlen($data_abertura) == 0 and $login_fabrica <> 1) $data_abertura = date("d/m/Y"); 
				echo $data_abertura; 
?>" <?if($login_fabrica==20)echo "readonly ";?> type="text" class="frm" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese com la fecha de abertura de la OS";else echo "Entre com a Data da Abertura da OS.";?>'); " tabindex="0"><br><font face='arial' size='1'>Ex.: <? echo date("d/m/Y"); ?></font>
			</td>
			<? if ($login_fabrica <> 6){ ?>
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "N. Serie";else echo "N. Série";?></font>
				<br>
				<input class="frm" type="text" name="produto_serie" size="8" maxlength="20" value="<? echo $produto_serie ?>" onblur="this.className='frm'; displayText('&nbsp;');<? if($login_fabrica==3 and $login_posto==6359){ echo " MostraEsconde('dados_1');";} ?>" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Digite el número de serie del producto";else echo "Digite aqui o número de série do aparelho.";?>');<? if($login_fabrica==3 and $login_posto==6359){ echo " MostraEsconde('dados_1');";} ?> "><br><font face='arial' size='1'><? if ($login_fabrica == 1) echo "(somente p/ linha DeWalt)"; ?></font>
...
?>
		...
		<tr>
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Nombre consumidor ";else echo "Nome Consumidor";?></font>
				<br>
				<input class="frm" type="text" name="consumidor_nome"   size="30" maxlength="50" value="<? echo $consumidor_nome ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese el nobre sel cliente";else echo "Insira aqui o nome do Cliente.";?>');">&nbsp;<!--<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_consumidor (document.frm_os.consumidor_nome, "nome")'  style='cursor: pointer'>-->
			</td>
			<td>
						
				<? if ($login_fabrica <> 19) { ?>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Identificación del consumidor";else echo "CPF/CNPJ do Consumidor";?></font>
				<br>
				<input class="frm" type="text" name="consumidor_cpf"   size="17" maxlength="18" value="<? echo $consumidor_cpf ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Digite la Identificación del cliente. Puede ser digitada directamente o separado com pontos y trazos";else echo "Digite o CPF do consumidor. Pode ser digitado diretamente, ou separado com pontos e traços.";?>');">&nbsp;<!--<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_consumidor (document.frm_os.consumidor_cpf,"cpf")'  style='cursor: pointer'>-->
			<? }else{
			...
			<td>
				...
				<br>
				<input class="frm" type="text" name="consumidor_fone"   size="15" maxlength="20" value="<? echo $consumidor_fone ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese el teléfono con código de area ";else echo "Insira o telefone com o DDD. ex.: 14/4455-6677.";?>');">
			</td>
		</tr>
		</table>
...
#			echo "<!-- ";
		...
		<tr valign='top'>
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Nombre distribuidor";else echo "Nome Revenda";?></font>
				<br>
				<input class="frm" type="text" name="revenda_nome" size="30" maxlength="50" value="<? echo $revenda_nome ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Digite el nombre del distribuidor donde el produto fue comprado";else echo "Digite o nome da REVENDA onde foi adquirido o produto.";?>');">&nbsp;<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_revenda (document.frm_os.revenda_nome, "nome")' style='cursor: pointer'>
			</td>
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "ID distribuidor";else echo "CNPJ Revenda";?></font>
				<br>
				<input class="frm" type="text" name="revenda_cnpj" size="20" maxlength="18" value="<? echo $revenda_cnpj ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese el número del catastro nacional de persona juridica";else echo "Insira o número no Cadastro Nacional de Pessoa Jurídica.";?>'); " <?if($sistema_lingua <> "ES") echo "onKeyUp=\"formata_cnpj(this.value, 'frm_os')\"";?>>&nbsp;<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_revenda (document.frm_os.revenda_cnpj, "cnpj")' style='cursor: pointer'>
			</td>
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Factura comercial";else echo "Nota Fiscal";?></font>
				<br>
				<input class="frm" type="text" name="nota_fiscal"  size="8"  maxlength="8"  value="<? echo $nota_fiscal ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese el número de factura comercial";else echo "Entre com o número da Nota Fiscal.";?>');" <? if (strlen($locacao) > 0) echo "readonly"; ?>>
			</td>
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Fecha compra";else echo "Data Compra/NF";?> </font>
				<br>
				<input class="frm" type="text" name="data_nf"    size="12" maxlength="10" value="<? echo $data_nf ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese con la fecha de compra. Verifique si el producto esta en garantía";else echo "Entre com a data da compra. Verifique se o produto está dentro do PRAZO DE GARANTIA.";?>');" tabindex="0" <? if (strlen($locacao) > 0) echo "readonly"; ?>><br><font face='arial' size='1'>Ex.: <? echo date("d/m/Y"); ?></font>
			</td>
		</tr>
		</table>
		<?
...
#			echo " -->";
		...
		<tr>
			...
			if ($login_fabrica <> 19) {
				...
				if($sistema_lingua=="ES") echo "Consumidor";else echo "Consumidor";
				...
				if($login_fabrica <> 1){
					...
					if($sistema_lingua=="ES") echo "o";else echo "ou";
			...
			<td>
				...
				<br>
				<input class="frm" type="text" name="acessorios" size="30" value="<? echo $acessorios ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;<?if($sistema_lingua == "ES") echo "Ingrese con la fecha de compra. Verifique si el producto esta en garantía";else echo "Texto livre com os acessórios deixados junto ao produto.";?>');">
			</td>
		<? } 
		}?>
<? if ($login_fabrica == 1 
