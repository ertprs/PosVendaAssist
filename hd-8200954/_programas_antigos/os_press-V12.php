    ...
    if ($resultado==1){
        ...
        if ($status_os==62){
            echo "<br>
                <center>
                <div style='font-family:verdana;border:1px solid #D3BE96;width:700px;align:center;background-color:#FCF0D8' align='center'>
                    <b style='font-size:14px;color:red;width:100%'>OS Sob Intervenção da Assistência da Fábrica</b><br>
                    <b style='font-size:11px'>$status_observacao</b>
                </div>
                </center><br>
            ";
...
if ($login_fabrica==3 AND $retorno==1 AND strlen($nota_fiscal_envio)==0){
    $sql_status = "SELECT    status_os,
                observacao
                FROM tbl_os_status
                WHERE os=$os
...
?></TD>
	...
	if ($login_fabrica==3 AND ( $bloqueada_pc=='t' OR $retorna_conserto='t')){
		if ($status_os=='72' AND $servico_realizado_peca=='20')
			$msg_peca_intervencao=" <b style='font-weight:normal;color:#FF6666;font-size:10px'>(aguardando autorização da fábrica)</b>";
		if ($status_os=='73' AND $servico_realizado_peca=='96'){
			$msg_peca_intervencao=" <b style='font-weight:normal;color:#CC0000;font-size:10px'>(pedido cancelado pela fábrica)</b>";
			$cancelou_peca = "sim";
		}
		if ($status_os=='73' AND $servico_realizado_peca=='20'){
			$msg_peca_intervencao=" <b style='font-weight:normal;color:#333333;font-size:10px'>(autorizado pela fábrica)</b>";
			$cancelou_peca = "sim";
		}
		if ($status_os=='62' AND $servico_realizado_peca=='20'){
			$msg_peca_intervencao=" <b style='font-weight:normal;color:#FF6666;font-size:10px'>(aguardando autorização da fábrica)</b>";
		}
		if ($status_os=='64' AND $servico_realizado_peca=='20'){
			$msg_peca_intervencao=" <b style='font-weight:normal;color:#333333;font-size:10px'>(autorizado pela fábrica)</b>";
			$cancelou_peca = "sim";
		}
		if ($status_os=='64' AND $servico_realizado_peca=='96'){
			$msg_peca_intervencao=" <b style='font-weight:normal;color:#CC0000;font-size:10px'>(pedido cancelado pela fábrica)</b>";
			$cancelou_peca = "sim";
		}
		}
		...
		if (strtolower($nf) <> 'pendente'){
			...
			if (pg_numrows ($resY) > 0) {
				...
				echo "<acronym title='Pendente com o fabricante.' style='cursor:help;'> $nf &nbsp;";
			}
		}
		?>
		</TD>
