...
if (strlen ($tipo) > 0 AND strlen ($comunicado) == 0) {
	...
	if ($login_fabrica == 14) $sql .= "AND tbl_produto.familia in ($familia_posto) ";
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
		...
		echo "<td nowrap>";
		echo "<font size='-1'>";
		if ($produto <> $produto_ant) echo pg_result ($res,$i,referencia) . " - " . pg_result ($res,$i,descricao_produto) ; 
