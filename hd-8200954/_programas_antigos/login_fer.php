...
if ($login_fabrica==24 and $login_posto <> 11722 and $login_posto <> 2252 and $login_posto <> 6359){
...
if (pg_numrows ($res) > 0 ) {
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
		...
		$sql2 = " SELECT posto FROM tbl_posto_linha
					WHERE posto = $login_posto 
					AND   distribuidor = '4311' limit 1 ";
		$res2 = pg_exec($con, $sql2);
		if(pg_numrows($res2) > 0){
			$posto_linha = pg_result($res2,0,0);
		}

//Adicionado o IF ELSE para o comunicado da britania de devolução de pecas em garantia.
//Modificado por Fernando. 22/11/2006
		if($login_fabrica == 3 AND $comunicado == '6521' AND $posto_linha == $login_posto){
...
if($login_fabrica==19 OR $login_fabrica==3 OR $login_fabrica==24 OR $login_fabrica==20 OR $login_fabrica==15 OR $login_fabrica==11 OR $login_fabrica==1 OR $login_fabrica==2 OR $login_fabrica==10 OR $login_fabrica==14) {
