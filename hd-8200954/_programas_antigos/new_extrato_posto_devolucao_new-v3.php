...
//echo "<br><br>NOTA: $total_nota |  $base_icms |  $valor_icms |  $base_ipi |  $valor_ipi | qtde=$qtde_peca_na_nota<br><br>";
		...
		$sql = "INSERT INTO tbl_faturamento		  
				(fabrica, emissao,saida, posto, distribuidor, total_nota, nota_fiscal, serie, natureza, base_icms, valor_icms, base_ipi, valor_ipi, extrato_devolucao, obs)
				VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',4311,$login_posto,$total_nota,'$nota_fiscal','2','Devolução de peças em garantia', $base_icms, $valor_icms, $base_ipi, $valor_ipi, $extrato, 'Devolução de peças do posto para à  Fábrica')";
		$res = pg_exec ($con,$sql);
...
//			$qtde_restatante     = pg_result ($resX,$x,qtde_restatante);
			...
			if ($peca_ant!=$peca){
				$qtde_acumulada = $qtde_pedente_temp;
			}else{
				$qtde_acumulada += $qtde_pedente_temp;
			}
			...
			if ($qtde_acumulada>$qtde_pedente_temp_AUX){
				$X = $qtde_pedente_temp;
				$qtde_pedente_temp=$qtde_acumulada-$qtde_pedente_temp_AUX;
			//	$qtde_acumulada -=  $X;
			}
			if ($qtde_pedente_temp+$qtde_acumulada>$qtde_pedente_temp_AUX AND $peca_ant==$peca){
			//	continue;
			//	$z--;
			}
...
#####################################################################################################################
	...
	if ($numero_linhas==5000){
		...
		if ($tem_mais_itens=='nao'){
			echo "<b>Não há mais peças para devolução.<br><br> <a href='new_extrato_posto_devolucao_new_itens.php?extrato=$extrato'>Clique aqui para consultar as notas de devolução</a></b>";
		}else{
