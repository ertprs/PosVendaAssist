...
if(strlen($msg)>0){
	...
	echo "<tr >";
	echo "<td bgcolor='FFFFFF' width='60' align='top'><img src='imagens/cadeado1.jpg' align='absmiddle'></td><td  class='Mensagem' bgcolor='FFFFFF' align='left'>$msg<br>Para restringir o acesso novamente, sair do sistema. Caso contrario o EXTRATO ficará com o acesso liberado!<br><center><a href='os_extrato_senha.php?acao=alterar'>Alterar Senha</a> &nbsp; - &nbsp; <a href='os_extrato_senha.php?acao=libera'>Liberar tela</a></center>";
	echo "</td>";
	echo "</tr>";
	echo "</table><br>";
...
//foi adicionado a pedido do natanael pois os valores do extrato estão errados.
if($login_fabrica == 19 and $login_posto <> 13663 and 1==2 ){
	...
	if (pg_numrows ($resX) > 0) {
		...
		echo "<tr>";
		echo "<TD><B>A tela de extrato está em manutenção no momento.</B></TD>";
		echo "</tr>";
		echo "<tr>";
		echo "<TD align='center'><B>Retornará em alguns instantes. Obrigado</B></TD>";
		echo "</tr>";
		echo "</table>";
	}
	exit ;
...
if($login_fabrica==11){
	...
	echo "</tr>\n";
	echo "<tr class='table_line'>\n";
	echo "<td align=\"center\"><B>Descrição da Nota Fiscal:</b><BR>
	*Serviços prestados em aparelhos de sua comercialização, sob garantia durante o mês de Outubro de 2006.*<BR>
	A N.F.deverá estar na Empresa até dia 06/11/06<BR>
	Devido Feriado dia 02/11/06, o pagamento será dia 09/11/06.</td>\n";
	echo "</tr>\n";
	echo "</table>";
}
if ($login_fabrica == 3) {
...
if($login_fabrica==19){
	...
	echo "<tr>";
	echo "<td align='center'><font size=1>* NF s e Recibos que chegarem entre os dias 20 a 05, serão pagos dia 10;<BR>
  NF s e Recibos que chegarem entre os dias 06 a 20, serão pagos dia 25;<BR>
...
#			    }
			...
			if ($login_fabrica == 1){
				...
				echo "<td><a href='os_extrato_detalhe.php?extrato=$extrato&posto=$posto'><img src='imagens/btn_detalhar_".$btn.".gif'></a></td>\n";
				if ($login_fabrica == 14) {
					echo "<td nowrap><a href='os_extrato_pecas_retornaveis.php?extrato=$extrato'>Peças Trocadas</a></TD>\n";
				}else{
					echo "<td><a href='os_extrato_pecas_retornaveis.php?extrato=$extrato'><img src='imagens/btn_pecasretornaveis_".$btn.".gif'></a></TD>\n";
				}
			}
