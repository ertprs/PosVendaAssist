...
if(($login_posto=='6359')and ($login_fabrica==6)){
...
if($login_fabrica ==20){
	...
	if($login_fabrica==20 AND ($tipo_atendimento==15 OR $tipo_atendimento==16)){
		$aux_data = explode('-',$xdata_abertura);
		$data_serv  = mktime (date("H"), date("i"), date("s"),$aux_data[1], $aux_data[2], $aux_data[0]);
		$data_serv= strftime ("%d/%m/%Y", $data_serv) ;
		echo $data_serv."<br>$aux_data[0]<br>$aux_data[1]";
	}
