...
$sql .= "GROUP BY tbl_os.posto                                                     ,
		...
		(tbl_os.mao_de_obra + tbl_familia.mao_de_obra_adicional_distribuidor) ,
		...
		to_char(tbl_extrato_extra.baixado,'DD/MM/YYYY')   ,
		tbl_os_extra.os_reincidente                                      ,
		tbl_extrato.protocolo                                            ,
		tbl_extrato_extra.obs, tbl_os_status.status_os 
...
//if ($ip=='201.43.28.101'){ echo $sql; exit;}
$res = pg_exec ($con,$sql);
...
if ($totalRegistros > 0){
	...
	for ($i = 0 ; $i < $totalRegistros; $i++){
		...
		$status_da_os           = trim(pg_result ($res,$i,status_da_os));
		if ($login_fabrica == 19) {
	...
	if ($login_fabrica == 19) {
	    ...
	    echo "<tr class='table_line'>\n";
		if($login_fabrica==20)
		echo "<td colspan=\"3\" align=\"center\" style='padding-right:10px'><b>TOTAL (MO + Peças)</b></td>\n";
