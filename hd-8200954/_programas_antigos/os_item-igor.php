...
if($login_fabrica==1 and $compressor=='t'){
...
//echo "$xdata - A$data_abertura";
	...
	if((strlen($xxdata)>0) and (strlen($xhora_chegada_cliente)>0) and (strlen($xhora_saida_cliente)>0) and (strlen($xkm_chegada_cliente)>0) and (strlen($xos_visita)==0) and (strlen($msg_erro)==0)){
...
/*			if ($login_fabrica == 5 and strlen($causa_defeito) == 0)
			...
			//Samuel HD 1256 qdo intelbras e for solução troca de placa, na serviço realizado também tem que ser troca de placa
			if($login_fabrica==14 and $solucao_os == 83 and strlen($xpeca) > 0 and $xservico <> 83)  $msg_erro .= "O Serviço realizado também deve ser troca de placa!";

			//Samuel HD 1754 qdo intelbras e for solução reset, na serviço realizado também tem que ser troca de placa
			/*
			if($login_fabrica==14 and ($solucao_os == 79 or $solucao_os == 81) and strlen($xpeca) > 0 )  $msg_erro .= "O Serviço realizado não aceita digitação de peças !";
			if($login_fabrica==14 and ($solucao_os <> 79 and $solucao_os <> 81) and strlen($xpeca) == 0 )  $msg_erro .= "É obrigatório a digitação de peças !";
			*/
						...
						if (strlen ($msg_erro) > 0) {
							...
							if (strlen($xpeca) > 0) {
								...
								if($login_fabrica == 14){//chamado = 1636 as vezes vem a referencia outras o codigo do banco para a Intelbras.
									...
									if (pg_numrows($resp) > 0) $encontrou = 't';
									else $encontrou = 'f';
...
//echo "1: $sql";
										...
										if ($login_fabrica==3  AND 1==1 AND $xservico=="20" AND ($intervencao_fabrica_peca=='t' OR $troca_obrigatoria_peca=='t') ){ // 20 trocar depois
							// envia email teste para avisar
							$email_origem  = "fabio@telecontrol.com.br";
							$email_destino = "fabio@telecontrol.com.br";
							$assunto       = "PROCESSO MP3 - OS ITEM COM INTERNVECAO cadastrada";
							$corpo.="<br>OS: $os \n";
							...
							// fim
											...
											if ($troca_obrigatoria_peca=='t'){
												$sql = "INSERT INTO tbl_os_status (os,status_os,data,observacao) values ($os,62,current_timestamp,'Peça da O.S. com troca obrigatória')";
												$msg_intervencao .= "<br>A peça $xpeca precisa de Intervenção da Assistência Técnica da Fábrica. Aguarde o contato da 	fábrica";	
											}
											else {
												...
												$msg_intervencao .= "<br>O produto e a peça $xpeca precisa de Intervenção da Assistência Técnica da Fábrica. Aguarde o contato da fábrica";
											}
...
//takashi 03-01-06 HD 934
		...
		//quando solução for troca de placa, deve existir pelo menos 1 serviço realizado como: troca de placa
		if($login_fabrica==14 and $solucao_os == 83  ){
			$x_solucao_os = $_POST['solucao_os'];
			if (strlen($x_solucao_os) == 0) $x_solucao_os = 'null';
			else                            $x_solucao_os = "'".$x_solucao_os."'";
			...
			SELECT os_item 
				...
				WHERE servico_realizado = 83 and os = $os";
			$res_solucao = @pg_exec($con,$sql);
			if(strlen(pg_errormessage($con))==0){
				if(pg_numrows($res_solucao)==0)
					$msg_erro .= "TESTE-O Serviço realizado deve ser troca de placa! sol:$solucao_os sol2: $x_solucao_os";
			}
		}

		//quando solução for troca de componente, nao pode existir o serviço troca de componente
		if($login_fabrica==14 and $solucao_os == 82 ){
...
<?
if($login_fabrica==3  AND 1==1 AND strlen($msg_intervencao)<0){ // para nao mostrar - fabio
...
?>
			...
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Cliente";else echo "Consumidor";?></font>
				<br>
				<font size="2" face="Geneva, Arial, Helvetica, san-serif">
			...
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Producto";else echo "Produto";?></font>
				<br>
				<font size="2" face="Geneva, Arial, Helvetica, san-serif">
			...
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "N. Série";else echo "N. Série";?></font>
				<br>
				<font size="2" face="Geneva, Arial, Helvetica, san-serif">
		...
		<tr>
			...
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">
					<?
					if($login_fabrica=='20'){
						if($sistema_lingua) echo "Reparación";else echo "Reparo";
					}else {
...
//Modificado por Fernando
			...
			if ($login_fabrica == 19 and $tipo_atendimento==3){
					$sql = "SELECT tbl_defeito_constatado.* 
							FROM tbl_defeito_constatado 
							WHERE fabrica = $login_fabrica and defeito_constatado=10021";
			}//hd1414 takashi 07-03-07
				#--------- Bosch ----------
				if ($login_fabrica == "20") {
					$sql = "SELECT tbl_defeito_constatado.* 
...
//if ($ip=="201.26.143.50") echo $sql;
			...
			<td nowrap>
				...
				?>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif"><? if($sistema_lingua) echo "Defecto";else echo "Defeito";?></font>
				<br>
...
<?
					...
					for ($i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
						...
						//--=== Tradução para outras linguas ============================= Raphael HD:1212
						$sql_idioma = "SELECT * FROM tbl_causa_defeito_idioma WHERE causa_defeito = ".pg_result ($res,$i,causa_defeito)." AND upper(idioma) = '$sistema_lingua'";
			...
			<td align="left" nowrap>
				...
				<?
					if($login_fabrica<>20) {echo "Solução";}
						...
						if($sistema_lingua) echo "Identificación";else echo "Identificação";
					}
				?>
				</font>
				<br>
...
//					if ($ip == "201.0.9.216") { echo nl2br($sql);}
					...
					for ($x = 0 ; $x < pg_numrows($res) ; $x++ ) {
						...
						$descricao_d = pg_result ($res,$x,descricao);

						//--=== Tradução para outras linguas ============================= Raphael HD:1212
						$sql_idioma = "SELECT * FROM tbl_servico_realizado_idioma WHERE servico_realizado = ".pg_result ($res,$x,servico_realizado)." AND upper(idioma) = '$sistema_lingua'";
...
#		if (strlen($faturado) > 0) $loop = $qtde_item - $faturado;
		...
		for ($i = 0 ; $i < $loop ; $i++) {
			...
			//--=== Tradução para outras linguas ============================= Raphael HD:1212
			if(strlen($peca[$i])>0 AND strlen($sistema_lingua)==2){
...
<tr>
	<td height="27" valign="middle" align="center" colspan="3" bgcolor="#FFFFFF">
		<br>
		<FONT SIZE="1"><?if($sistema_lingua) echo "Observación";else echo "Observação:";?></FONT> <INPUT TYPE="text" NAME="obs" value="<? echo $obs; ?>" size="70" maxlength="255" class="frm">
		<br><br>
		<FONT SIZE="1" COLOR="#ff0000"><?if($sistema_lingua) echo "El campo \"Observación\" es solamente para el control del servicio autorizado<br>El fabricante no es responsable por los datos digitados";else echo " campo \"Observação\" é somente para o controle do posto autorizado. <br>O fabricante não se responsabilizará pelos dados aqui digitados.";?>O</FONT>
		<br><br>
	</td>
</tr>
			...
			echo '
				<script language="JavaScript">
					...
					for (i=0;i<tmp.length;i++){
						if (tmp[i].src=="http://www.telecontrol.com.br/assist/imagens/btn_lista.gif")
							tmp[i].style.visibility="hidden";
						if (tmp[i].src=="http://www.telecontrol.com.br/assist/imagens/btn_lupa.gif")
							tmp[i].style.visibility="hidden";
					}
				</script>
				';
