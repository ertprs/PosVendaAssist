...
Martello ï¿½ 2073 - 595
Penha ï¿½ 80039 - 1537
Janaï¿½na ï¿½ 80330 - 1773
Bertolucci - 80568 - 7080
Tecservi ï¿½ 80459 - 5037
NL ï¿½ 80636 - 13951
Telecontrol ï¿½ 93509 - 4311
A.Carneiro ï¿½ 1256 - 564
-----Gaslar ï¿½ 24091 - 1008----- nao mais
...
//echo "<br><br>NOTA: $total_nota |  $base_icms |  $valor_icms |  $base_ipi |  $valor_ipi | qtde=$qtde_peca_na_nota<br><br>";
 			...
 			$sql = "INSERT INTO tbl_faturamento		  
					...
					VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',13996,$login_posto,$total_nota,'$nota_fiscal','UN','Devolução de peças em garantia', $base_icms, $valor_icms, $base_ipi, $valor_ipi, $extrato, 'Devolução de peças do posto para à  Fábrica')";
			$res = pg_exec ($con,$sql);
...
//echo "<br>$peca | $peca_preco |	$peca_qtde_total_nf | $peca_total_item ";
				...
				for ($y = 0 ; $y < pg_numrows ($resNF) ; $y++) {
					...
					if ($qtde_peca_inserir < $peca_qtde_total_nf){
						...
						$peca_nota       = pg_result ($resNF,$y,nota_fiscal);
						$peca_qtde       = pg_result ($resNF,$y,qtde);
						$peca_peca       = pg_result ($resNF,$y,peca);
						$peca_preco      = pg_result ($resNF,$y,preco);
						$peca_aliq_icms  = pg_result ($resNF,$y,aliq_icms);
						$peca_base_icms  = pg_result ($resNF,$y,base_icms);
						$peca_valor_icms = pg_result ($resNF,$y,valor_icms);
						$peca_linha      = pg_result ($resNF,$y,linha);
						$peca_aliq_ipi   = pg_result ($resNF,$y,aliq_ipi);
						$peca_base_ipi   = pg_result ($resNF,$y,base_ipi);
						$peca_valor_ipi  = pg_result ($resNF,$y,valor_ipi);
						$sequencia       = pg_result ($resNF,$y,sequencia);
...
<TR>
	...
	Preencha esta coluna com as quantidades de peçaas que serão devolvidas
	</TD>
</TR>
</table>
<? } ?>
...
//			$qtde_restatante     = pg_result ($resX,$x,qtde_restatante);
			...
			if ($pecas_pendentes=='sims' and 1==2){
