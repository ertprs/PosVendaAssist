...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align='center'  class='Tabela'>
<TR>
    <TD colspan="<? if ($login_fabrica == 1) { echo "9"; }else{ echo "8"; } ?>" class='inicio'>&nbsp;DIAGNÓSTICOS - COMPONENTES - MANUTENÇÕES EXECUTADAS</TD>
</TR>
<TR>
...
<?
    ...
    //--=== Tradução para outras linguas ===================================================================
        ...
        if ($login_fabrica == 3 AND 1==2 ) {
			...
			//campo usado para mostrar hint na nota fiscal
			$hint = "";
                    ...
                    if (pg_numrows ($resx) > 0) {
                        ...
                        if (strlen ($distribuidor) > 0) {
                            if ($login_fabrica == 3 AND $distribuidor <> 4311) {
                                $condicao_01 = "tbl_faturamento_item.os     = $os";
                            } else {
                        ...
                        if (pg_numrows ($resx) > 0) {
                            if ($login_fabrica == 3 AND $distribuidor <> 4311) {
								...
								//verifica se quem foi atendido foi o distribuidor
								if (pg_result($resx,0,posto) <> $login_posto) {
									$hint = "<acronym title='Nota fiscal da fábrica para o distribuidor. Pendente com o distribuidor.' style='cursor:help;'>";
									$link = 0;
								} else {
									$hint = "<acronym title='Atendido pelo fabricante.' style='cursor:help;'>";
									$link    = 1;
								}
							} else {
								$nf   = trim(pg_result($resx,0,nota_fiscal));
...
<!--     <TD class="conteudo" style="text-align:left;"><? echo pg_result ($res,$i,referencia) . " - " . pg_result ($res,$i,descricao); ?></TD> -->
    ...
    if (strtolower($nf) <> 'pendente'){
        if ($link == 1) {
            echo "$hint <a href='nota_fiscal_detalhe.php?nota_fiscal=".$nf."&peca=$peca' target='_blank'>$nf</a>";
        }else{
            echo "$hint $nf ";
...
if ($login_fabrica == "20") {
    ...
    if($sistema_lingua=='ES'){
        echo "<td align='center' bgcolor='#E1EAF1'><b>";
        if($sistema_lingua=='ES')echo "Descuento IVA";else echo "Desconto IVA";
        echo "</b></td>";
    }
