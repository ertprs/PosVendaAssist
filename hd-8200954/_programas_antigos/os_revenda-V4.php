...
//if ($ip == '201.0.9.216') echo "SQL : $sql<BR><BR>";
		$res = @pg_exec ($con,$sql);
		$msg_erro = pg_errormessage($con);
...
//if ($ip == '201.0.9.216') echo "Itens (3 - $i): $qtde_item | $qtde_linhas<br><br>";
				...
				if ($login_fabrica == 19) {
					if (strlen($aux_qtde) == 0) $msg_erro = "Favor indicar quantidade de produtos";
				}else{
...
//if ($ip == '201.0.9.216') echo "SQL ($i): $sql<br><br>";
						...
						if (pg_numrows($resZ) > 0) {
							...
							if (strlen($xxxextrato) == 0) {
								$msg_erro_serie .= "Nº de Série $produto_serie digitado é reincidente.<br>
								Favor consultar a ordem de serviço $xxxsua_os e acrescentar itens.<br>
...
//if ($ip == '201.0.9.216') echo "Referencia ($i): $referencia + erro: $msg_erro<br><br>";
					...
					if (strlen ($referencia) > 0) {
						...
						if (strlen($capacidade) == 0)	$xcapacidade = 'null';
						else							$xcapacidade = "'".$capacidade."'";

						
						
						if(strlen($aux_nota_fiscal)==0) $aux_nota_fiscal=$nota_fiscal;
						if (strlen ($msg_erro) == 0) {
...
//if ($ip == '201.0.9.216') echo "Insert item $i : $sql<BR><BR>";
