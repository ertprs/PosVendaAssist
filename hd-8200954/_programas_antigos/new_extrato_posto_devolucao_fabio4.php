...
if (strlen($_GET['extrato'])>0 AND strlen($_GET['linha'])>0 AND strlen($_GET['distrib'])>0 AND strlen($_GET['serie'])>0 AND strlen($_GET['pa'])>0 AND strlen($_GET['proximo'])>0 AND strlen($_GET['extrato_devolucao'])>0){
	...
	if (@pg_numrows($res) > 0) {
		...
		$total_item	= number_format($preco * $qtde,2,",",".");
		$nota_fiscal	= pg_result ($res,$x,nota_fiscal);
		$faturamento	= pg_result ($res,$x,faturamento);

		$resX = pg_exec ($con,"BEGIN TRANSACTION");
		
		$sql_update = "UPDATE tbl_faturamento_item SET extrato_devolucao = $extrato_devolucao WHERE faturamento_item = $faturamento_item";
...
Ajax.FecharNota = function (){
	if(Ajax.CheckReadyState(Ajax.request))	{
		...
		if (pecas.length==0){
			...
			alert("Por favor, adicione no mínimo 1 item para a nota de devolução. \n\nClique sobre o botão 'Adicionar mais um item' para incluir o item na nota");
			return true;
		}
	//}
	}
...
function proximo_item(extrato,extrato_devolucao,pa,linha,distrib,serie,tabela){

	id_tabela=tabela;
	Ajax.Request('new_extrato_posto_devolucao_fabio4.php?proximo=item&extrato='+extrato+'&extrato_devolucao='+extrato_devolucao+'&pa='+pa+'&linha='+linha+'&distrib='+distrib+'&serie='+serie, Ajax.Response);
}
...
function adicionaTotal(base_icms, valor_icms, base_ipi, valor_ipi, total) {
   ...
   tabela.setAttribute('width','600px');
...
function concluir_nota(extrato,extrato_devolucao,id_campo_nota,id_total){
	...
	Ajax.Request('new_extrato_posto_devolucao_fabio4.php?fechar_nota=nota&extrato='+extrato+'&extrato_devolucao='+extrato_devolucao, Ajax.FecharNota);
}
...
<TR>
	...
	Para o preenchimento das notas de devolução, primeiro preencha o cabeçalho da nota e em seguinda clique sobre o botão 'Adicionar mais um item' para adicionar os itens.<br>
	...
	<br>Siga este procedimento para todas a notas.<br><br>
	</TD>
	<TD width="19" class="table_line" style="text-align: left;">&nbsp;</TD>
</TR>
</table>
...
if (pg_numrows ($res) > 0) {
	...
	for ($i=0; $i < $qtde_for; $i++) {
		...
		echo "<table border='1' cellspacing='0' cellpadding='3' border-color='#000' style='border-collapse:collapse;font-size:12px' width='600' id='tbl_pecas_$i'>";
		echo "<thead>";
...
// 		echo "</tfoot>";
		...
		if ($nota_fiscal==0){
			...
			echo "<input type='button' id='next' value='+ Adiconar mais um item' id='next' name='next' onclick=\"javascript:this.disabled=true;proximo_item('$extrato','$extrato_devolucao','$produto_acabado','$linha','$distribuidor','$serie','tbl_pecas_$i');return false;\">";
			echo "</td>";
			echo "</tr>";
			echo "</tfoot>";
		}
