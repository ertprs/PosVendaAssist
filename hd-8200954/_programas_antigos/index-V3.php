...
if ( $botao == "Enviar"  OR $botao == "entrar" OR $hd=="OK") {
	...
	//--========================================================================--\\
		...
		if(count($tempemail)==2){
			...
			$sql = " SELECT posto_empregado, loja
					 FROM tbl_posto
					 JOIN tbl_empregado on tbl_empregado.posto_empregado = tbl_posto.posto
					 WHERE tbl_posto.email = '$login'
			...
			if (pg_numrows ($res) == 1) {
				$imp_sql=$sql;
				$posto_empregado = pg_result ($res,0,posto_empregado);
				...
				if (pg_numrows ($res) == 1) {
					...
					setcookie ("cook_admin",$posto_empregado);
					setcookie ("cook_posto_empregado",$posto_empregado);
