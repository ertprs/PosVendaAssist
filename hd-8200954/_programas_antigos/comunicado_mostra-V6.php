...
if (strlen ($tipo) > 0 AND strlen ($comunicado) == 0) {
	...
	if ($login_fabrica == 14) $sql .= "AND (tbl_comunicado.familia in ($familia_posto) OR tbl_comunicado.familia IS NULL)";
	
	$sql .= " ORDER BY tbl_comunicado.data DESC, tbl_produto.referencia " ;
...
//	echo "<td align='center'><font color='#ffffff'><b>Data</b></font></td>";
	...
	for ($i=0; $i<$total; $i++) {
		...
		if ( ($produto <> $produto_ant) AND ($produto <> '') ) {
			if ($i > 0) {
				echo "</span>\n";
				echo "</td>\n";
				echo "</tr>\n\n\n";
		...
		if (file_exists($gif) == true) 
			echo "<img src='comunicados/$Xcomunicado.gif'>";
		if (file_exists($jpg) == true) 
			echo "<img src='comunicados/$Xcomunicado.jpg'>";
		if (file_exists($doc) == true) 
			echo "<a href='comunicados/$Xcomunicado.doc' target='_blank'>Abrir arquivo </a>";
		if (file_exists($rtf) == true) 
			echo "<a href='comunicados/$Xcomunicado.rtf' target='_blank'>Abrir arquivo </a>";
		if (file_exists($xls) == true) 
			echo "<a href='comunicados/$Xcomunicado.xls' target='_blank'>Abrir arquivo </a>";
		if (file_exists($pdf) == true) 
			echo "<a href='comunicados/$Xcomunicado.pdf' target='_blank'>Abrir arquivo </a>";
		if (file_exists($ppt) == true) 
			echo "<a href='comunicados/$Xcomunicado.ppt' target='_blank'>Abrir arquivo </a>";
		if (file_exists($zip) == true) 
			echo "<a href='comunicados/$Xcomunicado.zip' target='_blank'>Abrir arquivo </a>";
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			if(pg_result ($res,$i,tipo) == ''){
				echo "<td nowrap>";
				echo "<a href='$PHP_SELF?tipo=zero'> Sem titulo </a>";
				echo "</td>";
				echo "<td align='right'>";
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		...
		<td align="center" colspan="2">
			...
			<select class='frm' name='tipo'>
				...
				<option value='Estrutura do Produto'        <? if ($tipo == "Estrutura do Produto")   echo "SELECTED ";?>>Estrutura do Produto</option>
				<?if ($login_fabrica == 15){?>
