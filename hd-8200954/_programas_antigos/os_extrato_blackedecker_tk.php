...
if (pg_numrows($res) > 0) {
	...
	/*	$pendente           = trim(pg_result($res,$i,pendente));
		...
		AND confirmacao_pendente NOTNULL AND pendente NOTnull";
//echo "$ysql";
		$yres = pg_exec($con,$ysql);
		if(pg_numrows($yres)>0){
			echo "<a href='javascript: AbrirJanelaObs($extrato,\"pendencia\");'>Abrir</a>";
		}
		echo "</td>\n";
		echo "<td align='center' nowrap>";
		$ysql = "SELECT tbl_extrato_status.obs                   ,
		...
		FROM tbl_extrato_status
		WHERE tbl_extrato_status.extrato = $extrato
		AND confirmacao_pendente IS NULL AND pendente is null";
//echo "$ysql";
		$yres = pg_exec($con,$ysql);
		if(pg_numrows($yres)>0){
