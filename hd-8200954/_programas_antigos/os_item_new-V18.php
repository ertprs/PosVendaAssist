...
/*			if ($login_fabrica == 5 and strlen($causa_defeito) == 0)
							...
							if (strlen($xpeca) > 0) {
								...
								if (strlen ($msg_erro) == 0) {
									...
									if ($login_fabrica==3  AND $xservico=="20" AND $bloqueada_garantia_peca=='t'){ 
										...
										$sql = "INSERT INTO tbl_os_status (os,status_os,data,observacao) values ($os,72,current_timestamp,'Peça da O.S.  bloqueada para garantia')";
									...
									elseif ($login_fabrica==3  AND $xservico=="20" AND ($intervencao_fabrica_peca=='t' OR $troca_obrigatoria_peca=='t') ){ // 20 trocar depois
										...
										$email_origem  = "suporte@telecontrol.com.br";
										$email_destino = "nelson.antunes@britania.com.br, fabio@telecontrol.com.br";
	...
	if (strlen ($msg_erro) == 0) {
		...
		if ($bloqueada_garantia_os=='t') //no caso se tiver peça bloqueada para garantia, deve justificar.
