...
if ($login_fabrica==3){
    ...
    if ($resultado==1){
        ...
        if ($status_os==72){
            echo "<br>
                <center>
                <div style='font-family:verdana;border:1px solid #666666;padding:10px;width:500px;align:center' align='center'>
                    <b style='font-size:15px;color:red'>Pedido de Peça Necessita de Autorização</b><br>
...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align='center' class='Tabela'>
    ...
    <TR>
        ...
        if (strlen($solucao_os)>0){
            ...
            $sql_data = "SELECT SUM(data_digitacao - '2006-11-05')as total_dias FROM tbl_os WHERE os=$os";
            $resD = pg_exec ($con,$sql_data);
            if (pg_numrows ($resD) > 0) {
...
<?
	...
	<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align='center'  class='Tabela'>
	<TR>
		<TD colspan="<? if ($login_fabrica == 1) { echo "9"; }else{ echo "7"; } ?>" class='inicio'>&nbsp;DIAGNÓSTICOS - COMPONENTES - MANUTENÇÕES EXECUTADAS</TD>
	</TR>
	<TR>
	...
	for ($i = 0 ; $i < $total ; $i++) {
		...
		if ($login_fabrica==3 AND $bloqueada_pc=='t'){
...
if ($login_fabrica==3 AND $cancelou_peca=="sim") {
	$sql_status = "SELECT    status_os,
			observacao,
	...
	if ($resultado>0){
		for ($j=0;$j<$resultado;$j++){
			...
			if ($status_os==72){
				...
				echo "<TR>";
				echo "<TD colspan=7 class='inicio'>&nbsp;JUSTIFICATIVA DO POSTO PARA O PEDIDO DE PEÇA</TD>";
				echo "</TR>";
				echo "<TR>";
				echo "<TD  class='conteudo' width='30px' align='center'>&nbsp; $status_data &nbsp;</TD>";
				echo "<TD  colspan=6 class='conteudo'>&nbsp; $status_observacao</TD>";

				echo "</TR>";
				echo "</TABLE>";
				if ($j==0){ // ou seja se o ultimo status é o 72, entao para
					break;
				}
			}
			if ($status_os==73){
				if (strlen($status_observacao)>0){
					...
					echo "<TABLE width='700px' border='0' cellspacing='1' cellpadding='2' align='center'  class='Tabela'>";
					echo "<TR>";
					echo "<TD colspan=7 class='inicio'>&nbsp;JUSTIFICATIVA DA FABRICA PARA O PEDIDO DE PEÇA</TD>";
					echo "</TR>";
					echo "<TR>";
