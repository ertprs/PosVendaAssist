...
function validaRegrablack($condicao,$referencia,$posto,$linha) {
	$_GET['condicao'] = $condicao;
	$_GET['produto_referencia'] = $referencia;
	$_GET['posto'] = $posto;
	$_GET['linha_form'] = $linha;
	ob_start();
	include "black_valida_regras.php";
	ob_get_clean();
	$black_preco = number_format ($black_preco,2,".",".");
	return $black_preco;
}
	...
	if (strlen ($msg_erro) == 0) {
		...
		for ($i = 0 ; $i < $qtde_item ; $i++) {
			...
			if (strlen($peca_referencia)>0) {
				...
				$black_preco =  validaRegrablack($condicao,$peca_referencia,$login_posto,$i);
			}
			...
			if (strlen ($peca_referencia) > 0 AND (strlen($preco)==0 or $preco == '0,00')){
				$msg_erro = "Não foi encontrado preço para a Peça $peca_referencia ($preco). Favor pesquisar e colocar a quantidade novamente, caso persistir, entrar em contato com a black!";
				$linha_erro = $i;
				break;
			}
...
//	alert("item_pesquisa_ajax_black_test.php?q=" + conteudo + "&cache_bypass=$cache_bypass");
	$('#'+campo1).autocomplete("item_pesquisa_ajax_black.php?q=" + conteudo + "&cache_bypass="+engana, {
		minChars: 3,
		delay: 150,
		width: 350,
		scroll: true,
...
function importarPecas(campo) {
	...
	if (condicao.length==0) {
		...
		for (i = 0; i < array_lote.length ; i++){
			...
			url = 'black_valida_regras.php?linha_form=' + linha + '&posto=<?= $login_posto ?>&produto_referencia=' + referencia + '&condicao=' + condicao + '&tipo_pedido=' + tipo_pedido +'&cache_bypass=<?= $cache_bypass ?>';
...
function fnc_black_preco (linha_form) {
	...
	url = 'black_valida_regras.php?linha_form=' + linha_form + '&posto=<?= $login_posto ?>&referencia_peca=' + peca_referencia + '&condicao=' + condicao + '&tipo_pedido=' + tipo_pedido + '&qtde=' + qtde +'&cache_bypass=<?= $cache_bypass ?>';
			...
			//echo "<option value=''></option>";
			 if (strlen($cook_pedido)==0) {
				...
				if (pg_num_rows($res) > 0) {
					...
					echo "<option value=''>Sem condição</option>";
				}
			} else {
...
<? } ?>
		...
		if (strlen($cook_pedido)==0) {
			...
			echo "<select size='1' name='tipo_pedido' class='frm' id='tipo_pedido'>";
			...
			?>
				<tr bgcolor="<? echo $cor ?>" nowrap>
					...
					<td align='center'>
						...
						<? echo "onblur='javascript: fnc_black_preco ($i);$comando'"; ?> >
					</td>
					...
					<td align='center'>
						...
						<input class="frm" id="juro_<? echo $i ?>" type="hidden" name="juro_<? echo $i ?>"   value="<? echo $juro ?>" >
					</td>
