...
if($key1 == md5($id) AND $key2 == md5($id2)){
	if(strlen($id)>0 AND strlen($id2)>0 AND strlen($key1)>0 AND strlen($key2)>0 ){
		...
		#alterado Gustavo HD 3392
		#------------------------------------------------------------------------
		
		$sql = "select
				tbl_pessoa_fornecedor.pessoa   ,
				tbl_pessoa_fornecedor.empresa  ,
				tbl_pessoa_fornecedor.senha    ,
				...
				where tbl_pessoa_fornecedor.pessoa = &id AND cotacao_fornecedor = $id2";

		$res = pg_exec ($con,$sql);
			
		if (pg_numrows ($res) == 1) {
			$fornecedor          = pg_result ($res,0,pessoa);
			$empresa             = pg_result ($res,0,empresa);
			$cotacao_fornecedor  = pg_result ($res,0,cotacao_fornecedor);
			$cotacao_login       = pg_result ($res,0,login);
			$cotacao_senha       = pg_result ($res,0,senha);
			$hd = "OK";
		}
	}
}
