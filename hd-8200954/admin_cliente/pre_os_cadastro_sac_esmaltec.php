	...
	if (strlen($q) > 2) {
		...
		if ($tipo_busca == "posto") {
			...
			if ($busca == "codigo") {
				...
				$sql .= "  AND (
								tbl_posto.nome ilike '%$q%'
							OR  tbl_posto_fabrica.nome_fantasia ilike '%$q%'
								)";
...
$key_api = (strpos($_SERVER['HTTP_HOST'], '.net.br'))?	'ABQIAAAA58Y5NwUpOJR6Pos3XqtrxBSXzuw64REmAHFbLybXzpS0ysbbShRqfU4U8Ml9-PIIJrRfhec89KxBWA':
														'AIzaSyBo3-4NvcXx9vDeb8p2s2sCqVoNbjF0G88';
...
if (strlen($btn_acao) > 0) {
	...
	if (strlen($produto_referencia) > 0) {
		...
		$res = pg_query($con, $sql); 
		$msg_erro = (strlen(pg_last_error()) > 0) ? pg_errormessage($con) : "";
	...
	if (strlen($callcenter) == 0) {
		...
		*/
		...
		if ($login_fabrica == 59 AND strlen($herdar_x) > 0 AND strlen($hd_chamado_herdar_x) > 0 AND strlen($callcenter) <= 0 and empty($msg_erro)) {
		...
		if ( ($login_fabrica == 52 or $login_fabrica == 30 or $login_fabrica == 85) and empty($msg_erro) ) {
				...
				for ($w = 1; $w <= $qtde_produto; $w++) {
					...
					if (strlen($produto_referencia) > 0) {
						...
						$sql_ref = "SELECT tbl_produto.produto
									  FROM tbl_produto
									  JOIN tbl_linha ON tbl_produto.linha = tbl_linha.linha
									 WHERE (tbl_produto.referencia = '$produto_referencia' or tbl_produto.referencia_pesquisa = '$produto_referencia')
									   AND tbl_linha.fabrica      = $login_fabrica
									 limit 1";
					...
					if (($login_fabrica == 52 or $login_fabrica == 30 or $login_fabrica == 85) and strlen($defeito_reclamado) == 0 and $xproduto != 'null') {
					...
					if (strlen($msg_erro) == 0 and $xproduto != 'null') {
						...
						if (pg_num_rows($res) > 0) {
							...
							if (strlen($msg_erro) == 0 AND $abre_os == 't') {
								...
								if (strlen($msg_erro) == 0) {
									...
									if (strpos(pg_last_error(), "Este posto está DESCREDENCIADO")) {
										$msg_erro = "Posto descredenciado entre em contato com o fabricante";
									} else {
										$msg_erro = (strlen(pg_last_error()) > 0) ? pg_errormessage($con) : "";
									}
	...
	if ($login_fabrica == 30 and $abre_os == 't' and strlen($callcenter) == 0 and empty($msg_erro) ) {
		...
		$resAA = pg_query($con, $sql);
		$msg_erro = (strlen(pg_last_error()) > 0) ? pg_errormessage($con) : "";

		if (pg_num_rows($resAA) > 0) {
			
			for ($u=0; $u < pg_num_rows($resAA); $u++) {
				
				$produtos = pg_fetch_result($resAA,$u,'referencia')."-".pg_fetch_result($resAA,$u,'descricao');
				$consumidor_nome = pg_fetch_result($resAA,$u,'nome');
				$hd_item_sua_os = pg_fetch_result($resAA, $u, 'sua_os');
				...
				$subject = "Nova OS $hd_item_sua_os cadastrada pela fábrica $marca_nome";
				$message = "Autorizada $posto_codigo - $posto_nome

				O Callcenter da Fábrica <b>$marca_nome</b>, abriu um atendimento que se tornou uma OS para ser atendido pelo seu posto autorizado.<br><br>

				<b>Segue as informações da OS:</b><br><br>

				Atendimento do chamado <b>nº $hd_chamado </b>- Aberto por $marca_nome em $admin_email<br><br>
				<b>Produto:</b> $produtos<br><br>
				<b>Consumidor:</b> $consumidor_nome<br><br>

				Favor completar este atendimento o mais rápido possível, e qualquer dúvida, entrar em contato com <b>$marca_nome</b>.";
				...
				if (strlen($msg_erro) == 0) {
					...
					if(!empty($posto)) {
						...
						$res = @pg_query($con,$sql); echo (strlen(pg_errormessage($con))>0)?$sql:'';
						$msg_erro = (strlen(pg_last_error()) > 0) ? pg_errormessage($con) : "";
					}
	...
	if (strlen($callcenter) > 0) {
		...
		if ($login_fabrica == 85){
			$sql = "SELECT status from tbl_hd_chamado where fabrica=85 and hd_chamado=$callcenter";
			$res = pg_query($con,$sql);

			if (pg_num_rows($res)>0){
				$status_chamado = pg_fetch_result($res, 0, 0);

				if ($status_chamado == 'Resolvido'){
					$msg_erro = "Atendimento já resolvido";
				}
			}
		}
	...
	*/
		...
		if (strlen($msg_erro) == 0) {
			...
			if (strlen($hd_chamado) > 0 && ( $login_fabrica != 11 || ( $login_fabrica == 11 && $xstatus_interacao == "'Aberto'" ) ) and empty($msg_erro) ) {//*ja tem cadastro no telecontrol/
			...
			if (strlen($msg_erro) == 0 && $abre_os == 't' && $_POST['tincaso']) {
				...
				*/
				...
				$rs = pg_query($con, 'BEGIN TRANSACTION');
				...
				if (strlen($msg_erro) == 0) {
					...
					if ((strlen($posto_km_tab) == 0) or ($posto_km_tab == '')){
		                                $posto_km_tab = 0;
                		        } else {
                                		$posto_km_tab = str_replace(',','.',$posto_km_tab);
                        		}
					...
					if (strlen($msg_erro) == 0) {
						...
						if (strlen($msg_erro) == 0) {
							...
							$res = pg_exec($con,$sql); echo (strlen(pg_errormessage($con))>0)?$sql:'';
...
$().ready(function(){
	$(".btn_consulta_cidades_atendidas").click(function(){
		...
		if(codigo != ""){
			var URL = "cidades_atendidas_cliente.php?codigo="+ codigo +"&nome="+$('#posto_nome_tab').val();
...
//	alert(linha);
	...
	/*Criar TR - Linha*/
		...
		el.onclick = function(){
			...
			var produto    = document.getElementById('produto_referencia_'+linha);
			var mapa_linha = $('#mapa_linha'+linha);
			fnc_pesquisa_produto2(produto,nome,'referencia',linha,mapa_linha);
		}
		...
		el.onclick = function(){
			...
			var produto    = document.getElementById('produto_referencia_'+linha);
			var mapa_linha = $('#mapa_linha'+linha);
			fnc_pesquisa_produto2(produto,nome,'descricao',linha,mapa_linha);
		}
		celula.appendChild(el);
		nova_linha.appendChild(celula);
...
function fnc_pesquisa_produto2 (campo, campo2, tipo,linha, mapa_linha) {
	var xcampo = null;
	if (tipo == "referencia" ) {
		var xcampo = $('#produto_referencia_'+linha).val();
		var campo = document.getElementById('produto_referencia_'+linha);
	}

	if (tipo == "descricao" ) {
		
		var xcampo = $('#produto_nome_'+linha).val();
		var campo2 = document.getElementById('produto_nome_'+linha);
	}

	if (xcampo != "") {
		var url = "";
		url = "produto_pesquisa_2.php?campo=" + xcampo + "&tipo=" + tipo + "&mapa_linha=t&voltagem=t";
		janela = window.open(url, "janela", "toolbar=no, location=no, status=yes, scrollbars=yes, directories=no, width=600, height=400, top=18, left=0");
		janela.referencia   = campo;
		janela.descricao    = campo2;
		janela.mapa_linha   = mapa_linha;
...
function mapa_rede(linha,estado,cidade,cep,endereco,numero,bairro, consumidor_cidade,consumidor_estado){
	...
	url = "mapa_rede_new.php?callcenter=true&pais=BR&estado="+estado.value+"&linha="+linha.value+"&cidade="+cidade.value+"&cep="+cep.value+"&consumidor_cidade="+consumidor_cidade.value+"&consumidor_estado="+consumidor_estado.value+"&consumidor="+endereco_completo+"&nome="+nome_cliente;
...
<!-- <script src="http://maps.google.com/maps?file=api&v=2&key=<?=$key_api?>" type="text/javascript"></script> -->
...
if ($sucesso){ ?>
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#249F2C 1px solid; background-color: #338D39;font-size:10px'>
		<tr>
			<td align='center'><? echo "<font color='#FFFFFF'>$sucesso</font>"; ?></td>
		</tr>
	</table>
<?
}
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" style='font-size:12px'>
<tr>
...
<tr>
	...
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px' name='tabela_itens' id='tabela_itens'>
		...
		<tr>
			...
			<td align='left'>
				...
				<input name="produto_referencia_<?=$i?>" id="produto_referencia_<?=$i?>"  class="input"  value='<? echo $produto_referencia ;?>'
				onblur="atualizaQuadroMapas();validaGarantia(document.frm_callcenter.produto_referencia_<?=$i;?>,document.frm_callcenter.data_nf);postoProximo(cep,consumidor_endereco,consumidor_numero,consumidor_bairro,consumidor_cidade,consumidor_estado);" type="text" size="15" maxlength="15"><img src='imagens/lupa.png' border='0' align='absmiddle'
				style='cursor: pointer'
				onclick="javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_referencia_<?=$i?>,document.frm_callcenter.produto_nome_<?=$i?>,'referencia',<?=$i?>,document.frm_callcenter.mapa_linha)">
			</td>
			...
			<td align='left'>
				...
				<input name="produto_nome_<?=$i?>" id="produto_nome_<?=$i?>"  size='20' class="input" value='<?php echo $produto_nome ;?>'
				onblur="javascript: 
				...
				style='cursor: pointer'
				onclick="javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_referencia_<?=$i;?>,document.frm_callcenter.produto_nome_<?=$i;?>,'descricao',<?=$i?>,document.frm_callcenter.mapa_linha)">
			</td>
			<td align='left'>
				<strong><acronym title='Campo Obrigatório'><font color="#AA0000" style="bold">Defeito Reclamado</font></acronym></strong>
			</td>
			<td align='left'><? if ($login_fabrica == 52 ) {
	...
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
			...
			<tr>
				...
				<td align='left'>
					...
					if(($login_cliente_admin == 3890) || ($login_cliente_admin == 3791) || ($login_cliente_admin == 3790)){
						echo "<input type='button' name='btn_consulta_cidades_atendidas' id='btn_consulta_cidades_atendidas' class='btn_consulta_cidades_atendidas input_req2' style='background-color:#DDDDDD' value='Cidades atendidas' title='Consulta cidades atendidas'/>";
					}
					?>
				</td>
				<td align='left'><strong><acronym title='Campo Obrigatório'><font style="bold">Distancia Km(ida/volta):</font></acronym></strong></td>
		...
		<tr>
		<?php if ($login_fabrica == 30 and $status_interacao != 'Resolvido'){ ?>
			
			<tr>
				<td colspan='6' align='center'><input type='submit' value='Gravar' name='btn_acao'></td>
			</tr>
		<?php }elseif ($login_fabrica <> 30){?>
			<tr>
				<td colspan='6' align='center'><input type='submit' value='Gravar' name='btn_acao'></td>
			</tr>
		<?} ?>
