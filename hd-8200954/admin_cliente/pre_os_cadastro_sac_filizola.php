		...
		if ($tipo_busca=="posto"){
			...
			if (pg_numrows ($res) > 0) {
				for ($i=0; $i<pg_numrows ($res); $i++ ){
					...
					//echo "$mapa_cidade";
					//echo "\n";
					$array = array("mapa_cidade"=>$mapa_cidade);
					$arrayresult[] = json_encode($array);
				}
			}
		}
...
if(strlen($btn_acao)>0 AND $login_fabrica != 96 AND $login_fabrica != 7){
		...
		if($login_fabrica==11){//HD 53881 27/11/2008
			...
			if(in_array($tipo_reclamadoclamacao, $sub_tipo_reclamacao)){
...
*/
	...
	$consumidor_nome			= substr($_POST['nome'], 0 ,49);		
	$consumidor_cpf				= $_POST['cnpj'];
	$consumidor_rg				= $_POST['ie'];
	$email			= $_POST['email'];
	$consumidor_fone			= $_POST['telefone'];
	$consumidor_cep				= str_replace('-', '',$_POST['consumidor_cep']);
	$consumidor_endereco			= substr($_POST['endereco'], 0, 59);
	$consumidor_numero			= $_POST['numero'];
	$consumidor_complemento		= $_POST['complemento'];
	$consumidor_bairro			= $_POST['bairro'];
 	$consumidor_cidade			= $_POST['cidade'];
	$consumidor_estado			= $_POST['estado'];

	$nota_fiscal			= $_POST['numero_nf'];
	$data_nf			= $_POST['data_nf'];
	...
	$numero_patrimonio_pib = $_POST['numero_patrimonio_pib'];
	$erp_produto = $_POST['erp_produto'];
	...
	if($login_fabrica == 7){
		if(strlen($consumidor_nome) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo Nome é Obrigatório";
		}		

		if(strlen($consumidor_cpf) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo CNPJ é Obrigatório";
		}else{
			$find = array('.','/','-');
			$replace = array('','','');
			$cnpjnumerico = str_replace($find, $replace, $consumidor_cpf);

			if(checa_cnpj($cnpjnumerico) != 0 AND strlen($msg_erro) == 0){
				$msg_erro = "CNPJ Inválido";
			}
		}

		if(strlen($consumidor_rg) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo IE é Obrigatório";
		}

		if(strlen($consumidor_cep) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo CEP é Obrigatório";
		}

		if(strlen($consumidor_endereco) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo Endereço é Obrigatório";
		}

		if(strlen($consumidor_numero) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo Número é Obrigatório";
		}

		if(strlen($consumidor_cidade) == 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Campo Cidade é Obrigatório";
		}

	}

	if($login_fabrica == 7){
		if(strlen($data_nf) != 0 && $data_nf != '__/__/____' && strlen($msg_erro) == 0){						
	...
	if($login_fabrica != 7){
		...
		$produto = $_POST['produto_1'];
		if(empty($produto) and strlen($msg_erro) == 0){			
			$msg_erro .= "Produto obrigatório";
		}	
		
		$camposextras = array(	'numero_contrato' => $numero_contrato,
							  	'numero_patrimonio_pib' => $numero_patrimonio_pib,
							  	'erp_produto' => $erp_produto);
		$jsoncamposextras = json_encode($camposextras);		
	...
	for($i = 1;$i <= $qtde_produto;$i++){
		...
		if((strlen($serie) != 0 OR strlen($produto) != 0 OR strlen($defeito_reclamado_descricao) != 0) AND strlen($msg_erro) == 0){
			...
			if( strlen($defeito_reclamado_descricao) == 0){
				if($login_fabrica == 7){
					$msg_erro = "Informe a descrição do defeito";
				}else{
					$msg_erro = "Informe a descrição do defeito - Linha {$i}";
				}
			}
			
			if(strlen($produto) == 0){
				if($login_fabrica == 7){
					$msg_erro = "Referência do Produto Inválido!";
				}else{
	...
	if(strlen($msg_erro)==0){
		...
		if(strlen($msg_erro)== 0 AND strlen($hd_chamado) != 0){			
			...
			if(strlen($consumidor_estado)>0 and strlen($consumidor_cidade)>0){
		 	$sql = "SELECT tbl_cidade.cidade
						FROM tbl_cidade
						WHERE tbl_cidade.nome = '$consumidor_cidade'
			...
			if(strlen($xcodigo_posto) == 0 || $xcodigo_posto == ""){
				$xcodigo_posto = 'null';
			}
		...
		if(strlen($msg_erro)== 0){
			...
			if($login_fabrica == 7){
				$assunto = utf8_decode('Novo Atendimento Cadastrado pela DR');	
			}else{
			...
			if($login_fabrica == 7){
				$message .= "de número <b>$hd_chamado</b>, favor fazer agendamento com o técnico responsavel</br>";
			}			
			...
			}elseif ($login_fabrica == 7) {				
				//$mail->AddAddress('anderson.luciano@telecontrol.com.br');				
				$mail->AddAddress('miraci@filizola.com.br');
				$mail->AddAddress('r.silva@filizola.com.br');
				$mail->AddAddress('marcos@filizola.com.br');
				$mail->AddAddress('odete@filizola.com.br');
				$mail->AddAddress('valsantos@filizola.com.br');
				$mail->AddAddress('tabata@filizola.com.br');
				//$mail->AddAddress('waldir@telecontrol.com.br');
				//$mail->AddAddress('colombo@filizola.com.br');
...
if(strlen($callcenter)>0){
	...
	$sql = "SELECT	tbl_hd_chamado_extra.hd_chamado as callcenter,
		...
		if(pg_numrows($res)>0){
			...
			$array_campos_adicionais = json_decode($campos_adicionais);
...
<!-- <script language='javascript' src='../admin/js/jquery-ui-1.8.23.custom/js/jquery-1.8.0.min.js'></script> -->


<!-- <script language='javascript' src='../admin/js/jquery-ui-1.8.23.custom/development-bundle/ui/jquery-ui-1.8.23.custom.js'></script> 
<script language='javascript' src='../admin/js/jquery-ui-1.8.23.custom/development-bundle/ui/jquery.ui.core.js'></script> 
<script language='javascript' src='../admin/js/jquery-ui-1.8.23.custom/development-bundle/ui/jquery.ui.widget.js'></script> 
<script language='javascript' src='../admin/js/jquery-ui-1.8.23.custom/development-bundle/external/jquery.bgiframe-2.1.2.js'></script>  -->

<script type="text/javascript" src="js/js_custom/jquery-1.7.1.min.js"></script>
<script type="text/javascript" src="js/js_custom/ui/jquery-ui-1.8.18.custom.js"></script>
<script type="text/javascript" src="js/js_custom/ui/minified/jquery.ui.core.min.js"></script>
<script type="text/javascript" src="js/js_custom/ui/minified/jquery.ui.position.min.js"></script>
<script type="text/javascript" src="js/js_custom/ui/minified/jquery.ui.widget.min.js"></script>
<script type="text/javascript" src="js/js_custom/external/jquery.bgiframe-2.1.2.js"></script>

<script type="text/javascript" src="js/js_custom/ui/minified/jquery.ui.autocomplete.min.js"></script>





<!-- <script language='javascript' src='../admin/js/jquery-ui-1.8.23.custom/development-bundle/ui/minified/jquery.ui.autocomplete.min.js'></script> -->
...
<!-- <link rel="stylesheet" href="../admin/js/jquery-ui-1.8.23.custom/development-bundle/themes/base/jquery.ui.core.css" type="text/css" media="print, projection, screen">
<link rel="stylesheet" href="../admin/js/jquery-ui-1.8.23.custom/development-bundle/themes/base/jquery.ui.theme.css" type="text/css" media="print, projection, screen">
<link rel="stylesheet" href="../admin/js/jquery-ui-1.8.23.custom/development-bundle/themes/base/jquery.ui.autocomplete.css" type="text/css" media="print, projection, screen"> -->

<link href="js/js_custom/themes/custom-theme/jquery.ui.all.css" rel="stylesheet" type="text/css"  />
<link href="js/js_custom/themes/custom-theme/jquery-ui-1.8.18.custom.css" rel="stylesheet" type="text/css" />
...
//Ao selecionar o produtoo preencher 
//o campo contrato ao qual o campo esta amarrado
var listaselecionada = "";

function selecionado(){
	listaselecionada = this.getAttribute('indice');
	
}


$(function(){	
	$("input[name=descricao_1]").autocomplete({		
		source: "../admin/produto_pesquisa_2_autocomplete.php",
		minLength: 2,
		delay: 300,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {			
			var indice = listaselecionada;			

			var result = $.parseJSON(ui.item.value);			
			$(this).val(result[indice].descricao);
			$("input[name=produto_1]").val(result[indice].referencia);
			$("input[name=numero_contrato]").val(result[indice].contrato);

			return false;
		}
	}).data("autocomplete")._renderItem = function (ul, item) {		
		var result = $.parseJSON(item.label);
		var lista = "";		
		if(result[0].referencia != null){
			for(i=0;i<result.length;i++){			
				if(result[i].referencia != null){
					var text = result[i].referencia + " - " + result[i].descricao;
				 	lista += $("<li></li>").data("item.autocomplete", item).append("<a indice='"+i+"' class='lista-produtos'>"+text+"</a>").appendTo(ul);
				}			
			}
		}else{
			return false;
		...
		var aux = document.getElementsByClassName('lista-produtos');
		for(i=0;i<aux.length;i++){
			aux[i].addEventListener('click',selecionado);
		}
...
$(function(){	
	$("input[name=produto_1]").autocomplete({		
		source: "../admin/produto_pesquisa_2_autocomplete.php",
		minLength: 2,
		delay: 300,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {			
			...
			$(this).val(result[indice].referencia);
			$("input[name=descricao_1]").val(result[indice].descricao);
			$("input[name=numero_contrato]").val(result[indice].contrato);
...
function add_linha_bosch_security(linha2) {
	...
	var dados = "<tr>";
		...
		dados += "<td style='padding: 0; width: 60px '>";
			dados += " <input name='serie_"+linha+"' id='serie_"+linha+"' class='frm' value='' style='width: 50px' maxlength='20' />";
		dados += "</td>";
		dados += "<td style='padding: 0; width: 25px '>";
			dados += "<input name='quantidade_"+linha+"' class='frm' value='1' maxlength='2' style='width: 15px' onblur='javascript: if(document.frm_callcenter.quantidade_"+linha+".value > 1){ alert(\"Atenção se os produtos desta linha tiverem série/lote ou defeitos diferentes devem ser digitados em linhas diferentes.\");}' />";
		dados += "</td>";
...
//		$('#container').disableTab(3);
		...
		$("input[name='telefone']").maskedinput("(999) 9999-9999");
...
$().ready(function() {
	...
	$("#revenda_cnpj").autocomplete({		
		source: "<?echo $PHP_SELF.'?tipo_busca=revenda&busca=codigo'; ?>",
		minLength: 3,
		delay: 150,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {						
			var result = $.parseJSON(ui.item.value);
			$(this).val(result.cnpj);
			$("#revenda").val(result.revenda) ;
			$("#revenda_nome").val(result.nome) ;

			return false;
		}
	}).data("autocomplete")._renderItem = function (ul, item) {
		var result = $.parseJSON(item.label);
		var text = result.cnpj+"-"+result.nome;
	...
	/* Busca pelo Nome */
	$("#revenda_nome").autocomplete({		
		source: "<?echo $PHP_SELF.'?tipo_busca=revenda&busca=nome'; ?>",
		minLength: 3,
		delay: 150,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {
			var result = $.parseJSON(ui.item.value);
			$(this).val(result.nome);
			$("#revenda").val(result.revenda) ;
			$("#revenda_cnpj").val(result.cnpj) ;
	...
	$("#mapa_cidade").autocomplete({		
		source: "<?echo $PHP_SELF.'?tipo_busca=mapa_cidade&busca=mapa_cidade'; ?>",
		minLength: 3,
		delay: 150,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {						
			var result = $.parseJSON(ui.item.value);
			$(this).val(result.mapa_cidade);			

			return false;
		}
	}).data("autocomplete")._renderItem = function (ul, item) {
		var result = $.parseJSON(item.label);
		var text = result.mapa_cidade;
	...
	/* Busca pelo Código */
	$("#codigo_posto_tab").autocomplete({		
		source: "<?echo $PHP_SELF.'?tipo_busca=posto&busca=codigo'; ?>",
		minLength: 3,
		delay: 150,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {						
			var result = $.parseJSON(ui.item.value);
			$(this).val(result.codigo_posto);			
			$("#posto_tab").val(result.posto) ;			
			$("#posto_nome_tab").val(result.nome) ;

			return false;
		}
	}).data("autocomplete")._renderItem = function (ul, item) {
		var result = $.parseJSON(item.label);
		var text = result.codigo_posto+"-"+result.nome;
	...
	/* Busca pelo Nome */
	$("#posto_nome_tab").autocomplete({		
		source: "<?echo $PHP_SELF.'?tipo_busca=posto&busca=nome'; ?>",
		minLength: 3,
		delay: 150,
		position: { my : "center top", at: "center bottom" },
		select: function (event, ui) {						
			...
			$(this).val(result.nome);			
			$("#posto_tab").val(result.posto) ;			
			$("#codigo_posto_tab").val(result.codigo_posto) ;
...
if($login_fabrica != 96 AND $login_fabrica != 7){ ?>
...
?>
	...
	?>
		<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
			...
			<tr>
				<td>
					Nome<br>
					<input type="text" name="nome" value="<?=$consumidor_nome?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					CNPJ<br>
					<input type="text" name="cnpj" value="<?=$consumidor_cpf?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					IE<br>
					<input type="text" name="ie" value="<?=$consumidor_rg?>" class='frm' style='width: 170px' <?php echo $readonly; ?> maxlength="20"  /> 
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>
					Telefone<br>
					<input type="text" name="telefone" value="<?=$consumidor_fone?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					CEP<br>
					<input type="text" name="consumidor_cep" id="consumidor_cep" value="<?=$consumidor_cep?>" class='frm' style='width: 170px' <?php echo $readonly; ?> onchange="buscaCEP(this.value, document.frm_callcenter.endereco, document.frm_callcenter.bairro, document.frm_callcenter.cidade, document.frm_callcenter.estado) ;"/> 
				</td>
			</tr>
			<tr>
				<td>
					Endereço<br>
					<input type="text" name="endereco" value="<?=$consumidor_endereco?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Número<br>
					<input type="text" name="numero" value="<?=$consumidor_numero?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Complemento<br>
					<input type="text" name="complemento" value="<?=$consumidor_complemento?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
			</tr>
			<tr>
				<td>
					Bairro<br>
					<input type="text" name="bairro" value="<?=$consumidor_bairro?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Cidade<br>
					<input type="text" name="cidade" value="<?=$consumidor_cidade?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Estado<br>
					<input type="text" name="estado" value="<?=$consumidor_estado?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
			</tr>
		</table>
	<?php
	...
	?>
		<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
			...
			<tr>
				<td>
					Nome<br>
					<input type="text" name="nome" value="<?php echo $dados['nome']?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					CNPJ<br>
					<input type="text" name="cnpj" value="<?php echo $dados['cnpj']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					IE<br>
					<input type="text" name="ie" value="<?php echo $dados['ie']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
			</tr>
			<tr>
				<td>
					E-mail<br>					
					<input type="text" name="email" value="<?php echo $dados['email']?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Telefone<br>
					<input type="text" name="telefone" value="<?php echo $dados['fone']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					CEP<br>
					<input type="text" name="cep" value="<?php echo $dados['cep']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
			</tr>
			<tr>
				<td>
					Endereço<br>
					<input type="text" name="endereco" value="<?php echo $dados['endereco']?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Número<br>
					<input type="text" name="numero" value="<?php echo $dados['numero']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Complemento<br>
					<input type="text" name="complemento" value="<?php echo $dados['complemento']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
			</tr>
			<tr>
				<td>
					Bairro<br>
					<input type="text" name="bairro" value="<?php echo $dados['bairro']?>" class='frm' style='width: 270px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Cidade<br>
					<input type="text" name="cidade" value="<?php echo $dados['cidade']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
				<td>
					Estado<br>
					<input type="text" name="estado" value="<?php echo $dados['estado']?>" class='frm' style='width: 170px' <?php echo $readonly; ?> /> 
				</td>
			</tr>
		</table>
	<?php
	...
	<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
		...
		<tr>
			...
			<?php }else{ ?>
				<td>
					...
					<input type="text" name="numero_contrato" value="<?php echo $numero_contrato;?>" class="frm" style="width: 123px"  readonly/>
				</td>
			<?php } ?>
			<td>&nbsp;</td>
		</tr>
		...
		if($login_fabrica == 7){ ?>
			<td colspan='6' style='padding: 0'>
				<tr>					
					<td>
						Numero do Patrimonio PIB</br>
						<input type="text" name="numero_patrimonio_pib" value="<?php echo $numero_patrimonio_pib;?>" class="frm" style="width: 125px"  />
					</td>
					<td>
						ERP do Produto</br>
						<input type="text" name="erp_produto" value="<?php echo $erp_produto?>" class="frm" style="width: 110px"  />
					</td>
				</tr>
			</td>							
		<?php } ?>
					...
					<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'  name='tabela_itens' id='tabela_itens'>
						...
						<?php
							if (strlen($callcenter)>0) {
								...
								$qtde_produto = pg_num_rows($res_produto);
								if ($qtde_produto>0) {
									$produto 	 = pg_result($res_produto,'0','referencia');
									$descricao 	 = pg_result($res_produto,'0','descricao');
									$serie 		 = pg_result($res_produto,'0','serie');
									$defeito_reclamado_descricao = pg_result($res_produto,'0','defeito_reclamado_descricao');
						...
						<tr>							
							...
							<td style='padding: 0; width: 60px '>
								...
								<input name="serie_<?=$i;?>" id="serie_<?=$i;?>" class="frm" value="<?php echo $serie;?>" style='width: 50px' maxlength='20' />
							</td>
							<?php
								if($login_fabrica != 7){
	...
	<?php }else{			
			if(strlen($callcenter) == 0){
				...
				<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>													
					<tr>
						<a href='callcenter_pendente_interativo.php'>Voltar</a>
					</tr>
				</table>
				<?php
			}
