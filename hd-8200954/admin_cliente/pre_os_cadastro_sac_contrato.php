...
// desativado enquanto não é atualizado o branch
// include_once '../classes/communicator.class.php';
// $mailer = new TcComm('smtp@posvenda');

$mostra_mapa_rede = !in_array($login_fabrica, array(3,85,156));
...
$title = ($login_fabrica == 156) ? "Abertura de Chamados Contrato" : "ATENDIMENTO SAC - ABERTURA DE PRÉ OS";
...
if (strlen($btn_acao)>0 AND !in_array($login_fabrica,array(96,156,158))) {
    ...
    if($login_fabrica == 85){
        ...
        if(strlen(trim($nome_fantasia)) > 0){
            $array_campos_adicionais['nome_fantasia'] = $nome_fantasia;
            $array_campos_adicionais['consumidor_cpf_cnpj'] = 'R';
        }
    }
    ...
    if ($login_fabrica== 3 or $login_fabrica == 24 or $login_fabrica == 85 or ($login_fabrica==5 and $indicacao_posto=='f')) { // HD 48900 58796
    ...
    $filial             = ($_POST['encaminha_filial'] == 't') ? $_POST['filial']['id'] : 'NULL';
   # $data_providencia   = is_date($_POST['data_providencia'], 'EUR', 'ISO') ? : 'NULL';
    ...
    if ($tab_atual == "extensao") {
        ...
        if ($_POST['data_providencia'] and $data_providencia == 'NULL')
            $msg_erro .= "Prazo Limite inválido!";
        if (strtotime($data_providencia) <= strtotime(date("Y-m-d")))
            $msg_erro .= "Prazo limite não pode ser anterior ao momento atual!";
    ...
    if(strlen($faq_situacao) > 0){ // HD 45991
        ...
        if(strlen($msg_erro) ==0 ){
            ...
            if(pg_numrows($res) > 0){
                ...
                $mailer->sendMail($destinatario,$assunto,$mensagem,$remetente);
            }
        }
    }
...
/*INSERINDO*/
    if(strlen($callcenter)==0){
        if(strlen($msg_erro)==0){
            ...
            }elseif($indicacao_posto=='f' && $login_fabrica != 85) {
        ...
        if (strlen($mr_codigo_posto) > 0) {
            $xcodigo_posto=$mr_codigo_posto;
        } else if (in_array($login_fabrica, array(85,156,158))) {
            $xcodigo_posto = 'NULL';
        }
        else {
        ...
        if ($mostra_mapa_rede and strlen($posto_km_tab)==0) {
        ...
        if(strlen($msg_erro)==0 and strlen($callcenter)==0) {
            ...
            if(count($array_campos_adicionais) > 0){
                ...
                if ($filial != 'NULL' and strlen($filial) > 0 ) {
                                pg_query($con, "INSERT INTO tbl_hd_chamado_extra_filial VALUES ($hd_chamado, $filial)");
                }
            ...
            if (in_array($login_fabrica,array(7,30,52,85))) {
                ...
                if ($qtde_produto>0 ) {
                    for ($w=1;$w<=$qtde_produto;$w++) {
                        ...
                        if($login_fabrica == 85 && strlen($serie) == 0){
                            $msg_erro .= "Informe o número de série do produto";
                        }
                        ...
                        if (strlen($produto_referencia)>0) {
                            ...
                            if(pg_numrows($res_ref)>0){
                                ...
                                if($login_fabrica == 85){
                                    $msg_erro .= "Informe a Referência do produto <br>";
                                }
                            $xproduto = "null";
                            }
                            }else{
                            ...
                            if (strlen($defeito_reclamado)==0 AND $login_fabrica != 85) {
                            ...
                            if (strlen($msg_erro)==0) {
                                ...
                                if($login_fabrica == 85){
                                    ...
                                    $sql = "UPDATE tbl_hd_chamado_extra SET produto = $xproduto, serie = '$serie', defeito_reclamado_descricao = '$defeito_reclamado'  WHERE hd_chamado = $hd_chamado";
                                    $res = pg_query($con,$sql);
                                }
                            }
                    }
            ...
            if (strlen($msg_erro) == 0){
                ...
                if ($login_fabrica == 85) {
                    ...
                    $assunto = "Abertura Protocolo Nestlé";
                    
                                        $mensagem .= "Protocolo: $hd_chamado <br> <br>";
                    $mensagem .= "<table border='1'><tr><td style='border:solid 1px;'>Produto</td><td style='border:solid 1px;'>Defeito Reclamado</td></tr>";

                    for ($k=1;$k<=$qtde_produto;$k++) {
                                                $produto_referencia = $_POST['produto_referencia_'.$k];
                        $produto_descricao  = $_POST['produto_nome_'.$k];
                                                $serie              = $_POST['serie_'.$k];
                                                $defeito_reclamado  = $_POST['defeito_reclamado_'.$k];

                        $mensagem .= "<tr><td style='border:solid 1px;'>".$produto_referencia ." - ". $produto_descricao ."</td><td style='border:solid 1px;'>".$defeito_reclamado."</td></tr>";
                    ...
                    $mailTc->addEmailDest("assistec@gelopar.com.br");
...
*/
    ...
    if(strlen($msg_erro)==0){
        ...
        if($ultimo_atendente <> $xtransferir){
            ...
            if(strlen($email_ultimo_atendente) >0 AND strlen($email_atendente) >0){
                ...
                if ($mailer->sendMail($email_atendente, stripslashes($assunto), $corpo, $email_ultimo_atendente)) {
    ...
    $nome             = trim(substr($_POST['nome'], 0 ,49));
    $cnpj             = trim($_POST['cnpj']);
    $ie               = trim($_POST['ie']);
    $email            = trim($_POST['email']);
    $telefone         = trim($_POST['telefone']);
    $cep              = str_replace("-", "", trim($_POST['cep']));
    $endereco         = trim(substr($_POST['endereco'], 0, 59));
    ...
    $consumidor_revenda  = (strlen($_POST['consumidor_revenda']) == 0) ? "" : $_POST["consumidor_revenda"];
    ...
    if(in_array($login_fabrica, array(156,158))){
        $pre_os = "f";
    }else{
        $pre_os = "t";
    }
    ...
    $responsavel      = retira_acentos($_POST['responsavel']);
    ...
    if($login_fabrica == 156 AND 1 == 2){
        if ($_POST['data_providencia'] and $data_providencia == 'NULL'){
            $msg_erro .= "Prazo Limite obrigatório!";
        }else{
            list($d,$m,$y) = explode("/", $data_providencia);
            ...
            if(!checkdate($m, $d, $y)){
                ...
                $data_providencia_formatada = "{$y[0]}-$m-$d";

                if (strtotime($data_providencia_formatada) <= strtotime(date("Y-m-d")))
                    $msg_erro .= "Prazo limite não pode ser anterior ao momento atual!";
            }

        }
    }else{
        $data_providencia_formatada = "NULL";
    }

    if($login_fabrica == 156){

        $numero_controle = $_POST["numero_controle"];
        $descricao_defeito = str_replace("\t", "", $_POST["descricao_defeito"]);

        $array_campos_adicionais = array("numero_controle" => $numero_controle, "responsavel" => $responsavel, "tel_responsavel" => $tel_responsavel, "tipo_consumidor_revenda" => $consumidor_revenda, "descricao_defeito" => $descricao_defeito);
    ...
    if(strlen($data_nf) == 0 AND strlen($msg_erro) == 0){
        ...
        if(!@checkdate($mi,$di,$yi)){
            ...
            $aux_data_nf = "'$aux_data_nf'";
        }
    }

    // Valida DADOS para Bosch Security - 96
    if(empty($numero_nf) AND strlen($msg_erro) == 0 AND !in_array($login_fabrica, array(156, 158))){
    ...
    if(strlen($anexo1) > 0 AND strlen($msg_erro) == 0){
        ...
        if($extensao != 'pdf' AND $extensao != 'jpg' AND $extensao != 'jpeg')
            $msg_erro = "Nota Fiscal de Envio, somente exntensões 'PDF e JPEG'<br>";
    }
    ...
    if(strlen($anexo2) > 0 AND strlen($msg_erro) == 0){
        ...
        $extensao = strtolower($extensao[0]);
        if($extensao != 'xml')
            $msg_erro = "Nota Fiscal de Envio, somente XML<br />";
    }

    if($tipo_atedimento == 0 AND strlen($msg_erro) == 0 AND !in_array($login_fabrica, array(156,158))){
        $msg_erro = "Tipo de Atendimento Inválido";
    }

    if(empty($consumidor_final) AND strlen($msg_erro) == 0 AND !in_array($login_fabrica, array(156,158))){
        $msg_erro = "Consumidor Final Inválido";
    }

    if(!in_array($login_fabrica, array(156,158))){
        if(!$_FILES['anexo_1']['name']) $msg_erro .= '<br>O anexo da NF de Envío é obrigatório!';
        if($tipo_atedimento != 93 and !$_FILES['anexo_3']['name']) $msg_erro .= '<br>O anexo da NF de Compra é obrigatório!';
    }else{
        $tipo_atedimento = "NULL";
    }
    ...
    for($i = 1;$i <= $qtde_produto;$i++){
        ...
        if ($login_fabrica == 158) {
            $array_campos_adicionais = array(
                "tensao" => utf8_encode(trim($_POST["tensao_".$i])),
                "tempo_instalacao" => utf8_encode(trim($_POST["tempo_instalacao_".$i])),
            );
        ...
        if(!empty($produto)){
            if(strlen($serie) == 0){
                $msg_erro .= "Número da Série Inválido! - Linha {$i}<br />";
            }

            if( strlen($defeito_reclamado_descricao) == 0 && $login_fabrica != 158){
                $msg_erro .= "Informe a descrição do defeito - Linha {$i}<br />";
            }

            if(empty($defeito_reclamado) && $login_fabrica == 158){
                $msg_erro .= "Informe o defeito - Linha {$i}<br />";
            }

            if(strlen($produto) == 0){
                $msg_erro .= "Referência do Produto Inválido! - Linha {$i}<Br />";
            }else{
                $sql = "
                        SELECT
                            produto
                        ...
                        WHERE
                            ...
                            AND tbl_linha.fabrica = $login_fabrica
                            AND (tbl_produto.referencia_fabrica = '$produto' OR tbl_produto.referencia = '$produto')
                        LIMIT 1
                    ";
                //echo "<br>".nl2br($sql);
                $res = pg_query($con,$sql);
                if(pg_num_rows($res)== 0){
                    $msg_erro .= "Referência do Produto Inválido ou Inexistente! - Linha {$i} <Br />";
                }else{
                    if(in_array($login_fabrica, array(156,158))){
    ...
    if($login_fabrica == 156){
        $sqlR = "SELECT admin_responsavel FROM tbl_cliente_admin WHERE cliente_admin = {$login_cliente_admin}";
        $resR = pg_query($con,$sqlR);
        $atendente = pg_fetch_result($resR, 0, admin_responsavel);

        if (empty($atendente)) {
            $msg_erro .= 'Não foi possível cadastrar o atendimento. Favor entrar em contato com a fábrica para atualização de seu cadastro no sistema.';
        }
    } else if ($login_fabrica == 158) {
        $sqlR = "
            SELECT admin, (CASE WHEN max_hd_chamado.hd_chamado IS NULL THEN 0 ELSE max_hd_chamado.hd_chamado END) AS atendimento, email
            FROM tbl_admin
            LEFT JOIN (
                SELECT MAX(hd_chamado) as hd_chamado, atendente
                FROM tbl_hd_chamado
                WHERE fabrica = {$login_fabrica}
                AND status = 'Aberto'
                GROUP BY atendente
            ) AS max_hd_chamado ON max_hd_chamado.atendente = tbl_admin.admin
            WHERE fabrica = {$login_fabrica}
            AND ativo IS TRUE
            AND cliente_admin_master IS TRUE
            ORDER BY atendimento ASC
            LIMIT 1;
        ";
        $resR = pg_query($con, $sqlR);

        if (!pg_num_rows($resR)) {
            $msg_erro = "Erro ao buscar atendentes<Br />";
        } else {
            $atendente       = pg_fetch_result($resR, 0, "admin");
            $atendente_email = pg_fetch_result($resR, 0, "email");
        }
    }else{
    ...
    if(strlen($msg_erro)==0){
        ...
        if(strlen($estado)>0 and strlen($cidade)>0){
            $sql = "SELECT tbl_cidade.cidade
                    FROM tbl_cidade
                    WHERE UPPER(fn_retira_especiais(tbl_cidade.nome)) = UPPER(fn_retira_especiais('{$cidade}'))
        ...
        if(strlen($msg_erro)== 0 AND strlen($hd_chamado) != 0){
            if ($login_fabrica == 158) {
                $colunaDefeito = ", defeito_reclamado, reclamado";

                if (empty($defeito_reclamado)) {
                    $defeito_reclamado = "null";
                }

                $valorDefeito = ", {$defeito_reclamado}, '{$obs}'";
            }
            ...
            $sql = "INSERT INTO tbl_hd_chamado_extra(
                    ...
                    if ($login_fabrica != 158) {
                        for($i = 1;$i <= $qtde_produto;$i++){
                            ...
                            $serie                       = $_POST["serie_$i"];
                            $produto                     = $_POST["produto_$i"];
                            $referencia_produto          = $_POST["produto_$i"];
                            $descricao                   = $_POST["descricao_$i"];
                            $quantidade                  = $_POST["quantidade_$i"];
                            $defeito_reclamado_descricao = $_POST["defeito_reclamado_descricao_$i"];
                            $defeito_reclamado           = $_POST["defeito_reclamado_$i"];

                            if((strlen($serie) != 0 OR strlen($produto) != 0 OR strlen($defeito_reclamado_descricao) != 0) and empty($msg_erro)){
                                ...
                                //HD 845189 - verifica se existe defeito reclamado cadastrado para o produto, caso exista e o usuário não selecionou nenhum valor no <select>, deverá mostrar msg de erro.
                                //inicio
                                $sql = "SELECT
                                            familia,
                                            linha,
                                ...
                                if(pg_num_rows($res)>0){
                                    ...
                                    if(pg_num_rows($res)>0 and (strlen($defeito_reclamado) == 0 or $defeito_reclamado == '' or $defeito_reclamado == 0)){
                                        $msg_erro = "Selecione um Defeito Reclamado para o produto: $referencia_produto";
                                    }
                                }
                                //fim
                                ...
                                if (strlen($defeito_reclamado) == 0 or $defeito_reclamado == 0){
                                    $defeito_reclamado = 'null';
                                }
                                ...
                                if (empty($msg_erro)){
                                    ...
                                    for($y = 1;$y <= $quantidade;$y++){
                                        ...
                                        if(strlen(pg_errormessage($con)) > 0){
                                            //$msg_erro .= pg_errormessage($con);
                                            $msg_erro .= "Erro ao cadastrar produto";
                                        }
        ...
        if(strlen($msg_erro)== 0){
            ...
            if(!in_array($login_fabrica, array(156,158))){
                ...
                //HD 798220 - Foi solicitado o uso desta classe para enviar corretamente o email
                require_once '../class/email/mailer/class.phpmailer.php';

                $assunto = utf8_decode('Nova Pré-OS cadastrada pelo SAC da Fábrica');
                ...
                $mail = new PHPMailer();
                $mail->IsHTML(true);
                $mail->From = 'Call-center <suporte@telecontrol.com.br>';
                $mail->FromName = 'Telecontrol';

                $mail->AddAddress('andre.dias@br.bosch.com');
                $mail->AddAddress('renato.lima2@br.bosch.com');
                //$mail->AddAddress('waldir@telecontrol.com.br');
            ...
            if(in_array($login_fabrica, array(158))){
                ...
                $message = "<br><br><br>Foi aberta uma Pré-OS de número <b>$hd_chamado</b><br>";
                $message .= "Favor dar continuidade no atendimento dessa Pré-OS.<br>";
                $message .= "-------------<br><br>";
                $message .= "<b>Esta é uma mensagem automática, não responda este e-mail</b>";
                ...
                $mail->From = 'Call-center <suporte@telecontrol.com.br>';
                $mail->FromName = 'Telecontrol';
                $mail->Subject = utf8_decode("TESTE TELECONTROL - Nova Pré-OS cadastrada pelo Cliente Admin");
                $mail->Body = "$message";

                if ($_serverEnvironment == "development") {
                    $mail->AddAddress("guilherme.curcio@telecontrol.com.br");
                    ...
                    $sqlNotCli = "
                        ...
                        AND cliente_admin_master IS TRUE
                    ";
                    $resNotCli = pg_query($con, $sqlNotCli);

                    if (pg_num_rows($resNotCli) > 0) {
                        $resNotCli = pg_fetch_all($resNotCli);

                        foreach ($resNotCli as $row => $adminNot) {
                            $mail->AddAddress($adminNot["email"]);
                        }
        ...
        if (strlen($msg_erro) == 0){
                ...
                $msg_retorno = ($login_fabrica == 156) ? "Atendimento aberto com Sucesso" : "Pré-os Cadastrada com Sucesso";

                $popup = "window.open('print_atendimento_gravado.php?hd_chamado=$hd_chamado','hd_chamado','scrollbars=yes,height=600, width=800');";

                if ($login_fabrica == 156 || $login_fabrica == 158) {
                    $popup = '';
                }
                ...
                if ($login_fabrica == 158) {
                    ...
                    $url = "$PHP_SELF?hd_chamado=$hd_chamado";
                }
    ...
    if ((strlen($anexo1) > 0 OR strlen($anexo2) > 0 OR strlen($anexo3) > 0) and !$msg_erro) {
        $dir = dirname(__FILE__) . "/anexos/";
        dir($dir);
        $cont = 0;
...
$callcenter  = (in_array($login_fabrica, [156,190])) ? $_GET['hd_chamado'] : $_GET['callcenter'];
$imprimir_os = trim($_GET['imprimir_os']);

if(strlen($callcenter) > 0){

    $cond_admin = (in_array($login_fabrica, [156,190])) ? " JOIN tbl_cliente_admin ON tbl_hd_chamado.cliente_admin = tbl_cliente_admin.cliente_admin JOIN tbl_admin ON tbl_admin.cliente_admin = tbl_cliente_admin.cliente_admin " : " JOIN tbl_admin ON tbl_hd_chamado.admin = tbl_admin.admin ";
    ...
    $sql = "SELECT tbl_hd_chamado_extra.hd_chamado as callcenter,
                   ...
                   tbl_hd_chamado_extra_filial.filial,
         ...
         LEFT JOIN tbl_hd_chamado_extra_filial ON tbl_hd_chamado.hd_chamado   = tbl_hd_chamado_extra_filial.hd_chamado
             WHERE tbl_hd_chamado.fabrica_responsavel = $login_fabrica
               AND tbl_hd_chamado.hd_chamado = $callcenter";
        $res = pg_exec($con,$sql);
        ...
        if(pg_numrows($res)>0){
            ...
            $familia                  = pg_fetch_result($res,0,'familia');
            $filial                   = pg_fetch_result($res,0,'filial');
            ...
            if(in_array($login_fabrica,array(85,156)) && strlen($array_campos_adicionais)){
                ...
                if(strlen($tipo_consumidor_revenda) > 0){
                    $consumidor_revenda = $tipo_consumidor_revenda;
                }
            ...
            if ($filial) {
                ...
                $filial = pg_fetch_assoc(
                    pg_query(
                        $con,
                        "SELECT tbl_filial.filial, tbl_filial.nome AS razao_social, tbl_filial.fantasia AS fantasia,
                                tbl_filial.cnpj, tbl_filial.ie, tbl_filial.telefone AS fone,
                                tbl_filial.cep, tbl_filial.endereco, tbl_filial.numero, tbl_filial.complemento,
                                tbl_filial.bairro, tbl_cidade.nome AS cidade, tbl_filial.estado,
                                tbl_filial.cod_filial
                           FROM tbl_filial
                           JOIN tbl_cidade USING(cidade)
                          WHERE fabrica = $login_fabrica AND filial = $filial_id"
                    ), 0
                );
            }
...
.input {
    ...
    border-right: #666666 1px double !important;
    border-top: #666666 1px double !important;
    border-left: #666666 1px double !important;
    border-bottom: #666666 1px double !important;
...
<link rel="stylesheet" type="text/css" href="js/jquery.tabs.css" media="print, projection, screen">
...
//  alert(linha);
    ...
    /*Criar TR - Linha*/
        ...
        el.setAttribute('style','width: 75%');
        celula.appendChild(el);
        ...
        el.setAttribute('class','input');
        el.setAttribute('style','width:80%');
        celula.appendChild(el);
        var el = document.createElement('img');
        el.setAttribute('src', 'imagens/lupa.png');
        el.setAttribute('border', '0');
...
function add_linha_bosch_security(linha2) {
    var linha = parseFloat($('#qtde_produto').val())+1;

    var dados = "<tr>";
        dados += "<td style='padding: 0; width: 95px'>";
            dados += "<input name='produto_"+linha+"'  class='frm' value='' type='text' maxlength='15'  style='width: 70px' />";
            dados += "<img  src='imagens/lupa.png' border='0' align='absmiddle' style='cursor: pointer' onclick='javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_"+linha+", document.frm_callcenter.descricao_"+linha+", \"referencia\", \"\")' >";
        dados += "</td>";
        dados += "<td style='padding: 0;width: 190px '>";
            dados += " <input name='descricao_"+linha+"'  class='frm' value='' type='text' maxlength='500'  style='width: 165px' />";
            dados += "<img src='imagens/lupa.png' border='0' align='absmiddle' style='cursor: pointer' onclick='javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_"+linha+", document.frm_callcenter.descricao_"+linha+", \"descricao\", \"\")''>";
        dados += "</td>";
        dados += "<td style='padding: 0; width: 60px '>";
            dados += " <input name='serie_"+linha+"' id='serie_"+linha+"' class='frm' value='' style='width: 50px' maxlength='25' />";
        dados += "</td>";
        dados += "<td style='padding: 0; width: 160px'>";
            dados += "<select name='defeito_reclamado_"+linha+"' id='defeito_reclamado_"+linha+"' style='width: 155px;' class='frm' onfocus='atualizaDefeito(document.frm_callcenter.produto_"+linha+".value,"+linha+",\"\");'> <option value='' id='opcoes' > - selecione -</option>" ;
            dados += "</select>";
        dados += "</td>";
        dados += "<td style='padding: 0; width: 150px '>";
            dados += "<input class='frm' type='text' name='defeito_reclamado_descricao_"+linha+"' id='defeito_reclamado_"+linha+"' value=''  style='width: 140px; margin-right: 5px' maxlength='100' />";
        dados += "</td>";
    dados += "</tr>";

    $("#tabela_itens tbody").append(dados);
    $('#qtde_produto').val(linha);
}
...
function buscaCEP(cep,endereco,bairro,cidade,estado) {
    if (endereco.value.length == 0 || 1 == 1) {
        http.open("GET", "ajax_cep.php?cep=" + escape(cep), true);
...
$(function() {
    ...
    <?php if ($produto and isset($callcenter) && ! empty($callcenter) ): ?>
...
<?
$w = 1;
if (!in_array($login_fabrica, array(2, 11, 25, 59)))
    $posicao -= ($posicao >= 10 and !in_array($login_fabrica, array(45, 46))) ? 4 : 1;

?>
$(function() {
<?php if ($login_fabrica == 85){ ?>
        $("#cpf").maskedinput("999.999.999/9999-99");
<?php } ?>
<?php
    // !129655 - Carregar dúvidas já cadastradas para este chamado [augusto]
    // !133157 - Carregar dúvidas quando o usuário não selecionou a aba dúvida, ou seja, a dúvida de produto [augusto]
    if ($produto and isset($callcenter) && ! empty($callcenter) ): ?>
        localizarFaq(document.frm_callcenter.produto_referencia.value,'faq_duvida_duvida','carregar');
        localizarFaq(document.frm_callcenter.produto_referencia.value,'faq_duvida_produto','carregar');
    <?php endif; ?>
...
//  $('#container').disableTab(3);
    //fxAutoHeight: true,
    $("#consumidor_fone,#telefone2,#filial_fone").maskedinput("(99) 9999-9999");
    $("#telefone3").maskedinput("(99) 99999-9999");
    $("#hora_ligacao").maskedinput("99:99");
    $("input[@rel='data']").maskedinput("99/99/9999");

    <?php if ($login_fabrica == 156): ?>
    $("#familia_1").change(function() {
        $("input[name='produto_1']").val("");
        $("input[name='descricao_1']").val("");
    });
    <?php endif ?>

    $("#cep").change(function() {
        var frm = document.frm_callcenter;
        buscaCEP(this.value, frm.consumidor_endereco, frm.consumidor_bairro, frm.consumidor_cidade, frm.consumidor_estado);
...
<? // Elgin automação, pesquisa e cadastro de filial ?>
    if ($("#encaminha_filial").length > 0) {
        ...
        $("#tbl_filial [name]").attr('disabled', 'disabled');
        $("#prazo_limite").maskedinput('99/99/9999 99:99?:99');
        ...
        // TODO NOW: complete CEP, ocultar botão depois de consultar, se está visível
        ...
        $("#encaminha_filial").change(function () {
            showHideFilial();
        });
        ...
        $("#BtnEditSubsidiary").click(function() {
            ...
            $(this).hide('fast');
            $("#filial.cnpj,#filial_nome,#filial_fantasia").unautocomplete();
        ...
        $("#BtnSaveSubsidiary").click(function() {
            ...
            $("#tbl_filial [name]").each(function(el, i) {
                params[i.name.replace(/filial\[(\w+)\]/, "$1")] = i.value;
            });
            var successMessage = (params.ajax == 'edit') ? "Filial atualizada.": "Filial cadastrada.";

            $("#tbl_filial [name],#"+selfID).attr('disabled', 'disabled');
            ...
            $.post(url_filial, params, function(data, statusText) {
                ...
                if (data.indexOf('|') < 1) {
                    alert(data);
                    $("#tbl_filial [name]").removeAttr('disabled');
                    return;
                }
                alert(successMessage);
                preencheFilial(data.split('|'));
                setAutocompleteFilial(); // ativa o autocomplete dos três campos
            });
        });

        $("#filial_cep").change(function() {
            var frm = document.frm_callcenter;
            buscaCEP(this.value, frm.filial_endereco, frm.filial_bairro, frm.filial_cidade, frm.filial_estado);
        }).keypress(function(ev) {
            return txtBoxFormat(this.form, this.name, '99999-999', ev);
        });

        setAutocompleteFilial();

        $("#filial_nome,#filial_cnpj,#filial_fantasia").result(function(ev, data, fmt) {
            preencheFilial(data);
        });
    }

    function showHideFilial() {
        if ($("#encaminha_filial[checked]").val() == 't') {
            $("#tbl_filial,#tbl_filial_title").show();
            $("#filial,#filial_nome,#filial_fantasia,#filial_cnpj").removeAttr('disabled');
        } else {
            $("#tbl_filial,#tbl_filial_title").hide();
            $("#tbl_filial [name]").attr('disabled', 'disabled');
        }
    }

      function setAutocompleteFilial() {
          $("#filial_nome").autocomplete(url_filial, {
              minChars: 3,
              delay: 200,
              matchContains: false,
              extraParams: {ajax: 'ajax', search_by: 'nome'},
              formatItem: formatACfilial
          });

          $("#filial_cnpj").autocomplete(url_filial, {
              minChars: 4,
              delay: 200,
              matchContains: false,
              cache: false,
              extraParams: {ajax: 'ajax', search_by: 'cnpj'},
              formatItem: formatACfilial,
              formatResult: function(row) {
                    var cnpj = $("#filial_cnpj").val().replace(/\D/g, '');
                    if (cnpj.length == 14 && row.length < 4 && confirm('Cadastrar uma nova filial?')) {
        ...
        $("#filial_fantasia").autocomplete(url_filial, {
            minChars: 6,
            delay: 200,
            matchContains: false,
            extraParams: {ajax: 'ajax', search_by: 'fantasia'},
            formatItem: formatACfilial
        });
    }
    ...
    $('#sel_tipo_at').change(function() {
        var valor = $(this).find('option:selected').val();
        var acao = (valor == '93' || valor== '0') ? 'none':'block';
        $('span.anexo_3').css('display', acao);
    });
    $('#sel_tipo_at').change();
...
function fnc_pesquisa_produto2 (campo, campo2, tipo, mapa_linha) {
    ...
    if ($("select[name^=familia]").length > 0) {
        var family_select_name = '#familia_' + $(xcampo).attr('name').replace(/.*(\d+)$/, "$1");

        var id_familia = $(family_select_name).val();
    ...
    if (xcampo.value != "" || id_familia != "") {
        var url = "";
        url = "produto_pesquisa_2.php?campo=" + xcampo.value + "&tipo=" + tipo + "&mapa_linha=t&voltagem=t" + familia;
        janela = window.open(url, "janela", "toolbar=no, location=no, status=yes, scrollbars=yes, directories=no, width=600, height=400, top=18, left=0");
        janela.referencia   = campo;
        janela.descricao    = campo2;
        janela.mapa_linha   = mapa_linha;
        ...
        if ($login_fabrica == 158) {
        ?>
            janela.voltagem = document.querySelector("input[name^=voltagem_]");
        <?php
        } else {
        ?>
            janela.voltagem     = document.frm_callcenter.voltagem;
...
function fnc_pesquisa_consumidor_callcenter (campo, tipo) {
    var url = "pesquisa_consumidor_callcenter_new.php?";
    if (campo.value != '' && 'nome cpf telefone cep atendimento'.split(' ').indexOf(tipo)>-1) {
        url += [tipo,'=',campo.value,'&','tipo=',tipo].join('');
    }
...
function fnc_tipo_atendimento(tipo) {
    ...
    if (tipo.value == 'C') {
        ...
        $('#cpf').attr('size', 18);
        $('#label_cpf').html('CPF:');
        $('#label_nome').unbind(function() {
            fnc_pesquisa_consumidor_callcenter (document.frm_callcenter.consumidor_nome, 'nome', 'assistencia');
        });
        $('#label_cnpj').unbind(function() {
            fnc_pesquisa_consumidor_callcenter (document.frm_callcenter.consumidor_cpf, 'cpf', 'assistencia');
        });
        $('#label_nome').unbind(function() {
            fnc_pesquisa_consumidor_callcenter (document.frm_callcenter.consumidor_nome, 'nome', 'revenda');
        });
        $('#label_cnpj').unbind(function() {
            fnc_pesquisa_consumidor_callcenter (document.frm_callcenter.consumidor_cpf, 'cpf', 'revenda');
        });
        $('#label_nome').click(function() {
            fnc_pesquisa_consumidor_callcenter (document.frm_callcenter.consumidor_nome, 'nome', 'consumidor');
        });
        $('#label_cnpj').click(function() {
            fnc_pesquisa_consumidor_callcenter (document.frm_callcenter.consumidor_cpf, 'cpf', 'consumidor');
        });
...
<? } elseif (in_array($login_fabrica, array(24, 30, 52, 59, 85))) { // HD 75777 ?>
function fnc_tipo_atendimento(tipo) {
        $('#cpf').val('');
    if (tipo.value == 'C') {
        $('#cpf').attr('maxLength', 14);
...
function atualizaDefeito(referencia,id_campo,valor){
    $.ajax({type: 'post',
        ...
        data: {referencia : referencia,valor:valor},
                async: false,
...
<body <? if ($login_fabrica==24) { ?> onload="javascript: var check = document.getElementById('indicacao_posto'); indicacao_suggar(check)"; <?}?>>
    <table width='700' border='0' align='center' cellpadding="0" cellspacing="2" style='margin: 0 auto;'>
        <tr class='msg_erro'>
            <td><? echo $msg_erro; ?></td>
        </tr>
    </table>
...
if(!in_array($login_fabrica,array(96,156,158))){ ?>
</div>
<div class="container-fluid">
        ...
        <td align='left'>
            ...
            <table width="100%" border='0'>
                <tr>
                    ...
                    <? if (strlen($callcenter)>0) {?>
                    <td align='left'><strong><? echo "Atendimento Nº $callcenter";?></strong></td>
                    <?}?>
                </tr>
            </table>
        </td>
    ...
    <tr>
        ...
        <div id='div_consumidor' style='padding:10px;border:#485989 1px solid; background-color: #e6eef7;background-color: #e6eef7;width:100%'>
            <table width='100%' border='0' align='center' cellpadding='2' cellspacing='2' style='font-size:11px;' id='tabela_consumidor'>
            <!--HD36903-->
            <?php if ($login_fabrica == 2) { ?>
            <tr>
                ...
                <td colspan='6'  align='left'>
                    <table border='0' cellpadding='3' cellspacing='0' width="50%">
                        <tr>
                            ...
                            <td align='left'>
                                CPF
                                <input type='radio' name='consumidor_revenda' value='C' <?PHP if (strlen($consumidor_cpf) == 14 or (strlen($consumidor_cpf) == 0 AND $login_fabrica <> 85)) { echo "CHECKED";}
                                if(strlen($callcenter) > 0) { echo " disabled"; }?> onclick="fnc_tipo_atendimento(this)">
                            </td>
                            <td align='left'>
                                CNPJ
                                <input type='radio' name='consumidor_revenda' value='R' <?PHP if (strlen($consumidor_cpf) == 18 or (strlen($consumidor_cpf) == 0 AND $login_fabrica == 85)) { echo "CHECKED";}
            ...
            <tr>
                <td align='left'><strong id="label_nome"><? echo ($login_fabrica <> 85) ? "Nome:" : "Razão Social";?></strong></td>
                <td align='left'>
                    <input type="hidden" name="consumidor_nome_anterior" value="<?php echo $consumidor_nome; ?>" />
                    <input name="consumidor_nome" id="consumidor_nome"  value='<?php echo $consumidor_nome ;?>' onkeyup="somenteMaiusculaSemAcento(this)" <? if($login_fabrica==11 and (strlen($callcenter) >0 or strlen($Id) >0) ){?>  readonly <?}?> <?if ($login_fabrica == 24) {?>class="input input_req2"<?} else {?> class="input input_req" <?}?> type="text" size="35" maxlength="500"
                ...
                <?
                if (!in_array($login_fabrica,array(85,156)) and ((strlen($consumidor_cpf) == 14 and strlen($callcenter) > 0) or strlen($callcenter) == 0)) {
                    echo "CPF:";
                    $limite ='14';
                    } elseif ((strlen($consumidor_cpf) == 18 and strlen($callcenter) > 0) or in_array($login_fabrica,array(85,156))) {
                    echo "CNPJ:";
                    $limite = "18";
                }?>
                </span></strong></td>
                <td align='left'>
                    ...
                    <input name="consumidor_cpf" id="cpf" value='<? echo $consumidor_cpf ;?>' class="input input_req" type="text" size="18" maxlength="<?=$limite?>" <?php
                    if ($login_fabrica == 156 or (strlen($consumidor_cpf) == 18 and strlen($callcenter) > 0)) echo " data-id-type='cnpj'";
                    if ($login_fabrica==11 and (strlen($callcenter) >0 or strlen($Id) >0)) {?> readonly <?}?>>
                ...
                <td align='left'><strong><?=($login_fabrica == 156) ? 'I.E.:' : 'RG:'?></strong></td>
                <td align='left'>
                    <input type="hidden" name="consumidor_rg_anterior" value="<?php echo $consumidor_rg; ?>" />
                    <input name="consumidor_rg" value='<? echo $consumidor_rg ;?>'  class="input input_req" type="text" size="14" maxlength="14" <? if($login_fabrica==11 and (strlen($callcenter) >0 or strlen($Id) >0) ){?>  readonly <?}?> >
                </td>
            </tr>
            <tr>
                ...
                <td align='left'>
                    ...
                    <input name="consumidor_email" value='<? echo $consumidor_email ?>' class="input input_req" type="text" size="40" maxlength="500" <?php echo $endereco_readonly; ?> >
                </td>
                ...
                <td align='left'>
                    ...
                    <input name="consumidor_fone" id="telefone" value='<? echo $consumidor_fone ;?>'  <? if ($login_fabrica == 24) { ?>class="input_req2"<? } else { ?> class="input input_req" <? } ?>  type="text" size="18" maxlength="15" onkeypress="return txtBoxFormat(this.form, this.name, '(99) 9999-9999', event);" <?php echo $endereco_readonly; ?> >
                </td>
                <td align='left'><strong>CEP:</strong></td>
                <td align='left'>
                    <input type="hidden" name="consumidor_cep_anterior" value="<?php echo $consumidor_cep; ?>" />
                    <input name="consumidor_cep" id="cep" value="<? echo $consumidor_cep ;?>"  class="input input_req" type="text" size="14" maxlength="9" <?php echo $endereco_readonly; ?> >
                </td>
            </tr>
            <tr>
                ...
                <td align='left'>
                    ...
                    <input name="consumidor_endereco" id='consumidor_endereco' value='<? echo $consumidor_endereco ;?>' class="input input_req" type="text" size="40" maxlength="500" <?php echo $endereco_readonly; ?> >
                </td>
                <td align='left'><strong>Número:</strong></td>
                <td align='left'>
                    <input type="hidden" name="consumidor_numero_anterior" value="<?php echo $consumidor_numero; ?>" />
                    <input name="consumidor_numero" id='consumidor_numero' value='<? echo $consumidor_numero ;?>' class="input input_req" type="text" size="18" maxlength="16" <?php echo $endereco_readonly; ?> >
                </td>
                ...
                <td align='left'>
                    ...
                    <input name="consumidor_complemento" id='consumidor_complemento' value='<? echo $consumidor_complemento ;?>' class="input input_req" type="text" size="14" maxlength="20" <?php echo $endereco_readonly; ?> >
                </td>
            </tr>
            <tr>
                ...
                <td align='left'>
                    ...
                    <input name="consumidor_bairro" id='consumidor_bairro' value='<? echo $consumidor_bairro ;?>' class="input input_req" type="text" size="40" maxlength="30" <?php echo $endereco_readonly; ?> >
                </td>
                <td align='left'><strong>Cidade:</strong></td>
                <td align='left'>
                    <input type="hidden" name="consumidor_cidade_anterior" value="<?php echo $consumidor_cidade; ?>" />
                    <input name="consumidor_cidade" id='consumidor_cidade' value='<? echo $consumidor_cidade ;?>'  <?if ($login_fabrica == 24) {?>class="input_req2"<?} else {?> class="input input_req" <?}?> type="text" size="18" maxlength="16" <?php echo $endereco_readonly; ?> >
            ...
            <? if($login_fabrica == 85){ ?>
                ...
                <tr>
                    <td colspan="1" align="left"><strong>Telefone Comercial:</strong></td>

                    <td colspan="1" align="left">
                        <input name="consumidor_fone2" id="telefone2" value="<?=$consumidor_fone2?>" class="input telefone" size="15" maxlength="14" autocomplete="off" spellcheck="true" type="text">
                    </td>
                    ...
                    <td colspan="1" align="left">
                        <input name="consumidor_fone3" id="telefone3" value="<?=$consumidor_fone3?>" class="input telefone" size="15" maxlength="14" autocomplete="off" spellcheck="true" type="text">
                    </td>
                </tr>
                ...
                <tr id="nome_fantasia">
                    ...
                    <td colspan="4" align="left">
                        <input name="nome_fantasia" value="<?=$nome_fantasia?>" class="input input_req" spellcheck="true" type="text">
                    </td>
                </tr>
            <? } ?>
            ...
            <tr>
                ...
                <?
                $origens = ($login_fabrica == 156) ?
                    array('Chat', 'Skype', '0800', 'Combinado com Consumidor', 'Revenda', 'Posto','Software House') :
                    array('Telefone' => 'Telefone', 'Email' => 'E-mail');

                echo array2select(
                    'origem', 'origem',
                    $origens,
                    $origem,
                    "class='select'", ($login_fabrica == 3 ? '' : ' '), ($login_fabrica == 156)
                ); ?>
                </td>
                ...
                <?php if ($login_fabrica == 156):
                    echo array2select(
                        ...
                        array('C' => 'Contrato', 'S' => 'Sem Contrato', 'G' => 'Garantia'),
                        $consumidor_revenda,
                        "class='select'", ' ', true
                    );
                else: ?>
                    <select name="consumidor_revenda" id='consumidor_revenda' class='select'>
                    <? if($login_fabrica ==3) { // HD 48900?>
                    <option value=''></option>
                    <? } ?>
                    <option value='C' <? if($consumidor_revenda == "C") echo "Selected" ;?>>Consumidor</option>
                    <option value='R' <? if($consumidor_revenda == "R" OR empty($consumidor_revenda)) echo "Selected" ;?>>Revenda</option>
                    </select>
                </td>
                <? endif; ?>
                <?php }?>
                <?php if ($login_fabrica == 156): ?>
                <td align="left"><input id="encaminha_filial" type="checkbox" <?=($filial)?'checked':''?> name="encaminha_filial" value="t"><strong>Atendimento para a Filial:</strong></td>
                <?php endif; ?>
            </tr>
        <? }else{ ?>
            <tr>
                ...
                <!-- <td><input id="prazo_limite" class="input_req" type="datetime" name="data_providencia" pattern="<?=$dateRegEx?>" value="<?=
                is_date($data_providencia, 'ISO', 'EUR')?>"></td> -->
            ...
            <tr>
                ...
                <td align='left'>
                            <?php if ($login_fabrica == 156):
                                echo array2select(
                                    ...
                                    array('C' => 'Contrato', 'S' => 'Sem Contrato', 'G' => 'Garantia'),
                                    $consumidor_revenda,
                                    "style='width:81px; font-size:9px'", ' ', true
                                );
                            else: ?>
                        <select name="consumidor_revenda" id='consumidor_revenda' class='select'>
                        <? if($login_fabrica ==3) { // HD 48900?>
                ...
                <td align="left"><input type="text" class='input' name="numero_controle" value=""></td>
        ...
        <div style='padding:10px;border:#485989 1px solid; background-color: #e6eef7;background-color: #e6eef7;width:100%;margin-bottom: 20px;'>

            <table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style='font-size:11px;' name='tabela_itens' id='tabela_itens'>
                <tr>
                    <td align='left' width="87"><strong>NF compra:</strong></td>
                    <td align='left'>
                        <input type="hidden"  name="nota_fiscal_anterior_$i" value="<?php echo $nota_fiscal; ?>"/>
                        <input name="nota_fiscal" style="width: 75%" id="nota_fiscal" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" value="<?php echo $nota_fiscal;?>" maxlength="10" />
                    </td>
                    <td align='left'><strong>Data NF:</strong></td>
                    <td align='left'>
                        <input type="hidden" name="data_nf_anterior_$i" value="<?php echo $data_nf; ?>" />
                        <input name="data_nf" style="width: 75%" id="data_nf" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" rel="data" value="<?php echo $data_nf ;?>">
                    </td>
                <?php

                    if (strlen($callcenter)>0) {

                        $sql_produto = "SELECT produto,descricao,referencia,familia,serie,
                                               defeito_reclamado,defeito_reclamado_descricao
                    ...
                    for ( $i = 1 ; $i <= $qtde_produto ; $i++ ) {
                        if (strlen($msg_erro)>0) {
                            ...
                            if (strlen($callcenter)>0) {
                                    ...
                                    $familia               = pg_fetch_result($res_produto,$i-1,'familia');
                ...
                <?php if ($login_fabrica == 156): ?>
                    ...
                    <td>
                        <input type="hidden" name="familia_anterior_<?=$i?>" value="familia_anterior">
                        <?=array2select(
                            "familia_$i", "familia_$i",
                ...
                <tr>
                    ...
                    <td align='left'>
                        ...
                        <input name="serie_<?=$i;?>" style='width: 75%' id="serie_<?=$i;?>" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" value="<?php echo $serie;?>" />
                    ...
                    <td align='left'>
                        ...
                        <input name="produto_referencia_<?=$i?>" class="input"  style="width:70%"  value='<? echo $produto_referencia ;?>'
                        onblur="javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_referencia_<?=$i;?>,document.frm_callcenter.produto_nome_<?=$i;?>,'referencia',document.frm_callcenter.mapa_linha); <?php  if ($login_fabrica <> 51){ # HD 41923 ?>
                            ...
                            <img src='imagens/lupa.png' border='0' align='absmiddle'
                        style='cursor: pointer'
                        onclick="javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_referencia_<?=$i;?>,document.frm_callcenter.produto_nome_<?=$i;?>,'referencia',document.frm_callcenter.mapa_linha)">
                    </td>
                    ...
                    <td align='left'>
                        ...
                        <input name="produto_nome_<?=$i?>"  class="input"  style="width:80%"  value='<?php echo $produto_nome ;?>'
                    ...
                    <td width="100">
                        <? if($login_fabrica != 85){?>
                            <input type='button' name='addlinha' class="btn-add" value='+ adicionar' onclick='function1(<?=$i?>)'>
                        <? } ?>
                    </td>
                </tr>
                <? }?>
        ...
        <div style='padding:10px;border:#485989 1px solid; background-color: #e6eef7;background-color: #e6eef7;width:100%;margin-bottom: 20px;'>
            <table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style='font-size:11px;padding: 10px;margin-bottom: 10px;'>
                <tr>
                    <td align='left' width="87"><strong>Linha:</strong></td>
                    <td align='left'>
                        <?
                            $sql = "SELECT  *
                                    FROM    tbl_linha
                            ...
                            if (pg_numrows($res) > 0) {
                                echo "<select name='mapa_linha' class='select'>\n";
                    ...
                    <td align='left'>
                        <select name='mapa_estado' class="select" id='mapa_estado'>
                    ...
                    <td align='left'><input type='text' id='mapa_cidade' class="input" name='mapa_cidade' value='<?=$mapa_cidade?>'>
                ...
                <tr>
                    ...
                    <?
                    if (strlen($callcenter)==0) {
                        if (in_array($login_fabrica, array(14,30,43,66,52,85))) {
                            $checked = "CHECKED";
                            $display = "block";
                        }
                        else {
        ...
        <? } ?>
        <div style='padding:10px;border:#485989 1px solid; background-color: #e6eef7;background-color: #e6eef7;width:100%;margin-bottom: 20px;'>
                <table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style='font-size:11px;padding: 10px;'>
                ...
                <tr>
                    ...
                    <td align='left' colspan='5'>
                        ...
                        <textarea name="reclamado_produto" rows="6" cols="110"  class="input" style='font-size:10px;width: 99%;' <? echo $read; ?>><?echo $reclamado ;?></textarea>
                    </td>
                </tr>

                <tr>
                    <td colspan="100%" style="text-align: center;"><br>
                        <input type="submit" class="btn-gravar" value="Gravar" name="btn_acao" />
                    </td>
                </tr>
            </table>
        </div>
        ...
        <?php if($login_fabrica == 85 AND !empty($callcenter)){ 

            $sql = "SELECT to_char(tbl_hd_chamado_item.data,'DD/MM/YYYY HH24:MM') AS data,comentario, tbl_hd_chamado.status FROM tbl_hd_chamado JOIN tbl_hd_chamado_item USING(hd_chamado) WHERE hd_chamado = $callcenter AND interno IS NOT TRUE ORDER BY hd_chamado_item DESC LIMIT 1";
            $res = pg_query($con,$sql);

            if(pg_num_rows($res) > 0){
        ?>
                <table style="width: 98%; border: #485989 1px solid; font-size: 10px; margin-bottom: 10px;" class="tabela">
                    <tr>
                        <td class="titulo_coluna" width="150">Data</td>
                        <td class="titulo_coluna" width="150">Situação</td>
                        <td class="titulo_coluna">Descrição</td>
                    </tr>
                    <tr>
                        <td><?=pg_fetch_result($res,0,'data')?></td>
                        <td><?=pg_fetch_result($res,0,'status')?></td>
                        <td><?=pg_fetch_result($res,0,'comentario')?></td>
                    </tr>
                </table>
        <?php 
            }
...
?>
    ...
    <?php
        if(!empty($hd_chamado)){
            ...
            $sql = "SELECT nome, to_char(tbl_hd_chamado.data,'DD/MM/YYYY HH:ii:ss') AS abertura FROM tbl_hd_chamado JOIN tbl_cliente_admin USING(cliente_admin) WHERE hd_chamado = $hd_chamado AND tbl_hd_chamado.fabrica = $login_fabrica";
            $res = pg_query($con,$sql);
    ?>
            <table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
                <tr>
                    <td align='center'>
                    <b><? echo "Atendimento $hd_chamado aberto por ".pg_fetch_result($res,0,nome). " em ". pg_fetch_result($res,0,abertura); ?></b>
                    </td>
                </tr>
            </table>
    <?
    ...
    <table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
        <tr>
            <td class='titulo_tabela' colspan='3'>Cadastro de Atendimento <?=($login_fabrica == 158) ? "(* obrigatório)" : ""?></td>
        </tr>
        <tr>
            <td width='300px'>&nbsp;</td>
            <td width='200px'>&nbsp;</td>
        ...
        <tr>
            ...
            <td>
                ...
                ?>
                    <input type="text" name="estado" value="<?php echo $dados['estado']?>" class='frm' style='width: 170px' />
                <?php
                }
                ?>
            </td>
        ...
        ?>
                ...
                <td align='left'>
                    ...
                    <?php
                        echo array2select(
                            ...
                            array('C' => 'Contrato', 'S' => 'Sem Contrato', 'G' => 'Garantia'),
                            $consumidor_revenda,
                            "class='frm' style='width:81px; font-size:9px'", ' ', true
                        );
                    ?>
                </td>

                <td align="left">
                    Nº Controle Cliente <br />
                    <input type="text" name="numero_controle" value="<?php echo $numero_controle; ?>" class="frm">
                </td>
                <!--
                <td>
                    Prazo Limite <br />
                    <input id="prazo_limite" class="frm" type="text" name="data_providencia"  value="<?=$data_providencia?>" style="width: 170px">
                </td>
                -->
            ...
            <tr>
                ...
                <td>
                    Responsável <br />
                    <input class="frm" id="resposavel" type="text" name="responsavel" value="<?=$responsavel?>">
                </td>

                <td colspan="2">
                    Telefone Responsavel <br />
                    <input class="frm" id="tel_responsavel" type="text" name="tel_responsavel" value="<?=$tel_responsavel?>">
                </td>
            </tr>

            <tr>
                <td align="left" colspan="3">
                    <input id="encaminha_filial" type="checkbox" <?=($_POST['encaminha_filial'] == 't')?'checked':''?> name="encaminha_filial" value="t"><strong>Atendimento para a Filial:</strong>
                </td>
            </tr>
        <?php
            }
    ...
    <table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
        ...
        <tr>
            <td colspan='6'>
                Tipo Atendimento<br>
                <select name='tipo_atedimento' id='sel_tipo_at' class='frm' style='width: 145px'>
        ...
        <tr>
            <td>
                Data da NF<br>
                <input type="text" name="data_nf" rel='data' value="<?php echo $data_nf?>" class='frm' style='width: 123px' maxlength='12' />
            </td>
            <td>
                Número da NF<br>
                <input type="text" name="numero_nf" value="<?php echo $nota_fiscal?>" class='frm' style='width: 110px' maxlength='20' />
            </td>
            ...
            <td colspan='3'>
                ...
                <input type="text" name="codigo_postagem" value="<?php echo $codigo_postagem?>" class='frm' style='width: 160px' maxlength='30' />
            </td>
            <?}?>
            <td>&nbsp;</td>
        </tr>
        ...
        <tr>
            <td colspan='5'>
                Consumidor Final<br>
                <input type="text" name="consumidor_final" value="<?php echo $consumidor_final?>" class='frm' style='width: 275px'  maxlength='100' />
            </td>
        ...
        <?}?>
                    ...
                    <table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'  name='tabela_itens' id='tabela_itens'>
                        ...
                        <?php
                            if (strlen($callcenter)>0) {
                                ...
                                $sql_produto = "SELECT produto,referencia,descricao,referencia,serie,defeito_reclamado,defeito_reclamado_descricao from tbl_hd_chamado_item join tbl_produto using(produto) where hd_chamado = $callcenter order by hd_chamado_item ";
                                ...
                                if($qtde_produto > 0){
                                    ...
                                    $produto                        = pg_fetch_result($res_produto, 0, "referencia");
                                    $descricao                      = pg_fetch_result($res_produto, 0, "descricao");
                                    $quantidade                     = 1;
                                    $defeito_reclamado_descricao    = pg_fetch_result($res_produto, 0, "defeito_reclamado_descricao");
                                    $defeito_reclamado              = pg_fetch_result($res_produto, 0, "defeito_reclamado");
                            ...
                            for ( $i = 1 ; $i <= $qtde_produto ; $i++ ) {
                                ...
                                if(strlen($callcenter) == 0){
                                    ...
                                    if ($login_fabrica == 158) {
                                        ...
                                        $tensao           = $_POST['tensao_'.$i];
                                        $tempo_instalacao = $_POST['tempo_instalacao_'.$i];
                                    }
                                }
                        ...
                        ?>
                        <tr>
                            <td style='padding: 0; width: 95px' nowrap>
                                <?php if($i == 1) echo "Referência<br>";?>
                            ...
                            <td style='padding: 0;width: 190px ' nowrap>
                                ...
                                <img src='imagens/lupa.png' border='0' align='absmiddle' style='cursor: pointer' onclick="fnc_pesquisa_produto2 (document.frm_callcenter.produto_<?=$i;?>, document.frm_callcenter.descricao_<?=$i;?>, 'descricao', '')">
                            </td>
                            <td style='padding: 0; width: 60px '>
                                <?php if($i == 1) echo (in_array($login_fabrica, array(156,158))) ? "Número de Série<br>" : "Série/Lote<br>";?>
                                <input name="serie_<?=$i;?>" id="serie_<?=$i;?>" class="frm" value="<?php echo $serie;?>" style='width: 80px' maxlength='25' />
                            </td>
                            <?
                            if($login_fabrica != 156){
                                if ($login_fabrica == 158) {
                                ?>
                                    <td style='padding: 0px 3px 0px 8px;'>
                                        <?php if($i == 1) echo "Voltagem<br>";?>
                                        <input type="text" name="voltagem_<?=$i?>" class="frm" value='<?php echo $voltagem ;?>' style='width: 35px' readonly />
                                    </td>
                                    <td style='padding: 0px 3px 0px 3px;'>
                                        <?php if($i == 1) echo "Tensão<br>";?>
                                        <input type="text" name="tensao_<?=$i?>" class="frm" value='<?php echo $tensao ;?>' style='width: 35px' />
                                    </td>
                                    <td style='padding: 0px 8px 0px 3px;' nowrap>
                                        <?php if($i == 1) echo "Tempo de Instalação<br>";?>
                                        <input type="text" name="tempo_instalacao_<?=$i?>" class="frm" value='<?php echo $tempo_instalacao ;?>' style='width: 85px' />
                                    </td>
                                <?php
                                }
                                ?>
                            ...
                            ?>
                                <td style='padding: 0; width: 150px '>
                                    <?php if($i == 1) echo ($login_fabrica == 156) ? "Defeito Reclamado <br />" : "Descrição Defeito<br>";?>
                                    <input class='frm' type='text' name='defeito_reclamado_descricao_<?php echo $i;?>' id='defeito_reclamado_<?php echo $i;?>' value='<?php echo $defeito_reclamado_descricao;?>'  style='width: 140px; margin-right: 5px' maxlength='100' />
                                </td>
                            <?php
                            }
                            ?>
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    ...
                                    <input name="descricao_defeito" value="<?php echo utf8_decode($descricao_defeito);?>" size="105" class="frm">
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            ?>
                <tr>
                    <td colspan="5">
                        Observação <br />
                        <textarea name="obs" style="width: 100%;" ><?=$obs?></textarea>
                    </td>
                </tr>
            <?php
            }
    ...
    ?>
    <br><br>
    <table width="700" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
        <tr>
        ...
        <tr>
            <td colspan='3'><br>
                <font size="1" color="#ff0000">
                    Enviar comprovante de Pagamento para os e-mails: <b>andre.dias@br.bosch.com</b> e <b>renato.lima2@br.bosch.com</b> indicando o Numero da OS
                </font><br><br>
            </td>
        </tr>
        <tr>
    ...
    <br><br>
    <table width="700" border="0" align="center" cellpadding="0" cellspacing="1" id="tbl_filial" class='formulario'>
        <tr>
            <td class='titulo_tabela' colspan='3'>Filial</td>
        </tr>

        <tr>
            <td colspan="3">
                <strong>* Para localizar uma revenda, digite a Razão Social ou CNPJ.</strong>
            </td>
        </tr>

        <?php
        if (!empty($msg_erro) and !empty($filial)) {
            $sql_filial = "
                SELECT tbl_filial.nome AS razao_social,
                    cnpj,
                    ie,
                    fantasia,
                    telefone AS fone,
                ...
                WHERE filial = $filial";
            $filial_id = $filial;
            $filial = pg_fetch_assoc(pg_query($con, $sql_filial));
        }
        ?>
        ...
        <tr>
            ...
            <td colspan="3">
                Loja <br />
                <input class="frm" id="filial_loja" type="text" name="filial[cod_filial]" value="<?=$filial['cod_filial']?>">
            </td>
        </tr>
        ...
        <tr>
            <td>
                Razão Social <br />
                <input id="filial" type="hidden" name="filial[id]" value="<?=$filial_id?>">
                <input class="frm" id="filial_nome" type="text" name="filial[razao_social]" value="<?=$filial['razao_social']?>">
            </td>

            <td>
                CNPJ <br />
                <input class="frm" id="filial_cnpj" type="text" name="filial[cnpj]" value="<?=$filial['cnpj']?>">
            </td>

            <td>
                Inscrição Estadual <br />
                <input class="frm" id="filial_ie" type="text" name="filial[ie]" value="<?=$filial['ie']?>">
            </td>
        </tr>
        ...
        <tr>
            <td>
                Fantasia <br />
                <input class="frm" id="filial_fantasia" type="text" name="filial[fantasia]" value="<?=$filial['fantasia']?>">
            </td>

            <td>
                Telefone <br />
                <input class="frm" id="filial_fone" type="text" name="filial[fone]" value="<?=$filial['fone']?>">
            </td>

            <td>
                CEP <br />
                <input class="frm" id="filial_cep" type="text" name="filial[cep]" value="<?=$filial['cep']?>">
            </td>
        </tr>
        ...
        <tr>
            <td>
                Endereço <br />
                <input class="frm" id="filial_endereco" type="text" name="filial[endereco]" value="<?=$filial['endereco']?>">
            </td>

            <td>
                Número <br />
                <input class="frm" id="filial_numero" type="text" name="filial[numero]" value="<?=$filial['numero']?>">
            </td>

            <td>
                Complemento <br />
                <input class="frm" id="filial_complemento" type="text" name="filial[complemento]" value="<?=$filial['complemento']?>">
            </td>
        </tr>
        ...
        <tr>
            <td>
                Bairro <br />
                <input class="frm" id="filial_bairro" type="text" name="filial[bairro]" value="<?=$filial['bairro']?>">
            </td>

            <td>
                Cidade <br />
                <input class="frm" id="filial_cidade" type="text" name="filial[cidade]" value="<?=$filial['cidade']?>">
            </td>
    ...
    <tr>
        <td colspan='6' align='center'><input type='submit' value='Gravar' name='btn_acao' <?php echo ($login_fabrica == 156 && strlen($hd_chamado) > 0) ? "style='display: none;'" : ""; ?>></td>
    </tr>
</table>
    <?php
    }
    ...
    ?>
        <br />
        <input type='submit' value='Gravar' name='btn_acao' style='padding: 2px 5px 2px 5px;' />
    <?php
    }
    ?>
</form>
