...
$campos_add = '';
	...
	if (strlen($nome) > 2) {
		...
		echo "<div class='lp_pesquisando_por'>Pesquisando pelo código: $codigo</div>";
			...
			for ($i = 0 ; $i < pg_num_rows($res); $i++) {
				...
				if(pg_num_rows($res) == 1){
					...
					if ($login_fabrica == 52)
						...
						echo "window.parent.retorna_posto('$codigo_posto','$posto',\"$nome\",'$cnpj',\"$cidade\",'$estado','$credenciamento','$num_posto', '$cep', \"$endereco\", '$numero', \"$bairro\"); window.parent.fncMostraBuscaOS('$codigo_posto', '$nome'); window.parent.Shadowbox.close();";
					else
						echo "window.parent.retorna_posto('$codigo_posto','$posto',\"$nome\",'$cnpj',\"$cidade\",'$estado','$credenciamento','$num_posto', '$cep', \"$endereco\", '$numero', \"$bairro\"); window.parent.Shadowbox.close();";
				...
				if ($login_fabrica == 52)
					...
					$onclick = "onclick= \"javascript: window.parent.retorna_posto('$codigo_posto','$posto','$nome','$cnpj','$cidade','$estado','$credenciamento','$num_posto', '$cep', '$endereco', '$numero', '$bairro'); window.parent.fncMostraBuscaOS('$codigo_posto', '$nome'); window.parent.Shadowbox.close();\"";
				else
					$onclick = "onclick= \"javascript: window.parent.retorna_posto('$codigo_posto','$posto','$nome','$cnpj','$cidade','$estado','$credenciamento','$num_posto', '$cep', '$endereco', '$numero', '$bairro'); window.parent.Shadowbox.close();\"";
