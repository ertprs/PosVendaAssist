...
if(isset($btn_acao)){
	...
	if (strlen($mes) == 0) {
		$msg_erro["msg"][] = "Para realizar a pesquisa é necessário selecionar um Mês";
		$msg_erro["campos"][] = "mes";
	}
	...
	if (in_array($login_fabrica, [167, 203])) {
		...
		$data_inicio = "01/$mes/$ano";
		if (!verifica_data_corte($data_corte, $data_inicio)) {
	...
	if (count($msg_erro['msg']) == 0) {
		...
		if ($_POST["gerar_excel"] && $count > 0) {
			...
			$thead .= "<thead>";
			$thead .= "<tr>";
			$thead .= "<th rowspan='2'>Código</th>";
			$thead .= "<th rowspan='2'>Nome</th>";
			if (in_array($login_fabrica, array(20))) {
				$thead .= "<th rowspan='2'>País</th>";
			}
			$thead .= "<th rowspan='2'>Linha</th>";
			if (in_array($login_fabrica, array(1))) {
				$thead .= "<th rowspan='2'>Marca</th>";
			}
			$thead .= "<th colspan='3'>Período</th>";
...
<div class="alert alert-warning">
	<h4>Data mínima para pesquisa 04/2019</h4>	
</div>
		...
		<div class="<?= ($login_fabrica != 1) ? 'span8' : 'span4'?>">
			<div class='control-group'>
                ...
                if ($login_fabrica == 117) {?>
                        <label class='control-label' for='linha'>Macro - Família</label>
                <?php
                } else { ?>
		...
		<table id="resultado_pesquisa" class='table table-striped table-bordered table-hover table-large'>
			<thead>
				<tr class='titulo_coluna'>
					<th rowspan="2" class="tac">Código</th>
					<th rowspan="2" class="tac">Nome</th>
					<? if (in_array($login_fabrica, array(20))) { ?>
						<th rowspan="2" class="tac">País</th>
					<? }
                    if (in_array($login_fabrica, array(117))) { ?>
                            <th rowspan="2" class="tac">Macro - Família</th>
                    <? } else { ?>
                            <th rowspan="2" class="tac">Linha</th>
                    <? } ?>

					<? if (in_array($login_fabrica, array(1))) { ?>
						<th rowspan="2" class="tac">Marca</th>
					<? } ?>
					<th colspan="3" class="tac">Período</th>
					<th rowspan="2" class="tac">Total</th>
				</tr>
				<tr class="titulo_coluna">
					<th><?= $data_1; ?></th>
					<th><?= $data_2; ?></th>
