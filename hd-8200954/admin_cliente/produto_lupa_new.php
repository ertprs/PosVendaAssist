...
if ($login_fabrica == 165 && $parametro == "numero_serie" && !$areaAdmin) {
...
<html>
				...
				$('#resultados').on('click', '.produto-item', function() {
					var info = JSON.parse($(this).attr('data-produto'));
 					if(info.reparo_na_fabrica == 't'){ //hd_chamado=2717074
						alert("Posto de assistência não tem autorização para realizar reparo ou descaracterizar (trocar peça, ressoldar e outros) este produto.\n \nCliente deverá ser orientado a ligar para 0800-775-1400.");
					}else{
	...
	<body>
					...
					<div class="span4">
						...
						<select name="parametro"  >
							...
							<? } else if (in_array($login_fabrica, array(52,158,161,165,169,170,175)) AND $parametro == "numero_serie") { ?>
								<option value="numero_serie"  <?=($parametro == "numero_serie")  ? "SELECTED" : ""?> >Número de série</option>
							<? } else { ?>
								...
								<? if (in_array($login_fabrica, [167, 203])) { ?>
									<option value="referencia_descricao" <?=($parametro == "referencia_descricao") ? "SELECTED" : ""?> >Referência/Descrição</option>
								<? } ?>
		...
		if ($login_fabrica == 152 || ($login_fabrica == 165 && isset($listaTroca)) || strlen($valor) >= 3) {
			switch ($parametro) {
			...
			if (isset($posto) && !in_array($login_fabrica, array(169, 170))) {
				$joinAdc .= " JOIN tbl_posto_linha ON tbl_posto_linha.linha = tbl_linha.linha AND tbl_posto_linha.posto = {$posto} ";
			}
			...
			if ($rows > 0) {
				if (in_array($login_fabrica, array(169, 170)) && isset($posto) && ($parametro == "numero_serie" OR $parametro == "referencia" OR $referencia == "descricao") ) {
					...
					if (!pg_num_rows($resPostoLinha)) {
						echo '
							<div class="alert alert_shadobox"><h4>Posto não atende a linha do produto selecionado</h4></div>
							</div></div></body></html>
						';
						exit;
					}
			...
			<div id="border_table">
				...
				if ($login_fabrica == 165 && $parametro == "numero_serie" && !$areaAdmin && !$postoInterno) {
					echo '<div class="alert alert-danger alert_shadobox"><h3><b>PRODUTO FORA DE GARANTIA</b></h3>
					 <h5 style="color:#000">Favor anexar a NF</h5></div>';
				} else {
					...
					if ($rowsProd > 0) {
						echo '<div class="alert alert_shadobox"><h4>Linha de Produto não atendida por seu PA, por gentileza entrar em contato com o consultor de sua região através do Help Desk</h4></div>';
					} else {
			...
			echo '<div class="alert alert_shadobox"><h4>Informe toda ou parte da informação para pesquisar!</h4></div>';
		}
		?>
			</div>
		</div>
