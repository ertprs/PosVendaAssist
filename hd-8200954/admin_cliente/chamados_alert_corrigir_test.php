...
//echo nl2br($sql) . '<br />';
	...
	$res_cidade = pg_exec( $con,$sql_cidade );
	
	$id_cidade = pg_result($res_cidade,0,'cidade');
...
<?php
	...
	echo $sql = "SELECT tbl_hd_chamado_item.hd_chamado,tbl_hd_chamado_item.hd_chamado_item, to_char(tbl_hd_chamado_item.data,'dd/mm/yyyy') as data_abertura, tbl_hd_chamado_extra.nome, 
			...
			WHERE tbl_hd_chamado_item.tincaso IS NOT NULL AND tbl_hd_chamado_item.os IS NULL";
	$res = pg_exec( $con,$sql );
	if( pg_num_rows( $res ) >0 ) {?>
<br />
