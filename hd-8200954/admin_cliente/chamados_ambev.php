...
$title="RELATÓRIO ANUAL DE OS ATENDIDAS";
...
<link rel="stylesheet" href="js/blue/style_cham_ambev.css" type="text/css" id="" media="print, projection, screen" />
...
<TABLE width='700' align='center' border='0' cellspacing='0' cellpadding='2'>
	...
	<tr>
		<td class="table_line" width='30'> &nbsp; </td>
		<TD class="table_line" style="text-align: center;">&nbsp;</TD>
		<td class="table_line" >
			<label for="regiao_chk">Região</label> <br />
	...
	<TR>
		...
		<TD class="table_line" align="left" nowrap>
			Cod Posto <br />
			<INPUT TYPE="text" NAME="codigo_posto" SIZE="17" value='<?=$codigo_posto?>' class='frm'><IMG src="imagens/lupa.png" style="cursor:pointer " align='absmiddle' onclick="javascript: fnc_pesquisa_posto (document.frm_pesquisa.codigo_posto,document.frm_pesquisa.nome_posto,'codigo')">
		</TD>

		<TD  class="table_line" style="text-align: left;" nowrap>
			Nome do Posto <br />
			<INPUT TYPE="text" NAME="nome_posto" size="35"  class='frm' value='<?=$nome_posto?>'> <IMG src="imagens/lupa.png" style="cursor:pointer" align='absmiddle' onclick="javascript: fnc_pesquisa_posto (document.frm_pesquisa.codigo_posto,document.frm_pesquisa.nome_posto,'nome')">
		</TD>
	...
	<TR>
		...
		<TD class="table_line" align="left">
			Referência <br />
			<INPUT TYPE="text" NAME="produto_referencia" SIZE="17"  class='frm' value='<?=$produto_referencia?>'><IMG src="imagens/lupa.png" style="cursor:pointer " align='absmiddle' onclick="javascript: fnc_pesquisa_produto (document.frm_pesquisa.produto_referencia,document.frm_pesquisa.produto_nome,'referencia')">
		</TD>
		<TD class="table_line" style="text-align: left;" width='280'>
			Descrição Produto <br />
			<INPUT TYPE="text" NAME="produto_nome" size="35"  class='frm' value='<?=$produto_nome?>'><IMG src="imagens/lupa.png" style="cursor:pointer " align='absmiddle' onclick="javascript: fnc_pesquisa_produto (document.frm_pesquisa.produto_referencia,document.frm_pesquisa.produto_nome,'descricao')">
		</TD>
	
		<td class="table_line" align="left" >
			Categoria <br />
			<select name="categoria" class="frm">
				<option value=""></option>
				<?php
					$sql = "SELECT cat, descricao FROM ambev_cat;";
					$res = pg_query($con,$sql);
					for($i=0; $i<pg_numrows($res);$i++) {
						$codigo = pg_result($res, $i, 'cat');
						$descricao = pg_result($res, $i, 'descricao');
						if (isset($_POST['categoria']) && $_POST['categoria'] == $codigo)
							$selected = 'selected';
						else $selected = '';
	...
	<TR>
		<TD class="table_line" colspan='5' style="text-align: center; padding:10px 0 10px 0;"><input type='submit' name='btn_consultar' value='Consultar'>
...
<?php
	...
	if(isset($btn_consultar)) {
		...
		if ($login_cliente_admin == 3890) {
			$sql = "SELECT responsabilidade FROM tbl_admin WHERE admin=$login_admin";
			$res_responsabilidade = pg_query($con, $sql);
			$responsabilidade = pg_result($res_responsabilidade, 0, 0);

			if (strlen($responsabilidade) > 0) {
				$responsabilidade = "'" . str_replace(",", "','", $responsabilidade) . "'";
				$cond_responsabilidade = " AND tbl_os.consumidor_estado IN ($responsabilidade) ";
			}
		}
		...
		if(!empty($categoria)) {
			$sql_joincat = "JOIN ambev_posto_cat ON tbl_os.posto = ambev_posto_cat.posto and ambev_posto_cat.cat = $categoria"; 
		}
		...
		$sql = "SELECT	tbl_os.os,
				...
				$cond3
				$cond4 
				$cond_responsabilidade
				order by os,consumidor_estado";
//		die(nl2br($sql));
		$res = pg_query( $con,$sql );
		if( pg_num_rows( $res ) >0 ) {?>
...
<?php
	echo "<div align='center' style='position: relative; left: 25'>";
		echo "<table border='0' cellspacing='1' cellpadding='0' align='center' style='margin-left:0%'>";
...
<table width='700px' align='center' id="chamados" class="tablesorter" cellpadding="0" cellspacing='1'>
<!--	-->
	<thead>
	...
	<?php
		...
		for( $i=0; $i< pg_num_rows($res);  $i++ ) {
			...
			if(!empty($finalizada)) {
				...
				$medio = str_pad($medio,3,"0",STR_PAD_LEFT);
			}
