	...
	if(strlen($msg_erro)==0){
		...
		if(strlen($msg_erro)== 0 AND strlen($hd_chamado) != 0){
			...
			$sql = "INSERT INTO tbl_hd_chamado_extra(
					...
					if(strlen($msg_erro)== 0){
						...
						$message="<b>Foi aberta uma Pré-OS de número $hd_chamado</b><br><br>";
						$message .= "<b> Favor dar continuidade no atendimento dessa Pré-OS.<br><br>";
		...
		if (strlen($msg_erro) == 0){
				...
				echo "<script language='javascript'>
					...
					window.open('print_atendimento_gravado.php?hd_chamado=$hd_chamado','hd_chamado',height=600, width=800); 
				</script>";
			}else{
				$res = pg_exec($con,"ROLLBACK TRANSACTION");
			}
...
//	alert(linha);
	...
	/*Criar TR - Linha*/
		...
		el.setAttribute('class','frm');
		el.setAttribute('style', 'width: 125px');
		celula.appendChild(el);
		var el = document.createElement('img');
		el.setAttribute('src', 'imagens/lupa.png');
		el.setAttribute('border', '0');
		...
		el.setAttribute('style', 'cursor: pointer; width: 20px');
		celula.appendChild(el);
		nova_linha.appendChild(celula);
		...
		el.setAttribute('style', 'cursor: pointer; width: 130px');
		celula.appendChild(el);
		nova_linha.appendChild(celula);

		nova_linha.appendChild(criaCelula(''));
...
?>
	...
	<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'  name='tabela_itens' id='tabela_itens'>
		...
		?>
			<tr>
				<td>
					<?php if($i == 1) echo "Série<br>";?>
					<input name="serie_<?=$i;?>" id="serie_<?=$i;?>" class="frm" value="<?php echo $serie;?>" style='width: 120px' />
				</td>
				<td>
					<?php if($i == 1) echo "Referência<br>";?>
					<input name="produto_<?=$i?>"  class="frm"  value='<? echo $produto ;?>' type="text" maxlength="15"  style='width: 120px' />
				...
				<td>
					...
					<input name="descricao_<?=$i?>"  class="frm" value='<?php echo $descricao ;?>'type="text" maxlength="500"  style='width: 123px' />
				...
				<td>
					...
					<input name="quantidade_<?=$i?>"  class="frm" value='<?php echo $quantidade ;?>' maxlength='2' style='width: 18px' />
				</td>
				<td>
					<?php if($i == 1) echo "Defeito Reclamado<br>";?>
					<input class='frm' type='text' name='defeito_reclamado_descricao_<?php echo $i;?>' id='defeito_reclamado_<?php echo $i;?>' value='<?php echo $defeito_reclamado_descricao;?>'  style='width: 130px' />
				</td>
				<td align='center' valign='middle' style='padding: 0; margin: 0; padding-top: 11px;'>
					<?php if($i == 1){?><input type='button' name='addlinha' value=' + ' onclick='add_linha_bosch_security(<?=$i?>)' /><?php }?>
				</td>
			</tr>
		<?php }?>
		</tbody>
	...
	<?php
		$posto = pg_fetch_array(pg_query("SELECT cnpj, nome FROM tbl_posto WHERE posto = 17673;"));	
	?>
	<br><br>
	<table width="700px" border="0" align="center" cellpadding="0" cellspacing="1" class='formulario'>
		<tr>
