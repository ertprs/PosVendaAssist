							...
							if($abre_os=='t') {
								...
								if(@pg_num_rows($resg)>0){
									...
									if(@pg_fetch_result($resx,0,0) == 't'){
										$msg_erro .= "Produto $referencia fora da garantia vencida em ".pg_fetch_result($resx,0,1);
									}
								}
							}
	...
	if(strlen($msg_erro)==0){
		...
		if ( strlen($msg_erro) == 0 && $abre_os=='t' && strlen( $_POST[ 'tincaso' ] ) > 0 ) {
			...
			if( strlen( $xcodigo_posto ) == 0 or $xcodigo_posto == 'null')	{
				$msg_erro .= 'Favor escolher um posto para abrir a Ordem de Serviço';
			}
			...
			if (strlen($msg_erro)==0) {
				...
				WHERE
				tbl_hd_chamado.hd_chamado=$hd_chamado
				AND tbl_hd_chamado_item.hd_chamado_item = $hd_chamado_item RETURNING os
				";
				$res = pg_query($con, $sql);
				...
				if (strlen($msg_erro)==0) {
					...
					if (strlen($msg_erro)==0) {
						...
						$sql = "INSERT INTO ambev_status_os 
								(ticasoalert,tiosesmaltec,tidescricao,tisubcodigo,tidata,tiobservacao) 
								values 
								($tincaso,$os_aberta,'Abertura','Pela Internet',current_timestamp,'Ordem de servico aberta')";
						$res = pg_exec($con,$sql);
					}
					else {
						$msg_erro = explode("CONTEXT", $erro_valida);
...
if(strlen($Id)>0){
	...
	if(pg_num_rows($res)>0){
		echo $consumidor_nome           = pg_fetch_result($res,0,nome);
		$cliente                   = pg_fetch_result($res,0,cliente);
		$consumidor_cpf            = pg_fetch_result($res,0,cpf);
		$consumidor_rg             = pg_fetch_result($res,0,rg);
		$consumidor_email          = pg_fetch_result($res,0,email);
...
<tr>
	...
	<div id='div_consumidor' style='display:inline; Position:relative;background-color: #e6eef7;width:100%'>
		...
		<? } ?>
		<tr>
			<td align='left'><acronym title='Campo Obrigatório'><font color="#AA0000"><strong>Nome:</strong></font></acronym></td>
			<td align='left'>
	...
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
		<tr>
			...
			<tr>
				...
				<td><input type='text' name='posto_km_tab' class="input" id='posto_km_tab'><div style='display:none' id='mapa'></div><input type='hidden' id='endereco_posto'/><input type='hidden' id='cep_posto'/></td>
			</tr>
		<tr>
			<td colspan='6'>
			<?
