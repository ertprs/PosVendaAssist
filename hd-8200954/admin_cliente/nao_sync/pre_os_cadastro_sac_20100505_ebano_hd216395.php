				...
				if ($qtde_produto>0	) {
					for ($w=1;$w<=$qtde_produto;$w++) {
						...
						if (strlen($produto_referencia)>0) {
							...
							if ($login_fabrica == 52 and strlen($defeito_reclamado)==0) {
...
<tr>
	...
	<div id='div_consumidor' style='display:inline; Position:relative;background-color: #e6eef7;width:100%'>
		...
		<tr>
			...
			<td align='left'>
				...
				<input name="consumidor_nome" id="consumidor_nome"  value='<?php echo $consumidor_nome ;?>' <? if($login_fabrica==11 and (strlen($callcenter) >0 or strlen($Id) >0) ){?>  readonly <?}?> <?if ($login_fabrica == 24) {?>class="input_req2"<?} else {?> class="input_req" <?}?> type="text" size="35" maxlength="500"
	...
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px' name='tabela_itens' id='tabela_itens'>
		...
		<tr>
			...
			<td align='left'>
				...
				<input name="nota_fiscal_$i" id="nota_fiscal_$i" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" value="<?php echo $nota_fiscal;?>" maxlength="10" />
			</td>
			<td align='left'><strong>Data NF:</strong></td>
			<td align='left'>
				<input type="hidden" name="data_nf_anterior_$i" value="<?php echo $data_nf; ?>" />
				<input name="data_nf_$i" id="data_nf_$i" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" rel="data" value="<?php echo $data_nf ;?>">
			</td>
		</tr>
		</thead>
		<tbody>
	...
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
		...
		<tr>
			...
			<?
			if(strlen($callcenter)==0){
				if ($login_fabrica == 14 or $login_fabrica == 43 or $login_fabrica == 66 or $login_fabrica == 52) {
					$checked = "CHECKED";
					$display = "block";
				}
				else {
