			...
			for($i=0;$i<pg_num_fields($res);$i++) {
				...
				if (strlen($valor)<=50) {
					...
					*/
					...
					if( $label2 == 'tinmodelofreezer' )
					{
						echo "<input type='text' name='produto' id='produto' />";
...
<table class='tabela' width='700px' align='center'>
	<tr class='subtitulo'>
		<td colspan='8'>Ordens de Serviço AMBEV</td>
	</tr>
	<tr class='titulo_coluna'>
		<td>OS</td>
	...
	?>
	<tr bgcolor="<?=$cor?>">
		<td><? if ($login_admin <> '3033') {?><a href='os_press.php?os=<?=$os?>' target='_blank'><?=$os ?></a><?}else {echo $os; } ?></td>
