...
if (strlen ($data) > 0) {
	$sql = "SELECT  DISTINCT
			...
			JOIN   (SELECT tbl_os_extra.os FROM tbl_os_extra JOIN tbl_extrato USING (extrato)
						WHERE  (tbl_os_extra.distribuidor = $login_posto OR tbl_extrato.posto = $login_posto)
...
flush();
	...
	if (pg_numrows($res) > 0) {
		...
		$sql = "SELECT  tbl_posto_fabrica.codigo_posto ,
				...
				JOIN    tbl_extrato ON tbl_os_extra.extrato = tbl_extrato.extrato
				WHERE   (tbl_os_extra.distribuidor = $login_posto OR tbl_os.posto = $login_posto)
				AND     tbl_extrato.data_geracao BETWEEN '$data_inicio' AND '$data_final'
