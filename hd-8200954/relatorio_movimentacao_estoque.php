...
if ($_POST["acao"]) { 
    ...
    if (empty($msg_erro["msg"])) {
        ...
        movimentacao.data_digitacao,
            ...
            AND estoque_posto.posto = {$login_posto}
...
if (pg_num_rows($resMovimentacaoEstoque) > 0) {
    ...
    <table class="table table-bordered table-large" style="margin: 0 auto;" >
        <thead>
            <tr>
                ...
                <td colspan="12"><?=$resMovimentacaoEstoque[0]["posto"]?></td>
            </tr>
            <tr>
                <th class="titulo_coluna" nowrap >Centro Distribuidor</th>
                <td colspan="12"><?=$postoUnidadeNegocio["centro_distribuidor_codigo"]." - ".$postoUnidadeNegocio["centro_distribuidor_descricao"]?></td>
            </tr>
            <tr>
                <th class="titulo_coluna" nowrap >Unidade de Negócio</th>
                <td colspan="12"><?=$postoUnidadeNegocio["unidade_negocio_principal"]?></td>
            </tr>
        ...
        <tbody>
            ...
            foreach ($resMovimentacaoEstoque as $row) {
                ...
                if (!empty($row->campos_adicionais)) {
                    $row->campos_adicionais = json_decode($row->campos_adicionais);
                    $unidade_negocio        = $row->campos_adicionais->unidadeNegocio;
                } else {
    ...
    $data    = date("dmYHi");
    $arquivo = "relatorio-movimentacao-estoque-{$login_fabrica}-{$login_posto}-{$data}.xls";

    system("touch /tmp/{$arquivo}");
    file_put_contents("/tmp/{$arquivo}", $html);
    copy("/tmp/{$arquivo}", "admin/xls/{$arquivo}");

    if (file_exists("admin/xls/{$arquivo}") && filesize("admin/xls/{$arquivo}") > 0) {
    ?>
        <hr />

        <p class="tac" >
            <button type="button" class="btn btn-success download-xls" data-xls="<?="admin/xls/{$arquivo}"?>" ><i class="icon-file icon-white" ></i> Download XLS</button>
        </p>
    <?php
    }
}
