...
Tecservi ï¿½ 80459 - 5037  --OK - falta exportar - ok
NL ï¿½ 80636 - 13951        -OK - falta exportar (jah estava digitada)- ok
...
//echo "<br><br>NOTA: $total_nota |  $base_icms |  $valor_icms |  $base_ipi |  $valor_ipi | qtde=$qtde_peca_na_nota<br><br>";
 			...
 			$sql = "INSERT INTO tbl_faturamento		  
					...
					VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',13996,$login_posto,$total_nota,'$nota_fiscal','2','Devolução de peças em garantia', $base_icms, $valor_icms, $base_ipi, $valor_ipi, $extrato, 'Devolução de peças do posto para à  Fábrica',$cfop_da_nota,99)";
			$res = pg_exec ($con,$sql);
...
//echo "<br><br>NOTA: $total_nota |  $base_icms |  $valor_icms |  $base_ipi |  $valor_ipi | qtde=$qtde_peca_na_nota<br><br>";
		...
		$sql = "INSERT INTO tbl_faturamento		  
				...
				VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',4311,$login_posto,$total_nota,'$nota_fiscal_tc','2','Devolução de peças em garantia', $base_icms, $valor_icms, $base_ipi, $valor_ipi, $extrato, 'Devolução de peças do posto para à  Fábrica')";
		$res = pg_exec ($con,$sql);
...
*/
	...
	if (strlen($msg_erro)==0 AND strlen($ressarcimento)>0 AND $ressarcimento==$extrato AND $qtde_produtos_ressarcimento>0){
		...
		if (strlen($ressarcimento_nota_fiscal)>0){
			...
			$sql = "INSERT INTO tbl_faturamento
					(fabrica, emissao,saida, posto, distribuidor, total_nota, nota_fiscal, serie, natureza, base_icms, valor_icms, base_ipi, valor_ipi, extrato_devolucao, obs)
					VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',13996,$login_posto,0,'$ressarcimento_nota_fiscal','2','Devolução de peças em garantia', 0, 0, 0, 0, $extrato, 'Devolução de peças do posto para à Fábrica')";
			$res = pg_exec ($con,$sql);
 ...
 OR $login_posto == '1352' OR $login_posto == '1515' OR $login_posto == '4311'
  OR $login_posto == '7072' OR $login_posto == '7076' OR $login_posto == '7076' OR
   $login_posto == '7294' OR $login_posto == '19046'){
	echo "<p>Tela em manutenção. Aguarde alguns instantes..</p>";
	echo "<br>";
	include "rodape.php"; 
	exit;
}
...
if ($ip == "201.26.23.85" AND 1==2){
	...
	if (count($array_nf_canceladas)>0){
		echo "<h3 style='border:1px solid #F7CB48;background-color:#FCF2CD;color:black;font-size:12px;width:600px;text-align:center'><b>As notas</b> ".implode(", ",$array_nf_canceladas)." foram <b>canceladas</b>. <br> Clique sobre o link de Pendências para o preenchimento das notas.</h3>";
	}
}
?>
<br>
...
#####################################################################################################################
		...
		if ( pg_numrows ($resX)>0){
			...
			for ($x = 0 ; $x < pg_numrows ($resX) ; $x++) {
				...
				if ($x%$numero_linhas==0 AND $x>0){
					...
					echo "<center>";
					echo "<b>Preencha este Nota de Devolução e informe o número da Nota Fiscal</b><br>Este número não poderá ser alterado<br>";
					echo "<br><IMG SRC='imagens/setona_h.gif' WIDTH='53' HEIGHT='29' BORDER='0' align='absmiddle'>Número da Nota: <input type='text' name='nota_fiscal_tc_$numero_nota_tc' size='10' maxlength='6' value='$nota_fiscal'>";
					echo "<br><br>";
					echo "</td>";
					echo "</tr>";
					echo "</table>";
	...
	if ($numero_linhas<>5000){
		...
		if($qtde_produtos_ressarcimento>0){
			...
			echo "<center>";
			echo "<b>Preencha este Nota de Devolução e informe o número da Nota Fiscal</b><br>Este número não poderá ser alterado<br>";
			echo "<br><IMG SRC='imagens/setona_h.gif' WIDTH='53' HEIGHT='29' BORDER='0' align='absmiddle'>Número da Nota: <input type='text' name='ressarcimento_nota_fiscal' size='10' maxlength='6' value='$nota_fiscal'>";
			echo "</td>";
			echo "</tr>";
			echo "</table>";
		}
	...
	if ($numero_linhas==5000){
		...
		if ($tem_mais_itens=='nao' AND $jah_digitado_tc>0){
			echo "<b>Não há mais peças para devolução.<br><br> <a href='extrato_posto_devolucao_lgr_itens.php?extrato=$extrato'>Clique aqui para consultar as notas de devolução</a></b>";
		}else{
