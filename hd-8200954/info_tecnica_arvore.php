...
$sql = "SELECT COUNT(comunicado) AS total_manuais
		FROM tbl_comunicado
		WHERE ativo IS NOT FALSE
		AND (tipo = 'Manual Técnico' OR tipo = 'Manual Tecnico')
...
<? if(($login_fabrica == 15 || $login_fabrica == 91 )  AND $tipo_comunicado == "video"){ ?>
	<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
		...
		?>
			...
			<td rowspan='2'class='detalhes' width='150'><?=traduz('Escolha a família que deseja consultar.')?></td>
		</tr>
	</table>
<? } else {
...
<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
	...
	<tr bgcolor = '#fafafa'>
				...
				for ($i = 0; $i < pg_numrows($res); $i++) {
					...
					if ($linha_anterior <> $linha) {
						...
						if (($login_fabrica <> 19 OR $nome <> "Metais") OR ($login_fabrica == 19 )) {
							echo "<a href='info_tecnica_visualiza.php?tipo=Atualização+de+Software&linha=$linha'>";
						}
						echo "$nome";
						echo "</a><br /></dt>";
					}
		...
		if($login_fabrica <> 87){
		?>
		<td rowspan='2'class='detalhes' width='150'><?=traduz('Escolha a ')?><?php if ($login_fabrica == 129) { echo 'linha'; }else{ echo 'família';} ?><?=traduz(' que deseja consultar.')?></td>
		<?php
		}
		?>
	</tr>
...
if (in_array($login_fabrica,array(1,35,45))) {
	if ($login_fabrica == 1) {
		...
		$lista_basica_vista = traduz('Lista básica');
		$lista_basica_vista_materiais = traduz('Lista básica de materiais');
	}
	?>
	<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
...
?>
    <table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
        ...
        <tr bgcolor = '#fafafa'>
            <td valign='top' class='menu'>
                <?=traduz("<br /><a href='produto_troca_direta.php' target='_blank'>Clique aqui caso o produto não tenha vista explodida</a><br /><br />");?>
            </td>
            <td rowspan='2'class='detalhes' width='150'>&nbsp;</td>
        </tr>
    </table>
...
if (in_array($login_fabrica, array(152,180,181,182))) {
	...
	$vet_tipo[] = traduz('Roteiros de Entrega Técnica');
	$vet_tipo[] = traduz('Manuais de Serviço');
	$vet_tipo[] = traduz('Manuais de Instrução');
	$vet_tipo[] = traduz('Documentação Padrão / Procedimentos');
		...
		<td  class="chapeu" colspan='2' >
			<?php 
				if ($login_fabrica == 178){
					echo "Material Técnico";
				}else if ($login_fabrica == 148){
					echo "Manual de Instruções";
				}else{
					echo "Esquema Elétrico";
				}
			?>
		</td>
	</tr>
              ...
              ORDER BY      nome,
				...
				if (pg_numrows($res) > 0) {
					...
					for ($i = 0; $i < pg_numrows($res); $i++) {
                        ...
                        $tipoHref = ($tipo == "Manual de instruções" || $tipo == "Manual de instrucoes") ? "Manual+de+instruções" : "Esquema+Elétrico";
...
if ($login_fabrica <> 3) {  //HD 18182 17700?>
	<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
		<tr bgcolor = '#efefef'>
			<td rowspan='3' width='20' valign='top'><img src='imagens/marca25.gif'></td>
			<td  class="chapeu" colspan='2' ><?=traduz('Alterações Técnicas')?></td>
		</tr>
					...
					for ($i = 0 ; $i < pg_numrows($res); $i++) {
						...
						if (strlen ($descricao) > 0) {
							...
							if ($login_fabrica == 175){
								echo traduz("- Ordem de produção: $versao");
							}
						} else {
							echo $comunicado_descricao;
						}
	...
	<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
		<tr bgcolor = '#fafafa'>
			<td rowspan='3' width='20' valign='top'><img src='imagens/marca25.gif'></td>
			<td class="chapeu" colspan='2' ><?=traduz('Manual Técnico')?></td>
		</tr>
		<tr bgcolor = '#fafafa'>
			<td colspan='2' height='5'></td>
		</tr>
			...
			<td valign='top' class='menu'><?php
				...
				if ($total_manual > 50) {//alterado Gustavo HD 8213
					...
					$sql = "SELECT DISTINCT tbl_familia.familia                                  ,
							...
							WHERE   tbl_linha.fabrica    = $fabrica_comunicado
							AND     tbl_comunicado.ativo IS NOT FALSE
							AND     (tbl_comunicado.tipo = 'Manual Técnico' OR tbl_comunicado.tipo = 'Manual Tecnico')
							AND    (tbl_comunicado.tipo_posto = $tipo_posto  OR  tbl_comunicado.tipo_posto IS NULL)
							AND    (tbl_comunicado.posto IS NULL OR tbl_comunicado.posto = $login_posto)
							".$sqlPostoLinha."
						UNION
