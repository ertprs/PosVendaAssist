<?php
	...
	if(isset($_POST['grava_hd_chamado'])){
		...
		$res_os = pg_exec($con,$sql_os);
			...
			$defeito_reclamado_descricao = "O protocolo teve origem no site Institucional Consulta OS - ".$defeito_reclamado_descricao;
			...
			if(strlen(trim($erro))==0){
				echo "<div class='alert alert-success' role='alert'>Contato Gravado com Sucesso! <br /> Número de protocolo: <strong>$hd_chamado</strong></div>";
			}else{
		    ...
		    $mensagem = "Foi aberto um chamado no callcenter de número $hd_chamado ";

			mail("daniel.cabral@gammaferramentas.com.br", "Aberto chamado no CONSULTA O.S", $mensagem, $headers);
