...
if(strlen($Enviar) > 0){
	...
	$aux_setores     = trim($_POST['setores']);
	...
	if(empty($aux_produto)) $aux_produto = "null";
	...
	if(strlen($aux_setores) == 0 AND strlen($msg_erro) == 0){
		$msg_erro = "Preencha o campo Setor";
	}
	...
	if(strlen($msg_erro) == 0) {
		...
		$tipo_contato = "reclamacao_produto";
			...
			$aux_msg = $aux_assunto." - ".$aux_msg;
			//$msg_consumidor = $aux_msg;
	...
	if (strlen ($msg_erro) == 0) {
		...
		$sqlFamilia = "SELECT descricao FROM tbl_familia WHERE fabrica = $login_fabrica AND familia = $aux_familia AND ativo IS TRUE ";
		...
		$sqlProduto = "SELECT descricao FROM tbl_produto WHERE fabrica_i = $login_fabrica AND produto = $aux_produto AND ativo ";
		$resProduto = pg_exec($con,$sqlProduto);

		if(pg_num_rows($resProduto) > 0){
			$produto_descricao = pg_fetch_result($resProduto, 0, 'descricao');
		}
		...
		if(pg_num_rows($resCidade) > 0){
			$cidade_descricao = pg_fetch_result($resCidade, 0, 'nome');
		}

		switch ($aux_setores) {
			case 'assistencia_tecnica':
				$email_setor = "mauricio.paz@gammaferramentas.com.br";
				$desc_setor = "ASSISTÊNCIA TÉCNICA";
				break;
			case 'comercial':
				$email_setor = "soraya.zanela@gammaferramentas.com.br";
				$desc_setor = "COMERCIAL";
				break;
			case 'financeiro':
				$email_setor = "denilson.cardoso@gammaferramentas.com.br";
				$desc_setor = "FINANCEIRO";
				break;
			case 'logistica':
				$email_setor = "laertes.adamovicz@gammaferramentas.com.br";
				$desc_setor = "LOGÍSTICA";
				break;
			case 'marketing':
				$email_setor = "marketing@gammaferramentas.com.br";
				$desc_setor = "MARKETING";
				break;
			case 'posvenda':
				$email_setor = "mauricio.paz@gammaferramentas.com.br";
				$desc_setor = "PÓS-VENDAS";
				break;
			case 'produto':
				$email_setor = "mauricio.paz@gammaferramentas.com.br";
				$desc_setor = "PRODUTOS";
        ...
        $headers 	.= "From: Fale Conosco <no_reply@telecontrol.com.br> \r\n";

		$msg 		= "Foi aberto um atendimento nº $hd_chamado<br/>
						Nome: $aux_nome<br/>
						CPF: $aux_cpf<br/>
						Email: $aux_email<br/>
						Telefone: $aux_telefone<br/>
						Cep: $aux_cep<br/>
						Número: $aux_numero<br/>
						Endereço: $aux_endereco<br/>
						Bairro: $aux_bairro<br/>
						Complemento: $aux_complemento<br/>
						Cidade: $cidade_descricao<br/>
						Estado: $estado<br/>
						Setor: $desc_setor<br/>
						Assunto: Reclamação da Assistência Técnica<br/>
						Família: $familia_descricao<br/>
						Produto: $produto_descricao<br/>
						Mensagem: $aux_msg<br/>
		";

		mail($email_setor, "FALE CONOSCO",	 $msg, $headers);
	...
	<html>
		<head>
			...
			<script language="JavaScript">
				...
				function buscaCidade (estado, cidade) {
					console.log("teste = "+ cidade + estado);
					$.ajax({
						async: false,
						url: "callcenter_cadastra_gamma.php",
						type: "POST",
						data: { buscaCidade: true, estado: estado, cidade : cidade},
						cache: false,
		...
		<body>
			<div class='box_content' style='margin: 0 auto;'>
				<h1 style="color:#9ba6a6; font-size:28px; font-family:'MyriadProBold',Verdana, Geneva, sans-serif; padding:10px 0; margin: 0; padding-bottom: 0;">Fale Conosco.</h1>
				<form method="post"  action="<?=$PHP_SELF?>" method="post" name="form1" id="formCentral" enctype="multipart/form-data">
					...
					<div>
						...
						<input type="text" name="email" id="email" value='<?php echo $aux_email;?>' tabindex='3'></input>


						<label for="cep">CEP:</label>
						<input type="text" name="cep" id="cep" onblur="buscaCEP(this.value )" value='<?php echo $aux_cep;?>' tabindex='5'></input>



						<label for="endereco">Endere&ccedil;o:</label>
						<input type="text"  name="endereco" id="endereco"  value='<?php echo $aux_endereco;?>' tabindex='7'></input>

						<label for="complemento">Complemento:</label>
						<input type="text" name="complemento" id="complemento" value='<?php echo $aux_complemento;?>' tabindex='9'></input>

						<label for='estado'>Estado:</label>
						<select name='estado' id='estado' tabindex='11'>
							<option></option>
							<?php
						...
						<select name="assunto" id='assunto' style="width:710px;" tabindex='13'>
							<option value='0' selected> - selecione -</option>
							<option value='Reclamação da Assistência Técnica' <?php if($aux_assunto == 'Reclamação da Assistência Técnica') echo " selected ";?>>Reclamação da Assistência Técnica</option>
							<option value='Dúvidas' <?php if($aux_assunto == 'Dúvidas') echo " selected ";?>>Dúvidas</option>
							<option value='Elogios' <?php if($aux_assunto == 'Elogios') echo " selected ";?>>Elogios</option>
							<option value='Reclamações' <?php if($aux_assunto == 'Reclamações') echo " selected ";?>>Reclamações</option>
							<option value='Sugestões' <?php if($aux_assunto == 'Sugestões') echo " selected ";?>>Sugestões</option>
						</select>
						...
						<select name='familia' id='familia'  onchange="buscaProduto(this.value )" tabindex='14'>
							<?php
					...
					<div>
						...
						<input style="width:30px; padding:0px; float:inherit;  margin: 0px 0px 0px 0px;" type='radio' name='consumidor_cpf_cnpj' id='consumidor_cfp' value='C'
							<?PHP
						...
						<input style="width:30px; padding:0px;  margin: 0px 0px 7px 0px;" type='radio' name='consumidor_cpf_cnpj'id='consumidor_cnpj' value='R'
							<?PHP
						...
						<input type="text" name="cpf" id="cpf" value='<?php echo $aux_cpf;?>' style="margin:0px 0px 15px 0px; padding:0;" tabindex='2'></input>

						<label for="telefone">Telefone:</label>
						<input type="text" name="telefone" id="telefone" value='<?php echo $aux_telefone;?>' tabindex='4'></input>

						<label for="numero">N&uacute;mero:</label>
						<input type="text" name="numero" id="numero" value='<?php echo $aux_numero;?>' tabindex='6'></input>

						<label for="bairro">Bairro:</label>
						<input type="text" name="bairro" id="bairro" value='<?php echo $aux_bairro;?>' tabindex='8'></input>

						<label for="cidade">Cidade:</label>
						<select name="cidade" id='cidade' title='Selecione um estado para escolher uma cidade' tabindex='10'>
							<option></option>
						</select>

						<label for="cidade">Setor:</label>
						<select name='setores' id='setores' tabindex='12'>
							<option></option>
							<?php
								foreach ($array_setores as $x => $y) {
									echo '<option value="'.$y.'"'.($aux_setores == $y ? ' selected="selected"' : '').'>'.$x."</option>\n";
								}
							?>
						</select>


						<label for='produto' style="margin-top:109px;" value='<?php echo $aux_produto;?>' >Produto:</label>
						<select name='produto' id='produto' tabindex='15'>
							<option></option>
						</select>
					</div>
					...
					<p>
						...
						<textarea name="msg" id="msg" style="float: left; width: 698px;" tabindex='16'><?php echo $aux_msg;?></textarea>
					</p>
