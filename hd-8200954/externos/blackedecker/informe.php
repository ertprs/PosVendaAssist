...
function carregaDados($con,$login_fabrica,$data_inicio,$data_fim,$pais,$grafico,$cidade = null,$marca = null,$posto = null) {
...
/* HD-3017902
    *Alterado em todos os gráficos
    *    DE => $resposta = json_decode(stripslashes($respostaJson),TRUE);
    *    PARA $resposta = json_decode($respostaJson,TRUE);
    *    DE => if (urlencode($cidade) != $resposta['cidade']) {
    *        continue;
    *    }
    *    PARA => if (urlencode($cidade) != strtoupper($resposta['cidade'])) {
    *        continue;
    *    }
*/
...
function geraGrafico01($json,$login_fabrica,$con,$marca = null, $cidade = null, $posto = null) {
    ...
    if (in_array($marca,$arrayMarcas)) {
        ...
        $seriesPie[] = array(
            "type"          => "pie",
            "name"          => utf8_encode("Puntuaciones ".$arrayPontMarcas[$marca]),
            "size"          => 200,
        ...
        $seriesPie[] = array(
            "name"          => utf8_encode("Resultados Generales"),
            "type"          => "pie",
            "size"          => 200,
            "dataLabels"    => array("enabled"=>true),
            "tooltip"       => array("pointFormat" => "<b>{point.percentage:.2f}%</b>"),
...
function geraGrafico02($json,$login_fabrica,$con,$marca = null, $cidade = null, $posto = null) {
    ...
    if (is_array($novoArrayEquip)) {
        ...
        // $graficoEquipamentos        = str_replace($arrayEquipamento['es'],$arrayEquipamentoTrad,$graficoEquipamentos);
        // $graficoEquipamentos        = str_replace($arrayEquipamento['pt'],$arrayEquipamentoTrad,$graficoEquipamentos);
...
function geraGrafico03($json,$login_fabrica,$con, $cidade = null, $posto = null, $marca = null) {
    ...
    //"Costo y/ó presupuesto de reparación",
    $tradValues = array(
        "Atención y soporte (Telefónica)",
        "Atención y soporte (Recepción del centro de servicio)",
        "Falla temprana de la herramienta",
        "Calidad del producto",
        "Tiempo de respuesta",
        "Costo y presupuesto de reparación",
        "Seguimiento de la reparación",
        "Tiempo de reparación",
        "Calidad de la reparación",
        "Servicio prestado por el centro de servicio"
    );
...
function geraGrafico05($json,$login_fabrica,$con,$posto = null, $cidade = null, $marca = null) {
    ...
    $tradSatisfacao = array(
        ...
        "Aspecto de las instalaciones de servicio",
    ...
    //HD-3110652  Adicionado o max_total
...
function geraGrafico06($json, $cidade = null, $posto = null) {
    ...
    $graficoSeriesHeat[] = array(
        "name"          => utf8_encode("Puntaje por Satisfacción"),
        "borderWidth"   => (int)1,
        "data"          => $graficoHeat,
        "dataLabels"    => array(
            "enabled"   => (bool)true,
...
if (filter_input(INPUT_POST,'bt_periodo')) {
    ...
    //Validação Datas
    if (strlen(filter_input(INPUT_POST,"data_inicio"))) {
    ...
    if (strlen(filter_input(INPUT_POST,"data_fim"))) {
    ...
    if (count($msg_erro["campos"])>0) {
        $msg_erro["msg"][] = "Rellene los campos requeridos";
    }
    ...
    if (count($msg_erro["msg"])==0) {
        ...
        if ($aux_data_inicial > $aux_data_final) {
            ...
            $msg_erro["msg"][] = "Intervalo de fechas Incorrecto.";
            ...
            if ($periodo_meses == 'f') {
                ...
                $msg_erro["msg"][] = "Las fechas deben haber un alcance de hasta 6 meses";
                $msg_erro["campos"][] = "data_inicio";
                $msg_erro["campos"][] = "data_fim";
...
if(filter_input(INPUT_POST,"grafico",FILTER_VALIDATE_INT)){
    ...
    echo carregaDados($con,1,$dataInicioAjax,$dataFimAjax,$paisAjax,$graficoAjax,$cidadeAjax,$marcaAjax,$postoAjax);
    exit;
}
...
<link href="../../admin/plugins/select2/select2.css" type="text/css" rel="stylesheet" media="screen" />
<link rel='stylesheet' type='text/css' href='../../plugins/multiselect/multiselect.css' />
...
<script src="../../admin/js/highcharts_4.2.5.js"></script>
<script src="../../admin/js/modules/heatmap_4.2.5.src.js"></script>
<script src="../../admin/js/modules/data.js"></script>
...
<style type="text/css">
    ...
    header h1{
        background: url("images/topo_informe.png") no-repeat scroll 0 0 /100% auto;
        display:block;
        height:105px;
        width:695px;
        margin:0 auto;
...
function grafico01(marca){
    ...
    optionMarcas    += "<option value=''>Seleccione la marca específica</option>";
    ...
    .fail(function(){
        $("#mostraGrafico").text("No ha sido posible cargar el gráfico.")
        .css({
...
function grafico02(marca){
    ...
    .done(function(result){
        ...
        if (result.marcaEscolhida != null) {
            subtitle = "nivel de fiabilidad de la marca "+result.marcaEscolhida;
        } else {
            subtitle = "Nivel de fiabilidad de cada equipo";
        }
        $('#casoNecessite').css("display","block").html(optionMarcas);
        $('#mostraGrafico').highcharts({
            chart: {
...
function grafico03(marca){
    ...
    .done(function(result){
        ...
        if (result.marcaEscolhida != null) {
            subtitle = "Basado en el motivo del cliente para puntuar el atendimiento de la marca "+result.marcaEscolhida;
        } else {
            subtitle = "Basado en el motivo del cliente para puntuar el atendimiento";
        }
        $('#casoNecessite').css("display","block").html(optionMarcas);
        $('#mostraGrafico').highcharts({
            chart: {
            ...
            title: {
                text: 'Relación de la puntuación'
            },
            subtitle: {
                text: subtitle
            },
            ...
            var acumulaValor = 0;
            var table   = "<table><tr>";
            table += "<th>Motivo de la calificación</th>";
            table += "<th>Q</th>";
            table += "<th>%</th>";
            table += "</tr>";
            $.each(chart.series, function () {
...
function grafico04(marca){
    ...
    .done(function(result){
        ...
        if (result.marcaEscolhida != null) {
            subtitle = "Nivel de fiabilidad a lo largo de los días de la resolución de la orden de servicio de la marca "+result.marcaEscolhida;
        } else {
            subtitle = "Nivel de fiabilidad a lo largo de los días de la resolución de la orden de servicio";
        }
        $('#casoNecessite').css("display","block").html(optionMarcas);
        $('#mostraGrafico').highcharts({
            chart: {
            ...
            title: {
                text: 'Resultado por DIAS EN CENTRO DE SERVICIO '
            },
            subtitle: {
                text: subtitle
            },
            ...
            tooltip: {
                formatter:function(){
                    var header = "<b>Días en C.S.: "+this.x+"</b><br/>";
                    var s = "";
                    var soma = 0;
                    $.each(this.points, function () {
                            s += '<br/><b style="color:'+this.color+'">' + this.series.name + '</b>: ' +
            ...
            var table   = "<table><tr>";
            table       += "<th>Dias de la reparación</th>";
            table       += "<th>Promotor</th>";
            table       += "<th>Passivo</th>";
            table       += "<th>Detractor</th>";
            table       += "<th>Total</th>";
...
function grafico05(marca){
    ...
    .done(function(result){
        ...
        if (result.marcaEscolhida != null) {
            subtitle = "Nivel de satisfacción con el servicio de centro de servicio de la marca "+result.marcaEscolhida;
        } else {
            subtitle = "Nivel de satisfacción con el servicio de centro de servicio";
        }
        $('#casoNecessite').css("display","block").html(optionMarcas);
        $('#mostraGrafico').highcharts({
            chart: {
            ...
            title: {
                text: 'Resultado por SATISFACCIÓN DEL CLIENTE '
            },
            subtitle: {
                text: subtitle
            },
            xAxis: [{
                title: {
                    text: 'En relación a tratamiento'
                },
                labels: {
                    align:"center",
    ...
    .done(function(result){
        ...
        $('#mostraGrafico').highcharts({
            ...
            xAxis: [{
                ...
                labels: {
                    autoRotationLimit:30, //hd_chamado=3110652
                    align:"center", //hd_chamado=3110652
                    padding:10 //hd_chamado=3110652
                },
                categories: result.categories,
                crosshair: true
            }],
            yAxis: [{
                ...
                max: result.max_total //HD-3110652
                /* Teste HD-3110652
                    //tickAmount: 6,
                    //tickInterval: 500,
                    //allowDecimals: true,
...
function grafico06(){
    ...
    .done(function(result){
        $('#mostraGrafico').highcharts({
            ...
            title: {
                text: 'Matriz de Relación - Satisfacción / Lealtad'
            },
            xAxis: {
            ...
            tooltip: {
                formatter: function () {
                    return '<b>' + this.series.yAxis.categories[this.point.y] + '</b> fue elegido por <br><b>' +
                        this.point.value + '</b> clientes que caen en <br><b>' + this.series.xAxis.categories[this.point.x] + '</b>';
                }
            },
...
$(function () {
    ...
    $("#cidade").change(function(){
        ...
        .done(function(data){
            retorno.push("<option value=''></option>");
            $(data).each(function(key,val){
//                 nomeCorrigido = $('<div/>').html(val.nome).text();
...
<title>Informe de satisfacción - Black+Decker</title>
</head>
<body>
<div id="tudo">
    <header>
...
?>
        <form id="frm_pesquisa" action="<?=$PHP_SELF?>" method="POST" class="form-search form-inline tc_formulario">
        <div class='container tc_container' style="background-color:#D3D3D3;">
            <div class="titulo_tabela">Parámetros de búsqueda</div>
            <div class='row-fluid'>
                <div class='span2'></div>
                <div class="span4">
                    <div class='control-group <?=(in_array("data_inicio", $msg_erro["campos"])) ? "error" : ""?>' >
                        <label class="control-label" for="data_inicio">Fecha de Inicio</label>
                        <div class="controls controls-row">
                            <div class="span5">
                                <h5 class='asteristico'>*</h5>
                                <input id="data_inicio" name="data_inicio" class="span12" type="text" value="<?=filter_input(INPUT_POST,'data_inicio')?>" />
                            </div>
                        </div>
                    </div>
                </div>
                <div class="span4">
                    <div class='control-group <?=(in_array("data_fim", $msg_erro["campos"])) ? "error" : ""?>' >
                        <label class="control-label" for="data_fim">Fecha de Finalización</label>
                        <div class="controls controls-row">
                            <div class="span5">
                                <h5 class='asteristico'>*</h5>
                                <input id="data_fim" name="data_fim" class="span12" type="text" value="<?=filter_input(INPUT_POST,'data_fim')?>" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class='row-fluid'>
                ...
                <div class="span4">
                    <div class='control-group'>
                        <label class='control-label' for='cidade'>Ciudad</label>
                        <div class="controls controls-row">
                            <div class="span12">
            ...
            <div class='row-fluid'>
                ...
                <div class="span4">
                    <div class='control-group'>
                        <label class='control-label' for='posto'>Centro de Servicio</label>
                        <div class="controls controls-row">
                            <div class="span12">
            ...
            <div class='row-fluid'>
                <div class="span12" align="center" >
                    <p class="tac">
                        <input type="submit" id="bt_periodo" class="btn btn-primary" value="Búsqueda" name="bt_periodo" >
                    </p>
                </div>
            </div>
        </div>
...
?>
    <div id="botoes">
        <button class="btn btn-small" id="grafico01">Puntuación por Marcas</button>
        <button class="btn btn-small" id="grafico02">Puntuación por Equipos</button>
        <button class="btn btn-small" id="grafico03">Relación de la puntuación</button>
        <button class="btn btn-small" id="grafico04">Dias en centro de servicio</button>
        <button class="btn btn-small" id="grafico05">Satisfacción del cliente</button>
        <button class="btn btn-small" id="grafico06">Satisfacción / Lealtad</button>
    </div>
    <br />
