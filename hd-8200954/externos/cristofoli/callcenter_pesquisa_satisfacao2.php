...
$atendimento    = filter_input(INPUT_GET,'atendimento');
$os             = filter_input(INPUT_GET,'os');
$venda          = filter_input(INPUT_GET,'venda');
$fabrica        = 161;
$fabrica_nome   = "CRISTÓFOLI";

$tipo = (empty($atendimento) && empty($venda))
    ? "ordem_de_servico_email"
    : ((!empty($atendimento) && empty($venda))
        ? "externo"
        : "externo_outros");

if (!empty($atendimento) && empty($venda)) {
	$sql = "SELECT  tbl_hd_chamado.fabrica
        	FROM    tbl_hd_chamado
        	WHERE   tbl_hd_chamado.hd_chamado = $atendimento
		";
} else if (empty($atendimento) && !empty($venda)) {
    $sql = "
        SELECT  tbl_venda.fabrica
        FROM    tbl_venda
        WHERE   venda = $venda
    ";
} else {
...
if ($_POST['ajax'] && $_POST['gravaPerguntas']) {
    ...
    $sqlObrigatorio = " SELECT pergunta_obrigatoria
                        FROM tbl_pesquisa
    ...
    if (pg_num_rows($resObrigatorio) > 0) {
        $pesquisa_obrigatoria = pg_fetch_result($resObrigatorio, 0, 'pergunta_obrigatoria');
    }
...
<div class="row-fluid logo-row">
    <div class="span12">
        <div id="topo">
            <div class="container">
                <br />
                <img class="logo_gl" src='../../logos/logo_cristofoli.jpg'/>
            </div>
        </div>
    </div>
</div><br /><br /><hr />
...
<div class='container'>
    <div style="padding-top: 5px; margin-left: 7px;">
        <strong>Obs*</strong> Perguntas destacada em <strong style='color:#F00;'>vermelho</strong> são de resposta obrigatórias.
    </div>
    <?php
    $sql = "
        SELECT  tbl_pesquisa_pergunta.ordem     ,
...
?>
        <table  id="pesquisa_satisfacao" class='table table-striped table-bordered table-hover table-fixed'>
            <thead>
                <tr>
                    <th colspan="100%" id="ver_pesquisa" class="titulo_tabela">Pesquisa de Satisfação do Cliente: <?=$descricao?></th>
                </tr>
            </thead>
            <tbody>
<?php
        ...
        foreach (pg_fetch_all($res) as $key) {
            ...
            $cond_os            = (empty($os) OR $os == "null") ? " os IS NULL " : " os  = $os " ;
            $cond_callcenter    = (empty($atendimento) OR $atendimento == "null") ? " hd_chamado IS NULL " : " hd_chamado  = $atendimento " ;
            $cond_venda         = (empty($venda) OR $venda == "null") ? " venda IS NULL " : " venda  = $venda " ;
                    ...
                    for ($x=0; $x < pg_num_rows($res); $x++) {
                        ...
                        switch ($item_tipo_resposta_tipo) {
                            ...
                            case 'date':
                                ...
                                $width="";
                                $html_pesquisa .= ' <input  type="text" class="data" style="width:'.$width.'" name="perg_opt'.$key['pergunta'].'"  class="frm date" value="'.$value_resposta.'" '.$disabled.' />';
