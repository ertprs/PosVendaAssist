...
if ($_GET['action']=='cidades') {
	...
	if(strlen($estado) > 0) {
		echo $sql_cidades =	"SELECT  LOWER(mlg_cidade)||'#('||count(mlg_cidade)||')' AS cidade
							FROM (SELECT posto,tipo_posto,UPPER(TRIM(TRANSLATE(contato_cidade,'áâàãäéêèëíîìïóôòõúùüçÁÂÀÃÄÉÊÈËÍÎÌÏÓÔÒÕÚÙÜÇ', 'aaaaaeeeeiiiioooouuucAAAAAEEEEIIIIOOOOUUUC')))
														AS mlg_cidade,
...
if ($_GET['action']=='postos') {
	...
	if (isset($_GET['cidade'])) $cidade= utf8_decode(getPost("cidade"));
	...
	$sql = "SELECT
				...
				UPPER(TRIM(TRANSLATE(tbl_posto_fabrica.contato_cidade,'áâàãäéêèëíîìïóôòõúùüçÁÂÀÃÄÉÊÈËÍÎÌÏÓÔÒÕÚÙÜÇ', 'aaaaaeeeeiiiioooouuucAAAAAEEEEIIIIOOOOUUUC')))
															AS cidade,
			...
			AND UPPER(TRIM(TRANSLATE(contato_cidade,'áâàãäéêèëíîìïóôòõúùüçÁÂÀÃÄÉÊÈËÍÎÌÏÓÔÒÕÚÙÜÇ',
						...
						= UPPER(TRIM(TRANSLATE('$cidade','áâàãäéêèëíîìïóôòõúùüçÁÂÀÃÄÉÊÈËÍÎÌÏÓÔÒÕÚÙÜÇ',
													'aaaaaeeeeiiiioooouuucAAAAAEEEEIIIIOOOOUUUC')))
        ...
        <thead>
            <tr align='center' class='bold'>
                <th style='width:215px' width='215'>Nome do Posto</th>
                <th style='width:235px' width='235'>Bairro - Endere&ccedil;o</th>
                <th style='width: 86px' width= '86'>Telefone</th>
                <th style='width: 40px' width= '40'>E-Mail</th>
                <th style='width: 40px' width= '40'>Mapa</th>
            </tr>
        </thead>
<?
...
?>
	<base href="https://posvenda.telecontrol.com.br/assist/externos/mapa_rede/imagens/">
	<link rel="stylesheet" href="../css/mf.css" type="text/css">
	<!-- JavaScript -->
	<script type="text/javascript" src="https://posvenda.telecontrol.com.br/assist/js/jquery-1.4.2.min.js"></script>
	<!--[if IE]>
	...
	<script type="text/javascript">
		var doc_self = document.location.href;
		$(document).ready(function() {
			...
			$('#voltar,#menu_sup_mapa').click(function () {
				if ($('#mapabr').is(':hidden')) $('#postos_estado').fadeOut('normal').parent().find('#mapabr').hide(10).delay(500).fadeIn('normal');
			});
		}); // FIM do jQuery
	</script>
</head>
...
<!-- Mapas de imagens -->
	...
	<map id="MapaEstados" name="MapaEstados">
		...
		<area shape="poly" name='AM' title="Amapá"				 coords="216,62,225,50,236,41,235,36,228,31,221,9,209,31,192,30,203,39,">
		...
		<area shape="poly" name='GO' title="Goiânia"			 coords="226,249,233,241,246,239,258,240,264,235,260,232,263,224,263,218,246,218,245,205,263,206,267,214,269,206,275,204,275,199,272,187,257,190,245,185,242,190,233,183,228,201,222,209,215,217,207,229,209,239,">
