...
// 					$link_mapa = "<a title='Localizar no mapa' href='https://maps.google.com/maps?f=q&source=s_q&hl=pt-BR&geocode=&q=$longitude,$latitude&sll=$end_mapa&ie=windows-1252' target='_blank'>";
// 				}else {
...
//  				}
				$link_mapa.= "<img src='https://www.impressionar.com.br/multimidia/_arquivos/mapa.jpg' width='16'></a>";
...
//  Adiciona um evento onClick para cada 'area' que vai alterar o valor do SELECT 'estado'
	...
	$('#mapabr').css('opacity',0.3);
	$('map area').click(function() {
        if ($('#fabrica').val()=="") return false;  // N„o roda se n„o tem f·brica selecionada
...
//  insere no select 'cidades'
	$('#fabrica').change(function() {
	    ...
	    if (fabrica == '') {
			...
			$('#cidades').val('').change();
            $('#estados').slideDown('fast');
            $('#estado').val('').change();
			$('#mapabr').css('opacity',1);
	...
	$('#estado').change(function() {
		...
		$.get(document.location.pathname, {'action': 'cidades','fabrica':fabrica,'estado': estado},
		  function(data){
	...
	$('#cidades').change(function() {
		...
		if (cidade) {
    		$.get("<?=$PHP_SELF?>", {'action': 'postos','fabrica':fabrica,'estado': estado,'cidade': cidade},
    		  function(data){
...
<body>
<h2 valign='middle'><img src='https://www.telecontrol.com.br/logo_telecontrol_2007.gif' width='15%'>&nbsp;&nbsp;AssistÍncia TÈcnica</h2>
    <center>
	<div id='frmdiv'>
		<form name='frm_mapa_rede_gama' action='<?=$PHP_SELF?>' method='post'>
			<fieldset for="frm_mapa_rede_gama">
				...
				<div id='mapabr'>
					...
					<img src="mapa_azul.gif" usemap="#Map2" border="0">
				</div>
