		...
		$sql_cidades =	"SELECT  LOWER(mlg_cidade)||'#('||count(mlg_cidade)||')' AS cidade
							...
							FROM tbl_posto_fabrica
								WHERE credenciamento<>'DESCREDENCIADO'
									...
									AND contato_estado='$estado' AND fabrica=51) mlg_posto
...
if ($_GET['action']=='postos') {
	...
	$sql = "SELECT
				...
				ARRAY(SELECT familia FROM tbl_posto_linha JOIN tbl_familia USING(familia)
									WHERE posto=tbl_posto_fabrica.posto AND tbl_familia.fabrica = $fabrica) AS familias
			FROM   tbl_posto
...
// 				}
				...
				echo "</td>";
				echo "\t\t\t<td>$fone</td>";
                if (strlen($email)>5 and strpos($email,"@")>0) {
					echo "\t\t\t<td title='$email'>";
                	echo "<a href='mailto:".strtolower($email)."'><img src='email_envelope.jpg'></a>";
                } else {
					echo "<img src='no_e-mail.jpg'></td>";
				}
...
$html_titulo = "Ga.Ma Italy - Mapa da Rede Autorizada";
...
* {
	...
	#mapabr {float: left;position:relative;top: -1em;height:340px}
		#mapabr h2 {
			padding: 0;
			padding-left: 1em;
			margin-top:   2em;
...
#tblres table td:nth-child(4) {text-align: center;background-color: white;}
...
<div id='frmdiv'>
	<form name='frm_mapa_rede_gama' action='<?=$PHP_SELF?>' method='post'>
		<fieldset for="frm_mapa_rede_gama" align='center'>
