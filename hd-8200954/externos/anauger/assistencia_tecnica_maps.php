...
/** Busca Cidades - Ajax cidades */
if (isset($buscaAjax) && $buscaAjax == 'cidades') {
    ...
    try {
        ...
        $sql = "SELECT DISTINCT UPPER(TRIM(fn_retira_especiais(PF.contato_cidade))) AS contato_cidade
                ...
                JOIN tbl_posto_linha   AS PL ON PL.posto = PF.posto AND PL.linha in($linha)
                WHERE PF.fabrica      = 177
                AND PF.contato_estado = '$estado'
                AND PF.credenciamento = 'CREDENCIADO'
                AND PF.posto <> 6359
                AND PF.divulgar_consumidor IS TRUE 
		ORDER BY 1 ASC";

	$sql = "SELECT upper(fn_retira_especiais(cidade)) AS contato_cidade FROM tbl_ibge WHERE estado = '$estado' ORDER BY cidade";
        ...
        if (pg_num_rows($res) > 0) {
            ...
            for ($i = 0; $i < pg_num_rows($res); $i++) {
                ...
                $options['option'][] = "<option value='$cidade'>$cidade</option>";
            }

        } else {

            $options['option'][] = "<option value='' selected >Esse estado nao antende essa linha</option>";
            $options['msg'] = True;
        }
        ...
        exit(json_encode(['error' => True, 'msg' => $e]));
    }
}
...
if (isset($_POST['endereco'])) {
    ...
    $geocode      = $TcMaps->geocode($rua, null, $bairro, $cidade, $estado, null, $cep);
        ...
        if (is_array($geocode)) {
          ...
          /*  if ($busca == 'busca_endereco') {

                $whereX = " AND tbl_posto_fabrica.contato_cidade = '$cidade' 
                            AND tbl_posto_fabrica.contato_estado = '$estado' ";

            } else {
	*/
                $calcDistancia = ",(111.045 * DEGREES(ACOS(COS(RADIANS({$latitude_consumidor})) * COS(RADIANS(tbl_posto_fabrica.latitude)) * COS(RADIANS(tbl_posto_fabrica.longitude) - RADIANS({$longitude_consumidor})) + SIN(RADIANS({$latitude_consumidor})) * SIN(RADIANS(tbl_posto_fabrica.latitude)))))::integer AS distance";
                
                $whereBusca = "WHERE x.distance <= 50";

                $orderBusca = " x.distance ASC ";
            //}
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (estado.length != 0 && cidade.length != 0 && cep.length == 0) {
            ...
            $(function() {
                ...
                $('#btn_acao').click(function() {
                    ...
                    (new Promise(function (resolve, reject) {
                        ...
                        var fabrica  = <?=$cod_fabrica;?>;
                        var busca_estado = 'busca_cep'; 

                        let estado = $('#estado').val();
                        let cidade = $('#cidade').val();
                        
                        if (estado.length != 0 && cidade.length != 0 && cep.length == 0) {
                            busca_estado = 'busca_endereco';
                        }
                            ...
                            complete: function(data) {
                                ...
                                if (data != "Nenhum Posto Autorizado localizado para esta linha e CEP!") {
                                    ...
                                    if (data == "Nenhum Posto Autorizado localizado para esta linha e CEP!") {
                                        ...
                                        $("#msgErro").html('Não encontramos nenhuma assistência próxima de sua residência, favor entrar em contato através dos canais:<br /><br /><strong>E-mail:</strong> assistenciatecnica@anauger.com.br <br /><strong>Fone:</strong> 11-4591-1661');
                                    } else {
                ...
                $(document).on('click', '#solicitar_asstec', function(){
                    var linha   = $("#linha").val(); 
                    var familia = $("#familia").val();
                    var cep     = $("#cep").val();
                    var dpto    = "Assistência Técnica";

                    window.location.href = "../callcenter/callcenter_cadastra_anauger.php?departamento=" + dpto + "&linha=" + linha + "&cep=" + cep + "&familia=" + familia;
                });
            });
            /* Loading Imagem */
            function loading(e) {
            ...
            function buscaCidade(estado, callback) {
                ...
                if(estado.length > 0 && linha.length > 0) {
                    ...
                    $.ajax({
                        ...
                        $("#select2-cidade-container").text('Selecione');
                        $("#msgErro").hide();
                        ...
                        if (r['error'] == true) {
                            ...
                            r['option'].forEach(function (option) {
                    ...
                    alert("Selecione a Linha para continuar");
                }
            }
        ...
        <div class="container-fluid" id="consulta_mapa_rede">
            <div class='row'>
                ...
                <div id="campos_localizacao"> 
                    ...
                    <div class='col-lg-4 col-md-4'>
                        <span style="color: #008080;">* Campos Alternativos ao CEP</span>
                        <br /><br />
                        <div class="form-group col-lg-12 col-md-12" id="divEndereco">
                            <div class="controls controls-row">
                                <label class="control-label" for="estado">Estado</label>
                                <div class="asterisco" style="color: #008080;">*</div>
                                <div class="controls controls-row">
                                    <select name="estado" id="estado" autofocus required onchange="buscaCidade(this.value)">
                                        <option value="">Selecione</option>
                                        <?php foreach ($estadosBR as $sigla => $estadoLbl) { ?>
                                            <option value="<?=$sigla?>"><?=$estadoLbl?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                        </div>
                        <div class="form-group col-lg-12 col-md-12" id="divCidade">
                             <div class="controls controls-row">
                                <label class="control-label" for="linha">Cidade</label>
                                <div class="asterisco" style="color: #008080;">*</div>
                                <div class="controls controls-row">
                                    <select name="cidade" id="cidade" autofocus required>
