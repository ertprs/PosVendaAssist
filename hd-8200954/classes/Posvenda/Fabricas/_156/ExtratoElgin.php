...
class ExtratoElgin
{
    ...
    public function relacionaOsCallcenter($fabrica, $posto, $extrato, $dia_extrato)
    {
        ...
        $sql = "UPDATE tbl_os_extra 
            ...
            AND tbl_os.tipo_atendimento <> 262
        ...
        $calcula_os_sem_valor = \Posvenda\Regras::get("calcula_os_sem_valor", "extrato", $fabrica);

        if ($calcula_os_sem_valor == true) {
            $nao_calcula_posto_interno = \Posvenda\Regras::get("nao_calcula_posto_interno", "extrato", $fabrica);
    ...
    public function removeExtratoZerado($extrato)
    {
		...
		$sql = "UPDATE tbl_extrato_lancamento set extrato = null where extrato = $extrato ; 
			DELETE FROM tbl_extrato
            WHERE extrato = $extrato
            AND total = 0";
