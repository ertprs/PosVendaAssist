<?php
namespace Posvenda\Fabricas\_170;
...
class Os extends OsPosvenda
{
    ...
    public function finaliza($con, $troca_produto_api = false, $login_admin = null, $origem = null)
	{
        ...
        $sqlVerAgendamento = "SELECT tbl_tecnico_agenda.data_agendamento,
                            ...
                            ORDER BY tbl_tecnico_agenda.tecnico_agenda desc limit 1";
        ...
        $sql_defeito = "SELECT
                        ...
                        JOIN tbl_diagnostico d ON d.defeito_constatado = odrc.defeito_constatado AND d.fabrica = {$this->_fabrica}
                        JOIN tbl_servico_realizado sr ON sr.servico_realizado = oi.servico_realizado AND (sr.gera_pedido IS TRUE OR sr.troca_produto IS TRUE OR sr.ativo IS TRUE) AND sr.fabrica = {$this->_fabrica}
                        WHERE o.os = {$this->_os}
                        AND (d.defeito = odrc.defeito
                        OR d.defeito = oi.defeito)";
        $res_defeito = pg_query($con, $sql_defeito);
        if (pg_num_rows($res_defeito) == 0) {
            throw new \Exception("{$this->_sua_os} - É necessário lançar o defeito constatado e defeito da peça na ordem de serviço");
        } else {
            $sql = "
                SELECT
            ...
            $os_cortesia = pg_fetch_result($res, 0, 'cortesia');
              ...
              if ($fora_garantia != 't' && $defeitos_fora_garantia == 0 && $os_cortesia != 't') {
                ...
                // $data_abertura_valida = pg_fetch_result($res, 0, 'data_abertura');
                ...
                if ($grupo_atendimento == "P" && $km_google != "t"){
                    ...
                    if (!in_array('notafiscal',$array_tipos_inseridos) || !in_array('produto',$array_tipos_inseridos) || !in_array('assinatura',$array_tipos_inseridos)) {
                        throw new \Exception("{$this->_sua_os} - É obrigatório anexar a nota fiscal do produto, foto do produto e a OS assinada pelo consumidor");
                    }
    ...
    public function getDadosOSExport($os)
    {
        ...
        $sql = "
	       SELECT DISTINCT
            ...
            OR (oi.os_item IS NOT NULL AND sr.ativo IS TRUE AND sr.gera_pedido IS NOT TRUE))
        ...
        if (!$query->execute() || $query->rowCount() == 0) {
            ...
            foreach ($res as $os) {
                ...
                if ($os['troca_de_peca'] == 't' && $os['peca_estoque'] != 't') {
        ...
        foreach ($array_dados as $os) {
            ...
            if (!empty($os['importacao_fabrica'])) {
                ...
                $sql = "
        		    ...
        		    WHERE comentario ~ '^(433|430|064|138|030|101)\s\-\s';
                ";
                $query = $pdo->prepare($sql);
    	    ...
    	    if (empty(trim($os['nota_fiscal'])) && (in_array($os['grupo_atendimento'], array('R','G')) || $os["consumidor_revenda"] == "R")) {
        	    ...
        	    if (empty(trim($data_nf)) && in_array($os['grupo_atendimento'], array('R','G'))) {
        	        $data_nf = $data_abertura;
        	    }
    	    }
    ...
    public function exportOS($dados, $LogExportacao = null)
    {
        ...
        foreach ($array_dados as $os => $dados) {
            ...
            if (empty(trim($dados['nota_fiscal'])) && (in_array($dados['grupo_atendimento'], array('R','G')) || $dados["consumidor_revenda"] == "R")) {
                ...
                if (empty(trim($data_nf)) && in_array($dados['grupo_atendimento'], array('R','G'))) {
                    $data_nf = $data_abertura;
                }
            }
            ...
            if (strlen($xml['NewDataSet']['ZCBSM_DADOS_ORDEM_SERVICOTable']['QMNUM']) > 0 && empty($xml['NewDataSet']['ZCBSM_PT_MENSAGEMTABLE']['MSGTY'])) {
                ...
                foreach ($xmlRetorno as $retorno) {
                    ...
                    if (in_array($retorno['MSGNO'], array("053", "043")) && empty($dados['os_posto'])) {
    ...
    public function getOsPendenteExportacao($data_inicial = null, $data_final = null, $posto = null, $os = null)
    {
        ...
        $sql = "
            SELECT DISTINCT
                ...
                o.exportado,
                o.posto,
                (SELECT COUNT(ao.auditoria_os) FROM tbl_auditoria_os ao WHERE ao.os = o.os AND ao.liberada IS NULL AND ao.cancelada IS NULL AND ao.reprovada IS NULL) AS auditoria,
                (
                    SELECT TO_CHAR(rsl.date_start, 'DD/MM/YYYY HH24:MI') AS data_primeira_integracao
            ...
            {$wherePosto}
        ";
        #$sql = "SELECT os FROM tbl_os WHERE fabrica = :fabrica AND os IN (48687993)";
        $query = $pdo->prepare($sql);
        $query->bindParam(':fabrica', $this->_fabrica, \PDO::PARAM_INT);
