...
class Client {
	...
	public static function makeTelecontrolClient($project,$resource,$token='churrus'){
		global $login_fabrica;
		$client = new Client();
		$client->url = Client::API_HOST.$project.'/'.$resource;
		if($token){
			$client->urlParams['token'] = $token;
		}
		$client->urlParams['fabrica']         = $login_fabrica;
		$client->urlParams['user_ip_address'] = $_SERVER['REMOTE_ADDR'];
		$client->urlParams['fabrica'] = $login_fabrica;
		return $client;
	}
