...
$historico_intervencao = (in_array($login_fabrica, array(1,2,3,6,11,14,25,35,43,45,50,72,74)) or $login_fabrica > 84);
...
if(strlen($interacao_os) > 0){
	...
	if(strlen($msg_erro) == 0){
		...
		$msg_erro = pg_errormessage($con);
		if(strlen($msg_erro) == 0){
			if ($login_fabrica == 45 || $login_fabrica == 80) { // HD 54576
				...
				if (pg_num_rows($resx) > 0) {
					...
					if (strlen($contato_estado) > 0) {
						...
						if (pg_num_rows($res_at) > 0) $destinatario = sprintf('"%s" <%s>', pg_fetch_result($res_at, 0, 0), $at_email);
...
if($mostra_valor_faturada =='sim' and !empty($os)) { // HD 181964
	echo "<script>window.open('http://www.telecontrol.com.br/assist/produto_valor_faturada.php?os=$os','','height=300, width=650, top=20, left=20, scrollbars=yes')</script>";
}
...
//				echo "a $nota_fiscal_distrib";
		...
		// HD 8412
		if (($login_fabrica == 35 or $login_fabrica == 3 or $login_fabrica == 14) and strlen($obs_os_item) >0) {
...
<?	if(strlen(trim($_GET['lu_fabrica'])) == 0) {
	...
	if($login_fabrica != 96){
		if ($consumidor_revenda[0] == 'R' || $troca_revenda == 't'){
			$sua_os_rev = substr($sua_os, 0, strpos($sua_os, '-'));

			$sql = "SELECT os_revenda FROM tbl_os_revenda WHERE fabrica = $login_fabrica AND sua_os = '$sua_os_rev'";
			$res = @pg_query($con,$sql);
			if(@pg_num_rows($res) > 0){
				$os_rev = pg_fetch_result($res,0,os_revenda);
				if ($temImg = temNF($os, 'bool','r')) {
					$img_nf = temNF($os, 'link', 'r');
				}
			}else{
				if ($temImg = temNF($os, 'bool')) {
					$img_nf = temNF($os, 'link');
				}
			}
		}else{
			if ($temImg = temNF($os, 'bool')) {
...
if($login_fabrica != 96){
	...
	<div>
		<h1>Imagem anexa</h1>
		<p>Clique na foto para visualizar melhor</p>
		<?=$img_nf?>
	</div>
	<?}
	if (temNF($os, 'bool', 2)) {
	$temImg = true;	// Não pode fazer igual o 1º porque poderia sobreescrever o 'true' ?>
	<?=$include_imgZoom?>
	<br>
	<div>
		<h1>Imagem anexa</h1>
	...
	if ($temImg) echo $include_imgZoom;
}else{
		$array_file = glob(dirNF($os)."/$os*");
		if(is_array($array_file)){
			echo "<div style='margin: 20px 0;'>";
				...
				foreach ($array_file as $file_name) {
					...
					$tipo_imagem = array('gif','png','jpg','jpeg');
					$file = array_reverse(explode('/',$file_name));
					$arquivo = dirNF($os,'url').'/'.$file[0];

					if (in_array($extensao,$tipo_imagem))
						$array_img[] = $arquivo;
					else
						$array_arquivo[] = $arquivo;
				}

				if(is_array($array_img)){
					foreach ($array_img as $arquivo) {
						echo "<a href='$arquivo' target='_blank'>"."<img src='$arquivo' title='Para ver a imagem completa, clique com o botão direito e selecione \"Abrir link em uma nova janela\" ou \"Mostrar Imagem\"' "."style='_display:inline;_zoom:1;max-height:150px;max-width:150px;_height:150px;*height:150px;'></a>&nbsp;";
					}
					echo $include_imgZoom;
				}

				if(is_array($array_arquivo)){
					echo "<br><br><table width='300px' border='0' cellspacing='1' cellpadding='0' align='center' class='Tabela'>";
						echo "<tr>";
							echo "<td class='inicio' style='text-align: center'>Arquivo</td>";
							echo "<td class='inicio' style='text-align: center'>Ação</td>";
						echo "</tr>";

						foreach ($array_arquivo as $arquivo) {
							$file = array_reverse(explode('/',$arquivo));
							echo "<tr>";
								echo "<td class='titulo2'>{$file[0]}</td>";
								echo "<td class='titulo2'>
										<input type='button' value='Download' onclick='window.open(\"$arquivo\")' />
									  </td>";
							echo "</tr>";
						}
