...
*/
	...
	***************************************************************************/
		...
		if (strlen ($msg_erro) == 0) {
			$res = pg_exec ($con,"COMMIT TRANSACTION");
			$msg = "$comp_tipo gravado com sucesso!";
			//header ("Location: $PHP_SELF?tipo=$tipo&ok=1");
...
#	if (strlen($busca_por)==0) $msg_erro  .= "Prencha o campo para a pesquisa";
	...
	if ($campo_pesquisa=='nome')
		$sql_adicional .= "AND tbl_pessoa.nome like '%$busca_por%'";

	if ($campo_pesquisa=='cpf')
		$sql_adicional .= "AND tbl_pessoa.cnpj like '%$busca_por%'";

	if ($campo_pesquisa=='endereco')
		$sql_adicional .= "AND tbl_pessoa.endereco like '%$busca_por%'";
	...
	if ($campo_pesquisa=='email')
		$sql_adicional .= "AND tbl_pessoa.email like '%$busca_por%'";
...
if ($btn_acao=='pesquisar'){
	...
	$cond2="";
	if(strlen($nome) > 0) $cond2 .= " AND tbl_pessoa.nome ILIKE '%$nome%' ";
	if(strlen($cnpj) > 0) $cond2 .= " AND tbl_pessoa.cnpj ILIKE '%$cnpj%' ";
	if(strlen($email)> 0) $cond2 .= " AND tbl_pessoa.email ILIKE '%$email%' ";
...
### COMPROVANTE
	...
	if (pg_numrows($res)>0){
		for ($i=0;$i<pg_numrows($res);$i++){
			...
			$ref_aux = $comp_tipo."|".$comp_descricao."|".$comp_valor."|".$comp_xrg."|".$comp_xcpf."|".$comp_xres."|".$comp_data;
			array_push($array_comprovante,explode("|",$ref_aux));
...
function removeroReferencia(iidd){
	var tbl = document.getElementById('tbl_referencias');
...
//***************Comprovante******************
	function adicionaLinhaComprovante() {
		...
		if(document.getElementById('comp_tipo').value=="")     { alert('Informe o Tipo');   return false}
		if(document.getElementById('comp_desc').value=="")      { alert('Informe Descrição'); return false}
		if(document.getElementById('comp_valor').value=="")     { alert('Informe Valor'); return false}
		...
		var celula = criaCelula(document.getElementById('comp_tipo').value);
		celula.style.cssText = 'text-align: left; color: #000000;font-size:10px';
		...
		el.setAttribute('value',document.getElementById('comp_tipo').value);
		celula.appendChild(el);
		...
		el.setAttribute('value',document.getElementById('comp_desc').value);
		celula.appendChild(el);
		...
		el.setAttribute('value',document.getElementById('comp_valor').value);
		celula.appendChild(el);
		...
		el.setAttribute('value',document.getElementById('comp_xrg').value);
		celula.appendChild(el);
		...
		el.setAttribute('value',document.getElementById('comp_xcpf').value);
		celula.appendChild(el);
		...
		el.setAttribute('value',document.getElementById('comp_xres').value);
		celula.appendChild(el);
		...
		// coluna 3 - Descrição
		var celula = criaCelula(document.getElementById('comp_desc').value);
		celula.style.cssText = 'text-align: center; color: #000000;font-size:10px';
		linha.appendChild(celula);

		// coluna 4 - Valor
		var celula = criaCelula(document.getElementById('comp_valor').value);
		celula.style.cssText = 'text-align: center; color: #000000;font-size:10px';
		linha.appendChild(celula);

		// coluna 5 - Xerox Rg
		var celula = criaCelula(document.getElementById('comp_xrg').value);
		celula.style.cssText = 'text-align: center; color: #000000;font-size:10px';
		linha.appendChild(celula);

		// coluna 6 - Xerox CPF
		var celula = criaCelula(document.getElementById('comp_xcpf').value);
		celula.style.cssText = 'text-align: center; color: #000000;font-size:10px';
		linha.appendChild(celula);

		// coluna 7 - Xerox Residencia
		var celula = criaCelula(document.getElementById('comp_xres').value);
		celula.style.cssText = 'text-align: center; color: #000000;font-size:10px';
		linha.appendChild(celula);
		...
		document.getElementById('comp_valor').value='';
		document.getElementById('comp_tipo').focus();
						...
						<table align='left' width='100%' border='0' class='tabela'>
								...
								<tr>
									<td colspan='6' align='center'>
										...
										<input name='pesquisar' type='button' class='botao' onclick="this.form.btn_acao.value='pesquisar_simples';this.form.submit();" value='Pesquisar'>
									</td>
								</tr>
						</table>
						</form>
						...
						<!-- <a href='<? echo $PHP_SELF ?>?tipo=<?=$tipo?>&btn_acao=cadastrar'><img src='imagens/edit2.png' align='absmiddle'> Cadastar um novo <?=$tipo?></a> -->
									...
									-->
											...
											<tr>
												<td class='Label'><?=$CPF_CNPJ?></td>
												<td align='left' >
													<input class="Caixa" type="text" name="cnpj"   size="15" maxlength="14" value="<? echo $cnpj ?>" >
												...
												<td colspan='5'><br>
													<div id="container-Dentro">
														<ul>
															...
															<li><a href="#tab2Entrega"><span><img src='imagens/mail-gold.png' align=absmiddle> Endereço de Entrega</span></a></li>
															<li><a href="#tab3Cobranca"><span><img src='imagens/mail-gold.png' align=absmiddle> Endereço de Cobrança</span></a></li>
...
**************************************************************************************************-->
														<div id="tab5Comprovante">
															...
															<table  width='100%'>
																	<tr>
																	<td class='Label'  align='left'>Tipo
																		<input class="Caixa" type="text" name="comp_tipo" id="comp_tipo"   size="12" maxlength="30" value="">
																		</td>
																		<td class='Label'  align='left'>Descrição <input class="Caixa" type="text" name="comp_desc"   size="30" maxlength="100" value=""></td>
																		<td class='Label'  align='left'>Valor <input class="Caixa" type="text" name="comp_valor" id="comp_valor"   size="10" maxlength="15" value=""></td>
																	   <td class='Label'>RG
																		<input type="checkbox" name="comp_xrg" value='t' <?if($comp_xrg=="t")echo "checked";?>>
																		</td>
																		<td class='Label'>CPF
																		<input type="checkbox" name="comp_xcpf"  value='t' <?if($comp_xcpf=="t")echo "checked";?>> 
																		</td>
																		<td class='Label'>Res
																		<input type="checkbox" name="comp_xres" value='t' <?if($comp_xres=="t")echo "checked";?>> 
																		</td>

																		<!-- botao -->
																		<td class='Label'  align='left'><input type='button' name='btn_acao' value='Adicionar Comprovante' onClick='javascript:adicionaLinhaComprovante()' ></td>
																	</tr>
															</table>
															<br>
