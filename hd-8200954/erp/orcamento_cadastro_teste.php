...
if (isset($_POST['nf_posto']) AND isset($_POST['nf_posto'])){
	$nf_posto = trim($_POST['nf_posto']);
	if(strlen($nf_posto ) > 0) {
		...
		$res = pg_exec ($con,$sql);
		if (pg_numrows ($res) == 0) {
			echo "Este orçamento não é de garantia";
		}else{
			 $orcamento_garantia   = pg_result ($res,0,0);
			 echo "Garantia do orçamento  $orcamento_garantia";
		}
	}
	exit;
}
...
if (strlen ($orcamento) > 0) {
	...
	if (pg_numrows ($res) == 1) {
		...
		$sql = "SELECT  tbl_orcamento_os.tecnico           ,
				...
				CASE WHEN CURRENT_TIMESTAMP - tbl_orcamento_os.data_visita > '0 minutes' THEN 'sim' ELSE 'nao' END AS efetivar_visita,
			...
			$prateleira_box     = trim (pg_result ($res,0,prateleira_box))    ;
			$orcamento_garantia = trim (pg_result ($res,0,orcamento_garantia))    ;
		}else{
			$sql22 = "SELECT  *
...
function adiconarPecaTbl() {
	...
	if(document.getElementById('add_qtde').value > document.getElementById('peca_estoque').value) {
		alert('Estoque indisponível');
	}
	if (document.getElementById('add_preco').value==''){
		document.getElementById('add_preco').value=0;
	}
...
//	}
	var tbl = document.getElementById('tbl_pecas');
	var oRow = iidd.parentNode.parentNode;
...
function recalcular(){
	...
	valores_total_geral = valores_sub_total+valores_frete+valores_acrescimos-valores_descontos-total;
		...
		<tr >
			...
			<td nowrap class='Label'>
				...
				if(this.value!=''){document.location='<? echo $PHP_SELF ?>?orcamento='+this.value}
					...
					<option value=''></option>
					<?php
					$sql = "SELECT	LPAD(orcamento::text,4,'0') as orcamento,
			...
			<td>
				...
				<td class='LabelTitulo' nowrap colspan='2' align='right'>
					...
					<?
						...
						if($tipo_orcamento=='TROCA'){
							echo "<INPUT TYPE='hidden' NAME='tipo_orcamento' value='troca'>";
...
?>
	...
	<tr>
		<td valign="top" align="left">
			...
			<table style=' border:#485989 1px solid; background-color: #e6eef7 ' align='center' width='750' border='0'>
			<tr>
				<td rowspan='8' width='20px' valign='top'><img src='imagens/titulo_produto.gif'></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input class="Caixa" type="text" name="produto_acessorios" size="10" value="<? echo $produto_acessorios ?>" ></td>
			</tr>
			<tr>
				<td nowrap class='Label'>Box</td>
				<td><input class="Caixa" type="text" name="prateleira_box" size="15" maxlength="20" value="<? echo $prateleira_box ?>" ></td>
				<td nowrap class='Label'>Observação</td>
				<td colspan='3'><input class="Caixa" type="text" name="obs" size="30" value="<? echo $obs ?>" ></td>
			</tr>
			<input class="Caixa" type="hidden" name="produto_voltagem" size="15" maxlength="20" value="">
			<!-- Informações da OS - FIM  -->
			<?if(strlen($orcamento_garantia)==0){?>
			<tr>
				<td nowrap class='Label'>Nota Fiscal do Posto</td>
				<td><input class="Caixa" type="text" name="nf_posto" id='nf_posto' size="15" maxlength="20" value="<? echo $nota_posto ?>" >&nbsp;<a href='#' title='Quando o produto retornar a assistência técnica, digite a nota fiscal que foi dada ao cliente para validar se a garantia é válida' class='ajuda'>?</a></td>
			</tr>
			<?}else{?>
			<tr>
				<td nowrap class='Label'>Garantia do Orçamento: </td>
				<td><a href='orcamento_cadastro.php?orcamento=<?=$orcamento_garantia?>' target='_blank'><?=$orcamento_garantia?></a>&nbsp;<a href='#' title='Quando o produto retornar a assistência técnica, digite a nota fiscal que foi dada ao cliente para validar se a garantia é válida' class='ajuda'>?</a></td>
			</tr>
			<?}?>
			<tr>
...
echo "<tr height='20'>";
	...
	echo "<td align='center' class='Label'>Tabela de preços ";
	$sqltab="SELECT *
				FROM tbl_tabela
				WHERE fabrica=$login_empresa
				AND tabela_principal IS TRUE";
	$restab=pg_exec($con,$sqltab);
	if(pg_numrows($restab) > 0) {
		$tabela  = trim(pg_result($restab,0,tabela));
	}
	$sql="SELECT *
			FROM tbl_tabela
			WHERE fabrica=$login_empresa
	...
	if (pg_numrows($res) > 0) {
		...
		for ($x = 0 ; $x < pg_numrows($res) ; $x++){
			...
			echo "<option value='$aux_tabela'";
			if ($tabela==$aux_tabela){
				echo " SELECTED ";
			}
...
if($qtde_item>0){
	for ($k=0;$k<$qtde_item;$k++){
			...
			echo "<input type='hidden' name='descricao_peca_$k' id='peca_descricao_$k'            value='$item_descricao[$k]'>";
			echo "<input type='hidden' name='peca_preco_$k'     id='peca_preco_$k'            value='$item_preco[$k]'>";
...
?>
<tr>
	<td><img height="6" width="16" src="../imagens/spacer.gif"></td>
</tr>
<tr>
	<td valign="top" align="left">
...
?>
		...
		<?
		//--===== SITUAÇÂO DA OS -  ========================================================================
		echo "<table style=' border:#485989 1px solid; background-color: #e6eef7' align='center' width='100%' height='100%' border='0'>";
		echo "<tr bgcolor='#e6eef7' >";
		...
		if ($tipo_orcamento=='FORA_GARANTIA'){
			echo "<tr  valign='top'>";
			echo "<td width='45%' class='Label' valign='top' nowrap ><label name='previsao_conserto'>Previsão Término do Conserto</label>&nbsp;<a href='#' title='Quando o fornecedor de peças der uma previsão da chegada da peça, calcular o tempo de conserto e previsão de quando o produto vai estar consertado. Caso contrário, deixar em branco.' class='ajuda'>?</a></td>";
...
if ($tipo_orcamento<>'VENDA'){
	...
	echo "<table style=' border:#485989 1px solid; background-color: #e6eef7' align='left' width='250' border='0'>";
	echo "<tr bgcolor='#e6eef7'>";
	...
	if (strlen($aprovado)==0 ){
		...
		if(strlen($apro_data_aprovacao)>0){
			...
			//echo "$login_empregado $login_empregado_nome";
			echo "<tr>";
...
//Adicionado Por Paulo-HD:3401
$cotacao            = trim($_GET["cotacao"]);
$total       = "0.00";
if (strlen($orcamento) > 0) {
	...
	if (pg_numrows($res) > 0) {
		...
		$total          = number_format(trim(pg_result($res,0,total)),2,".","");
		$subtotal       = number_format(trim(pg_result($res,0,subtotal)),2,".","");
		}
}
//FIM - HD:3401

$valores_total_geral -= $total;
...
echo "<table style=' border:#485989 1px solid; background-color: #e6eef7' align='left'  border='0' id='tabela_totais'>";
echo "<tr height='10px'>";
...
echo "</tr>";
	...
	echo "</tr>";
	echo "<tr>";
	echo "<td class='Label'><a href=\"
		javascript:
		if (document.frm_os.btn_acao.value!='Gravar'){
			alert('Aguarde');
		}else {
			document.frm_os.btn_acao.value='Gravando...';
			gravar_os(document.frm_os,'sim','orcamento_troca.php?cliente=$cliente&pedido=$pedido','nao');
		}\"
		>Troca de Produto</a></td>";

	echo "<td class='Label' align='right'><span id='total'>$total</span></td>";
	echo "</tr>";
	echo "<tr>";
...
echo "<table style=' border:#B63434 1px solid; background-color: #EED5D2' align='center' width='750' border='0'height='100%'>";
echo "<tr>";
...
if ($tipo_orcamento=='VENDA'){
	...
	<input type='button' name='btn_acao' id='btn_acao' class='frm' value='Gravar' onClick=\"
	if (this.value!='Gravar'){
		alert('Aguarde');
	}else {
		this.value='Gravando...';
		gravar_os(this.form,'sim','orcamento_cadastro.php?redic=1','nao');
