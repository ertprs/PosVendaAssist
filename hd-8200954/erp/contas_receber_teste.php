	...
	if (http.readyState == 4) {
		if (http.status == 200) {
			...
			if (typeof (results[0]) != 'undefined') {
				if (results[0] == 'ok') {
					com.innerHTML   = results[1] ; //retorna a "LISTA DE CONTAS A receber"
...
function Exibir2(componente,solicita, documento, acao) {
	...
	var conta_pg	= document.getElementById('conta_receber').value;
	...
	if(acao=='insert'){
		...
		if(acao=='baixar'){
			...
			var valor_cobrado = document.getElementById('valor_cobrado').innerHTML;
...
//
//			if (parseFloat(valor) < parseFloat(valor_cobrado)){
//				if (confirm('O valor calculado para receber hoje é de R$ '+valor_cobrado+'\nO valor que você informou para receber é de R$ '+valor_pago+'\n\nO valor é inferior ao que o sistema calculou.\n\nDeseja que seja criado uma nova receita com o valor restante?')){
//					url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&acao="+escape(acao)+"&conta_receber="+conta_pg+"&valor_pago="+escape(valor_pago)+"&obs="+(obs)+"&pagamento="+(pagamento)+"&quitar=nao&dividir=sim";
//				}else{
...
//						url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&acao="+escape(acao)+"&conta_receber="+conta_pg+"&valor_pago="+escape(valor_pago)+"&obs="+(obs)+"&pagamento="+(pagamento)+"&quitar=sim";						
//						
//					}
//					else{
...
//			}
		...
		url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&acao="+escape(acao)+"&conta_receber="+conta_pg+"&valor_pago="+escape(valor_pago)+"&obs="+(obs)+"&pagamento="+(pagamento)+"&quitar=sim";
			...
			if(acao=='alterar'){
				desbloqueia_campos('frm_receber');
				msg.style.display='inline';
				url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&acao="+escape(acao)+"&documento="+documento+"&fornID="+fornID+"&valor="+valor+"&vencimento="+vencimento+"&pagamento="+pagamento+"&valor_pago="+escape(valor_pago)+"&conta_receber="+conta_pg+"&obs="+obs+"&multa_p="+multa_p+"&multa_valor="+multa_valor+"&juros_mora_p="+juros_mora_p+"&juros_mora_valor="+juros_mora_valor+"&desconto="+desconto+"&desconto_p="+desconto_p+"&protestar="+protestar+"&desconto_pontualidade="+desconto_pontualidade+"&valor_custas_cartorio="+valor_custas_cartorio;


			}else{
				msg.style.display = 'none';
				url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&programa2="+escape(solicita) ;
			}
		}
	}
...
//FUNÇAO USADA PARA CARREGAR UMA CONTA_receber DA LISTA DE PENDENTES
function retornaReceber(http,componente) {
...
//			alert(http.responseText);
			...
			if (typeof (results[0]) != 'undefined') {
				if (results[0] == 'ok') {
					...
					document.getElementById('doc_final').innerHTML	= "<b>"+results[1]+"-"+results[2]+"</b>";
					document.getElementById('fornID').value			= results[3];
					document.getElementById('fornName').innerHTML	= results[4];
					...
					document.getElementById('conta_receber').value	= results[8];

//					document.getElementById('multa_valor').value	= results[9];
					document.getElementById('juros_mora_valor').value = results[9];
//					document.getElementById('desconto').value		= results[11];
//					document.getElementById('descontos_abatimentos').innerHTML = results[11];

//					if (results[12]=='t')	document.getElementById('desconto_pontualidade').checked=true;
//					else					document.getElementById('desconto_pontualidade').checked=false;
...
//					document.getElementById('valor_cobrado').innerHTML		= results[15];
					...
					document.getElementById('data_digitacao').innerHTML		= ' '+results[11];
					document.getElementById('data_digitacao2').innerHTML	= ' '+results[11];
					document.getElementById('cendente_nome').innerHTML		= ' '+results[12];
					document.getElementById('valor_cobrado').innerHTML= ' '+results[13];
					document.getElementById('mora_multa').innerHTML			= results[14];
...
function exibirreceber(componente,conta_receber, documento, acao) {
	url = "contas_receber_retorno_cadastro_ajax.php?ajax=mostra&acao="+escape(acao)+"&conta_receber="+escape(conta_receber);
	http.open("GET", url , true);
	http.onreadystatechange = function () { retornaReceber (http,componente) ; } ;
	http.send(null);
}

//FUNÇAO USADA PARA CARREGAR UMA CONTA_receber DA LISTA DE PENDENTES
function retornaCliente(http,componente) {
...
function exibirCliente(componente,conta_receber, documento, acao) {
var solicita= document.getElementById('documento').value;
	if(acao=='insert'){
		...
		var obs			= document.getElementById('obs').value;
		var fatID		= document.getElementById('faturamentoID').value;
		url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&acao="+escape(acao)+"&documento="+escape(documento)+"&valor="+ escape(valor)+"&fornID="+escape(fornID)+"&vencimento="+vencimento+"&obs="+obs+"&fatID="+fatID;
	}else{
		msg.style.display = 'none';
		url = "contas_receber_retorno_cadastro_ajax.php?ajax=sim&programa2="+escape(solicita) ;
	}
	url = "contas_receber_retorno_cad_forn_ajax?ajax=mostra&acao="+escape(acao)+"&conta_receber="+escape(conta_receber);
	http.open("GET", url , true);
	http.onreadystatechange = function () { retornaCliente (http,componente,solicita) ; } ;
	http.send(null);
}
		...
		if (http.status == 200) {
			...
			if (typeof (results[0]) != 'undefined') {
				if (results[0] == 'ok') {
					...
					document.getElementById('msg_fat').innerHTML= "<font color='#ff0000'>existem faturamentos para esse Cliente</font>";
					document.getElementById('msg_fat').style.display='inline';
				}else{
					com5.innerHTML   = "&nbsp;&nbsp;<font color='#0000ff'>Sem faturamentos para esse Cliente</font>";
				}
			}else{
				alert ('Fechamento nao processado');
			}
...
function exibirFat(componente,conta_receber, documento, acao) {
	var solicita= document.getElementById('documento').value;
	var fornID= document.getElementById('fornID').value;
	url = "contas_receber_retorno_cadastro_ajax?ajax=mostra&acao=faturamento&fornID="+escape(fornID);
	http.open("GET", url , true);
	...
	if(document.getElementById('fornID').value==''){
		if(se_existe('forn_nao_cad')){
			if(document.getElementById('forn_nao_cad').value==1)
				document.getElementById('msg_fat').style.display = 'inline';
			document.getElementById('msg_fat').innerHTML= "Para cadastrar esse Cliente <a href='#' onclick = 'exibirForn'>clique aqui!</a>"
		}else{
...
function limpar(){
	...
	document.getElementById('conta_receber').value= "";
...
//FUNÇAO USADA PARA ATUALISAR, INSERIR E ALTERAR		
function gravar_cliente(formulatio) {
//	ref = trim(ref);
	var acao='cadastrar';

	url = "contas_receber_retorno_cad_forn_ajax.php?ajax=sim&acao="+acao;
...
<body>
<? //echo "login_posto".$login_posto; echo " login_empresa".$login_empresa; echo " login_loja".$login_loja; echo " login_fabrica".$login_fabrica; ?>
<br><br>
...
<center>
<INPUT TYPE='button' name='bt_cad_forn' id='bt_cad_forn' value='Gravar' onClick="if (this.value=='Gravando...'){ alert('Aguarde');}else {this.value='Gravando...'; gravar_cliente(this.form);}">
...
<span class='campoL' id="cendente_nome" >
			</span>
  </td>
  <td width="168">
	...
	<span class='campo'>
				...
				<script type="text/javascript">
					new CAPXOUS.AutoComplete("nf", function() {
						...
						return "contas_receber_retorna_nf_ajax.php?fornID=" + document.getElementById('fornID').value +"&typing=" + this.text.value;
					});
				</script>	
	</span>
  </td>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
   <tr>
    ...
    <td width="28%" colspan="2">
		...
		<span class='campoL' id='fornName'></span>
   ...
   <tr>
	...
	<?
				...
				echo "&nbsp;&nbsp;";
				echo "<INPUT TYPE='button' $baixar_display name='bt_cad'    id='bt_cad'    value='Lançar novo pagamento' onClick=\"if (teste_cadastrar(this))Exibir2('dados','','','insert')\">";
				echo "&nbsp;&nbsp;";
				echo "<INPUT TYPE='button' $bt_cad_display name='bt_limpar' id='bt_limpar' value='Limpar'    onClick=\"limparForm('frm_receber');limpar();\">";
				echo "&nbsp;";
				echo "<INPUT TYPE='button' $bt_cad_display name='bt_cancelar' id='bt_cancelar' value='Cancelar'    onClick=\"limparForm('frm_receber');limpar();\">";
...
if (strlen($receber_id)>0)
	echo "<script language='javascript'>setTimeout('exibirreceber(\"dados\",\"$receber_id\",\"\",\"mostra\")',1500);</script>";
