...
if ($btn_acao == "Gravar") {
	...
	if(strlen($referencia)              > 0){
		...
		$sql = "select max(codigo_produto)+1  from tbl_fabrica where fabrica=$login_empresa;";
		$res = pg_exec($con,$sql);
		$xreferencia = pg_result($res,0,0);
	...
	if (strlen ($msg_erro) == 0){
		...
		if (strlen($id_peca)==0){
			...
			if (strlen($referencia) ==0 ){
				$sql = "update tbl_fabrica set codigo_produto = $xreferencia where fabrica = $login_empresa";
				$res = pg_exec ($con,$sql);
				$msg_erro .= pg_errormessage($con);
			}
