...
if(strlen($btn_acao) > 0) {
	...
	if(strlen($tipo) == 0) {
		$msg_erro.="Selecione o tipo de carta<BR>";
	}
	if(strlen($descricao) == 0) {
		$msg_erro="Por favor, colocar a descrição da carta<BR>";
	}
	if(strlen($conteudo) == 0) {
		$msg_erro="Por favor, colocar o conteúdo da carta<BR>";
	}
	if(strlen($email) == 0) {
		$msg_erro="Por favor, preenche campo e-mail a ser cadastrado";
	}
	if(strlen($msg_erro) == 0) {
		if(strlen($loja_carta)==0){
			$sql="INSERT INTO tbl_loja_carta (
					tipo           ,
					email          ,
					descricao      ,
					conteudo       ,
...
if ($excluir == "excluir") {
	if (strlen ($msg_erro) == 0){
		$sql = "DELETE FROM tbl_loja_carta
				WHERE loja_carta = $loja_carta
