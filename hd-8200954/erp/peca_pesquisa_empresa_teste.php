...
if ($tipo == "descricao") {
	...
	if(strlen($tabela) > 0) {
		if($login_empresa == 10){
			$sql.=" LEFT JOIN tbl_tabela_item ON tbl_peca.peca =tbl_tabela_item.peca
	...
	if($login_empresa == 10){
		$sql.=" WHERE   (tbl_peca.fabrica in (3, $login_empresa) OR tbl_peca.fabrica in ( 3,$empresa) )
...
";
	...
	if($login_empresa == 10){
		$sql.=" WHERE   (tbl_peca.fabrica in (3, $login_empresa) OR tbl_peca.fabrica in ( 3,$empresa) )
			AND     upper(tbl_peca.referencia) like '%$referencia%' ";
	}else{
		$sql.=" WHERE   (tbl_peca.fabrica = $login_empresa OR tbl_peca.fabrica = $empresa)
			AND     upper(tbl_peca.referencia) like '%$referencia%' ";
	}
...
//	echo $sql;
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		$preco_tab           = trim(pg_result($res,$i,preco));
		...
		// HD 52781
		if(strlen($peca) > 0) {
			$referencia_de = $referencia;
			$descricao_de = $descricao;
			for($xx=0;$xx<$contax;$xx++){
				$sql_para="SELECT peca_para,
								  para,
								  (select descricao from tbl_peca where tbl_peca.peca = tbl_depara.peca_para) as descricao,
								  tbl_peca_item.valor_venda,
								  tbl_tabela_item.preco,
								  tbl_estoque.qtde as estoque,
								  tbl_estoque_extra.quantidade_entregar,
							...
							LEFT JOIN    tbl_peca_item ON tbl_peca_item.peca=tbl_depara.peca_para
							LEFT JOIN tbl_tabela_item on tbl_tabela_item.peca = tbl_peca.peca
							LEFT JOIN tbl_estoque ON tbl_estoque.peca=tbl_depara.peca_para
							LEFT JOIN tbl_estoque_extra ON tbl_estoque_extra.peca = tbl_depara.peca_para ";

							if($login_empresa == 10){
								$sql_para.=" WHERE   (tbl_peca.fabrica in (3, $login_empresa) OR tbl_peca.fabrica in ( 3,$empresa) ) ";
							}else{
								$sql_para.=" WHERE   (tbl_peca.fabrica = $login_empresa OR tbl_peca.fabrica = $empresa)";
							}
							$sql_para .= " AND tbl_depara.peca_de = $peca ";

				$res_para=pg_exec($con,$sql_para);

				if(pg_numrows($res_para) >0){
					$peca         = trim(@pg_result($res_para,0,peca_para));
					$referencia   = trim(@pg_result($res_para,0,para));
					$descricao    = trim(@pg_result($res_para,0,descricao));
					$preco        = trim(pg_result($res_para,0,valor_venda));
					$preco_tab    = trim(pg_result($res_para,0,preco));
					if($preco == 0 or strlen($preco)==0) {
						$preco = $preco_tab;
					}
					$estoque             = trim(pg_result($res_para,0,estoque));
					$quantidade_entregar = trim(pg_result($res_para,0,quantidade_entregar));
					$data_atualizacao    = trim(pg_result($res_para,0,data_atualizacao));
					$ativo               = trim(pg_result($res_para,0,ativo));
		...
		if($para > 0) {
			echo "<tr >\n";
			echo "<td nowrap colspan='100%'>\n";
			echo "Peça mudou De: $referencia_de Para $referencia";
			echo "</td>\n";
			echo "</tr>\n";
		}
