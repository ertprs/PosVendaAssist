...
<?
						...
						if($botao== "Adicionar") {
							...
							if(strlen($requisicao) ==0){
								...
								$sql= " INSERT INTO tbl_requisicao (data, hora, empregado, status) 
										VALUES(current_date, current_time, $empregado, 'aberto');";
								$res= pg_exec($con, $sql);
							...
							if(strlen($descricao) > 0 and strlen($quantidade) > 0 and strlen($erro)==0) {
								...
								if(pg_errormessage($con) or strlen($erro)>0){
									...
									echo "<script language='JavaScript'>
										window.location= 'requisicao.php#tab0Incluir&erro=$erro';
									</script>";
									exit;
								}else{
								...
								if(strlen($item)>0){
									$sql = " SELECT requisicao_item
											 ...
											 WHERE requisicao =$requisicao 
												 AND tbl_requisicao.empregado = $empregado
...
?>
						...
						<?	//if(($status_requisicao == "aberto") or (strlen($nova)>0)) {?>
								...
								<tr bgcolor='#fcfcfc'>
									...
									<td nowrap align='left'><br>
									Quantidade<br>
									  <input class='Caixa' type='text' name='quantidade' value='<?echo $quantidade;?>' size='10' maxlength=10>
									</td>
...
<?
						...
						echo "</tr>";
						echo "<tr>";
						echo "<td align='left'><BR><font size=2>Número Requisição:</font> &nbsp; <input class='Caixa' type='text' size='5' maxlength='5' name='numero_requisicao'> <BR><BR>";
						echo "</td>";
						
						$sqlstatus = "SELECT  distinct status
										FROM  tbl_requisicao
										ORDER BY status";
						...
						if (pg_numrows($resstatus) > 0) {
							...
							echo "<option value='aberto'  if ($sstatus == 'aberto')"; echo " ' selected ';>TODAS EM ABERTO</option>";
							echo "<option value='finalizado' if ($sstatus == 'finalizado') echo 'selected ';>FINALIZADA</option>";
						...
						if($btn_requisicao=="PESQUISAR") {
							...
							if(strlen($sstatus) > 0) {
								$sql.="AND tbl_requisicao.status='$sstatus' ";
							} else {
								if(strlen($numero_requisicao) > 0) {
							...
							if(strlen($numero_requisicao) > 0 and strlen($sstatus) > 0) {
								$sql.="AND tbl_requisicao.requisicao=$numero_requisicao
										AND tbl_requisicao.status='$sstatus' ";
							}
