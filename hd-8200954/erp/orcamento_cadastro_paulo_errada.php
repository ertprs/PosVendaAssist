...
//	linha.appendChild(celula);
	...
	//10 brinde HD 6120 Paulo 
	var celula = criaCelula(document.getElementById('brinde').value);
	var el = document.createElement('input');
	el.setAttribute('type', 'checkbox');
	el.setAttribute('align', 'center');
	el.setAttribute('name', 'peca_brinde_' + iteration);
	el.setAttribute('id', 'peca_brinde_' + iteration);
	el.setAttribute('value','brinde');
	el.onclick=function(){recalc()};
	celula.appendChild(el);
	linha.appendChild(celula);
...
//	document.getElementById('add_servico_realizado').selectedIndex=0;
	...
	if(document.getElementById('brinde').checked==true) {
		aux_valor = parseFloat(aux_valor) - parseFloat(total_valor_peca);
	}
...
function recalc(){
	var tbl = document.getElementById('tbl_pecas');
	var qtde_linhas = tbl.rows.length;
	var total_brinde = 0;
	var total = 0;
	for (var i=0; i< qtde_linhas; i++){
		if ($('#peca_preco_'+i).val()){
			if ($('#peca_qtde_'+i).val()){
				tmp = $('#peca_qtde_'+i).val() * $('#peca_preco_'+i).val() ;
				total += tmp;
				if ($('#peca_brinde_'+i).checked){
					total_brinde += tmp;
				}
			}
		}
	}
	document.getElementById('valores_brindes').innerHTML = total_brinde;
	recalcular();
}
...
echo "<tr height='20'>";
	...
	echo "<input type='hidden' name='peca_brinde' id='peca_brinde_'>\n";
...
echo "<td align='center' class='Label'><b>Brinde</b></td>";
...
if($qtde_item>0){
	for ($k=0;$k<$qtde_item;$k++){
			...
			echo "<td><input type='checkbox' name='brinde' onClick='javascript: recalc()'></td>";
