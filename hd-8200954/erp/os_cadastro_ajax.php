...
if (isset($_COOKIE['cook_fabrica'])){
	$cookie_fabrica = $_COOKIE['cook_fabrica'];
}

#if (strlen($fabrica)>0 AND $cookie_fabrica != $fabrica){
if (strlen($cookie_fabrica)==0){
	...
	$login_posto_fabrica = $_COOKIE['cook_posto_fabrica'];
}
...
if (strlen($os)>0 AND ($login_fabrica==3 OR $login_fabrica==11)){
	$sql = "SELECT  status_os, observacao, data
			FROM    tbl_os_status
			WHERE   os = $os
			...
			if ($os_bloqueada_garantia=='t'){ //no caso se tiver peça bloqueada para garantia, deve justificar.
				...
				if ($login_fabrica==11){
					...
					if (strlen($email_sap)>0){
						$email_origem  = "suporte@telecontrol.com.br";
						$email_destino = "$email_sap,fabio@telecontrol.com.br";
...
$().ready(function() {
	...
	<? ## quando for abrir um OS, nao pode alterar o defeito reclamado
		if (strlen($os)>0){
			echo "return true;";
		}
	?>
...
function bloquear(){
	...
	$('#tbl_pecas').block('<h3>Lançamento de Peças Bloqueado</h3>', { border: '2px solid #727272' }); 
...
function fnc_pesquisa_produto2 (campo, campo2, tipo, voltagem) {
	...
	if (xcampo.value != "") {
		var url = "";
		url = "produto_pesquisa_2_assist.php?campo=" + xcampo.value + "&tipo=" + tipo + "&proximo=t";
		janela = window.open(url, "janela", "toolbar=no, location=no, status=yes, scrollbars=yes, directories=no, width=600, height=400, top=18, left=0");
...
function fnc_pesquisa_lista_basica (produto_referencia, peca_referencia, peca_descricao, peca_preco, voltagem, tipo, peca_qtde) {
	var url = "";
	if (tipo == "tudo") {
			url = "peca_pesquisa_lista_assist.php?produto=" + produto_referencia + "&descricao=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}

	if (tipo == "referencia") {
			url = "peca_pesquisa_lista_assist.php?produto=" + produto_referencia + "&peca=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}

	if (tipo == "descricao") {
			url = "peca_pesquisa_lista_assist.php?produto=" + produto_referencia + "&descricao=" + peca_descricao.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}
...
<tr>
	<td valign="top" align="left">
		...
		<?
		$login_fabrica_nome = "Selecione o fabricante!";
		if (strlen($login_fabrica)>0){
			...
			$res = pg_exec ($con,$sql);
			if (pg_numrows ($res)>0) {
				$login_fabrica_nome = pg_result($res,0,nome);
			}
		}
		echo "<center><h3 style='aling:center;font-weight:bold'>Ordem de Serviço: $login_fabrica_nome</h3></center><br>";
...
<tr>
	<td align="left">
		...
		<table style=' border:#485989 1px solid; background-color: #e6eef7' align='center' width='750' border='0'>
		<tr>
			<td class='TituloBarra' align='left' colspan='4'>Produto</td>
		</tr>
		...
		<? if ($login_fabrica==15 OR $login_fabrica==3) {?>
			<tr>
...
<tr>
	<td valign="top" align="left">
		...
		<table style=' border:#485989 1px solid; background-color: #e6eef7' align='center' width='750' border='0'>
		<tr>
			<td class='TituloBarra' align='left' colspan='6'>Consumidor</td>
		</tr>
		<tr>
			<td class='Label'>Nome Consumidor:</td>
			<td><input class="frm" type="text" name="consumidor_nome" rel='os' size="40" maxlength="50" value="<? echo $consumidor_nome ?>"></td>
			<td class='Label'>Telefone:</td>
			<td><input class="frm" type="text" name="consumidor_fone" id="consumidor_fone" rel='os' size="15" maxlength="50" value="<? echo $consumidor_fone ?>"></td>
			<td class='Label'>CEP:</td>
			<td><input class="frm" type="text" name="consumidor_cep" rel='os' size="10" maxlength="8" value="<? echo $consumidor_cep ?>"></td>
		</tr>
		<tr>
...
<tr>
	<td valign="top" align="left">
		...
		<table style=' border:#485989 1px solid; background-color: #e6eef7' align='center' width='750' border='0'>
		<tr>
			<td class='TituloBarra' align='left' colspan='6'>Revenda</td>
		</tr>
		<tr>
			<td class='Label'>Nome Revenda:</td>
			<td><input class="frm" type="text" name="revenda_nome" rel='os' size="27" maxlength="50" value="<? echo $revenda_nome ?>" >
		...
		<tr>
			...
			<td><input class="frm" type="text" name="revenda_cnpj" rel='os' size="20" maxlength="18" value="<? echo $revenda_cnpj ?>">
			...
			<td class='Label'>Telefone:</td>
			<td><input class="frm" type="text" name="revenda_fone" id="revenda_fone" rel='os' size="15" maxlength="20" value="<? echo $revenda_fone ?>"></td>
			<td class='Label'>CEP:</td>
			<td><input class="frm" type="text" name="revenda_cep" rel='os' size="10" maxlength="9" value="<? echo $revenda_cep ?>"></td>
		</tr>
		<tr>
			<td class='Label'>Endereço:</td>
...
echo "<tr>";
echo "<td class='TituloBarra2' align='left' colspan='2'>Análise de Produto: <span id='dados' style='display:inline;font-weight:normal'><i><u> Não informado</i></u></span>";
echo "</td>";
echo "</tr>";
echo "<tr>";
...
echo "<tr>";
echo "<td class='TituloBarra3' align='left' colspan='2'>Peças</td>";
...
echo "<td align='center' class='TituloBarra4'><b>Código</b></td>";
echo "<td align='center' class='TituloBarra4'><b style='font-size:10px'>Lista<br>Básica</b></td>";
echo "<td align='center' class='TituloBarra4'><b>Descrição</b></td>";
echo "<td align='center' class='TituloBarra4'><b>Defeito</b></td>";
echo "<td align='center' class='TituloBarra4'><b>Serviço</b></td>";
echo "</tr>";
