...
if ($atualizar=="sim" AND isset($_POST['num_pedido']) AND isset($_POST['orcamento'])){
	...
	$num_pedido = explode(" ",$num_pedido);

	if(strlen($num_pedido[0])>0){
		$num = "'".$num_pedido[0]."'";
	}else{
		$num = "NULL";
	}

	if(strlen($num_pedido[1])>0){
		$data = "TO_DATE('$num_pedido[1]', 'DD/MM/YYYY')";
	}else{
		$data = "NULL";
	}
...
if ($atualizar=="sim" AND isset($_POST['nf_pedido']) AND isset($_POST['orcamento'])){
	...
	$nf_pedido = explode(" ",$nf_pedido);

	if(strlen($nf_pedido[0])>0){
		$num = "'".$nf_pedido[0]."'";
	}else{
		$num = "NULL";
	}

	if(strlen($nf_pedido[1])>0){
		$data = "TO_DATE('$nf_pedido[1]', 'DD/MM/YYYY')";
	}else{
		$data = "NULL";
	}
...
if ($atualizar=="sim" AND isset($_POST['nf_saida']) AND isset($_POST['orcamento'])){
	$orcamento      = trim($_POST['orcamento']);
	$nf_saida       = trim($_POST['nf_saida']);

	$nf_saida = explode(" ",$nf_saida);

	if(strlen($nf_saida[0])>0){
		$num = "'".$nf_saida[0]."'";
	}else{
		$num = "NULL";
	}

	if(strlen($nf_saida[1])>0){
		$data = "TO_DATE('$nf_saida[1]', 'DD/MM/YYYY')";
	}else{
		$data = "NULL";
	}
...
function gravar_status(linha,id,tipo){
	...
	$(function(){
		...
		$("input[@rel='num_pedido']").maskedinput("99999999 99/99/9999");
		$("input[@rel='nf_pedido']").maskedinput("999999 99/99/9999");
		$("input[@rel='nf_saida']").maskedinput("999999 99/99/9999");
...
$().ready(function() {
	...
	$("input[@rel='nf_saida']").blur(function(){
		var campo = $(this);
...
<table width='98%'>
	<tr>
		<td width='10'><img src='imagens/status_vermelho.gif' align='absmiddle'></td>
		<td><a href='<?=$PHP_SELF?>?filtro=AGUARDANDO_ANALISE'>OR Aguardando Análise</a></td>
	</tr>
	<tr>
		<td width='10'><img src='imagens/status_verde.gif' align='absmiddle'></td>
		<td><a href='<?=$PHP_SELF?>?filtro=AGUARDANDO_APROVACAO'>OR Aguardando Aprovação</a></td>
	</tr>
	<tr>
		<td><img src='imagens/status_amarelo.gif' align='absmiddle'></td>
		<td><a href='<?=$PHP_SELF?>?filtro=AGUARDANDO_PECA'>OR Aguardando Peça</a></td>
	</tr>
	<tr>
		<td><img src='imagens/status_rosa.gif' align='absmiddle'></td>
		<td ><a href='<?=$PHP_SELF?>?filtro=AGUARDANDO_CONSERTO'>OR Aguardando Conserto</td>
	</tr>
	<tr>
		<td><img src='imagens/status_azul.gif' align='absmiddle'></td>
		<td><a href='<?=$PHP_SELF?>?filtro=CONSERTADO'>OR Consertada</td>
	</tr>
		<tr>
		<td><img src='imagens/status_cinza.gif' align='absmiddle'></td>
		<td><a href='<?=$PHP_SELF?>?filtro=REPROVADOS'>OR Reprovado</td>
	</tr>
	<tr>
		<td></td>
	...
	<tr>
		<td bgcolor='#91F79D' style='border: #000000 1px solid;'>&nbsp;</td>
...
<?
	...
	if (strlen($filtro)>0){
		...
		if(strtoupper($filtro)=="AGUARDANDO"){
			$filtro = " AND tbl_orcamento.data_reprovacao IS NULL
						AND tbl_orcamento.data_aprovacao IS NULL";
		}else
		if(strtoupper($filtro)=="AGUARDANDO_ANALISE"){
			$filtro="	AND tbl_orcamento.data_aprovacao IS NULL
						AND tbl_orcamento_os.defeito_constatado IS NULL ";
		}else
		if(strtoupper($filtro)=="AGUARDANDO_APROVACAO"){
			$filtro = " AND tbl_orcamento.data_aprovacao IS NULL
						AND tbl_orcamento.data_reprovacao IS NULL
						AND tbl_orcamento_os.defeito_constatado IS NOT NULL";
		}else
		if(strtoupper($filtro)=="AGUARDANDO_CONSERTO"){
			$filtro = " AND tbl_orcamento.data_aprovacao IS NOT NULL
						AND tbl_orcamento_os.defeito_constatado is NOT null
						AND tbl_orcamento_os.data_conserto IS NULL
						AND tbl_orcamento.nf_pedido IS NOT NULL";
		}else
		if(strtoupper($filtro)=="AGUARDANDO_PECA"){
			$filtro = " AND tbl_orcamento.data_aprovacao IS NOT NULL
						AND tbl_orcamento_os.data_conserto IS NULL
						AND tbl_orcamento.nf_pedido IS NULL
						AND (SELECT peca FROM tbl_orcamento_item WHERE orcamento = tbl_orcamento.orcamento LIMIT 1) > 0 ";
		}else
	...
	if($qtde_os>0){
		...
		for ($x = 0 ; $x < $qtde_os ; $x++ ) {
			...
			$sql = " SELECT COUNT(*) FROM tbl_orcamento_item WHERE orcamento = $orcamentoY ";
			$res2 = pg_exec ($con,$sql) ;
			$qtde_itens = pg_result ($res2,0,0);

			$img = "";
			if (strlen($aprovacao)==0 AND strlen($defeito_constatado)>0){
				$img = "<img src='imagens/status_verde.gif' width='8'>";
			}
			if (strlen($aprovacao)==0 AND strlen($defeito_constatado)==0){
				$img = "<img src='imagens/status_vermelho.gif' width='8'>";
			}
			if (strlen($reprovacao)>0){
				$img = "<img src='imagens/status_cinza.gif' width='8'>";
			}
			if (strlen($aprovacao)>0){
				$img = "<img src='imagens/status_rosa.gif' width='8'>";
			}
			if(strlen($reprovacao)==0 AND strlen($aprovacao)>0 AND strlen($defeito_constatado)==0) {
				$img = "<img src='imagens/status_vermelho.gif' width='8'>";
			}
			if(strlen($reprovacao)==0 AND strlen($aprovacao)>0 AND strlen($defeito_constatado)>0 ) {
				$img = "<img src='imagens/status_rosa.gif' width='8'>";
			}
			#if(strlen($reprovacao)==0 AND strlen($aprovacao)>0 AND $qtde_itens>0 AND #strlen($defeito_constatado)>0) {
			#	$img = "<img src='imagens/status_amarelo.gif' width='8'>";
			#}
			if(strlen($reprovacao)==0 AND strlen($aprovacao)>0 AND $qtde_itens>0 AND strlen($nf_pedido)==0 AND strlen($data_nf_pedido)==0) {
				$img = "<img src='imagens/status_amarelo.gif' width='8'>";
			}
			if(strlen($reprovacao)==0 AND strlen($aprovacao)>0 AND strlen($data_conserto)>0) {
				$img = "<img src='imagens/status_azul.gif' width='8'>";
			}
			...
			if(strlen($orcamento_garantia)>0 ) $cor='#91F79D';
			echo "<tr bgcolor=$cor>";
	...
	//		echo "<td align='center'>$vendedor</td>";
			...
			if(strlen($aprovacao)>0 ) echo "<input type='text' name='num_pedido_$x' alt='$orcamentoY' rel='num_pedido' value='$num_pedido $data_pedido' size='18' maxlength='19'>";
			else                     echo " - ";
			echo "</td>";

			echo "<td align='center'>";
			if(strlen($aprovacao)>0 ) echo "<input type='text' name='nf_pedido_$x' alt='$orcamentoY' rel='nf_pedido' value='$nf_pedido $data_nf_pedido' size='18' maxlength='18'>";
			else                     echo " - ";
			echo "</td>";

			echo "<td align='center'>";
			if(strlen($aprovacao)>0) echo "<input type='text' name='nf_saida_$x' alt='$orcamentoY' rel='nf_saida' value='$nf_saida $data_nf_saida' size='18' maxlength='18'>";
			else                     echo " - ";
			echo "</td>";
