...
*/
	...
	if (($fornID== 13996 || $fornID== 6153 || $fornID== 6155 )){
		$sql_adicional = " tbl_faturamento.fabrica=3 AND tbl_faturamento.posto=4311 ";
	}
	else {
		$sql_adicional = " tbl_faturamento.posto = $login_posto AND tbl_faturamento.distribuidor = '$fornID'";
	}

	$dig = trim($typing);
	$sql_digito = "";
	if(strlen($dig)>0){
		$sql_digito = " and nota_fiscal like '%$dig%'";
	}
	...
	$sql="SELECT tbl_faturamento.vencimento             ,
						...
						SUM(tbl_contas_receber.valor) AS total_pago      ,
						TO_CHAR(emissao,'DD/MM/YYYY') as emissao
				FROM tbl_faturamento
				LEFT JOIN tbl_contas_receber using(faturamento)
				WHERE 
				$sql_adicional
				$sql_digito
				...
				HAVING COALESCE(SUM(tbl_contas_receber.valor),0) < COALESCE(tbl_faturamento.total_nota,0)
				ORDER BY faturamento DESC limit 10 OFFSET $atual; ";
