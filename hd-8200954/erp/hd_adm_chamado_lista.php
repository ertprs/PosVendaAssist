...
if($valor_chamado <> ''){
	$sql .= " AND tbl_hd_chamado.hd_chamado = '$valor_chamado'";
}else{
	$sql .= " AND tbl_hd_chamado.fabrica_responsavel = $login_empresa";

	if ($atendente_busca <> '' ) $sql .= " AND tbl_hd_chamado.atendente = $atendente_busca";
	if ($status_busca=="p")      $sql .= " AND tbl_hd_chamado.status ILIKE 'Aprovação' ";
	if ($status_busca=="a")      $sql .= " AND tbl_hd_chamado.status ILIKE 'Análise' ";
	if ($status_busca=="r")      $sql .= " AND tbl_hd_chamado.status ILIKE 'Resolvido' ";
	if ($status_busca=="e")      $sql .= " AND tbl_hd_chamado.status ILIKE 'Execução' ";
	if ($status_busca=="c")      $sql .= " AND tbl_hd_chamado.status ILIKE 'Cancelado' ";
	if ($status_busca=="n")      $sql .= " AND ((tbl_hd_chamado.status ILIKE 'Novo') OR (tbl_hd_chamado.status IS NULL))";
	if ($status_busca=='')       $sql .= " AND ((tbl_hd_chamado.status <> 'Cancelado' AND tbl_hd_chamado.status <> 'Resolvido')) ";
}
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<td background='/assist/helpdesk/imagem/fundo_tabela_centro_esquerdo.gif' ><img src='/assist/imagens/pixel.gif' width='9'></td>";
	echo "<td colspan='8'><CENTER><B>Atendente</B></CENTER></td>";
...
#===========================
	...
	if (@pg_numrows($res) > 0) {
		...
		echo "	<td nowrap><img src='/assist/imagens/pixel.gif' width='5'><strong>Loja </strong></td>";
...
//inicio imprime chamados
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
			...
			$loja_nome            = pg_result($res,$i,nome);
	...
	*/
			...
			if($status == "Novo" OR $login_admin==$suporte) {echo "<a href='hd_adm_atendimento.php?hd_chamado=$hd_chamado'>";}
			else echo "<a href='adm_chamado_detalhe.php?hd_chamado=$hd_chamado'>";
	
			echo "<acronym title='$titulo'>$interno ";
			echo substr($titulo,0,20)."...</acronym></a></td>";
			
			if (($status != 'Resolvido') and ($status != 'Cancelado')) echo "<td nowrap><font color=#FF0000><B>$status </B></font></td>";
			else                                                      echo "<td nowrap>$status </td>";
	
			echo "<td nowrap><font size='1'>&nbsp;$data &nbsp;</font></td>";
			echo "<td><font size='1'>$empregado_nome";
			echo "</font></td>";
			echo "<td><font size='1'>&nbsp;$loja_nome&nbsp;</font></td>";
