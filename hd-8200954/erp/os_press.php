...
## VERIFICAÇÂO DAS SEM MOVIMENTAÇÃO
...
if ($login_fabrica==11){
	$sql="	SELECT	tbl_os.os,
					...
					to_char(tbl_os_item.digitacao_item - tbl_os.data_digitacao,'DD') as dias
			FROM tbl_os
			...
			WHERE tbl_os.fabrica = $login_fabrica
				...
				AND (
					to_char(tbl_os_item.digitacao_item - tbl_os.data_digitacao,'DD') IN (7,14,21,28)
					OR
					to_char(tbl_os_item.digitacao_item - tbl_os.data_digitacao,'DD') IN (5,13,19,26)
				)
	";
	$res1 = pg_exec ($con,$sql);
	if (pg_numrows($res1) >0) {
		for ($k=0;$k<pg_numrows($res1);$k++){
			$mom_os             = trim(pg_result($res1,$k,os));
			$mom_sua_os         = trim(pg_result($res1,$k,sua_os));
			$mom_abertura       = trim(pg_result($res1,$k,data_abertura));
			$mom_digitacao      = trim(pg_result($res1,$k,data_digitacao));
			$mom_digitacao_item = trim(pg_result($res1,$k,digitacao_item));
			$mom_dias           = trim(pg_result($res1,$k,dias));
			#echo "$mom_sua_os - $mom_dia";
		}
	}
}
...
if (($login_fabrica==3 OR $login_fabrica==11) AND 1==1 && strlen(trim($_POST['btn_acao']))>0 AND $_POST['btn_acao']=='gravar'){
...
if (($login_fabrica==3 OR $login_fabrica==11) AND $_POST['btn_acao']=='confirmar'){
...
if ($sistema_lingua=='ES') $title = "Confirmación de Orden de Servicio";
else $title = "Confirmação de Ordem de Serviço";
...
if($sistema_lingua=='ES') echo "ATENCIÓN";
else echo "ATENÇÃO";
echo "</font></b></td>";
    echo "</tr>";
    echo "<tr>";
...
if ($sistema_lingua == 'ES') echo "ORDEN DE SERVICIO COM NUMERO DE SERIE  <u>$serie_r</u> REINCIDENTE. ORDEN DE SERVICIO ANTERIOR";
else echo "ORDEM DE SERVIÇO COM NÚMERO DE SÉRIE: <u>$serie_r</u> REINCIDENTE. ORDEM DE SERVIÇO ANTERIOR:<br>";
    ...
    if($status_os==67){
		...
		if ($sistema_lingua=='ES') echo "ORDEN DE SERVICIO CON MISMO DISTRIBUIDOR Y FACTURA COMERCIAL REINCIDENTE. ORDEN DE SERVICIO ANTERIOR: <a href='os_press.php?os=$os_reincidente' target='_blank'>$sos</a>";
		else echo "ORDEM DE SERVIÇO COM MESMA REVENDA E NOTA FISCAL REINCIDENTE. ORDEM DE SERVIÇO ANTERIOR: <a href='os_press.php?os=$os_reincidente' target='_blank'>$sos</a>";
    }elseif($status_os==70){
		if ($sistema_lingua=='ES') echo "ORDEN DE SERVICIO CON MESMA DISTRIBUIDOR, FACTURA COMERCIAL Y PRODUCTO REINCIDENTE. ORDEN DE SERVICIO ANTERIOR: <a href='os_press.php?os=$os_reincidente' target='_blank'>$sos</a>";
		else echo "ORDEM DE SERVIÇO COM MESMA REVENDA, NOTA FISCAL E PRODUTO REINCIDENTE. ORDEM DE SERVIÇO ANTERIOR: <a href='os_press.php?os=$os_reincidente' target='_blank'>$sos</a>";
    }else{
...
<TR>
    <TD class="inicio" height='20' width='110' nowrap>&nbsp;&nbsp;Troca de Produto: </TD>
    <TD class="conteudo" height='20' width='130' nowrap><? echo " &nbsp;&nbsp;$nome_atendimento"; ?></TD>
	<TD class="inicio" height='20' width='50' >&nbsp;&nbsp;Motivo: </TD>
...
<TABLE width="700" border="0" cellspacing="1" cellpadding="0" class='Tabela' >
<TR>
    <TD  bgcolor="#FFE1E1" height='20'><h1><?
if($sistema_lingua=="ES"){
	ECHO "ORDEN DE SERVICIO EXCLUÍDA";
}else{
	echo "ORDEM DE SERVIÇO EXCLUÍDA";
	}
...
if ($login_fabrica==3 OR $login_fabrica==11){
	$sql_status = "SELECT    status_os,
				...
				FROM tbl_os_status
				WHERE os=$os
				AND status_os IN (72,73,62,64,65)
		...
		if ($status_os==72){
			echo "<br>
				...
				<div style='font-family:verdana;border:1px solid #D3BE96;width:700px;align:center;background-color:#FCF0D8' align='center'>
					...
					<b style='font-size:11px'>A peça solicitada necessita de autorização. <br>Aguarde a fábrica analisar seu pedido.</b>
				</div>
				</center><br>
			";
		}
...
if (($login_fabrica==3 OR $login_fabrica==11) AND $retorno==1 AND strlen($nota_fiscal_envio)==0){
	$sql_status = "SELECT    status_os,
				observacao
				FROM tbl_os_status
				WHERE os=$os
	...
	if ($resultado==1){
		...
		if ($status_os==65){
			...
			if ($login_fabrica==3){
				...
				echo "<br>
					<center>
					<b style='font-size:'15px''>Este produto deve ser enviado para a Assistência Técnica da Fábrica.</b><br></center><br>
				";
			}
		}
		if ($status_os==72){
			echo "<br>
				<center>
				<b style='font-size:'15px''>Pedido de Peça Necessita de Autorização</b><br>
				<div style='font-family:verdana;border:1px dashed #666666;padding:10px;width:400px;align:center' align='center'>
					<b style='font-size:12px'>A peça solicitada necessita de autorização. <br>Aguarde a fábrica analisar seu pedido.</b>
				</div></center><br>
			";
		}
		if ($status_os==73){
...
if (($login_fabrica==3 OR $login_fabrica==11) AND $retorno==1 AND strlen($msg_erro)>0){
...
if ( ($login_fabrica==3 OR $login_fabrica==11) AND $retorno==1 AND !$nota_fiscal_envio AND !$data_nf_envio AND !$numero_rastreamento_envio) {
?>
<br>
...
if (($login_fabrica==3 OR $login_fabrica==11) AND $retorno==1 AND $nota_fiscal_envio AND $data_nf_envio AND $numero_rastreamento_envio) {
	if (strlen($envio_chegada)==0){
		echo "<BR><b style='font-size:14px;color:#990033'>O Produto foi enviado a fábrica mas a fábrica ainda não confirmou seu recebimento.<br> Aguarde a fábrica confirmar o recebimento, efetuar o reparo e retornar o produto ao seu posto.</b><BR>";
	}else {
		if (strlen($data_nf_retorno)==0){
			...
			if (strlen($retorno_chegada)==0){
				echo "<BR><b style='font-size:14px;color:#990033'>O reparo do produto foi feito pela fábrica e foi enviado ao seu posto em $data_nf_retorno<br>Confirme após o recebimento</b><BR>";
			}
			else {
...
<table width='700' border="0" cellspacing="1" cellpadding="0" class='Tabela' align='center'>
    <tr >
        <td rowspan='4' class='conteudo' width='300' ><center>OS FABRICANTE<br>&nbsp;<b><FONT SIZE='6' COLOR='#C67700'>
            <?
    ...
    <tr>
        ...
        <td class='titulo' width='100' height='15'><?if($sistema_lingua=='ES')echo "CERRADA";else echo "FINALIZADA";?>&nbsp;</td>
    ...
    <tr>
        ...
        <td class='titulo' width='100' height='15'><?if($sistema_lingua=='ES')echo "	";else echo "FECHADO EM";?>
 ...
 &nbsp;</td>
        ...
        if(strlen($data_fechamento)>0 AND strlen($data_abertura)>0){
            ...
            if($total_de_dias_do_conserto==0) {
				if ($sistema_lingua=='ES') echo 'en el mismo día' ; 
				else echo 'no mesmo dia' ;
			}
            else echo $total_de_dias_do_conserto;
            if($total_de_dias_do_conserto==1) {
				if ($sistema_lingua=='ES') echo ' día' ;
				else echo ' dia' ;
			}
            if($total_de_dias_do_conserto>1) {
				if ($sistema_lingua=='ES') echo ' días' ;
				else echo ' dias' ;
			}
        }else{
            echo "NÃO FINALIZADO";
...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align='center' class='Tabela'>
    ...
    <TR>
        ...
        if (strlen($solucao_os)>0){
            ...
            if ( ($total_dias > 0 AND $login_fabrica==6) OR ($login_fabrica==11) ){
                $sql="select descricao from tbl_solucao where solucao=$solucao_os and fabrica=$login_fabrica limit 1";
                $xres = pg_exec($con, $sql);
...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align='center' class='Tabela'>
    ...
    <TR>
        <TD class="titulo" height='15'><?if($sistema_lingua=='ES')echo "NOMBRE";else echo "NOME";?>&nbsp;</TD>
        <TD class="conteudo" height='15' width='300'>&nbsp;<? echo $consumidor_nome ?></TD>
        <TD class="titulo"><?if($sistema_lingua=='ES')echo "TELÉFONO";else echo "FONE";?>&nbsp;</TD>
...
<?if($sistema_lingua=='ES')echo "DIAGNÓSTICOS - COMPONENTES - MANTENIMIENTOS EXECUTADOS";else echo "DIAGNÓSTICOS - COMPONENTES - MANUTENÇÕES EXECUTADAS";?>	
...
?>
		...
		<TD class="titulo2"><?				if($sistema_lingua=='ES'){
					echo "FAC. COMERCIAL";  
				}else{	
					echo "NOTA FISCAL";
				}
	...
	for ($i = 0 ; $i < $total ; $i++) {
		...
		if ($login_fabrica == 3) {
			...
			if (strlen($pedido) > 0) {
				...
				//distribuidor Telecontrol 4311, a nota fiscal é também pega do faturamento
		...
		//FIM NF para BRITANIA (DISTRIBUIDORES E FABRICANTES chamado 141) =========================


		}elseif ($login_fabrica==11){ # Agora o pedido da peça ta amarrado no faturamento item: Fabio 09/08/2007
					...
					if (pg_numrows ($resx) > 0) {
						...
						if (pg_numrows ($resx) > 0) {
							...
							if ($login_fabrica==1){
								...
								$data_nf = "";
								$link    = 1;
//								if($login_fabrica==6){ //hd 3437
								if($login_fabrica==6 and strlen($data_finalizada)>0){
									$nf = "Atendido";
								}
							}
						}
		...
		if (($login_fabrica==3 OR $login_fabrica==11) AND ( $bloqueada_pc=='t' OR $retorna_conserto='t')){
			...
			if ($status_os=='72' AND $servico_realizado_peca==$id_servico_realizado)
				$msg_peca_intervencao=" <b style='font-weight:normal;color:#FF6666;font-size:10px'>(aguardando autorização da fábrica)</b>";
			if ($status_os=='73' AND $servico_realizado_peca==$id_servico_realizado_ajuste){
				$msg_peca_intervencao=" <b style='font-weight:normal;color:#CC0000;font-size:10px'>(pedido cancelado pela fábrica)</b>";
				$cancelou_peca = "sim";
			}
			if ($status_os=='73' AND $servico_realizado_peca==$id_servico_realizado){
				$msg_peca_intervencao=" <b style='font-weight:normal;color:#333333;font-size:10px'>(autorizado pela fábrica)</b>";
				$cancelou_peca = "sim";
			}
			if ($status_os=='62' AND $servico_realizado_peca==$id_servico_realizado){
				$msg_peca_intervencao=" <b style='font-weight:normal;color:#FF6666;font-size:10px'>(aguardando autorização da fábrica)</b>";
			}
			if ($status_os=='64' AND $servico_realizado_peca==$id_servico_realizado){
				$msg_peca_intervencao=" <b style='font-weight:normal;color:#333333;font-size:10px'>(autorizado pela fábrica)</b>";
				$cancelou_peca = "sim";
			}
			if ($status_os=='64' AND $servico_realizado_peca==$id_servico_realizado_ajuste){
				$msg_peca_intervencao=" <b style='font-weight:normal;color:#CC0000;font-size:10px'>(pedido cancelado pela fábrica)</b>";
				$cancelou_peca = "sim";
			}
		}
		?>
		<TD class="conteudo" style="text-align:left;"><? echo pg_result($res,$i,referencia_peca) . " - " . $descricao_peca; echo $msg_peca_intervencao?></TD>
		...
		<TD class="conteudo" style="text-align:CENTER;" nowrap <? if (strlen($data_nf)==0) echo "colspan='2'"; ?>>
		<?
		if (strtolower($nf) <> 'pendente' and strtolower($nf) <> 'atendido'){
			...
			if (pg_numrows ($resY) > 0) {
				...
				if( strtolower($nf) <> 'atendido'){
				echo "<acronym title='Pendente com o fabricante.' style='cursor:help;'>";
				}
				echo "$nf &nbsp;";
			}
		}
		...
		if (strlen($data_nf) > 0){
			echo "<TD class='conteudo' style='text-align:CENTER;' nowrap>";
			echo "$data_nf ";
			echo "</TD>";
		}
		?>
		...
		if ($login_fabrica==3) {
			...
			if (strlen($nota_fiscal_distrib) > 0) {
				if ($link_distrib == 1) {
					echo "<acronym title='Nota Fiscal do Distribuidor.' style='cursor:help;'><a href='nota_fiscal_detalhe.php?nota_fiscal=".$nota_fiscal_distrib."&peca=$peca' target='_blank'>$nota_fiscal_distrib  - $data_nf_distrib</a>";
				}else{
	...
	if($login_fabrica == 1 AND ($tipo_atendimento == 17 OR $tipo_atendimento == 18)){
		echo "<tr align='center'>";
			...
			echo "<td class='conteudo'></td>";
			echo "<td class='conteudo'></td>";
			echo "<td class='conteudo' align='center'><center>$nota_fiscal_saida</center></td>";
			echo "<td class='conteudo' align='center'><center>$data_nf_saida</center></td>";
		echo "<tr>";
	}
...
if ($login_fabrica == "20") {
    ...
    if($sistema_lingua=='ES'){
        echo "<td align='center' bgcolor='#E1EAF1'><b>";
        if($sistema_lingua=='ES')echo "Impuesto IVA";else echo "Desconto IVA";
        echo "</b></td>";
    }
