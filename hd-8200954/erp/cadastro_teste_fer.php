	...
	if($login_empresa == 49){
		...
		if(strlen($cnpj) > 0 AND strlen($nome) == 0){
			...
			if(pg_numrows($res) > 0){
				...
				if(pg_numrows($res) > 0){
					$posto = pg_result($res,0,0);
					$msg_erro = "Posto Não gravado. Complete as informações";
				}else{
					$msg_erro = "Posto não encontrado na base de dados";
				}
			}
		}
...
*/
		...
		if(strlen($posto) == 0 AND strlen($nome) > 0 AND strlen($msg_erro) == 0){
			$msg_erro = "Verifica se tem novo PA. Caso não tenha grava novo posto";
		}
		//echo "$sql";
