...
<table class='Pesquisa' align='center' width='750' border='0'>
	...
	<tbody>
	<tr>
		<th nowrap width='80'>Orçamento:</th>
	...
	<tr >
		<th nowrap width='80'>Aprovação:</th>
		<td nowrap colspan='3'>
			<select name='aprovacao'  class='Caixa'>
				<option value=''></option>
				<option value='aprovados'  <?if($aprovacao =='aprovados')  echo "SELECTED";?>>Aprovados</option>
				<option value='reprovados' <?if($aprovacao =='reprovados') echo "SELECTED";?>>Reprovados</option>
				<option value='aguardando' <?if($aprovacao =='aguardando') echo "SELECTED";?>>Aguardando Aprovação</option>
				<option value='todos'      <?if($aprovacao =='todos')      echo "SELECTED";?>>Todos</option>
			</select>
		</td>
	</tr>
	<tr >
		<th nowrap width='80'>Marca:</th>
		<td>
...
<table width='98%'>
	<tr>
		<td bgcolor='#FFFFCC' style='border: #000000 1px solid; width:20px'>&nbsp;</td>
		<td>Reincidência</td>
	</tr>
</table>
<? 
...
if(strlen($marca)>0){
	$filtro .= " AND tbl_orcamento_os.marca = $marca";
}
...
if(strlen($filtro)==0) {
	$filtro =" AND tbl_orcamento.aprovado IS TRUE AND tbl_orcamento.status<>35 AND tbl_orcamento_os.fechamento is null";
}
...
if(pg_numrows($res)>0){
	...
	echo "<b>Nenhum Orçamento encontrado</b>";
}
?>
