...
function pegaCaixa (data,dados,cor) {
	url = "ajax_caixa.php?ajax=sim&acao=detalhes&data=" + escape(data);
...
if(strlen($btn_acao)>0){
	...
	if(strlen($de)>0){
		...
		$sql = "SELECT '$aux_de'::date < current_date";
		$res     = @pg_exec($con,$sql);
		$aux     = @pg_result ($res,0,0);
		if($aux == 't') $msg_erro = "Data não pode ser menor que a data de hoje(".date("d/m/Y").")";
	}
	...
	if(strlen($ate)==0){
		...
		$sql = "select ('$aux_de'::date+interval '30 day')::date,to_char('$aux_de'::date+interval '30 day','DD/MM/YYYY');";
		$res     = @pg_exec($con,$sql);
		$aux_ate = @pg_result ($res,0,0);
		$ate     = @pg_result ($res,0,1);
...
if(strlen($msg_erro)==0){
	$sql = "SELECT TO_CHAR(a.vencimento,'DD/MM/YYYY') AS data_baixa,
		...
		FROM (
			(
				...
				FROM tbl_pagar 
				left JOIN tbl_pessoa ON tbl_pessoa.pessoa = tbl_pagar.pessoa_fornecedor 
	...
	if (@pg_numrows($res) > 0) {
		...
		echo "<TD colspan='2' align='left'><b>FLUXO DE CAIXA</b></TD>";
