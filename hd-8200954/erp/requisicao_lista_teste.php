...
if($_POST["botao"]=="Continuar"){
	...
	if(strlen($erro)==0){
		...
		$res  = pg_exec($con, "create temp table tmp_requisicao_lista(
								requisicao_lista serial,
								data TIMESTAMP,
								empregado int4,
								status varchar(30));");
		if(strlen(pg_errormessage($con))>0){
			$erro.= "Erro ao CRIAR TMP_REQUISICAO_LISTA";
		}

		$res  = pg_exec($con, "create temp table tmp_requisicao_lista_item(
								requisicao_lista_item serial,
								peca int4,
								quantidade_disponivel int4,
								quantidade_entregar int4,
								quantidade_solicitada int4,
								quantidade_comprar int4,
								selecao varchar(15),
								requisicao_lista integer,
								media_7 real,
								media_20 real,
								media_40 real,
						);");
		...
		$sql= "INSERT INTO 
				tmp_requisicao_lista (data, empregado, status) 
				VALUES(current_timestamp,  $login_empregado, 'em cotacao');";

		$res= pg_exec($con, $sql);

		//echo "<br>1-sql: $sql";
		//$sql=" SELECT (max(requisicao_lista)+1) as requisicao_lista from tmp_requisicao_lista; ";

		$sql=" SELECT CURRVAL('tmp_requisicao_lista_requisicao_lista_seq') as requisicao_lista;";
		$res= pg_exec($con, $sql);
			...
			if(strlen($erro)==0 and strlen($selecao)>0) {
				...
				if (strlen($bloquear)>0){
					...
					$sql= " UPDATE tbl_requisicao_item 
							SET 
								empregado		= $empregado,
								data_bloqueada	= current_timestamp
...
SELECT 
									...
									$quantidade_comprar, 
									tbl_estoque.qtde, 
									sum(tmp_requisicao_lista_item.quantidade_solicitada) as qtd, 
									sum(tmp_requisicao_lista_item.quantidade_comprar) as qtd2,
									...
									cast(tbl_estoque_extra.media_7 as numeric(12,2)),
									'a comprar'
								FROM tmp_requisicao_lista
								JOIN tmp_requisicao_lista_item USING(requisicao_lista)
								JOIN tbl_estoque on tbl_estoque.peca =  tmp_requisicao_lista_item.peca
								JOIN tbl_estoque_extra on tbl_estoque_extra.peca = tbl_estoque.peca
								WHERE requisicao_lista = $requisicao_lista and selecao ='sim'
								GROUP BY 
									tmp_requisicao_lista_item.peca, 
									tbl_estoque.qtde, 
					...
					<form method='POST' name="Pesquisar" action="requisicao_mostra_listas.php">
						<tr>
							<td class='Label' align='center'><BR>Usuário:
								<?		$sql2 = "SELECT 
												DISTINCT (tmp_requisicao_lista.empregado),
												pessoa_empregado.nome
												from tmp_requisicao_lista
												JOIN tbl_cotacao using(requisicao_lista)
												JOIN tmp_requisicao_lista_item USING(requisicao_lista)
												JOIN tbl_empregado on tbl_empregado.empregado = tmp_requisicao_lista.empregado
					...
					<FORM ACTION='<?echo $PHP_SELF;?>' METHOD='POST'>
						<tr >
							<td class='menu_top' nowrap colspan='13' align='right' >
								<a href='<?echo $PHP_SELF;?>?requisicao_sistema=gerar'><font color='#0000ff'>GERAR REQUISIÇÃO DO SISTEMA</font></a>
							</td>
						</tr>
						<tr bgcolor='#596D9B'>
