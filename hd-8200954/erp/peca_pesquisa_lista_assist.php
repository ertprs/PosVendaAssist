...
//if ($login_fabrica == 11 AND $login_posto <> 6359) $produto = "";
...
if ($ip=="201.13.180.14"){ echo "$sql";}
	$res = pg_exec ($con,$sql);
	if (@pg_numrows ($res) == 0) {
		if ($sistema_lingua=='ES') echo "<h1>Ninguna lista básica de repuesto encuentrada para esta herramienta</h1>";
...
if ($tipo == "descricao") {
	...
	if($sistema_lingua == "ES") { 
		...
		echo "<font face='Arial, Verdana, Times, Sans' size='2'>Pesquisando por <b>descrição da peça</b>: ";
	}
	echo "<h4><i>$descricao</i></font>";
	echo "<p>";
	...
	$res = pg_exec($con,$sql);
//if ($ip=="201.13.180.14"){ echo "$sql"; exit;}
if (@pg_numrows($res) == 0) {
		if ($login_fabrica == 1) {
...
//if ($ip=="201.92.126.18"){ echo "$sql"; exit;}
	if (@pg_numrows($res) == 0) {
		if ($login_fabrica == 1) {
...
if($sistema_lingua == "ES") echo "<h1>Pieza '$referencia' no encuentrada <br>para el producto $produto_referencia</h1>";
...
/*	if ($login_fabrica == 3 && getenv("REMOTE_ADDR") == "201.0.9.216") {
	...
	if($login_fabrica == 6) { 
		$resT = pg_exec($con,"SELECT tabela FROM tbl_posto_linha JOIN tbl_linha using(linha) WHERE posto = $login_posto AND fabrica = $login_fabrica limit 1"); 
