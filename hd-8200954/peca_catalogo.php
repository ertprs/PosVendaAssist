...
if ($tipo == "tudo") {
	...
	if ($sistema_lingua=='ES') echo "<h4>Buscando toda la lista básica de la herramienta: <br><i>$produto_referencia - $produto_descricao</i></h4>";
...
//if ($ip="201.13.179.45"){ echo "$sql"; exit;}
	$res = pg_exec ($con,$sql);
	if (@pg_numrows ($res) == 0) {
		if ($sistema_lingua=='ES') echo "<h1>Ninguna lista básica de repuesto encuentrada para este producto</h1>";
...
if ($tipo == "descricao") {
	...
	if ($sistema_lingua=='ES') echo "<h4>Buscando por <b>descripción de repuesto </b>: <i>$descricao</i></h4>";
	else echo "<h4>Pesquisando por <b>descrição da peça</b>: <i>$descricao</i></h4>";
	echo "<p>";
...
if ($tipo == "referencia") {
	...
	if ($sistema_lingua=='ES') echo "<font face='Arial, Verdana, Times, Sans' size='2'>Pesquisando por <b>referência de repuesto</b>: <i>$referencia</i></font>";
	else echo "<font face='Arial, Verdana, Times, Sans' size='2'>Pesquisando por <b>referência da peça</b>: <i>$referencia</i></font>";
	echo "<br><br>";
...
//if ($ip == '201.13.179.45') echo ($produto_referencia); echo ($produto);
	...
	if (@pg_numrows($res) == 0) {
		if ($login_fabrica == 1) {
			...
			if ($sistema_lingua=='ES') echo "<h1>Repuesto '$referencia' no encuentrada<br>para el producto $produto_referencia</h1>";
	...
	if (pg_numrows($resX) == 0) {
		...
		if (strlen($peca_fora_linha) > 0) {
			...
			if ($login_fabrica == 1) echo "É obsoleta,<br>não é mais fornecida";
				...
				if ($sistema_lingua=='ES') echo "Fuera de línea";
...
//		echo "Não há previsão de chegada da Peça Código $peca_previsao.<br>Favor encaminhar e-mail para <a href='mailto:assistenciatecnica@britania.com.br'>assistenciatecnica@britania.com.br</a>, informando o número da Ordem de Serviço e o código do Posto Autorizado.<br>Somente serão aceitas requisições via email! NÃO utilizar o 0800.";
		...
		echo "<br>";
		if ($sistema_lingua=='ES') echo "Para repuesto con plazo de fornecimiento superio a 25 días, la planta tomará las medidas necesárias para atendimient del consumidor";
