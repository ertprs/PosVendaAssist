...
if(strlen($ajax)>0){
	...
	if(strlen($peca)>0){
		$sql = "SELECT 	tbl_estoque_posto_movimento.peca                              , 
						...
						(select observacao from tbl_os_status where tbl_os_status.os = tbl_estoque_posto_movimento.os order by tbl_os_status.data desc limit 1 ) as obs
		...
		//echo "<div style='border: 1px solid #cdcdcd;background-color: #FFFFFF;width:50px;align:right'><a href='javascript:fechar();'><B>Fechar</b></a></div>";
			...
			//echo "<caption align='right' bgcolor='#FFFFFF'><a href='javascript:fechar(". pg_result ($res,0,peca) .");'><B>Fechar</b></a>";
	//		echo "</caption>";
			echo "<thead>";
			echo "<tr>";
			echo "<th><font color='#FFFFFF'><B>Movimentação</B></FONT></th>";
...
$title = "ESTOQUE DE PEÇAS EM GARANTIA";
$layout_menu = 'pedido';
...
function retornaMovimentacao(peca){
	...
	url = "estoque_pecas.php?ajax=true&peca="+ peca;
...
<?
$sql = "SELECT 	DISTINCT tbl_peca.referencia,tbl_peca.peca,
				tbl_peca.descricao, 
				...
				ORDER BY emissao DESC LIMIT 1) as emissao_dias,
				(SELECT to_char(tbl_faturamento.emissao,'DD/MM/YYYY')
				FROM tbl_faturamento 
				JOIN tbl_estoque_posto_movimento on tbl_estoque_posto_movimento.faturamento = tbl_faturamento.faturamento
				WHERE tbl_estoque_posto.peca = tbl_estoque_posto_movimento.peca
				AND tbl_estoque_posto_movimento.faturamento notnull
				ORDER BY emissao DESC LIMIT 1) as emissao
...
if(pg_numrows($res)>0){
?>
<table width="100%" border="0" cellspacing="1" cellpadding="3" align='center' bgcolor='#596D9B'>
		<tr>
			<td class='menu_top'>Peca</td>
			<td class='menu_top'>Saldo</td>
			<td class='menu_top'>Devolução</td>
			<td class='menu_top'>Compra</td>
		</tr>
<?
...
?>
		<tr>
			<td class='table_line1' bgcolor='<? echo $cor;?>'>
			
			<?echo "<a href=\"javascript:mostraMovimentacao($peca);\">$peca_referencia - $peca_descricao</a>";?><BR>
			<div id='dados_<? echo $peca; ?>' style='position:absolute; display:none; border: 1px solid #949494;background-color: #b8b7af;width:700px;'></div>
			...
			<td align='center' class='table_line1' bgcolor='<? echo $cor;?>'>
			<? if($devolve==true){ ?>
				<input type='text' onblur="Calcula(<? echo $x; ?>,'devolucao')" name='devolucao_<? echo $x; ?>'  id='devolucao_<? echo $x; ?>' value='' size='3' maxlength='3'>
			<? } ?>
			</td>
			<td align='center' class='table_line1' bgcolor='<? echo $cor;?>'>
			<? if($devolve==true){ ?>
				<input type='text' onblur="Calcula(<? echo $x; ?>,'compra')" id='compra_<? echo $x; ?>' name='compra_<? echo $x; ?>' value='' size='3' maxlength='3'>
			<? } ?>
			</td>
...
<BR><BR><center><img border='0' src='imagens/btn_confirmar.gif' onClick="javascript: if (document.frm_estoque.btn_confirmar.value == '' ) { document.frm_estoque.btn_confirmar.value='Confirmar' ; document.frm_estoque.submit(); } else { alert ('Aguarde submissão'); }"></center>
