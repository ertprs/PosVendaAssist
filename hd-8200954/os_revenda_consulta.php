...
if($login_fabrica == 1){
	include("os_revenda_blackedecker_consulta.php");
	exit;
}
...
$sql = "SELECT		distinct
		...
		AND			tbl_os_revenda.fabrica    = $login_fabrica AND (1=2 ";
...
if(strlen($chk4) > 0){
	...
	$monta_sql .= "OR (tbl_os_revenda.digitacao BETWEEN '$mes_inicial' AND '$mes_final') ";
...
if(strlen($chk10) > 0){
	...
	if ($numero_os){
		...
		if ($login_fabrica == 1) {
			$numero_os = substr($numero_os, strlen($login_posto), strlen($numero_os));
		}
...
if (pg_numrows($res) == 0) {
	...
	echo "<TD>REVENDA</TD>\n";
	echo "<TD width='170' colspan='3'>AÇÕES     </TD>\n";
	echo "</TR>\n";
	...
	echo "<TD>$posto_codigo - $sua_os</TD>\n";
	echo "<TD align='center'>$data</TD>\n";
	echo "<TD><ACRONYM TITLE=\"$revenda_nome\">".substr($revenda_nome,0,35)."</ACRONYM></TD>\n";
	echo "<TD width=85>";
...
//echo $sql; exit;
	...
	echo "</TD>\n";
	echo "<TD width=85>";
	if ($login_fabrica == 1 and pg_numrows($resX) == 0 OR strlen($explodida) == 0) echo "<a href='os_revenda_finalizada.php?os_revenda=$os_revenda&btn_acao=explodir'><img src='imagens/btn_explodir.gif'></a>";
	echo "</TD>\n";
	echo "<TD width=85><a href='os_revenda_print.php?os_revenda=$os_revenda' target='_new'><img src='imagens/btn_imprimir_".$btn.".gif'></a></TD>\n";
	echo "</TR>\n";
	}
}
