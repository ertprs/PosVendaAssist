								...
								if($login_fabrica == 11 and strlen($erro) == 0){ #HD 96191
									...
									if (pg_num_rows($res_nf) > 0){
										...
										for ($x = 0 ; $x < pg_num_rows($res_nf); $x++) {
											$sua_os_nf .= trim(pg_fetch_result($res_nf,$x,sua_os))."<br>";
											$finalizada_nf = trim(pg_fetch_result($res_nf,$x,finalizada));

											$sql_nf2 = "SELECT CURRENT_TIMESTAMP - '$finalizada_nf'";
											$res_nf2 = @pg_query($con,$sql_nf2);
											$tempo = @pg_fetch_result($res_nf2,0,0);
											if(substr($tempo,0,5)!="00:00"){
												$erro.="Nota Fiscal já utilizada para devolução da OS<br> ".$sua_os_nf;
											}
										}
									}
								}
...
function filtro(bolinha){
	...
	gerarCookie('bolinha_atual',bolinha,0);
}

$().ready(function() {
	var bolinha;
		bolinha = lerCookie('bolinha_atual');
	if (bolinha == null) bolinha = '';
	...
	if (listar.length == 0) {
		excluirCookie('bolinha_atual');
		var bolinha = lerCookie('bolinha_atual');
	}
...
if ((strlen($sua_os) > 0 AND $btn_acao_pesquisa == 'continuar') OR strlen($listar) > 0 OR strlen ($codigo_posto) > 0 OR (strlen($prateleira_box) > 0 AND $btn_acao_pesquisa == 'continuar' ) ){
		...
		$sql = "SELECT  tbl_os.os                                                  ,
				...
				/* HD 204146: Fechamento automático de OS */
				AND     (tbl_os.data_fechamento IS NULL OR tbl_os.sinalizador=18)
...
//HD 4291 PAULO  HD 14121
	if(usaDataConserto($login_posto, $login_fabrica) or ($login_fabrica ==20 AND $login_pais=='BR')) {
		##### LEGENDAS - INÍCIO #####
			echo "<br>";
			echo "<div align='left' style='position: relative; left: 10'>";
...
#@@@@@@@@@@####################################################################@@@@@@@@@@#
#@@@@@@@@@@   Quando Alterar os status dessa tela, também tem que alterar da   @@@@@@@@@@#
#@@@@@@@@@@   Dynacom, fica na /telecontrol/www/consulta_os_dynacom.php        @@@@@@@@@@#
#@@@@@@@@@@####################################################################@@@@@@@@@@#
...
//  HD 263030 - Se não tem defeito constatado mas tem pedido (não deveria, mas...) mudar para 'aguardando peça'
//              Tirei a conferência de pedido de peças do 'else's
							...
							if ($login_fabrica == 3) {
								if (strlen($pedido) > 0 and (($peca <> $peca_anterior and $pedido<>$pedido_anterior) or ($peca <> $peca_anterior and $pedido == $pedido_anterior))) {
										...
										if (pg_num_rows ($resx) == 0) {
											$bolinha="amarelo";
										}elseif ($login_posto =='4311' and pg_num_rows($resx) >0) {
											//hd 22576
											$bolinha="amarelo";
										}
								}else {
								...
								$resX = pg_query ($con,$sql);
								if (pg_num_rows ($resX) > 0) {
									$conta_item=pg_fetch_result($resX,0,conta_item);
									$conta_os  =pg_fetch_result($resX,0,conta_os);
									if(strlen($conta_item) > 0){
										if($login_posto==4311) {
												...
												$resX = pg_query ($con,$sql);
												if (pg_num_rows ($resX) > 0) {
													$embarcado      = trim(pg_fetch_result($resX,0,embarcado));
												}
												if($embarcado==$conta_item ){
													$bolinha="rosa";
												}
										}else{
											$sql = "SELECT	count(tbl_faturamento_item.faturamento_item) as item
													FROM tbl_faturamento_item
													JOIN tbl_os_item ON tbl_os_item.pedido = tbl_faturamento_item.pedido and tbl_faturamento_item.peca = tbl_os_item.peca
													WHERE tbl_os_item.os_item in (SELECT os_item FROM tbl_os_produto JOIN tbl_os_item using(os_produto) WHERE os=$conta_os
													)	";
											$resX = pg_query ($con,$sql);
											if (pg_num_rows ($resX) > 0) {
												$item      = trim(pg_fetch_result($resX,0,item));
											}
											if($item >= $conta_item ){
												$bolinha="rosa";
											}
								...
								if (strlen($faturamento_item)>0 ){
									...
									if (pg_num_rows ($resx) == 0) {
										...
										$nota_fiscal=pg_fetch_result($resx,0,nota_fiscal);
										...
										if (pg_num_rows ($resx) == 0) {
											...
											if (pg_num_rows ($resxx) == 0) {
												if ($login_fabrica==1){
													...
													$resxxx = pg_query ($con,$sqlxxx);

													if (pg_num_rows ($resxxx) > 0) {
														$bolinha="amarelo";
													}
												}else{
													$bolinha="amarelo";
...
?>
		...
		<tr bgcolor=<?
			echo $cor;
			echo " rel='$bolinha' ";
			?> <? if (($erros == $os or strpos($erros,$os) > 0) and strlen ($msg_erro) > 0 )?>>
...
<? } ?>
			<? if (strlen($flag_bloqueio) == 0) { ?>
					<input type="checkbox" class="frm" name="ativo_<?echo $i?>" id="ativo" value="t" <? if($login_fabrica==3){ ?> onClick='javascript:mostraDados(<?echo $i; ?>);' <? } ?>>
			 <? } ?></td>
			<? //Alterado por Wellington 06/12/2006 a pedido de Luiz Antonio, posto Jundservice (Lenoxx) deve abrir os_item ao clicar na OS ?>
			<?
			//HD 4291 Paulo
			if((usaDataConserto($login_posto, $login_fabrica) or ($login_fabrica ==20 AND $login_pais=='BR') ) and strlen($bolinha)> 0 ) {
	...
	if((usaDataConserto($login_posto, $login_fabrica) or ($login_fabrica ==20 AND $login_pais=='BR') ) and strlen($bolinha)> 0 ) { echo $bolinha; }
...
?>
		<tr bgcolor=<?
			...
			?> <? if ((strpos($erros,$os) > 0 or $erros == $os) and strlen ($msg_erro) > 0 )?>>
			<input type='hidden' name='os_<? echo $i ?>' value='<? echo pg_fetch_result ($res,$i,os) ?>' >
			<input type='hidden' name='conta_<? echo $i ?>' value='<? echo $i;?>'>
			<input type='hidden' name='consumidor_revenda_<? echo $i ?>' value='<? echo pg_fetch_result ($res,$i,consumidor_revenda)?>'>
			<td align="center">
	...
	<td><?
		if((usaDataConserto($login_posto, $login_fabrica) or ($login_fabrica ==20 AND $login_pais=='BR') ) and strlen($bolinha)> 0 ) { echo $bolinha; } ?><a href='<? if ($cor == "#FFCC66" or ($login_fabrica==11 and $login_posto==14254)) echo "os_item"; else echo "os_press"; ?>.php?os=<? echo $os ?>' target='_blank'><? if ($login_fabrica == 1)echo $login_codigo_posto; echo $sua_os; ?></a></td>
			<td><? echo pg_fetch_result ($res,$i,data_abertura) ?></td>
			<td NOWRAP ><? echo substr (pg_fetch_result ($res,$i,consumidor_nome),0,10) ?></td>
