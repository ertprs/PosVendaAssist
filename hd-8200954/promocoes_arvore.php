...
<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
	<tr bgcolor = '#fafafa'>
		...
		<?if($login_fabrica == 19){?>
			<td  class="chapeu" colspan='2' >Formulários</td>
		<?}else{?>
			<td  class="chapeu" colspan='2' >Promoções</td>
		<?}?>
	</tr>
...
if($total_esquemas > 50){
	$sql = "SELECT DISTINCT tbl_familia.familia                                  ,
			...
			AND     tbl_comunicado.tipo = 'Promoções'
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			if($linha_anterior <> $linha) {
				echo "<br><dt>&nbsp;&nbsp;<b>»</b> <a href='info_tecnica_visualiza.php?tipo=Promoções&linha=$linha'>$nome</a><br></dt>";
			}
			echo "<dd>&nbsp;&nbsp;<b>»</b> <a href='info_tecnica_visualiza.php?tipo=Promoções&linha=$linha&familia=$familia'>$descricao</a><br></dd>";
	...
	if($login_fabrica == 19){
		$cond_sql = "AND ((tbl_comunicado.tipo = 'Promocao') OR (tbl_comunicado.tipo = 'Formulários'))";
	}else{
		$cond_sql = "AND tbl_comunicado.tipo =  'Promocao'";
	}
...
//	echo $sql;
	...
	if (pg_numrows ($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			$com_tipo              = trim(pg_fetch_result($res, $i, 'tipo'));
...
<br>
		...
		<?if($login_fabrica == 19){?>
			<td rowspan='2'class='detalhes' width='350'>Escolha o produto que deseja consultar os Formulários.</td>
		<?}else{?>
			<td rowspan='2'class='detalhes' width='350'>Escolha o produto que deseja consultar as  Promoções.</td>
		<?}?>
	</tr>
	<tr bgcolor='#D9E2EF'>
