...
<table width='100%' border='0' cellspacing='0' align='center' cellpadding='2' id='Menu'>
	<tr height='50' >
		<td bgcolor='#ffeac0' align='center' width='250' height='50'><img src='../logos/telecontrol_new.gif'></td>
		<td bgcolor='#ffeac0' align='left'  valign='middle'><b>Produtos</b></td>
	</tr>
</table>
<br>
...
if ($tipo == "descricao") {
	...
	$sql = "SELECT DISTINCT tbl_produto.*,
				tbl_revenda_produto.referencia AS referencia_revenda
			FROM     tbl_produto
...
if ($tipo == "referencia") {
	...
	$sql = "SELECT  tbl_produto.*,
			...
			JOIN     tbl_revenda_produto USING (produto)
 			JOIN     tbl_linha ON tbl_produto.linha = tbl_linha.linha
			WHERE    (tbl_revenda_produto.referencia LIKE '%$referencia%' OR tbl_produto.referencia LIKE '%$referencia%')
			AND      tbl_linha.fabrica           = $login_fabrica
			AND      tbl_revenda_produto.revenda = $login_revenda
			ORDER BY tbl_produto.descricao";
	$res = pg_exec ($con,$sql);
...
if (pg_numrows($res) == 1) {
	...
	echo "referencia.value = '".trim(pg_result($res,0,referencia_revenda))."';";
	echo "this.close();";
	echo "</script>\n";
}
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		$referencia_revenda = trim(pg_result($res,$i,referencia_revenda));
...
//takashi 06/07/06 chamado 300 helpdesk
		...
		echo "<font face='Arial, Verdana, Times, Sans' size='-1' color='#000000'>$referencia_revenda</font>\n";
//takashi 06/07/06 chamado 300 helpdesk
		echo "</a>\n";
//takashi 06/07/06 chamado 300 helpdesk
		...
		echo "referencia.value = '$referencia_revenda'; ";
		if ($_GET["proximo"] == "t") echo "proximo.focus(); ";
		echo "this.close() ; \" >";
