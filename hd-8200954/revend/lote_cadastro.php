...
if ($acao == "gravar" AND $ajax == "sim") {
	...
	$revenda_posto = trim($_POST['revenda_posto']);
	...
	if(strlen($nota_fiscal)  ==0) $msg_erro .= "Digite o número da nota fiscal<br>";
	if(strlen($data_nf)      ==0) $msg_erro .= "Digite a data da nota fiscal<br>";
	if(strlen($responsavel)  ==0) $msg_erro .= "Digite o nome do responsável<br>";
	if(strlen($lote)         ==0) $msg_erro .= "Digite o número do lote<br>";
	if(strlen($revenda_posto)==0) $msg_erro .= "Selecione o posto no qual deseja enviar o lote<br>";
	else{
		$sql = "SELECT posto,fabrica FROM tbl_revenda_posto WHERE revenda_posto=$revenda_posto";
		$res = pg_exec ($con,$sql);
		if(pg_numrows($res)>0){
	...
	$fnc            = @pg_exec($con,"SELECT fnc_formata_data('$data_nf')");
	if (strlen ( pg_errormessage ($con) ) > 0) {
		$msg_erro .= pg_errormessage ($con) ;
	}
	if (strlen($msg_erro) == 0) $data_nf = @pg_result ($fnc,0,0);
	
	//if (strlen($erro) == 0) $aux_data_inicial = @pg_result ($fnc,0,0) ." 00:00:00";
	if (strlen($erro) == 0) $data_nf = @pg_result ($fnc,0,0);
				...
				if (strlen($msg_erro) == 0) {
					...
					if (strlen($produto_referencia) > 0 ) {
						...
						$sql =	"
								...
								AND     tbl_revenda_produto.referencia = '$xproduto_referencia'
						...
						if (@pg_numrows($res) == 1) {
							...
							$msg_erro .= "<br>Produto $produto_referencia não cadastrada.";
							$linha_erro = $i;
						}
...
if(strlen($importa) > 0){
	...
	if(strlen($responsavel)==0) $msg_erro .= "Digite o nome do responsável<br>";
	if(strlen($lote)       ==0) $msg_erro .= "Digite o número do lote<br>";
	if(strlen($posto)      ==0) $msg_erro .= "Selecione o posto no qual deseja enviar o lote<br>";
	...
	if (strlen($msg_erro) == 0) $aux_data_nf = @pg_result ($fnc,0,0);
			...
			if (!copy($arquivo["tmp_name"], $nome_arquivo)) {
				...
				while (!feof($f)){
					...
					if($buffer <> "\n" and strlen(trim($buffer))>0){
						list($referencia, $qtde) = explode("\t", $buffer);
						...
						if(strlen($produto)==0){
							...
							if (pg_numrows($res) == 1) $produto = pg_result($res,0,produto);
							else                       $msg_erro = "Produto $referencia não cadastrado.";
						}
						if(strlen($msg_erro)>0)    $msg_erro = "Erro na linha $i:".$msg_erro;
							...
							if(pg_numrows($res)==0){
								...
								$sql = "UPDATE tbl_lote_revenda_item SET qtde = qtde + $qtde WHERE produto = $produto AND lote_revenda=$lote_revenda";
								$res = @pg_exec($con,$sql);
								$msg_erro = pg_errormessage($con);
							}
						}
...
$aba = 2;
$title = "Cadastro de Lote de Revenda";
...
<script type="text/javascript" charset="utf-8">
	$(function(){
		$('#data_nf').datePicker({startDate:'01/01/2000'});
		$("#data_nf").maskedinput("99/99/9999");
		$('#data_nf2').datePicker({startDate:'01/01/2000'});
		$("#data_nf2").maskedinput("99/99/9999");
...
function fnc_pesquisa_produto (campo, campo2, tipo) {
	...
	if (xcampo.value != "") {
		var url = "";
		url = "produto_pesquisa.php?campo=" + xcampo.value + "&tipo=" + tipo + "&proximo=t";
...
<?
$qtde_item=0;
echo "<table style=' border:#485989 1px solid; background-color: #e6eef7' align='center' width='750' border='0' cellspacing='0' cellpadding='4'>";

echo "<tr height='20' bgcolor='#BCCBE0'>";
echo "<td align='left' colspan='4'><b>Lote de Produto</b> </td>";
echo "</tr>";
...
<div id="container-Principal">
	<ul>
		<li>
			<a href="#manual" ><span><img src='../imagens/rec_produto.png' width='10' align="absmiddle" >Cadastro Manual</span></a>
		</li>
		<li>
			<a href="#auto" ><span><img src='../imagens/duv_produto.png' width='10' align=absmiddle>Upload.</span></a>
		</li>
	</ul>
<div id="manual" class='tab_content'>
<form name="frm_os" method="post" action="<? echo $PHP_SELF ?>">
<?
	echo "<table width='100%'>";
	echo "<tr height='20' class='Conteudo'>";
	echo "<td align='right' height='20'><b>Lote</b>&nbsp;</td>";
	...
	echo "<td align='left' ><input type='text' name='nota_fiscal' id='nota_fiscal' value='$nota_fiscal' maxlength='6' size='10'class='Caixa'></td>";
	echo "<td align='right' ><b>Data</b>&nbsp;</td>";
	echo "<td align='left' ><input type='text' name='data_nf' id='data_nf' value='$data_nf' class='Caixa' size='12' ></td>";
	echo "</tr>";
	
	
	echo "<tr height='20' class='Conteudo'>";
	echo "<td align='right' valign='top'><b>Posto de Destino</b>&nbsp;</td>";
	...
	$sql = "SELECT DISTINCT
		...
		WHERE tbl_revenda_posto.revenda = $login_revenda
		AND   tbl_revenda_posto.fabrica = $login_fabrica
		ORDER BY fabrica_nome,posto_nome;";
		$res = pg_exec ($con,$sql);
	
		if(pg_numrows($res)>0){
			for ($i = 0 ; $i < pg_numrows($res); $i++){
				if($fabrica_nome <> trim(pg_result ($res,$i,fabrica_nome))){
					$fabrica_nome = trim(pg_result ($res,$i,fabrica_nome));
					if($i<>'0') echo "<hr width='98%'>";
 					echo "<b>$fabrica_nome</b><br>";
				}
				$revenda_posto = trim(pg_result ($res,$i,revenda_posto));
				$posto_nome    = trim(pg_result ($res,$i,posto_nome));
				

				echo "&nbsp;<input type='radio' name='revenda_posto' value='$revenda_posto'>&nbsp;$posto_nome <br>";
			}
		}
	...
	echo "<td colspan='4'>";
		...
		echo "<td align='left' bgcolor='#BCCBE0' ><b>Destino</td>";
		echo "</tr>";
		echo "<tr class='Conteudo' valign='top'>";
		echo "<td align='left' valign='top'>";
		echo "&nbsp;<input type='radio' name='tipo' value='0'   id='C'> Reparos<br>";
	...
	echo "<table style=' border:#485989 1px solid; background-color: #e6eef7' align='center' width='100%' border='0' id='tbl_pecas'>";
	echo "<thead>";
	echo "<tr height='20' bgcolor='#BCCBE0'>";
...
<?
	...
	echo "<td align='left' ><input type='text' name='nota_fiscal' id='nota_fiscal' value='$nota_fiscal' maxlength='6' size='10'class='Caixa'></td>";
	echo "<td align='right' ><b>Data</b>&nbsp;</td>";
	echo "<td align='left' ><input type='text' name='data_nf' id='data_nf2' value='$data_nf' size='12' class='Caixa' ></td>";
	echo "</tr>";
	...
	$sql = "SELECT  tbl_posto.posto,
		...
		AND   tbl_revenda_posto.fabrica = 3
		AND   tbl_revenda_posto.revenda = $login_revenda
		ORDER BY nome;";
		$res = pg_exec ($con,$sql);
		...
		if(pg_numrows($res)>0){
			for ($i = 0 ; $i < pg_numrows($res); $i++){
				...
				echo "&nbsp;<input type='radio' name='posto' value='$posto'>&nbsp;$codigo_posto - $nome<br>";
			}
		}
