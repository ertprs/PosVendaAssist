...
<?if($login_fabrica != 42) { # HD 396332?>

<div class='noPrint' align='center'>
<input align='center' type=button name='fbBtPrint' value='Agrupar por OS'
onclick="window.location='os_extrato_detalhe_print.php?extrato=<? echo $extrato; ?>'">
<br>
<hr class='noPrint'>
</div>
...
if (pg_num_rows($res) == 0) $tipo_posto = "P"; else $tipo_posto = "D";
...
//--=== DADOS DO POSTO============================================================================================--\\
	...
	if (pg_num_rows($res2) > 0) {
		$codigo        = trim(pg_fetch_result($res2,0,codigo_posto));
		$posto         = trim(pg_fetch_result($res2,0,posto));
		$nome          = trim(pg_fetch_result($res2,0,nome));
		$endereco      = trim(pg_fetch_result($res2,0,endereco));
		$cidade        = trim(pg_fetch_result($res2,0,cidade));
		$estado        = trim(pg_fetch_result($res2,0,estado));
		$cep           = substr(pg_fetch_result($res2,0,cep),0,2) .".". substr(pg_fetch_result($res2,0,cep),2,3) ."-". substr(pg_fetch_result($res2,0,cep),5,3);
		$fone          = trim(pg_fetch_result($res2,0,fone));
		$fax           = trim(pg_fetch_result($res2,0,fax));
		$contato       = trim(pg_fetch_result($res2,0,contato));
		$email         = trim(pg_fetch_result($res2,0,email));
		$cnpj          = trim(pg_fetch_result($res2,0,cnpj));
		$ie            = trim(pg_fetch_result($res2,0,ie));
		$banco         = trim(pg_fetch_result($res2,0,banco));
		$agencia       = trim(pg_fetch_result($res2,0,agencia));
		$conta         = trim(pg_fetch_result($res2,0,conta));
		$data_extrato  = trim(pg_fetch_result($res2,0,data));
		$protocolo     = trim(pg_fetch_result($res2,0,protocolo));
...
?>
        <td style="text-align:center;">TAXA ADM.</td>
<?php
		}
	}
	echo "	</TR>\n";
	...
	for ($i = 0 ; $i < $totalRegistros; $i++){
		...
		$total_extrato   = pg_fetch_result ($res,0,total_extrato) ;
...
*/
			...
			if($login_fabrica == 20){
				...
				$res_produto = pg_query($con,$sql3);

				$produto_referencia = trim(pg_fetch_result ($res_produto,0,referencia));
				$produto_descricao  = trim(pg_fetch_result ($res_produto,0,descricao));
...
?>
                    <td style="text-align:right;padding-right:2px;" nowrap><?=number_format($taxa_administrativa,2,",",".")?></td>
<?php
                }
			}
	...
	if ($login_fabrica == 6){
        ...
        if ($login_fabrica == 42 && $prestacao_servico == 't') {
?>
        <td style="text-align:right;padding-right:2px;" nowrap><?=number_format ($total_taxa,2,",",".")?></td>
<?php
        }
	}
	echo "</tr>\n";
...
//--=== PEÇAS DA OS ============================================================--\\
	...
	if (pg_num_rows($res) > 0) {
		...
		for ($x = 0; $x < pg_num_rows($res); $x++) {
			...
			$peca       = trim(pg_fetch_result($res,$x,peca))      ;

			$valor_liquido  = ($login_fabrica == 20) ? number_format (pg_fetch_result($res,$x,preco) ,2,",",".") : number_format (pg_fetch_result($res,$x,custo_peca) ,2,",",".");

				$valor_peca = ($login_fabrica == 20) ? pg_fetch_result($res,$x,'preco'):pg_fetch_result($res,$x,'custo_peca');
				$valor_liquido_total += $valor_peca;

			$nome_peca        = trim(pg_fetch_result($res,$x,nome_peca));
			...
			if (pg_num_rows($res_idioma) >0) {
				$nome_peca  = trim(pg_fetch_result($res_idioma,0,descricao));
			}
			...
			echo "<font face='Verdana, Arial, Helvetica, sans' color='#000000' size='-2'>". trim(pg_fetch_result($res,$x,ref_peca)) ." - ". $nome_peca ."</font>\n";
			echo "</td>\n";
			...
			echo "<font face='Verdana, Arial, Helvetica, sans' color='#000000' size='-2'> ". number_format($valor_peca,2,",",".") ."</font>\n";
			echo "</td>\n";
	...
	if (pg_num_rows($res_avulso) > 0) {
		...
		for ($j = 0 ; $j < pg_num_rows($res_avulso) ; $j++) {
			...
			echo "<tr class='Conteudo' >\n";
			echo "<td class='Conteudo' width='45%'>" . pg_fetch_result($res_avulso, $j, descricao) . "&nbsp;</td>";
			echo "<td class='Conteudo' width='45%'>" . pg_fetch_result($res_avulso, $j, historico) . "&nbsp;</td>";
			echo "<td class='Conteudo' width='10%' align='right'> " . number_format( pg_fetch_result($res_avulso, $j, valor), 2, ',', '.') . "&nbsp;</td>";
			echo "</tr>";
		}
			echo "</table>\n";
	...
	if($login_fabrica==11 OR $login_fabrica==20 or $login_fabrica ==42){
		if (pg_num_rows($res_extrato) > 0) {
			...
			echo number_format( pg_fetch_result($res_extrato, 0, total), 2, ',', '.');
			echo "&nbsp;</B></td>\n";
			echo "</tr>\n";
			echo "</table>\n";
		}
