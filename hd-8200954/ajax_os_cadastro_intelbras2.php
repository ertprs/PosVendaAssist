...
if(strlen($produto_referencia)>0){
	...
	if(pg_numrows($resD) > 0) {
		for($i=0; $i<pg_numrows($resD); $i++) {
			...
			echo "<option value = '$defeito_constatado'>".$codigo." - ".$descricao."</option>";
		}
	}
	...
	$sql = "SELECT  tbl_produto.produto   ,
				...
				WHERE   tbl_subproduto.produto_pai in (select produto from tbl_produto where upper(referencia)=upper('$subconjunto') limit 1)
