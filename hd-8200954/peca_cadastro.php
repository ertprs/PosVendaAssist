...
$fabricas_com_imagens_pecas   = array(1,3,5,6,10,11,19,35,45,46,50,51,85,88,91,96,115,116,120,201,124,129,132,138,140,141,144,148,153,156,161,172,174,178);
...
$fabrica_peca_dev_obrigatoria = (isFabrica(1,3,6,15,20,24,35,40,45,46,50,52,59,72,80,81,85,86,90,91,94,98,99,104,114,115,117,120,201,121,122,125,127,129,131,134,138,139,140,141,142,144,145,146,147,151,152,153,154,157,160,162,169,170,171,183) or $replica_einhell);

$fabrica_usa_bloqueada_venda =  (isFabrica(1, 2, 35, 91, 151, 153, 160, 161, 162,163, 164, 165, 166,167,171,173,174,175,176,178,179,183,184,191,200,203) or $replica_einhell);

$label_class_fiscal = isFabrica(1,42) ? 'Código de Origem' : 'Classificação Fiscal';
...
if($excluir_peca == "sim"){
	...
	if (strlen($peca_item_foto)>0 AND strlen($peca)>0){
		...
		if (!unlink($caminho)){
			$msg_erro .= 'Imagem não pôde ser excluída.';
		}
...
if ($btnacao == "deletar" and strlen($peca) > 0 ) {
	...
	if (strlen($msg_erro) !== 0) {
		...
		$msg_erro = "Esta peça não pode ser excluída";
		$res = pg_exec ($con,"ROLLBACK TRANSACTION");
	}else{
		if (in_array($login_fabrica, array(158))) {
			...
			curl_close($ch);
			if (!$response) {
				$msg_erro = "Erro ao tentar deletar da aplicação mobile!";
				$res = pg_exec ($con,"ROLLBACK TRANSACTION");
			}else{
...
if ($btnacao == "gravar") {
	...
	$cla_fiscal = $_POST["classificacao_fiscal"];
		if (strlen($cla_fiscal) > 10) {
			$msg_erro .= "Classificação Fiscal não pode ser maior que 10 caracteres. <br />";
		}
	...
	if ($login_fabrica <> 20) {
		if (strlen($_POST["descricao"]) > 0) {
			...
			$msg_erro = "Digite a descrição da peça.";
		}
	} else {
		if (strlen($_POST['descricao']) > 0) {
			...
			$msg_erro = "Digite uma descrição português ou espanhol para o produto.";
		}
	}
	...
	if (strlen($_POST["referencia"]) > 0) {
		...
		$msg_erro = "Digite a referência da peça ";
	}
	...
	if (strlen($_POST["peso"]) > 0) {
		...
		if($login_fabrica == 177) {
			$msg_erro .= "Favor digitar o peso da peça";
		}else{
			$aux_peso = "null";
		}
	}
	...
	*/
	...
	if ($login_fabrica == 6) {
		...
		if($devolucao_obrigatoria == 't' and $devolucao_estoque_fabrica == 't'){
			$msg_erro .= "Deve ser selecionado apenas uma opção entre 'Devolução Obrigatória' e 'Devolução Estoque Fábrica' ";
		}
	}
	...
	if ($login_fabrica == 30) {
		if ($remessa_garantia_compressor=='t' and $remessa_garantia=='t') {
			$msg_erro .= "Por favor Selecione Remessa em Garantia ou Remessa em Garantia Compressor, a mesma peça não pode ser marcado nas duas opções.";
		}
	}
	...
	if($login_fabrica == 85){
		...
		if($aux_liquidacao != "null" && (strlen($_POST["preco_peca"]) == 0 || $preco == "0,00")){
			$liquidacao = 't';
			$msg_erro .= "Por favor insira o valor do Preço em Liquidação <br />";
		}else{
			$liquidacao = "";
		}
		...
		if($aux_liquidacao == "null" && $aux_at_shop == "'t'"){
			$msg_erro .= "Por favor insira a opção de Peça em Liquidação e o valor do Preço em Liquidação <br />";
		}
		...
		if(strlen($peca) > 0){
			...
			if (pg_num_rows($res_lb) > 0) {
				...
				if (strlen(pg_last_error()) > 0) {
					...
					$msg_erro .= 'Erro ao Alterar Item Revenda!<br>';
				}else{
					pg_query($con,'COMMIT');
				}
			}
	...
	if ($anexo_peca_os) {
		if ($anexo_os == 't') {
			...
			if ($qaxpos > $fabrica_qtde_anexo_peca_os) {
				$msg_erro .= "Quantidade de anexos por peça permitida .";
			}
    ...
    // echo "ANTES DO JSONo<br>" . $parametros_adicionais;
		...
		if(in_array($login_fabrica, array(30,151,158))){
			...
			if(strlen($familia_peca) > 0){ //hd_chamado=2543280
				...
				$msg_erro .= "Selecione a Família da peça";
			}
		}
	...
	foreach ( $aIdxFotos as $fotoSufixo ) {
		...
		if (!empty($_FILES[$idx]['tmp_name'])) {
			...
			if ( $ok ) {
				...
				$msg_erro .= "<p>Não foi possível fazer o upload da imagem da <em>Loja Virtual</em></p>";
			}
		}
	}
			...
			for ($i=0; $i<$qtde_de_fotos; $i++){
				...
				if(strlen($Nome)>0){
					...
					if(preg_match('/(jpeg|jpg|png)$/i', $Tipo)){
							...
							if(strtolower($ext)=="pdf"){
								if (file_exists($Caminho_foto)) { //Imagem anterior!
									if (!unlink($Caminho_foto)) {
										$msg_erro .= "Não foi possível excluir o arquivo $peca".".pdf!<br>\n";
									} else {
						...
						$msg_erro .= "O formato do arquivo ( $Nome ) inválidos !<br>São permitidas apenas imagens jpg e png";
					}
				}
			}
		}
...
#$msg_erro .= "ERRO";
		...
		if(in_array($login_fabrica, array(158))){
			...
			if (!$response) {
				$msg_erro = "Erro ao tentar incluir/alterar da aplicação mobile!";
			}
			$response = json_decode($response, TRUE);
		...
		$("input.item_revenda_produto").change(function (){
			...
			$.ajax({
				...
				complete: function(data){
					...
					console.log(data);
					if(data == "erro"){
						alert('Ocorreu um erro na atualização da etiqueta !');
					} else {
...
?>
        $("#cadastro_lb").click(function(e){
            ...
            Shadowbox.open({
                ...
                player: "iframe",
                title:      "Incluir Peça em Lista Básica",
                width:  800,
                height: 500
            });
        });
				...
				if(cont_click%2 == 0){
					...
					$.ajax({
						...
						complete: function(data){
							...
							data = data.responseText;
							if(data == "fail"){
								alert("Para que e peça seja mostrada na Loja Virtual e página inicial do Posto, favor inserir na Lista Básica de um Produto.");
							}
...
if ($login_fabrica == 186 && strlen($peca) > 0) {
  ...
  ?>
	...
	<? } ?>
	<tr><td colspan="100%" class="titulo_tabela" >Cadastro de Pe&ccedil;as</td></tr>
	<tr>
		<td width="50">&nbsp;</td>
		<?php if ($login_fabrica == 171) {//hd-3625122 - fputti?>
			<td>Referência FN</td>
		    <td>Referência Grohe</td>
		<?php } else { ?>
			<td>Referência *</td>
		<?php } ?>
		<?php if ($login_fabrica == 187) {?>
			<td>Referência MCASSAB</td>
		<?php } ?>
		<td>Descrição *</td>
		<td>Origem *</td>
	</tr>
	<tr>
		...
		<?php } ?>
		<td ><input style="font:11px Arial;font-weight: bold;" class='frm' type="text" name="descricao" value="<?=$descricao?>" size="<?php echo (in_array($login_fabrica, array(94)))?'58':'30'; ?>" maxlength="<?php echo (in_array($login_fabrica, array(1, 10)))?'150':'50'; ?>"><a href="javascript: pesquisaPeca (document.frm_peca.descricao,'descricao')"><img src="imagens/lupa.png" ></a></td>
		<td ><select name='origem' size='1' class='frm' style="font:11px Arial;font-weight: bold;">
		<? if($login_fabrica == 91){?>
			<option style="font:11px Arial;font-weight: bold;" value='NAC' <? if ($origem == 'NAC' OR $origem == 1) echo " selected " ?> > Fabricação </option>
			<option style="font:11px Arial;font-weight: bold;" value='IMP' <? if ($origem == 'IMP' OR $origem == 2) echo " selected " ?> > Terceiro Importado </option>
			<option style="font:11px Arial;font-weight: bold;" value='TER' <? if ($origem == 'TER') echo " selected " ?> > Terceiro Nacional</option></td>
		<? }else{ ?>
			<option style="font:11px Arial;font-weight: bold;" value='NAC' <? if ($origem == 'NAC' OR $origem == 1) echo " selected " ?> > Fabricação </option>
			<option style="font:11px Arial;font-weight: bold;" value='IMP' <? if ($origem == 'IMP' OR $origem == 2) echo " selected " ?> > Importado </option>
			<option style="font:11px Arial;font-weight: bold;" value='TER' <? if ($origem == 'TER') echo " selected " ?> > Terceiros </option>
			<? if($login_fabrica == 1){ ?>
					<option style="font:11px Arial;font-weight: bold;" value='FAB/SUB' <? if ($origem == 'FAB/SUB') echo " selected " ?> > Fabricação/Subsidiado </option>
					<option style="font:11px Arial;font-weight: bold;" value='IMP/SUB' <? if ($origem == 'IMP/SUB') echo " selected " ?> > Importado/Subsidiado </option>
					<option style="font:11px Arial;font-weight: bold;" value='TER/SUB' <? if ($origem == 'TER/SUB') echo " selected " ?> > Terceiros/Subsidiado </option>
					<option style="font:11px Arial;font-weight: bold;" value='FAB/SA' <? if ($origem == 'FAB/SA') echo " selected " ?> > Fabricação/Semi acabado </option>
					<option style="font:11px Arial;font-weight: bold;" value='IMP/SA' <? if ($origem == 'IMP/SA') echo " selected " ?> > Importado/Semi acabado </option>
				<? } ?>
			</select>
	...
	<?php
		$colspanEstoque = ($login_fabrica != 186) ? "colspan='2'" : "";
	?> 
	<tr>
		<td width="50">&nbsp;</td>
		<td>IPI <?php echo ($login_fabrica!=163)?"*":""; ?></td>
		<td $colspanEstoque>Estoque</td>
	...
	<tr>
		...
		?>
		<td>Múltiplo <?php echo $obrigatorio;?></td>
	</tr>
...
?>
	...
	if($login_fabrica == 101){ ?>
		<tr>
			<td width="50">&nbsp;</td>
			<td>Endereço/Localização no Depósito da Fábrica</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
		</tr>
		<tr>
	...
	if ($login_fabrica == 20) { ?>
		<tr>
			<td width="50">&nbsp;</td>
			<td width='100' height='15'>Família&nbsp;</td>
			<td >&nbsp;</td>
			<td >&nbsp;</td>
		</tr>
		<tr>
...
if ($login_fabrica == 2) {
	...
	<tr>
		<td width='50'>&nbsp;</td>
		<td>Mão de Obra Peça></td>
		<td>Mão de Obra Posto</td>
		<td >Mão de Obra Admin</td>
	</tr>
...
<table width='700px' align='center' border='0' cellpadding='2' cellspacing='1'  bgcolor='#D9E2EF' style="font:11px Arial">
	<tr><td>&nbsp;</td></tr>
    <tr class="subtitulo"><td align="center" colspan="<?=$outras_info_colspan ?>">Outras Informações</td></tr>
	<tr>
		<td valign="top" width="350">
			<table style="font-size:11px;">
				<tr>
					...
					<td align='left'><?=($login_fabrica == 1) ? "Garantia da peça" : "Garantia Diferenciada" ?> (meses)</td>
				<tr>

				<?php
				if ($fabrica_peca_dev_obrigatoria || isset($usaLGR)) {
				?>
				<tr>
					<td align='right'>
						<input type='checkbox' <? if ($devolucao_obrigatoria == 't' ) echo " checked " ?> name='devolucao_obrigatoria' value='t'>
					</td>
					<td align='left'>Devolução Obrigatória</td>
				</tr>
    			<?
    			}
				...
				<tr>
					...
					<td align='left'>Devolução Estoque Fábrica</td>
				</tr>
				<?
				}
				?>
				...
				<tr>
                    ...
                    <td align='left'>Peça não cadastrada na FN</td>
                </tr>
				<?php endif ?>

				<tr>
					<td align='right'>
						<acronym title="Esta informação trabalha em conjunto com a informação ITEM APARÊNCIA no cadastro de postos. Deixando setado como SIM, ela é bloqueada para ser lançada em Ordem de Serviço, e setando o posto autorizadao para ITEM APARÊNCIA, as Ordens de Serviço de Revenda irão permitir a digitação de Peças que sejam Itens de Aparência.">
						<input type='checkbox' <? if ($item_aparencia == 't' ) echo " checked " ?> name='item_aparencia' value='t'>
						</acronym>
					</td>
					<td align='left'><acronym title="Esta informação trabalha em conjunto com a informação ITEM APARÊNCIA no cadastro de postos. Deixando setado como SIM, ela é bloqueada para ser lançada em Ordem de Serviço, e setando o posto autorizadao para ITEM APARÊNCIA, as Ordens de Serviço de Revenda irão permitir a digitação de Peças que sejam Itens de Aparência.">Item de Aparência</acronym></td>
				</tr>
				...
				<tr>
					...
					<td align='left'>Acumula para Kit</td>
				</tr>
				...
				if (in_array($login_fabrica, [3,169,170])) { /* HD 35521 */
					if ($login_fabrica == 3) { ?>
						<tr>
							...
							<td align='left'>Peça sob Intervenção (<b>SAP<b>)</td>
						</tr>
						...
						<tr>
							...
							<td align='left'>Peça sob Intervenção (<b>Carteira</b>)</td>
						</tr>
					<?php } else { ?>
						<tr>
							<td align='right'>
							...
							<td align='left'>S- LCP (<b>SAP<b>)</td>
						</tr>
					<?php }
				}
				...
				<tr>
					...
					<td align='left'>
						<?
						echo ($login_fabrica==2 OR $login_fabrica==11 or $login_fabrica == 172) ? "Peça sob Intervenção" : "Retorna para Conserto";
						?>
					</td>
				</tr>
				<? } ?>
				...
				<tr>
					...
					<td align='left'>Mão-de-obra Diferenciada (TROCA)</td>
				</tr>
				<?}?>
				...
				<tr>
					...
					<td align='left'>
						<?if ($login_fabrica==3 OR $login_fabrica==11 or $login_fabrica == 172) {?>
							Bloqueada para Garantia
							</acronym>
						<?}else{?>
							Bloqueada para Garantia
						<?}?>
						<acronym class='ac' title="Se marcada,não irá deixar lançar a peça na OS"><img src='imagens/help.png'></acronym></p>
					</td>
				</tr>
				...
				<?php
                    ...
                    <?php
                        if ($fabrica_qtde_anexo_peca_os): ?>
                            <tr id="tr_qtde_anexo" <?=($anexo_os != 't')?' style="display:none"':''?>>
                                <td></td>
                                <td>
                                    <label for="qtde_fotos">Máx. Imagens por Peça na OS: </label>
                    ...
                    <tr>
                        <td align='right'>
                            <input type='checkbox' <? echo ($bloqueada_venda == 't' ) ? " checked " : "";?> name='bloqueada_venda' value='t'>
                        </td>
                        <td align='left'>Bloqueada para Venda</td>
                    </tr>
                    <?
                    }
                    if ($login_fabrica == 1) {?>
                    	<tr>
	                        ...
	                        <td align='left'>Item de Revenda</td>
	                    </tr>
                    <?php
                    }
                    ...
                    ?>
                        <tr>
                            ...
                            <td align='left'>Obrigatório PO-Peça</td>
                        </tr>
                    <?php
                    }
                    ?>
                    ...
                    <tr>
                        ...
                        <td align='left'>Controla Saldo</td>
                    </tr>
                    <? } ?>
            ...
            <td valign="top">
                <table  style="font-size:11px;">
                    ...
                    <tr>
                        ...
                        <td align='left'><?php echo ($login_fabrica == 153)? " Componente " : " Acessório " ?></td>
                    </tr>
                    <?
                    }
                    ...
                    ?>
                        ...
                        <td align='left'>Aguarda Inspeção</td>
                    </tr>
                    <?php
                    }
                    ?>
...
<?php
                            if (in_array($login_fabrica,array(11,172))) {
?>
                            <acronym title="Se marcada,a OS entrará em Intervenção de Suprimentos">Peça Crítica</acronym>
<?php
                            } else {
                                echo ($login_fabrica==35) ? 'Peça Crítica Garantia' : 'Peça Crítica';
                            }
?>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td align='left'>Peça Crítica Venda</td>
                    </tr>
                    <?} ?>
                    <tr>
                        <td align='right'>
                        ...
                        <td align='left'>Produto Acabado</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td align='left'>Mero Desgaste</td>
                    </tr>
                    <?php
			   }
			   // arr_login_fabrica_ativo da Black é separado
                    ...
                    <tr>
                        ...
                        <td align='left'><font color='#880000'>ATIVO</font></td>
                    </tr>
                    <?php }
                    ...
                    <tr>
                        ...
                        <td align='left'>Cobra Serviço</td>
                    </tr>
<?php
                    }
                    if (in_array($login_fabrica,array(11,172))) {
...
?>
                    <tr>
                        ...
                        if ($login_fabrica == 50) {
                        ?>
                            <td align='left'>Peça única na OS</td>
                        <?php
                        } else {
                        ?>
                            <td align='left'><nobr>Peça crítica</nobr><br><nobr>única na OS</nobr></td>
                        <?php
                        }
                        ?>
                    </tr>
...
?>
                    <tr>
                        ...
                        <td align='left'>Troca Obrigatória</td>
                    </tr>
                    <? } ?>
                    ...
                    <tr>
                        ...
                        <td align='left'>Gerar Troca de Produto</td>
                    </tr>
                    <? } ?>
                    ...
                    <tr>
                        ...
                        <td align='left'><font color='#880000'><? echo ($login_fabrica == 30) ? 'Status da Rede' : 'ATIVO'; ?> <?php echo $obrigatorio;?></font></td>
                    </tr>
                    <? } ?>
                    <?php if (in_array($login_fabrica,array(91))) { ?>
                        <tr>
                         ...
                         <td align='left'>Necessita Etiqueta</td>
                        </tr>
                    <? } ?>
                    ...
                    <tr>
                        ...
                        <td align='left'><font color='#880000'>Status uso Interno</font></td>
                    </tr>
                    <? } ?>
                    ...
                    <tr>
                        ...
                        <td align='left'>Faturar Manualmente</font></td>
                    </tr>
                    <? } ?>
                    ...
                    <tr>
                        ...
                        <td align='left'><?=(in_array($login_fabrica, array(169,170))) ? "Peça Faltante" : "Pre - Selecionada"?></td>
                    ...
                    <tr>
                        ...
                        <td><?= ($login_fabrica == 30) ? 'Remessa em Garantia Compressor' : 'Compressor'; ?></td>
                    </tr>
                    <?php
                    }
                    ?>
                    ...
                    <?php
	                    if( in_array($login_fabrica, array(167,175,203))){//HD-3428297
	                    	if ($login_fabrica == 175){
	                    		$label_serie = "Número de Série";
	                    	}else{
	                    		$label_serie = "Série da Peça";
	                    	}
	                ?>
                		<tr>
                            <td align='right'>
                    ...
                    <? if($login_fabrica==3){?>
                        <tr>
                            ...
                            <td align='left'>Nº Série Obrigatório (Informática)</td>
                        </tr>
                        <tr>
                            <td align='right'>
                            <input type='checkbox' <? if ($upload_fotos == 't' ) echo " checked " ?> name='upload_fotos' value='t' >
                            </td>
                            <td align='left'>Upload Fotos</td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td align='left'>Serial LCD Obrigatório</td>
                        </tr>
                    <?}?>
...
?>
    ...
    <tr bgcolor='#D9E2EF' style='font-size:12px'>
        <td colspan='13' align='center'>Comentários</td>
    </tr>
    <tr>
...
?>
        <tr>
            <td align="center" colspan="3">
                <button id="cadastro_lb" name="cadastro_lb">Modelo de Produtos</button>
            </td>
        </tr>
<?php
        }
?>
    ...
    <tr  bgcolor='#D9E2EF' >
        <td align='center' colspan='8'>Defeitos na peça</td>
    </tr>
    <tr  bgcolor='#FFFFFF'>
    <?
    ...
    <table width='700' align='center' border='0' cellpadding='2' cellspacing='1'  bgcolor='#F0F7FF'>
    <tr  bgcolor='#D9E2EF' >
        <td align='center' colspan='8'>Serviços na peça</td>
    </tr>
    <tr  bgcolor='#FFFFFF'>
    <?
    ...
    <?if(strlen($peca)>0 and in_array($login_fabrica, $fabricas_com_imagens_pecas)) {?>
        ...
        <? if(strlen($peca) > 0) { ?>
        <tr bgcolor='#D9E2EF' >
        <td align='center' colspan='2' class='subtitulo'>Imagem da peça - <?echo $referencia ;?></td>
        </tr>
        <? } ?>
	                    ...
	                    while (false !== ($filename = readdir($dh))) {
	                        ...
	                        if (strpos($filename,$peca) !== false){
	                            ...
	                            if(substr($filename, 0,$po)==$xpeca){
	                                ...
	                                <? }
	                                echo "<img src='../imagens/excluir_loja.gif' class='excluir' id='peca$peca' style='cursor:pointer' alt='Excluir Imagem' title='Excluir imagem' />";
	                            }
	                        }
	                    }
	                            ...
	                            while (false !== ($filename = readdir($dh))) {
	                                ...
	                                if (strpos($filename,$Xreferencia) !== false){
	                                    ...
	                                    ?>
	                                        ...
	                                        <img src='../<?echo $caminho_dir;?>/pequena/<?echo $filename; ?>' border='0'>
	                                        <input type='hidden' name='peca_imagem' value='<?echo $filename; ?>'>
	                                        <img src='../imagens/excluir_loja.gif' class='excluir' id='peca<?=$peca?>' style='cursor:pointer' alt='Excluir Imagem' title='Excluir imagem' />
	                                        </a>
	                        <?			}
	                                }
	                            }
    ...
    ?>
            ...
            if(strlen($peca) > 0 or $login_fabrica == 10){
                ...
                echo 'Selecione a imagem para a peça '.$referencia.' <input type="file" size="35" value="Procurar foto" name="arquivos[]" class="multi {accept:\'pdf|jpg|jpeg\', max:\''.$qtde_imagens.'\', STRING: {remove:\'Remover\',selected:\'Selecionado: '.$file.'\',denied:\'Tipo de arquivo inválido: '.$ext.'!\'}}" />';

                if ($login_fabrica<>10){
                    if ($login_fabrica == 6){

                        echo '<br /> * Formatos aceitos: PDF, jpg e JPEG <br /> * Se uma imagem estiver cadastrada ela será sobreposta';

                    }else{

                        echo '<br />* Se uma imagem estiver cadastrada ela será sobreposta';
        ...
        ?>
            ...
            <td bgcolor='#D9E2EF' align='left' >
                <p style="font:11px Arial;font-weight: bold;">Loja Virtual <acronym class='ac' title='Clique aqui para inserir na loja virtual'><img src='imagens/help.png'></acronym></p>
            </td>
            <td align='left'>
            ...
            <? if($login_fabrica==3){ //HD 102203 ?>
                ...
                <input type="checkbox" value="ok" name="at_shop" <?if($at_shop == "t") echo "CHECKED";?>>
                    Clique aqui para inserir na AT Shop.
                </td>
                </tr>
            <? } ?>
    ...
    <!-- 	<tr >
     ...
     -->
        ...
        <tr style="font: Arial 11px;">
            <td bgcolor='#D9E2EF'><p style="font:11px Arial;font-weight: bold;">Linha<acronym class='ac' title='Linha'><img src='imagens/help.png'></acronym></p></td>
            <td>
                <?
                $sql = "
                    SELECT  *
        ...
        <tr>
            <td bgcolor='#D9E2EF' align='left'  nowrap><p style="font:11px Arial;font-weight: bold;">Família&nbsp;<acronym class='ac' title='Família'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'>
            <?
        ...
        <tr>
        <td bgcolor='#D9E2EF' align='left'  nowrap><p style="font:11px Arial;font-weight: bold;">Marca&nbsp;<acronym class='ac' title='Marca'><img src='imagens/help.png'></acronym></p></td>
    ...
    ?>
        ...
        <? } ?>
        <tr>
            <td bgcolor='#D9E2EF'  align='left'  nowrap><p style="font:11px Arial;font-weight: bold;">Qtde máxima por posto&nbsp;<acronym class='ac' title='Quantidade máxima que pode ser pedido desta peça'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'><input NAME="qtde_max_site" size="20" maxlength='5' value='<?=$qtde_max_site?>' class='frm'>
            </td>
        </tr>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Qtde multipla por peça&nbsp;<acronym class='ac' title='Quantidade de venda por múltiplo. Ex: 3,6,9...'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'><input NAME="multiplo_site" size='20' maxlength='5' value='<?=$multiplo_site?>' class='frm'>
            </td>
        </tr>
        <?if($login_fabrica!=3 && $login_fabrica!=85){?>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Qtde Inicial Disponível&nbsp;<acronym class='ac' title='Quantidade incial disponivel para venda desta peça'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'><input NAME="qtde_disponivel_inicial_site" size='20' maxlength='5' value='<?=$qtde_disponivel_inicial_site?>' class='frm'>
            </td>
        </tr>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Data Inicial &nbsp;<acronym class='ac' title='Data incial em que a peça foi colocada em liquidação'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'>
        ...
        <?}?>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Qtde Disponível&nbsp;<acronym class='ac' title='Quantidade disponivel para venda desta peça'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'><input NAME="qtde_disponivel_site" size='20' maxlength='5' value='<?=$qtde_disponivel_site?>' class='frm'>
            </td>
        </tr>
        <? if($login_fabrica == 10) { ?>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Qtde Mínima no estoque&nbsp;<acronym class='ac' title='Quantidade mínima no estoque'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'><input NAME="qtde_minima_estoque" size='20' maxlength='5' value='<?=$qtde_minima_estoque?>' class='frm'>
            </td>
        </tr>
        <? } ?>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Prioridade no Site&nbsp;<acronym class='ac' title='Prioridade de exibição da peça na Loja Virtual. (1,2,3...1000)'><img src='imagens/help.png'></acronym></p></td>
        ...
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Peça em Liquidação&nbsp;<acronym class='ac' title='Peça será exibida no link de LIQUIDAÇÃO'><img src='imagens/help.png'></acronym></p></td>
        ...
        if($login_fabrica == 85){
            ...
            <tr id="box_preco" <? echo ($liquidacao == "t") ? "" : "style='display: none';";?>>
                <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Preço em Liquidação</p></td>
                <td><input style="font:11px Arial;font-weight: bold;" class='frm' type="text" name="preco_peca" value="<? echo $preco_peca; ?>" size="10" maxlength="20"></td>
            </tr>
            <?php
        }
        ...
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Não cobrar frete&nbsp;<acronym class='ac' title='Não cobrará o frete desta peça'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'> <input type="checkbox" value="ok" name="frete_gratis" <?if($frete_gratis == "t") echo "CHECKED";?>>
            </td>
        </tr>
        <tr>
            <td bgcolor='#D9E2EF'  align='left'><p style="font:11px Arial;font-weight: bold;"> Preço da Peça&nbsp;<acronym class='ac' title='Cadastra o preço da peça'><img src='imagens/help.png'></acronym></p></td>
            <td align='left'><input NAME="preco_peca" size='20' maxlength='10' value='<?=$preco_peca?>' class='frm'>
        </tr>
        <?PHP }?>
        <tr>
            <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Preço anterior&nbsp;<acronym class='ac' title='Preço será mostrado abaixo da peça como preço anterior'><img src='imagens/help.png'></acronym></p></td>
        ...
        if($login_fabrica == 85){
            ?>
            <tr>
                <td bgcolor='#D9E2EF'  align='left' ><p style="font:11px Arial;font-weight: bold;">Mostrar liquidação para o Posto</p></td>
        ...
        <?php
            if($login_fabrica != 3 && $login_fabrica !=  85 && $login_fabrica != 35){
                ...
                $aFoto     = array();
                $aFotoLabel['pq'] = 'Imagem Pequena';
                $aFotoLabel['g'] = 'Imagem Grande';
                $aFotoDescr['p'] = 'Imagem pequena do produto, é exibida na listagem de produtos, carrinho, mais produtos, etc ...';
                $aFotoDescr['g'] = 'Imagem do detalhe do produto. Irá aparecer na página de detalhes do produto, como primeira image selecionada';
        ...
        <tr>
            <td bgcolor="#D9E2EF" style="font-size: 10px" align="left">
                ...
                <?php echo ( isset($aFotoLabel[$_i]) ) ? $aFotoLabel[$_i] : 'Imagem Adicional '.$_i ; ?>
        ...
        <tr class="titulo_tabela_novo">
            <td colspan="2">Informações Referentes à Loja virtual</td>
        </tr>
        <tr>
    ...
    <div id="wrapper" style="background-color:#D9E2EF; width:700px;">
        ...
        <input type="button"  value="Gravar" onclick="javascript: if (document.frm_peca.btnacao.value == '' ) { document.frm_peca.btnacao.value='gravar' ; document.frm_peca.submit() } else { alert ('Aguarde submissão') } return false;" ALT="Gravar formulário" border='0' >
        <input type="button" value=" Apagar peça " onclick="javascript: if (document.frm_peca.btnacao.value == '' ) { document.frm_peca.btnacao.value='deletar' ; document.frm_peca.submit() } else { alert ('Aguarde submissão') } return false;" ALT="Apagar produto" border='0' >
    ...
    <div>
        <strong>ÚLTIMA ATUALIZAÇÃO: </strong><?echo $admin ." - ". $data_atualizacao;?>
         - <a rel='shadowbox' href='relatorio_log_alteracao_new.php?parametro=tbl_peca&id=<?php echo $peca; ?>' name="btnAuditorLog">Visualizar Log Auditor</a>
    </div>
    <? } ?>
    <? } ?>
    </P>
    ...
    if ($login_fabrica == 20){
        echo "<br>";
        echo "<a href='peca_tipo_fleg.php?peca=$peca'>GERAÇÃO DE LISTAGEM</a>\n<br>\n<br>\n";
    }
    ...
    if (strlen($peca) > 0) {
        ...
        if (pg_numrows($res) > 0) {
            ...
            echo "<td $coluna>";
            echo "Equipamentos que Possuem esta Peça";
            echo "</td>";
    ...
    if ($listartudo == 1){
        ...
        if(pg_numrows($res) >0) {
            ...
            if($login_fabrica == 45){
                ...
                echo "<p id='id_download2'><a href='xls/$fileName'><img src='imagens/excel.png' height='20px' width='20px' align='absmiddle'>&nbsp;&nbsp;&nbsp;Download Excel</a></p><br />";
                $data = date("d-m-Y-H:i");
                $head = "<table>
                         <tr>
                            <td>Referência</td>
		...
		echo "<p id='id_download2'><a href='xls/$fileName'><img src='imagens/excel.png' height='20px' width='20px' align='absmiddle'>&nbsp;&nbsp;&nbsp;Download CSV</a></p><br />";
	    	...
	    	if(in_array($login_fabrica, array(104,140))){
            	...
            	echo "</table>\n<div id='wrapTable' style='width:716px;padding-left:16px;height:70%;overflow-x:hidden;overflow-y:auto;min-height:200px;margin-bottom:1em;'>\n";
                    ...
                    echo "<td align='left' width='150'>Referência</td>";
                    echo ($login_fabrica == 101) ? "<td align='left'>Endereçamento</td>" : "";
                    echo "<td align='left'>Descrição</td>";
                    echo ($login_fabrica == 104) ? "<td align='left'>Produtos</td>" : "";
                    
                    
                    echo ( in_array($login_fabrica,array(20)) ) ? "<td align='left'>Família</td>" : "";

                    //hd-3625122 - fputti
                    echo (in_array($login_fabrica,array(171))) ? "<td align='center'>Referência Fábrica</td>" : "";

                    echo ($login_fabrica == 104) ? "<td>Qtde Demanda</td>" : "";
                    
                    echo (in_array($login_fabrica, $arr_login_fabrica_ativo) or $login_fabrica > 86) ? "<td align='left'>Status</td>" : "";
                    echo isFabrica(104,140) ? '<td align="center">Devolução Obrigatória</td><td align="center">Item de Aparência</td><td align="center">Bloqueada Garantia</td><td align="center">Acessórios</td><td align="center">Peça Crítica</td><td align="center">Produto Acabado</td>' : '';
                    echo isFabrica(42) ? '<td align="center">Disponibilidade</td>' : '';
                    echo isFabrica(42) ? '<td align="center">Previsão Chegada</td>' : '';
                    echo ($login_fabrica == 20) ? "<td align='left'>Tipo</td>" : "";
		    ...
		    $disponibilidade = ($disponibilidade == "I") ? "Indisponível" : "Disponível";
                ...
                if($login_fabrica == 20){
                    ...
                    echo "<td align='left'>";
                    echo ($acessorio=='t') ? "Acessório" : (($acessorio=='f') ? "Peça" : " - ");
                    echo "</td>";
                }
                echo "</tr>";
                ...
                if(in_array($login_fabrica,array(45,104,140))){
                    ...
                    $bloqueada_garantia   = (pg_fetch_result($res,$i,"bloqueada_garantia") == "t") ? "Sim" : "Não";
                    if(in_array($login_fabrica, array(104,140))){
                    	$devolucao_obrigatoria   = (pg_fetch_result($res,$i,"devolucao_obrigatoria") == "t") ? "Sim" : "Não";
                    	$item_aparencia 		 = (pg_fetch_result($res,$i,"item_aparencia") == "t") ? "Sim" : "Não";
                    	$peca_critica 			 = (pg_fetch_result($res,$i,"peca_critica") == "t") ? "Sim" : "Não";
                    	$produto_acabado 		 = (pg_fetch_result($res,$i,"produto_acabado") == "t") ? "Sim" : "Não";
                    	$acessorio  			 = (pg_fetch_result($res,$i,"acessorio") == "t") ? "Sim" : "Não";                    	

                    }
		    		$peca_critica         = (pg_fetch_result($res,$i,"peca_critica") == "t") ? "Sim" : "Não";
		   			$status		          = (pg_fetch_result($res,$i,"ativo") == "t") ? "Ativo" : "Inativo";
    ...
    <div id="wrapper" style="width:700px;">
        <input type="button" onClick="window.open('peca_consulta.php')" value="Consultar Peças por Tipo" >
    ...
    if ($login_fabrica<>11 and $login_fabrica <> 172 and !$listartudo){ // HD 56689?>
        ...
        <input type="button" onClick="location.href='<?echo $PHP_SELF;?>?listartudo=1'" value="Listar todas as Peças">
    ...
    if ($login_fabrica == 1) {?>
        ...
        <table width='600' border='0' cellspacing='2' cellpadding='2' align='center'>
        	<tr>
        		...
        		<a href="peca_cadastro.php?gerar_excel=true" target="_blank" style="text-decoration: none; "><img src="imagens/	document-txt-blue-new.png" height="20px" width="20px" align="absmiddle">&nbsp;&nbsp;Gerar Arquivo Peças Item de Revenda</a>
        		</td>
        </tr>
        </table>
    <?php
