...
if (strlen($btn_acao) > 0 AND $btn_acao=="digitou_as_notas") {
	...
	for($i=0;$i<$numero_de_notas;$i++){
		...
		$natureza = "Devolução de Garantia ";
		if ($login_fabrica == 50){
		...
		$sql = "INSERT INTO tbl_faturamento
				...
				VALUES ($login_fabrica,'$data_preenchimento','$data_preenchimento',$posto_da_fabrica,$login_posto,'$cfop',$total_nota,'$nota_fiscal','2','$natureza', $base_icms, $valor_icms, $base_ipi, $valor_ipi, $extrato, 'Devolução de peças do posto para à Fábrica','$movimento')";
		$res = pg_exec ($con,$sql);
		...
		for($x=1;$x<=$qtde_peca_na_nota;$x++){
			...
			if (pg_numrows ($resNF)==0 ){
				...
				$corpo.="MENSAGEM AUTOMÁTICA. extrato_posto_lgr.php(posto) NÃO RESPONDA A ESTE E-MAIL \n\n $msg_erro \n $sql_nf";
...
//					echo "<input type='hidden' name='id_nota_$numero_nota-linha' value='$linha'>\n";
					...
					echo "<br><IMG SRC='imagens/setona_h.gif' WIDTH='53' HEIGHT='29' BORDER='0' align='absmiddle'>Número da sua Nota Fiscal: <input type='text' name='nota_fiscal_$numero_nota' size='10' maxlength='7' value='$nota_fiscal'>";
					echo "<br><br>";
					echo "</td>";
					echo "</tr>";
					echo "</table>";
			...
			if (pg_numrows($resConf)>0){
				...
				if (pg_numrows($resConf)>0 OR $qtde_devolucao == 0){
					...
					if (count($notas_array)==1){
						...
						$msg_notas = "O Fabricante não acusou o recebimento das notas N° ".implode(",",$notas_array).". Favor entrar em contato c/ a Taiz TEL: 071 3379-1997, P/ posterior liberação da M.O.";
					}
				}else{
