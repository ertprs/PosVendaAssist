...
class Imap {
    ...
    public function getMessages($tipo = 'UNSEEN', $texto = 'text') {
        $this->attachments_dir = rtrim($this->attachments_dir, '/');
        $stream = $this->imapStream;
        $emails = imap_search($stream, $tipo);
        $messages = array();
        $chaveNew = 0;
        ...
        if ($emails) {
            ...
            foreach ($emails as $email_number) {
                ...
                $messages[$chaveNew]      = $this->loadMessage($uid, $texto);
                ...
                $messages[$chaveNew]["subject"]      = $assunto_decodificado[0]->text;
                $messages[$chaveNew]["from"]["name"] = $remetente_decodificado[0]->text;                
                $chaveNew++;
            }
        }
		return array(
    ...
    public function setEmailNaoLido($uid) {
        imap_setflag_full($this->imapStream, $uid, "\\UnSeen", ST_UID);
    }

    public function setEmailLido($uid) {
        imap_setflag_full($this->imapStream, $uid, "\\Seen", ST_UID);
    }
