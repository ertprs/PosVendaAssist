...
if($login_fabrica == 145){
	$sql_campo = ", tbl_os.tipo_atendimento ";
}
if($login_fabrica == 74 ){
...
$sql = "SELECT  tbl_os.sua_os                                                   ,
				...
				tbl_os_extra.mao_de_obra_desconto                AS mao_de_obra_desconto,
...
if ($totalRegistros == 0){
	...
	if ($login_fabrica == 15){
		echo "<IMG SRC=\"admin/logos/latina.jpg\" width='120' height='40' ALT='EXTRATO' align=left >";
	}else{

		if($login_fabrica == 20){
			echo "<IMG SRC=\"logos/cabecalho_print_$login_fabrica_nome.jpg\" width='90' ALT='EXTRATO' align=left >";
		}else{
			if(file_exists("logos/cabecalho_print_$login_fabrica_nome.gif")){
				echo "<IMG SRC=\"logos/cabecalho_print_$login_fabrica_nome.gif\" width='120' height='40' ALT='EXTRATO' align=left >";
			}
		}
	}
	echo "	<BR><b>$title<br>";
...
//--=== DADOS DO POSTO============================================================================================--\\
	...
	if (pg_numrows($res2) > 0) {
		...
		$numero       = trim(pg_result($res2,0,'numero'));
...
//--=== OS DENTRO DO EXTRATO =====================================================================================--\\
	...
	if($login_fabrica == 145){
		echo "<TD align='center' width='35%'>PRODUTO</TD>";
	}
    ...
    } else if (in_array($login_fabrica, array(50,52,85)) || ($login_fabrica == 24 && $login_tipo_posto == 256)){
		echo "<td align='center' colspan=2>MO</td>\n";
		echo "<td align='center' colspan=2>KM</td>\n";
		if ($login_fabrica == 85 || $login_fabrica == 52){
            if ($login_fabrica == 85) {
                echo "<td align='center' colspan=2>BONIFICACAO</td>\n";
            }
			echo "<td align='center' colspan=2>PEDÁGIO</td>\n";
		}
	}elseif ($login_fabrica == 90){
                echo "<td align='center' colspan=2>MO</td>\n";
		echo "<td align='center' colspan=2>PEÇAS</td>\n";
                echo "<td align='center' colspan=2>KM</td>\n";
                echo "<td align='center' colspan=2>VISITA</td>\n";
	} else if (in_array($login_fabrica, array(169,170))) { ?>
		<td align="center" colspan="2"><?= traduz('Valor OS SAP') ?></td>
	<? } else if (in_array($login_fabrica, array(15,74)) || isset($novaTelaOs)) {
		if (isset($novaTelaOs)) {
		...
		if (!isset($novaTelaOs) || (isset($novaTelaOs) && !$nao_calcula_peca)) {
			echo "<td align='center' colspan=2>".traduz('PEÇAS')."</td>\n";
		}
		...
		echo "</td>\n";
		if(!in_array($login_fabrica,array(104,105,106,108,111,121,123,125)) &&  !$telecontrol_distrib) {
			echo "<td align='center' colspan=2>";
	...
	if(($inf_valores_adicionais || $login_fabrica == 145) && !in_array($login_fabrica, array(169,170))) {
		echo "<TD align='center' width='35%'>".traduz('VALORES ADICIONAL')."</TD>";
	}

	if ($login_fabrica == 145) {
		echo "<TD align='center' width='35%'>".traduz('TOTAL')."</TD>";
		echo "<TD align='center' width='35%'>".traduz('TIPO ATENDIMENTO')."</TD>";
	}
	echo "	</TR>\n";
	...
	for ($i = 0 ; $i < $totalRegistros; $i++){
		...
		$valor_adicional = pg_result ($res,0,'valor_adicional') ;
		$obs             = pg_result ($res,0,obs) ;
		$visita          = trim(pg_result ($res,$i,extra_instalacao));
		...
		if(in_array($login_fabrica,[40,90,120,201])) {
			$pecas = 0;
		}
		...
		if ($login_fabrica == 6){
			...
			$total_mao_de_obra += $valor_total_hora_tecnica;
		} else {
			//if ($tipo_posto == "P") {
			//}else{
...
*/
		...
		if($login_fabrica == 20 or $login_fabrica == 145){
		...
		if($login_fabrica == 145){
			echo "<TD class='Conteudo' align='left' nowrap>$consumidor_str &nbsp;</TD>\n";
			echo "<TD class='Conteudo' align='left' nowrap>$produto_descricao &nbsp;</TD>\n";
		}
/*
		if ($tipo_posto == "P") {
		...
		*/
		...
		if ($login_fabrica == 145) {
			if ($tipo_atendimento > 0) {
				...
				$tipo_atendimento_descricao = pg_fetch_result($res2,0,descricao);
			}
		}
		...
		if ($login_fabrica == 6){
		...
		}elseif (in_array($login_fabrica, array(50,52,85)) || ($login_fabrica == 24 && $login_tipo_posto == 256)){
			...
			if (in_array($login_fabrica, array(52, 85))){
                if ($login_fabrica == 85) {
                    ...
                    echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($valor_adicional,2,",",".") . "</td>\n";

                }
				echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($pedagio,2,",",".") . "</td>\n";
			}
		}elseif ($login_fabrica == 90){
                         echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($mao_de_obra,2,",",".") . "</td>\n";
                         echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($pecas,2,",",".") . "</td>\n";
			 echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($qtde_km_calculada,2,",",".") . "</td>\n";
   			 echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($visita,2,",",".") . "</td>\n";
			...
			<td class="Conteudo" colspan="2" align="right" style="padding-right:2px;"><?= number_format($valor_total_hora_tecnica, 2, ",", "."); ?></td>
		<? } else if (isset($novaTelaOs)) {
            $nao_calcula_km = \Posvenda\Regras::get("nao_calcula_km", "mao_de_obra", $login_fabrica);
            $nao_calcula_peca = \Posvenda\Regras::get("nao_calcula_peca", "mao_de_obra", $login_fabrica);

			echo "<td class='Conteudo' colspan=2 align='center' style='padding-right:2px'>  " . number_format ($mao_de_obra,2,",",".") . "</td>\n";
			...
			if (isset($fabrica_usa_valor_adicional) || in_array($login_fabrica, array(145))) {
				$total_valor_adicional += $valores_adicionais;
				echo "<td class='Conteudo' align='right' style='padding-right:2px'>  " . number_format ($valores_adicionais,2,",",".") . "</td>\n";
			}

			if($login_fabrica == 145){
				$total_fabrimar = $qtde_km_calculada + $valor_adicional + $pecas_posto + $mao_de_obra;
				echo "<td class='Conteudo' align='right' style='padding-right:2px'> " . number_format ($total_fabrimar,2,",",".") . "</td>\n";
				echo "<td class='Conteudo' align='right' style='padding-right:2px'> $tipo_atendimento_descricao </td>\n";
			}
		} else {
			//if ($tipo_posto == "P") {
				echo "<td class='Conteudo' colspan=2 align='center' style='padding-right:2px'>  " . number_format ($mao_de_obra,2,",",".") . "</td>\n";
			...
			//}
			if(!in_array($login_fabrica,array(104,105,106,108,111,121,123,125)) and !$telecontrol_distrib) {
				echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($pecas_posto,2,",",".") . "</td>\n";
				echo "<td class='Conteudo' colspan=2 align='right' style='padding-right:2px'>  " . number_format ($qtde_km_calculada,2,",",".") . "</td>\n";
			}
		}
		...
		echo "	</TR>\n";
        if ($login_fabrica == 52 && strlen($obs_adicionais) > 0){
?>
            <tr>
                <td colspan="7" class='Conteudo' align='left' style='padding-right:2px'>
                    <b>SOBRE KM: </b><?=$obs_adicionais?>
                </td>
            </tr>
<?php
        }
	}
	if(!in_array($login_fabrica,array(24,104,105,106,108,111,121,123,125)) and !$telecontrol_distrib) {
		...
		if ($login_fabrica == 6){
			...
			if (in_array($login_fabrica, array(52, 85))){
                if ($login_fabrica == 85) {
                    ...
                    echo "<td colspan=2 align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_bonificacao,2,",",".") . "</b></td>\n";
                }
				echo "<td colspan=2 align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_pedagio,2,",",".") . "</b></td>\n";
			}
 		}elseif ($login_fabrica == 90){
                         echo "<td colspan=2 align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_mao_de_obra,2,",",".") . "</b></td>\n";
                         echo "<td colspan=2 align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_pecas,2,",",".") . "</b></td>\n";
			 echo "<td colspan=2 align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_km,2,",",".") . "</b></td>\n";
			 echo "<td colspan=2 align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_visita,2,",",".") . "</b></td>\n";
		} else if (in_array($login_fabrica, array(169,170))) { ?>
			<td colspan="2" nowrap></td>
		<? }elseif (in_array($login_fabrica, array(15,74)) || isset($novaTelaOs)){
			if (isset($novaTelaOs)) {
                $nao_calcula_km = \Posvenda\Regras::get("nao_calcula_km", "mao_de_obra", $login_fabrica);
                $nao_calcula_peca = \Posvenda\Regras::get("nao_calcula_peca", "mao_de_obra", $login_fabrica);
            }
		...
		if ((isset($fabrica_usa_valor_adicional) || in_array($login_fabrica, array(145))) && !in_array($login_fabrica, array(169,170))) {
			echo "<td colspan='1'align='right' bgcolor='$cor' style='padding-right:2px' nowrap><b>  " . number_format ($total_valor_adicional,2,",",".") . "</b></td>\n";
		}
	}
	echo "</tr>\n";
	...
	if ($login_fabrica == 19) {
		...
		if ($login_fabrica == 6){
			if (strlen($liberado>0)){
				...
				echo "</b></td>\n";
				echo "<td colspan=\"3\" bgcolor='$cor' align='center'><b>  " . number_format ($total_mao_de_obra + $total_mao_de_obra_revenda + $total_pecas + $total_pecas_revenda,2,",",".") . "</b></td>\n";
			}
		}else{
			echo "<tr class='Conteudo'>\n";
			echo "<td colspan=\"2\" align=\"center\" style='padding-right:2px'><b>";
			if (in_array($login_fabrica,array(104,105,106,108,111,121,123,125)) || $telecontrol_distrib || isset($novaTelaOs)) {
				echo 'TOTAL';
			} else if($login_fabrica == 74) {
				echo "TOTAL (MO + PEÇAS + KM)";
			} else if($login_fabrica == 90) {
				echo "TOTAL (MO + PEÇAS + KM + VISITA)";
            } else if (in_array($login_fabrica, array(15,50))) {
				echo "TOTAL (MO + KM)";
			} else {
				if ($login_fabrica == 52){
					if($sistema_lingua == "ES") echo "TOTAL (MO + KM + PEAJE)";
					else                        echo "TOTAL (MO + KM + PEDÁGIO)";
				} else if ($login_fabrica == 85){
					if($sistema_lingua == "ES") echo "TOTAL (MO + BONIFICACIÓN + KM + PEAJE)";
					else                        echo "TOTAL (MO + BONIFICAÇÃO + KM + PEDÁGIO)";

				} else {
					if($sistema_lingua == "ES") echo "TOTAL (MO + PIEZAS + KM)";
					else                        echo "TOTAL (MO + Peças + KM)";
				}
			}
			...
			if($login_fabrica == 74){
				...
				echo "<td colspan='2'></td>";
                                ...
                                echo "</b></td>\n";
                                echo "<td colspan=\"7\" bgcolor='$cor' align='center'><b>  " . number_format ($total_mao_de_obra + $total_pecas + $total_km + $total_visita,2,",",".") . "</b></td>\n";
			}
			else if (in_array($login_fabrica, array(15, 50))){
				echo "</b></td>\n";
				echo "<td colspan=\"5\" bgcolor='$cor' align='center'><b>  " . number_format ($total_mao_de_obra + $total_mao_de_obra_revenda + $total_km,2,",",".") . "</b></td>\n";
			}else{
				echo "</b></td>\n";
				if ($login_fabrica == 52){
					echo "<td colspan=\"5\" bgcolor='$cor' align='center'><b>  " . number_format ($total_mao_de_obra + $total_mao_de_obra_revenda + $total_pecas + $total_pecas_revenda +$total_km+$total_pedagio,2,",",".") . "</b></td>\n";
				} else if ($login_fabrica == 85) {
                    echo "<td colspan=\"8\" bgcolor='$cor' align='center'><b>  " . number_format ($total_mao_de_obra + $total_mao_de_obra_revenda + $total_pecas + $total_pecas_revenda + $total_bonificacao + $total_km+$total_pedagio,2,",",".") . "</b></td>\n";
				} else if (in_array($login_fabrica, array(169,170))) { ?>
					<td colspan="8" bgcolor="<?= $cor; ?>" align="center"><b><?= number_format($total_mao_de_obra, 2, ',', '.'); ?></b></td>
				<? } else {
					...
					echo "<td colspan=\"$colspan\" bgcolor='$cor' align='center'><b>  " . number_format ($total_mao_de_obra + $total_mao_de_obra_revenda + $total_pecas + $total_pecas_revenda +$total_km + $valor_adicional,2,",",".") . "</b></td>\n";
				}
			}
		}
	}
...
##### LANÇAMENTO DE EXTRATO AVULSO - FIM #####
	...
	$valor_adicional = pg_fetch_result($res_extrato,0,'valor_adicional');
	if(empty($valor_adicional)) $valor_adicional = 0 ;

	if(in_array($login_fabrica, array(11,20,43,85))){ // HD 118073
	...
	if($login_fabrica==2){
		...
		$res_os = pg_exec($con,$sql5);
		if(pg_numrows($res_os) > 0){
		echo "<br><br>";
		echo "<table width='650' align='center' border='1' cellspacing='0' cellpadding='1' style='border-collapse: collapse' bordercolor='#000000'>\n";
		echo "<tr class='Titulo'>\n";
		echo "<td class='Conteudo' nowrap colspan='6'><B>Ordens de Serviço em Aberto<B></td>\n";
		echo "</tr>\n";
		echo "<tr class='Conteudo2'>\n";
		echo "<td class='Conteudo2' nowrap nowrap><B>OS</B></td>\n";
		echo "<td class='Conteudo2' align='center' nowrap><B>Data Abertura</B></td>\n";
		echo "<td class='Conteudo2' align='center' nowrap><B>Nome Consumidor</B></td>\n";
		echo "<td class='Conteudo2' align='center' nowrap><B>Referência</B></td>\n";
		echo "<td class='Conteudo2' align='center' nowrap><B>Descrição Produto</B></td>\n";
		echo "<td class='Conteudo2' align='center' width='300' nowrap><B>Motivo</B></td>\n";
		echo "</tr>\n";


		for($i=0;$i<pg_numrows($res_os);$i++){
			$os				 = trim(pg_result ($res_os,$i,os));
			$consumidor_nome = strtoupper(trim(pg_result ($res_os,$i,consumidor_nome)));
			$data_abertura   = trim (pg_result ($res_os,$i,data_abertura));
			$data_fechamento = trim (pg_result ($res_os,$i,data_fechamento));

			$sql = "SELECT referencia,descricao FROM tbl_produto JOIN tbl_os ON tbl_os.produto = tbl_produto.produto WHERE tbl_os.os = $os";
				...
				echo "</tr>\n";
				echo "<tr class='Conteudo2'>\n";
				echo "<td class='Conteudo2' nowrap nowrap>$os</td>\n";
				echo "<td class='Conteudo2' align='center' nowrap>$data_abertura</td>\n";
				echo "<td class='Conteudo2' align='center' nowrap>$consumidor_nome</td>\n";
				echo "<td class='Conteudo2' align='center' nowrap>$produto_referencia</td>\n";
				echo "<td class='Conteudo2' align='center' nowrap>$produto_descricao</td>\n";
				echo "<td class='Conteudo2' align='center' height='80' nowrap>&nbsp;</td>\n";
				echo "</tr>\n";

		}
		$total = pg_numrows($res_os);
		echo "<tr class='Conteudo'>\n";
		echo "<td colspan='5' align='right' style='font-size:12px;'><b>Total</b></td>";
		echo "<td align='center' style='font-size:12px;'><b>$total</b></td>";
		echo "</tr>\n";
		echo "</table>";
		}
	}
	#### OS ABERTAS DYNACOM - FIM ######
}
...
<TR>
	<TD height='20' class="Conteudo" colspan='4'><?= traduz('PAGAMENTO') ?></TD>
</TR>
<TR>
	<TD align='left' class="Conteudo" width='20%'><?= traduz('EXTRATO PAGO EM:') ?></TD>
	<TD class="Conteudo" width='15%'><? echo $baixado; ?></TD>
	<TD align='left' class="Conteudo" width='15%'><center><?= traduz('OBSERVAÇÃO') ?>:</center></TD>
