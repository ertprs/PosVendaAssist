		...
		if ($tipo_busca=="produto"){
			...
			if (pg_numrows ($res) > 0) {
				for ($i=0; $i<pg_numrows ($res); $i++ ){
					...
					if (((strlen($login_unico)>0 OR strlen($login_simples)>0) AND $login_fabrica==10) OR ($login_fabrica==3 && pg_numrows ($res3) == 0)){
						$sql2 = "SELECT preco
								FROM tbl_tabela_item
								WHERE peca  = $peca
								AND   tabela IN (
		...
		for($w=0;$w<$qtde_linha;$w++){
			...
			if(($liquidacao=="NAO" AND strlen($peca)>0) OR ($liquidacao == "SIM" and strlen($qtde)>0 AND strlen($peca)>0)){
				...
				if($at_shop == 't'){
					...
					if (((strlen($login_unico)>0 OR strlen($login_simples)>0) AND $login_fabrica==10 ) OR $login_fabrica==1 OR ($login_fabrica==3 && pg_numrows ($res) == 0)){
				...
				if ( $login_fabrica==3 AND strlen($linha)>0){
...
if($at_shop=="t" AND $login_fabrica==3){
