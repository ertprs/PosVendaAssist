...
<form name='frm_cadastro' method='post'>
	<div id='laudo' style='width:700px;'>
		<table align='center' width='100%' id='logos' cellpadding="0" cellspacing="0">
			<tr>
				<td align='center'><img src="imagens_admin/dewalt_logo.gif" width='140'></td>
				<td align='center'>
		...
		<table align='center' width='100%'>
			...
			<?php if(empty($laudo)){ ?>
					...
					<tr>
						<td colspan='2'>
							<input type='radio' name='motivo_troca' <?php echo ($disabled == true)? " disabled ": " " ; ?> value='fabrica' onclick='javascript: informaPeca();' <?php echo ($motivo_troca == 'fabrica') ? 'checked' : '';?> > Falta de peça na fábrica
						</td>
						<?php if($login_fabrica == 1){?>
							<td colspan='2'>
			...
			<?php }else{ ?>
					...
					<?php if(strlen($obs) > 0 ){?>
					<tr>
						<td colspan=2><b><?php echo ($titulo == 'Outros')? "Observação": "Pecas"; ?>:</b><br>
			...
			<tr id='linha_peca'>
				<td colspan="4">
					...
					<?php } else if(!empty($laudo) and ($motivo_troca == 'autorizada' or $motivo_troca == 'fabrica')) {
								...
								echo "<tr><td>Peças que faltaram</td></tr>";
		...
		<table align='center' width='100%'>
			...
			<tr>
				<td colspan='2'>
					Análise feita por: <hr style='width:300px;margin-left:100px; border:solid 1px; color:#000;margin-top:-5px;'>
				</td>
			</tr>
			...
			<tr>
				<td width='500'>
					<br />
					Assinatura: <hr style='width:330px;margin-left:70px; border:solid 1px; color:#000;margin-top:-5px;'>
				</td>
				<td>
					Carimbo:
				</td>
