		...
		if (strlen($qtde_linhas)>0 AND strlen($acao)>0 ){
			...
			for ($i=0 ; $i < $qtde_linhas ; $i++){
				...
				if ($acao == "nao_recebida"){
					...
					$res = pg_exec($con,$sql);
					if (pg_numrows($res)>0){
						$posto       = pg_result($res,0,distribuidor); # distribuidor é o posto
...
if ($verPopup == "sim"){
	...
	if ($btn_acao=="gravar_conferencia"){
		...
		if ($qtde_faltante == 0 OR strlen($qtde_faltante)==0){
			...
			$sql = "INSERT INTO tbl_comunicado (
						...
						'O Fabricante acusou o recebimento parcial de sua NF. n° $nota_fiscal, favor entrar em contato urgente c/ o Sr. Alberto TEL:071 3379-1997, para sua regularização',
						'LGR',
						$login_fabrica,
						'f',
						't',
	...
	if(strlen($cancelada)==0){
		echo "<tr>\n";
		echo "<td><h4><center>
		<input type='button' name='gravar' value='Gravar Conferência' onClick=\"javascript:
...
<?
	...
	if ($btnacao=='filtrar' && (strlen($filtroRecebidos)>0 || strlen($filtroTodos)>0 || strlen($posto_codigo)>0  || strlen($nota_devolucao)>0)){
		...
		$lista .=  "<tr ><td class='Titulo' colspan='8'>Notas Fiscais de Devolução</td></tr>";
