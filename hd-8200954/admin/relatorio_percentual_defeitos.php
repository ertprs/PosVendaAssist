...
$title 		 = traduz("RELATÓRIO DE PERCENTUAL DOS DEFEITOS POR PRODUTOS");
	...
	if ($login_fabrica == 117) {
		...
		if (pg_num_rows($res) > 0){
	        	...
	        	$head .=    "<thead>
	        					<tr>
	        						<th bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;' colspan='{$colspan}' >
	        							". traduz("RELATÃ“RIO DE PERCENTUAL DOS DEFEITOS POR PRODUTOS") . " - $mes_inicial/$ano_inicial Ã  $mes_final/$ano_final
	        						</th>
	        					</tr>
	        					<tr>
			...
			for ($i=0; $i<pg_num_rows($res); $i++){
			 ...
			 $head .= "<th bgcolor='#596D9B' color='#FFFFFF' style='color: #FFFFFF !important;'>" . traduz("Total") . "</th>
		                        </tr>
		                    </thead>
		                    <tbody>";
			...
			$body .= "<tr>
						<td bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;'>" . traduz("PQ Garantia"). "</td>";
			$total_pq_garantia = 0;
			for ($i=0; $i<pg_num_rows($res); $i++){
			...
			$body .= "</tr>";
				...
				$texto_ano = traduz("Atend. PerÃ­odo");
				$body .= "<tr>
	        					<td bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;'>" .  traduz("Atend. PerÃ­odo") . "</td>";
	        		$acumula_ano = 0;
				...
				### MONTA LINHA COM TOTAL DE OS DO MÃŠS
				$body .= "<tr>
							<td bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;'>" . traduz("Atend. Mês") . "</td>";
				$acumula_mes = 0;
				...
				### % NO ANO
				$body .= "<tr>
	        					<td bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;'>" . traduz("% No PerÃ­odo") . "</td>";
		...
		if (pg_num_rows($res) > 0){
			...
			$fileTemp = "/tmp/relatorio-percentual-defeitos-garantia-{$login_fabrica}-{$data_inicial}-{$data_final}.xls";
	        	...
	        	$head = "<table border='1'>
	        				<thead>
		                        <tr>
		                            <th bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;' colspan='{$colspan}' >" . traduz("RELATÃ“RIO DE PERCENTUAL DOS DEFEITOS POR PRODUTOS") . " - $mes_inicial/$ano_inicial Ã  $mes_final/$ano_final</th>
		                        </tr>
		                        <tr>
		                            <th bgcolor='#596D9B' color='#FFFFFF' style='color: #FFFFFF !important;'>#</th>";
		       ...
		       $head .= "<th bgcolor='#596D9B' color='#FFFFFF' style='color: #FFFFFF !important;'>" . traduz("Total") . "</th>
	        	...
	        	$body .= "
	        	<tr>
	        		<td bgcolor='#D9E2EF' color='#333333' style='color: #333333 !important;'>" . traduz("% No Mês") . "</td>";
...
$msgErrorPattern03 = traduz("Não foram encontrados resultados para esta pesquisa.");
...
if(!empty($_POST))
{
	...
	if($login_fabrica == 117){
		...
		if(empty($mes_inicial)){
			$msg_erro["msg"][]    = traduz("Preencha o Mês Inicial");
			$msg_erro["campos"][] = traduz("mes_inicial");
		}

		if(empty($ano_inicial)){
			$msg_erro["msg"][]    =  traduz("Preencha o Ano Inicial");
			$msg_erro["campos"][] =  traduz("ano_inicial");
		}

		if(empty($mes_final)){
			$msg_erro["msg"][]    = traduz("Preencha o Mês Final");
			$msg_erro["campos"][] = traduz("mes_final");
		}

		if(empty($ano_final)){
			$msg_erro["msg"][]    = traduz("Preencha o Ano Final");
			$msg_erro["campos"][] = traduz("ano_final");
		}

		if((int)$ano_inicial == (int)$ano_final && (int)$mes_inicial > (int)$mes_final){
			$msg_erro["msg"][]    = traduz("O Mês Inicial não pode ser maior que Mês Final");
			$msg_erro["campos"][] = traduz("mes_inicial");
		}

		if((int)$ano_inicial > (int)$ano_final){
			$msg_erro["msg"][]    =  traduz("O Ano Inicial não pode ser maior que Ano Final");
			$msg_erro["campos"][] =  traduz("ano_inicial");
		}
		...
		if(strtotime($data_final) > strtotime($data_inicial . ' +36 month')){
			$msg_erro["msg"][]    =  traduz("O perÃ­odo não pode maior que 36 meses");
		}
		if (empty($linha)) {
...
?>
<script type="text/javascript" src="plugins/jquery_multiselect/js/jquery.multi-select.js"></script>
<link rel="stylesheet" type="text/css" href="plugins/jquery_multiselect/css/multi-select.css" />
...
<form name='frm_percentual' action='<? echo $PHP_SELF ?>' class="form-search form-inline tc_formulario" method="POST">
	<div class="titulo_tabela"><?php echo traduz("Parãmetros de Pesquisa"); ?></div>
	<br />
			...
			?>
				...
				<div class='row-fluid'>
			        ...
			        <div class='span2'>
			        	<div class='control-group <?=(in_array("mes_inicial", $msg_erro["campos"])) ? "error" : ""?>'>
		                    <label class='control-label' for='mes_inicial'><?php echo traduz("Data Inicial"); ?></label>
			        ...
			        <div class='span2'>
			        	<div class='control-group <?=(in_array("mes_final", $msg_erro["campos"])) ? "error" : ""?>'>
		                    <label class='control-label' for='mes_final'><?php echo traduz("Data Final"); ?> </label>
		                    <div class='controls controls-row'>
		                        <div class='span10'>
		                            <h5 class='asteristico'>*</h5>
                ...
                <div class="row-fluid">
                    ...
                    <div class='span4'>
                        ...
                        <label class='control-label' for='macro_familia'><?php echo traduz("Macro - FamÃ­lia");?></label>
                        <div class='controls controls-row'>
                            <div class='span12'>
			        ...
			        <div class='span4'>
			        	<div class='control-group <?=(in_array("familia2", $msg_erro["campos"])) ? "error" : ""?>'>
		                    <label class='control-label' for='familia2'><?php echo traduz("FamÃ­lia"); ?></label>
		                    <div class='controls controls-row'>
		                        <div class='span11'>
			    ...
			    <div class="row-fluid">
			    	...
			    	<div class='span4'>
			        	<div class='control-group <?=(in_array("produto", $msg_erro["campos"])) ? "error" : ""?>'>
		                    <label class='control-label' for='produto'><?php echo traduz("Produto"); ?></label>
		                    <div class='controls controls-row'>
		                        <div class='span11'>
			...
			?>
				...
				<div class="container tc_container">
					<div class='row-fluid'>
						...
						<div class='span4'>
							<div class='control-group'>
								<label class='control-label' for='posto'><?php echo traduz("Por posto"); ?></label>
								<div class='controls controls-row'>
									<div class='span4'>
	...
	<br />
		<center>
			<input class="btn" type="button" onclick="frm_percentual.submit();" value="<?php echo traduz("Pesquisar"); ?>">
		</center>
	<br />
</form>
<br />
...
if ((strlen($mes) > 0 AND strlen($ano) > 0) OR (strlen($data_inicial) > 0 && strlen($data_final) > 0) && count($msg_erro["msg"]) == 0){
	...
	if($_POST["familia"] != "t" && $_POST["posto"] != "t"){
		echo "<div class='alert alert-warning'>". traduz("Para ver a descrição do Produto passe o mouse sobre o código") . "</div>";
	}
	...
	if (strlen($posto) > 0)
	{
		...
		fputs ($fp,"<html>");
		fputs ($fp,"<head>");
		fputs ($fp,"<title>". traduz("DEFEITOS POR PRODUTOS") . "- $data");
		fputs ($fp,"</title>");
	...
	if (strlen($familia) == 0){
		if(strlen($posto) == 0){
			...
			if (pg_numrows($res) == 0){
				...
				echo "	<tbody>
						<tr class='table_line' bgcolor='#F7F7F7'>
							<td class='menu_top'>".traduz("PQ Garantia")."</td>";
							$total_pq_garantia = 0;
							for ($i=0; $i<pg_numrows($res); $i++){
				...
				### MONTA LINHA COM TOTAL DE OS DO ANO
				$acumula_ano = 0;
				$texto_ano = ($login_fabrica == 117) ? traduz("Atend. PerÃ­odo") : traduz("Atend. Ano");
				...
				echo "	<tr class='table_line' BGCOLOR='#F7F7F7'>
							<td class='menu_top'>" . traduz("Atend. Mês") . "</td>";
							for ($i=0; $i<count($contaano); $i++){
				...
				### % NO ANO
				$texto_porc_ano = ($login_fabrica == 117) ? traduz("% No PerÃ­odo") : traduz("% No Ano");
				echo "	<tr class='table_line'BGCOLOR='#F7F7F7'>
				...
				if($login_fabrica != 117){
				echo "	<tr class='table_line' bgcolor='#F1F4FA'>
							<td class='menu_top'>" . traduz("% No Mês") . "</td>";
							$total_porcen_mes = 0;
							for ($i=0; $i<pg_numrows($res); $i++){
								@$porc_mes = ($contaano[$i] / $acumula_mes) * 100;
								$total_porcen_mes += $porc_mes;
				...
				if($login_fabrica != 117){
					echo "<tr class='table_line' bgcolor='#F1F4FA'>
					<td class='menu_top'>". traduz("% Média") . "</td>";
					for ($i=0; $i<pg_numrows($res); $i++){
						echo "<td>&nbsp;</td>\n";
					}
					if($login_fabrica == 117){
			...
			if (pg_num_rows($resX) > 0) {
				...
				fputs ($fp,"<tr>");
				fputs ($fp,"<td bgcolor='#E9F3F3' align='center'>" . traduz("POSTO") . "</td>");
				fputs ($fp,"<td bgcolor='#E9F3F3' align='center'>" . traduz("UF") . "</td>");
		...
		if (strlen($posto) == 0){
			...
			if (pg_numrows($res) == 0){
				...
				### MONTA LINHA COM TOTAL DE OS DO MES
				echo "	<tr class='table_line' bgcolor='#F1F4FA'>
							<td class='menu_top'>". traduz("Atend. Ano") . "</td>";
							$total_conta = 0;
							for ($i=0; $i<pg_numrows($res); $i++){
								echo "<td align='right' style='padding-right:5px;'>".pg_result($res,$i,conta)."</td>\n";
				...
				### % NO ANO
				echo "	<tr class='table_line' bgcolor='#F1F4FA'>
							<td class='menu_top'>" . traduz("% No Ano") . "</td>";
							$total_porcen_mes = 0;
							for ($i=0; $i<pg_num_rows($res); $i++){
				...
				### % NO MÃŠS
				echo "	<tr class='table_line'BGCOLOR='#F7F7F7'>
							<td class='menu_top' bgcolor='#F1F4FA'>" . traduz("% No Mês") . "</td>";
							$total_porcen_mes = 0;
							for ($i=0; $i<pg_numrows($res); $i++){
								@$porc_mes = ($contaano[$i] / $total_conta) * 100;
								$total_porcen_mes += $porc_mes;
				...
				if($login_fabrica != 117){
				echo "	<tr class='table_line' bgcolor='#F1F4FA'>
							<td class='menu_top'>" . traduz("% MÃ©dia") . "</td>";
							for ($i=0; $i<pg_numrows($res); $i++){
								echo "<td>&nbsp;</td>\n";
							}
					echo "</tr>";
	...
	if ($showMsg != 1){
		...
		<div class='container'>
				...
				<thead>
				<tr class='titulo_coluna'>
					<th align='center' colspan=5>". traduz("Cinco Maiores Defeitos (Convencionais)") ."</th>
				</tr>
				</thead>
				<tbody>";
				if($login_fabrica == 138) {
	...
	if ($showMsg != 1)
	{
		...
		if(pg_num_rows($res2) > 0){
			...
			<table class='table table-striped table-bordered table-hover table-large' width='700'>
				<thead>
					<tr class='titulo_coluna'>
						<th align='center' colspan=5>". traduz("Cinco Peças que mais Deram Defeitos") . "</th>
					</tr>
				</thead>
				<tbody>";
			# ------------------------------------ 6 ------------------------------------ #
			...
			echo "
				<table class='table table-striped table-bordered table-hover table-large' width='700px'>
					<thead class='titulo_coluna'>
						<tr>
							<th align='center' colspan='2'>" . traduz("BI - Familia x QTDE") . "</th>
						</tr>
						<tr>
							<th>".traduz("Familia")."</th>
