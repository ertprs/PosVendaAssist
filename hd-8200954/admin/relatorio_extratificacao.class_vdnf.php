...
/**
 *
 * relatorio_extratificacao.class_vdnf.php
 *
 * @author  Francisco Ambrozio
 * @version 2014.08
 *
 * Vers„o do RelatÛrio de EstratificaÁ„o que usa como base para
 *  os resultados a data da venda do produto e n„o da fabricaÁ„o.
 *
 */
...
class relatorioExtratificacao
{
	...
	private function preparaTabelaTemp()
	{
        ...
        if ($this->login_fabrica == '24') {
            ...
            $this->temp_tbls = array(
                'os_serie' => $arr_fab_nome[$this->login_fabrica] . '_os_serie_' . $this->familia,
                'numero_serie' => $arr_fab_nome[$this->login_fabrica] . '_numero_serie_' . $this->familia,
            );
            ...
            $prepare = pg_prepare("tmp_numero_serie", "SELECT numero_serie, serie, produto INTO TEMP numero_serie_$login_admin from tbl_numero_serie join tbl_produto using(produto) where fabrica = $1 and familia = $2 and fabrica_i = $1 $condProdutos; " );
	...
	private function prepareStatements()
	{
		...
		if (empty($this->pecas)) {
			$prepare = pg_prepare("oss", "select distinct(os) as os, data_nf, data_abertura into temp temp_oss from {$this->temp_tbls['os_serie']} join {$this->temp_tbls['numero_serie']} USING(serie,produto) where data_fabricacao between $1 and $2 and data_nf >= data_fabricacao order by data_nf");
		} else {
		    $cond = implode(', ', $this->pecas);
		    $condFornecedores = $this->montaCondFornecedores();
		    $condProdutos = $this->montaCondProdutos();
            ...
            $prepare = pg_prepare("oss", "select distinct(os) as os, data_nf, data_abertura into temp temp_oss from {$this->temp_tbls['os_serie']} join {$this->temp_tbls['numero_serie']} using(serie,produto) JOIN peca_serie_$login_admin USING(os) $joinNSFornecedor where data_fabricacao between $1 and $2 and data_nf >= data_fabricacao order by data_nf");
		}
	...
	private function producao($month = '23')
	{
		...
		$sql4 = "SELECT $c_qtde_produto_produzido as total, cf.mes, cf.ano,$c_cfe
				 ...
				 WHERE cf.fabrica = $this->login_fabrica and cf.familia = $this->familia $cond_produto";
	...
	private function popOS($ano, $mes, $idx, $break = true)
	{
		...
		for ($i = 1; $i <= $this->meses; $i++) {
            ...
            if ($i == 1) {
                ...
                $sqlTotal = "SELECT count(os) as total, fn_qtos_meses_entre(data_nf, data_abertura) as meses into temp temp_total_os from temp_oss group by meses order by meses";
	...
	private function extratifica($resultado)
	{
        ...
        if (true === $temps['numero_serie']) {
            $this->executePreparedStatements('tmp_numero_serie', array($this->login_fabrica,$this->familia));
        }
