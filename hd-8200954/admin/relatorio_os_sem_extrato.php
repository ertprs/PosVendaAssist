...
if($_GET['ajax']=='sim') {
	...
	if ($listar == "ok") {
		...
		$sql = "SELECT interv_reinc.os
				...
				SELECT
				ultima_reinc.os,
				(SELECT status_os FROM tbl_os_status WHERE tbl_os_status.os =
				ultima_reinc.os AND status_os IN (13,19,67,68,70) ORDER BY data DESC
				LIMIT 1) AS ultimo_reinc_status
				FROM (SELECT DISTINCT os FROM tbl_os_status WHERE status_os IN
				(13,19,67,68,70) ) ultima_reinc
				) interv_reinc
				...
				AND $cond_1 
					AND tbl_os.os not in (select os from tmp_interv_$login_admin)
