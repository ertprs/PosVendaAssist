...
if(strlen($btn_acao) > 0){
	if (strlen($posto_codigo) == 0 AND strlen($posto_nome) == 0 ) $msg_erro .= "É obrigatório o preenchimento do posto<br>";
	if(strlen($qtde_dias) == 0)                                   $msg_erro .= "É obrigatório o preencimento da quantidade de dias<br>";
	if ($qtde_dias<10)                                            $msg_erro .= "No mínimo 10 dias.";
}
...
#exit;
	...
	if (pg_numrows($res) > 0) {
		...
		if($login_fabrica == 3 ){
			fputs ($fp,"<td nowrap>NF ENVIO</td>");
			fputs ($fp,"<td nowrap>EMISSÃO NF</td>");
		}
		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			if ($ant_nota_fiscal == $nota_fiscal) 		fputs ($fp,"<td>&nbsp;</td>");
			else 		fputs ($fp,"<td nowrap align='center'>$nota_fiscal</td>");
			...
			if($login_fabrica == 3 ){
				fputs ($fp,"<td nowrap align='center'>$fat_nf</td>");
				fputs ($fp,"<td nowrap align='center'>$fat_emissao</td>");
			}
			/* IGOR HD 6161 - 23/10/2007*/
			if($login_fabrica == 20 ){
