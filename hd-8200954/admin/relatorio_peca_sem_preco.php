...
if ($_POST["btn_acao"] == "liberar_os") {
	...
	if (pg_num_rows($res) > 0) {
		...
		echo json_encode(array("retorno" => utf8_encode("Serviço não encotrado.")));
	}
	exit;
}
			...
			?>
				<table class="tablesorter" width="700" >
						<tr>
							...
							<th><?php echo traduz("Data de Digitação"); ?></th>
...
$title     = traduz("RELATÓRIO DE PEÇAS EM OS E SEM PREÇO");
				...
				<div class='control-group <?=(in_array("data", $msg_erro["campos"])) ? "error" : ""?>'>
					<div class="controls controls-row">
						<div class="span7 input-append">
							<label class="control-label" for="tabela"><?php echo traduz("Tabela de Preço");?> &nbsp;&nbsp;&nbsp;&nbsp; </label>
	...
	$(document).on('click','button.btn_liberar_peca', function(){
		if (confirm('Deseja liberar os para fechamento ?')) {
	        ...
	        $.ajax({
	        	...
	        	alert("Não foi possível liberar OS, tempo limite esgotado!");
	        }).done(function(data) {
	            data = JSON.parse(data);
	            if (data.retorno == "success") {
	                $(btn).text("Peça liberada");
	                setTimeout(function(){
	                	$("#"+os_item).remove();
	                }, 1000);
	            }else{
...
?>
			...
			<tr class="titulo_coluna">
				<?php if ($login_fabrica == 171) { ?>
					<th width="250" nowrap><?php echo traduz("Referência Fábrica"); ?></th>
				<?php }?>

				<th width="150" nowrap><?=traduz('Referência Peça')?></th>
				<th width="350" nowrap><?=traduz('Descrição Peça')?></th>
				<?php if ($login_fabrica == 178){ ?>
				<th width="200"><?=traduz('OS')?></th>
				<th width="350"><?=traduz('Ação')?></th>
				<?php } ?>
		...
		?>
			...
			?>
					...
					?>
						...
						<td class='tac'> <button class="btn_liberar_peca btn btn-small btn-primary" data-os_item="<?=$os_item?>">Liberar Peça Faturamento</button></td>
						<?php } ?>
					</tr>
			<?php
				}
