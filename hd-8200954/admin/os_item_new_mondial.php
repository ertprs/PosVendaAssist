...
//
	...
	if (strlen ($msg_erro) == 0) {
		...
		for ($i = 0 ; $i < $qtde_item ; $i++) {
			...
			$xpreco_orcamento= $_POST['preco_orcamento_' . $i]; /* TESTE Fabrica 3 POSTO DE TESTE*/
			...
			if ((strlen ($xos_produto) > 0 or strlen($xorcamento_item)>0) AND strlen($xpeca) == 0) {
				...
				if (strlen($xpeca) > 0 and strlen($msg_erro) == 0) {
					...
					if (strlen ($msg_erro) == 0) {
						...
						if ($login_fabrica==3 AND strlen($os_revenda_item)>0 AND ($xservico == 643 OR $xservico == 644)){
							...
							if (strlen($orcamento)==0){
								$msg_erro .= "Erro ao criar o orçamento. Tente novamente ou contate o Suporte Telecontrol através do e-mail: suporte@telecontrol.com.br";
							}
							...
							if (strlen($msg_erro)== 0){
								if (strlen($xorcamento_item) == 0){
									...
									(orcamento,peca,defeito,servico_realizado,preco,qtde)
									VALUES ($orcamento,$xpeca,$xdefeito,$xservico,$xpreco_orcamento,$xqtde)";
									$orca = pg_query($con, $query);
									$msg_erro .= pg_errormessage($con);
								}else{
...
function pecaDefeito(defeito, produto, i, peca_selecionada,peca_pai) {
//alert("def:"+defeito+" - prod: "+produto+" - I: "+i);
	if(defeito ==""){
		alert ("Selecione o defeito da peça.");
	}
//	alert("os_item_new_mondial_peca_defeito_ajax.php?defeito=" + defeito+"&produto="+produto+"&peca_selecionada="+peca_selecionada+"peca_pai="+peca_pai) ;

	$.ajax({
		type: "GET",
		url: "os_item_new_mondial_peca_defeito_ajax_test.php",

		data: "defeito=" + defeito+"&produto="+produto+"&peca_selecionada="+peca_selecionada+"&peca_pai="+peca_pai,
		cache: false,
		beforeSend: function() {
...
function fnc_pesquisa_peca_lista_mondial (produto_referencia, peca_referencia, peca_descricao, defeito, voltagem, tipo, peca_qtde) {
	...
	if (tipo == "tudo") {
		url = "<? echo $url;?>.php?<?if (strlen($_GET['os'])>0) echo 'os='.$_GET['os'].'&';?>produto=" + produto_referencia + "&descricao=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&defeito=" + defeito.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}

	if (tipo == "referencia") {
		url = "<? echo $url;?>.php?<?if (strlen($_GET['os'])>0) echo 'os='.$_GET['os'].'&';?>produto=" + produto_referencia + "&peca=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&defeito=" + defeito.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}
...
function fnc_pesquisa_peca_lista_intel (produto_referencia, peca_referencia, peca_descricao, peca_preco, tipo) {
	var url = "";
	if (tipo == "tudo") {
...
// SOMENTE LORENZETTI
		...
		if(strlen($os) > 0){
			...
			if(pg_num_rows($res) > 0) {
				...
				if($informatica == 't'){
					echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif' color='#ffffff'><b>N. Série Peça Trocada</b></font></td>";
				}
		...
		//HD 38818 4/9/2008 ----------------------------
		if($informatica == 't'){
			echo "<td align='center'><font size='1' face='Geneva, Arial, Helvetica, san-serif' color='#ffffff'><b>N. Série Peça</b></font></td>";
		}
		...
		for ($i = 0 ; $i < $loop ; $i++) {
			...
			if ($os_item_aparencia == 't' AND $posto_item_aparencia == 't' and $os_item_subconjunto == 'f') {	// HD 7033 16/11/2007
				...
				<input class='frm' type='text' name='peca_$i' id='peca_peca_$i'size='15' value='$peca[$i]'"; 
				echo " onblur=\"javascript: pega_peca('$os','peca_peca_$i','descricao_$i'); atualizaQtde(document.frm_os.peca_$i,document.frm_os.qtde_$i);\" ";
				echo ">&nbsp;<img src='imagens/btn_lupa.gif' border='0' align='absmiddle'  ";
				//hd 12634 29/1/2008
				echo " onclick='javascript: fnc_pesquisa_peca_lista_intel (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.preco_$i , \"referencia\")'";
				...
				if($informatica == 't'){
					echo "</td>";
						echo "<td align='center' nowrap>";
						echo "<input class='frm' type='text' name='peca_serie_$i' id='peca_serie_$i' size='13'  maxlength='20' value='$peca_serie[$i]' ";
						echo ">&nbsp;<img src='imagens/btn_lupa.gif' border='0' align='absmiddle'";
				...
				echo "<td>
					<select class='frm' size='1' name='defeito_$i' style='width:200px' onchange='limpaPecas()' onfocus='defeitoLista(document.frm_os.peca_$i.value,$i,$os)'>";
				...
				// Email em 15/10/2009 - Conversa com Vanderlei Tosta e Rogério mostrar somente
				// troca de peça gerando pedido. Deve treinar os postos para e então depois de 
				// 2 meses volta ao normal...para escolher o serviço da peça e então ter informações
				// para aparecer no FCR das peças com problemas que nao geram pedido.
				$sql = "SELECT *
