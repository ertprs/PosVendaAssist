		...
		if(strlen($faq_situacao) > 0){ // HD 45991
			...
			if(strlen($msg_erro) ==0 ){
				...
				if(pg_numrows($res) > 0){
					...
					$mailer->sendMail($destinatario, utf8_encode($assunto), utf8_encode($mensagem), $remetente);
				}
			}
		}
...
// HD 120306 - envia e-mail para o posto informando pre-OS cadastrada
				if(($login_fabrica==14 or $login_fabrica==43 or $login_fabrica==66) and $abre_os=='t'){
					...
					$headers = "From: Call-center <helpdesk@telecontrol.com.br>\n";
			...
			if (strlen($msg_erro) == 0){
				...
				echo "<script language='javascript'>alert('Pré-os Cadastrada com Sucesso')
					window.location('$PHP_SELF?indicacao_posto=$indicacao_posto&callcenter=$hd_chamado')</script>";
			}else{
				$res = pg_exec($con,"ROLLBACK TRANSACTION");
			}
}/*INSERINDO*/
...
*/
	...
	if (strlen($msg_erro) == 0){
		$res = pg_exec($con,"COMMIT TRANSACTION");
		echo "<script language='javascript'>alert('Pré-os Alterada com Sucesso');
		window.location('$PHP_SELF?callcenter=$hd_chamado')</script>";
	}else{
		//echo $msg_erro;
		$res = pg_exec($con,"ROLLBACK TRANSACTION");
	}
...
function buscaCEP(cep,endereco,bairro,cidade,estado) {
	if (endereco.value.length == 0 || 1 == 1) {
		http.open("GET", "../admin_cliente/ajax_cep.php?cep=" + escape(cep), true);
...
function fnc_pesquisa_produto2 (campo, campo2, tipo, mapa_linha) {
	...
	if (xcampo.value != "") {
		var url = "produto_pesquisa_2.php?campo=" + xcampo.value + "&tipo=" + tipo + "&voltagem=t";
		if (familia !== undefined) {
			url += "&familia=" + familia;
		}

		if ($("#mapa_linha").length) {
			url += "&mapa_linha=t";
		}
...
<? } elseif (in_array($login_fabrica, array(24, 30, 52, 59))) { // HD 75777 ?>
function fnc_tipo_atendimento(tipo) {
		$('#cpf').val('');
	if (tipo.value == 'C') {
		$('#cpf').attr('maxLength', 14);
...
<tr>
	...
	<div id='div_consumidor' style='display:inline; Position:relative;background-color: #e6eef7;width:100%'>
		...
		<tr>
			...
			<?
			if ($login_fabrica != 156 and ((strlen($consumidor_cpf) == 14 and strlen($callcenter) > 0) or strlen($callcenter) == 0)) {
				echo "CPF:";
				$limite ='14';
                } elseif ((strlen($consumidor_cpf) == 18 and strlen($callcenter) > 0) or $login_fabrica == 156) {
				echo "CNPJ:";
				$limite = "18";
			}?>
			</span></strong></td>
		...
		<tr>
			...
			<td align='left'>
				...
				<input name="consumidor_complemento" id='consumidor_complemento' value='<? echo $consumidor_complemento ;?>' class="input_req" type="text" size="14" maxlength="14" <?php echo $endereco_readonly; ?> >
			</td>
		</tr>
		<tr>
		...
		<tr>
            ...
            echo array2select(
                ...
                $origens,
                $origem,
                "style='width:110px; font-size:9px'", ($login_fabrica == 3 ? '' : null), ($login_fabrica == 156)
            ); ?>
			</td>
			<!--HD36903-->
			<?PHP if ($login_fabrica != 2) {?>
	...
	<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
        <?php if ($login_fabrica == 156): ?>
        <tr>
            <td align="left"><strong>Família:</strong></td>
            <td align="left">
<?
                echo array2select(
                'familia', 'familia',
                pg_fetch_pairs($con, "SELECT familia, descricao FROM tbl_familia WHERE ativo AND fabrica = $login_fabrica"),
                $familia,
                'style="width:120px;font-size:9px"', 'ESCOLHA', true
            );
?>
            </td>
            <td coldspan="2"></td>
		...
		<tr>
			...
			<td align='left'>
				...
				onclick="javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_referencia,document.frm_callcenter.produto_nome,'referencia',document.frm_callcenter.mapa_linha)">
			</td>
			...
			<tr>
				...
				<td align='left'>
					...
					<input name="voltagem" id="voltagem" class="input" value='<?php echo $voltagem;?>' >
				</td>
				<td align='left'><strong>Série:</strong></td>
				<td align='left'>
					<input type="hidden" name="serie_anterior" value="<?php echo $serie; ?>" />
					<input name="serie" id="serie" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" value="<?php echo $serie;?>" />
				</td>
		</tr>
		<tr>
			...
			<td align='left'>
				...
				<input name="nota_fiscal" id="nota_fiscal" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" value="<?php echo $nota_fiscal;?>" />
			</td>
		...
		<? if($login_fabrica==2) {?> 
			<tr>
				...
				<td align='left'>
					<input name="cnpj_revenda" id="cnpj_revenda" class="<?php echo ($login_fabrica==24) ? 'input_req' : 'input' ; ?>" value="<?php echo $cnpj_revenda;?>" />
				</td>
			</tr>
		<? }
		if($login_fabrica==3) {?>
	...
	<? if (!in_array($login_fabrica, array(3,156))) { //HD 40086 ?>
	<table width="100%" border='0'>
		<tr>
