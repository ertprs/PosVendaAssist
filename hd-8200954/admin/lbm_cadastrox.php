...
// echo $sql;
		$email_ok = mail ("$email_gerente" , "Lista Básica Apagara" , "Toda a lista básica do produto $produto_referencia - $produto_descricao acaba de ser apagada no site TELECONTROL" , "From: Telecontrol <helpdesk@telecontrol.com.br>" , "-f helpdesk@telecontrol.com.br" );
	}
//  }
	#---------------------------------------------------
	...
	if (strlen ($msg_erro) == 0) {
		...
		if (strlen($arquivo["tmp_name"]) > 0 && $arquivo["tmp_name"] != "none"){
			...
			if (strlen($msg_erro) == 0) {
				...
				$nome_anexo = "/var/www/assist/www/admin/documentos/produto.xls";
				...
				if (strlen($msg_erro) == 0) {
					if (copy($arquivo["tmp_name"], $nome_anexo)) {
						...
						for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
								...
								switch($j) {
									...
									case 3:
										...
										$sql = " SELECT peca 
												...
												AND   (tbl_peca.referencia_pesquisa =  '$referencia' or tbl_peca.referencia ='$referencia') ";
										$res = @pg_query($con,$sql);
										if(@pg_num_rows($res) > 0){
							...
							if(strlen($msg_erro) == 0 and strlen($peca) > 0 and strlen($qtde) > 0) {
...
#	26/05/2010 MLG - HD 243869
							if ($login_fabrica != 1) {
								...
								if (pg_num_rows ($res) > 0) {
									...
									$email_ok = mail ("$email_gerente" , "Alteração de Lista Básica" , "A lista básica do produto $produto_referencia - $produto_descricao acaba de ser alterada no site TELECONTROL" , "From: Telecontrol <helpdesk@telecontrol.com.br>" , "-f helpdesk@telecontrol.com.br" );
								}
								#---------------------------------------------------
								header ("Location: $PHP_SELF");
								exit;
...
#	26/05/2010 MLG - HD 243869
			if ($login_fabrica != 1) {
				...
				if (pg_num_rows ($res) > 0) {
					...
					$email_ok = mail ("$email_gerente" , "Duplicação de Lista Básica" , "A lista básica do produto $produto_referencia - $produto_descricao acaba de ser criada a partir de uma duplicação no site TELECONTROL" , "From: Telecontrol <helpdesk@telecontrol.com.br>" , "-f helpdesk@telecontrol.com.br" );
				}
			}
			#---------------------------------------------------
...
#	26/05/2010 MLG - HD 243869
		if ($login_fabrica != 1) {
			...
			if (pg_num_rows ($res) > 0) {
				...
				$email_ok = mail ("$email_gerente" , "Item apagado da Lista Básica" , "Uma peça foi apagada da lista básica do produto $produto_referencia - $produto_descricao no site TELECONTROL" , "From: Telecontrol <helpdesk@telecontrol.com.br>" , "-f helpdesk@telecontrol.com.br" );
			}
		}
		#---------------------------------------------------
...
if (strlen ($btn_lista) > 0) {
	...
	if (strlen ($referencia) == 0) {
		...
		$res = pg_query($con,$sql);
		if(pg_num_rows($res) == 0){
			$msg_erro = "Referencia $referencia não encontrada";
			$erro_btn_lista = true;
		}
	}
}
...
function fnc_pesquisa_peca (campo, campo2, tipo, campo_preco) {
	...
	if (campo.value.length < 3 && campo2.value.length < 3) {
		alert('Digite pelo menos 3 caracteres da referência ou descrição da peça para fazer a pesquisa.');
		return false;
	}
...
<DIV ID='wrapper'>
<form name="frm_lbm" method="post" action="<? echo $PHP_SELF ?>" enctype='multipart/form-data'><?php
if (strlen($msg_erro) > 0) {
...
if (strlen($btn_lista) == 0 or ($erro_btn_lista)) {?>
	...
	<p align='center'><input type='image' src='imagens/btn_listabasicademateriais.gif' onclick='document.frm_lbm.btn_lista.value="listar"; document.frm_lbm.submit();' style="cursor:pointer;" />
	...
	if (strlen($produto) > 0) {
		...
		if ($login_fabrica == 45) {//HD 231820
			echo '<br />';
			echo '<br />';
			echo "<a href='javascript:void(0)' onclick='fnc_impresssao($produto)'>Versão para impressão</a>";
			echo '<br />';
			echo '<br />';
		}
	...
	<br />
	<p align="center"><a href='<?echo $PHP_SELF?>?'>Clique aqui para pesquisar outro produto</a></p><?php
}?>
<!--
...
if (strlen ($btn_lista) > 0 OR strlen ($produto) > 0) {
	...
	if (strlen ($produto) > 0) {
		...
		for ($i = 0; $i < $qtde_linhas; $i++) {
			...
			echo "<td bgcolor='$cor' nowrap>";
				echo "<input type='text' name='peca_pai_$i' value='$peca_pai' size='15' maxlength='20'
				onblur=\"javascript: if (this.value.length > 2) {fnc_pesquisa_peca (document.frm_lbm.peca_pai_$i , document.frm_lbm.descricao_pai_$i , 'referencia_pai', 0);}\">";
			...
			echo "<td bgcolor='$cor' nowrap>";
			echo "<input type='text' name='descricao_pai_$i' value='$descricao_pai' size='30' maxlength='50' onblur=\"javascript: if (this.value.length > 2) {fnc_pesquisa_peca (document.frm_lbm.peca_pai_$i , document.frm_lbm.descricao_pai_$i , 'descricao_pai');}\">";
			...
			if ($login_fabrica == 5  or $login_fabrica == 1) echo " onblur=\"javascript: if (this.value.length > 2) {fnc_pesquisa_peca (document.frm_lbm.peca_$i , document.frm_lbm.descricao_$i , 'referencia', 0);}\"";
			if($login_fabrica == 14 and $ativo == 'f' and strlen($ativo) > 0){ echo " readonly='readonly' "; }
			echo ">";
			...
			echo "<td bgcolor='$cor' nowrap valign='top'>";
			echo "<input type='text' name='descricao_$i' value='$descricao' size='50' maxlength='50'";
			if ($login_fabrica == 5) echo " onblur=\"javascript: if (this.value.length > 2) {fnc_pesquisa_peca (document.frm_lbm.peca_$i , document.frm_lbm.descricao_$i , 'descricao');}\"";
			if($login_fabrica == 14 and $ativo == 'f' and strlen($ativo) > 0){ echo " readonly='readonly' "; }
			echo ">";
			echo "&nbsp;<img src='../imagens/btn_buscar5.gif' border='0' align='absmiddle'";
			if ($login_fabrica == 45) echo " onclick='javascript: fnc_pesquisa_peca (document.frm_lbm.peca_$i , document.frm_lbm.descricao_$i , \"descricao\", document.frm_lbm.preco_$i)' style='cursor:pointer'><br>&nbsp;"; else echo " onclick='javascript: fnc_pesquisa_peca (document.frm_lbm.peca_$i , document.frm_lbm.descricao_$i , \"descricao\")' style='cursor:pointer'>";
			echo "<br><font size='-3' color='#ffffff'>$xdescricao_peca_de</font>";
			echo "</td>";
			...
			echo "</td>";
			echo "<td>";
			echo "<button type='button' style='border: 0 solid transparent;cursor: pointer'
						   onclick='document.frm_lbm.btn_acao.value = \"apagar\";".
								   "document.frm_lbm.apagar.value = \"$lbm\";".
								   "document.frm_lbm.submit()'>
						   <img src='../imagens/btn_apagar.gif' alt='Clique aqui para apagar este item da lista básica.' />
				   </button><br />&nbsp;";
			echo "</td>";
	...
	if ($login_fabrica == 1 or $login_fabrica == 51) {?>
		<fieldset style="width: 500">
			...
			<table width='400' align='center' border='0'>
				...
				<tr>
					<td align='center'>
						<select name='type_duplicar_origem'>
							<option value=''></option>
							<option value='Tipo 1' <? if ($type_duplicar_origem == 'Tipo 1') echo "selected"; ?>>Tipo 1</option>
							<option value='Tipo 2' <? if ($type_duplicar_origem == 'Tipo 2') echo "selected"; ?>>Tipo 2</option>
							<option value='Tipo 3' <? if ($type_duplicar_origem == 'Tipo 3') echo "selected"; ?>>Tipo 3</option>
							<option value='Tipo 4' <? if ($type_duplicar_origem == 'Tipo 4') echo "selected"; ?>>Tipo 4</option>
							<option value='Tipo 5' <? if ($type_duplicar_origem == 'Tipo 5') echo "selected"; ?>>Tipo 5</option>
							<option value='Tipo 6' <? if ($type_duplicar_origem == 'Tipo 6') echo "selected"; ?>>Tipo 6</option>
							<option value='Tipo 7' <? if ($type_duplicar_origem == 'Tipo 7') echo "selected"; ?>>Tipo 7</option>
							<option value='Tipo 8' <? if ($type_duplicar_origem == 'Tipo 8') echo "selected"; ?>>Tipo 8</option>
							<option value='Tipo 9' <? if ($type_duplicar_origem == 'Tipo 9') echo "selected"; ?>>Tipo 9</option>
						</select>
					</td>
					<td align='center'>
						<select name='type_duplicar_destino'>
							<option value=''></option>
							<option value='Tipo 1' <? if ($type_duplicar_destino == 'Tipo 1') echo "selected"; ?>>Tipo 1</option>
							<option value='Tipo 2' <? if ($type_duplicar_destino == 'Tipo 2') echo "selected"; ?>>Tipo 2</option>
							<option value='Tipo 3' <? if ($type_duplicar_destino == 'Tipo 3') echo "selected"; ?>>Tipo 3</option>
							<option value='Tipo 4' <? if ($type_duplicar_destino == 'Tipo 4') echo "selected"; ?>>Tipo 4</option>
							<option value='Tipo 5' <? if ($type_duplicar_destino == 'Tipo 5') echo "selected"; ?>>Tipo 5</option>
							<option value='Tipo 6' <? if ($type_duplicar_destino == 'Tipo 6') echo "selected"; ?>>Tipo 6</option>
							<option value='Tipo 7' <? if ($type_duplicar_destino == 'Tipo 7') echo "selected"; ?>>Tipo 7</option>
							<option value='Tipo 8' <? if ($type_duplicar_destino == 'Tipo 8') echo "selected"; ?>>Tipo 8</option>
							<option value='Tipo 9' <? if ($type_duplicar_destino == 'Tipo 9') echo "selected"; ?>>Tipo 9</option>
						</select>
					</td>
				</tr>
			</table>
	...
	<fieldset style="width: 500">
		...
		<br>
		<center><img src='imagens_admin/btn_duplicar.gif' style="cursor: pointer;" onclick='document.frm_lbm.btn_acao.value = "duplicar" ; document.frm_lbm.submit()'></center>
		<br>
	</fieldset><?php
	}
}?>
