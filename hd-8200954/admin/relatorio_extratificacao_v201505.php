...
include 'relatorio_extratificacao.class_v201505.php';
/**
 *
 * REGRAS:
 *
...
$produtos_unico = $relatorio->getProdutosUnico();
$pecas = $relatorio->getPecas();
$postos = $relatorio->getPostos();
$periodo = $relatorio->getPeriodo();

$data_inicial_global = $relatorio->getDataInicialGlobal();
if (!empty($data_inicial_global)) {
    $dia_ini = explode("-", $data_inicial_global);
    $data_inicial_global = $dia_ini[1]."/".$dia_ini[0];
}

$data_final_global = $relatorio->getDataFinalGlobal();
if (!empty($data_final_global)) {
    $dia_fin = explode("-", $data_final_global);
    $data_final_global = $dia_fin[1]."/".$dia_fin[0];
}
...
<script type='text/javascript' src='../plugins/plugins/jquery.mask.js'></script>
...
<script>
    ...
    function pareto(fabricacao, abertura, meses, familia, matriz_filial, peca01, peca02, fornecedores, datas, revendas, produtos, pecas, posto) {
        ...
        if( $("#sem_peca").is(":checked") ){
            ...
            if ($login_fabrica == 24) { ?>
                ...
                var url="relatorio_extratificacao_pareto_sem_peca_vdnf.php?ab=" + abertura + "&nf=" + fabricacao + "&fm=" + meses + "&f=" + familia + "&defeito=" + sem_peca + "&po=" + posto + "&dcg=" + desconsiderar_conversor +"&matriz_filial="+matriz_filial;
            <?php
            } else { ?>
                var sem_peca = $("#sem_peca").val();
                var url="relatorio_extratificacao_pareto_sem_peca.php?ab="+abertura+"&fb="+fabricacao+"&fm="+meses+"&f="+familia+"&defeito="+sem_peca+"&po="+posto+"&regiao="+regiao+"&rv="+revendas+"&pd="+produtos;
            <?php
            } ?>
        }else{
            <?php
            if ($login_fabrica == 24) { ?>
                var url="relatorio_extratificacao_pareto_vdnf.php?nf=" + fabricacao + "&ab=" + abertura + "&fm=" + meses + "&fa=" + familia + "&p1=" + peca01 + "&p2=" + peca02 + "&fo=" + fornecedores + "&d=" + datas + "&pd=" + produtos + "&pc=" + pecas + "&po=" + posto + "&dcg=" + desconsiderar_conversor+"&matriz_filial="+matriz_filial;
            <?php
            } else { ?>
                var url="relatorio_extratificacao_pareto.php?fb="+fabricacao+"&ab="+abertura+"&fm="+meses+"&fa="+familia+"&p1="+peca01+"&p2="+peca02+"&fo="+fornecedores+"&rv="+revendas+"&d="+datas+"&pd="+produtos+"&pc="+pecas+"&po="+posto+"&regiao="+regiao;
            <?php
            } ?>
        }
    ...
    function mostraRelatorio(relatorio) {
		...
		<?php if (in_array($login_fabrica, [50])): ?>
    ...
    $.ajax({
            ...
            success: function(data) {
                ...
                <?php if ($login_fabrica == 50 || $login_fabrica == 24 || $login_fabrica == 120 or $login_fabrica == 201 or $login_fabrica == 201): ?>
    ...
    function getProdutosByFamiliaUnico(familia, apagaVal) {
        $.ajax({
            url: "produtos_familia.php?familia=" + familia + "&unico=sim",
            dataType: "text",
            success: function(data) {
                if (apagaVal) {
                    $('#produto_input_unico').val('');
                    $('#produtos_unico').val('');
                }
                autocompleteProdutoUnico(data);
            }
        });
    }
    ...
    function autocompleteProdutoUnico (data) {
        var produto_unico = $.parseJSON(data);
        if (!produto_unico) {
            return false;
        }

        $('#produto_input_unico').autocomplete({
            source: produto_unico,
            select: function (event, ui) {
                $("#produto_input_unico").val(ui.item['value']);
                var data = ui.item['data'];
                $("#produto_input_hidden_unico").val(data);

                return false;
            }
        }).data("uiAutocomplete")._renderItem = function (ul, item) {
            var text_unico = item["value"];

            return $("<li></li>").data("item.autocomplete", item).append("<a>"+text_unico+"</a>").appendTo(ul);
        };
    }
  ...
  function valida_mes(this_ano) {
    mes_ano = $(this_ano).val()
    ...
    let mes_ano_array = mes_ano.split('/')   
    
    if (mes_ano_array[0].length != 2 || mes_ano_array[1].length != 4) {
        alert('Data Inv·lida')
        $(this_ano).val('')
        $("#meses").focus()
        return false
    ...
    let mes = mes_ano_array[0]
    let ano = mes_ano_array[1]
    ...
    $(function() {
        ...
        <?php if ($login_fabrica == 24) { ?>
                $("#data_inicial_global, #data_final_global").mask('00/0000')

                $("#data_inicial_global, #data_final_global").blur(function() {
                    valida_mes(this)
                });
        ...
        if (familia) { getPecasByFamilia(familia, false); getProdutosByFamilia(familia, false); getProdutosByFamiliaUnico(familia, false); };
        ...
        $('#add_produto_unico').click(function(){
            var produto_unico = removeAcento($('#produto_input_unico').val().toUpperCase());
            var produto_id_unico = $("#produto_input_hidden_unico").val();

            if (produto_unico) {
                var option_unico = '<option value="' + produto_unico + '" class="' + produto_id_unico + '">' + produto_unico + '</option>';
                var hidden_unico = '<input type="hidden" name="produto_unico[]" id="' + produto_id_unico + '" value="' + produto_id_unico + '" />';
                $('#produtos_unico').append(option_unico);
                $('#produtos_unico').append(hidden_unico);
                $("#produto_input_unico").val('');
            }
        });
        ...
        $('#rm_produto_unico').click(function(){
            $("select[name=produtos_unico] option:selected").each(function () {
                var hidden_unico = $(this).attr("class");
                $(this).remove();
                $("input[id='"+hidden_unico+"']").remove();
            });
        });
        ...
        <?php if ($login_fabrica != 24 and $login_fabrica != 120 and $login_fabrica != 201) { ?>
            $.ajax({
            url: "json_postos.php",
            dataType: "text",
            success: function(data) {
...
<div style="width:700px; margin:auto; padding: 0px 10px; font-weight: bold; margin-bottom: 20px;">
    ...
    <?php if ($login_fabrica == 24): ?>
    <br />Obs: RelatÛrio IRC Global È atualizado 1 vez por dia na madrugada. </br>Obs: Intervalo entre Data Inicial e Data Final n„o pode ser superior a 24 Meses.<br /><br />
    <?php endif ?>
</div>
    ...
    <form name="frm_consulta" action="<?=$_SERVER['PHP_SELF'];?>" method="POST" name="frm">
        <table cellspacing="1" align="center" class="form">
            <?php if ($login_fabrica == 24) { ?>
                    <tr>
                        <td>
                            <label for="data_inicial_global">Data inicial</label>
                            <input type="text" name="data_inicial_global" id="data_inicial_global" placeholder="mm/aaaa" value="<?=$data_inicial_global?>" class="frm" style="width: 50px;">
                        </td>
                        <td>
                            <label for="data_final_global">Data Final</label>
                            <input type="text" name="data_final_global" id="data_final_global" placeholder="mm/aaaa" value="<?=$data_final_global?>" class="frm" style="width: 50px;">
                        </td>
                    </tr>
            <?php } ?>
            <tr>
            ...
            <tr>
                <td>
                    ...
                    <select name="familia" id="familia" onChange="getPecasByFamilia(this.value, true); getProdutosByFamilia(this.value, true); getProdutosByFamiliaUnico(this.value, true);">
                        <option value=""></option>
                        <?php
            ...
            <?php if ( in_array($login_fabrica, array(24,50,120,201)) ): ?>
                 <tr>
                    <td colspan="2">
                ...
                <td colspan="2">
                    <select name="produtos" id="produtos" multiple style="width: 345px;">
                        ...
                        if (!empty($produtos)) {
                            ...
                            if (!empty($produtos_unico)) {
                                $ref_fab = implode(",", $produtos_unico);
                                $ref_fab = "'".str_replace(",", "','", $ref_fab)."'";
                                
                                $sql = "SELECT produto FROM tbl_produto WHERE referencia_fabrica IN ($ref_fab) AND fabrica_i = $login_fabrica";
                                $res = pg_query($con, $sql);
                                $res_prod = pg_fetch_all($res);
                                if (count($res_prod) > 0) {
                                    $cod_prod = [];
                                    foreach ($res_prod as $key => $value) {
            ...
            <?php if ($login_fabrica == 24) { ?>
                    <tr>
                        <td colspan="2">
                            <label for="produto_input_unico">ReferÍncia ⁄nica</label>
                            <input type="text" name="produto_input_unico" id="produto_input_unico" size="33" class="frm" />
                            <input type="hidden" name="produto_input_hidden_unico" id="produto_input_hidden_unico" size="33" class="frm" />
                            <input type="button" id="add_produto_unico" value="Adicionar" />
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="2">
                            <select name="produtos_unico" id="produtos_unico" multiple style="width: 345px;">
                                <?php
                                if (!empty($produtos_unico)) {
                                    
                                    $condProds_unico = implode(', ', $produtos_unico);
                                    $condProds_unico = "'".str_replace(",", "','", $condProds_unico)."'";
                                    $condProds_unico = str_replace(",' ", ",'", $condProds_unico);
                                    $sqlProds_unico = "SELECT DISTINCT count(produto) AS qtde_familia,
                                                                       referencia_fabrica
                                                        FROM tbl_produto
                                                        WHERE referencia_fabrica IN ($condProds_unico)
                                                        AND fabrica_i = $login_fabrica
                                                        GROUP BY referencia_fabrica";
                                    $qryProds_unico = pg_query($con, $sqlProds_unico);

                                    if (pg_num_rows($qryProds_unico) > 0) {
                                        while ($fetch_unico = pg_fetch_assoc($qryProds_unico)) {
                                            $ref_completa = $fetch_unico['referencia_fabrica'].' - '.$fetch_unico['qtde_familia'].' PRODUTOS ADICIONADOS';
                                            echo '<option value="' , $ref_completa , '" class="' , $fetch_unico['referencia_fabrica'] , '">' , $ref_completa , '</option>';
                                        }
                                    }
                                    
                                    foreach ($produtos_unico as $prod) {
                                        echo '<input type="hidden" name="produto_unico[]" id="' , $prod , '" value="' , $prod , '" >';
                                    }
                                }

                                ?>
                            </select>
                            <input type="button" id="rm_produto_unico" value="Remover" />
                        </td>
                    </tr>
            <?php } ?>

            <?php if ($login_fabrica != 120 and $login_fabrica != 201) { ?>
            <tr>
                <td colspan="2">
