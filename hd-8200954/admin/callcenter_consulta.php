...
function informacoes2(mesmo_dia,dia1,dias2,dias3,mais_dias) {
	var url = "";
        url = "callcenter_consulta_popup.php?mesmo_dia=" + mesmo_dia + '&dia1=' + dia1 + '&dias2=' + dias2 + '&dias3=' + dias3 + '&mais_dias=' + mais_dias;
...
$sql = "SELECT  tbl_callcenter.callcenter                                    ,
		...
		if ($situacao=="PENDENTES"){ $sql .=" LEFT JOIN tbl_providencia        ON tbl_providencia.callcenter = tbl_callcenter.callcenter";}
		$sql .= " WHERE	tbl_callcenter.fabrica = $login_fabrica
...
//exit;
		...
		if(pg_numrows($res) > 0){
			...
			echo "<TD>DATA INTERAÇÂO</TD>";
			echo "<TD>MAIS DE 3 DIAS SEM RESP.</TD>";
			echo "</TR>";
			for($i=0;$i<pg_numrows($res);$i++){
				$resposta_dada     = @pg_result($res,$i,resposta_dada);
				$interacao         = @pg_result($res,$i,interacao);
				$data_chamado      = @pg_result($res,$i,data_chamado);
				$call_chamado      = @pg_result($res,$i,callcenter);
				$abertura_chamado  = @pg_result($res,$i,abertura_chamado);
				$data_interacao    = @pg_result($res,$i,data_interacao);

//	echo "<br>Ultima Int.: $interacao - Aberto: $data_chamado - Chamado $call_chamado";

				if(strlen($interacao) == 0 AND $data_chamado > "3") $resp_dada++;

				if($data_chamado > "3" AND $interacao > "3"){
					$resp_dada++;
				}

				echo "<tr  style='font-size: 10px'>";
				echo "<td><a href='callcenter_press.php?callcenter=$call_chamado'>$call_chamado</a></td>";
				echo "<td>$abertura_chamado</td>";
				if(strlen($data_interacao) > 0) echo "<td>$data_interacao</td>";
				else echo "<td>-</td>";
				if((strlen($interacao) == 0 AND $data_chamado > "3") OR (($data_chamado > "3" AND $interacao > "3"))) echo "<td style='color: #FF0000'>Sem Resp.</td>";
				else echo "<td>OK</td>";
				echo "</tr>";
//				echo "<br>$call_chamado - Data chamado: $abertura_chamado Ultima interação: $data_interacao";
				$data_interacao = '';
			}
			echo "</table>";
		}
	...
	//echo "<br>$sql";
		if($pendentes > 0){
			$total_pendentes = $total_pen - $resp_dada;
//			echo "<br><br><a href= \"javascript: informacoes('$registros','$resp_dada','$total_pendentes')\"><FONT SIZE='2'>Gerar Relatório (Gráfico) Em aberto</FONT></a>&nbsp;&nbsp;";
		}
		...
		if(strlen($chk5) > 0){
			$sql = "select tbl_callcenter.callcenter, 
					...
					(select TO_CHAR(data_gravacao - tbl_callcenter.data,'DD') 
						...
						order by providencia desc limit 1) as diferenca
				WHERE tbl_callcenter.fabrica = $login_fabrica 
				AND tbl_callcenter.excluida IS NOT TRUE AND (1=2 ";
	...
	*/
	...
	//		echo "<br> mesmo dia: $mesmo_dia <br>1= $dia1 <br>2= $dias2 <br>3= $dias3 <br>Mais dias= $mais_dias <br>";

			echo "<br><br><a href= \"javascript: informacoes2('$mesmo_dia','$dia1','$dias2','$dias3','$mais_dias')\"><FONT SIZE='2'>Gerar Relatório (Gráfico) Qtde dias.</FONT></a>&nbsp;&nbsp;";
		}
	}
}
