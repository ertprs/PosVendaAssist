...
<?= (in_array($login_fabrica, [193])) ? '<div id="interacao_os">' : '' ?>
    <span>Interação de OS</span>
...
<?= (in_array($login_fabrica, [193])) ? '</div>' : ''; ?>

<script>
function atendimentosRefresh (params) {
    ...
    $.ajax({
	    ...
	    $(list).find("tr.sem_pendencias").hide();
        },
	    ...
	    let totalOS = data.oss;

	    if (data.qtde > 0 && totalOS.length > 0) {
