...
if (strlen($codigo_posto2) > 0) {
    ...
    $sql = "SELECT SUM(coalesce(pecas,0) + coalesce(mao_de_obra,0) + coalesce(avulso,0)) AS total,
             ...
             WHERE tbl_extrato.fabrica = $login_fabrica
               ...
               AND to_char(data_geracao,'YYYY-MM') IN (SELECT DISTINCT to_char(data_geracao,'YYYY-MM') 
                                                         FROM tbl_extrato 
        ...
        for ($i = 0; $i < pg_numrows($resgrafico); $i++) {
            ...
            if ($posto_anterior <> $posto) {
                ...
                if ($i <> 0) {
                    ...
                    $y++;// usado para indicação de produto
            ...
            while ($data_geracao <> $mes[$x]) { //repete o loop até que o mes seja igual e anda um mes.
            ...
            $x++; //após armazenar o valor no mes correspondente anda 1 mes no vetor
