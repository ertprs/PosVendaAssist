...
function preenche_os(cliente) {
	if (typeof formulario.os != "undefined") {
		...
		os.value = consumidores[cliente]['sua_os'];
	}
}
...
function retorna_dados_posto(cliente) {
	url = "pesquisa_consumidor_callcenter_new_ajax.php?acao=sql&sql=SELECT estado, cidade, codigo_posto, nome, fone, email FROM tbl_posto JOIN tbl_posto_fabrica ON tbl_posto.posto=tbl_posto_fabrica.posto WHERE tbl_posto_fabrica.fabrica=<? echo $login_fabrica; ?> AND tbl_posto.posto="+consumidores[cliente]['posto'];
...
switch($_GET["tipo"]) {
	...
	case "telefone":
		if (strlen($localizar_numeros) >= 8) {
			if ($busca_exata) {
				$busca["O"][] = "AND (tbl_os.consumidor_fone = '$localizar' OR  tbl_os.consumidor_fone = '$localizar_numeros')";
				...
				$busca["O"][] = "AND (tbl_os.consumidor_fone ILIKE '%$localizar%' OR  tbl_os.consumidor_fone ILIKE '%$localizar_numeros')";
...
$sql_busca["C"] = "
	(
		...
		tbl_cidade.estado,
		(SELECT tbl_os.sua_os FROM tbl_os WHERE tbl_os.os=tbl_hd_chamado_extra.os) AS sua_os,
		$busca_produtos_select_C
		serie,
	...
	for($i = 0; $i < pg_num_rows($res); $i++) {
		...
		if ($atendimento) {
			...
			if ($status == "Resolvido" || $status == "Cancelado") {
				if ($os) {
					$os_link = "<a href=\"javascript:retorna_dados_cliente($i); preenche_os($i); retorna_dados_produto($i); retorna_dados_posto($i); window.close();\"  title='Clique neste link para abrir um novo atendimento relativo à OS $sua_os'> $sua_os</a>";
				}
				else {
				...
				if ($os) {
					$os_link = "<a href=\"javascript:opener.document.location = 'callcenter_interativo_new.php?callcenter=$atendimento'; window.close();\"  title='Clique neste link para continuar o atendimento $atendimento'> $os</a>";
				}
				else {
					$os_link = "<font class=semos>SEM ORDEM SERVIÇO</font>";
					$linkos = "";
