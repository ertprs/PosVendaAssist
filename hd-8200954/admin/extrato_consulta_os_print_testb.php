...
*/
	...
	if ($login_fabrica==50 or $login_fabrica == 85 or $login_fabrica == 90){
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++){
		...
		$visita				= trim(pg_result ($res,$i,taxa_visita)) ;
		$qtde_km			= trim(pg_result ($res,$i,qtde_km_calculada));
		...
		if (($i%42==0)&&($i!=0)){
		   ...
		   //MONTA O CABEÇALHO DEPOIS DA QUEBRA DE PAGINA
			...
			if ($login_fabrica==50 or $login_fabrica == 85 or $login_fabrica == 90){
				echo "<TD class='table_line' align='center' nowrap>$qtde_km_calculada &nbsp;</TD>\n";
				if($login_fabrica == 90) {
					echo "<TD class='table_line' align='center' nowrap>".number_format($visita,2,'.','')."</TD>";
					$t_visita += $visita;
				}
			}else
				echo "<TD class='table_line' align='center' nowrap>$pecas</TD>\n";
	...
	if ($login_fabrica==50 or $login_fabrica==52 or $login_fabrica==72 or $login_fabrica == 90){
	...
	if($login_fabrica == 90){
		echo "<TD class='table_line' align='center' nowrap>".number_format($t_visita,2,'.','')."&nbsp;</TD>\n";
	}
...
if ($login_fabrica==11 AND count($os_array)>0){
	...
	if (pg_numrows($res_extrato) > 0) {
		...
		if($login_fabrica == 90){
			echo "<tr>\n";
			echo "<td align='left' nowrap><B>Total Visita:</B></td>\n";
			echo "<td align='right' nowrap><B>".
					number_format( $t_visita, 2, ',', '.')
				 ."</B></td>\n";
			echo "</tr>\n";
		}
