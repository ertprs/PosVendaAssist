...
if (!function_exists("traduz")) {
	function traduz($inputText,$con,$cook_idioma_pesquisa,$x_parametros = null){
		...
		/* Nao buscar mais no Banco. Colocado por Fabio em 30/09/2008 */
		/*
		$sql = "SELECT msg_text
				FROM tbl_idioma
				JOIN tbl_msg USING (idioma)
				WHERE UPPER(idioma_id) = UPPER('$cook_idioma_pesquisa')
				AND   msg_id    = '$inputText' ";
		$x_res = pg_exec($con,$sql);

		if (@pg_numrows($x_res) == 0) {
			# Senao encontrou, procura a tradução para a linguagem padrão.
			$sql = "SELECT msg_text
					FROM tbl_idioma
					JOIN tbl_msg USING (idioma)
					WHERE msg_id    = '$inputText'
					ORDER BY tbl_idioma.sequencia ASC
					LIMIT 1";
			$x_res = pg_exec($con,$sql);
		}
		if (@pg_numrows($x_res) > 0) {
			$inputText = pg_result($x_res,0,0);
			if ($x_parametros){
				if (!is_array($x_parametros)){
					$x_parametros = explode(",",$x_parametros);
				}
				while ( list($x_variavel,$x_valor) = each($x_parametros)){
					$inputText = preg_replace('/%/',$x_valor,$inputText,1);
				}
			}
		}else{
			$sql = "INSERT INTO tmp_traducao_falha (msg_id,idioma)
					VALUES ('$inputText', '$cook_idioma_pesquisa')";
			$x_res = @pg_exec($con,$sql);
		}
		return $inputText;
		*/
	...
	if (strlen($status)>0) {
		...
		switch ($status) {
			...
			$sqlMostra = "SELECT	DISTINCT os,
										...
										consumidor_email
										into TEMP tmp_mostra_vermelho_$posto
							FROM tbl_os
					...
					$sqlMostra = "SELECT *FROM tmp_mostra_vermelho_$posto";
			break;
			case 'amarelo':

			$sqlMostra = "SELECT	DISTINCT os into TEMP tmp_mostra_vermelho_$posto
									FROM tbl_os
									JOIN tbl_os_extra USING(os)
									JOIN tbl_produto USING(produto)
									LEFT JOIN tbl_marca ON tbl_marca.marca = tbl_produto.marca
				...
				$sqlMostra = "SELECT DISTINCT os,
					...
					AND os not in (SELECT os FROM tmp_mostra_vermelho_$posto)";
			break;
			...
			$sqlMostra = "SELECT	DISTINCT os
				...
				$sqlTemp = "SELECT DISTINCT os
										into TEMP tmp_os_amarelo_$posto
									FROM tbl_os
									JOIN tbl_os_extra USING(os)
									JOIN tbl_os_produto using (os)
									JOIN tbl_os_item USING (os_produto)
				...
				$sqlMostra = "SELECT DISTINCT os,
									...
									AND os not in (SELECT os from  tmp_os_amarelo_$posto)
									AND os not in (SELECT os FROM tmp_mostra_vermelho_$posto)";
							//echo nl2br($sqlMostra);
			break;
			case 'azul':
						...
						if ($excluida == "f" || strlen($excluida) == 0) {
							if (strlen ($admin) == 0) {
								if($login_fabrica == 1 AND ($tipo_atendimento == 17 OR $tipo_atendimento == 18) AND $valores_adicionais > 0)
									$html .= "<a href='javascript: if (confirm('".traduz("deseja.realmente.excluir.a.os",$con,$cook_idioma)." $sua_os ?') == true) {
									window.location='$PHP_SELF?excluir=$os'; }'><p id='excluir_$i' border='0'></p></a>";
								else
									$html .= "<a href=\"javascript: if (confirm('".traduz("deseja.realmente.excluir.a.os",$con,$cook_idioma)." $sua_os ?') == true) { if(disp_prompt($os, '$sua_os') == true){alert('123');window.location='$PHP_SELF?excluir=$os';} }\"><img id='excluir_$i' border='0' src='imagens/btn_excluir.gif'></a>";
					...
					if ($excluida == "f" || strlen($excluida) == 0) {
						if($login_fabrica == 1 AND ($tipo_atendimento == 17 OR $tipo_atendimento == 18)){
							...
							if($login_fabrica==19){
								...
								if($login_fabrica<>15){
									if($login_fabrica==11 and strlen($consumidor_email)>0 and $posto==14301){
										...
										if ($login_fabrica == 20 and $posto == 6359) {
											...
											if($consumidor_revenda=='R' and $login_fabrica == 11){
												...
												$html .=  "<a href='javascript: if(confirm('".traduz("os.revenda.devera.ser.fechada.na.tela.fechamento.de.os",$con,$cook_idioma)."') == true) {window.location='os_fechamento.php?sua_os=$sua_os_x&btn_acao_pesquisa=continuar';}'><img id='sinal_$i' border='0' src='/assist/imagens/btn_fecha.gif'></a>";
													...
													$html .=  "<a href=\"javascript: if (confirm('$confirme') == true) {
															fechaOS ($os,'sinal_$i','excluir_$i', 'lancar_$i');}\"><img id='sinal_$i' border='0' src='/assist/imagens/btn_fecha.gif'></a>";
											}
										}
									}
								}else{
...
if ($_POST['btn_acao']=='Gravar') {
	...
	if (strlen($codigo_posto)==0) {
		$msg_erro = "Escolha um posto para fazer a pesquisa";
	} else {
	...
	if (strlen($msg_erro)==0) {
			...
			echo "Clique em um status abaixo para visualizar as Ordens de Serviço do posto<br> $nome_posto";
			...
			$sqlStatus = "SELECT count(*),os
							into TEMP tmp_os_vermelha_$posto
							FROM tbl_os
		...
		//	echo nl2br($sqlStatus);
			$sqlStatus = "SELECT count(*) from tmp_os_vermelha_$posto";
			$resStatus = pg_exec($con,$sqlStatus);
			...
			echo "<td align='left'><font size='2'><b>&nbsp;  <a href=\"javascript: mostrarOs('vermelho',$posto)\">";
			echo "Aguardando Análise" ;/*OS Aguardando Análise*/
			echo " ($vermelho)</a></b></font></td><BR>";
			echo "</tr>";
			
			$sqlStatus = "SELECT DISTINCT os
			into TEMP tmp_amerelo_os_$posto
			FROM tbl_os
			JOIN tbl_os_produto using (os)
			JOIN tbl_os_item USING (os_produto)
			JOIN tbl_produto ON tbl_os.produto = tbl_produto.produto
			...
			AND data_conserto IS NULL
			AND tbl_os_item.peca not in (select peca from tbl_faturamento_item where tbl_faturamento_item.pedido = tbl_os_item.pedido)
			AND os not in (select os from tmp_os_vermelha_$posto)" ;
			$resStatus = pg_exec($con,$sqlStatus);
		
			$sqlStatus = "SELECT * FROM tmp_amerelo_os_$posto";
			$resStatus = pg_exec($con,$sqlStatus);
			if(pg_num_rows($resStatus)>0) {
			...
			echo "<td align='left'><font size='2'><b>&nbsp;  <a href=\"javascript: mostrarOs('amarelo',$posto)\">";
			echo "Aguardando Peça"; /*OS Aguardando Peça*/
			...
			$sqlStatus = "SELECT count(*)
							...
							AND   (excluida IS NULL OR excluida = 'f')
							AND os not in (select os from tmp_os_vermelha_$posto)
							AND os not in (select os from tmp_amerelo_os_$posto)";
							//echo nl2br($sqlStatus);
			$resStatus = pg_exec($con,$sqlStatus);
			if(pg_num_rows($resStatus)>0) {
			...
			echo "<td align='left'><font size='2'><b>&nbsp;  <a href=\"javascript: mostrarOs('rosa',$posto)\">";
			echo "Aguardando Conserto"; /*OS Aguardando Conserto*/
			...
			echo "<td align='left'><font size='2'><b>&nbsp;  <a href=\"javascript: mostrarOs('azul',$posto)\">";
			echo "Os Consertada"; /*OS Consertada*/
			...
			echo "<tr height='18'>";
			echo "<td width='18'></td>";
			echo "<td align='left'><font size='2'><b>&nbsp;  <a href=\"javascript: mostrarOs('todas',$posto)\">";
			echo "Todas";; /*Todas*/
...
<script language='javascript'>
	...
	function mostrarOs(status,posto) {
		window.location = 'status_os_posto.php?status='+status+'&posto='+posto;
	}
...
<TR>
	...
	<IMG src="imagens_admin/btn_pesquisar_400.gif" onClick="javascript: if (document.frm_pesquisa.btn_acao.value == '') { document.frm_pesquisa.btn_acao.value = 'Gravar'; document.frm_pesquisa.submit(); } else { alert('Aguarde') } " style="cursor:pointer " alt="Preencha as opções e clique aqui para pesquisar"></TD>
</TR>
</TABLE>
</FORM>
<?
