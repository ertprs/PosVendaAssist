...
<table width='500' align='center' border='0' cellspacing='2' cellpadding='2' bgcolor='#d9e2ef'>
<tr class='topo'>
	<td colspan='4'>OS auditadas</td>
</tr>
<TR class='menu_top'>
    <TD style="width: 10px">&nbsp;</TD>
	<TD width="210px;" align='left'><input type='radio' name='data_tipo' value='digitacao' <?if(strlen($data_tipo)==0 or $data_tipo =='digitacao') echo "CHECKED"; ?>>Data Digitação</TD>
...
if (strlen($btn_acao) > 0 AND strlen($msg_erro) == 0){
	...
	if ($qtde_registros > 0) {
		...
		if ($qtde_registros < $numero_max_registros){
			...
			if($login_fabrica==3)
				echo "<td nowrap><font color='#FFFFFF'>TÉCNICO RESPONSÁVEL</FONT></td>";
			echo "</tr>";
		}else{
		...
		if($login_fabrica==3)
			fputs ($fp,"<td >TÉCNICO RESPONSÁVEL</td>");
		fputs ($fp,"</tr>");
		...
		//	$produto_descricao    = substr($produto_descricao,0,15);/*hd 4790 011007 takashi*/
			...
			if ($qtde_registros < $numero_max_registros){
				...
				if($login_fabrica==3)
					echo "<td nowrap align='left'>$tecnico</td>";
				echo "</tr>";
			}
			...
			if($login_fabrica==3)
				fputs ($fp,"<td bgcolor='$cor' >$tecnico</td>");
			fputs ($fp,"</tr>");
		}
		fputs ($fp,"</table>");
		fputs ($fp,"</body>");
		...
		echo `rm xls/relatorio_auditoria_previa-$login_fabrica-$login_admin.xls`;
		//echo `htmldoc --webpage --size 297x210mm --fontsize 11 --left 8mm -f xls/relatorio_auditoria_previa-$login_fabrica-$login_admin.xls /tmp/assist/relatorio_auditoria_previa-$login_fabrica-$login_admin.htm`;

		echo `cp /tmp/assist/relatorio_auditoria_previa-$login_fabrica-$login_admin.htm xls/relatorio_auditoria_previa-$login_fabrica-$login_admin.xls `;
