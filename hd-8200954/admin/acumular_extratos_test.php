...
if($btn_acao == "gravar"){
	...
	if(strlen($msg_erro)==0){
		...
		$trinta_dias = pg_fetch_result($resX,0,0);
		...
		*/
		...
		if (pg_num_rows($res)>0){
			...
			for($x=0; pg_num_rows($res)>$x;$x++){
				...
				$total = pg_fetch_result($res, $x,'total');
				...
				$msg_erro .= pg_errormessage($con);
				if (strlen($msg_erro) == 0) {
					$res2 = pg_query($con,"ROLLBACK TRANSACTION");
					$sql = "SELECT protocolo FROM tbl_extrato WHERE extrato = $extrato;";
					$res2 = @pg_query($con,$sql);
					$extrato_mostra = @pg_fetch_result($res2, 0, protocolo);
					$msg.= "<BR>";
