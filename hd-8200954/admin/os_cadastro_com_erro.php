...
if ($btn_troca == "trocar" && strlen($msg_erro) == 0) {
	...
	if ($login_fabrica != 51 && $login_fabrica != 81) {
		if (strlen($_POST['situacao_atendimento']) == 0) $msg_erro = '<br />solicite a situacao de atendimento'; 
...
//		HD 79774 - Paulo César 10/03/2009 sempre gera pedido para fabrica 3
	...
	elseif ($troca_garantia_produto == -2) {
				...
				if(($login_fabrica == 3 or $login_fabrica==45 or $login_fabrica==35 OR $login_fabrica==25) AND pg_num_rows($res)==1 ) {
					...
					if($login_fabrica == 3){
						$sql = "UPDATE tbl_os SET
							...
							data_conserto           = CURRENT_TIMESTAMP
							WHERE os = $os AND fabrica = $login_fabrica";
					} else if ($login_fabrica == 35 || $login_fabrica == 11 || $login_fabrica == 81 || $login_fabrica == 72) {
						//HD 65952
						//HD 163061
		...
		if (strlen ($msg_erro) == 0) {
            if ($login_fabrica <> 3 and $login_fabrica <> 35 AND $login_fabrica <> 11 AND $login_fabrica <> 81 AND $login_fabrica <> 72) {
	...
	<tr>
		...
		<tr valign="top">
			...
			if ($login_fabrica == 50) {?>
				...
				<td nowrap>
					<font size="1" face="Geneva, Arial, Helvetica, san-serif">Número de Série</font>
					<br />
			...
			<td nowrap>
				...
				<br />
				<input class="frm" type="text" name="produto_serie" id="produto_serie" size="15" <? if ($login_fabrica == 35) {?> maxlength="12" <?} else {?> maxlength="20" <?}?> value="<?=$produto_serie?>" <? if ($login_fabrica == 50) {?>onchange="this.value = this.value.toUpperCase();"<?}?> /><?php
				if ($login_fabrica == 25) {?>
...
*/
	...
	if(pg_num_rows($res)>0) {
		...
		echo "<font style='font-size:12px'>Produto já trocado!<br> Preencha o formulário abaixo para trocar o produto novamente! </font>";
		echo "</td>";
		echo "</tr>";
		echo "</table>";
	}
...
?>
		...
		<tr class='subtitulo'>
		<td colspan='100%'>
		Responsável pela troca: <b><?=$login_login?></b> | OS <a href="os_press.php?os=<? echo $os; ?>" target="_blank" title="Cliique para ver a OS em outra janela" style='color:#FFFFFF; text-decoration:underline; '><b><?=$sua_os?></b></a>
		</td>
		</tr>
		<tr>
		<td align='center'><?php
		...
		if ($login_fabrica == 81 ) {
			...
			if (pg_num_rows($resressarcimento)>0) {
				...
				$ressarcimento_troca = pg_result($resressarcimento,0,1);
				if ($ressarcimento_troca == 't') {
					echo "<br><font color='#FFFFFF'>Foi Aberto o Chamado $hd_chamado_troca solicitando o ressarcimeno deste produto, para efetivar o ressarcimento complete os campos abaixo</font>";
				}
			}
...
<?
		if (in_array($login_fabrica,array(3,45,35,11,25,51,14,66,24,19,81,40,80,72,10)) or $login_fabrica > 80) {
			if ($login_fabrica == 3 or $login_fabrica == 81) {
			...
			if (pg_num_rows($res) > 0) {
				echo "<select class='Caixa' style='width: 400px;' name='familia_troca' id='familia_troca' onChange='listaProduto(this.value,\"$aux_marca\");valida_campo_ant();'>\n";
				echo "<option value=''>ESCOLHA</option>\n";
			...
			echo "<td align='left'>";
			echo "<select name='troca_garantia_produto'  class='Caixa' style='width: 400px;'"; if ($login_fabrica == '81') { echo "onchange='javascript: if (this.value == \"-1\") {document.getElementById(\"dados_ressarcimento\").style.display = \"block\"} else {document.getElementById(\"dados_ressarcimento\").style.display = \"none\"}'";} echo ">";
			...
			echo "<td align='left'><input type='text' name='ri' value='" . $_POST[ri] . "' size='10' maxlength='10' class='Caixa'>";
			echo "</td>";
			echo "</tr>";
			echo "</table>";
			echo "<b>Observação para nota fiscal</b><br><textarea name='observacao_pedido' cols='100' rows='3' class='Caixa'>".  $_POST["observacao_pedido"] . "</textarea><br>";
			...
			if($login_fabrica <> 51 and $login_fabrica <> 81){
				...
				echo "<td align='left'><b>Efetuar Troca Por</td>";
			}
			echo "</tr>";
			...
			echo "<input type='radio' name='setor' $revenda_checked value='Revenda'> Revenda<br>";
			echo "<input type='radio' name='setor' $carteira_checked value='Carteira'> Carteira<br>";
			echo "<input type='radio' name='setor' $sac_checked value='SAC'> SAC<br>";
			echo "<input type='radio' name='setor' $procon_checked value='Procon'> Procon<br>";
			echo "<input type='radio' name='setor' $sap_checked value='SAP'> SAP<br>";
			echo "</td>";
			...
			if ($login_fabrica == 81) {
				...
				echo "<div id='dados_ressarcimento' style='display:$display';>
						<table border='0'>
							<tr>
								<td colspan='3'><font size='+1'>Informações Bancarias para o Ressarcimento</font></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type='text' name='favorecido_conta' class='frm' value='$favorecido_conta'></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type='30' name='conta' id='conta' class='frm' value='$conta'></td>
								<td>R$ <input type='30' name='valor' id='valor' class='frm' value='$valor'></td>
							</tr>
						</table>
				</div>";
			}
