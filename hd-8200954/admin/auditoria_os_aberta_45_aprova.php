	...
	for ($x = 0; $x < $qtde_os; $x++) {
		...
		if (strlen($xxos) > 0 AND strlen($msg_erro) == 0) {
			...
			if (pg_numrows($res_os)>0){
				...
				if ($status_da_os == 140 || $status_da_os == 141) {

					if ($select_acao == "142") {
						$sql = "INSERT INTO tbl_os_status
								(os,status_os,data,observacao,admin)
								VALUES ($xxos,142,current_timestamp,'OS acima de 45 dias ainda aberta. Liberada para Alteração.',$login_admin)";
						$res = pg_exec($con,$sql);
					...
					if ($select_acao == "143") {
						$sql = "INSERT INTO tbl_os_status
								(os,status_os,data,observacao,admin)
								VALUES ($xxos,143,current_timestamp,'OS acima de 45 dias ainda aberta. Cancelada pela fábrica.',$login_admin)";
						$res = pg_exec($con,$sql);
						$msg_erro .= pg_errormessage($con);
					}
...
if ($btn_pesquisa == 'Pesquisar') {//FABRICA 15
...
$title = "Auditoria de OS aberta a mais de 45 dias";
...
<form name="frm_pesquisa" method="post" action="<?echo $PHP_SELF?>">
	...
	<TABLE width="700" align="center" border="0" cellspacing='0' cellpadding='0' class='PesquisaTabela'>
		...
		<caption>Relatório de Auditoria de OS aberta a mais de 45 dias</caption>
...
if (strlen($posto) > 0 or strlen($btn_pesquisa) > 0) {
	...
	if ($status_os == '140') $aprovacao ="140";
	if ($status_os == '142') $aprovacao ="142";
	...
	$sql =  "SELECT 
			...
			SELECT  
				...
				(SELECT tbl_status_os.descricao FROM tbl_os_status JOIN tbl_status_os USING(status_os) WHERE tbl_os.os = tbl_os_status.os AND status_os IN ($sql_tipo) AND tbl_os_status.fabrica_status = $login_fabrica
	...
	if (pg_numrows($res) > 0) {
		...
		for ($x = 0; $x < pg_numrows($res); $x++) {
			...
			echo "<tr bgcolor='$cor' id='linha_$x'>";
				echo "<td align='center' width='0'>";
					if ($status_os == 140 || $status_os == 141 || $status_os == 142){
		...
		echo "<tr>";
			...
			echo "<select name='select_acao' size='1' class='frm'>";
				...
				echo "<option value='142'"; if ($_POST["select_acao"] == "142") echo " selected"; echo ">LIBERAR ALTERAÇÃO</option>";
				if ($login_fabrica == 3) {
					echo "<option value='143'"; if ($_POST["select_acao"] == "143") echo " selected"; echo ">CANCELAR OS</option>";
				}
			echo "</select>";
