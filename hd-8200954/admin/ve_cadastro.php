...
$fabrica_usa_S3 = ($login_fabrica >= 70 and file_exists('/aws-amazon/sdk/sdk.class.php'));
$fileMaxSize    = in_array($login_fabrica, array(42, 81, 114)) ? 4*1048576 : 2097152; // 2 * 1048576 = 2MiB
...
if ($fabrica_usa_S3) {
	...
	$s3 = new anexaS3('ve', (int) $login_fabrica, $comunicado);
}
...
if (trim($btn_acao) == "gravar") {
	...
	if($login_fabrica == 15 AND $tipo == "Vídeo"){
		$link       = trim($_POST['link']);
		if(empty($link)){
			$msg_erro .= "Informe o link do vídeo <br>";
			$link = 'null';
		}
	}
	...
	if($aux_tipo == "null"){
		$msg_erro = "Escolha um tipo de comunicado";
	}
...
//Quando selecionando o 'Aviso Posto Unico' faz a validacão para saber se entrou com os dados do posto
	...
	if ((strlen($linha)==0 and strlen($descricao)==0) or
		...
		if (strlen($produto_referencia) > 0){
			$sql = "SELECT produto, referencia FROM tbl_produto JOIN tbl_linha ON (tbl_linha.linha = tbl_produto.linha) WHERE (referencia = '$produto_referencia' OR  referencia_fabrica = '$produto_referencia') AND tbl_linha.fabrica = $login_fabrica; ";
			$res = pg_exec ($con,$sql);
	...
	if (count($replicar)>0 AND $multiplo=='muitos' AND strlen($msg_erro) == 0){
	...
	if (strlen($linha)==0 AND $produto=='null' AND $numero_multiplos==0 AND count($replicar) == 0) {
	...
	if (strlen ($msg_erro) == 0) {
		...
		if (strlen($arquivo["tmp_name"]) > 0 && $arquivo["tmp_name"] != "none" && empty($link)){

			// Verifica o MIME-TYPE do arquivo
			if (!preg_match("/\/(rtf|ppt|txt|pdf|x-pdf|acrobat|msword|doc|text|vnd.msword|vnd.ms-word|winword|word|x-msw6|x-msword|zz-winassoc-doc|pjpeg|jpeg|jpg|png|gif|bmp|pps|vnd.ms-excel|xls|richtext|plain|vnd.ms-powerpoint|zip|x-zip-compressed)$/", $arquivo["type"])){
...
//echo $arquivo["tmp_name"];
				...
				if (strlen($msg_erro) == 0) {
					...
					// Include the S3 SDK for TELECONTROL if exists
					if ($fabrica_usa_S3) {
						//$s3->uploadFileS3($comunicado, $arquivo);
						...
						if ($s3->uploadFileS3($comunicado, $arquivo)) {
							...
							$msg_erro .= "O arquivo não foi enviado!!! " . $s3->_erro; // . $erroS3;
						}
						...
						if (copy($arquivo["tmp_name"], $imagem_dir)) {
							...
							$msg_erro .= "O arquivo não foi enviado!!!";
						}
					}
					}
				}
...
if (strlen($comunicado) > 0) {
	...
	if (pg_numrows ($res) > 0) {
		...
		$link                   = trim(pg_result($res,0,video));
...
<tr>
	...
	<td align='left'>
<?php
	$tipo_comunicado = array('Vista Explodida', 'Esquema Elétrico');
		if ($login_fabrica == 14 || $login_fabrica == 66) {//HD 265319 - MAXCOM
			$tipo_comunicado[] = "Apresentação do Produto";
			$tipo_comunicado[] = "Árvore de Falhas";
			$tipo_comunicado[] = "Boletim Técnico";
			$tipo_comunicado[] = "Descritivo Técnico";
			$tipo_comunicado[] = "Estrutura do Produto";
			$tipo_comunicado[] = "Informativo Técnico";
			$tipo_comunicado[] = "Politica de Manutenção";
			$tipo_comunicado[] = "Teste Rede Autorizada";
			$tipo_comunicado[] = "Manual de Trabalho";
		}
		if ($login_fabrica == 66) {//HD 265319 - MAXCOM
			$tipo_comunicado[] = 'Manual De Produto';
			$tipo_comunicado[] = 'Versões EPROM';
		}
		if ($login_fabrica <> 3){ // HD 17700 18182
				$tipo_comunicado[] = 'Alterações Técnicas';
			if ($login_fabrica <> 11){ // HD 54608
				$tipo_comunicado[] = 'Manual Técnico';
			} else {
				$tipo_comunicado[] = 'Manual do Usuário';
				$tipo_comunicado[] = 'Apresentação do Produto';
				$tipo_comunicado[] = 'Informativo técnico';
			}
		}
		if ($login_fabrica == 45) {//HD 231820
			$tipo_comunicado[] = 'Foto';
			$tipo_comunicado[] = 'Vídeo';
		}
		if ($login_fabrica == 15) {
			$tipo_comunicado[] = 'Diagrama de Serviços';
		}
		if ($login_fabrica == 19) {
			$tipo_comunicado[] = 'Peças de Reposição';
			$tipo_comunicado[] = 'Produtos';
			$tipo_comunicado[] = 'Lançamentos';
			$tipo_comunicado[] = 'Informativos';
			$tipo_comunicado[] = 'Promoções';
			$tipo_comunicado[] = 'Peças Alternativas';
		}

		if ($login_fabrica == 3) {
			$tipo_comunicado[] = 'Atualização de Software';
		}
		echo array2select('tipo', 'tipo', $tipo_comunicado, $tipo, ' class="frm"', 'ESCOLHA');
?>
	</td>
</tr>
...
<tr>
	...
	<td align='left'>
	Ativo<INPUT TYPE="radio" NAME="ativo" value='t' checked >
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	Inativo<INPUT TYPE="radio" NAME="ativo" value='f' <? if ($ativo == 'f'){ echo "checked"; } ?> >
	<?
	if($login_fabrica == 11) {
	?>
			...
			<table border="0" style="float: right">
				<tr>
					<td>Exibir na Tela de Entrada do Site</td>
					<td><input type='checkbox' style="width: 10px;" name='obrigatorio_site' value='t' <? if ($obrigatorio_site == "t") echo "checked"; ?> class='frm'></td>
				</tr>
			</table>
...
?>
<tr id="anexo" style="display:<?=$display_anexo?>;">
...
<tr id="video" style="display:<?=$display_video?>;">
	<td align="right">Link do Vídeo *</td>
	<td colspan='2' align="left"><input type='text' name='link' size='65' value="<?=$link?>" class='frm'></td>
</tr>
<? if (strlen($comunicado) > 0 AND strlen($extensao) > 0) {

	$linkVE = "../comunicados/$comunicado." . $extensao; 

	if ($fabrica_usa_S3 /*and !file_exists($linkVE)*/) {
		$linkVE = $s3->url;
		if (is_bool($linkVE))
			$linkVE = '#'; //Deshabilita o link se não houver.
	}
?>
<tr>
	...
	<td colspan='2' align="left">
		...
		<input type="button" value="Abrir Arquivo" onclick="window.open('<?=$linkVE?>');" />
	...
	<td align='left'>
		<select class='frm' name='psq_tipo'>
			...
			if ($login_fabrica == 15){?>
				...
				<option value='Vídeo' <? if ($tipo == "Vídeo")   echo " SELECTED ";?>>Vídeo</option>
			<?}?>
			<?if ($login_fabrica == 19){?>
