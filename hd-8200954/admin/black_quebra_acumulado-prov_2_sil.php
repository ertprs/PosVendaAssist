...
if ($relatorio == "gerar" ) {	
	...
	FROM 
		(
			...
			JOIN tbl_produto		ON tbl_os.produto	= tbl_produto.produto  and tbl_produto.produto in (9785, 9786, 9788)
	...
	if($todas == 3  ) {
		...
		WHERE tbl_os_item.servico_realizado IN (62, 90,115) and tbl_produto.produto in(9785, 9786, 9788)
		;
	...
	if( $todas == 6 ) {
		...
		INTO TEMP TABLE x
		FROM tbl_os_item
		JOIN tbl_os_produto ON tbl_os_item.os_produto = tbl_os_produto.os_produto and tbl_produto.produto in(9785, 9786, 9788)
		JOIN $tmp_table     ON $tmp_table.os = tbl_os_produto.os
	...
	$sql = "
		...
		INTO TEMP TABLE y
		FROM tbl_os_item
		JOIN tbl_os_produto ON tbl_os_item.os_produto = tbl_os_produto.os_produto and tbl_os_produto.produto in(9785, 9786, 9788)
		JOIN $tmp_table     ON $tmp_table.os = tbl_os_produto.os
	...
	if($todas == 1) {
		...
		if ($todas == 2 OR $todas == 3 ) { 
			...
			INTO TEMP TABLE z
			FROM $tmp_table
			JOIN tbl_os_produto ON $tmp_table.os = tbl_os_produto.os and tbl_produto.produto in(9785, 9786, 9788)
  ...
  if (strtoupper($btnacao) == "LISTAR" and @pg_numrows($res)>0)  {
	...
	*/
	...
	SELECT	tbl_produto.descricao AS nome, 
			...
			COUNT(*)              AS ocorrencia
	FROM $tmp_table xos
	JOIN tbl_produto 	ON xos.produto			= tbl_produto.produto and tbl_produto.produto in(9785, 9786, 9788)
	JOIN tbl_linha 		ON tbl_produto.linha	= tbl_linha.linha
	GROUP BY 
			tbl_produto.descricao, 
			tbl_produto.voltagem, 
