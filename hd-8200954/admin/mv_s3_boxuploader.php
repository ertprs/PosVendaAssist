...
if ($_REQUEST['ajax_listar_anexos']) {
	try {
		...
		$anexos = $s3->get_object_list('br.com.telecontrol.os-anexo', ['prefix' => "{$fabrica}/{$ano}/{$mes}/{$os}_"]);
...
if ($_REQUEST['ajax_os_processada']) {
	try {
		...
		if($fabrica != 169){
			$cond = "AND s3_os_nf_key IS NULL";
		}
		$sql = "
			UPDATE tbl_os SET s3_os_nf_key = 'ok'
...
if ($_REQUEST['ajax_transferir_arquivo']) {
	...
	$sql_da = "SELECT data_abertura FROM tbl_os WHERE os = $os";
	$res_da = pg_query($con, $sql_da);

	list($ano, $mes, $dia) = explode('-', pg_fetch_result($res_da, 0, 'data_abertura'));
	...
	foreach ($anexos as $anexo) {
		try {
			...
			$s3->copy_object(
				['bucket' => 'br.com.telecontrol.os-anexo', 'filename' => $s3_caminho . '/' . $nome.'.'.$extensao],
			...
			$insert = "
					...
					VALUES
					('{$tdocs_id}', {$fabrica}, 'os', 'ativo', '{$obs}', 'os', {$os})
				";
...
if ($_POST) {
	...
	$sqlFila = "
SELECT fabrica, os, data_abertura AS data
FROM tbl_os
WHERE fabrica = {$fabrica}
AND s3_os_nf_key IS NULL
AND tbl_os.data_digitacao BETWEEN '2019-07-01 00:00' and '2019-12-31 23:59:59'
ORDER BY os DESC
";
	if($fabrica == 169){
		$sqlFila = "SELECT tbl_os.os,tbl_os.fabrica,data_abertura AS data FROM tbl_os LEFT JOIN tbl_tdocs ON tbl_os.os = tbl_tdocs.referencia_id AND tbl_tdocs.fabrica = 169 WHERE tbl_os.fabrica = 169 AND tbl_os.data_digitacao BETWEEN '2017-01-01 00:00' and '2017-04-30 23:59:59' AND tbl_tdocs.tdocs IS NULL;";
	}
