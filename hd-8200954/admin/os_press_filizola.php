...
if (strlen ($os) > 0) {
	...
	if (pg_numrows ($res) == 1) {
		...
		if (pg_numrows($resAbriu) > 0){
			...
			$sqlAbriu = "SELECT nome from tbl_posto where posto = $posto";
			$resAbriu = pg_exec ($con,$sqlAbriu);
			if (pg_numrows($resAbriu) > 0){
				$abriu_os = pg_result($resAbriu,0,nome);
			}
		}
	}
}
...
<TR>
	<TD class="titulo" colspan= '2'>CODIGO DO POSTO</TD>
	<TD class="titulo" colspan= '2'>NOME DO POSTO</TD>
</TR>
<TR>
	<TD class="conteudo" colspan= '2'><? echo $codigo_posto ?></TD>
	<TD class="conteudo" colspan= '2'><? echo $posto_nome ?></TD>
</TR>
...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align="center" align="center">
<TR>
...
<?
	...
	for ($i = 0 ; $i < $total ; $i++) {
		...
		if (strlen($pedido) > 0) {
			...
			if(strlen($pedido_cliente2) > 0) {
					$sqlx  = "SELECT DISTINCT trim(tbl_faturamento.nota_fiscal) As nota_fiscal
					FROM    tbl_faturamento
					JOIN    tbl_faturamento_item USING (faturamento)
					WHERE   tbl_faturamento_item.pedido    = $pedido_cliente2
					AND     tbl_faturamento_item.peca = $peca;";
					$resxx = pg_exec ($con,$sqlx);
...
<p>
<? //HD 254633
	$sql_tipo = "SELECT tbl_posto_fabrica.tipo_posto 
		    FROM tbl_posto_fabrica
		   WHERE tbl_posto_fabrica.posto=$posto
		   AND tbl_posto_fabrica.tipo_posto = 214
		   OR tbl_posto_fabrica.tipo_posto = 215";
	$res_tipo = pg_exec($con,$sql_tipo);
	$total_tipo = pg_numrows($res_tipo);
?>
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align="center">
...
<TR>
	...
	<?
		if ($total_tipo > 0) { //HD 254633
			echo "<TD class='titulo'>REPRESENTANTE</TD>";
		}
	?>
	
	<TD class="titulo">CLASS. OS</TD>
	<TD class="titulo">NATUREZA</TD>
...
<TR>
	...
	<TD class="conteudo"><? echo $tipo_atendimento_descricao ?></TD>
...
<div id='container' style="text-align:center;">
	<div id="contentleft2" style="width: 150px; text-align:center;">
		&nbsp;
	</div>
