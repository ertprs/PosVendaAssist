	...
	$sql = "SELECT  tbl_os_extra.linha           ,
			...
			JOIN (
				...
				JOIN (
					...
					AND    (tbl_extrato.posto = $posto OR tbl_extrato.posto IN (SELECT posto FROM tbl_posto_linha WHERE distribuidor = $posto))
				...
				AND tbl_os_extra.linha IN (SELECT DISTINCT linha FROM tbl_posto_linha WHERE tbl_posto_linha.distribuidor = $posto)
