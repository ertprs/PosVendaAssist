...
$title = "Relatório de peças atendidas em garantia";
...
if(strlen($btn_acao)>0){
	...
	if(strlen($posto_codigo)>0){
		...
		if(pg_numrows($res)>0){
			...
			$cond_fat = " tbl_faturamento.posto = $posto ";
			$cond_ped = " tbl_pedido.posto = $posto ";
		}else{
...
/*IGOR - HD 7507 - NÃO SOMAVA CORRETAMENTE OS VALORES DAS PEÇAS*/

	$sql = "
		/*1º ENCONTRAR OS FATURAMENTOS DO POSTO OU DE TODOS*/
		SELECT  tbl_faturamento.faturamento,
			tbl_faturamento.posto,
			tbl_faturamento_item.pedido,
			tbl_faturamento_item.peca,
			tbl_faturamento_item.qtde
		into temp table tmp_fat_lenoxx1
		FROM tbl_faturamento_item 
		JOIN tbl_faturamento on tbl_faturamento.faturamento = tbl_faturamento_item.faturamento 
		...
		CREATE INDEX tmp_fat_lenoxx1_faturamento ON tmp_fat_lenoxx1(faturamento);	
		CREATE INDEX tmp_fat_lenoxx1_pedido ON tmp_fat_lenoxx1(pedido);	


		/*AGORA GERA OS DADOS DO RELATÓRIO COM OS PEDIDOS E O POSTO*/

		SELECT tbl_posto_fabrica.codigo_posto, 
			tbl_posto_fabrica.posto, 
			tbl_posto.nome, 
			sum(tmp_fat_lenoxx1.qtde) as qtde_faturada 
		FROM tmp_fat_lenoxx1
		JOIN tbl_pedido on tmp_fat_lenoxx1.pedido = tbl_pedido.pedido AND tbl_pedido.tipo_pedido = 84 
		JOIN tbl_posto on tbl_posto.posto = tbl_pedido.posto 
		...
		GROUP BY tbl_posto_fabrica.codigo_posto, 
			tbl_posto_fabrica.posto, 
			tbl_posto.nome 
		ORDER by sum(tmp_fat_lenoxx1.qtde) desc;";
	...
	if (pg_numrows($res) > 0) {
		...
		echo "<td colspan='3' height='20'><font size='2'>TOTAL DE PEÇAS ATENDIDAS EM GARANTIA</font></td>";
		echo "</tr>";
...
/*$sql = "SELECT tbl_peca.referencia as peca_referencia,
			...
			FROM tbl_faturamento_item
			JOIN tbl_faturamento on tbl_faturamento.faturamento = tbl_faturamento_item.faturamento
			JOIN tbl_os_item ON tbl_os_item.faturamento_item = tbl_faturamento_item.faturamento_item
			JOIN tbl_pedido           on tbl_faturamento_item.pedido = tbl_pedido.pedido
			AND  tbl_pedido.tipo_pedido = 84
			JOIN tbl_pedido_item      on tbl_pedido_item.peca = tbl_faturamento_item.peca
			AND  tbl_pedido_item.pedido = tbl_faturamento_item.pedido
...
/*IGOR - HD 7507 - NÃO SOMAVA CORRETAMENTE OS VALORES DAS PEÇAS*/
	$sql = "
		...
		SELECT  tbl_faturamento.faturamento,
			...
			tbl_faturamento_item.peca,
			tbl_faturamento_item.qtde
		into temp table tmp_fat_lenoxx2
		FROM tbl_faturamento_item 
		JOIN tbl_faturamento on tbl_faturamento.faturamento = tbl_faturamento_item.faturamento 
		WHERE tbl_faturamento.fabrica = $login_fabrica 
			AND tbl_faturamento.emissao between '$xdata_inicial' and '$xdata_final'
		...
		CREATE INDEX tmp_fat_lenoxx2_faturamento ON tmp_fat_lenoxx2(faturamento);	
		CREATE INDEX tmp_fat_lenoxx2_pedido ON tmp_fat_lenoxx2(pedido);

		SELECT tbl_peca.referencia as peca_referencia, 
			tbl_peca.descricao as peca_descricao, 
			sum(tmp_fat_lenoxx2.qtde) as qtde_faturada, 
			tmp_fat_lenoxx2.nota_fiscal, 
			to_char(tmp_fat_lenoxx2.emissao,'DD/MM/YYYY') as data_emissao, 
			to_char(tmp_fat_lenoxx2.saida,'DD/MM/YYYY') as data_saida, 
			tbl_pedido.pedido 
		FROM tmp_fat_lenoxx2
		JOIN tbl_os_item ON tbl_os_item.faturamento_item = tmp_fat_lenoxx2.faturamento_item 
		JOIN tbl_pedido on tmp_fat_lenoxx2.pedido = tbl_pedido.pedido AND tbl_pedido.tipo_pedido = 84 
		JOIN tbl_peca on tmp_fat_lenoxx2.peca = tbl_peca.peca 
		JOIN tbl_posto on tbl_posto.posto = tbl_pedido.posto 
		JOIN tbl_posto_fabrica on tbl_posto_fabrica.posto = tbl_posto.posto AND tbl_posto_fabrica.fabrica = $login_fabrica
		WHERE 	tbl_pedido.fabrica=$login_fabrica
			AND tbl_pedido.pedido_blackedecker notnull 
		...
		GROUP BY tbl_peca.referencia, 
			tbl_peca.descricao, 
			tmp_fat_lenoxx2.nota_fiscal, 
			tmp_fat_lenoxx2.emissao, 
			tmp_fat_lenoxx2.saida, 
			tbl_pedido.pedido 
		ORDER by qtde_faturada desc;";
...
$assunto       = "LENOXX - Relatorio pecas faturadas";
$corpo.="<br>OS: $os \n";
