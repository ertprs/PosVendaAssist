...
if ($arquivos != "" AND strlen($posto) > 0) {
	...
	$sql = "SELECT nome, cidade, estado, posto , fabricantes, descricao, email, linhas, funcionario_qtde, os_qtde
	...
	if(pg_numrows($res) > 0){
		for($i = 0; $i < pg_numrows($res); $i++){
			...
			echo "</tr>";
			echo "<tr>";
				echo "<td><b>Linhas:</b> $linhas</td>";
			echo "</tr>";
			echo "<tr>";
				echo "<td><b>Qtde de funcionários:</b> $funcionario_qtde</td>";
			echo "</tr>";
			echo "<tr>";
				echo "<td><b>Qtde de OS mês:</b> $os_qtde</td>";
			echo "</tr>";
			echo "</table>";
			...
			foreach($xarquivos as $xlistar){
				...
				if(substr_count($xlistar, $posto) > 0 AND ($testa_1 == $xlistar OR $testa_2 == $xlistar OR $testa_3 == $xlistar)) {
					echo "<p align='center'><img src='../credenciamento/fotos/$xlistar'></p>";
					$z++;
				}
			}
		}
