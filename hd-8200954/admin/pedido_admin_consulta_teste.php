...
if (strlen($_GET["cancelar"])>0 AND strlen($_GET["pedido"])>0) {
	...
	if($login_fabrica == 51 or $login_fabrica == 81 or $login_fabrica == 2 or $login_fabrica == 10 or $login_fabrica == 80 or $login_fabrica == 95) {
	...
	if($cancelar <> "todo"){
		...
		if($qtde_cancelar > $qtde_aux){
			$msg_erro = "Quantidade informada maior do que a quantidade de itens cadastrados";
		}
	}
...
*/
			if (( $login_fabrica == 81 or $login_fabrica == 51 or $login_fabrica == 10 or $login_fabrica == 95) and ($login_admin == $ronaldo or $login_fabrica == 95)) {
				$sql = "SELECT  PE.pedido      ,
						        PE.distribuidor,
						        PI.pedido_item ,
						        PI.peca        ,
			...
			$sql = "SELECT  PI.pedido_item,
				...
				AND     PE.fabrica     = $login_fabrica";
				if ($login_fabrica <> 2 and $login_fabrica <> 51 and $login_fabrica <> 81 and $login_fabrica<>10 and $login_fabrica<>95) {
			...
			if (pg_num_rows ($res) > 0) {
				...
				if(strlen($msg_erro)==0){
					if(strlen($distribuidor)>0){
						if($login_fabrica <>51 and $login_fabrica <>81 and $login_fabrica<>10 and $login_fabrica <>2 and $login_fabrica <> 95){
							$sql  = "SELECT fn_pedido_cancela($distribuidor,$login_fabrica,$pedido,$peca,$aux_motivo)";
							$resY = @pg_query ($con,$sql);
							$msg_erro .= pg_errormessage($con);
						}else {
					...
					/*	$sql  = "SELECT trim(tbl_faturamento.nota_fiscal) As nota_fiscal ,
						...
						*/
						...
						if ($pendente == 0) {
							...
							if($login_fabrica==2 or $login_fabrica==95){
...
if(isset($_GET['cancelaTudo']) and !empty($pedido)){
	...
	if(pg_numrows($res) > 0){
		for($i = 0; $i < pg_numrows($res); $i++){
			...
			$sql = "UPDATE tbl_pedido_item SET qtde_cancelada = (q.qtde - q.qtde_faturada)
						FROM tbl_pedido_item q
					WHERE tbl_pedido_item.pedido_item = $pedido_item
					AND q.pedido_item = $pedido_item";
			$res = pg_exec($con,$sql);
		}
	}
	...
	$sql = "INSERT INTO tbl_pedido_status(pedido,status,observacao) VALUES($pedido,14,'Todos os itens não faturados foram cancelados')";
	$res = pg_exec($con,$sql);
	...
	if (strlen($msg_erro)==0){
		$res = pg_query($con,"COMMIT TRANSACTION");
		$msg_ok = "Pedido $pedido cancelado.";
	}else{
		$res = pg_query($con,"ROLLBACK TRANSACTION");
	}
}
...
function cancelaTudo(pedido){
	if (confirm('Deseja cancelar todos os ítens desse pedido?')){
		var url = "<?=$PHP_SELF?>?cancelaTudo=1&pedido="+pedido;
		window.location=url;
	}
}
</script>
...
<table width="700" border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#ffffff">
			...
			<table width="700" border="0" cellspacing="1" cellpadding="2" align='center' class='Tabela'>
				...
				for ($i = 0 ; $i < pg_num_rows ($res) ; $i++) {
					...
					if ($login_fabrica <> 14 and $login_fabrica<>24) {
						if ($login_fabrica <> 1 and $login_fabrica <> 93 and $login_fabrica <> 7 and $login_fabrica <> 10 and $login_fabrica <> 3) {
							if(!empty($tabela) and !empty($peca)) {
				...
				<?
					if($login_admin == $ronaldo or $login_admin == 822) {
						echo "<tr><td colspan='100%' align='right'>";
						echo 'Motivo:<br><textarea name="motivo_cancelamento_pedido" id="motivo_cancelamento_pedido" class="frm"></textarea>
						<a href="javascript:void(0)" onclick="if (confirm(\'Deseja cancelar este pedido?\')) window.location = \''.$PHP_SELF.'?cancelar=todo&pedido='.$pedido.'&motivo=\'+document.getElementById(\'motivo_cancelamento_pedido\').value;">
						<img src="imagens/btn_x.gif" title="Cancelar Pedido" border="0">Cancelar o pedido</a>';
						echo "</td></tr>";
					}
	...
	if ($detalhar == "ok") {
		...
		if ( ($tipo_pedido_id <> 94 and (strpos(strtoupper($condicao),"GARANTIA") !== false or $login_fabrica<>1) ) OR (strlen($pedido_cliente)>0 AND $login_fabrica==7 ) ) {
			...
			if ($login_fabrica <> 11 AND $login_fabrica <> 51 AND $login_fabrica <> 81 AND $login_fabrica <> 59 AND $login_fabrica <> 80 AND $login_fabrica <> 10 AND $login_fabrica <> 95) {
				$sql = "SELECT  distinct
				...
				if($login_fabrica == 81 or $login_fabrica == 80 or $login_fabrica==10 or $login_fabrica == 95){
					$sql = "SELECT  DISTINCT
						tbl_pedido_item.pedido_item,
					...
					JOIN    tbl_pedido_item   ON  tbl_pedido_item.pedido    = tbl_pedido.pedido
					JOIN    tbl_peca          ON  tbl_peca.peca             = tbl_pedido_item.peca
					LEFT JOIN tbl_os_item     ON  tbl_os_item.peca          = tbl_pedido_item.peca AND tbl_os_item.pedido        = tbl_pedido.pedido AND tbl_pedido_item.pedido_item = tbl_os_item.pedido_item
					LEFT JOIN tbl_os_produto  ON  tbl_os_produto.os_produto = tbl_os_item.os_produto
					LEFT JOIN tbl_os          ON  tbl_os.os                 = tbl_os_produto.os
					WHERE   tbl_pedido_item.pedido = $pedido
					AND     tbl_pedido.fabrica     = $login_fabrica ";
			...
			if (pg_num_rows($res) > 0) {
				for ($i = 0 ; $i < pg_num_rows ($res) ; $i++) {
					...
					if($i==0) {
						...
						if($login_fabrica==45 or $login_fabrica == 51 or $login_fabrica == 81 or $login_fabrica == 2 or $login_fabrica == 80 or $login_fabrica==10 or $login_fabrica==95) echo "<td align='center'><b>Ação</b></td>";
						echo "</tr>";
						echo "</thead>";
					}
					...
					if($login_fabrica <> 1 ){
						if(in_array($login_fabrica,array(3,24,35,45,50,43,5,7,14,10,81,40,42,30,90,80,72,91,85)))  {
			...
			//if($ip=='187.39.214.147') echo nl2br($sql);
					...
					if($login_fabrica == 2 || $login_fabrica == 51){
						$sql .= " AND os = $os ";
					}
					$resY = pg_query ($con,$sql);
					...
					if($login_fabrica == 51 or $login_fabrica == 81 or $login_fabrica == 2 or $login_fabrica == 10 or $login_fabrica == 80 or $login_fabrica == 95 and $status_pedido <> 14 and $status_pedido <> 4) {
						if($login_fabrica == 2){
							$qtde_cancelada = 0;
						...
						if( (strtolower($nf)=='pendente' AND pg_num_rows ($resY) == 0 AND $login_fabrica <> 51) or $qtde > ($qtde_faturada+$qtde_cancelada)) {
							if (pg_num_rows ($resY) == 0 and $login_fabrica<>3) {
...
if($login_fabrica == 95 and $status_pedido <> 4 and $status_pedido <> 14){
