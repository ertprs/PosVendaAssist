...
if ($_GET['ajax']){
  ...
  if(isset($_GET['ver_respostas'])){
        ...
        ### PESQUISA POR PAISES ###
            $like_any = "array['%\"";
                $conteudo = str_replace(',', '"%\',\'%"', $paises);
            $like_any .= $conteudo."\"%']";
            $array_paises = $like_any;


            $sql2 = "SELECT tbl_laudo_tecnico_os.os,
            ...
            FROM tbl_os
                ...
                AND tbl_laudo_tecnico_os.observacao LIKE ANY ($array_paises) AND tbl_os.os = $os
                AND titulo ILIKE 'Pesquisa de%' ORDER BY tbl_laudo_tecnico_os.data;";
            $res2 = pg_query($con,$sql2);

            if(pg_num_rows($res2) > 0){

                $idioma = pg_fetch_result($res2, 0, 'observacao');

                $lingua =json_decode($idioma);

                if($lingua->language == 'pt'){
                    $pesquisa = 'pt';
        ...
        */
        ...
        $sql = "SELECT tbl_laudo_tecnico_os.laudo_tecnico_os,tbl_laudo_tecnico_os.os,
                tbl_os.sua_os,
