...
if (empty($conteudo)) {
	if (!empty($pecas)) {
		...
		echo '<meta http-equiv="Refresh" content="0; url=relatorio_extratificacao_devolucao_pareto_detalhe_vdnf.php?ab=' . $abertura . '&nf=' . $data_fabricacao . '&fm=' . $meses . '&p=' . implode('|', $pecas) . '&f=' . $familia . '&fo=' . $fo . '&d=' . $d . '&po=' . $po . $paramsProd . $paramMatriz . '" />';
		exit;
	}
    ...
    /*$sql3 = "SELECT os_laudo into temp temp_os from temp_oss where (select fn_qtos_meses_entre(data_nf, data_recebimento)) $cond $cond_produtos;";*/
    $sql3 = "SELECT os_laudo into temp temp_os from temp_oss where ( select case fn_qtos_meses_entre(data_nf, data_recebimento) when 0 THEN 1 else fn_qtos_meses_entre(data_nf, data_recebimento) end as meses ) $cond $cond_produtos;";
    ...
    if (empty($cond_produtos)) {
        ...
        $sql_total_geral = "SELECT count(os_laudo) as total_geral from temp_os";
    }
	...
	if ($total_geral > 0) {
		...
		$sql_os_com_peca = "SELECT count(distinct(temp_os.os_laudo)) AS total_os
							FROM temp_os
							JOIN tbl_os_laudo ON temp_os.os_laudo = tbl_os_laudo.os_laudo
...
tbl_peca.descricao,
tbl_peca.peca,
count(tbl_os_laudo_peca.qtde) as total
FROM temp_os
JOIN tbl_os_laudo ON temp_os.os_laudo = tbl_os_laudo.os_laudo
join tbl_os_laudo_peca on tbl_os_laudo_peca.os_laudo = tbl_os_laudo.os_laudo
JOIN tbl_peca ON tbl_os_laudo_peca.peca = tbl_peca.peca
group by tbl_peca.referencia, tbl_peca.descricao, tbl_peca.peca order by total desc limit $limit
		...
		if (pg_num_rows($query) == 0) {
            ...
            header("Location: relatorio_extratificacao_devolucao_pareto_sem_peca_vdnf.php?ab=$abertura_request&nf=$data_fabricacao_request&fm=$meses&f=$familia&po=$posto{$paramsProd}");
            exit;
		}
		...
		if (pg_num_rows($query) > 0) {
			...
			$conteudo.= '<table class="tabela" cellspacing="1" align="center" style="width: 500px;">';
			$conteudo.= '<tr class="titulo_tabela">';
			$conteudo.= '<th colspan="2">Totais de Laudo OS\'s</th>';
			$conteudo.= '</tr>';
			$conteudo.= '<tr>';
			$conteudo.= '<td align="right">Com peça</td><td style="cursor: pointer" onClick="detalhe(\'' . $abertura_request . '\', \'' . $data_fabricacao_request . '\', \'' . $meses . '\', \'' . implode('|', $pecas) . '\', \'' . $familia . '\', \''.$matriz_filial.'\', \'' . implode('|', $produtos) . '\', \'' . $posto . '\', \'' . $desconsidera_conversor . '\')">' . $total_os_com_peca . '</td>';
			$conteudo.= '</tr>';
			$conteudo.= '<tr>';
			$conteudo.= '<td align="right">Sem peça</td><td style="cursor: pointer" onClick="sem_peca(\'' . $abertura_request . '\', \'' . $data_fabricacao_request . '\', \'' . $meses . '\', \'' . $familia . '\', \'' . implode('|', $produtos) . '\', \'' . $posto . '\', \''.$matriz_filial.'\' )">' . $total_os_sem_peca . '</td>';
			$conteudo.= '</tr>';
			$conteudo.= '</table><br/>';
        ...
        header("Location: relatorio_extratificacao_devolucao_pareto_sem_peca.php?ab=$abertura_request&fb=$data_fabricacao_request&fm=$meses&f=$familia&po=$posto{$paramsProd}");
        exit;
	}
	...
	<head>
		...
		<script>
            function detalhe(abertura, data_fabricacao, meses, peca, familia, matriz_filial, produtos, posto, dcg) {
                ...
                var url = "relatorio_extratificacao_devolucao_pareto_detalhe_vdnf.php?ab=" + abertura + "&nf=" + data_fabricacao + "&fm=" + meses + "&p=" + peca + "&f=" + familia +"&matriz_filial="+ matriz_filial+ "&pd=" + produtos + "&po=" + posto + "&dcg=" + dcg;
                window.open (url, "detalhe", "height=640,width=1040,scrollbars=1");
            }

            function sem_peca(abertura, data_fabricacao, meses, familia, produtos, posto,matriz_filial) {
                if (!produtos) { produtos = '' };
                if (!posto) { posto = '' };
				if (!matriz_filial) { matriz_filial = '' };

                var url = "relatorio_extratificacao_devolucao_pareto_sem_peca_vdnf.php?ab=" + abertura + "&nf=" + data_fabricacao + "&fm=" + meses + "&f=" + familia + "&pd=" + produtos + "&po=" + posto+ "&matriz_filial="+ matriz_filial;
                window.open (url, "detalhe", "height=640,width=1040,scrollbars=1");
            }
		</script>
		<?php if (!empty($conteudo) and $conteudo <> "eeeDados não encontrados!"): ?>
