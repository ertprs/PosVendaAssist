...
if (strlen($produto) > 0) {
	...
	$cond_vw_produto = " AND vw_quebra_produto.produto = $produto ";
...
if (!empty($tipo_os)) {
	...
	$cond_vw_tipo_os = " AND vw_quebra_produto.consumidor_revenda = '$tipo_os' ";
} else {
	$cond_tipo_os = '';
	$cond_vw_tipo_os = '';
}
...
if (strlen($aux_data_inicial) > 0 AND strlen($aux_data_final) > 0) {
	...
	if($login_fabrica == 138) {
			...
			$join = "JOIN tbl_os_produto ON tbl_os.os = tbl_os_produto.os
				JOIN        tbl_produto ON tbl_produto.produto = tbl_os_produto.produto";
	}else{
	
	
			$join =	" JOIN        tbl_produto ON tbl_produto.produto = tbl_os.produto";
	}
	...
	$sql .= "			GROUP BY    tbl_os.fabrica        , tbl_os.consumidor_revenda,
							tbl_produto.produto   ,
							tbl_produto.referencia,
							tbl_produto.descricao , ";
	...
	$sql .= "	date_trunc('day', tbl_os.data_digitacao),
			...
			$cond_vw_produto $cond_vw_tipo_os";
...
if($login_fabrica == 20 OR $login_fabrica == 15){
	...
	if($login_fabrica == 20 and $pais != 'BR'){
		$sql_data .= " tbl_extrato.data_geracao BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59' ";
	}else{
		if($login_fabrica == 20){
			$sql_data .=	" tbl_extrato_extra.exportado BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59' ";
		}else{
			$sql_data .= " tbl_extrato.data_geracao BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59' ";
		}
	}
	...
	$sql = "
				...
				INTO TEMP tmp_fcr_xls_$login_admin
				FROM tbl_os_extra
				...
				CREATE INDEX tmp_fcr_xls_OS_$login_admin ON tmp_fcr_xls_$login_admin(os);

				SELECT tbl_os.produto, COUNT(*) AS qtde
				INTO TEMP tmp_fcr_xls2_$login_admin
				FROM tbl_os
				JOIN tmp_fcr_xls_$login_admin fcr ON tbl_os.os = fcr.os
				JOIN tbl_posto ON tbl_os.posto = tbl_posto.posto
				WHERE tbl_os.excluida IS FALSE
				AND $cond_2
				AND $cond_3
				AND $cond_4
				AND $cond_5
				...
				CREATE INDEX tmp_fcr_xls2_PRODUTO_$login_admin ON tmp_fcr_xls2_$login_admin(produto);
				...
				SELECT tbl_produto.produto        ,
					...
					FROM tbl_produto
					$join_produto_idioma
					JOIN tmp_fcr_xls2_$login_admin fcr1 ON tbl_produto.produto = fcr1.produto
				WHERE $cond_1
...
//echo nl2br($sql); exit;
	...
	if (pg_numrows($res) > 0) {
		...
		echo `rm -f /tmp/assist/field-call-rate-serie-$login_fabrica.xls`;

		$fp = fopen ("/tmp/assist/field-call-rate-serie-$login_fabrica.xls","w");
		...
		if (in_array($login_fabrica, array(117))) {
			fputs ($fp,"<td bgcolor='#E9F3F3' align='center'><b>MACRO - FAMÍLIA</b></td>");
		}else{
		...
		for ($i=0; $i<pg_numrows($res); $i++){
			...
			// SQL RETIRADO PARA MELHORAR PERFORMANCE
			 $sql = "SELECT  tbl_produto.referencia          AS produto_referencia           ,
					...
					WHERE   tbl_os.fabrica = $login_fabrica
					AND     tbl_os.data_digitacao::date BETWEEN '$aux_data_inicial' AND '$aux_data_final'
					$cond_produto $cond_tipo_os
					$cond_7
...
//			echo $sql; exit;
		...
		//$dest_dir = '/www/assist/www';
		$dest_dir = __DIR__;

		system("mv  /tmp/assist/field-call-rate-serie-$login_fabrica.xls $dest_dir/xls/field-call-rate-serie-$login_fabrica.$data.xls");
