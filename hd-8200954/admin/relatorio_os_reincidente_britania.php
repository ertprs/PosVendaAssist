...
$btn_acao_relatorio = strtoupper($_REQUEST["btn_acao"]);
...
<?
if (strlen($btn_acao_relatorio) > 0 && strlen($msg_erro) == 0) {
...
if ($btn_acao_relatorio == 'PESQUISAR'){
	...
	if (strlen($btn_acao_relatorio)>0 AND strlen($msg_erro)==0){
		...
		for($i = 3; $i <= 10; $i++) {
		 ...
		 if (isset($zestado)) {
            if ($zestado != "selecionar_todos") {
                $where_estado = " WHERE posto_estado = '$zestado' ";                
            } 
        }
		...
		SELECT * FROM tmp_os_reinc_$login_admin $where_estado;";
...
<TR>
	...
	<TD>Estado<br>
		<select name="estado">
		<option value="selecionar_todos">Selecionar Todos</option>
		<?php 
		    ...
		    echo "<option value='0' >Todos Estados</option>";
