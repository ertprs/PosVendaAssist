...
// adicionado por Fabio - Altera o status para liberado da Assis. Tec. da Fábrica caso tenha intervencao.
	...
	if ($qtdex>0){
		...
		if ($statuss=='62' || $statuss=='65' || $statuss=='72' || $statuss=='87' || $statuss=='116'){
			...
			if ($login_fabrica==24) {
				$id_servico_realizado			= 5851;
				$id_servico_realizado_ajuste	= 5851;
				$id_solucao_os					= 701;
...
//		HD 79774 - Paulo César 10/03/2009 sempre gera pedido para fabrica 3
		...
		if (strlen ($msg_erro) == 0) {
			if($login_fabrica<>3 and $login_fabrica <> 35 AND $login_fabrica <> 11){
			...
			if (strlen ($msg_erro) == 0) {
				if($causa_troca == 1 or $causa_troca== 7 or $causa_troca==32) {
					...
					if(@pg_num_rows($res_ot) > 0){
						...
						if($qtde_troca == 0){
							$msg_erro = "Para essa causa, deve informar a peça que gerou a troca.";
						}
					}
				}
			}
...
if ($btn_acao == "continuar") {
	...
	if (strlen (trim ($classificacao_os)) == 0) {
		...
		if ($login_fabrica == 7){
			$msg_erro .= " Classificação da OS é obrigatória.";
		}
	}
	...
	if ($login_fabrica==7 AND strlen($posto) > 0){// HD 70398
		...
		$res = pg_query ($con,$sql);
		if(pg_num_rows($res)>0){
			$msg_erro = "Este posto está DESCREDENCIADO. Não é possível cadastrar OS.";
		}
	}
	if (strlen($msg_erro)==0){
...
if (strlen ($os) > 0) {
	...
	if (pg_num_rows ($res) == 1) {
		...
		if($login_fabrica==11 or $login_fabrica==19) $defeito_reclamado = pg_fetch_result($res,0,defeito_reclamado);
...
function fnc_pesquisa_numero_serie (campo, tipo) {
	...
	if (tipo == "produto_serie") {
		url = "pesquisa_numero_serie.php?produto_serie=" + campo.value + "&tipo=produto_serie"+revenda_fixo_url;
	}
	if (tipo == "cnpj") {
		url = "pesquisa_numero_serie.php?cnpj=" + campo.value + "&tipo=cnpj"+revenda_fixo_url;
	}
	...
	<td valign="top" align="left">
		...
		<? if ($login_fabrica==7) { // HD 75762 para Filizola ?>
			<div style='border: #D3BE96 1px solid;
				...
				<center>
				<font size="2" face="Geneva, Arial, Helvetica, san-serif">Classificação da OS</font>
...
*/
		...
		<tr valign="top">
			...
			<td nowrap>
				...
				<br>
				<input name="data_abertura" id='data_abertura' size="12" maxlength="10" value="<? echo $data_abertura ?>" type="text" class="frm" tabindex="0" ><br><font face='arial' size='1'>Ex.: 25/11/2007</font><br>
			</td>
			...
			<? }?>
				...
				if ($login_fabrica==11) {
					if ( (strlen($admin_os) > 0 and strlen($os) > 0) or (strlen($os)==0)) {
						...
						echo "<font size='1' face='Geneva, Arial, Helvetica, san-serif'>Pagar Mão-de-Obra</font>";
						echo "</td>";
					}
				} ?>
		</tr>
		...
		<? } ?>
			...
			?>
				...
				/*HD: 110888 - LIBERADO PARA SUGGAR*/
				 $aExibirDefeitoReclamado = array(7,28,35,19,50,30,59,24);
			...
			<tr valign='top'>
				...
				if ($login_fabrica == 19) { // HD 49849
					...
					echo "<br>";
					echo "<SELECT NAME='tipo_os' style='width:150px'>";
				...
				if ($login_fabrica == 50) {
					echo "<td valign='top' align='left'>
						...
						echo "<br>";
					echo "<INPUT TYPE='text' name='data_fabricacao' class='frm' value='$data_fabricacao' size='12' maxlength='20'>";
					echo "</td>";
				}
				?>
...
?>
		...
		<tr valign="top">
			<td colspan='2'>&nbsp;</td>
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">Nota Fiscal Saída</font>
				<br>
			...
			<td>
				...
				<br>
				<input class="frm" type="text" name="data_nf_saida"  size="12" maxlength="10" value="<? echo $data_nf_saida ?>" tabindex="0" ><br><font face='arial' size='1'>Ex.: 03/03/2009</font>
			</td>
		</tr>
		<? } ?>
		</table>
...
<? } ?>
		...
		<table width='700' align='center' border='0' cellspacing='2' cellpadding='2'>
			<tr class="top">
				<td  class="top">Histórico de Orientações do SAC ao Posto Autorizado  </td>
			</tr>
		</table>
		<hr>

		<table  width='700' border="1" cellspacing="5" cellpadding="3" bgcolor='#CCCCCC' class="bordasimples">
			<tr>
				<td>
...
?>
		...
		<?
		if ($login_fabrica == 3 OR $login_fabrica == 45 or $login_fabrica==35 OR $login_fabrica == 11 OR $login_fabrica==25 OR $login_fabrica == 51 OR $login_fabrica == 14 OR $login_fabrica == 66 OR $login_fabrica == 24 OR $login_fabrica == 19) {
			if ($login_fabrica == 3) {
				echo "<table border='0' cellspacing='0' width='600'>";
				...
				$res = pg_query ($con,$sql);
				if (pg_num_rows($res) > 0) {
						echo "<select class='Caixa' style='width: 400px;' name='marca_troca' id='marca_troca' onChange='buscaFamilia(this.value);'>\n";
						echo "<option value=''>ESCOLHA</option>\n";
					for ($x = 0 ; $x < pg_num_rows($res) ; $x++){
						$aux_marca     = trim(pg_fetch_result($res,$x,marca));
						$aux_descricao  = trim(pg_fetch_result($res,$x,nome));
						echo "<option value='$aux_marca'>$aux_descricao</option>\n";
					}
				}else $aux_marca = '001';
				echo "</td>";
				echo "</tr>";
			...
			echo "<td align='left'>";
			if ($login_fabrica == 3) {
				$sql = "SELECT  tbl_familia.descricao, tbl_familia.familia
				FROM    tbl_familia
				JOIN tbl_produto using(familia)
				WHERE   tbl_familia.fabrica = $login_fabrica
				and tbl_produto.marca='$xmarca'
...
//			echo "<option value='-1' >RESSARCIMENTO FINANCEIRO</option>";
			...
			for ($i = 0 ; $i < pg_num_rows($resTroca) ; $i++) {
				echo "<option value='" . pg_fetch_result ($resTroca,$i,causa_troca) . "'";
			...
			if($login_fabrica <> 51){
				...
				echo "<input type='radio' name='fabrica_distribuidor' value='fabrica'> Fábrica<br>";
				echo "<input type='radio' name='fabrica_distribuidor' value='distribuidor'> Distribuidor<br>";
			}
