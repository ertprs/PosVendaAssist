...
$a_usam_intervencao = array(2 => "7,67", 3 => "20,96", 6 => "1,35", 11 => "61,498", 14 => "",19 => "",25 => "", 35 => "", 43=>"",45 => "640,639", 51 => "673,671", 52 => "",85 => "8430,8431");
...
if (trim($_GET['janela'])=="sim") {
	...
	$sql =  "SELECT tbl_os.os                                                     ,
				...
				(SELECT TO_CHAR(data,'DD/MM/YYYY') AS data FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN (62,64,65,127) ORDER BY data DESC LIMIT 1) AS status_data2
			FROM tbl_os
...
<?php if ($btnacao=='filtrar'){
	...
	$sql =  "SELECT interv.os
			...
			SELECT
			ultima.os,
			(SELECT status_os FROM tbl_os_status WHERE status_os IN (62,64,65,127) AND tbl_os_status.os = ultima.os ORDER BY data DESC LIMIT 1) AS ultimo_status
			FROM (SELECT DISTINCT os FROM tbl_os_status WHERE status_os IN (62,64,65,127) AND tbl_os_status.fabrica_status=$login_fabrica ) ultima
			) interv
			WHERE interv.ultimo_status IN (62,65,127)";
	/*HD 15731 - Habilitado p/ Otimização*/
	$sql =  "
			SELECT DISTINCT os INTO TEMP tmp_interv_$login_admin_2
			...
			FROM ( SELECT os FROM tmp_interv_$login_admin_2 ) ultima
			) interv
			WHERE interv.ultimo_status IN (62,65,127);
	...
	$sql =  "SELECT DISTINCT tbl_os.os                                                        ,
				...
				(SELECT status_os FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN (67,68,70) ORDER BY data DESC LIMIT 1) AS reincindente,
				(SELECT observacao FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN (62,64,65,127) ORDER BY data DESC LIMIT 1) AS status_descricao,
				(SELECT status_os FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN (62,64,65,127) ORDER BY os_status DESC LIMIT 1) AS status_os,
				(SELECT data FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN (62,64,65,127) ORDER BY data DESC LIMIT 1) AS status_pedido
			FROM tbl_os
				JOIN tbl_posto ON tbl_posto.posto = tbl_os.posto
				JOIN tbl_posto_fabrica ON tbl_posto_fabrica.posto = tbl_posto.posto AND tbl_posto_fabrica.fabrica = $login_fabrica
				JOIN tbl_produto ON tbl_produto.produto = tbl_os.produto
			...
			WHERE tbl_os.fabrica = $login_fabrica
				...
				AND (SELECT status_os FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN (62,64,65,127) ORDER BY data DESC LIMIT 1) IN (62,65,127)
				$os_array
				$sql_adicional
				$sql_ordem ";
		...
		if ($debug) pre_echo (
			...
			for ($i = 0 ; $i < $total ; $i++) {
				...
				if ($status_os=="62" or $status_os=="127"){
					$colspan="";
					...
					if ( $produto_troca_obrigatoria=='t' || $peca_troca_obrigatoria=='t' || 1==1){
						...
						if (in_array($login_fabrica,array(3,14,25,45,51,19,81)) or ($login_fabrica==35 and produto_troca_obrigatoria=='t')) {
