...
$title = "RELATÓRIO DE ATENDIMENTO DO SAC";
		...
		<tr>
			<td bgcolor='#DBE5F5' valign='bottom'>
				<table width='100%' border='0' cellspacing='0' cellpadding='0' >
					<td width="100%">
						<font size='4'>
							FILTRO PARA SELEÇÃO DE PERÍODO DE DADOS
						</font>
					</td>
					<table width='100%' border='0' cellspacing='0' cellpadding='0' >
						...
						<tr class="Conteudo" bgcolor="#D9E2EF">
							<td width='100%' colspan="4" align = 'center' style="text-align: center;">
								<img src="imagens_admin/btn_pesquisar_400.gif" onClick="document.frm_pesquisa.submit();" style="cursor:pointer " alt="Preencha as opções e clique aqui para pesquisar">
							</td>
						</tr>
					</table>
				</table>
...
<?
// Verifica se o post não esta vazio
if (!empty($_POST)) {
	...
	if (strtotime($ano_inicial.'-'.$mes_inicial.'-01') <= strtotime($ano_final.'-'.$mes_final.'-01')) {
		...
		if ($vet == 't'){
			// DETERMINA AS VARIÁVEIS A SEREM UTILIZADAS \\
			...
			// Processo para localização no banco dos MOTIVOS
			$sql_motivo = "
					SELECT categoria
					FROM tbl_hd_chamado 
			...
			// Processo para localização no banco dos MARCAS
			$sql_marca = "
			...
			for ($i = 0; $i < pg_num_rows($res_motivo); $i++) {
				...
				switch ($motivo_banco[$i]) {
					case 'informacao'         : $motivo[$i] = 'Informação'; 
						break;
					case 'procon'             : $motivo[$i] = 'Procon'; 
						break;
					case 'reclamacao'         : $motivo[$i] = 'Reclamação'; 
						break;
					case 'reclamacao_produto' : $motivo[$i] = 'Reclamação do Produto'; 
						break;
					case 'ressarcimento'      : $motivo[$i] = 'Ressarcimento'; 
						break;
					case 'sugestao'           : $motivo[$i] = 'Sugestão'; 
						break;
			...
			echo "<table align='center' border='0' cellspacing='1' cellpadding='5'>";
				echo "<tr class='menu_top'>\n";
					// Primeira coluna (DESCRIÇÃO DOS MOTIVOS)
			...
			//  FIM DA PRIMEIRA LINHA TÍTULO  \\
				...
				// Processo para localização no banco das QTD de MOTIVOS
				$sql_geral = "
...
//*************************************MARCA***************************************************\\
		...
		$dta_final       = substr($vet_data_final, 0,10 )." 23:59:59";
			...
			echo "<table align='center' border='0' cellspacing='1' cellpadding='5'>";
				echo "<tr class='menu_top'>\n";
					// Primeira coluna (DESCRIÇÃO DAS MARCAS)
			...
			//  FIM DA PRIMEIRA LINHA TÍTULO  \\
				...
				// Processo para localização no banco das QTD de MARCAS
				$sql_geral = "
				...
				//  INÍCIO DO PREENCHIMENTO DA TABELA COM DADOS  \\
				//  Laço principal feito pelo número de MARCAS
				for ($i = 0; $i < $cont_marca; $i++) {
					...
					for($x=1; $x <= $cont_data; $x++){
						...
						// Query para localização do TOTAL POR MARCA
						$sql_total = "
									SELECT count (tbl_marca.nome) as TOTAL
									FROM tbl_hd_chamado 
										JOIN tbl_hd_chamado_extra USING(hd_chamado)
		...
		echo "</table>";
			...
			echo $msg_erro .= "A período não pode ser superior a 12 meses";
		}
	}else{
		echo $msg_erro .= "A selção do período inicial não pode ser maior que a do período final";
	}
}
