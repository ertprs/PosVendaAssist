...
if(!in_array($login_fabrica,[50]) && !$telecontrol_distrib) { // Hd 42839
...
if ($login_fabrica == 10 or $login_fabrica == 51 or $login_fabrica == 81){
	$login_fabrica_aux = '10,51,81';
}

if ($telecontrol_distrib) {
	$nfe_campo = "tbl_faturamento.chave_nfe, ";
}
...
if ((strlen ($faturamento) > 0) or (strlen($pedido) > 0 and strlen($peca) > 0)) {
	$sql = "SELECT	tbl_faturamento.faturamento,
			...
			WHERE	tbl_faturamento.fabrica     in ( $login_fabrica_aux )";
	...
	if (pg_numrows ($res) > 0) {
		...
		if ($telecontrol_distrib) {
			$chave_nfe = pg_fetch_result($res, 0, 'chave_nfe');
		}
		$transportadora		= pg_result ($res,0,'nome');
...
<br>
<TABLE width="650px" border="0" align="center" cellspacing="0" cellpadding="3">
<TR>
...
<TR>
	...
	<?php if ($telecontrol_distrib) { ?>
			...
			<TR>	
				<TD class="menu_top">Chave de Acesso da Nota Fiscal</TD>
				<TD class="table_line" colspan='7'><? echo $chave_nfe ?>&nbsp</TD>
	<?php } ?>
</TR>
<? } ?>
</TABLE>
...
if (strlen ($faturamento) > 0) {
	...
	if (pg_numrows ($res) > 0) {
		echo "<TABLE width='650' border='0' align='center' cellspacing='0' cellpadding='3'>";
