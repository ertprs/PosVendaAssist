...
function AbrePeca(produto,data_inicial,data_final,tipo, matriz_filial){
//alert(tipo);
    var matriz_filial = matriz_filial || '0';
    var dcg = 'false';

    <?php if ($login_fabrica == 24): ?>
    dcg = $("#desconsidera_conversor").is(":checked");
    <?php endif ?>

	janela = window.open("relatorio_field_call_rate_pecas_grupo2.php?produto=" + produto + "&data_inicial=" + data_inicial + "&data_final=" + data_final + "&tipo=" +tipo  + "&tipo_os=<?echo $tipo_os;?>&dcg=" + dcg +"&matriz_filial="+matriz_filial ,"produto",'resizable=1,scrollbars=yes,width=750,height=550,top=0,left=0');
	janela.focus();
}
...
<? if($login_fabrica==24){ ?>
    <tr>
        <td>
            <input type="radio" name="matriz_filial" value="02" <?if($matriz_filial == 02 OR $matriz_filial == ''){echo " checked ";} ?>> Matriz - 02
        </td>
        <td></td>
        <td>
            <input type="radio" name="matriz_filial" value="04"<?if($matriz_filial == 04){echo " checked ";} ?> > Filial - 04
        </td>
    </tr>
<? } ?>
    <tr>
        <td colspan="3">
            ...
            ?>
            <input type="checkbox" id="desconsidera_conversor" name="desconsidera_conversor" <?php echo $checked_desconsidera_conversor ?>>
            Desconsiderar OS de conversão de gás
        </td>
    </tr>
...
if (strlen($btn_acao) > 0 and empty($msg_erro)) {
	...
	if (pg_num_rows($res) > 0) {
		...
		for ($i=0; $i<pg_num_rows($res); $i++){
			...
			echo "<TR >";            
            echo "<TD  align='left'><B><a href='javascript:AbrePeca(\"$referencia_fabrica\",\"$x_data_inicial\",\"$x_data_final\",\"grupo\", \"$matriz_filial\");'>$referencia_fabrica</a></b></td>";
			echo "<TD align='left'>";
				$sql = "SELECT 
				...
				if(pg_num_rows($xres)>0){
					for($y=0;pg_num_rows($xres)>$y;$y++){
						...
						echo "<a href='javascript:AbrePeca(\"$produto\",\"$x_data_inicial\",\"$x_data_final\",\"produto\",\"$matriz_filial\");'>$ref_produto</a> ";                        
					}
				}
