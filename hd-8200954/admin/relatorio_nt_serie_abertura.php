...
if($_GET['ajax']=='sim') {
	...
	if (strlen($erro) == 0) {
		$data_inicial = (strlen($_POST["data_inicial_01"])>0) ? trim($_POST["data_inicial_01"]) :  trim($_GET["data_inicial_01"]);
	...
	if (strlen($erro) == 0) {
		...
		if (strlen($erro) == 0) {
			$data_final = (strlen($_POST["data_final_01"])>0) ? trim($_POST["data_final_01"]) : trim($_GET["data_final_01"]);
	...
	if(strlen($aux_data_inicial) > 0 and strlen($aux_data_final) > 0) {
		$cond_2 = " AND tbl_os.data_abertura BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59' ";
	}
	
	$status = $_GET['status'];
	
	switch($status) {
		case 'aberta': $cond_3 = "AND tbl_os.finalizada is null ";
		break;
		case 'fechada': $cond_3 = "AND tbl_os.finalizada is not null ";
		break;
	}
	...
	if ($listar == "ok") {
		...
		if (pg_numrows($res) > 0) {
			...
			$resposta  .= "<td align='left'><font size='1'><b>&nbsp; OS sem peça</b></font></td>";
			$resposta  .= "</tr>";
			...
			fputs ($fp,"<html>");
			fputs ($fp,"<head>");
			fputs ($fp,"<title>Relatório de Número de Série da Linha NT - $data");
			fputs ($fp,"</title>");
...
$title = "RELATÓRIO DE SÉRIE DA LINHA NT ABERTURA";
...
function Exibir (componente,componente_erro,componente_carregando,fabrica) {
	...
	var var4 = document.frm_relatorio.status.value;
	var var5 = document.frm_relatorio.familia.value;


	
	var parametros = 'data_inicial_01='+var1+'&data_final_01='+var2+'&codigo_posto='+var3+'&status='+var4+'&familia='+var5+'&ajax=sim&tempo='+ new Date().getTime();
	...
	<tr>
		<td bgcolor='#DBE5F5' valign='bottom'>
			...
			<table width='100%' border='0' cellspacing='1' cellpadding='2' >
				...
				<tr>
					<td width="10">&nbsp;</td>
					<td align='right' nowrap><font size='2'>Status da OS</td>
					<td align='left'>
			...
			<input type='button' onclick="javascript:Exibir('dados','erro','carregando','<?=$login_fabrica?>');" style="cursor:pointer " value='Consultar' id='botao'>
		</td>
	</tr>
</table>
</FORM>
