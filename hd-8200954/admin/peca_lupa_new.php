...
if($login_fabrica == 125){
	if(isset($_GET['kit'])==true){
		$kit = $_GET["kit"];
	}

	if(isset($_GET['fora_kit']) == true){
		$fora_kit = $_GET["fora_kit"];
	}
}
...
$whereItemAparencia    = "";
if ($_REQUEST["pesquisa_produto_acabado"] || in_array($login_fabrica, array(119,129,178))) {
            ...
            $(function () {
				...
				?>
					...
					$('button.retorna-peca').on('click', function() {
						...
						json = json.replace(/\\\"/g, '\"');
						json = JSON.parse(json);
						let qtde = parseInt($(this).prevAll('.div-preco').find('.span3').val());
		...
		<div id="container_lupa" style="overflow-y:auto;">
			...
			<form action="<?=$_SERVER['PHP_SELF']?>" method='POST' >
				<div class="row-fluid">
					...
					if ($catalogo_peca) {
					?>
						<div class="span1"><h3><small><?=traduz('Peça')?>:</small></h3></div>
						<div class="span2">
					<?php
					} else {
					?>
					...
					?>
						...
						<select name="parametro" >
							...
							<option value="serie_peca" <?=($parametro == "serie_peca") ? "SELECTED" : ""?> ><?=traduz('Número de série')?></option>
							<?php } ?>
						</select>
					</div>
					<div class="span4">
					...
					?>
						...
						<div class='row-fluid'>
							<div class='span1'><h3><small><?=traduz('Lista básica')?>:</small></h3></div>
							<div class='span6'>
								<select name='produto' class='span12'>
									<option value=''><?=traduz('Produto')?></option>
									<?php
									$sqlProduto = "
			...
			if (strlen($valor) >= 3 || $catalogo_peca) {
					...
					switch ($parametro) {
						case 'referencia':
							...
							if ($login_fabrica == 143) { 
								require "../classes/Posvenda/Fabricas/_143/PedidoWackerNeuson.php";
            					
            					$disponibilidade = new PedidoWackerNeuson($valor);
				...
				if($kit == true and $login_fabrica == 125){
					$joinKit = " join tbl_kit_peca ON tbl_kit_peca.peca = tbl_peca.peca and tbl_kit_peca.fabrica = $login_fabrica ";
					$campoKit = "tbl_kit_peca.kit_peca, ";
				}
				...
				if($versao and $login_fabrica != 183){
				...
				if(strlen($marca)>0 and $login_fabrica != 183){
				...
				if(strlen($preco_peca)>0 && !$catalogo_peca and $login_fabrica != 183){
					...
					$join_lista = (in_array($login_fabrica, array(119,153,143,151,160))) ? " LEFT " : " INNER ";
				...
				if ((isset($whereAdc) && strlen($valor) >= 3) || $catalogo_peca) {
					...
					if(in_array($login_fabrica, [169,170]) and $telapedido == true){
					...
					if ($rows > 0) {
							...
							<table class="table table-striped table-bordered table-hover table-lupa" >
								<thead>
									<tr class='titulo_coluna'>
										...
										<? if(isset($preco_peca) or $tabela){ echo "<th>".traduz("Preço")."</th>";}?>
										<? if(strlen($marca)){ echo "<th>".traduz("Marca")."</th>";}?>
										<? if($login_fabrica == 87){ echo "<th>".traduz("Empresa")."</th>";}?>
						...
						for ($i = 0 ; $i <$rows; $i++) {
							...
							if ($login_fabrica == 87) {
								$marca_peca = pg_fetch_result($res, $i, 'marca_peca');

								if (!empty($marca_peca)) {
									$sql_emp = "SELECT tbl_empresa.descricao FROM tbl_marca JOIN tbl_empresa USING(empresa) WHERE marca = $marca_peca AND tbl_marca.fabrica = $login_fabrica";
							...
							if($login_fabrica == 125 and $kit == true){
								$kit_peca = pg_fetch_result($res, $i, 'kit_peca');
								$r["kit_peca"] = $kit_peca;
							}
							...
							?>
								<div class='span3' style='border: 2px solid #ECECEC; border-radius: 8px; padding: 2px; margin-bottom: 10px;' >
									...
									?>
										...
										<button type='button' disabled class='btn btn-success btn-block' style='cursor: not-allowed;' ><i class='fa fa-cart-plus'></i> <?=traduz('Adicionar ao carrinho')?></button>
									<?php
									} else {
									?>
										...
										if ((double) $preco > 0) {
										?>
											<button type='button' class='btn btn-success btn-block retorna-peca' data-json='<?=addslashes(json_encode($r))?>' ><i class='fa fa-cart-plus'></i> <?=traduz('Adicionar ao carrinho')?></button>
										<?php
										} else {
										?>
											<button type='button' class='btn btn-success btn-block' onclick='alert('<?=traduz("Não é possível adicionar peças sem preço ao carrinho")?>');' ><i class='fa fa-cart-plus'></i> <?=traduz('Adicionar ao carrinho')?></button>
										<?php
										}
									}
									?>
								...
								if (empty($para)) {
									if($bloqueada_venda == 't' and isset($telapedido)){
										$complemento_descricao = traduz("Peça bloqueada para venda");
										echo "<tr >";
									}else{
										$complemento_descricao = "";
										if($login_fabrica == 125 and $fora_kit == true){
											echo "<tr onclick='window.parent.retorna_peca_fora_kit(".json_encode($r)."); window.parent.Shadowbox.close();' >";
										}else{
									...
									if($login_fabrica == 143 && $disponibilidade) { echo "<td class='cursor_lupa'>{$disponibilidade}</td>"; }
									if(strlen($marca)){ echo "<td class='cursor_lupa'>{$marca_nome}</td>";}
									if($login_fabrica == 87){ echo "<td class='cursor_lupa'>{$desc_emp}</td>";}
									echo "</tr>";
								}else{
									...
									if ($imagemPeca) {
										...
										echo "<td class='cursor_lupa'>{$referencia}</td>";
										echo "<td class='cursor_lupa'>{$descricao} <br ><strong class='text-error'>".traduz("Mudou Para").": </strong><span style='font-family:Arial, Verdana, Times, Sans-Serif;size:10px;color:black;font-weight:bold'>{$para} - {$para_descricao}  </span> <span style='color:red;'>{$complemento_descricao}</span></td>";
										...
										if ($login_fabrica == 87) {
											echo "<td class='cursor_lupa'>{$desc_emp}</td>";
										}
									echo "</tr>";
				...
				echo '
					<div class="alert alert_shadobox">
					    <h4>'.traduz("Informar toda ou parte da informação para pesquisar!").'</h4>
					</div>';
			}
			?>
	</div>
