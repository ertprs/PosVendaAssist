...
if (strlen($HTTP_GET_VARS["aprovar"]) > 0) {
	...
	if (strlen($erro) == 0) {
		header("Location: extrato_aprovado.php");
		exit;
	}
}
...
function Alterar(extrato) {
	var janela_extrato = null;
	janela_extrato = this.open('envio_new_financeiro.php?extrato=' + extrato,'1', 'height=150,width=300,location=no,scrollbars=no,menubar=no,toolbar=no,resizable=no')
	janela_extrato.parentwin = self;
}
...
function Obs(extrato) {
	var janela_extrato = null;
	janela_extrato = this.open('imprime_obs_extrato.php?extrato=' + extrato,'1', 'height=300,width=400,location=no,scrollbars=no,menubar=no,toolbar=no,resizable=no')
	janela_extrato.parentwin = self;
}
	...
	$sql = "SELECT  distinct
			...
			FROM (
					(
						...
						AND     tbl_extrato_pagamento.data_pagamento NOTNULL
