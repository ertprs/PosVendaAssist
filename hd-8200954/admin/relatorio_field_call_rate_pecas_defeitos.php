...
if ($btn_acao == 1) {
	...
	if(strlen($linha) > 0){
		if ($login_fabrica <> 14) {
			...
			$mostraMsgLinha = "<br>na FAMÍLIA $linha_nome";
		}
...
$title = "RELATÓRIO - FIELD CALL-RATE : PEÇAS";
...
<TR>
	<td width="100">&nbsp;</td>
	<TD>Referência Peça<br><input class='frm' type="text" name="peca_referencia" value="<? echo $referencia_de ?>" size="15" maxlength="20">&nbsp;<img src="imagens/lupa.png" onclick='fnc_pesquisa_peca (document.frm_pesquisa.peca_referencia, document.frm_pesquisa.peca_descricao,"referencia")' style='cursor:pointer'></TD>
	<TD>Descrição Peça<br><input class='frm' type="text" name="peca_descricao" value="<? echo $descricao_de ?>" size="15" maxlength="50">&nbsp;<img src="imagens/lupa.png" onclick='fnc_pesquisa_peca (document.frm_pesquisa.peca_referencia, document.frm_pesquisa.peca_descricao,"descricao")' style='cursor:pointer'></TD>
	<td width="70">&nbsp;</td>
</TR>
<? } ?>
...
<? } ?>
	...
	<TR>
		<td width="100">&nbsp;</td>
		<TD colspan = '2'>
		<input type='checkbox' name='troca' value='t' <? if (strlen($troca) > 0) echo "checked"; ?>><? if($login_fabrica==20) echo "Acessórios"; else echo "Troca de Peça";?></TD>
		<td width="70">&nbsp;</td>
	</TR>
<? if($login_fabrica==20){?>
	<TR>
...
<?}?>
	...
	<TR>
		...
		<TD colspan="2">
			...
			<input type="button" style="background:url(imagens_admin/btn_pesquisar_400.gif); width:400px;cursor:pointer;" value="&nbsp;" onclick="javascript: if ( document.frm_pesquisa.btn_acao.value == '' ) { document.frm_pesquisa.btn_acao.value='1'; document.frm_pesquisa.submit() ; } else { alert ('Aguarde submissão da OS...'); }" alt='Clique AQUI para pesquisar'>
		</TD>
		<td width="70">&nbsp;</td>
	</TR>
</TABLE>
...
# exit;
	...
	if (pg_numrows($res) > 0) {
		...
		echo "<thead>";
		echo "<TR  height='25'>";
		echo "<TD width='120' height='15'><b>Referência</b></TD>";
		echo "<TD width='55%' height='15'><b>Peça</b></TD>";
		...
		echo "</tbody>";
		echo "<tr><td colspan='2' align='left'><font size='2'><b><CENTER>Total de Peças com Defeitos</b></td>";
