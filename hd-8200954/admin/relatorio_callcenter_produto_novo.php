...
if (strlen($mes_pesquisa) > 0 AND strlen($ano_pesquisa) == 4 AND $btn_acao == 'consultar') {
	...
	$sql = "SELECT  tbl_callcenter.natureza                                      ,
			...
			AND       tbl_callcenter.data BETWEEN '$data_inicial'::date AND '$data_final'::date
			GROUP BY  tbl_callcenter.natureza, tbl_defeito_reclamado.descricao, tbl_produto.linha, tbl_produto.nome_comercial, tbl_produto.produto, tbl_defeito_reclamado.defeito_reclamado
			ORDER BY  tbl_callcenter.natureza, tbl_defeito_reclamado.descricao, tbl_produto.linha, tbl_produto.nome_comercial";
	$res = pg_exec ($con,$sql);
//echo $sql; exit;
	...
	$sql = "SELECT  natureza                                           ,
			...
			GROUP BY  natureza, defeito_descricao, nome_comercial, produto, defeito_reclamado, linha
			ORDER BY  linha, natureza, defeito_descricao, nome_comercial";
	$res = pg_exec ($con,$sql);
	...
	$sql = "SELECT * FROM y_$login_fabrica order by linha";
	$res = pg_exec ($con,$sql);
...
#		echo "<hr>";
	...
	echo "TOTAL GERAL DE CHAMADAS: $total_chamadas";
	echo "</td>";
