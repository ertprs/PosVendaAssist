...
if($telecontrol_distrib) {
	header('Location:relatorio_peca_saida.php');
	exit;
}
	...
	if (!count($msg_erro["msg"])) {
		...
		if($telecontrol_distrib AND $pedidos_faturados_no_periodo == 't'){
			...
			$sql = "SELECT DISTINCT tbl_peca.peca,
				...
				join tbl_faturamento_item on tbl_pedido.pedido = tbl_faturamento_item.pedido and tbl_pedido.fabrica = $login_fabrica and tbl_faturamento_item.peca = tbl_pedido_item.peca
				...
				SELECT distinct peca,referencia, descricao, pedido, nome_posto, condicao, data_pedido, nota_fiscal, data_nota_fiscal FROM tmp_gf_$login_admin order by referencia
                ";
	...
	if(isset($_POST['gerar_excel'])){
		...
		for ($i = 0; $i < pg_num_rows($resSubmit); $i++) {
				...
				if($pedidos_faturados_no_periodo == 't'){
					...
					$data_pedido = mostra_data(substr(pg_fetch_result($resSubmit,$i,'data_pedido'),0,10));
