...
include 'cabecalho_pop_pecas_alternativas.php';
?>
...
function retornox(peca, referencia, descricao) {
	...
	opener.document.frm_peca_alternativa.referencia_<? echo $controle ?>.value = referencia;
	opener.document.frm_peca_alternativa.descricao_<? echo $controle ?>.value  = descricao;
	opener.document.frm_peca_alternativa.descricao_<? echo $controle ?>.focus()
	window.close();
}
// -->
</script>
...
if($tipo == "referencia"){
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		if ($_GET['forma'] == 'reload') {
			echo "<a href=\"javascript: opener.document.location = retorno + '?peca_alternativa=$peca' ;\" > " ;
		}else{
