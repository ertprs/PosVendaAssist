...
//TESTE
//if($login_fabrica == 6){
//	header ("Location: callcenter_cadastro_3_fer.php?callcenter=$callcenter");
//	exit;
//}
...
// adicionado por Fabio 01/12/2006 - A pedido do Samuel - Cancelar chamado
$cancelar = trim($_POST['cancelar']);
$motivo	= trim($_POST['cancelar_motivo']);
$cancelar_motivo = " -> CHAMADO CANCELADO EM ".date("d-m-Y H:i:s")." - MOTIVO: $motivo";
$cancelado="";
if($btn_acao == "continuar" && $cancelar=='on'){
	$sql = "UPDATE tbl_callcenter SET
					excluida = 't',solucao=solucao||'$cancelar_motivo'
			WHERE  callcenter = $callcenter
			AND    fabrica    = $login_fabrica";
	$res = pg_exec ($con,$sql);
	$btn_acao=""; // cancela a acao de continuar
	header("Location:callcenter_pendencias.php?cancelado=$callcenter");
	exit();
}
...
/* ============= Funï¿½o PESQUISA DE CONSUMIDOR POR NOME ====================
Nome da Funï¿½o : fnc_pesquisa_consumidor_nome (nome, cpf)
=================================================================*/
...
/* ============= Funï¿½o FORMATA CNPJ =============================
Nome da Funï¿½o : formata_cnpj (cnpj, form)
		Formata o Campo de CNPJ a medida que ocorre a digitaï¿½o
		Parï¿½.: cnpj (numero), form (nome do form)
=================================================================*/
...
/* ============= Funï¿½o FORMATA CPF =============================
Nome da Funï¿½o : formata_cpf (cpf, form)
		Formata o Campo de CPF a medida que ocorre a digitaï¿½o
		Parï¿½.: cpf (numero), form (nome do form)
=================================================================*/
...
/* ========== Funï¿½o AJUSTA CAMPO DE DATAS =========================
Nome da Funï¿½o : ajustar_data (input, evento)
		Ajusta a formataï¿½o da Mï¿½cara de DATAS a medida que ocorre
		a digitaï¿½o do texto.
=================================================================*/
...
function verificarForm(){
	if ( document.frm_callcenter.cancelar.checked){
		if (document.frm_callcenter.cancelar_motivo.value==""){
			alert ("Digite um motivo para o cancelamento!");
			return;
		}
		if (!confirm("Deseja realmente cancelar este chamado?"))
			return;	
	}
	document.frm_callcenter.btn_acao.value='continuar' ;
	document.frm_callcenter.submit();
}
</script>
...
if (pg_numrows($res) > 0){
	...
	echo "<TR class='menu_top'>\n";
	echo "<TD colspan=3>HISTï¿½ICO DO ATENDIMENTO</TD>\n";
	echo "</TR>\n";
	echo "<TR class='menu_top'>\n";
...
?>
			...
			<TR class='table_line'>
				<TD align=center>
					<select name='enderecada'>
						...
						<option value="Gerente Assistï¿½cia Tï¿½nica" <? if ($enderecada == "Gerente Assistï¿½cia Tï¿½nica") echo " selected"?>>Gerente Assistï¿½cia Tï¿½nica</option>
		...
		<TABLE width=400 border="0" cellpadding="3" cellspacing="3" align='center'>
					<TR>
						<td colspan=2 class='menu_top'>Cancelar Chamado</td>
					</TR>
					<TR class='table_line'>
						<td><b>CANCELAR</b> <input type='checkbox' name='cancelar'>  
						</td>
						<td><b>Motivo:</b> <input type='text' name='cancelar_motivo' size='30'></td>
					</TR>
		</table>
		<br>
		<TABLE width=600 border="0" cellpadding="3" cellspacing="3">
...
<!-- 					<img src='imagens/btn_voltar.gif' style="cursor:pointer" onclick="javascript: window.location='callcenter_cadastro_2.php?callcenter=<? echo $callcenter; ?>'" ALT="Pï¿½ina anterior" border='0'> -->
						<img src='imagens/btn_continuar.gif' style="cursor:pointer" onclick='javascript:verificarForm();' ALT="Continuar" border='0'>
					<!--
					<input type="hidden" name="btn_acao" value="">
					<img src='imagens/btn_continuar.gif' style="cursor:pointer" onclick="javascript: if (document.frm_callcenter.btn_acao.value == '' ) { document.frm_callcenter.btn_acao.value='continuar' ; document.frm_callcenter.submit() } else { alert ('Aguarde submissï¿½') }" ALT="Continuar" border='0'>					
					-->
				</TD>
			</TR>
		</TABLE>
