...
$title = "CADASTRAMENTO DE MOTIVO SINTÉTICO";
...
if(strlen($motivo_sintetico) == 0){
    $title_page = "Cadastro de Motivo Sintético";
}else{
    $title_page = "Alteração de Motivo Sintético";
}
...
if (isset($_POST["btn_acao"])) {
    ...
    if(strlen($descricao)==0){
        $msg_erro["msg"]["obg"] ="Por favor insira a descrição do motivo sintético";
    ...
    if (count($msg_erro) == 0) {
        if(empty($motivo_sintetico)){
            ...
            if (pg_num_rows($res) > 0) {    
                $msg_erro["msg"]["obg"] = "Código de motivo sintético já cadastrado!";
...
<div class='alert'>Para efetuar alterações, clique na descrição do motivo sintético.</div>

<table id="defeito_constatado" class="table table-striped table-bordered table-hover table-fixed">
    <thead>
        <tr class="titulo_tabela">
            <th colspan="3" nowrap>Motivos sintéticos cadastrados</th>
        </tr>
