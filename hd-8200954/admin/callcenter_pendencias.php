...
$title = "Relatório de Atendimentos Pendentes";


if ($login_admin == '10') {
	header("Location: callcenter_pendencias_fer.php");
	exit;
}
...
$sql = "SELECT  tbl_callcenter.callcenter                                        ,
		...
		AND    (tbl_providencia.providencia = ( SELECT MAX(providencia) 
												FROM tbl_providencia 
												WHERE callcenter = tbl_callcenter.callcenter
											) OR 
												 tbl_providencia.providencia IS NULL
		)
		ORDER BY tbl_callcenter.data ASC";
		//chamado 3476 - select alterado para buscar apenas a ultima providencia...
...
if (strlen ($_GET['cancelado']) > 0) {
?>
<table border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#ffffff" width = '730'>
<tr>
	<td valign="middle" align="center" class='confirmacao'>
		<? echo "Chamado No. ".$_GET['cancelado']." foi cancelado com sucesso." ?>
	</td>
</tr>
</table>
<br>
...
if (@pg_numrows($res) == 0) {
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++){
		...
		echo "<TD nowrap>";
		echo "<a href='callcenter_press.php?callcenter=$callcenter'><img src='imagens/btn_consultar_".$btn.".gif'></a>";
		if ($solucionado <> 't') {
...
if ($registros > 0){
	...
	echo " (Pï¿½ina <b>$valor_pagina</b> de <b>$numero_paginas</b>)";
	echo "</font>";
	echo "</div>";
}
