...
if ($gera_automatico != 'automatico'){
	...
	$res = pg_exec($con,$sql);
	if (pg_numrows($res) > 0) {
		$msg_aguard = "<div class='alert alert-error tac'>Por favor, aguarde, o relatório está sendo processado e dentro de instantes você receberá a confirmação por email.</div>";
		...
		if(!isset($aux)){
			echo "<div class='alert alert-block tac texto_avulso'>O relatório está sendo processado e dentro de instantes você receberá a confirmação por email.</div>";
		}
