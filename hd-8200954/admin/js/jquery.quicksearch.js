jQuery(function ($) {
	$.fn.quicksearch = function (opt) {
		...
		function make_form_input()
		{
			...
			return '<input type="text" value="' + val + '" rel="' + options.randomElement  + '" class="' + options.inputClass + '" id="' + options.randomElement + '" /> <br>';
		}
		...
		function make_form()
		{
			...
			return '<' + f + ' action="#" ' + 'id="'+ options.formId + '" ' + 'class="quicksearch">' + ' ' + make_form_label() + ' ' + make_form_input() + ' '+ make_form_loader() +'</' + f + '>';
		}
		...
		function limpaResultado(){
			$('input[rel="' + options.randomElement + '"]').val('');
		}
				...
				setTimeout(function () {
					...
					if (k != "" && k != " ") {
						...
						if (score[k].length === 0){
							...
							var verifica = 0;
							var os_ok = 0;
							$('#mensagem_os').html("").removeClass('mensagem_os');
							$(el).each(function (i) {
								var resultado = select_element(this);
								if (score[k][i]) {
									verifica++;
									resultado.show();
									var sinalizador = 1;
									...
									$("div[rel=sinalizador_os]:visible").each(function(){
											os = $(this).attr('alt');
											if( os.length >0) {
												$.ajax({
													type: "GET",
													url: "extrato_posto_mao_obra_novo_britania.php",
													data:"atualiza_sinalizador="+sinalizador+"&atualiza_os="+os,
													async: false,
													complete: function (resultado){
														var resposta = resultado.responseText;
														if ($.trim(resposta) == 'OK') {
															os_ok++;
															
														}
														$('#ok_'+os).html(resposta);
													}
												})
											}
										}
							...
							if(os_ok < verifica) {
								alert('Foram encontradas ' +verifica+' OSs, mas foram atualizadas apenas '+os_ok);
							}else{
								$('#mensagem_os').html('Foram encontradas ' + verifica + ' OSs, e atualizadas todas elas').addClass('mensagem_os');
							}
							limpaResultado();
						}
					} else {
