...
TINY.table=function(){
	var dateReg = /[0-9]{1,4}\W[0-9]{1,2}\W[0-9]{1,4}/i;
	var curReg  = /([a-z]{3}|[a-z]\$|\$|\,|€)\s?/gi;
	...
	function cp(f,c){
		var g,h,tmp,f2,c2, f=g=f.v.toLowerCase(), c=h=c.v.toLowerCase();
		if (!(f.match(dateReg) && c.match(dateReg))) {
			if (f.match(curReg)){
				f=f.replace(/\./g,'').replace(/,/,'.');
				c=c.replace(/\./g,'').replace(/,/,'.');
			}
// 			if (c.indexOf('R$')>=0){c.replace(/\./g,'');c.replace(/,/,'.');}
			var i=parseFloat(f.replace(curReg,''));
			var n=parseFloat(c.replace(curReg,''));
