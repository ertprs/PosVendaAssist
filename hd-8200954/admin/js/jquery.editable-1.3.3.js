...
$.fn.editable = function(options){
	...
	options.toEditable = function(){
		...
		// Submit Event
		if(opts.submit){
			$('<button style="z-index:100;position:relative"/>').appendTo($this)
						.html(opts.submit)
...
$.editableFactory = {
	...
	'textarea': {
		toEditable: function($this,options){
			$('<textarea style="z-index:99;position:absolute"  id="editor_'+opts.idEdit+'"/>').appendTo($this)
							.val($this.data('editable.current'));
		},
		getValue: function($this,options){
			return $this.children().val();
	...
	'select': {
		toEditable: function($this,options){
			$select = $('<select rel="sinalizadores" />').appendTo($this);
