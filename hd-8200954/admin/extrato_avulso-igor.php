...
//if ($ip == '201.0.9.216') echo "1) $sql<br>";
	...
	if (($login_fabrica <> 11 and  $login_fabrica <> 20 ) and (strlen($extrato) == 0)){
...
if ($ip == '201.27.212.208') echo $msg_erro;
	if (strlen ($msg_erro) == 0) {
		...
		if($login_fabrica == 20){
			echo "ROLLBACK: sql: $txt_bosch <br>erro: $msg_erro";
		}
...
<tr>
	...
	<?
	if ($login_fabrica == 11 or $login_fabrica == 20 ) {
...
<TR class='menu_top'>
	<TD ALIGN='center' colspan='3'>LANÇAMENTO DE EXTRATO AVULSO</TD>
</TR>
...
for ($i = 0; $i < 5; $i++){
	...
	$imposto_iva         = @pg_result($res,$i,imposto_iva);
...
//	$debito_credito      = @pg_result($res,$i,debito_credito);
	...
	if (pg_numrows($res1) > 0) {
		echo "<select class='frm' style='width: 200px;' name='lancamento_$i' $disabled >\n";
...
//	echo "<TD><input type='text' class='frm' name='historico_$i' value='$historico' size='50' maxlength='50'></TD>";
	echo "<TD><textarea name=\"historico_$i\" rows=\"3\" cols=\"50\" class='frm' $disabled >$historico</TEXTAREA></TD>";
	echo "<TD><input type='text' class='frm' name='valor_$i' value='$valor' size='10' maxlength='10' $disabled >";
