...
if (strlen($liberar) > 0){
    ...
    if($login_fabrica ==11 or $login_fabrica ==25 || ($login_fabrica == 30)){
        ...
        if($recalculo_pendente=='t'){
            if ($login_fabrica == 30) {
                $msg_erro = "O extrato $liberar está pendente de recálculo, recalcular antes de liberar";
                $extrato = $liberar;
            }
            else {
        ...
        if (strlen ($msg_erro) == 0) {
            if($login_fabrica == 52){
                ...
                if($email_posto != ""){
                    ...
                    $mensagem    .="<br /><br />O(s) extrato(s) Fricon Nº $liberar foi liberado, favor enviar a nota fiscal de prestação de serviços para pagamento e informar no corpo da nota os dados bancários\n";
                    $mensagem    .="<br /><br />----------\n";
                    $mensagem    .="<br />Qualquer dúvida entrar em contato com a Fricon.";
                    $headers= "From:".$remetente."\nContent-type: text/html\n";

                    mail($destinatario, utf8_encode($assunto), utf8_encode($mensagem), $headers);
                }else{
                    echo "<script language='javascript'>alert('Não foi possivel encontrar o email do posto, favor atualizar os dados');</script>";
                }
            }
    ...
    //              }
        ...
        if($login_fabrica == 52 AND strlen($msg_erro) == 0 AND strlen($extrato) > 0){
            ...
            if($email_posto != ""){
                ...
                $mensagem    .="<br /><br />O(s) extrato(s) Fricon Nº $extrato foi liberado, favor enviar a nota fiscal de prestação de serviços para pagamento e informar no corpo da nota os dados bancários\n";
                $mensagem    .="<br /><br />----------\n";
                $mensagem    .="<br />Qualquer dúvida entrar em contato com a Fricon.";
                $headers=  "From".$remetente."\nContent-type: text/html\n";
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
       ...
       // $("#estados").multiSelect();
        ...
        {
            if( confirm("Deseja realmente acumular o(s) extrato(s) para o próximo mês?") )
            {
        ...
        {
            if( confirm("Deseja realmente acumular o extrato para o próximo mês?"))
            {
...
function pesquisaPosto(campo,tipo)
{
    ...
    }else alert("Informar toda ou parte da informação para realizar a pesquisa!");
}
...
function contadorOS(){
    ...
    $("#qtdeOS").text('Ordem Serviço: '+valor);
                ...
                $.ajax({
                    ...
                    complete: function(data){
                        ...
                        if(dados == extrato){
                            ...
                            $('.extrato_aprova_'+extrato).hide();
                            $("label[for='extrato_aprovado_"+extrato+"']").css("display","block").html('Não aprovado');
                        }
                    }
                });
            }
...
if(strlen($btnacao) > 0){
    ...
    if($login_fabrica == 42){
        ...
        //é referente ao campo Data Envio Financeiro
        ...
        //referente ao campo Valor NF Serviços
...
echo "<TR align='left'>\n";
echo "<TD width='25'>&nbsp;</TD>";
echo "<TD ALIGN='left'>Nº de extrato </TD>";
//hd-1098022: Fricon -> se form Fricon coloca campo Pesquisa OS para filtrar resultados pelo número da OS
if($login_fabrica==52){
...
if($login_fabrica == 91){
    echo "</tr><tr>";
    echo "<TD width='25'>&nbsp;</TD>";
    echo "<td width='100' align='left'>  Data da Baixa Início </td>";
...
if($login_fabrica == 42){
    ...
    echo "<TR align='left'>";
    echo "<TD width='50'>&nbsp;</TD>";
    echo "<TD>Nº NF M.O. </td>",
         "<td > Valor NF M.O. </td>",
         "<td >Nº NF Peças </td>",
         "<td > Valor NF Peças </td>";
    echo "</tr>";
    //campos
    echo "<TR align='left'>";
    echo "<TD width='50'>&nbsp;</TD>";
...
echo "  <TD width='15'>";
echo "CNPJ</td>",
"<td colspan='2'>Razão Social</tr><tr align='left'><TD width='25'>&nbsp;</TD><td>";
...
if (($login_fabrica == 15 AND $liberacao == 't') or
    ...
    //monta sql de acordo com a condição do extrato
...
//se for Fricon (52) monta condição para filtrar tbm pela OS
    if(!empty($nroOs)){
...
//select que traz os results sets da consulta (ação do botao Filtrar)
    $sql = "SELECT DISTINCT {$camposSql}
            ...
            if($login_fabrica == 42){
                if(!empty($condSqlMakita)){
                    //Na sql normal está fazendo um left join, mas como é preciso filtrar os resultados, tem que fazer um Inner Join
    ...
    if (pg_num_rows ($res) > 0) {
        ...
        //  if ($login_fabrica == 14)
                ...
                echo "<th align='center' class='titulo_coluna' nowrap style='width:85px;'>Código</th>";
                ...
                if ($login_fabrica == 1) {
                    ...
                    echo "<th align='center' class='titulo_coluna'>Total Peça</th>\n";
                    ...
                    if ($login_fabrica == 6) {//hd 3471
                        echo "<th align='center' class='titulo_coluna'><acronym title='Média de valor pago nos últimos 6 meses' style='cursor: help;'>Média</th>\n";

                    }
                    // SONO - 04/09/206 exibir valor_liquido para intelbras //
                    if ($login_fabrica == 14 || $login_fabrica == 91) {
                        echo "<th align='center' class='titulo_coluna' nowrap>Total Líquido</th>\n";
                    }
                }
                ...
                if($login_fabrica == 151) echo "<th align='center' class='titulo_coluna' nowrap>Nota Fiscal de Serviço</th>";
                ...
                if ($login_fabrica == 24 ) {
                    ...
                    echo "<th align='center' class='titulo_coluna'>
                                <span title=\"Opção para acumular válida apenas para extratos com valor de até R$50,00.\">Acumular</span>
                ...
                if ($login_fabrica == 35) {
                    echo "<th align='center' class='titulo_coluna'>Ações</th>";
                }
                ...
                ?>
                    ...
                    <th class='titulo_coluna'>Impressão <input type="checkbox" onclick="checkPrintCheckBox($(this)[0])" /></th>
                <?php
                endif;
                ...
                if($login_fabrica == 30){
                ?>
                    <th align="center" class='titulo_coluna'>Nº OC</th>
            ...
            if($login_fabrica == 151){
                ...
                if(strlen($nf_autorizacao) == 0){
                    $nf_autorizacao = "<a href='../nota_servico_extrato.php?extrato=$extrato' rel='shadowbox; width= 400; height= 250;'>Informar Nota de Serviço</a>";
                }else{
		...
		if ($login_fabrica == 20) {
            ...
            if($login_fabrica == 45 && strlen($aprovado) > 0 && strlen($baixado) > 0){
                echo "<td colspan='4'>Extrato já Aprovado</td>";
            }else{
    ...
    //                          echo " <input type='checkbox' class='frm' name='liberar_$i' id='liberar' value='$extrato'>";
                ...
                if ($msg_os_deletadas == "") {
                    ...
                    elseif (strlen($aprovado) == 0 OR $login_fabrica == 8 or $login_fabrica == 104 or $login_fabrica == 105)
                        echo "<a href='extrato_avulso.php?extrato=$extrato&posto=$posto'><img class='extrato_adicionar_$extrato' src='imagens/btn_adicionar_azul.gif' id='img_adicionar_$i' ALT = 'Lançar itens no extrato'></a>";
                    echo "</td>\n";
