...
<body>
<BR>
<table width = '500' align = 'center' cellpadding='5' cellspacing='0' border='1' >
<form name='frm_relatorio' action='<? echo $PHP_SELF; ?>' method='post' >
<tr>
	<td colspan='8' class='table_top'><center><B>Relatório de conferência das peças devolvidas</B></center></td>
</tr>
<tr>
	...
	<td width='50' class='table_line'>&nbsp;</td>
	<td align='right' class='table_line'>Posto: </td>
	<td class='table_line' ><input type='text' name='codigo_posto' size='10' maxlength='20' ></td>
</tr>
<tr>
	<td colspan='8' class='table_line'><center><input type='submit' name='btn_acao' value='Gerar Relatório'></center></td>
</tr>
</form>
</table>
...
echo "<br><a href='relatorio_peca_devolvida_imprimir.php?btn_acao=posto&ano=$ano&mes=$mes&codigo_posto=$xcodigo_posto&posto=$xposto'  target='blank'><font face='Verdana, Geneva, Arial, Helvetica, sans-serif'>Versão para impressão</font></a>";
?>	<BR>
...
<?
	...
	if (strlen($ano) > 0 && strlen($mes) > 0 && strlen($codigo_posto) > 0 && strlen($posto) > 0){
		...
		if ($total_rows > 0){
		echo "<br>
<a href='relatorio_peca_devolvida_imprimir.php?btn_acao=posto&ano=$ano&mes=$mes&codigo_posto=$codigo_posto'  target='blank'>
<font face='Verdana, Geneva, Arial, Helvetica, sans-serif'>Versão para impressão</font></a>";
?>
