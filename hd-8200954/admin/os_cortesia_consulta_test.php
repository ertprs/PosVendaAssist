...
if(strlen($numero_os)==0){ #HD 306769
...
if(strlen($chk6) > 0){
	...
	if(strlen($msg_erro)==0){
		...
		$cont = 0;
		while($aux_data_inicial <= $aux_data_final){//enquanto uma data for inferior a outra {      
		  $aux_data_inicial += 86400; // adicionando mais 1 dia (em segundos) na data inicial
		  $cont++;
		}
...
<!--
	...
	function deleteOS(os,numero){
		
		if(confirm('Deseja mesmo exluir esta Ordem de Serviço?')){
			$.ajax({
				url: "<?php echo $_SERVER['PHP_SELF']; ?>?delOS=1&idOS="+os,
				cache: false,
...
if (@pg_numrows($res) == 0) {
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++){
		...
		echo "<TR class='table_line' id='requisito_".$i."' style='background-color: $cor;'>\n";
		...
		if((strlen($explodida) == 0 and strlen($consumidor_revenda)==0)){
			echo "<input type='button' value='Alterar' width='60' height='10' onclick=\"javascript:window.open('os_revenda_cortesia.php?os_revenda=$os')\" >";	
		}elseif(strlen($consumidor_revenda)==0 and strlen($explodida) >0){
			echo "";
		}elseif (($excluida == "f" OR strlen($excluida) == 0)) {
			echo "<input type='button' value='Alterar' width='60' height='10' onclick=\"javascript:window.open('os_cortesia_cadastro.php?os=$os')\" >";
		}
		...
		if(strlen($consumidor_revenda)==0 and ($excluida == "f" OR strlen($excluida) == 0)){
			echo  "<input type='button' value='Imprimir' width='60' height='10' onclick=\"javascript:window.open('os_revenda_print.php?os_revenda=$os')\" >";
			
		}elseif ($excluida == "f" OR strlen($excluida) == 0) {
			echo "<input type='button' value='Consultar' width='60' height='10' onclick=\"javascript:window.open('os_press.php?os=$os')\">";
		}
		echo "</TD>\n";

		echo "<TD>";
		if (strlen($data_fechamento) == 0 AND ( $excluida == "f" OR strlen($excluida) == 0 ) ) echo "<input type='button' value='Excluir' width='60' height='10' onclick='deleteOS($os,$i)'>";
		echo "</TD>\n";
