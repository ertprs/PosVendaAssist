...
if ($btn_acao == "ALTERAR") {
	...
	// Transferencia de OS - Feito por Fábio 22/02/2007
	// Se a OS está no extrato, retira do extrato e coloca o lança um extrato avulso no lugar
	...
	if (strlen($erro) == 0) {
		...
		if (pg_numrows($res) == 1) {
			...
			$mo_os= pg_result($res,0,mao_de_obra);
		}else{
	...
	// Se a OS está no extrato, retira e deconto o valor da MO no próximo extrato.
	if (strlen($extrato) > 0 && strlen($erro)==0) {
		$sql =	"SELECT extrato,
		...
		$res = pg_exec($con, $sql);
		if (pg_numrows($res) == 1) {
			$extrato_anterior     = pg_result($res,0,extrato);
			$data_geracao = pg_result($res,0,data_geracao);
			$sql =	"UPDATE tbl_os_extra 
					SET	extrato = NULL
					WHERE extrato=$extrato_anterior
					AND os=$os";
			$res = @pg_exec($con, $sql);
			...
			$sql = "INSERT INTO tbl_extrato_lancamento (
						...
						$extrato_anterior,
						54   ,
						'Transferência da OS $sua_os para posto $posto_codigo_destino'    ,
						$mo_os
					)";
			$res = @pg_exec($con, $sql);
			$erro .= pg_errormessage($con);
			...
			$sql =	"SELECT extrato
					FROM tbl_extrato
					WHERE fabrica=$login_fabrica AND aprovado IS NULL
					AND posto=$posto";
			$res = pg_exec($con, $sql);
			if (pg_numrows($res) >0) {
				$extrato	=	pg_result($res,0,extrato);
...
// 			if (strlen($msg_erro) == 0){
// 				echo "<br>Coloca o extrato aprovado=null ->: ".$sql = "UPDATE tbl_extrato SET aprovado = null 
// 						WHERE  extrato = $extrato 
...
// 			}
			if (strlen($msg_erro) == 0){
				$sql = "INSERT INTO tbl_extrato_lancamento (
							...
							$extrato       ,
							36   ,
							'OS $os foi transferida para este posto. Participou do extrato $extrato_anterior',
							-$mo_os
						)";
...
if (strlen($sua_os) > 0) {
	...
	if (strlen($sua_os) > 0)       $sql .= " AND tbl_os.sua_os = '$sua_os'";
