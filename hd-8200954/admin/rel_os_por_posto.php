...
if (strlen($btn_acao) > 0 AND strlen($msg_erro) == 0){
	...
	if (pg_numrows($res) > 0) {
		echo "<table width='700'>";
		echo "<tr class='menu_top_5'>";
		echo "<td colspan='5'>OSs ATÉ 5 DIAS DE ABERTO </td>";
		echo "</tr>";
		echo "<tr class='menu_top_5'>";
	...
	#----------------------- Sem fechamento há mais de entre 5 e 20 dias  --------------------------
	...
	$sql = "SELECT  distinct
			...
			WHERE     tbl_os.fabrica            = $login_fabrica
			AND       tbl_posto_fabrica.fabrica = $login_fabrica
			AND       tbl_os.data_abertura::date BETWEEN (current_date - INTERVAL '20 days')::date AND (current_date - INTERVAL '6 days')::date ";
	...
	if (pg_numrows($res) > 0) {
		echo "<table width='700'>";
		echo "<tr class='menu_top_5_20'>";
		echo "<td COLSPAN=5>OSs SEM FECHAMENTO ENTRE 6 E 20 DIAS </td>";
		echo "</tr>";
		echo "<tr class='menu_top_5_20'>";
		echo "<td width='20%'>NÚMERO DA OS</td>";
		echo "<td>CÓDIGO DO POSTO</td>";
	...
	$sql = "SELECT  distinct
			...
			WHERE     tbl_os.fabrica            = $login_fabrica
			AND       tbl_posto_fabrica.fabrica = $login_fabrica
			AND       tbl_os.data_abertura::date BETWEEN (current_date - INTERVAL '30 days')::date AND (current_date - INTERVAL '21 days')::date ";
	...
	if (pg_numrows($res) > 0) {
		echo "<table width='700'>";
		echo "<tr class='menu_top_20'>";
		echo "<td COLSPAN=5>OSs SEM FECHAMENTO HÁ MAIS DE 20 DIAS </td>";
		echo "</tr>";
		echo "<tr class='menu_top_20'>";
		echo "<td width='20%'>NÚMERO DA OS</td>";
		echo "<td>CÓDIGO DO POSTO</td>";
	...
	if (pg_numrows($res) > 0) {
		echo "<table width='700'>";
		echo "<tr class='menu_top_30'>";
		echo "<td COLSPAN=5>OSs QUE EXCEDERAM O PRAZO LIMITE DE 30 DIAS</td>";
		echo "</tr>";
		echo "<tr class='menu_top_30'>";
		echo "<td width='20%'>NÚMERO DA OS</td>";
		echo "<td>CÓDIGO DO POSTO</td>";
