...
$title = "Auditoria de OS aberta a mais de $dias dias";
...
<caption>Relatório de Auditoria de OS aberta a mais de <?php echo $dias; ?> dias</caption>
...
if (strlen($posto) > 0 or strlen($btn_pesquisa) > 0) {
	...
	$sql =  "
			...
			FROM (
				...
				FROM ( SELECT os FROM tmp_interv_90_$login_admin) ultima
			) interv
			JOIN tbl_os USING(os)
			WHERE tbl_os.fabrica = $login_fabrica 
			AND interv.ultimo_status IN ($aprovacao)
			$Xos
			;
			...
			SELECT tbl_os.fabrica,
					...
					tbl_os.excluida,
					tbl_os.finalizada
			INTO TEMP tmp_os_90_aprova_$login_admin 
			...
			CREATE INDEX tmp_os_90_aprova_${login_admin}_os ON tmp_os_90_aprova_$login_admin(os);

			SELECT  
				tmp_os_90_aprova_$login_admin.os,
				tmp_os_90_aprova_$login_admin.sua_os,
				tbl_posto.posto,
				tbl_posto_fabrica.codigo_posto,
				tbl_posto.nome,
				tmp_os_90_aprova_$login_admin.data_abertura,
				tmp_os_90_aprova_$login_admin.data_digitacao,
				tbl_produto.referencia as produto_referencia, 
				tbl_produto.descricao as produto_descricao,
				(SELECT status_os FROM tbl_os_status WHERE fabrica_status = $login_fabrica AND tmp_os_90_aprova_$login_admin.os = tbl_os_status.os AND status_os IN ($sql_tipo) ORDER BY data DESC LIMIT 1) AS status_os         ,
				(SELECT observacao FROM tbl_os_status WHERE fabrica_status = $login_fabrica AND tmp_os_90_aprova_$login_admin.os = tbl_os_status.os AND status_os IN ($sql_tipo) ORDER BY data DESC LIMIT 1) AS status_observacao,
				(SELECT tbl_status_os.descricao FROM tbl_os_status JOIN tbl_status_os USING(status_os) WHERE fabrica_status = $login_fabrica AND tmp_os_90_aprova_$login_admin.os = tbl_os_status.os AND status_os IN ($sql_tipo) ORDER BY data DESC LIMIT 1) AS status_descricao
			FROM tmp_interv_$login_admin X
			JOIN tmp_os_90_aprova_$login_admin ON tmp_os_90_aprova_$login_admin.os = X.os
			JOIN tbl_posto using(posto)
			JOIN tbl_posto_fabrica on tbl_posto.posto = tbl_posto_fabrica.posto and tbl_posto_fabrica.fabrica = $login_fabrica
			JOIN tbl_produto using(produto)
			WHERE tmp_os_90_aprova_$login_admin.fabrica = $login_fabrica";
			if (!empty($_GET['linha']) && is_numeric($_GET['linha'])) {
				$sql .=	" AND linha = " . abs($_GET['linha']);
			}
			$sql .= " AND excluida is not true 
					  AND finalizada is null
					  $sql_add
					  ORDER BY tmp_os_90_aprova_$login_admin.sua_os; ";
#	echo nl2br($sql);
#	die;
	...
	if (pg_numrows($res) > 0) {
		...
		for ($x = 0; $x < pg_numrows($res); $x++) {
			 ...
			 */
			if ($login_fabrica == 15) {
				$status_observacao = str_replace("90", "60", $status_observacao);
				$status_descricao  = str_replace("90", "60", $status_descricao);
			}
