...
try {
	...
	$state = $_GET['state'];
	$tela = $_GET['tela'];
	#$method = ($method == "webservice") ? "database" : $method;

	if ($method == "webservice" && empty($state) ) {
		...
		if (is_object($address)) {
			...
			if (empty($msg_erro)) {
				...
				if (!empty($address->return->cidade) && !empty($address->return->uf)) {
					if ($busca_ura) {
	...
	} elseif((empty($method) || $method == "database") && empty($state)) {
		include "dbconfig.php";
		include "includes/dbconnect-inc.php";
		$cep = str_replace("-","",$cep);
		$cep = str_replace(".","",$cep);
		...
		if (array_key_exists($state, $array_estados)) {
	        $sql = "SELECT DISTINCT * FROM (
	                    SELECT UPPER(fn_retira_especiais(nome)) AS cidade FROM tbl_cidade WHERE UPPER(estado) = UPPER('{$state}') AND pais = 'BR'
	                    UNION (
