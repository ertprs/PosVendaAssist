...
//exit;
	...
	$yres = pg_exec($con,$sql);
	$fim          = pg_numrows($yres) -1;
	for ($x = 0; $x < pg_numrows($yres); $x++) {
			$total = $total + pg_result($yres,$x,ocorrencia);
		}
	$n_ocorrencia_anterior =0;
	for ($x = 0; $x < pg_numrows($yres); $x++) {

		$y = pg_result($yres,$x,ocorrencia);
		$p_ocorrencia = ( $y/ $total ) * 100;

		if ($x==0) {
			$ocorrencia = pg_result($yres,$x,ocorrencia);
			
			$descricao  = substr(pg_result($yres,$x,descricao), 0, 35);
			$porc_ocorrencia = $p_ocorrencia;
		}
		elseif ($x>=9){
			$fim          = pg_numrows($yres) -1;
			$n_ocorrencia = pg_result($yres,$x,ocorrencia);
...
//echo "<br><br>Total".$n_ocorrencia;
			...
			$n_descricao  = substr(pg_result($yres,$x,descricao), 0, 35);
