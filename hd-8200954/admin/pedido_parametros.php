...
if ($_POST['ajax_status']) {
    if ($_POST['periodo']) {
        $resultado = "<option value=''>".traduz("Selecione o Status")."</option>";
        ...
        if ($periodo == '2') {
            ...
            $msg .= traduz("Pedidos lançados ontem");
        }
        ...
        if ($periodo == '3') {
            ...
            $msg .= traduz(" Pedidos lançados nesta semana");
        }
        ...
        if ($periodo == '4') {
            ...
            $msg .= traduz("Pedidos lançados na semana anterior");
        }
        ...
        if ($periodo == '5') {
            ...
            $msg .= traduz("Pedidos lançados neste mês");
        }
    } else {
    ...
    if (pg_numrows ($res) > 0) {
        ...
        $resultado = "<option value=''>".traduz("Sem resultados")."</option>";
    }
    ...
    if ($login_fabrica == 42) {
        if ($_GET["reintegrar"]) {
            ...
            if(strlen(pg_errormessage($con))>0){
                $msg_erro = traduz("Falha na reintegração do pedido!");
            } else {
        ...
        switch($tipo){
            case "bloquear":
                ...
                if(!pg_last_error($con)){
                    ...
                    echo traduz("Não foi possível realizar a operação");
                }
            break;
            case "liberar":
                $res = pg_query($con,"BEGIN TRANSACTION");
            ...
            case "cancelar":
                ...
                for($i=0; $i<pg_num_rows($res); $i++){
                    ...
                    $aux_motivo = traduz("'Cancelamento admin'");
                ...
                $assunto = traduz("Cancelamento do Pedido $pedido");
                $mensagem = traduz("O pedido $pedido foi cancelado pelo admin, em caso de dúvida entrar em contato com o financeiro@acaciaeletro.com.br");
                $res = $mailTc->sendMail(
                    $contato_email,
                    $assunto,
                    $mensagem,
...
$title = traduz("RELAÇÃO DE PEDIDOS LANÇADOS");
...
?>
    ...
    $("#linha_produto_m").multiselect({
        selectedText: "selecionados # de #"
    });

    $("#familia_produto").multiselect({
        selectedText: "selecionados # de #"
    });
...
function fcn_valida_formDatas2()
{
	...
	var pesquisa = $("input[name=frm_submit]").val();
        ...
        if (($('input[name=pedidos_nao_exportados]').is(":checked") == true) || ($('input[name=pdd_rpst]').is(":checked") == true) || ($('input[name=chk_opt]').is(":checked") == true) || ($("input[name=data_inicial_01]").val() != "" && $("input[name=data_final_01]").val() != "") || $("input[name=numero_pedido]").val() != "" || $("input[name=arquivo_pedido]").val() != "")
        {
            ...
            var msg = '<?=traduz("Preencha os campos obrigatórios")?>';
		...
		if (nome_posto.length > 0 && cod_posto.length == 0) {
			...
			if (msg == "") {
				msg = '<?=traduz("Preencha o código do posto")?>';
...
<div class="container">
    ...
    <div class="alert alert-danger">
        <h4><?=traduz('GERAÇÃO E EXPORTAÇÃO DE PEDIDOS')?></h4><br />
        <p class="tal"><b><?=traduz('Pedidos de Peças:')?></b> <?=traduz('São gerados toda Segunda-feira, Quarta-feira e Sexta-feira. A exportação para Gama é realizada todos dias.')?></p>
        <p class="tal"> <b><?=traduz('Pedidos de Troca de Produtos:')?></b> <?=traduz('São gerados todos os dias, a exportação e o faturamento destes pedidos é manual através do Telecontrol')?></p>
    </div>
    <?php }?>
<div class="row">
		...
		<div class="span2"></div>
			<div class="span4">
				<label class="checkbox">
					<INPUT TYPE="checkbox" <?= (strlen($_POST["pdpp"]) > 0) ? "checked" : ""; ?> NAME="pdpp" value="25" rel="5"><?=traduz('Pedidos com Pagamento Pendente')?>
				</label>
			</div>
		<div class="span2"></div>
	</div>
...
<? if ($login_fabrica == 1) { ?><br />
	...
	<div class="row-fluid">
		...
		<div class="span4">
			<label class="radio">
				<input type="radio" name="tipo_pedido" value="" checked ><?=traduz('Todos')?>
			</label>
		</div>
		<div class="span4">
			<label class="radio">
				<input type="radio" name="tipo_pedido" <?= ($_POST["tipo_pedido"] == "87|peca") ? "checked" : ""; ?> value="87|peca"><?=traduz('Garantia Peças')?>
			</label>
		</div>
		<div class="span2"></div>
	</div>
	<div class="row-fluid">
		...
		<div class="span4">
			<label class="radio">
				<input type="radio" name="tipo_pedido" <?= ($_POST["tipo_pedido"] == "87|produto") ? "checked" : ""; ?> value="87|produto"><?=traduz('Garantia Produtos')?>
			</label>
		</div>
		<div class="span4">
			<label class="radio">
				<input type="radio" name="tipo_pedido" <?= ($_POST["tipo_pedido"] == "86") ? "checked" : ""; ?> value="86"><?=traduz('Faturado')?>
			</label>
		</div>
		<div class="span2"></div>
	</div>
	<div class="row-fluid">
		...
		<div class="span4">
			<label class="radio">
				<input type="radio" name="tipo_pedido" <?= ($_POST["tipo_pedido"] == "94") ? "checked" : ""; ?> value="94"><?=traduz('Locador')?>
			</label>
		</div>
		<div class="span4">
			<label class="radio">
				<input type="radio" name="tipo_pedido" <?= ($_POST["tipo_pedido"] == "93") ? "checked" : ""; ?> value="93"><?=traduz('Acessórios')?>
			</label>
		</div>
		<div class="span2"></div>
	</div>
...
<? $disabled = (strlen($_POST["chk_opt5"]) > 0) ? "disabled" : ""; ?>
	...
	<div class="row-fluid">
        ...
        <div class="span2">
            <div class='control-group ' id="campo_nume+ro_pedido">
                    <label class="control-label" for='numero_pedido'><?=traduz('Número do pedido')?></label>
        ...
        <div class="span2">
            <div class='control-group'>
                    <label class="control-label" for='arquivo_pedido'><?=traduz('Arquivo de pedido')?></label>
                    <div class="controls controls-row">
	...
	<div class="row-fluid">
		...
		<div class="span4">
			<div class="control-group" id="campo_cod_posto">
				<label class="control-label" for=""><?=traduz('Código Posto')?></label>
				<div class="controls controls-row">
    ...
    <?php if(in_array($login_fabrica, [169,170])){ ?>
        ...
        <div class="span2"></div>
        <div class="span4">
            <div class="control-group" id="escritorio_venda">
                <label class="control-label" for=""><?=traduz('Escritório de Venda')?></label>
                <div class="controls controls-row">
                    <div class='span7'>
                        <select name="escritorio_venda" class="frm">
                            <option value="">Selecione um Escritório</option>
                            <?php 
                            $sqlEscritorio = "select codigo, descricao from tbl_escritorio_venda WHERE ativo is true ";
                            $resEscritorio = pg_query($con, $sqlEscritorio);
                            for($es = 0; $es<pg_num_rows($resEscritorio); $es++){
                                $codigo = pg_fetch_result($resEscritorio, $es, 'codigo');
                                $descricao = pg_fetch_result($resEscritorio, $es, 'descricao');
                                
                                $selected = " ";
                                if($escritorio_venda == $codigo){
                                    $selected = " selected ";
                                }
        ...
        <div class="span4">
            <div class="control-group" id="equipe_venda">
                <label class="control-label" for=""><?=traduz('Equipe de Venda')?></label>
                <div class="controls controls-row">
                    <div class="span7 ">
                        <select name="equipe_venda" class="frm">
                            <option value="">Selecione uma Equipe</option>
                            <?php 
                            $sqlEscritorio = "select codigo, descricao from tbl_equipe_venda WHERE ativo is true ";
                            $resEscritorio = pg_query($con, $sqlEscritorio);
                            for($es = 0; $es<pg_num_rows($resEscritorio); $es++){
                                $codigo = pg_fetch_result($resEscritorio, $es, 'codigo');
                                $descricao = pg_fetch_result($resEscritorio, $es, 'descricao');
        ...
        <div class='row-fluid'>
            ...
            <div class='span4'>
                <div class='control-group <?=(in_array("cliente", $msg_erro["campos"])) ? "error" : ""?>'>
                    <label class='control-label' for='cliente_codigo'><?=traduz('Código Cliente')?></label>
                    <div class='controls controls-row'>
                        <div class='span7 input-append'>
            ...
            <div class='span4'>
                <div class='control-group <?=(in_array("cliente", $msg_erro["campos"])) ? "error" : ""?>'>
                    <label class='control-label' for='cliente_nome'><?=traduz('Nome Cliente')?></label>
                    <div class='controls controls-row'>
...
<? if (!in_array($login_fabrica, [6,104])) { ?> 
	<div class="row-fluid">
	<div class="span2"></div>
		<div class="span4">
			<div class="control-group">
				<label class="control-label" for=""><?=traduz('Referência Peça')?></label>
				<div class="controls controls-row">
					<div class="span7 input-append">
		...
		<div class="span4">
			<div class="control-group">
				<label class="control-label" for=""><?=traduz('Descrição Peça')?></label>
				<div class="controls controls-row">
...
<? if ($login_fabrica == 1) { ?>
	<div class="row-fluid">
		<div class="span2"></div>
			<div class="span4">
				<div class="control-group">
					<label class="control-label" for=""><?=traduz('Código Representante')?></label>
					<div class="controls controls-row">
						<div class="span7 input-append">
			...
			<div class="span4">
				<div class="control-group">
				<label class="control-label" for=""><?=traduz('Descrição Representante')?></label>
					<div class="controls controls-row">
						<div class="input-append">
...
if(in_array($login_fabrica, [6,104,151])){ ?>
	<div class="row-fluid">
		<div class="span2"></div>
			<div class="span4">
				<div class="control-group">
					<label class="control-label" for=""><?=traduz('Referência do produto')?></label>
					<div class="controls controls-row">
						<div class="span7 input-append">
			...
			<div class="span4">
				<div class="control-group">
				<label class="control-label" for=""><?=traduz('Descrição do produto')?></label>
					<div class="controls controls-row">
						<div class="input-append">
...
<?php if(in_array($login_fabrica, array(120,151))){//hd_chamado=2765193 ?>
	<br />
	<div class="row-fluid">
		<div class="span2"></div>
				<div class="span4">
					<label class="control-label" for=""><?=traduz('Linha')?></label>
					<div class="controls controls-row">
                        <? if($login_fabrica == 151){
                            echo "<select class='frm' name='linha_produto[]' id='linha_produto_m' multiple='multiple'>";
                        } else { ?>
                ...
                <? if($login_fabrica == 151){ ?>             
                    <div class="span4">    
                        <label class="control-label" for=""><?=traduz('Família')?></label>
                        <div class="controls controls-row">
                              <select class='frm' name="familia_produto[]" id="familia_produto" multiple="multiple">
                                <?php
                                ...
                                foreach (pg_fetch_all($res) as $key) {
                                    $selected_familia = ( isset($_POST['familia_produto']) and ($_POST['familia'] == $key['familia']) ) ? "SELECTED" : '' ;
                                ?>
...
if (in_array($login_fabrica, array(1,72,151))) { #HD 280384 ?>
	<div class="row-fluid">
		<div class="span2"></div>
        ...
        <?php }?>
        <?php if ($login_fabrica == 1 OR $login_fabrica == 151) {?>
        <div class="span4">
            <label class="control-label" for="">Estado</label>
            <div class="controls controls-row">
...
<? if($login_fabrica == 3){ ?>
	...
	<div class="row-fluid">
		...
		<div class="span2">
			<label class="radio">
				<input type="radio" name="tipo_pedido" <?= ($_POST["tipo_pedido"] == "3") ? "checked" : ""; ?> value="3"><?=traduz(' Garantia')?>
			</label>
		</div>
		<div class="span2">
			<label class="radio">
				<input type="radio" <?= ($_POST["tipo_pedido"] == "2") ? "checked" : ""; ?> name="tipo_pedido" value="2"><?=traduz(' Faturado')?>
			</label>
		</div>
		<div class="span3"></div>
	</div>
...
<? } ?>
        ...
        <div class="span2"></div>
                <div class="span4">
                    <label class="control-label" for=""><?=traduz('Status do Pedido')?></label> <span class="add-on" data-toggle="tooltip" data-placement="top" title="Apresenta os status de pedidos referentes ao periodo selecionado"><i class="icon-question-sign"></i></span>
                    <div class="controls controls-row">
                    <? if (isset($_REQUEST['pedido_status'])) {
                        ...
                        <? if (!isset($_REQUEST['pedido_status'])) { ?>
                            <option><?=traduz('Selecione um período')?></option>
                        <? } else { ?>
            ...
            <div class="span4">
                <label class="control-label" for=""><?=traduz('Tipo de Pedido')?></label>
                <?
    ...
    <?php if ($login_fabrica == 104) { 
        if (isset($_REQUEST['tipo_peca'])) { $tipoPeca = $_REQUEST['tipo_peca']; } ?>
        <div class="row-fluid" >
            <div class="span2" ></div>
            <div class="span4">
                <label class="control-label" for="">Tipo de Peça</label>
                <select name="tipo_peca" class="frm" >
                    <option selected></option>
                    <option value='1' <? if ($tipoPeca == 1) echo "selected";?>> Devolução obrigatória </option>
                    <option value='2' <? if ($tipoPeca == 2) echo "selected";?>> Item de aparência </option>
                    <option value='3' <? if ($tipoPeca == 3) echo "selected";?>> Peça acumulada para kit </option>
                    <option value='4' <? if ($tipoPeca == 4) echo "selected";?>> Peça retorno para conserto </option>
                    <option value='5' <? if ($tipoPeca == 5) echo "selected";?>> Bloqueada para garantia </option>
                    <option value='6' <? if ($tipoPeca == 6) echo "selected";?>> Acessórios </option>
                    <option value='8' <? if ($tipoPeca == 8) echo "selected";?>> Peça Crítica </option>
                    <option value='9' <? if ($tipoPeca == 9) echo "selected";?>> Produto Acabado </option>
                </select>
            </div>
        </div>
    <?php } ?>
	...
	?>
		<div class="row-fluid" >
			<div class="span2" ></div>
			<div class="span4">
				<label class="control-label" for=""><?=traduz('Inspetor')?></label>
				<div class="controls controls-row">
            ...
            <div class="span4">
				<label class="control-label" for=""><?=traduz('Tipo de Posto')?></label>
				<div class="controls controls-row">
        ...
        <div class="span4">
          ...
          <div class="controls controls-row">
            <select name='estado_posto_autorizado' class='frm'>
                    ...
                    <?php
                      foreach ($array_estados("BR") as $sigla => $estados) {
...
?>
    <div class="row-fluid">
        <div class="span2"></div>
        <div class="span4">
            <label class="control-label" for=""><?=traduz('Destinatário Troca')?></label>
            <div class="controls controls-row">
                <select name='destinatario_troca' id='destinatario_troca' class='frm' <?=empty($destinatario_troca) ? "disabled" : ""?>>
                    <option value=''><?=traduz(' - Selecione -')?></option>
                    <option value='posto' <?=($destinatario_troca == "posto") ? "selected" : ""?>><?=traduz(' Posto Autorizado')?></option>
                    <option value='consumidor' <?=($destinatario_troca == "consumidor") ? "selected" : ""?>><?=traduz(' Consumidor')?></option>
                </select>
            </div>
        </div>
    </div>
...
?>
    <div class="row-fluid">
        <div class="span2"></div>
        <div class="span8">
            <label class="control-label" for=""><?=traduz('Categoria do Pedido')?></label>
                <div class="controls controls-row">
                <select name="categoria_pedido" class="frm">
                    <option value=""><?=traduz('SELECIONE')?></option>
                    <option <?=($categoria_pedido == "cortesia") ? "selected" : ""?> value="cortesia"><?=traduz('CORTESIA')?></option>
                    <option <?=($categoria_pedido == "credito_bloqueado") ? "selected" : ""?> value="credito_bloqueado"><?=traduz('CRÉDITO BLOQUEADO')?></option>
                    <option <?=($categoria_pedido == "erro_pedido") ? "selected" : ""?> value="erro_pedido"><?=traduz('ERRO DE PEDIDO')?></optiuon>
                    <option <?=($categoria_pedido == "kit") ? "selected" : ""?> value="kit"><?=traduz('KIT DE REPARO')?></option>
                    <option <?=($categoria_pedido == "midias") ? "selected" : ""?> value="midias"><?=traduz('MÍDIAS')?></option>
                    <option <?=($categoria_pedido == "outros") ? "selected" : ""?> value="outros"><?=traduz('OUTROS')?></option>
                    <option <?=($categoria_pedido == "valor_minimo") ? "selected" : ""?> value="valor_minimo"><?=traduz('VALOR MÍNIMO')?></option>
                    <option <?=($categoria_pedido == "vsg") ? "selected" : ""?> value="vsg"><?=traduz('VSG')?></option>
                </select>
            </div>
        </div>
        <div class="span1"></div>
    ...
    <div class="row-fluid">
        ...
        <div class="span3">
            <label class="checkbox">
                <INPUT TYPE="checkbox" <?= (strlen($_POST["pdd_rpst"]) > 0) ? "checked" : ""; ?> NAME="pdd_rpst" value="S" ><?=traduz('Pedidos Representante')?>
            </label>
        </div>
        <div class="span3">
            <label class="checkbox">
                <INPUT TYPE="checkbox" <?= (strlen($_POST["pedidos_nao_exportados"]) > 0) ? "checked" : ""; ?> NAME="pedidos_nao_exportados"><?=traduz('Pedidos Não Exportados')?>
            </label>
        </div>
        <div class="span3">
            <label class="checkbox">
                <INPUT TYPE="checkbox" <?= (strlen($_POST["valor_pedido"]) > 0) ? "checked" : ""; ?> VALUE='T' NAME="valor_pedido"><?=traduz('Valor de Pedido')?>
            </label>
        </div>
        <div class="span2"></div>
    </div>
    <div class="row-fluid">
        ...
        <div class="span3">
            <label class="checkbox">
                <INPUT TYPE="checkbox" NAME="chk_opt10" <?= (strlen($_POST["chk_opt10"]) > 0) ? "checked" : ""; ?> value="finalizado" rel="10"><?=traduz('Pedido Não Finalizado')?>
            </label>
        </div>
        <div class="span3">
            <label class="checkbox">
                <INPUT TYPE="checkbox" NAME="chk_opt11" <?= (strlen($_POST["chk_opt11"]) > 0) ? "checked" : ""; ?> value="promocional" rel="11"><?=traduz('Pedido Promocional')?>
            </label>
        </div>
        <div class="span3">
            <label class="checkbox">
                <INPUT TYPE="checkbox" NAME="chk_opt12" <?= (strlen($_POST["chk_opt12"]) > 0) ? "checked" : ""; ?> value="sedex" rel="12"><?=traduz('Pedido Sedex')?>
            </label>
        </div>
        <div class="span1"></div>
    </div>
...
?>
		<div class="row-fluid">
			<div class="span2"></div>
				<div class="span4">
					<label class="radio">
                        <div id="click" style="display:inline-block; position:relative;">
						    <INPUT TYPE="radio" class="checkbox_periodo" NAME="chk_opt" value="1" id='chk_opt1' rel="1" <?= $required ?> <?= ($_POST["chk_opt"] == "1") ? "checked" : ""; ?>><?=traduz('Pedidos Lançados Hoje')?>
				...
				<div class="span4">
					<label class="radio">
                        <div id="click" style="display:inline-block; position:relative;">
						    <INPUT TYPE="radio" class="checkbox_periodo" NAME="chk_opt" value="2" rel="2" <?= $required ?> <?= ($_POST["chk_opt"] == "2") ? "checked" : ""; ?>><?=traduz('Pedidos Lançados Ontem')?>
                            <div class='click_periodo' style="position:absolute; left:0; right:0; top:0; bottom:0;"></div>
                        </div>
					</label>
				</div>
		...
		<div class="row-fluid">
			<div class="span2"></div>
				<div class="span4">
					<label class="radio">
                        <div id="click" style="display:inline-block; position:relative;">
						    <INPUT TYPE="radio" class="checkbox_periodo" NAME="chk_opt" value="3" rel="3" <?= $required ?> <?= ($_POST["chk_opt"] == "3") ? "checked" : ""; ?>><?=traduz('Pedidos Lançados Nesta Semana')?>
                            <div class='click_periodo' style="position:absolute; left:0; right:0; top:0; bottom:0;"></div>
                        </div>
					</label>
				</div>
				<div class="span4">
					<label class="radio">
                        <div id="click" style="display:inline-block; position:relative;">
						    <INPUT TYPE="radio" class="checkbox_periodo" NAME="chk_opt" value="4" rel="4" <?= $required ?> <?= ($_POST["chk_opt"] == "4") ? "checked" : ""; ?>><?=traduz('Pedidos Lançados Na Semana Anterior')?>
                            <div class='click_periodo' style="position:absolute; left:0; right:0; top:0; bottom:0;"></div>
                        </div>
					</label>
				</div>
		...
		<div class="row-fluid">
			<div class="span2"></div>
				<div class="span4">
					<label class="radio">
                        <div id="click" style="display:inline-block; position:relative;">
						    <INPUT TYPE="radio" class="checkbox_periodo" NAME="chk_opt" value="5" rel="5" <?= $required ?> <?= ($_POST["chk_opt"] == "5") ? "checked" : ""; ?>><?=traduz('Pedidos Lançados Neste Mês')?>
                            <div class='click_periodo' style="position:absolute; left:0; right:0; top:0; bottom:0;"></div>
                        </div>
					</label>
				</div>
				...
				<div class="span4">
					<label class="checkbox">
                        <div  style="display:inline-block; position:relative;">
						    <INPUT TYPE="checkbox"  NAME="csv" value="csv" rel="4"  <?= ($_POST["csv"] == "csv") ? "checked" : ""; ?>><?=traduz('Download de Arquivo em CSV')?>
        ...
        <? if($login_fabrica == 160 or $replica_einhell) { ?>
            <div class="row-fluid">
                ...
                <div class="span4">
                    <label class="checkbox">
                        <div  style="display:inline-block; position:relative;">
                            <INPUT TYPE="checkbox" <?= ($_POST["csv_detalhado"] == "csv_detalhado") ? "checked" : ""; ?> NAME="csv_detalhado" value="csv_detalhado" rel="4"><?=traduz('Relatório Detalhado por Peça')?>
                            <div class='' style="position:absolute; left:0; right:0; top:0; bottom:0;"></div>
                        </div>
                    </label>
                </div>
        ...
        <? } ?>
		<br />
		<input class="btn" type="button" onclick="javascript: fcn_valida_formDatas2();" alt="<?=traduz("Preencha as opções e clique aqui para pesquisar")?>" value="<?=traduz("Pesquisar")?>">
		<br /><br />
...
if(pg_numrows($resT) > 0){
	...
	if($total > 0 && !in_array($login_fabrica, array(14,40,152,154,165,180,181,182)) && !isset($FaturamentoManualArquivo)  ){
        if ($login_fabrica != 94) {
?>
		<form name="frm_pesquisa1" METHOD="POST" ACTION="pedido_nao_exportado_consulta.php">
			<button style="width: 40%;" class="btn btn-primary" src="imagens/btn_exibirpedidosnaoexportados.gif" alt="<?=traduz("Preencha as opções e clique aqui para pesquisar")?>"" onclick="javascript: fcn_valida_formDatas();" style="cursor:pointer "><?=traduz('Pesquisar pedidos não exportados')?></button>
			<BR />
		</form>
...
?>
		<form name="frm_pesquisa3" method="POST" action="pedido_nao_faturado_consulta.php" id="frm_pesquisa3">
			<button class="btn btn-primary" style="width: 40%;" src="imagens/btn_exibirpedidosnaofaturados.gif" alt="<?=traduz("Preencha as opções e clique aqui para pesquisar")?>" onclick="document.getElementById('frm_pesquisa3').submit();"><?=traduz('Pesquisar pedidos exportados e não faturados')?></button>
			<br /><br />
		</form>
	...
	<?php
		...
		// }
		if(in_array($login_fabrica, array(40,152,154,158,165,173,178,180,181,182)) || isset($FaturamentoManualArquivo)){
	?>
	<form name="frm_pesquisa4" method="POST" action="consulta_pedido_nao_faturado.php" id="frm_pesquisa4">
		...
		<a href="consulta_pedido_nao_faturado.php">
			<input class="btn btn-default" type="button" class="btn" id="pedido_faturar" name="pedido_faturar" value="<?=traduz("Pedidos a Faturar")?>" style="margin-top: 5px; margin-bottom: 5px;"/>
		</a>
		<a href="upload_faturar_pedido.php">
			<input class="btn btn-success" type="button" class="btn" id="upload_faturamento" name="upload_faturamento" value="<?=traduz("Realizar Faturamento")?>" style="margin-top: 5px; margin-bottom: 5px;"/>
		</a>
	<?php if ($login_fabrica == 177) { ?>
		<a href="baixa_pedido_posto_interno.php" target="_blank" class="btn btn-info" type="button" id="pedido_posto_interno" style="margin-top: 5px; margin-bottom: 5px"><?=traduz('Pedidos do Posto Interno')?></a>
	<?php } ?>
		<br /><br />
	</form>
	<?
