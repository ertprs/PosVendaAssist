...
#		if (strlen($produto_descricao) > 0)   $sql .= " AND tbl_produto.descricao = '$produto_descricao';";
		...
		$msg_erro = " Informe o produto para realizar a pesquisa. ";
	}
}
}
...
<table width="700px" class='formulario' border="0" cellspacing="0" cellpadding="2" align="center">
	<? if (strlen($msg_erro) > 0) { ?>
		<tr>
			<td align='center' class='msg_erro' colspan='6'><?echo $msg_erro?></td>
		</tr>
...
<td>&nbsp;</td>
		...
		<td colspan="2" ALIGN='left'>Descrição do Produto</td>
	...
	<tr>
		<td colspan="6"><input type="button" style="background:url(imagens_admin/btn_pesquisar_400.gif); width:400px;cursor:hand;" value="&nbsp;" onclick="javascript: document.frm_relatorio.acao.value='PESQUISAR'; document.frm_relatorio.submit();" alt="Clique AQUI para pesquisar"></td>
	</tr>
</table>
</form>
...
//	$data_final   = date("Y-m-t", mktime(23, 59, 59, $pesquisa_mes, 1, $pesquisa_ano));
	...
	echo "</table>";
		...
		if( $acao && strlen( $msg_erro ) == 0 )
		{
		echo "<div>Não foram encontrados resultados para esta pesquisa</div>";
		}
	}
}
$mes = $_GET['mes'];
