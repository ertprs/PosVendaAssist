...
$fabrica_kits_produtos = array(15,91,3,30);
if(in_array($login_fabrica,$fabrica_kits_produtos)){
	header("Location: kit_pecas_produtos_cadastro.php");
}
...
if (trim($btn_acao) == "gravar") {
	...
	if($login_fabrica == 123){
		...
		if(strlen($referencia_kit) > 0){
			$sql = "SELECT tbl_peca.peca FROM tbl_peca  
					...
					WHERE tbl_peca.referencia = '$referencia_kit' 
					and tbl_peca.fabrica = $login_fabrica
					and tbl_produto.referencia = '$referencia'";
			$res = pg_query($con, $sql);
			if(pg_num_rows($res)>0){
				$peca_kit = pg_fetch_result($res, 0, peca);
			}else{
				$msg_erro = "Falha ao encontrar peça relacionada ao kit. <Br>";
			}
		}
	}
	...
	if (strlen($msg_erro) == 0) {
		...
		for ($i = 0; $i < $qtde_linhas; $i++) {
			...
			$xpeca = trim($_POST['peca_referencia_' . $i]);
		...
		echo '<div id="div_pecas" style="display:' . (isset($_GET['msg']) || strlen($msg_erro) > 0 || isset($_GET['kit_peca']) ? 'block' : 'none') . '">';
			...
			echo "<table width='400' align='center' border='0'>";
				...
				for ($i = 0; $i < $qtde_linhas; $i++) {
					...
					$peca = "peca_referencia_$i" ;
					$peca = $$peca;

					$descricao = "peca_descricao_$i" ;
					$descricao = $$descricao;
					...
					echo "<tr>";
						...
						echo "<td nowrap>";
							echo "<input type='text' name='peca_referencia_$i' value='$peca' size='20' maxlength='20'>";
							echo "&nbsp;<img src='../imagens/btn_buscar5.gif' border='0' align='absmiddle'";
							 echo " onclick='javascript: fnc_pesquisa_peca (document.frm_kit.peca_referencia_$i , document.frm_kit.peca_descricao_$i , \"referencia\")' style='cursor:pointer'><br />";
							echo "<font size='-3' color='#ffffff'>$peca</font>";
						echo "</td>";
						echo "<td bgcolor='$cor' nowrap>";
							echo "<input type='text' name='peca_descricao_$i' value='$descricao' size='50' maxlength='50'>";
							echo "&nbsp;<img src='../imagens/btn_buscar5.gif' border='0' align='absmiddle'";
							 echo " onclick='javascript: fnc_pesquisa_peca (document.frm_kit.peca_referencia_$i , document.frm_kit.peca_descricao_$i , \"descricao\")' style='cursor:pointer'><br />";
