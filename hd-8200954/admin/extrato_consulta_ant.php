...
if (strlen($liberar) > 0){
	...
	if (strlen($msg_erro)==0 and $login_fabrica==11) {
		$sql = "SELECT email FROM tbl_posto 
				JOIN tbl_extrato USING(posto) 
				WHERE extrato = $liberar";
		$res = @pg_exec($con,$sql);

		$xemail = trim(pg_result($res,0,0));
		if (strlen($xemail) > 0) {
			...
			$headers="Return-Path: <luiz@lenoxxsound.com.br>\nFrom:".$remetente."\nBcc:luiz@lenoxxsound.com.br,wellington@telecontrol.com.br \nContent-type: text/html\n"; 
		...
		if (strlen($extrato) > 0 AND strlen($msg_erro) == 0) {
			...
			if (strlen($msg_erro)==0 and $login_fabrica==11) {
				...
				if (strlen($xemail) > 0) {
					...
					$destinatario = $xemail; 
					$assunto      = "SEU EXTRATO FOI LIBERADO"; 
					$mensagem     = "* O EXTRATO Nº".$extrato." ESTÁ LIBERADO NO SITE: www.telecontrol.com.br *"; 
					$headers="Return-Path: <luiz@lenoxxsound.com.br>\nFrom:".$remetente."\nBcc:luiz@lenoxxsound.com.br,wellington@telecontrol.com.br \nContent-type: text/html\n"; 
...
if ($ip == "201.92.127.116") { echo $sql;flush();}
		...
		if ($login_fabrica == 6 OR $login_fabrica == 14 OR $login_fabrica == 15 OR $login_fabrica == 11 OR $login_fabrica==20) {
			echo "<td align='center'>";
