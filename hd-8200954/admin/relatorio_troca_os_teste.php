...
if ($btn_finalizar == 1) {
	...
	if(!empty($data_inicial) && !empty($data_final) ) { //validaçao de datas
		if(strlen($erro)==0){
...
<?
	...
	echo "<center><b>ATENÇÃO</b></center>Este relatório de BI considera toda  OS que está finalizada, sendo possível fazer a pesquisa com os dados abaixo.";
	echo "</div><br />";?>
...
<TABLE width="700" align="center" border="0" cellspacing="1" cellpadding="0" class='formulario espaco'>
			...
			if (pg_numrows($res) > 0) {
				...
				for ($x = 0 ; $x < pg_numrows($res) ; $x++){
					...
					if ($familia == $aux_familia){
						echo " SELECTED "; 
						$mostraMsgLinha = " da FAMÍLIA $aux_descricao";
					}
		...
		<td class="primeiro">
			...
			if (pg_numrows($res) > 0) {
				...
				for ($x = 0 ; $x < pg_numrows($res) ; $x++){
					...
					if ($pais == $aux_pais){
						echo " SELECTED "; 
						$mostraMsgPais = " do PAÍS $aux_nome";
					}
					echo ">$aux_nome</option>\n";
				}
				echo "</select>\n";
	...
	<TR>
		...
		<button type="button" onclick="javascript: if ( document.frm_pesquisa.btn_finalizar.value == '0' ) { document.frm_pesquisa.btn_finalizar.value='1'; document.frm_pesquisa.submit() ; } else { alert ('Aguarde submissão da OS...'); }" style="cursor:pointer;">Pesquisar</button>
		</TD>
	</TR>
	</TFOOT>
</TABLE>
