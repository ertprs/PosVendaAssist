...
$title = "CADASTRAMENTO ANÁLISE DE PRODUTOS";
...
if(strlen($analise_produto) == 0){
    $title_page = "Cadastro de Análise Produtos";
}else{
    $title_page = "Alteração de Análise Produtos";
}
...
if (isset($_POST["btn_acao"])) {
    ...
    if(strlen($descricao)==0){
        $msg_erro["msg"]["obg"] ="Por favor insira a descrição da Análise";
    ...
    if (count($msg_erro) == 0) {
        if(empty($analise_produto)){
            ...
            if (pg_num_rows($res) > 0) {    
                $msg_erro["msg"]["obg"] = "Código de Análise já cadastrado!";
...
<div class='alert'>Para efetuar alterações, clique na descrição da análise do produto.</div>

<table id="analise_produto" class="table table-striped table-bordered table-hover table-fixed">
    <thead>
        <tr class="titulo_tabela">
            <th colspan="3" nowrap>Análises do produto cadastradas</th>
        </tr>
...
<?
    ...
    <div class='alert alert-warning'><h4>Não existem análises cadastradas</h4></div>
<?}    
