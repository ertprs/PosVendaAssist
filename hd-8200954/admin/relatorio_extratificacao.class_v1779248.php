...
class relatorioExtratificacao
{
	...
	private function prepareStatements()
	{
        ...
        if (empty($this->pecas)) {
            $prepare = pg_prepare("total_os", "select count(distinct(os)) as total from {$this->temp_tbls['os_serie']} join {$this->temp_tbls['numero_serie']} USING(serie,produto) $join_regiao where data_fabricacao between $1 and $2 and data_abertura between $3 and $4 $cond_regiao");
		} else {
		    $cond = implode(', ', $this->pecas);
	...
	private function producao($month = '23')
	{
		...
		$sql4 = "SELECT qtde_produto_produzido as total, cf.mes, cf.ano, cf.cfe
				 ...
				 WHERE cf.fabrica = $this->login_fabrica and cf.familia = $this->familia $cond_regiao AND cf.produto IS NULL";
