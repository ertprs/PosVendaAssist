...
if ($verPopup == "sim"){
  ...
  if(in_array($login_fabrica, array(35,114))){
    ...
    $devolucao = ($movimento =='NAO_RETOR.') ? 'NÃO RETORNÁVEL':"RETORNÁVEL";
    ...
    if (in_array($login_fabrica, array(153))) {
        ...
        $cabecalho_produto .= "<b>CONFERÊNCIA</b>\n";
        ...
        $cabecalho_produto .= "<b> RETORNÁVEL - PRODUTOS </b><br>\n";
        $cabecalho_produto .= "</td>\n";
        $cabecalho_produto .= "</tr>\n";

        $cabecalho_produto .= "<tr>\n";
        $cabecalho_produto .= "<td>Natureza <br> <b>Devolução de Garantia</b> </td>\n";
        ...
        $cabecalho_produto .= "<table border='1' cellspacing='0' cellpadding='3' border-color='#000' style='border-collapse:collapse;font-size:12px' width='650' >\n";
        $cabecalho_produto .= "<tr>\n";
        $cabecalho_produto .= "<td>Razão Social <br> <b>$razao</b> </td>\n";
        $cabecalho_produto .= "<td>CNPJ <br> <b>$cnpj</b> </td>\n";
        $cabecalho_produto .= "<td>Inscrição Estadual <br> <b>$ie</b> </td>\n";
        $cabecalho_produto .= "</tr>\n";
        $cabecalho_produto .= "</table>\n";
        ...
        $cabecalho_produto .= "<table border='1' cellspacing='0' cellpadding='3' border-color='#000' style='border-collapse:collapse;font-size:12px' width='650' >\n";
        $cabecalho_produto .= "<tr>\n";
        $cabecalho_produto .= "<td>Endereço <br> <b>$endereco </b> </td>\n";
        ...
        $cabecalho .= "<b> RETORNÁVEL - PEÇAS </b><br>\n";
        $cabecalho .= "</td>\n";
        $cabecalho .= "</tr>\n";
    ...
    if ($telecontrol_distrib){
       echo "<td><center><IMG SRC='imagens/setona_h.gif' WIDTH='53' HEIGHT='29' BORDER='0' align='absmiddle'>Número da Nota: <input type='text' name='ressarcimento_nota_fiscal' size='10' maxlength='6' value='$nota_fiscal' onblur='alteraNumeroNota(this.value,$nota_fiscal,$login_posto)'></center></td>";
       echo "<tr>
                <td align='center'><div id='div_msg' style='display:none; border: 1px solid #949494;background-color: #F1F0E7;width:180px;'></div>
            </td>
            </tr>";
    }else{
       echo "<td><h4><center>Nº<BA> da NF de Devolução: $nota_fiscal</center></h4></td>\n";
    }
    echo "</tr>";
...
<table border='1' cellspacing='0' cellpadding='3' border-color='#000' style='border-collapse:collapse;font-size:12px;text-align:center' width='650' >
  ...
  <tr>
    ...
    <td>
      ...
      <input type='button' name='gravar' value='Gravar  Observação' onClick="javascript:
         if (this.form.btn_acao.value == ''){
        ...
        <td bgcolor='#F3F8FE'>
            <TABLE width='100%' align='center' border='0' cellspacing='0' cellpadding='2'>
                    ...
                    <tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
                        ...
                        <td><input type="text" name="data_inicial" id="datai" size="12"  value="<?php echo $data_inicial?>" class="frm"> &nbsp;&nbsp;&nbsp;&nbsp;até&nbsp;&nbsp;&nbsp;&nbsp;
...
<?
    ...
    if ($btnacao=='filtrar' && (strlen($filtroRecebidos)>0 || strlen($filtroTodos)>0 || strlen($posto_codigo)>0  || strlen($nota_devolucao)>0 || !empty($filtroNaoObrigatorio) || !empty($filtroObrigatorio))){
        ...
        $thead .=  "Status;"."Posto;"."Extrato;". "Nota Fiscal;". utf8_encode("Data Emissão").";Data Conferencia;". "Valor Nota;". "Cancelada;"."\r\n";
        fwrite($file, $thead);
...
//      $lista .=  "<td></td>";
        ...
        for ($i=0;$i<$qtde_notas;$i++){
            ...
            if (strlen($conferencia)>0 AND $devolucao_concluida=='t' AND strlen($cancelada)==0){
                ...
                $status = utf8_encode("Não Conferida");
            }
            ...
            if($login_fabrica == 35){
                $lista .= "<td nowrap  align='center' title='Número de Postagem $numero_postagem'>$numero_postagem</td>";
                $lista .= "<td nowrap  align='center' title='Código de Rastreio: $rastreio'>$rastreio</td>";
            }
            if ($login_fabrica == 50) {
