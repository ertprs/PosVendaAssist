...
<script>
function AbreDefeito(peca,data_inicial,data_final,linha,estado,pais,posto,produto,tipo_pesquisa){
	janela = window.open("relatorio_field_call_rate_defeitos_tk.php?peca=" + peca + "&data_inicial=" + data_inicial + "&consumidor_revenda=<? echo $consumidor_revenda?>&data_final=" + data_final + "&linha=" + linha + "&estado=" + estado  + "&pais=" + pais + "&posto=" + posto+ "&tipo_pesquisa=" + tipo_pesquisa + "&produto=<? echo $produto ?>","peca",'scrollbars=yes,width=750,height=450,top=315,left=0');
	janela.focus();
}
function AbreSerie(peca,data_inicial,data_final,linha,estado,pais,posto,produto,tipo_pesquisa){
	janela = window.open("relatorio_field_call_rate_serie_tk.php?peca=" + peca + "&data_inicial=" + data_inicial + "&consumidor_revenda=<? echo $consumidor_revenda?>&data_final=" + data_final + "&linha=" + linha + "&estado=" + estado + "&pais=" + pais +"&posto=" + posto+ "&tipo_pesquisa=" + tipo_pesquisa + "&produto=<? echo $produto ?>","peca",'resizable=1,scrollbars=yes,width=750,height=450,top=315,left=0');
	janela.focus();
}
function AbreSemPeca(produto,data_inicial,data_final,linha,estado,pais,posto,defeito_constatado,solucao,tipo_pesquisa){
	janela = window.open("relatorio_field_call_rate_sem_peca_tk.php?data_inicial=" + data_inicial + "&consumidor_revenda=<? echo $consumidor_revenda?>&data_final=" + data_final + "&linha=" + linha + "&estado=" + estado + "&pais=" + pais +"&posto=" + posto+ "&defeito_constatado=" + defeito_constatado + "&solucao=" + solucao + "&tipo_pesquisa=" + tipo_pesquisa + "&produto=<? echo $produto ?>","peca",'resizable=1,scrollbars=yes,width=750,height=450,top=315,left=20');
	janela.focus();
}
</script>
</HEAD>
...
if ($login_fabrica == 20 and $pais !='BR'){
	...
	$xsql="SELECT tbl_os.defeito_constatado,";
		...
		if($login_fabrica == 20 ){$xsql.=" and  tbl_posto.pais = '$pais' ";}
			$xsql.=" AND $tipo_data BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59' 
			AND $cond_3
...
//if ($ip == "201.68.13.116") { echo nl2br($xsql);}
	...
	if(pg_numrows($xres) > 0){
		...
		//	$xporcentagem = round($xporcentagem,0);
			...
			echo "<TD class='conteudo101'><a href='javascript:AbreSemPeca(\"$produto\",\"$data_inicial\",\"$data_final\",\"$linha\",\"$estado\",\"$pais\",\"$posto\",\"$defeito_constatado\",\"$solucao\",\"$tipo_pesquisa\")'>$defeito_constatado_descricao</A></TD>";
			//}
...
<?
	...
	for($i=0; $i<pg_numrows($res); $i++){
		...
		echo "<TR class='bgTRConteudo$cor'>";
		echo "	<TD class='conteudo10' align='center'><a href='javascript:AbreDefeito(\"$peca\",\"$data_inicial\",\"$data_final\",\"$linha\",\"$estado\",\"$pais\",\"$posto\",\"\",\"$tipo_pesquisa\")'>$referencia</a></TD>";
		...
		echo "	<TD class='conteudo10' align='center'><a href='javascript:AbreSerie(\"$peca\",\"$data_inicial\",\"$data_final\",\"$linha\",\"$estado\",\"$pais\",\"$posto\",\"\",\"$tipo_pesquisa\")'>#série</a></TD>";
		echo "</TR>";
	}
		echo "<TR >";
