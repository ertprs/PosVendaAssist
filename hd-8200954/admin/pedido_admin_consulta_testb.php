	...
	if ($detalhar == "ok") {
		...
		if ( ($tipo_pedido_id <> 94 and (strpos(strtoupper($condicao),"GARANTIA") !== false or $login_fabrica<>1) ) OR (strlen($pedido_cliente)>0 AND $login_fabrica==7 ) ) {
			...
			if ($login_fabrica <> 11 AND $login_fabrica <> 51 AND $login_fabrica <> 81 AND $login_fabrica <> 59 AND $login_fabrica <> 80 AND $login_fabrica <> 10) {
				...
				if($login_fabrica == 81 or $login_fabrica == 80 or $login_fabrica==10){
					$sql = "SELECT  DISTINCT
						tbl_pedido_item.pedido_item,
			...
			if (pg_num_rows($res) > 0) {
				for ($i = 0 ; $i < pg_num_rows ($res) ; $i++) {
					...
					if($i==0) {
						...
						#HD 347649 - INICIO
						if($login_fabrica <> 11 && $login_fabrica <> 24) 
							echo "<td align='center'><b>Nota Fiscal</b></td>";
					...
					if($login_fabrica <> 1 ){
						if(in_array($login_fabrica,array(3,24,35,45,50,43,5,7,14,10,81,40,30,90,80,72,91)))  {
		...
		# HD 347649 - INICIO 
		#Caso seja Suggar, exibirá Notas fiscais que atenderam este pedido", listando somente os números de nota fiscal do pedido, sem fazer relação com o número da OS
		if($login_fabrica == 24){
			
			$sqlNF = 'SELECT DISTINCT(tbl_faturamento.nota_fiscal) AS nota_fiscal,
							TO_CHAR(tbl_faturamento.emissao,\'DD/MM/YYYY\') AS emissao
					    FROM tbl_faturamento
					    JOIN tbl_faturamento_item USING (faturamento)
					   WHERE tbl_faturamento_item.pedido = '.$pedido.';';
			$resNF = pg_query($con,$sqlNF);
			$totalNotas = pg_num_rows($resNF);
			
			if($totalNotas > 0){
				
				?>
				<br />
				<table width="700" cellpadding="2" cellspacing="1" align="center" class="Tabela">
					<thead>
						<tr class="titulo_tabela">
							<th colspan="2">Notas fiscais que atenderam este pedido</th>
						</tr>
						<tr class="subtitulo">
							<th align="left">Nota Fiscal</th>
							<th align="left">Data de Emissão</th>
						</tr>
					</thead>
					<tbody>
						<?php for($i=0;$i<$totalNotas;$i++){
							$cor = ($i % 2) ? "#F7F5F0" : "#F1F4FA";

							$notaFiscal = pg_fetch_result($resNF,$i,'nota_fiscal');
							$emissaoNotaFiscal = pg_fetch_result($resNF,$i,'emissao');
							?>
							<tr bgcolor="<?php echo $cor;?>">
								<td><?php echo $notaFiscal;?></td>
								<td><?php echo $emissaoNotaFiscal;?></td>
							</tr>
						<?php } ?>
					</tbody>
				</table>
