...
class relatorioExtratificacaoDevolucao
{
    ...
    private function montaCondProdutos()
    {
        ...
        if (in_array($this->login_fabrica, array('24','50'))) {
            $tbl = '';
        } else {
            $tbl = 'tbl_produto.';
        }
    ...
    private function montaCondRevendas()
    {
        ...
        $tbl = $this->fabrica_nome . '_os_devolucao_serie.';
    ...
    private function setArrOS15M()
    {
        ...
        foreach ($arr_result as $k => $fetch) {
            ...
            $this->arr_os[$k] = array("mes" => sprintf("%02d", $mes) . '/' . $ano, "os_laudo" => array());
    ...
    private function preparaTabelaTemp()
    {
        ...
        if (in_array($this->login_fabrica, array('24'))) {
            $tbl = '';
        }
        ...
        if (!empty($this->pecas)) {
            ...
            $prepare = pg_prepare("tmp_peca_serie","select tbl_os_laudo.os_laudo,os_laudo_peca,peca into temp peca_serie_$login_admin from tbl_os_laudo_peca join tbl_os_laudo on tbl_os_laudo.os_laudo = tbl_os_laudo_peca.os_laudo 
                where tbl_os_laudo.fabrica = $1 and tbl_os_laudo_peca.peca in ($cond); " );            
        ...
        if (in_array($this->login_fabrica, array('24'))) {
            $this->temp_tbls = array(
                'os_serie' => $this->fabrica_nome . '_os_devolucao_serie',
                'numero_serie' => $this->fabrica_nome . '_devolucao_numero_serie',
            );
			if($this->login_fabrica == 24) {
				$matriz_filial = $_POST["matriz_filial"];
				$this->matriz_filial = $matriz_filial;
				if($matriz_filial == '02'){
					$cond_matriz_filial = " AND ".$this->fabrica_nome."_os_devolucao_serie.matriz IS TRUE ";
				}else{
					$cond_matriz_filial = " AND ".$this->fabrica_nome."_os_devolucao_serie.matriz IS FALSE ";
				}
			}

            if (!empty($condProdutos) or !empty($condSemPeca) or !empty($condPosto)) {
                $sql = "SELECT *  
                            INTO TEMP os_devolucao_serie_$login_admin
                        ...
                        create index os_serie_os_$login_admin on os_devolucao_serie_$login_admin(os_laudo);
                        create index os_serie_sp_$login_admin on os_devolucao_serie_$login_admin(serie,produto);
                        create index os_serie_familia_$login_admin on os_devolucao_serie_$login_admin(familia);

                ";

                $qry = pg_query($sql);
                $this->temp_tbls['os_serie'] = 'os_devolucao_serie_' . $login_admin;
            }
        } 
        return $return;
    ...
    private function prepareStatements()
    {
		...
		if (empty($this->pecas)) {
            ...
            if ($this->login_fabrica == 24) {
                ...
                $prepare = pg_prepare(
                    "oss",
                    "select distinct(os_laudo) as os_laudo,data_nf, data_recebimento
                ...
                $prepare = pg_prepare("total_os", "select count(distinct(os_laudo)) as total from {$this->temp_tbls['os_serie']} join {$this->temp_tbls['numero_serie']} USING(serie,produto) $join_regiao where data_fabricacao between $1 and $2 and data_recebimento between $3 and $4 $cond_familia_os $cond_regiao $cond_revendas");
            }
            ...
            if ($this->login_fabrica == 24) {
                ...
                $prepare = pg_prepare(
                    ...
                    into temp temp_oss from {$this->temp_tbls['os_serie']}
                    join {$this->temp_tbls['numero_serie']} using(serie,produto)
                    JOIN peca_serie_$login_admin USING(os_laudo)
                    $joinNSFornecedor
                    where data_fabricacao between $1 and $2
                    and data_nf >= data_fabricacao
                    $cond_conversor
    ...
    private function populacaoPopOS($m, $curr)
    {
        ...
        if ($m == 1) {
            $this->pop_tmp[$m] = $this->arr_os_15M[$curr - 1]["os_laudo"][0];
        } else {
            /**
             *
            ...
            for ($i = $s; $i >= 0; $i--) {
                ...
                for ($j = 0; $j <= $ctl; $j++) {
                    $this->pop_tmp[$m]+= $this->arr_os_15M[$i]["os_laudo"][$j];
                }
        ...
        if ($this->login_fabrica == 24) {
            ...
            for ($i = 1; $i <= $this->meses; $i++) {
                ...
                if ($i == 1) {
                    ...
                    $sqlTotal = "SELECT count(os_laudo) as total, case fn_qtos_meses_entre(data_nf, data_recebimento) when 0 THEN 1 else fn_qtos_meses_entre(data_nf, data_recebimento) end as meses into temp temp_total_os from temp_oss group by meses order by meses";
                ...
                array_push($this->arr_os[$idx]["os_laudo"], $total_os);
...
//         echo "<br>";
    ...
    private function montaResultado()
    {
        ...
        foreach ($this->arr_os as $key => $value) {
            ...
            $sum = array_sum($value['os_laudo']);
            $count = count($value['os_laudo']);
            $producao[$value['mes']] = $tot;
            $oss[$value['mes']] = $sum;
        ...
        $count = count($this->arr_os[0]['os_laudo']);

        for ($i = 0; $i < $count; $i++) {
            foreach ($this->arr_os as $key => $value) {
                $arr_tmp[$value['mes']] = $value['os_laudo'][$i];
            }
            $arr_single[] = $arr_tmp;
        }
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">Laudo OS\'s</td>';
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">Taxa Falha - Laudo OS\'s (%)</td>';
...
/*
        if ($this->login_fabrica == 24) {
            foreach (array(1,4,6,15) as $mes) {
                $os = 'os_' . $mes . 'M';
                $this->result_view.= '<tr>';
                    $this->result_view.= '<td class="titulo_coluna">Laudo OS\'s - ' . $mes . 'M</td>';
        ...
        $this->result_view.= '<tr>';
            ...
            $this->result_view.= '<td colspan="25">rrrr&nbsp;</td>';
        $this->result_view.= '</tr>';
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">Taxa Falha - Laudo OS\'s (%) - Atual</td>';
            foreach ($this->arr_falha as $falha) {
                $this->result_view.= '<td>' . str_replace('.', ',', $falha) . '%</td>';
            }
        $this->result_view.= '</tr>';
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">Laudo OS\'s - Atual</td>';
            foreach ($oss as $key => $value) {
                $this->result_view.= '<td>' . $value . '</td>';
            }
        $this->result_view.= '</tr>';
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">Taxa Falha - Laudo OS\'s (%) - Anterior</td>';
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">Laudo OS\'s - Anterior</td>';
            foreach ($this->arr_os_anterior as $key => $value) {
                $this->result_view.= '<td>' . $value . '</td>';
        ...
        $this->result_view.= '<tr>';
            $this->result_view.= '<td class="titulo_coluna">DiferenÁa Laudo OS\'s</td>';
