...
//echo $sql;
	...
	$sql = "UPDATE x_$login_fabrica SET nome_comercial = 'MOBILE' WHERE nome_comercial = 'MÓBILE'";
	$resX = pg_exec ($con,$sql);
	$sql = "UPDATE x_$login_fabrica SET nome_comercial = 'MS3 112' WHERE nome_comercial = 'MS3-112'";
	$resX = pg_exec ($con,$sql);
...
//echo $sql; exit;
		...
		for ($x = 0 ; $x < pg_numrows ($resProd) ; $x++) {
			$nome_comercial_if  = RemoveAcentos($nome_comercial);
			$nome_comercial_if2 = RemoveAcentos(pg_result ($resProd,$x,nome_comercial));

			if ($nome_comercial_if < $nome_comercial_if2 ) {
//				echo $nome_comercial_if;
//				echo " < ";
//				echo $nome_comercial_if2;
				break;
			}
