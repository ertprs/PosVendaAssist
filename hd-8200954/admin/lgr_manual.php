		...
		if (strlen($xxfaturamento_item) > 0 AND strlen($msg_erro) == 0){
			...
			if (pg_numrows($res_item)>0){
				$sql = "UPDATE tbl_faturamento_item
						SET
							devolucao_obrigatoria_lgr = $upd_devolucao_obrigatoria_lgr 
						WHERE faturamento_item= $xxfaturamento_item";
				$res_item = pg_exec($con,$sql);
			}
...
<TR>
	...
	<TD>Nota Fiscal<br><input type="text" name="nota_fiscal" id="nota_fiscal" size="20" maxlength="20" value="<? echo $nota_fiscal ?>" class="frm"></TD>
	<TD></TD>
</TR>
<TR>
...
if (strlen($btn_acao)  > 0 AND strlen($msg_erro)==0) {
	...
	$sql =  "
			...
			ORDER BY tbl_faturamento.emissao, tbl_faturamento.nota_fiscal, tbl_peca.descricao;";
	...
	if(pg_numrows($res)>0){
		...
		for ($x=0; $x<pg_numrows($res);$x++){
			...
			echo "<td align='center' width='0'>";
				...
				if ($devolucao_obrigatoria_lgr=='t'){
					echo " CHECKED ";
				}
				echo ">";
