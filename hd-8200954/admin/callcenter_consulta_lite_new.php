...
if($login_fabrica==25){
	header ("Location: callcenter_consulta_lite_interativo.php");
...
if(strlen($msg_erro) == 0){
	...
	if(strlen($chk9) > 0){
		...
		if ($nome_consumidor){
			...
			$cond9 = "  tbl_hd_chamado_extra.nome LIKE '".$nome_consumidor."%' ";
		}
	}
	...
	if(strlen($chk13) > 0){
		// numero_os
		if ($numero_os){
			#desabilitei para analise posterior, nao tem join e da add-missing - Fabio -  21/02/2008
	...
	$sql = "SELECT tbl_hd_chamado.hd_chamado,
					...
					to_char(tbl_hd_chamado.data,'HH24:MI') AS horario,
...
//	echo $sql;
	...
	if (@pg_numrows($res) == 0) {
		...
		$data = date('Ymd');
		$arquivo_nome     = "callcenter_consulta_lite_new_xls-$login_fabrica-$data.xls";
		$arquivo ="/var/www/assist/www/admin/xls/callcenter_consulta_lite_new_xls-$login_fabrica-$data.xls";
		$fp = fopen($arquivo, "w");

		fputs($fp, "<TABLE width='600' align='center' border='0' cellspacing='1' cellpadding='1'>");

		fputs($fp, "<TR class='menu_top'>");
		fputs($fp, "<TD colspan=7 align='center' bgcolor='#D7D7D7'><strong>Relação de Atendimentos Lançados</strong></TD>");
		fputs($fp, "</TR>");
		fputs($fp, "<TR class='menu_top'>");
		fputs($fp, "<TD bgcolor='#0099FF'>DATA</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>HORÁRIO</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>PRODUTO</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>Nº CHAMADO</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>CLIENTE</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>OS</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>ASSUNTO</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>STATUS</TD>");
		fputs($fp, "<TD bgcolor='#0099FF'>ATENDENTE</TD>");
		fputs($fp, "</TR>");
		...
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++){
			...
			$horario            = trim(pg_result ($res,$i,horario));
			$assunto            = trim(pg_result ($res,$i,titulo));
			...
			if($login_fabrica == 6) {
				echo "<TD align=center nowrap>$horario</TD>\n";
			}
			...
			echo "$status";
			echo "</TD>\n";
			echo "<TD align='center'><a href='cadastra_callcenter.php?callcenter=$callcenter' target='blank'><img src='imagens/btn_consultar_".$btn.".gif'></a></TD>\n";
			echo "</TR>\n";
		}
		if(pg_numrows($res2) > 0) {

			for ($i = 0 ; $i < pg_numrows ($res2) ; $i++){
				$callcenter         = trim(pg_result ($res2,$i,hd_chamado));
				$data               = trim(pg_result ($res2,$i,data));
				$sua_os             = trim(pg_result ($res2,$i,sua_os));
				$os                 = trim(pg_result ($res2,$i,os));
				$serie              = trim(pg_result ($res2,$i,serie));
				$consumidor_nome    = trim(pg_result ($res2,$i,cliente_nome));
				$posto_nome         = trim(pg_result ($res2,$i,codigo_posto));
				$produto_nome       = trim(pg_result ($res2,$i,produto_descricao));
				$produto_referencia = trim(pg_result ($res2,$i,produto_referencia));
				$status             = trim(pg_result ($res2,$i,status));
				$admin              = trim(pg_result ($res2,$i,admin));
				$horario            = trim(pg_result ($res2,$i,horario));
				$assunto            = trim(pg_result ($res2,$i,titulo));

				$sql_atendente = "SELECT login
									FROM tbl_admin
									WHERE admin = $admin
									LIMIT 1";

				$res3 = pg_exec($con,$sql_atendente);
				$login = pg_result ($res3,0,login);
				...
				fputs($fp, "<TD align=center nowrap>$data</TD>");
				fputs($fp, "<TD align=center nowrap>$horario</TD>");
				fputs($fp, "<TD nowrap>$produto_nome</TD>");
				fputs($fp, "<TD align=center nowrap>$callcenter</TD>");
				fputs($fp, "<TD nowrap>$consumidor_nome</TD>");
				fputs($fp, "<TD nowrap>$sua_os</TD>");
				fputs($fp, "<TD nowrap>$assunto</TD>");
				fputs($fp, "<TD align=center nowrap>$status</TD>");
				fputs($fp, "<TD align='center' nowrap>$login</TD>");
				fputs($fp, "</TR>");
			}
		}
		echo "</TABLE>\n";
