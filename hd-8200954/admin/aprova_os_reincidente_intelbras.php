				...
				if ($status_da_os == 67 or $status_da_os == 68 or $status_da_os == 70 ){
					...
					if($select_acao == "00"){
						...
						if($status_os == "13"){
							$sql = "INSERT INTO tbl_os_status
									(os,status_os,data,observacao,admin)
									VALUES ($xxos,131,current_timestamp,'$motivo - Os excluída',$login_admin)";
							$res = pg_exec($con,$sql);
							$msg_erro .= pg_errormessage($con);
...
$title = "Auditoria de OSs reincidentes";
...
<caption>Auditoria de OS reincidente</caption>
	...
	<TD colspan=4>Status<br>
		<select class='frm' name='status_os'>
			...
			<?php 
				...
				$sql = "select *from tbl_status_os where status_os in(67, 68,70,131,19)";
				$res = pg_exec($con,$sql);
	...
	if(pg_numrows($res)>0){
		...
		for ($x=0; $x<pg_numrows($res);$x++){
			...
			echo "<td align='center' width='0'>";
				if($status_os==67 or $status_os==68 or $status_os ==70){
			...
			echo "<td style='font-size: 9px; font-family: verdana' title='$sua_os - Motivo: ".$obs_reincidencia."'>".substr($obs_reincidencia,0,50). "</td>";	
