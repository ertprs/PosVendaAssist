...
$status_reinc =($login_fabrica == 30) ? " 132,19" : (($login_fabrica ==24) ? "67,70,19" : "");
...
if($btn_acao == 'Pesquisar'){
	...
	if(strlen($aprova) == 0){
		$aprova = "aprovacao";
		$aprovacao = ($login_fabrica == 30) ? "132" : (($login_fabrica == 24) ? "67,70" : "");
...
if (strlen($btn_acao)  > 0 AND strlen($msg_erro)==0) {
	...
	$sql =  "SELECT interv.os
			INTO TEMP aprova_reinc_nf_$login_admin
			FROM (
			SELECT 
			ultima.os, 
			(SELECT status_os FROM tbl_os_status WHERE status_os IN ($status_reinc) AND tbl_os_status.os = ultima.os and fabrica_status = $login_fabrica ORDER BY data DESC LIMIT 1) AS ultimo_status
			FROM (SELECT DISTINCT os FROM tbl_os_status WHERE status_os IN ($status_reinc) and fabrica_status = $login_fabrica ) ultima
			) interv
			WHERE interv.ultimo_status IN ($aprovacao)
			$Xos
			;
			
			CREATE INDEX aprova_reinc_nf_OS_$login_admin ON aprova_reinc_nf_$login_admin(os);
			...
			SELECT	tbl_os.os                                                   ,
					...
					(SELECT status_os FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN ($status_reinc) ORDER BY data DESC LIMIT 1) AS status_os         ,
					(SELECT observacao FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND status_os IN ($status_reinc) ORDER BY data DESC LIMIT 1) AS status_observacao,
					(SELECT tbl_status_os.descricao FROM tbl_os_status JOIN tbl_status_os USING(status_os) WHERE tbl_os.os = tbl_os_status.os AND status_os IN ($status_reinc) ORDER BY data DESC LIMIT 1) AS status_descricao
				FROM aprova_reinc_nf_$login_admin X
