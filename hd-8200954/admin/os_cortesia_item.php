...
if ($btn_acao == "gravar") {
	...
	if(empty($os_produto)) {
		$sql = "INSERT INTO tbl_os_produto (os,produto) select os, produto FROM tbl_os WHERE os = $os RETURNING os_produto;";
		$res = pg_query($con, $sql);
	...
	if (strlen($troca_faturada) == 0) {
		...
		if (strlen($msg_erro) == 0) {
			...
			if (strlen ($msg_erro) == 0 and strlen($aux_data_conserto) == 10) {
				$sql = "UPDATE tbl_os SET data_conserto = '$aux_data_conserto' WHERE os = $os";
				$res = @pg_query($con,$sql);
				$msg_erro .= pg_errormessage($con);
			}
	...
	<?php if($login_fabrica == 1){ ?>
		<tr>
			<td style='text-align: left'>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">Data de Conserto</font><br />
				<input type='text' name='data_conserto' class='frm date' value='<?php echo $data_conserto;?>' size='12' /> 
			</td>
		</tr>
	<?php } ?>
</table>
...
echo "<table width='600' border='0' align='center'  cellpadding='1' cellspacing='3 class='border'>";
		echo "<tr>";
