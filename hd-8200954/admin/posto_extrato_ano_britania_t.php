...
//				echo "$data_geracao<>".$mes[$x];
	...
	*/
	...
	foreach($databarx as $data_extrato){
		$td_data .="|".$data_extrato;
	}
	
	$maior = 0;
	foreach($data1y as $i => $valor_posto){
		$maior= ($valor_posto > $maior) ? $valor_posto:$maior;
		$total_posto .=($i == 11)?$valor_posto:$valor_posto.",";
	}
	$maior = round($maior,-2);
	$maior += 200;
	$menor = $maior / 10;
	...
	foreach($data1y as $i => $valor_media){
		$total_media .=($i == 11)?$valor_media:$valor_media.",";
	}

		echo "<p>";
		echo "<iframe src='http://chart.apis.google.com/chart?chs=700x400&chd=t:0|$total_posto|1|$total_media&cht=lxy&chdl=Posto|Media&chco=FF0000,00FF00&chxt=x,y&chxl=0:$td_data|1:|0$total_y&chds=0,$maior&chg=9,5' align='center' height='450' width='750'></iframe></p>";
