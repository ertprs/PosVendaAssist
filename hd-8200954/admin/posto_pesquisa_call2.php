...
if ($tipo == "nome") {
	...
	$sql = "SELECT   tbl_posto.*, tbl_posto_fabrica.codigo_posto, tbl_posto_fabrica.credenciamento
			FROM     tbl_posto
...
if (pg_numrows($res) == 1) {
	...
	if ( ($credenciamento <> 'DESCREDENCIADO' AND $login_fabrica == 3 AND $os == 't') OR $login_fabrica <> 3 OR $os <> 't') {
		...
		if ($login_fabrica == 3){
			echo "fone.value = '".trim(pg_result($res,0,fone))."';";
		}
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		if (($credenciamento <> 'DESCREDENCIADO' AND $login_fabrica == 3 AND $os == "t") OR $login_fabrica <> 3  OR $os <> "t") {
			...
			if ($login_fabrica == 3){
				echo " fone.value = '$fone';";
			}
			if ($_GET["proximo"] == "t") echo "proximo.focus(); ";
			echo "this.close() ; \" >";
		}

		echo "<font face='Arial, Verdana, Times, Sans' size='-1' color='#0000FF'>$nome";
