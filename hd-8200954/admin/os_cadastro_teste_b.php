...
if ($btn_troca == "trocar") {
	...
	if (isset($_POST["modalidade_transporte"]) && strlen($_POST["modalidade_transporte"]) == 0) {
		$msg_erro .= "Selecione a Modalidade do Transportebr>";
	}
	else {
...
if ($btn_troca == "trocar" && strlen($msg_erro) == 0) {
	...
	$res = pg_query ($con,$sql);
	if(pg_num_rows($res)>0 && $login_fabrica != 81){
...
// adicionado por Fabio - Altera o status para liberado da Assis. Tec. da Fábrica caso tenha intervencao.
	...
	if($login_fabrica == 81 && $troca_garantia_produto == -2) {
			/* verificar se foi trocado ou feito pedido, e nao deixar fazer ressarcimento
			se tiver tbl_os_troca para a OS é pq foi feito troca, tbl_os_troca.pedido não for nulo é pq já gerou pedido e nao pode mais trocar, só ressarcir */ 
			$sql_verifica = '
				SELECT tbl_os_troca.os_troca
				FROM tbl_os
				JOIN tbl_os_troca USING (os)
				WHERE tbl_os_troca.pedido IS NOT NULL 
				AND tbl_os.os = ' . $os . '
				AND tbl_os.fabrica = ' . $login_fabrica . ';
			'; 
			$res_verifica = pg_query($con,$sql_verifica);
			if(pg_numrows($res_verifica) > 0){
				echo '<p class="texto_avulso">ATENÇÃO: Troca não permitida, produto já foi trocado</p>';
			}
	}
	...
	if ($troca_garantia_produto == "-1") {//resarcimento financeiro
		...
		if ($login_fabrica == 81) {
			...
			$sql_troca = "SELECT ressarcimento
					FROM tbl_os_troca
					WHERE os = $os
					AND ressarcimento IS NOT NULL";
			$res_troca = pg_query($con, $sql_troca);

			if (pg_numrows($res_troca) > 0) {
				$ress = pg_result($res_troca, 0,0);
				if($ress == 't')
					$msg_erro = "Já foi feito um ressarcimento desta OS.";
			}
			...
			if(strlen($msg_erro)==0) {
				...
				"; // bloqueia ressarcimento em OS embarcada e nao faturada.. OS embarcada e faturada pode ressarcir
				$res_des = pg_query($con,$sql);
				//die(nl2br($sql));
				if(pg_num_rows($res_des) > 0)
					$msg_erro = 'Essa OS já foi embarcada.';
			}
			
			if(strlen($msg_erro) == 0) {
				// verifica se o pedido está pendente e nao estiver faturado
				$sql = "
					SELECT 
					distinct(tbl_pedido_item.pedido_item), tbl_pedido_item.peca, tbl_pedido_item.pedido
					FROM tbl_os
					JOIN tbl_os_produto USING(os)
					JOIN tbl_os_item USING(os_produto)
					JOIN tbl_pedido_item ON tbl_pedido_item.pedido = tbl_os_item.pedido
				...
				$res_pen = pg_query($con,$sql);
				if(pg_numrows($res_pen) > 0) { //cancela pedido antes de ressarcir
					for($i = 0; $i < pg_numrows($res_pen); $i++) {

						$pedido = pg_result($res_pen,$i, 'pedido');
						$peca	= pg_result($res_pen,$i, 'peca');
						
						$sqlFn = "
							SELECT fn_pedido_cancela(null,$login_fabrica,$pedido,$peca,'Ressarcimento de OS - Produto Pendente');
						";
						$res_cancela = pg_exec ($con,$sqlFn);

					}
					
					$assunto = "Cancelamento de pedido n. $pedido";
					$message = "Esta é uma mensagem automática, não responda este E-mail\n <br />
								Prezado,  \n <br />
								O pedido de peça $pedido, referente à OS $os foi cancelado, devido ao ressarcimento financeiro efetuado.\n <br />
								--- <br />
								Suporte Telecontrol
								";
					...
					$headers .= "Cc: gabriel.rolon@telecontrol.com.br";

					//mail("brayan@telecontrol.com.br",$assunto,$message,$headers);
					// todo enviar email para o ronaldo avisando, pois está cancelando o envio.
				}
			...
			if (strlen($agencia)==0) {
				$msg_erro .= "Para efetuar o ressarcimento digite a agência titular<br>";
			}
...
//		HD 79774 - Paulo César 10/03/2009 sempre gera pedido para fabrica 3
	...
	elseif ($troca_garantia_produto == -2 && strlen($msg_erro) == 0) {

		// todo verificar troca
				...
				if($login_fabrica == 51 or $login_fabrica == 81){#HD52537 alterado apenas para a Gama pois não sei se as outras fábrica atualiza o pedido_item
					...
					if (@pg_num_rows($res_dis) > 0) {
						for($x=0;$x<@pg_num_rows($res_dis);$x++){
							...
							$sql2 = "SELECT fn_pedido_cancela_garantia($distrib,$login_fabrica,$pedido_pedido,$pedido_peca,$pedido_os_item,'Troca de Produto'); ";
		...
		if (strlen ($msg_erro) == 0) {
			if($login_fabrica<>3 and $login_fabrica <> 35 AND $login_fabrica <> 11 AND $login_fabrica<>81){
	...
	if (strlen($msg_erro) == 0) {
		...
		header("Location: $PHP_SELF?os=$os&ok=s&osacao=trocar");
		exit;
	}else{
		$res = pg_query($con,"ROLLBACK TRANSACTION");
	}
...
if ($btn_acao == "continuar") {
	...
	if (strlen($msg_erro) == 0){
		...
		if ($login_fabrica == 1 || $login_fabrica == 2) {
	...
	if($login_fabrica == 7 and strlen($consumidor_cpf) > 0){ // HD 46309
		...
		if(strlen($cpf_erro) > 0){
			$msg_erro .= "CPF do consumidor inválido <br>";
		}
	}
...
#	if ($login_fabrica == 1 AND strlen($consumidor_cpf) == 0) $msg_erro .= " Tamanho do CPF/CNPJ do cliente inv?lido.";
	...
	if (($login_fabrica == 35 OR $login_fabrica==28 OR $login_fabrica==50) AND $pedir_defeito_reclamado_descricao == 't' AND ($defeito_reclamado_descricao == 'null' OR strlen($defeito_reclamado_descricao) == 0)){
	...
	*/
	...
	if (strlen ($msg_erro) == 0 ) {
		...
		if (strlen($os) == 0) {
			...
			if($login_fabrica <> 14 and $login_fabrica <> 52) {
...
<table border="0"  class="formulario" cellpadding="0" cellspacing="0" align="center" width="700" style="display: <? echo $display_frm_os; ?>">
	...
	<tr>
		...
		<tr valign="top">
			...
			<td nowrap>
				...
				Número de Série
				<br>
				<input class="frm" type="text" name="produto_serie" id="produto_serie" size="15" maxlength="20" value="<? echo $produto_serie ?>" onChange="javascript: this.value=this.value.toUpperCase();">
				<img src='imagens/lupa.png' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_numero_serie (document.frm_os.produto_serie, "produto_serie")' style='cursor: pointer'>
			</td>
			<? }
				else{ ?>
				<td>&nbsp;</td>
		...
		<? } ?>
			...
			?>
				...
				/*HD: 110888 - LIBERADO PARA SUGGAR*/
				 $aExibirDefeitoReclamado = array(3,7,28,35,19,50,30,59,24);
				 $aExibirDefeitoReclamado = array_flip($aExibirDefeitoReclamado);
				 ...
				 if ( isset($aExibirDefeitoReclamado[$login_fabrica]) ) {
				?>
			...
			<tr valign='top'>
				...
				if($pedir_defeito_reclamado_descricao == 't'){
					...
					echo "<select name='defeito_reclamado' class='frm' id='defeito_reclamado' style='width: 220px;' onfocus='listaDefeitos(document.frm_os.produto_referencia.value);' $defeito_reclamado_onchange >";
		...
		<tr>
			...
			<td width="170">
				...
				<SELECT NAME="fisica_juridica" class="frm">
					<OPTION value="<?php if(strlen($xfisica_juridica)>0) echo $xfisica_juridica; ?>">
						<?php
							if($xfisica_juridica=="F")
								echo "Pessoa Física";
							elseif($xfisica_juridica=="J")
								echo "Pessoa Jurídica";
						?>
					</OPTION>
					<OPTION VALUE="F" <?php if($xfisica_juridica=="F") echo "SELECTED"; ?>>Pessoa Física</OPTION>
					<OPTION VALUE="J" <?php if($xfisica_juridica=="J") echo "SELECTED"; ?>>Pessoa Jurídica</OPTION>
				</SELECT>
			</td>
			<?}?>
			<? if($login_fabrica <> 1){?>
...
<tr class="top">
	<td width="300"><font size="1" face="Geneva, Arial, Helvetica, san-serif">Endereço</font></td>

	<td width="170"><font size="1" face="Geneva, Arial, Helvetica, san-serif">Número</font></td>

	<td><font size="1" face="Geneva, Arial, Helvetica, san-serif">Complemento</font></td>
...
<tr>
	...
	<td width="69">
		<input class="frm" type="text" name="consumidor_numero" id="consumidor_numero"  size="10" maxlength="20" value="<? echo $consumidor_numero ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Digite o número do endere?o do consumidor.');">
	</td>

	<td>
		<input class="frm" type="text" name="consumidor_complemento"  id="consumidor_complemento" size="15" maxlength="30" value="<? echo $consumidor_complemento ?>" <? if($login_fabrica==50){?>onChange="javascript: this.value=this.value.toUpperCase();"<?}?> onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on'; displayText('&nbsp;Digite o complemento do endere?o do consumidor.');">
	</td>
...
<tr>
	...
	<td>
		...
		<select name="consumidor_estado" class='frm'>
...
<tr class="top">
	<td><font size="1" face="Geneva, Arial, Helvetica, san-serif">E-mail</font>
		<BR>
		<input class="frm" type="text" name="consumidor_email"  size="30" maxlength="50" value="<? echo $consumidor_email ?>" <? if($login_fabrica==50){?>onChange="javascript: this.value=this.value.toUpperCase();"<?}?> onblur="this.className='frm'; displayText('&nbsp;');" onfocus="this.className='frm-on';">
	</td>
<? if ($login_fabrica==7) { ?>
...
?>
		...
		<tr>
			<td>
				...
				?>
				<input class="frm" type="text" name="nota_fiscal" id="nota_fiscal" size="8"  maxlength="<? echo $maxlength; ?>" id="nota_fiscal" value="<? echo $nota_fiscal ?>"
				<?php
				if($login_fabrica==45){?>
...
<? } ?>
		...
		<tr>
			...
			<font size="1" face="Geneva, Arial, Helvetica, san-serif">Revenda</font>&nbsp;<input type="radio" name="consumidor_revenda" value='R' <? if ($consumidor_revenda == 'R') echo " checked"; ?>>&nbsp;&nbsp;</td>
			<? } ?>
		</tr>
		...
		<? }else{
			...
			<? } else {?>
				...
				<br>
				<textarea name='orientacao_sac' rows='4' cols='100' class="frm"><? echo trim($orientacao_sac); ?></textarea>
			<? }?>
		<? } ?>
		<br />
		</center>
...
?>
<tr><td>
	<table width='650' align='center' border='0' cellspacing='0' style='border:solid 1px;' class='formulario'>
	<tr >
...
//		$res_verifica = pg_query($con,$sql);
	...
	//display:none quando ele não deve estar disponível
	echo '<table style="display:' . $display_frm_troca . '" align="center" width="700"><tr><td>';

	if(pg_num_rows($res)>0) {
		echo "<table width='700' align='center' border='0' cellspacing='0' >";
		echo "<tr class='texto_avulso'>";
		echo "<td align='center' > ";
		echo "Produto já trocado!<br> Preencha o formulário abaixo para trocar o produto novamente!";
		echo "</td>";
		echo "</tr>";
		echo "</table> <br/>";
	}
	...
	AND tbl_os.fabrica=$login_fabrica
	";
	$res_produto = pg_query($con,$sql);

	if (($troca_garantia == 't' AND (!isset($troca_efetuada))OR $ok=='s') AND $login_fabrica !=81 ) {
		...
		if( $login_fabrica <> 3 or ($login_fabrica == 3 AND $permissao=='t') ){
			...
			if($login_fabrica==81){
				...
				if (pg_num_rows($resressarcimento)>0) {
					...
					$ressarcimento_troca = pg_result($resressarcimento,0,1);
					if ($ressarcimento_troca == 't') {
						echo "<br><p class='texto_avulso'>Foi Aberto o Chamado $hd_chamado_troca solicitando o ressarcimento deste produto, para efetivar o ressarcimento complete os campos abaixo</p>";
					}
				}
			}
...
?>
	...
	<tr class='subtitulo'><td colspan='100%'>Detalhes do Produto</td></tr>
	
	<tr>
		<td width='210' class='Conteudo' style='padding-left:10px;'>
			<b>Produto da OS</b>
		</td>
		<td>
			<?
			echo  "[".pg_fetch_result($res_produto, 0, referencia) . "]" . pg_fetch_result($res_produto, 0, descricao); 
		?>
		</td>
	</tr>
	
	<tr>
		<td class='Conteudo'  style='padding-left:10px;'>
			<b>Posto</b>
		</td>
		<td>
			<?
	...
	<td colspan='2'>
<?
		if (in_array($login_fabrica,array(3,45,35,11,25,51,14,66,24,19,81,40,80,72)) or $login_fabrica > 80) {
			
				//HD 194731: Informações do produto e posto
		
			if ($login_fabrica == 3 or $login_fabrica == 81) {
				echo "<table border='0' cellspacing='0' width='680' align='center' border='0' class='formulario'>";
				...
				if (pg_num_rows($res) > 0) {
					echo "<select class='frm' style='width: 400px;' name='marca_troca' id='marca_troca' onChange='buscaFamilia(this.value);'>\n";
				...
				echo "<table border='0' cellspacing='0' width='700'>";
				echo "<tr class='Conteudo'>";
				#echo "<td align='left'><b>Marca do Produto</td>";
				echo "<td align='left'>";
			...
			if (pg_num_rows($res) > 0) {
				echo "<select class='frm' style='width: 400px;' name='familia_troca' id='familia_troca' onChange='listaProduto(this.value,\"$aux_marca\");valida_campo_ant();'>\n";
				echo "<option value=''>ESCOLHA</option>\n";
			...
			echo "<tr class='Conteudo'>";
			echo "<td align='left' nowrap><b>Trocar pelo produto/Ressarc.:</td>";
			echo "<td align='left'>";
			echo "<select name='troca_garantia_produto'  class='frm' style='width: 400px;'"; if ($login_fabrica == '81') { echo "onchange='javascript: if (this.value == \"-1\") {document.getElementById(\"dados_ressarcimento\").style.display = \"block\"} else {document.getElementById(\"dados_ressarcimento\").style.display = \"none\"}'";} echo ">";
			...
			if (strlen($familia_troca)) {
				...
				for($p = 0; $p < pg_num_rows($res); $p++) {
					...
					if($login_fabrica == 81 && $aux_produto == '-1'){
			...
			echo "</tr>";
			echo "<tr class='Conteudo'>";
			echo "<td align='left' nowrap><b>Causa da Troca/Ressarc.</td>";
			echo "<td align='left'>";
			...
			echo "<select name='causa_troca' size='1' class='frm' style='width: 400px;'>";
			echo "<option value='' ></option>";
			...
			echo "<td align='left'><input type='text' name='ri' value='" . $_POST[ri] . "' size='10' maxlength='10' class='frm'>";
			echo "</td>";
			echo "</tr>";
			echo "<tr class='conteudo'><td colspan='2'>";
			echo "Observação para nota fiscal<br><textarea name='observacao_pedido' cols='100' rows='3' class='frm'>".  $_POST["observacao_pedido"] . "</textarea>";
			echo "</td></tr>";
			echo "</table>";
			...
			if(pg_num_rows($resTroca)>0) {
				echo "<br><table border='0' cellspacing='1' width='680' align='center' class='tabela'>";
				echo "<tr class='subtitulo'><td colspan='100%'>Detalhes das Peças</td></tr>";
			...
			echo "<table border='0' cellspacing='0' width='680' align='center'>";
			echo "<tr class='subtitulo'><td colspan='100%'>Detalhes das Troca</td></tr>";
			echo '<tr >
				<td colspan="100%">
					<b>Responsável pela troca:</b> '.$login_login.' | OS <a href="os_press.php?os='.$os.'" target="_blank" title="Cliique para ver a OS em outra janela" style="text-decoration:underline; "><b>'.$sua_os.'</b></a>
				</td>
			</tr>';
			echo "<tr><td>&nbsp;</td></tr>";
			echo "<tr class='Conteudo'>";
			echo "<td align='left'>";
			echo "<fieldset>";
			echo "<legend>Setor Responsável</legend>";
			switch($_POST["setor"]) {
				case "Revenda":
			...
			echo "<table>
						<tr>
							<td>
								<input type='radio' name='setor' $revenda_checked value='Revenda'> Revenda
							</td>
							<td>
								<input type='radio' name='setor' $carteira_checked value='Carteira'> Carteira
							</td>
						</tr>
						<tr>
							<td>
								<input type='radio' name='setor' $sac_checked value='SAC'> SAC 
							</td>
							<td>
								<input type='radio' name='setor' $procon_checked value='Procon'> Procon
							</td>
						</tr>
						<tr>
							<td colspan='2'>
								<input type='radio' name='setor' $sap_checked value='SAP'> SAP
							</td>
						</tr>
					</table>";
			echo "</fieldset>";
			...
			echo "<fieldset>";
			if($login_fabrica <> 51 and $login_fabrica <> 81){
				echo "<legend><b>Situação do Atendimento</legend>";
			}else{
			...
			if($login_fabrica <> 51 and $login_fabrica <> 81){
				...
				echo "<input type='radio' name='situacao_atendimento' $checked_0 value='0'> Produto em garantia<br>";
				echo "<input type='radio' name='situacao_atendimento' $checked_50 value='50'> Faturado 50%<br>";
				echo "<input type='radio' name='situacao_atendimento' $checked_100 value='100'> Faturado 100%<br>";
			}else{
				...
				echo "<input type='radio' name='fabrica_distribuidor' $fabrica_checked value='fabrica'> Fábrica<br>";
				echo "<input type='radio' name='fabrica_distribuidor' $distribuidor_checked value='distribuidor'> Distribuidor<br>";
			}
			...
			if ($login_fabrica <> 14) {
				if($login_fabrica==3 and 1==2){
					echo "<hr width='95%'><input type='checkbox' name='gerar_pedido' id='gerar_pedido' value='t' checked='checked' disabled> Gerar pedido";
				}else{
					if ($_POST["gerar_pedido"]) {
						$checked = "checked";
					}
					echo "<hr width='95%'><input type='checkbox' $checked name='gerar_pedido' id='gerar_pedido' value='t'> Gerar pedido";
				}
			}
			echo "</fieldset>";
			echo "</td>";
			...
			if($login_fabrica==3 or $login_fabrica==81) {
				...
				echo "<td align='left' valign='top'>";
				echo "<fieldset>";
				echo "<legend>Destino</legend>";
				echo "<input type='radio' name='envio_consumidor' $t_checked value='t'> direto ao consumidor<br>";
				echo "<input type='radio' name='envio_consumidor' $f_checked value='f'> para o posto<br>";
				echo "</fieldset>";
				echo "</td>";
				...
				echo "<td align='left'>";
				echo "<fieldset>";
				echo "<legend>Modalidade de Transporte</legend>";
				echo "<input type='radio' name='modalidade_transporte' $urgente_checked value='urgente'";
				echo "> RI Urgente<br>";
				echo "<input type='radio' name='modalidade_transporte' $normal_checked value='normal' ";
				echo "> RI Normal<br>";
				echo "</fieldset>";
				echo "</td>";
				echo "</tr>";
			...
			if ($login_fabrica == 81) {
				...
				echo "<div id='dados_ressarcimento' style='display:$display';>
						<table border='0' width='680' align='center'>
							<tr><td>&nbsp;</td></tr>
							<tr class='subtitulo'>
								<td colspan='100%'>Informações Bancárias para o Ressarcimento</td>
							</tr>
							<tr>
							...
							<tr>
								<td><input type='text' name='cpf_ressarcimento' id='cpf_ressarcimento' class='frm' value='$cpf_ressarcimento' ></td>
								<td><input type='text' name='favorecido_conta' class='frm' value='$favorecido_conta' size='45'></td>
							</tr>
							<tr>
							...
							<tr>";
								...
								echo "<td colspan='2'>
											<select name='banco' id='banco' class='frm' style='width:450px;'>
							...
							<tr>
								<td width='600'>Agência</td>
								<td style='padding-left:70px;' >Conta Corrente</td>
								<td>Valor (R$)</td>
							</tr>
							
							<tr>
								<td><input type='30' name='agencia' id='agencia' class='frm' value='$agencia'></td>
								<td style='padding-left:70px;'><input type='30' name='conta' id='conta' class='frm' value='$conta'></td>
								<td> <input type='30' name='valor' id='valor' class='frm' value='$valor'></td>
							</tr>
						</table>
				</div>";
			}
		...
		<tr>
			<td colspan="100%" align="center">
				<input type='button' value='Trocar' onclick="javascript: fazer_troca(); ">
			</td>
		</tr>
		<?
