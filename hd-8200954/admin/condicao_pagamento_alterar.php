...
//PROMOÇÃO
$array_condicao[957] ="30DD (promoção)";
$array_condicao[958] ="30/60DD (promoção)";
//$array_condicao[62] ="900 (garantia)";

$array_id_condicao = array(55,53,51,52,57,73,54,56,957,958);
...
if($btn_acao== "Gravar") {
	if((strlen($posto_codigo)>0) and (strlen($id_condicao)>0)) {
		...
		if(pg_numrows($res)>0){
			...
			header ("Location: condicao_pagamento_alterar.php?erro_msg=Não foi encontrado o posto: $posto_codigo!");
		}		
		$res = pg_exec ($con,"BEGIN TRANSACTION");

		//VERIFICA SE É OPÇÃO EM GARANTIA
		if(strlen($garantia)>0){
			...
			//SE NÃO EXISTIR EM GARANTIA, ENTAO INSERE
			if(pg_numrows($res)==0){
				$sql= "INSERT INTO 
				...
				if(pg_result_error($res)){
					...
					$erro_msg.= "Falha no Cadastro em Garantia!";
				}
			}
		}else{
			$sql= "
			...
			//EXISTIR EM GARANTIA, ENTAO DELETA
			if(pg_numrows($res)>0){
				$sql= "
					DELETE from tbl_black_posto_condicao 
					WHERE posto = $posto 
						AND id_condicao = 62;";
		...
		//VERIFICA SE JA FOI CADASTRADO UMA CONDIÇÃO PARA O POSTO E QUE NAO SEJA EM GARANTIA
		$sql= "
			SELECT * 
			FROM tbl_black_posto_condicao 
			WHERE posto = $posto
		...
		if(pg_numrows($res)>0){
			//FAZ UPDATE		
			// nao alterar qd for em garantia
			$sql= "UPDATE tbl_black_posto_condicao 
				SET  data		= current_timestamp,  
					 condicao	= '$condicao',  
					 id_condicao= $id_condicao 
				WHERE posto		= $posto and id_condicao <> 62;";
...
(document.frm_os.posto_codigo,document.frm_os.posto_nome,'codigo')"></A>
			...
			<td nowrap>
				...
				<br>
				<input class="frm" type="text" name="posto_nome" size="35" value="<? echo $posto_nome ?>">&nbsp;<img src='imagens/btn_buscar5.gif' style="cursor:pointer" border='0' align='absmiddle' onclick="javascript: fnc_pesquisa_posto2 (document.frm_os.posto_codigo,document.frm_os.posto_nome,'nome')" style="cursor:pointer;"></A>
			</td>
		...
		<tr>
			...
			if(strlen($posto)>0){
				...
				?>
				<input type='checkbox' name='garantia' value='62' <?echo $checked;?> ><font size="1" face="Geneva, Arial, Helvetica, san-serif">Condição em Garantia</font>
...
if($relatorio == "Clique aqui para ver a relação") {
	$sql1= "Select codigo_posto, condicao 
			from tbl_black_posto_condicao 
			join tbl_posto_fabrica on tbl_posto_fabrica.posto = tbl_black_posto_condicao.posto and tbl_posto_fabrica.fabrica = 1
