...
if (in_array($login_fabrica, array(11,172))) {
	if (!empty($_POST["atendimento_lenoxx[]"]))
...
if ((strlen ($posto_codigo)==0) AND (strlen($posto_nome)==0) AND (strlen($posto_cidade)==0) AND (strlen($posto_estado)==0) AND (strlen($credenciamento)==0) AND (strlen($tipo_posto)==0) AND (strlen($codigo_posto_codigo)==0) AND strlen($btn_acao)>0) {
	if ( $login_fabrica == 86)  {
		if(count($linha)==0){
			$msg_erro["msg"][] = traduz("Escolha um parâmetro para a pesquisa");
		}
	}else{
...
if((!empty($data_inicial)) AND (!empty($data_final))){
	if(empty($_POST["credenciamento"])){
		$msg_erro["msg"][] = traduz("Escolha um status para a pesquisa");
	}
}
...
if ( (count($msg_erro) == 0) && ((strlen ($posto_codigo) > 0) OR (strlen($posto_nome) > 0) OR (strlen($posto_cidade) > 0) OR (strlen($posto_estado) > 0) OR (strlen($linha) > 0) OR (count($linha) > 0) OR (strlen($credenciamento) > 0) OR (strlen($tipo_posto) > 0) OR (strlen($codigo_posto_codigo) > 0 ) OR (strlen($data_inicial) > 0 AND strlen($data_final) > 0))) {
	...
	$sql = "SELECT	$distinct
					tbl_posto.posto                 ,
					REPLACE(tbl_posto_fabrica.nome_fantasia, ';', ',') AS nome_fantasia ,
					REPLACE(tbl_posto.nome, ';', ',') AS nome                  ,
					tbl_posto.cnpj,
					tbl_posto.ie                    ,
					tbl_posto_fabrica.$campo_telefone AS fone,
					REPLACE(tbl_posto_fabrica.contato_endereco, ';', ',') AS endereco,
					tbl_posto_fabrica.contato_bairro   AS bairro,
					tbl_posto_fabrica.contato_cep      AS cep   ,
					tbl_posto_fabrica.contato_numero as numero,
					REPLACE(tbl_posto_fabrica.contato_complemento, ';', ',') as complemento,
					tbl_posto_fabrica.contato_cidade   AS cidade,
					tbl_posto_fabrica.contato_estado   AS estado,
					tbl_posto_fabrica.contato_pais 	   AS pais,
					REPLACE(tbl_posto_fabrica.contato_bairro, ';', ',')   AS bairro,
					...
					if ($login_fabrica == 183){
						$sql .= "tbl_posto_fabrica.pedido_faturado,";
					}
			...
			}elseif (in_array($login_fabrica, array(151,152,180,181,182))) {
				 $sql .= ", tbl_linha.nome as linha_descricao ";
			}
	...
	//HD 110541
	if (!empty($atendimento_lenoxx) && count($atendimento_lenoxx) > 0) $sql .= " AND tbl_posto_fabrica.atendimento IN ('". implode("', '", $atendimento_lenoxx) ."')";
	...
	if (strlen($linha) > 0 || count($linha) > 0 ) {
		...
		$codigo_posto = pg_fetch_result($res_sql, $i, 'codigo_posto');

		$sql_linha = "SELECT DISTINCT tbl_linha.nome as nome_linha
					FROM tbl_linha 
					JOIN tbl_posto_linha ON tbl_posto_linha.linha = tbl_linha.linha 
					AND tbl_posto_linha.posto = (SELECT DISTINCT posto 
		...
		$res_linhas_posto = pg_query($con, $sql_linha);
	}
	...
	if(isset($_POST["gerar_excel"]) && $_POST["gerar_excel"]== "true"){
		...
		if(in_array($login_fabrica, array(152,180,181,182))){
			$head .= traduz("Endereço;Numero;Complemento;Bairro;Cidade;UF;País;Cep;Fone;$desc_contato_cel Fax;CNPJ;Inscrição Estadual;Nome para Contato;E-mail;");
		}else{
			$head .= traduz("Endereço;Numero;Complemento;Bairro;Cidade;UF;Cep;Fone;$desc_contato_cel Fax;CNPJ;Inscrição Estadual;Nome para Contato;E-mail;");
		}
		
		if (in_array($login_fabrica, [35]) && $detalhado == 't') {
			$head .= traduz("Representante Legal;");
		}
		...
		if($login_fabrica == 86){
			if($status == "CREDENCIADO"){
				$head .= traduz("Data Inclusão;");
			}if($status == "DESCREDENCIADO"){
				$head .= traduz("Data Descredenciamento;");
			}if($status == "EM DESCREDENCIAMENTO"){
				$head .= traduz("Data Inclusão;Data Em Descredenciamento;Dias;Observação;");
			}
		}
		...
		if ($login_fabrica == 183) {
			$head .= traduz(utf8_encode("Pedir Peças;"));
		}
		...
		if($login_fabrica == 35 OR $login_fabrica == 91){
			...
			$head .= traduz("Status"). ";";
			if ($tem_fabrica_gestao) {
				$head .= traduz("Data de Credenciamento;");
			}

			if (in_array($login_fabrica, [151])) {
				$head .= traduz("Último Credenciamento;Último Descredenciamento;Último Em Credenciamento;Último Em Descredenciamento;");
			}

			if (in_array($login_fabrica, [169,170])) {
				$head .= "Valor Km;";
			}
			$head .= traduz( "Tipo Posto"). ";" ;

			if(in_array($login_fabrica, [104])){
				$head .= traduz("Digita OS") . ";" ;
				$head .= utf8_encode(traduz("Prestação de Serviço")) . ";";
				$head .= traduz("Pedido em Garantia (Manual)") . ";";
				$head .= traduz("Pedido Faturado (Manual)") . ";";
				$head .= traduz("Pedido Via Distribuidor") . ";";
			}

			if (in_array($login_fabrica, [151])) {
				$head .= traduz("Divulgação;");
			}

			if (!in_array($login_fabrica,array(104,169,170))){

				$head .= traduz("Linha;");

			} else {
				$head .= traduz("Prestação de Serviço Isenta de M.O.;");
			}
        ...
        if ($login_fabrica == 117) {
                $head = traduz("Nome Fantasia;Razão Social;Endereço;Numero;Bairro;Cidade;UF;Cep;Fone;CNPJ;Inscrição Estadual;Nome para Contato;E-mail;Status;Tipo Posto;");
                $head .="\n";
        }
		...
		for($i = 0; $i<$count;$i++) {
			...
			$pais 			= trim(pg_result($res_consulta,$i,pais));
			...
			$prestacao_servico_sem_mo = trim(pg_fetch_result($res_consulta,$i, 'prestacao_servico_sem_mo')) == t ? 'Sim': 'Não';
			...
			if($login_fabrica == 104){
				$prestacao_servico = trim(pg_fetch_result($res_consulta, $i, 'prestacao_servico'));
				$pedido_faturado = trim(pg_result($res_consulta,$i,"pedido_faturado"));
				$pedido_distribuidor = trim(pg_result($res_consulta, $i, "pedido_via_distribuidor"));
			...
			if(in_array($login_fabrica, array(152,180,181,182))){
				...
				$sql_estado = "SELECT nome FROM tbl_estado_exterior WHERE estado = '$estado' AND pais = '$pais'";
				$res_estado = pg_query($con, $sql_estado);

				if(pg_num_rows($res_estado) > 0){
					$estado = pg_fetch_result($res_estado, 0, 'nome');
				}
			}
			...
			if (empty($linha)) {
				$linhas = pg_fetch_all($res_linhas_posto);

				foreach ($linhas as $linhas_nome) {
					foreach ($linhas_nome as $linha_nome) {
						$linha .= $linha_nome . ';';
					}
				}
			}
			...
			if (in_array($login_fabrica, array(104))) {
				...
				if ($parametros_adicionais["posto_auditado"] == "") { 
					$posto_auditado = "NAO"; 
				} else {
					$posto_auditado = "SIM";	
				}
			...
			if(in_array($login_fabrica, array(152,180,181,182))){
				$body .= "$endereco;$numero;$complemento;$bairro;$cidade;$estado;$pais;$cep;$fone; $valor_contato_cel $contato_fax;$cnpj;$ie;$contato;$email;";
			}else{
			...
			if($login_fabrica == 35 OR $login_fabrica == 91){
				...
				if(in_array($login_fabrica, [104])){
					$body .= ($digita_os == 't') ? 'Sim;' : utf8_encode('Não;');
					$body .= ($prestacao_servico == 't') ? 'Sim;' : utf8_encode('Não;');
					$body .= ($pedido_em_garantia == 't') ? 'Sim;' : utf8_encode('Não;');
					$body .= ($pedido_faturado == 't') ? 'Sim;' : utf8_encode('Não;');
					$body .= ($pedido_distribuidor == 't') ? 'Sim;' : utf8_encode('Não;');
				}

				if (in_array($login_fabrica, [169,170])) {
					$body .= "$prestacao_servico_sem_mo;";
				}
				if (in_array($login_fabrica, [151])) {
					$body .= "$divulgar_consumidor;";
				}
...
<script type='text/javascript'>
    ...
    $(function() {
    	...
    	<?php if (strlen($_POST['posto_estado']) == 0) { ?> 
	    	$("#posto_estado").append("<option value=''><?=traduz('TODOS OS ESTADOS');?></option>");
        <?php } ?>
        ...
        <?php
	$estadosPais = $array_estados($pais);
	foreach ($estadosPais as $siglaEstado => $descEstado) { ?>
	    var estado = '<?= $descEstado ?>';
	    var sigla = '<?= $siglaEstado ?>';
...
$(function(){
	...
	$("#atendimento_lenoxx").multiselect();
...
<form name='frm_posto' method='POST' ACTION='<?=$PHP_SELF?>' align='center' class='form-search form-inline tc_formulario'>
	...
	<div class='row-fluid'>
			...
			<div class='span4'>
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
					<label class='control-label' for='descricao_posto'><?php echo traduz("CNPJ");?></label>
					<div class='controls controls-row'>
						<div class='span12 input-append'>
			...
			<div class='span4'>
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
					<label class='control-label' for='codigo_posto'><?php echo traduz("Tipo Posto");?></label>
					<div class='controls controls-row'> <?
						$sql = "SELECT  *
								FROM    tbl_tipo_posto
			...
			<div class='span4'>
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
					...
					<div class='controls controls-row'>
						...
						<select name='posto_estado' class='frm <?=$addressState?>' id='posto_estado'></select>
					</div>
				</div>
			</div>
			<div class='span4' id="cidade_info">
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
		...
		<div class='row-fluid'>
			...
			<div class='span4' id="bairro_info">
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
					<label class='control-label' for='descricao_posto'><?php echo traduz("Bairro"); ?></label>
					<div class='controls controls-row'>
						<div class='span12 input-append'>
			...
			<div class='span4'>
				<div class='control-group <?=(in_array("inspetor", $msg_erro["campos"])) ? "error" : ""?>'>
					<label class='control-label' for='inspetor'><?php echo traduz("Inspetor"); ?></label>
					<div class='controls controls-row'>
						<div class='span12 input-append'>
							<?php
			...
			<div class='span4'>
				<div class='control-group <?=(in_array("posto_digita", $msg_erro["campos"])) ? "error" : ""?>'>
					<label class='control-label' for='posto_digita'><?php echo traduz("Posto pode Digitar");?></label>
					<div class='controls controls-row'>
		...
		<div class='row-fluid'>
			...
			<div class='span4'>
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
					<label class='control-label' for='descricao_posto'><?php echo traduz("Status");?></label>
					<div class='controls controls-row'>
						<div class='span12 input-append'>
							<select class='frm' style='width: 150px;' name='credenciamento' >
								<option value=''><?php echo traduz("TODOS");?></option>
								<option value='CREDENCIADO' <? if ($credenciamento== "CREDENCIADO") echo " SELECTED "; ?> ><?php echo traduz("CREDENCIADO");?></option>
								<option value='DESCREDENCIADO' <? if ($credenciamento== "DESCREDENCIADO") echo " SELECTED "; ?> ><?php echo traduz("DESCREDENCIADO");?></option>
								<option value='EM CREDENCIAMENTO' <? if ($credenciamento== "EM CREDENCIAMENTO") echo " SELECTED "; ?> ><?php echo traduz("EM CREDENCIAMENTO");?></option>
								<option value='EM DESCREDENCIAMENTO' <? if ($credenciamento== "EM DESCREDENCIAMENTO") echo " SELECTED "; ?> ><?php echo traduz("EM DESCREDENCIAMENTO");?></option>
								<?
								if ( in_array($login_fabrica, array(11,172)) ) {
				...
				<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
					...
					<div class='controls controls-row'>
						<input class="form-check-input" type="checkbox" name="detalhado" <?php echo (isset($_POST['detalhado']))? " checked " : " "; ?> value="t">
						    <?php echo traduz("Mostrar Informações Detalhadas");?>
					</div>
				</div>
			</div>
			<?php } ?>
      ...
      <div class='span4'>
        <div class='control-group'>
          <label class='control-label' for='contrato'><?php echo traduz("Contrato");?></label>
          <div class='controls controls-row'>
            <div class='span12'>
              <select class='frm' name='contrato'>
                <option value=''><?php echo traduz("TODOS");?></option>
                <option value='t' <? if ($contrato== "t") echo " SELECTED "; ?> ><?php echo traduz("POSTO COM CONTRATO");?></option>
                <option value='f' <? if ($contrato== "f") echo " SELECTED "; ?> ><?php echo traduz("POSTO SEM CONTRATO");?></option>
                </select>
            </div>
          </div>
        </div>
		...
		if( in_array($login_fabrica, array(11,172)) ){ ?>
			<div class='row-fluid'>
				<div class='span2'></div>
					<div class='span4'>
						<div class='control-group <?=(in_array("posto", $msg_erro["campos"])) ? "error" : ""?>'>
							<label class='control-label' for='codigo_posto'><?php echo traduz("Atendimento");?></label>
							<div class='controls controls-row'>
								<select class='frm span12' style='width: 150px;' id="atendimento_lenoxx" name='atendimento_lenoxx[]' multiple="multiple">
									<option value='b' <?= array_key_exists('b', array_flip($atendimento_lenoxx)) ? "selected" : "" ?>><?= traduz("BALCÃO");?></option>
		 							<option value='r' <?= array_key_exists('r', array_flip($atendimento_lenoxx)) ? "selected" : "" ?>><?= traduz("REVENDA");?>");?></option>
									<option value='t' <?= array_key_exists('t', array_flip($atendimento_lenoxx)) ? "selected" : "" ?>><?= traduz("BALCÃO/REVENDA");?></option>
									<option value='e' <?= array_key_exists('e', array_flip($atendimento_lenoxx)) ? "selected" : "" ?>><?= traduz("EXCLUSIVO");?></option>
								</select>
							</div>
						</div>
					</div>
            ...
            <div class="span4">
                <div class="control-group">
                    <div class="controls controls-row tac">
                        <button type="button" class="btn" value="Gravar" alt="Gravar formulário" onclick="submitForm($(this).parents('form'),'ok');" ><?php echo traduz("Filtrar"); ?></button>
                    </div>
                </div>
            </div>
...
<? if($login_fabrica == 183) : ?>
<figure class="highcharts-figure" id="grafico-credenciado" style="display:none">
...
if ( (count($msg_erro) == 0) && ((strlen ($posto_codigo) > 0) OR (strlen($posto_nome) > 0) OR (strlen($posto_cidade) > 0) OR (strlen($posto_estado) > 0) OR (strlen($linha) > 0) OR (count($linha) > 0) OR (strlen($credenciamento) > 0) OR (strlen($tipo_posto) > 0) OR (strlen($codigo_posto_codigo) > 0 ) OR (strlen($data_inicial) > 0 AND strlen($data_final) > 0)) ) {
	...
	if (pg_numrows ($res_consulta) > 0) {?>
		<table width='850px' id="relatorio_listagem" name="relatorio_listagem" class='table table-striped table-bordered table-hover table-large'>
			<thead>
				<tr class = 'titulo_coluna'>
					<th align='center' width='50%' nowrap><?php echo traduz("Código");?></th>
					<th align='center' width='50%' nowrap><?php echo traduz("Nome Fantasia");?></th>
					<th align='center' width='50%' nowrap><?php echo traduz("Razão Social");?></th>
					<?php if (in_array($login_fabrica, array(104))) { ?>
						<th align='center' width='50%' nowrap><?=traduz('Posto em Auditoria');?></th>
					<?php }
					if (in_array($login_fabrica, array(11,80,169,170,172))) { ?>
						<th align='center' norwap><?=traduz('Linha');?></th>
					<?php } ?>
					<th align='center' width='15%' nowrap><?php echo traduz("Endereço");?></th>
					<th align='center' width='15%' nowrap><?php echo traduz("Número");?></th>
					<th align='center' width='15%' nowrap><?php echo traduz("Complemento");?></th>
					<th align='center' width='15%' nowrap><?php echo traduz("Bairro");?></th>
					<th align='center' width='15%' nowrap><?php echo traduz("Cidade");?></th>
					<th align='center' width='5%' nowrap><?php echo traduz("UF");?></th>
					<?php if (in_array($login_fabrica, array(152,180,181,182))) { ?>
						<th align='center' norwap><?=traduz('País');?></th>
					<?php } ?>
					<th align='center' width='5%' nowrap><?php echo traduz("Cep");?></th>
					<th align='center' width='50%' nowrap><?php echo traduz("Fone");?></th>
					<?php if($login_fabrica == 151){ ?>
						<th align='center' width='50%' nowrap><?php echo traduz("Fone 2");?></th>
						<th align='center' width='50%' nowrap><?php echo traduz("Fone 3");?></th>
						<th align='center' width='50%' nowrap><?php echo traduz("Celular");?></th>
						<th align='center' width='50%' nowrap><?php echo traduz("Fax");?></th>
					<?php } ?>
					<?php if($login_fabrica == 35){ ?>
						<th align='center' width='50%' nowrap><?php echo traduz("Celular Contato");?></th>
						<th align='center' width='50%' nowrap><?php echo traduz("Fax");?></th>
					<?php } ?>
					<th align='center' width='20%' nowrap><?php echo traduz("CNPJ");?></th>
					<th align='center' width='20%' nowrap><?php echo traduz("Inscrição Estadual");?></th>
					<th align='center' width='50%' nowrap><?php echo traduz("Nome para Contato");?></th>
					<th align='center' width='50%' nowrap><?php echo traduz("E-mail");?></th>
					<?php
					if (in_array($login_fabrica, [35]) && $detalhado == 't') { ?>
						<th class="tac"><?php echo traduz("Representante Legal"); ?></th>
					<?php
					}

						if($login_fabrica==45) echo "<th align='center' width='50%' nowrap>" . traduz("Linhas") . "</th>";
						if($login_fabrica==151) echo "<th align='center' width='50%' nowrap>" . traduz("Valor Mão Obra") . "</th>";

					?>
					<th align='center' width='10%' nowrap>Status	</th>

					<?php if (in_array($login_fabrica, [151])) { ?>
						<th align='center'><?php echo traduz("Último Credenciamento");?></th>
						<th align='center'><?php echo traduz("Último Descredenciamento");?></th>
						<th align='center'><?php echo traduz("Último Em Credenciamento");?></th>
						<th align='center'><?php echo traduz("Último Em Descredenciamento");?></th>
					<?php } ?>
					<?php
					if (in_array($login_fabrica,array(169,170))){
					...
					?>
					<th align='center' width='10%' nowrap><?=traduz ("Tipo Posto")?></th>

					<? if(in_array($login_fabrica, [104])) {
						echo "<th align='center' nowrap>".traduz("Digita OS")."</th>";
						echo "<th align='center' nowrap>".traduz("Prestação de Serviço")."</th>";
						echo "<th align='center' nowrap>".traduz("Pedido em Garantia (Manual)")."</th>";
						echo "<th align='center' nowrap>".traduz("Pedido Faturado (Manual)")."</th>";
						echo "<th align='center' nowrap>".traduz("Pedido Via Distribuidor")."</th>";
					}?>

					<?= (in_array($login_fabrica, [183])) ? "<th align='center' nowrap>".traduz ("Pedir Peça")."</th>" : "" ?>

					<?= (in_array($login_fabrica, [151])) ? "<th align='center' nowrap>".traduz ("Divulgação")."</th>" : "" ?>
					<?php if($login_fabrica ==35 AND $detalhado == 't'){?>
						<th align='center' width='10%' nowrap><?php echo traduz("Região Suframa");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Capital/Interior");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Transportadora");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Desconto");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Valor KM");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Item Aparência");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Divulga Consumidor");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Observação Interna");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Observação (Cadence)");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Observação (Oster)");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Informações de Cobrança");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Banco");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Agência");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Conta");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Tipo de Conta");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Nome Favorecido");?></th>
						<th align='center' width='10%' nowrap><?php echo traduz("Observação Conta");?></th>
						...
						echo "<th align='center' width='10%' nowrap>" . traduz("Pedido Faturado") . "</th>";
						echo "<th align='center' width='10%' nowrap>" . traduz("Pedido em Garantia") . "</th>";
						echo "<th align='center' width='10%' nowrap>" . traduz("Digita OS") . "</th>";

						?>
					<?php } ?>

					<?if(in_array($login_fabrica, array(151,152,157,180,181,182))) echo "<th align='center' width='50%' nowrap>Linha</th>";?>			

					<?php if($login_fabrica == 86){ ?>
						<?php if($status == "CREDENCIADO"){?>
							<th align='center' width='10%' nowrap><?php echo traduz("Data Inclusão"); ?></th>
						<?php }	?>

						<?php if($status == "DESCREDENCIADO"){ ?>
							<th align='center' width='10%' nowrap><?php echo traduz("Data Descredenciamento"); ?></th>
						<?php }	?>

						<?php if($status == "EM DESCREDENCIAMENTO"){ ?>
							<th align='center' width='10%' nowrap><?php echo traduz("Dias/Desc"); ?></th>
							<th align='center' width='10%' nowrap><?php echo traduz("Data Inclusão"); ?></th>
							<th align='center' width='10%' nowrap><?php echo traduz("Data Em Descredenciamento"); ?></th>
							<th align='center' width='10%' nowrap><?php echo traduz("Observação"); ?></th>
						<?php }	?>						
					<?php }	?>		
					...
					if($login_fabrica == 35 OR $login_fabrica == 91) 
						...
						echo "<th align='center' width='50%' nowrap>".traduz('Data Status')."</th>";
					
					if ($login_fabrica == 35) {
						echo "<th align='center' width='50%' nowrap>".traduz('Isento Anexo NF da OS')."</th>";
					}

					if(in_array($login_fabrica,[169,170]))
						echo "<th align='center' width='50%'>".traduz('Prestação de Serviço Isenta de M.O.')."</th>";

					if ($gerar_xls != "t") {
						echo "<th align='center' width='5%'>" . traduz("Ações") . " </th>";
					}?>
				</tr>
			</thead><tbody>
		<?
		...
		for ($i = 0 ; $i < $count ; $i++) {
			...
			if($login_fabrica == 104){
				$prestacao_servico = trim(pg_result($res_consulta, $i, prestacao_servico));
				$pedido_faturado = trim(pg_result($res_consulta, $i, pedido_faturado));
				$pedido_distribuidor = trim(pg_result($res_consulta, $i, pedido_via_distribuidor));
				$pedido_em_garantia = trim(pg_result($res_consulta, $i, pedido_em_garantia));
			}
			...
			<?php if (in_array($login_fabrica, array(104))) {
				...
				echo '<td align="left" nowrap style="color: ', $cor_auditado, ';">', $posto_auditado, '</td>';
			...
			if(in_array($login_fabrica, [104])){
				...
				$aux_digita_os    = ($digita_os == 't') ? 'Sim' : 'Não';
				$aux_prestacao    = ($prestacao_servico == 't') ?  'Sim' : 'Não';
				$aux_garantia     = ($pedido_em_garantia == 't') ? 'Sim' : 'Não';
				$aux_faturado     = ($pedido_faturado == 't') ? 'Sim' : 'Não';
				$aux_distribuidor = ($pedido_distribuidor == 't') ? 'Sim' : 'Não';

				echo "<td class=table_line align='center'>$aux_digita_os</td>";
				echo "<td class=table_line align='center'>$aux_prestacao</td>";
				echo "<td class=table_line align='center'>$aux_garantia</td>";
				echo "<td class=table_line align='center'>$aux_faturado</td>";
				echo "<td class=table_line align='center'>$aux_distribuidor</td>";
			}
			...
			if ($login_fabrica == 183){
				if($pedido_faturado == 't'){
					echo "<td align='left' class='tac' nowrap>Sim</td>";
				}else{
					echo "<td align='left' class='tac' nowrap>Não</td>";
				}
			}
			...
			if(in_array($login_fabrica,array(151,157,180,181,182))) echo "<td class=table_line align='left'>$linha</td>";
			...
			if(in_array($login_fabrica,[169,170])) {
				echo "<td nowrap><center>$prestacao_servico_sem_mo</center></td>";
			}	
			if ($gerar_xls != "t") {
			//echo "<TD><a href = 'posto_consulta_detalhe.php?posto=$posto'><img src='imagens/btn_consultar_azul.gif'></a></TD>";
			echo "<td><a href = 'posto_consulta_detalhe.php?posto=$posto' target='_blank'><button type='button' class='btn-mini btn-success'>" . traduz("Consultar") ."</button></a></td>";
			}
			echo "</tr>";
		}
		...
		echo "<tr class='titulo_tabela'><td colspan='8' class='titulo_tabela'>" . traduz("LEGENDA STATUS") . "</td></tr>";
		echo "<tr>";
...
<? if($login_fabrica == 183 && empty($status) && $res_consulta) : ?>

	<script type="text/javascript">

		$('#grafico-credenciado').css('display', 'block');
		var total_credenciado = '<?=$total_credenciado;?>';
		var total_descredenciado = '<?=$total_descredenciado;?>';
		...
		Highcharts.chart('container', {
		    ...
		    title: {
		        text: 'Relação entre postos Credenciados x Descredenciados'
		    },
		    plotOptions: {
		        pie: {
		    ...
		    series: [{
		        ...
		        data: [{
		            name: 'Credenciados',
		            y: parseInt(total_credenciado)
		        }, {
		            name: 'Descredenciados',
		            y: parseInt(total_descredenciado)
