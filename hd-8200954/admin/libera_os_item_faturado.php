...
if(strlen($ajax)>0){
	...
	$sql =	"SELECT tbl_pedido.pedido                                , 
		...
		AND  tbl_pedido.tipo_pedido in(4,112)
...
function retornaMovimentacao(pedido){
	...
	url ="<?echo $PHP_SELF;?>?ajax=true&pedido="+pedido;
...
//			echo "Pedido $aceitar aceito<BR>";
			...
			if (strlen($msg_erro) == 0) {
				...
				$assunto      = "Tectoy liberou pedido $pedido"; 
				$mensagem     = "Verificar pedido $pedido, foi liberado pelo admin"; 
...
//			echo "Pedido $recusar recusado<BR>";
			$sql = "UPDATE tbl_pedido set controle_exportacao = current_timestamp , status_pedido = 17
					where pedido = $pedido and fabrica = $login_fabrica";
			//echo "$sql<BR>";
			$res = @pg_exec ($con,$sql);
			$msg_erro = pg_errormessage($con);
			...
			if (strlen($msg_erro) == 0) {
				...
				$remetente    = "Telecontrol <telecontrol@telecontrol.com.br>"; 
				$destinatario = "takashi@telecontrol.com.br"; 
				$assunto      = "Tectoy recusou pedido $pedido"; 
				$mensagem     = "Verificar pedido $pedido, foi recusado pelo admin"; 
				$headers="Return-Path: <telecontrol@telecontrol.com.br>\nFrom:".$remetente."\nContent-type: text/html\n"; 
			//	@mail($destinatario,$assunto,$mensagem,$headers);
	...
	for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
		...
		if ($posto_codigo_anterior != $posto_codigo) {
			if ($i != 0) {
				...
				echo "<td>\n";
				echo "<input type='checkbox' name='liberar' id='lib_$i'value='liberar' onclick=\"SelecionarLiberar('os_item', this, $qtde_liberar_inicio, $qtde_liberar_final); \" class='frm'>";
				echo "</td>\n";
				echo "<td>\n";
				echo "<input type='checkbox' name='recusar'id='rec_$i' value='recusar' onclick=\"SelecionarLiberar('recusar', this, $qtde_liberar_inicio, $qtde_liberar_final);\" class='frm'>\n";
				echo "</td>\n";
			...
			echo "<tr class='Titulo'>\n";
			echo "<td colspan='6'><b>Posto: $posto_codigo - $posto_nome</b></td>\n";
			echo "</tr>\n";
...
//		if ($os_item == $_POST["os_item_".$i]) $checked = "checked";
		...
		echo "<input type='checkbox' name='os_item_$i' id='os_item_$i' value='$pedido' class='frm' >";
		echo "</td>\n";
		echo "<td bgcolor='#dcc6c6' align='center'><input type='checkbox' $checked name='recusar_$i' id='recusar_$i' value='$pedido' class='frm' ></td>\n";
		echo "<td nowrap align='center'><a href='pedido_cadastro.php?pedido=$pedido' target='_blank'>$pedido</a></td>\n";
		echo "<td align='center'>$tipo_pedido</td>";
	...
	echo "<td>";
	echo "<input type='checkbox' name='recusar' id='rec_$i' value='recusar' onclick=\"SelecionarLiberar('recusar', this,$qtde_liberar_inicio,$qtde_liberar_final);\" class='frm'></td>\n";
