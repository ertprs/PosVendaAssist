...
$title = "Relatório de OSs de Sanitização";
...
if ($_POST) {
    ...
    if (empty($msg_erro["msg"])) {
        ...
        $tabela = "temp_os_sanitizacao_{$login_admin}_{$login_fabrica}";
        ...
        $sql = "
            SELECT
                ...
                EXTRACT(DAY FROM tbl_os.exportado) AS dia_exportada,
            ...
            AND tbl_tipo_atendimento.grupo_atendimento = 'S'
...
<?php
    ...
    $sql = "
        SELECT dia_finalizada, mes, familia, tecnico, tipo_posto, unidade_negocio, COUNT(os) AS qtde_os
        FROM {$tabela}
        WHERE finalizada IS NOT NULL
        GROUP BY dia_finalizada, mes, familia, tecnico, tipo_posto, unidade_negocio
        ORDER BY dia_finalizada ASC, mes, familia, tecnico, tipo_posto, unidade_negocio
    ";
    $res = pg_query($con, $sql);
...
if (pg_num_rows($res) > 0) {
    ...
    $xls  = "relatorio-oss-sanitizacao-{$login_fabrica}-{$login_admin}-".date("YmdHi").".xls";
