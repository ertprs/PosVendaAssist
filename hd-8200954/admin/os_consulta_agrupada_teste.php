...
if ((strlen($_POST['btn_acao']) > 0 or strlen($_GET['btn_acao']) > 0) AND strlen($msg) == 0) {
		ob_start(); //Brayan HD#292017, resultados abaixo do formulario @result
		$join_especifico = "";
...
//	$resT = @pg_exec ($con,"/* QUERY -> $sqlT  */");
	...
	if (pg_numrows($res) > 0) {
		echo "<table border='0' cellpadding='0' cellspacing='1' class='tabela'  align='center' width='86%'>";
		...
		$msg_resultado="Nenhum resultado encontrado";
	}
	// ------------------------------ Resumo (HD 104151)
...
<?php if ( $login_fabrica == 3 && $resultados <= 0 ): ?>
    <div class="texto_avulso">
		O resumo da quantidade de produtos, só é exibido caso seja filtrado um "Número de uma Nota Fiscal de Compra e um Posto"
...
<?
	if(strlen($msg)>0){
		echo "<tr >";
		echo "<td class='msg_erro' colspan='3'>$msg</td>";
		echo "</tr>";
	}
?>
	<tr>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td colspan="100%" class="espaco">
			<table>
				<tr>
					<td width="130px">
						Número de Série<br />
						<input type="text" name="serie"     size="10" value="<?echo $serie?>"     class="frm">
					</td>
					<td width="130px">
						NF. Compra<br />
						<input type="text" name="nf_compra" size="10" value="<?echo $nf_compra?>" class="frm">
					</td>
					<td>
						CPF Consumidor<br />
						<input type="text" name="consumidor_cpf" size="10" value="<?echo $consumidor_cpf?>" class="frm">
					</td>
				</tr>
			</table>
		</td>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td class="espaco">
			<fieldset style="width:120px;">
				<legend>Mostrar Apenas</legend>
				<input type="radio" name="os_situacao" value="APROVADA" <? if ($os_situacao == "APROVADA") echo "checked"; ?>> OS´s Aprovadas<br />
				<input type="radio" name="os_situacao" value="PAGA" <? if ($os_situacao == "PAGA") echo "checked"; ?>> OS´s Pagas</td>
			</fieldset>
		</td>
		<td>
			Admin<br />
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td colspan='3' class="espaco"> OS em aberto da Revenda = CNPJ
		<input class="frm" type="text" name="revenda_cnpj" size="8" value="<? echo $revenda_cnpj ?>" maxlength="8"> /0001-00
		</td>
	</tr>
	<tr class="Conteudo" bgcolor="#D9E2EF">
		<td colspan='3' align='center' style="padding:10px 0 10px;"><input type="submit" name="btn_acao" value="Pesquisar"></td>
	</tr>
</table>
</form>
<br />
<?php
	...
	echo isset($msg_resultado) ? $msg_resultado : '';
?>
