...
if(strlen($btn_acao)>0){
		...
		if($login_fabrica==11){//HD 53881 27/11/2008
			...
			if($tipo_reclamacao=="reclamacao_at_info"){
		...
		if($tab_atual == "reclamacao_empresa"){
			...
			if(strlen($reclamado)==0){
				echo '2';$msg_erro = "Insira a reclamação";
			}else{
				$xreclamado = "'".$reclamado."'";
			}
		}
		...
		if($tab_atual == "reclamacoes"){
			...
			if(strlen($reclamado)==0){
				echo '3';$msg_erro = "Insira a reclamação";
			}else{
				$xreclamado = "'".$reclamado."'";
			}
		}
	...
	if(strlen($msg_erro)==0){
		...
		if($ultimo_atendente <> $xtransferir){
			...
			if(strlen($email_ultimo_atendente) >0 AND strlen($email_atendente) >0){
				...
				if ( @mail($email_atendente, stripslashes($assunto), $corpo, "From: ".$email_ultimo_atendente." \n $body_top " ) ){
	...
	$sql = "SELECT	tbl_hd_chamado_extra.hd_chamado as callcenter,
		...
		if(pg_numrows($res)>0){
			...
			if ($login_fabrica==59){
				...
				if($login_fabrica == 11){
					if($natureza_chamado == 'reclamacao_at_info' or $natureza_chamado == 'reclamacao_at'){
						$natureza_chamado2 = $natureza_chamado;
	...
	<div id='div_consumidor' style='display:inline; Position:relative;background-color: #e6eef7;width:100%'>
		...
		<tr>
			...
			<td align='left'>
				...
				<input name="consumidor_cpf" id="cpf" value='<?echo $consumidor_cpf ;?>' class="input" type="text" size="18" maxlength="14" onkeypress="return txtBoxFormat(this.form, this.name, '999.999.999-99', event);" <?PHP if($login_fabrica==11 and (strlen($callcenter) >0 or strlen($Id) >0) ){?>  readonly <?}?> >
		...
		<tr>
			...
			<td align='left'>
				...
				onblur="javascript: fnc_pesquisa_produto2 (document.frm_callcenter.produto_referencia,document.frm_callcenter.produto_nome,'referencia',document.frm_callcenter.mapa_linha); <?php if ($login_fabrica <> 51){ # HD 41923 ?>
					mostraDefeitos('Reclamação',document.frm_callcenter.produto_referencia.value);
					<?php } ?>
...
<tr>
	...
	<ul>
		...
		<li>
			...
			<span>
			<!--<img src='imagens/rec_produto.png' width='10' align="absmiddle" alt='Reclamação Produto/Defeito'>-->Produto/Defeito</span>
			</a>
		</li>
		<?PHP if($login_fabrica != 2) {?>
		<li>
		...
		<?
			if ($login_fabrica == 11){
				if($tab_atual == 'reclamacao_at_info') {
		...
		?>
			...
			<span><!--<img src='imagens/rec_empresa.png' width='10' align="absmiddle" alt='Reclamação Assistência Técnica'>-->
			<? if($login_fabrica==11){
					echo "A.T.";
				}else{
					echo "Recl. A.T.";
		...
		<li>
			...
			<span><!--<img src='imagens/rec_empresa.png' width='10' align="absmiddle" alt='Reclamação Assistência Técnica'>-->Reclamações</span>
			</a>
		</li>
		<?PHP
		}
		...
		<?}?>
		<?if(1 == 2 /* $login_fabrica==46 OR $login_fabrica == 11 Samuel Tirou esta aba, Troca de Produto é somente permitido na OS, não pode ser feita no call-center*/ ){?>
		<li>
	...
	<div id="reclamacao_produto" class='tab_content'>
			...
			<?PHP
				if (1 ==2 /*$login_fabrica != 45 AND $login_fabrica != 3 Samuel retirou isto...a consulta do posto mais próximo é através do Mapa da Rede */ ) {
			?>
	...
	<? if($login_fabrica <> 2){ ?>
				...
				<tr>
					...
					<td align='center'>
						...
						if(pg_numrows($pe)>0) {
							...
							echo "Qual é a sua reclamação SR.(a)?<BR>	ou<BR> O Sr.(a) diz que...., correto?";
						}?>
					</td>
					<td align='right' width='150'></td>
				</tr>
		...
		<div id="reclamacao_at" class='tab_content'>
			...
			<? if($login_fabrica==11){ ?>
				...
				<table width='100%' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
				<tr>
					<td align='left' class="padding" width='50%'><input type="radio" name="tipo_reclamacao" value="reclamacao_at" <?PHP if ($natureza_chamado2 == 'reclamacao_at' OR $natureza_chamado2 =='') { echo "CHECKED";}?>> RECLAMAÇÃO DA  ASSIST. TÉCN.</td>
					<td align='left' class="padding" width='50%'><input type="radio" name="tipo_reclamacao" value="reclamacao_at_info" <?PHP if ($natureza_chamado2 == 'reclamacao_at_info') { echo "CHECKED";}?>>INFORMAÇÕES DE A.T</td>
				</tr>
				</table>
			<? }
		...
		<div id="duvida_produto" class='tab_content'>
				...
				<tr>
					...
					<td align='center'>
						...
						if(pg_numrows($pe)>0) {
							...
							echo "Qual é a sua dúvida SR.(a)?<BR>	ou<BR>A dúvida do Sr.(a) é sobre como...., correto?";
						}
						?>
					</td>
					<td align='right' width='150'></td>
			...
			<tr>
				...
				<? if($login_fabrica==2) {
						...
						echo "<a href=\"javascript:listaFaq(document.frm_callcenter.produto_referencia.value)\">Listar todas dúvidas cadastradas ou cadastrar a nova</a>";
						echo "</td>";
					}else{
						$coluna ="6";
					}
...
-->
			...
			<tr>
				...
				<td align='left'><strong>Numero do Processo:</strong></td>
...
-->
			...
			<tr>
				...
				<td align='left'>
					...
					onblur="javascript: fnc_pesquisa_produto (document.frm_callcenter.troca_produto_referencia,document.frm_callcenter.troca_produto_nome,'referencia');mostraDefeitos('Reclamação',document.frm_callcenter.troca_produto_referencia.value)" type="text" size="15" maxlength="15" <?if(strlen($callcenter)>0) echo " READONLY "?>>
					<img src='imagens/lupa.png' border='0' align='absmiddle'
					style='cursor: pointer'
				...
				<td align='left'>
					...
					onblur="javascript: fnc_pesquisa_produto (document.frm_callcenter.troca_produto_referencia,document.frm_callcenter.troca_produto_nome,'descricao');mostraDefeitos('Reclamação',document.frm_callcenter.troca_produto_referencia.value)" type="text" size="35" maxlength="500" <?if(strlen($callcenter)>0) echo " READONLY "?>>
					<img src='imagens/lupa.png' border='0' align='absmiddle'
					style='cursor: pointer'
					onclick="javascript: fnc_pesquisa_produto (document.frm_callcenter.troca_produto_referencia,document.frm_callcenter.troca_produto_nome,'descricao')">
				</td>
