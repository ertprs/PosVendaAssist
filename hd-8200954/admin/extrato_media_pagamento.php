...
$title = "Consulta e Manutenção de Extrato da Media de Pagamento";
...
//echo nl2br($sql)."<br><br>".pg_numrows($res)."<br><br>"; exit;
	...
	if (pg_numrows ($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
			...
			$extrato      = trim(pg_result($res,$i,extrato));
			//$inicio_digitacao = trim(pg_result($res,$x,inicio));
			//$final_digitacao  = trim(pg_result($res,$x,'final'));
			...
			if ($i == 0) {
				...
				echo "<td align='center'>Digitações</td>\n";
				echo "<td align='center'>Nota Credito</td>\n";
				echo "<td align='center'>Valor Extrato</td>\n";
				echo "<td align='center' nowrap>Data Extrato</td>\n";
				echo "<td align='center'>Data Aprovação</td>\n";
