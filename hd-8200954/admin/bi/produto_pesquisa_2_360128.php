...
if (pg_numrows($res) == 1) {
	...
	echo "<script language='JavaScript'>\n";
	if (strlen($_GET['lbm']) > 0) echo "window.opener.document.getElementsByName('$campo_produto')[0].value = '".trim(pg_result($res,0,produto))."';";

	if ($login_fabrica == 1) {
		echo "window.opener.document.getElementsByName('$campo_descricao')[0].value  = '".str_replace ('"','',trim(pg_result($res,0,descricao)))." ".trim(pg_result($res,0,voltagem))."';";
	}else{
		echo "window.opener.document.getElementsByName('$campo_descricao')[0].value  = '".str_replace ('"','',trim(pg_result($res,0,descricao)))."';";
	}

	if ($_GET["proximo"] == "t") 
		echo "proximo.focus();";

	echo "window.opener.document.getElementsByName('$campo_referencia')[0].value = '".trim(pg_result($res,0,referencia))."';";
	echo "this.close();";
	echo "</script>\n";
}
...
echo "<tr class='titulo_coluna'><td>Código</td><td>Nome</td><td>Voltagem</td><td>&nbsp;</td>";
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		if (strlen($_GET['lbm']) > 0) 
			echo "window.opener.document.getElementsByName('$campo_produto')[0].value = '$produto'; ";

		if ($login_fabrica == 1) {
			echo "window.opener.document.getElementsByName('$campo_descricao')[0].value = '$descricao $voltagem'; if (window.voltagem) { window.opener.document.getElementsByName('$campo_voltagem')[0].value = '$voltagem' ; }";
			if ($_GET["voltagem"] == "t") echo "window.opener.document.getElementsByName('$campo_voltagem')[0].value = '$voltagem'; ";
		}else{
			echo "window.opener.document.getElementsByName('$campo_descricao')[0].value = '$descricao'; ";
		}

		echo "window.opener.document.getElementsByName('$campo_referencia')[0].value = '$referencia'; ";
