...
if ($btn_finalizar == 1) {
	...
	if(strlen($_GET["classificacao"]) > 0) $classificacao = trim($_GET["classificacao"]);
	...
	$multiplo           = trim($_GET['radio_qtde_produtos']);
...
<?if (strlen($btn_finalizar) > 0 and strlen($msg_erro) == 0) {
...
if ($listar == "ok") {
	...
	if (strlen($aux_data_inicial)>0 AND strlen($aux_data_final)>0){
		$cond_9 = "AND   bi_os.$tipo_data BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59'";
		$cond_99 = "AND   tbl_os.$tipo_data BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59'";
		if ($tipo_data == 'data_finalizada'){
			$cond_99 = "AND   tbl_os.finalizada BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59'";
		}
		if ($login_fabrica == 5 AND $tipo_data == 'data_pedido') {
			$new_data_inicial = (substr($aux_data_inicial,0,4) - 1) . substr($aux_data_inicial,4,6);
			$cond_9  = " AND 1 = 1";
			$cond_99 = " AND tbl_pedido.data BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59'";
		}
	}
	...
	if ($login_fabrica == 5 AND $tipo_data == 'extrato_geracao'){
			if ($tipo_data == 'extrato_geracao'){
				$cond_99 = "AND   tbl_extrato.data_geracao BETWEEN '$aux_data_inicial' AND '$aux_data_final'";
			}

			$sql = "select DISTINCT tbl_extrato.extrato, tbl_os_extra.os
					INTO TEMP tmp_extrato_x
					from tbl_extrato
					JOIN tbl_os_extra ON tbl_os_extra.extrato = tbl_extrato.extrato
					where tbl_extrato.fabrica = $login_fabrica 
					$cond_99;

					CREATE INDEX tmp_extrato_x_os ON tmp_extrato_x(os);
					...
					'' as pedido,
					'' as data_pedido,
					tmp_extrato_x.extrato
					INTO TEMP tmp_os_$login_admin
					FROM tbl_os
					JOIN tmp_extrato_x on tmp_extrato_x.os = tbl_os.os
					JOIN tbl_posto USING(posto)
					JOIN tbl_Posto_fabrica on tbl_posto_fabrica.posto = tbl_posto.posto and tbl_Posto_fabrica.fabrica = $login_fabrica
					JOIN tbl_produto USING(produto)
					WHERE tbl_os.fabrica = $login_fabrica
	 ...
	 if ($login_fabrica == 5 AND $tipo_data == 'data_pedido') {
		 	$sql = "select DISTINCT tbl_os_produto.os,tbl_os_item.pedido, tbl_pedido.data,
					tbl_os_item.peca, tbl_os_item.defeito, tbl_os_item.servico_realizado
					INTO TEMP tmp_pedido_x
					from tbl_pedido
					...
					CREATE INDEX tmp_pedido_x_os ON tmp_pedido_x(os);
					...
					tbl_os.solucao_os,
					'' as extrato,
					tmp_pedido_x.pedido as pedido,
					TO_CHAR (tmp_pedido_x.data, 'DD/MM/YYYY')  AS data_pedido
					INTO TEMP tmp_os_$login_admin
					FROM tbl_os
					JOIN tmp_pedido_x on tmp_pedido_x.os = tbl_os.os
					JOIN tbl_posto USING(posto)
					JOIN tbl_Posto_fabrica on tbl_posto_fabrica.posto = tbl_posto.posto and tbl_Posto_fabrica.fabrica = $login_fabrica
					JOIN tbl_produto USING(produto)
					WHERE tbl_os.fabrica = $login_fabrica
	...
	$sql .= "CREATE INDEX tmp_os_os_$login_admin ON tmp_os_$login_admin(os);
					";

	$sql .= "select distinct tmp_os_$login_admin.produto,
					tmp_os_$login_admin.ativo,
					tmp_os_$login_admin.referencia,
					tmp_os_$login_admin.descricao,
					...
					tmp_os_$login_admin.data_nf, ";

    if ($login_fabrica == 5) {
	   $sql .= "    TO_CHAR (bi_os.data_digitacao, 'DD/MM/YYYY')  AS data_digitacao,
					TO_CHAR (bi_os.data_finalizada, 'DD/MM/YYYY') AS data_finalizada,
					tmp_os_$login_admin.data_conserto,
					TO_CHAR((SELECT data FROM tbl_pedido WHERE pedido=bi_os_item.pedido), 'DD/MM/YYYY') AS data_pedido,
					bi_os_item.data_pedido,
					bi_os_item.pedido, 
					bi_os.extrato,";
	}

	$sql .= "		tmp_os_$login_admin.serie,
					...
					tmp_os_$login_admin.nota_fiscal,
					tmp_os_$login_admin.codigo_posto,
					tmp_os_$login_admin.nome,
					tmp_os_$login_admin.estado,
					tmp_os_$login_admin.cnpj::text,
				...
				left join tbl_solucao on tbl_solucao.solucao = tmp_os_$login_admin.solucao_os and tbl_solucao.fabrica = $login_fabrica
    ...
    if ($login_fabrica == 5 AND ($tipo_data == 'extrato_geracao' OR $tipo_data == 'extrato_aprovacao' OR $tipo_data == 'extrato_exportacao')) {
	   $sql .= " join tbl_os_extra on tbl_os_extra.os     = bi_os.os ";
    }
	...
	if (pg_numrows($res) > 0) {
		...
		#echo "<br /><b style='font-size:10px; color:red'> Se o resultado da pesquisa for menor de 5.000 registros mostra-se na tela e gera arquivo, caso contrário somente gera-se o arquivo <br></b>";
		...
		if ($login_fabrica == 5){//HD 179811
			$conteudo .= "<Th>Cód. Componente</Th>";
			$conteudo .= "<Th>Componente</Th>";
...
#		}
		...
		if ($formato_arquivo=='CSV'){
			...
			if ($login_fabrica == 5){//HD 179811
				$conteudo .= "CÓD. COMPONENTE;COMPONENTE;DEFEITO;SERVIÇO;";
			}
		...
		for ($i = 0; $i < pg_numrows($res); $i++) {
			...
			$sua_os      = str_replace(';','',trim(pg_result($res,$i,sua_os)));
			...
			if ($login_fabrica == 5) {//HD 179811
				...
				$sql = 'SELECT tbl_peca.referencia as cod_componente,
                               tbl_peca.descricao as componente,
                         ...
                         WHERE bi_os_item.os = ' . $ocorrencia;
				...
				while ($row = pg_fetch_assoc($resul)) {
					$vet['cod_componente'][$y] = trim($row['cod_componente']);
					$vet['componente'][$y]     = trim($row['componente']);
					$vet['defeito'][$y]        = trim($row['defeito']);
					$vet['servico'][$y]        = trim($row['servico']);
					$y++;
				}

				if ($qtde_pecas == 0) {
					 
					$conteudo .= "<TD align='center' nowrap>Não Consta</TD>";
					$conteudo .= "<TD align='center' nowrap>Não Consta</TD>";
					$conteudo .= "<TD align='center' nowrap>Não Consta</TD>";
					$conteudo .= "<TD align='center' nowrap>Não Consta</TD>";

					$str_componente = 'Não Consta;Não Consta;Não Consta;Não Consta;'.$mao_de_obra."\n";

				} else if ($y == 1) {
					
					$conteudo .= "<TD align='center' nowrap>".$vet['cod_componente'][0]."</TD>";
					$conteudo .= "<TD align='center' nowrap>".$vet['componente'][0]."</TD>";
					$conteudo .= "<TD align='center' nowrap>".$vet['defeito'][0]."</TD>";
					$conteudo .= "<TD align='center' nowrap>".$vet['servico'][0]."</TD>";
					
					$str_componente = $vet['cod_componente'][0].';'.$vet['componente'][0].';'.$vet['defeito'][0].';'.$vet['servico'][0].';'.$mao_de_obra."\n";

				} else {
					
					$conteudo .= "<TD align='center' nowrap>";
					for ($w = 0 ; $w < count($vet['cod_componente']); $w++) {
						$conteudo .= $vet['cod_componente'][$w] . " <br />";//27 ponto e virgulas
						$str_componente .= $w == 0 ? $vet['cod_componente'][$w].';'.$vet['componente'][$w].';'.$vet['defeito'][$w].';'.$vet['servico'][$w] : ";;;;;;;;;;;;;;;;;;;;;;;;;;;" . $vet['cod_componente'][$w].';'.$vet['componente'][$w].';'.$vet['defeito'][$w].';'.$vet['servico'][$w].";\n";
						$str_componente .= $w == 0 ? ";".$mao_de_obra."\n" : '';
					}
					$conteudo .= "</TD>";
					
					$conteudo .= "<TD align='center' nowrap>";
					for ($w = 0 ; $w < count($vet['componente']); $w++) {
						$conteudo .= $vet['componente'][$w] . " <br />";
					}
					$conteudo .= "</TD>";

					$conteudo .= "<TD align='center' nowrap>";
					for ($w = 0 ; $w < count($vet['defeito']); $w++) {
						$conteudo .= $vet['defeito'][$w] . " <br />";
					}
					$conteudo .= "</TD>";

					$conteudo .= "<TD align='center' nowrap>";
					for ($w = 0 ; $w < count($vet['servico']); $w++) {
						$conteudo .= $vet['servico'][$w] . " <br />";
					}
					$conteudo .= "</TD>";
