...
$sql = "SELECT 
   ...
   SUM(BI.mao_de_obra) AS mao_de_obra,
   SUM(BI.qtde_pecas) AS qtde_pecas
INTO TEMP tmp_bi_fcr_os
FROM bi_os BI
JOIN tbl_produto PR ON PR.produto = BI.produto
JOIN tbl_linha LI ON LI.linha = BI.linha
JOIN tbl_familia FA ON FA.familia = BI.familia
WHERE BI.fabrica = 1
AND BI.pais = 'BR' AND BI.data_digitacao BETWEEN '2007-11-01' AND '2008-10-31'
AND BI.estado IN ('PR', 'SC', 'RS')
/*AND BI.linha = 198*/
GROUP BY 
   PR.produto ,
   PR.ativo ,
...
ALTER TABLE tmp_bi_fcr_os ADD column total_pecas double precision;

UPDATE tmp_bi_fcr_os SET total_pecas = x.total_pecas
FROM
	(SELECT 
		PR.produto ,
		PR.ativo ,
		...
		FA.descricao AS f_nome ,
		LI.nome AS l_nome ,
		sum(bi_os_item.qtde * bi_os_item.custo_peca) AS total_pecas
	FROM bi_os BI
	JOIN tbl_produto PR ON PR.produto = BI.produto
	JOIN tbl_linha LI ON LI.linha = BI.linha
	JOIN tbl_familia FA ON FA.familia = BI.familia
	...
	GROUP BY 
		...
		f_nome ,
		l_nome) as x
	WHERE x.produto = tmp_bi_fcr_os.produto
	and x.ativo = tmp_bi_fcr_os.ativo
	and x.referencia = tmp_bi_fcr_os.referencia
	and x.descricao = tmp_bi_fcr_os.descricao
	and x.f_nome = tmp_bi_fcr_os.f_nome
	and x.l_nome = tmp_bi_fcr_os.l_nome;

SELECT * FROM tmp_bi_fcr_os order by ocorrencia desc;";
...
$res = pg_exec ($con,$sql);
	...
	if (pg_numrows($res) > 0) {
		...
		$conteudo .="<Th width='50' height='15'>Total Peças</Th>";
		...
		for ($i=0; $i<pg_numrows($res); $i++){
			...
			$conteudo .="<TD align='center' nowrap>$total_pecas</TD>";
		...
		$total_pecasx = number_format($total_pecasx,2,",",".");
		$conteudo .="</tbody>";

		$conteudo .= "<tfoot>";
		$conteudo .= "<tr class='table_line'><td colspan='";if($login_fabrica==3 OR $login_fabrica==5) $conteudo .= "5";else $conteudo .= "4";$conteudo .="><font size='2'><b><CENTER>TOTAL</b></td>";
		$conteudo .="<td colspan='2'><font size='2' color='009900'><b>$total</b></td>";
		$conteudo .="<td><font size='2' color='009900'><b>$total_peca</b></td>";
		$conteudo .="<td><font size='2' color='009900'><b>$total_pecasx</b></td>";
