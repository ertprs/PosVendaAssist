...
/*	if(strlen($peca)>0){
	...
	$sql = "SELECT  count(distinct BI.os)        AS os,
		...
		GROUP BY    dc_codigo   ,
					dc_descricao,mes;
					select os,custo_peca,dc_codigo,dc_descricao,mes from tmp_defeito_reclamado_$login_fabrica";
	...
	if ($login_fabrica==11){
		if (pg_numrows($res) > 0) {
			...
			for ($i=0; $i<pg_numrows($res); $i++){
				$dc_codigo      = pg_result($res,$i,dc_codigo);
				...
				$meses = array(1 => "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro");
				$mes = $meses[$mes];
				$fechar="<center><A HREF=\"javascript:camada('$dc_descricao_antiga');\">fechar</a></center>";
				if ($dc_descricao_antiga==""){
					$cabecalho = "<DIV ID=\"$dc_descricao\" STYLE='POSITION: absolute; BORDER: 15px solid #FFFFFF; BACKGROUND: #FFFFFF; margin:0px; display:none'><table style='font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 11px;'><tr align='center'><td width='300' style='background-color:#D9E2EF;'><b>Defeito Constatado</b></td><td width='60' style='background-color:#D9E2EF;'><b>$mes</b></td>";
					$linha = "<tr><td>$dc_descricao</td><td align='center'>$os</td>";
				}else{
					if ($dc_descricao_antiga==$dc_descricao and $dc_descricao_antiga<>""){
						$cabecalho = $cabecalho."<td width=60 style='background-color:#D9E2EF;'><b>$mes</b></td>";
						$linha = $linha."<td align='center'>$os</td>";
					}else{
						$div='div'.str_replace('.', '',str_replace(' ', '', $dc_descricao_antiga)); 
						$$div= $cabecalho."</tr>".$linha."</tr></table>".$fechar."</div>";
						$cabecalho ="<DIV ID=\"$dc_descricao\" STYLE='POSITION: absolute; BORDER: 15px solid #FFFFFF; BACKGROUND: #FFFFFF;  margin:0px; display:none'><table style='font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 11px;'><tr align='center'><td width='300' style='background-color:#D9E2EF;'><b>Defeito Constatado</b></td><td width='60' style='background-color:#D9E2EF;'><b>$mes</b></td>";
						$linha = "<tr><td>$dc_descricao</td><td align='center'>$os</td>";
					}
				}
				$dc_descricao_antiga=$dc_descricao;
			}
			$fechar="<center><A HREF=\"javascript:camada('$dc_descricao');\">fechar</a></center>";
			$div='div'.str_replace('.', '',str_replace(' ', '', $dc_descricao)); $$div=$cabecalho."</tr>".$linha."</tr></table>".$fechar."</div>";
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i=0; $i<pg_numrows($res2); $i++){
			...
			if ($login_fabrica==11){
				$div_e='div'.str_replace('.', '',str_replace(' ', '', $de_descricao));
				$div_ex=$$div_e;
			}
			echo "<TR>";
			echo "<TD align='left' nowrap>";
			if ($login_fabrica==11){
				echo "<A HREF=\"javascript:camada('$de_descricao');\">$de_codigo - $de_descricao</a><br>$div_ex</TD>";
			}else{
	...
	if (pg_numrows($res) > 0) {
		...
		if ($login_fabrica==11){
			for ($i=0; $i<pg_numrows($res); $i++){
				...
				$meses = array(1 => "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro");
				$mes = $meses[$mes];
				$fechar="<center><A HREF=\"javascript:camada('$referencia_antiga');\">fechar</a></center>";
				if ($referencia_antiga==""){
					$cabecalho = "<DIV ID=\"$referencia\" STYLE='POSITION: absolute; BORDER: 15px solid #FFFFFF; BACKGROUND: #FFFFFF;  margin:0px; display:none'><table style='font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 11px;'><tr align='center'><td width=300 style='background-color:#D9E2EF;'><b>Produto</b></td><td width=60 style='background-color:#D9E2EF;'><b>$mes</b></td>";
					$linha = "<tr><td>$referencia - $descricao</td><td align='center'>$os</td>";
				}else{
					if ($referencia_antiga==$referencia and $referencia_antiga<>""){
						$cabecalho = $cabecalho."<td width=60 style='background-color:#D9E2EF;'><b>$mes</b></td>";
						$linha = $linha."<td align='center'>$os</td>";
					}else{
					$div='div'.str_replace('.', '',str_replace(' ', '', $referencia_antiga)); 
					$$div=  $cabecalho."</tr></thead>".$linha."</tr></table>".$fechar."</div>";
					$cabecalho ="<DIV ID=\"$referencia\" STYLE='POSITION: absolute; BORDER: 15px solid #FFFFFF; BACKGROUND: #FFFFFF;  margin:0px; display:none'><table style='font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 11px;'><tr align='center'><td width=300 style='background-color:#D9E2EF;'><b>Produto</b></td><td width=60 style='background-color:#D9E2EF;'><b>$mes</b></td>";
					$linha = "<tr><td>$referencia - $descricao</td><td align='center'>$os</td>";
					}
				}
			...
			$fechar="<center><A HREF=\"javascript:camada('$referencia');\">fechar</a></center>";
			$div='div'.str_replace('.', '',str_replace(' ', '', $referencia)); 
			$$div= $cabecalho."</tr></thead>".$linha."</tr></table>".$fechar."</div>";
		}
		$sql2 = "SELECT SUM(os)                 AS soma_os,
						SUM(custo_peca)         AS soma_custo_peca,
		...
		for ($i=0; $i<pg_numrows($res2); $i++){
			...
			echo "<TD align='left' nowrap>";
			if ($login_fabrica==11){
				echo "<A HREF=\"javascript:camada('$referencia');\">$referencia - $descricao</a><br>$div_ex</TD>";
			}else{
