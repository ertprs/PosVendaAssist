...
if ($listar == "ok") {
    ...
    if ($login_fabrica == 5) {
	   $sql .= "join tbl_pedido   on tbl_pedido.pedido   = bi_os_item.pedido
...
//echo nl2br($sql);exit;
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i=0; $i<pg_numrows($res); $i++) {
			...
			$uf_posto        = trim(pg_result($res,$i,estado));
			$dr_descricao    = trim(pg_result($res,$i,dr_descricao));
			$dc_descricao    = trim(pg_result($res,$i,dc_descricao));
			$dc_solucao      = trim(pg_result($res,$i,dc_solucao));
			$desc_servico    = trim(pg_result($res,$i,descricao_servico));
			...
			$cnpj_posto      = trim(pg_result($res,$i,cnpj));
			$ocorrencia      = trim(pg_result($res,$i,os));
			$mao_de_obra     = trim(pg_result($res,$i,mao_de_obra));
