...
<TABLE width="700" align="center" border="0" cellspacing="0" cellpadding="2" class="formulario" id='Formulario'>
	...
	<TR>
		...
		<TD colspan='2'>
			<table>
				<tr>
					<td><input type='radio' name='tipo_data' value='data_digitacao' <?if($tipo_data=="data_digitacao") echo "CHECKED";?>> Digitação</td>
					<td><input type='radio' name='tipo_data' value='data_abertura' <?if($tipo_data=="data_abertura") echo "CHECKED";?>> Abertura</td>
					<td><input type='radio' name='tipo_data' value='data_fechamento' <?if($tipo_data=="data_fechamento") echo "CHECKED";?>> Fechamento</td>
				</tr>

					<tr>
					<td><input type='radio' name='tipo_data' value='data_finalizada'<?if($tipo_data=="data_finalizada") echo "CHECKED";?>> Finalizada</td>
					<td><input type='radio' name='tipo_data' value='extrato_geracao'<?if($tipo_data=="extrato_geracao") echo "CHECKED";?>> Geração de Extrato</td>
					<td><input type='radio' name='tipo_data' value='extrato_aprovacao'<?if($tipo_data=="extrato_aprovacao") echo "CHECKED";?>> Aprovação do Extrato
					<?if($login_fabrica==20){?>
	...
	<tr><td colspan="4">&nbsp;</td></tr>
		...
		<TR>
			<td width="70">&nbsp;</td>
			<Td colspan="3">SELECIONE &nbsp;&nbsp;&nbsp; Um produto
...
<? if ($login_fabrica == 7) { ?>
	...
	<TR>
		<td width="70">&nbsp;</td>
		<TD colspan="3">Classificação de OS&nbsp;&nbsp;
			<?
			$sql = "SELECT  *
...
<? }?>
	...
	<tr>
		<td width="70">&nbsp;</td>
		<TD align='left' colspan="3">Tipo Arquivo para Download</TD>
	</tr>
	<tr>
		<td width="70">&nbsp;</td>
		<TD colspan="3">
	...
	<tr>
		...
		<td colspan='3' align='left'>
			<input type='checkbox' name='mostra_peca' value='mostra_peca' <? if($login_fabrica == 30){ echo "checked";}?>> Mostrar custo de peça mesmo se pedido estiver pendente!
		</td>
	</TR>
...
if ($listar == "ok") {
	...
	if (pg_numrows($res) > 0) {
		...
		$conteudo .="<TABLE width='700' border='0' cellspacing='2' cellpadding='2' align='center'  name='relatorio' id='relatorio' class='tablesorter'>";
		$conteudo .="<thead>";
		$conteudo .="<TR>";
