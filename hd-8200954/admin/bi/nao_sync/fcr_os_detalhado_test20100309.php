...
if ($listar == "ok") {
	...
	if (strlen($aux_data_inicial)>0 AND strlen($aux_data_final)>0){
		echo $cond_9 = "AND   bi_os.$tipo_data BETWEEN '$aux_data_inicial' AND '$aux_data_final'";
	}
	...
	$sql = "select tbl_produto.produto,
				...
				from bi_os
					...
					tbl_Posto_fabrica.fabrica=5
					left join bi_os_item on bi_os_item.os=bi_os.os
					left join tbl_servico_realizado SR on SR.servico_realizado= bi_os_item.servico_realizado and
					SR.fabrica=5
					left join tbl_defeito_constatado DC on DC.defeito_constatado= bi_os.defeito_constatado and
					DC.fabrica=5
					left join tbl_defeito_reclamado DR on DR.defeito_reclamado= bi_os.defeito_reclamado and
					DR.fabrica=5
					join tbl_produto on tbl_produto.produto = bi_os.produto
					join tbl_linha on tbl_linha.linha = bi_os.linha and
					tbl_linha.fabrica=5
					join tbl_familia on tbl_familia.familia = bi_os.familia and
					tbl_familia.fabrica=5
		WHERE bi_os.fabrica = 5
...
echo nl2br($sql);exit;
	...
	if (pg_numrows($res) > 0) {
		...
		if ($formato_arquivo=='CSV'){
			$conteudo = "";
			$conteudo .= "OS;CONSUMIDOR;DT ABERTURA OS;DT FECHAMENTO OS;REFERÊNCIA;PRODUTO;SÉRIE;N. NF;DATA NF;REVENDA;CÓD POSTO;POSTO;CNPJ;UF POSTO;DEFEITO RECLAMADO;DEFEITO CONSTATADO;SERVIÇO;LINHA;FAMÍLIA;QTDE. PECAS;M.O \n";
		}
		fputs ($fp,$conteudo);
		...
		for ($i=0; $i<pg_numrows($res); $i++){
			...
			if ($formato_arquivo=='CSV'){
				$conteudo = "";
				$conteudo .= $ocorrencia.";".$consumidor.";".$data_abertura.";".$data_fechamento.";".$referencia.";".$descricao.";".$serie.";".$nota_fiscal.";".$data_nf.";".$revenda_nome.";".$codigo_posto.";".$nome_posto.";'$cnpj_posto' ;".$uf_posto.";".$dr_descricao.";".$dc_descricao.";".$desc_servico.";".$$linha_nome.";".$familia_nome.";".$qtde_pecas.";".$mao_de_obra.";\n";
			}
			fputs ($fp,$conteudo);
		}
		$conteudo = "";
