...
if ($listar == "ok") {
	...
	$sql_base = "SELECT  BI.os           AS ocorrencia 
		...
		if(pg_numrows($res_base)>0){
				...
				$arquivo_nome     = "relatorio_detalhado_os-$login_fabrica-$ano-$mes-$data.xls";
...
#echo nl2br($sql); exit;
	...
	if (pg_numrows($res) > 0) {
		...
		echo "<br><p id='id_download2' style='display:none'><a href='../xls/$arquivo_nome.zip'><img src='/assist/imagens/excel.gif'><br><font color='#3300CC'>Fazer download do relatório detalhado</font></a></p><br>";
		echo "<p id='id_download' style='display:none'><a href='../xls/$arquivo_nome' target='_blank'><img src='/assist/imagens/excel.gif'><br><font color='#3300CC'>Fazer download do relatório resumido em  ".strtoupper($formato_arquivo)."</font></a></p>";
