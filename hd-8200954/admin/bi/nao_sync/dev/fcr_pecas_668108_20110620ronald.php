...
<? }?>
	...
	<TR>
		...
		<TD colspan="2" align="center"><input type="button" style="cursor:pointer;" value=" Pesquisar " onclick="javascript: if ( document.frm_pesquisa.btn_finalizar.value == '0' ) { document.frm_pesquisa.btn_finalizar.value='1'; document.frm_pesquisa.submit() ; } else { alert ('Aguarde submissão da OS...'); }" alt='Clique AQUI para pesquisar'>
		</TD>
	</TR>
	<tr><td colspan="2">&nbsp;</td></tr>
	</TFOOT>
...
if ($listar == "ok") {
	...
	if (pg_numrows($res) > 0) {
		...
		if ($login_fabrica == 7 and strlen($classificacao)>0) {
			$conteudo .="<td style='background-color:#596d9b;text-align:center;padding-right: 15px'>Classificação</th>";
		}
