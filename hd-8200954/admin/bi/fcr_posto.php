...
function AbrePeca(codigo_posto,data_inicial,data_final,linha,estado,posto,pais,marca,tipo_data,aux_data_inicial,aux_data_final){
	janela = window.open("fcr_posto_item.php?codigo_posto=" + codigo_posto + "&data_inicial=" + data_inicial + "&data_final=" + data_final + "&linha=" + linha + "&estado=" + estado +"&posto=" + posto +"&pais=" + pais +"&marca=" + marca + "&tipo_data=" + tipo_data +"&aux_data_inicial="+aux_data_inicial+"&aux_data_final="+aux_data_final,"peca",'resizable=1,scrollbars=yes,width=750,height=550,top=0,left=0');
	janela.focus();
}
...
if ($listar == "ok") {
	...
	$sql = "
		SELECT  posto_codigo,
				...
				sum(total_31) AS total_31,
				posto
		FROM (
			(
				SELECT
					count(os)                             AS total_15    ,
					0                                     AS total_30    ,
					0                                     AS total_31    ,
				...
				AND   BI.data_fechamento - BI.data_abertura < 15
				 $cond_1 $cond_2 $cond_3 $cond_4 $cond_5 $cond_6 $cond_7 $cond_8 $cond_9
				GROUP BY posto_codigo,posto_nome,PO.posto
			)UNION(
				SELECT
					0                                     AS total_15    ,
					count(os)                             AS total_30    ,
					0                                     AS total_31    ,
					PF.codigo_posto                       AS posto_codigo,
					PO.nome                               AS posto_nome  ,
					PO.posto
				...
				WHERE BI.fabrica = $login_fabrica
				AND   BI.data_fechamento - BI.data_abertura > 15
				AND   BI.data_fechamento - BI.data_abertura < 30
				 $cond_1 $cond_2 $cond_3 $cond_4 $cond_5 $cond_6 $cond_7 $cond_8 $cond_9
				GROUP BY posto_codigo,posto_nome,PO.posto
			)UNION(
				SELECT
		...
		GROUP BY posto_codigo,posto_nome,posto
		ORDER BY posto_nome;";
...
//	echo $sql;exit;
	...
	if (pg_numrows($res) > 0) {
		...
		$arquivo_nome     = "bi-posto-$login_fabrica.$login_admin.".$formato_arquivo;
		...
		$conteudo .="<thead>";
		$conteudo .="<TR>";
		$conteudo .="<Th width='100' height='15'>Código</TD>";
		$conteudo .="<Th height='15'>Posto</TD>";
		$conteudo .="<Th width='50' height='15'>até 15 dias</Th>";
		$conteudo .="<Th width='50' height='15'>entre 15 e 30 dias</Th>";
		$conteudo .="<Th width='50' height='15'>acima de 30 dias</Th>";
		$conteudo .="<Th width='50' height='15'>Total</Th>";
		$conteudo .="</TR>";
		$conteudo .="</thead>";
		$conteudo .="<tbody>";
...
//		}
		for ($i=0; $i<pg_numrows($res); $i++){
			...
			$total_15     = trim(pg_result($res,$i,total_15));
			$total_30     = trim(pg_result($res,$i,total_30));
			$total_31     = trim(pg_result($res,$i,total_31));
			...
			$subtotal= $total_15 + $total_30 + $total_31;
...
//			$valor_peca  = number_format($valor_peca,2,",",".");
			...
			$conteudo .="<a href='javascript:AbrePeca(\"$posto_codigo\",\"$data_inicial\",\"$data_final\",\"$linha\",\"$estado\",\"$posto\",\"$pais\",\"$marca\",\"$tipo_data\",\"$aux_data_inicial\",\"$aux_data_final\");'>";
			$conteudo .="$posto_codigo</TD>";
			$conteudo .="<TD align='left' nowrap>$posto_nome</TD>";
			$conteudo .="<TD align='center' nowrap>$total_15</TD>";
			$conteudo .="<TD align='center' nowrap title=''>$total_30</TD>";
			$conteudo .="<TD align='center' nowrap>$total_31</TD>";
			$conteudo .="<TD align='center' nowrap>$subtotal</TD>";
			$conteudo .="</TR>";
		}
		$total       = $gtotal_15+$gtotal_30+$gtotal_31;
		$conteudo .="</tbody>";

		$conteudo .= "<tfoot>";
		$conteudo .= "<tr class='table_line'><td colspan='2'><font size='2'><b><CENTER>TOTAL</b></td>";
		$conteudo .="<td align='center'><font size='2' color='009900'><b>$gtotal_15</b></td>";
		$conteudo .="<td align='center'><font size='2' color='009900'><b>$gtotal_30</b></td>";
		$conteudo .="<td align='center'><font size='2' color='009900'><b>$gtotal_31</b></td>";
		$conteudo .="<td align='center'><font size='2' color='009900'><b>$total</b></td>";
		$conteudo .="</tr>";
		$conteudo .= "</tfoot>";
		$conteudo .=" </TABLE></div>";
