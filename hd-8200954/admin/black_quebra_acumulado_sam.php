...
<center>
<font face='arial' color='<? echo $cor_forte ?>'><b>Apenas das OS aprovadas</b></font>
<center>
...
if ($relatorio == "gerar" ) {
	...
	$sql = "SELECT  tbl_produto.referencia_fabrica  AS referencia ,
					tbl_produto.produto             ,
			...
			) fcr ON tbl_produto.produto = fcr.produto 
			UNION
...
*/
	...
	if (pg_numrows($res) > 0) {
		...
		for ($x = 0; $x < pg_numrows($res); $x++) {
			...
			$soma_mobra = pg_result($res,$x,mao_de_obra);
			$soma_peca  = pg_result($res,$x,pecas);
			...
			echo "<a href='black_quebra_acumulado_pecas.php?produto=$produto&referencia=$referencia&voltagem=$voltagem&data_inicial=$x_data_inicial&data_final=$x_data_final&linha=$linha&estado=$estado' target='_blank'>";
			...
			echo "<font face='Verdana, Arial, Helvetica, sans' color='#000000' size='2'>";
			echo "<a href='black_quebra_acumulado_os.php?produto=$produto&referencia=$referencia&voltagem=$voltagem&data_inicial=$x_data_inicial&data_final=$x_data_final&linha=$linha&estado=$estado' target='_blank'>";
			echo $ocorrencia;
			echo "</a>";
			echo "</font>";
			echo "</td>";
