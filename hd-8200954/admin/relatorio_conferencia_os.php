...
if ($periodo) {
	...
	if ($periodo < 2000 || $periodo > 3000) {
		$msg_erro = "Ano informado é inválido";
	}
}
...
if(strlen($posto)) {
	...
	elseif ($acao == "PESQUISAR") {
		header("location:os_auditar_posto.php?posto=$posto&status=aprovacao");
		die;
	}
}
...
<? } ?>
	...
	font-size: 10px;'>
		...
		<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
			<td width="10">&nbsp;</td>
			<td colspan='2'>Ano (data da digitação)</td>
			<td width="10">&nbsp;</td>
		</tr>
		<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
			<td width="10">&nbsp;</td>
			<td colspan='2'>
				...
				<select id='periodo' name='periodo' class='frm'>
					...
					for($a = $ano_inicial; $a <= intval(date("Y")); $a++) {
						if ($a == intval($periodo)) {
							$selected = "selected";
						}
						else {
...
if ($acao == "PESQUISAR" && strlen($msg_erro) == 0) {
	...
	echo "
	<table id='tabela_principal' style='$estilo_principal'>
	$cabecalho
	<tbody>";
