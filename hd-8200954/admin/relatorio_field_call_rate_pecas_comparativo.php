...
<TABLE WIDTH = '600' align = 'center' class='Conteudo'>
	...
	<TR>
		...
		echo "<tr>";
		echo "<td colspan='3' class='Titulo' nowrap>PROCESSANDO OS SEGUINTES MESES</td>";
		echo "</tr>";
		echo "<tr>";
		for($x=0;$x<3;$x++){
			$fator = 2 -$x;
...
for($x=0;$x<3;$x++){
	...
	if($x==0){
		...
		$sql = "INSERT INTO temp_fcr_pecas2_osex_$login_admin (os,data_analise)
			...
			INSERT  INTO temp_fcr_pecas2_os_$login_admin (os,data_analise)
			SELECT tbl_os.os , data_analise
			FROM tbl_os 
			JOIN tbl_posto ON tbl_os.posto = tbl_posto.posto
			JOIN tbl_produto ON tbl_os.produto = tbl_produto.produto
...
if (pg_numrows($res) > 0) {
	...
	for ($i=0; $i<pg_numrows($res); $i++){
		...
		if($peca_anterior <> $peca){
			...
			$qtde_mes[$peca_total][3] = $descricao;
			$x=0;
			$peca_anterior = $peca;
	...
	for ($i=0; $i<pg_numrows($res); $i++){
		...
		//IMPRIME O VETOR COM OS DOZE MESES E LOGO APÓS PULA LINHA E ESCREVE O NOVO familia
			if($i<>0 AND $peca_anterior<>$peca ){
				...
				for($a=0;$a<3;$a++){//imprime os doze meses
...
if(pg_numrows($res) > 0){
	...
	echo "<TD class='Titulo'>PEÇA</TD>";
	echo "<TD class='Titulo'>OCORRÊNCIAS</TD>";
	...
	for($i=0; $i<pg_numrows($res); $i++){
		...
		$data_analise = pg_result($res,$i,data_analise);
		if($login_fabrica == 20 and $pais !='BR'){
			$descricao  = pg_result($res,$i,descricao_espanhol);		
			if(strlen($descricao) == 0){
				$descricao = "<font color = 'red'>Tradução não cadastrada</font>";
		...
		echo "<TR class='$cor'>";
		echo "<TD class='Conteudo' align='center'>$referencia</TD>";
		echo "<TD class='Conteudo' align='left'>$descricao</TD>";
		echo "<TD class='Conteudo' align='center'>$ocorrencia $data_analise</TD>";
		echo "<TD class='Conteudo' align='center'>". number_format($porcentagem,2,",",".") ."%</TD>";
		echo "</TR>";
	}
	echo "<TR >";
