...
$fab_usa_ped_item_preco = array(1,3,24,74);
...
if (strlen ($pedido) > 0) {
	...
	if ($login_fabrica <> 14) {
		if(in_array($login_fabrica,$fab_usa_ped_item_preco)){
	...
	for ($i = 0 ; $i < pg_num_rows($resI) ; $i++) {
		if ($login_fabrica <> 14) {
			...
			$preco       = trim(pg_fetch_result ($resI,$i,preco));
			$preco_total = trim(pg_fetch_result ($resI,$i,total));
			$peca        = trim(pg_fetch_result ($resI,$i,peca));
			$total += $preco_total;
		}
	}
}
...
>
	<tr>
		...
		if ($login_fabrica == 1) {
			$pedido_blackedecker = "00000".pg_fetch_result($res,0,pedido_blackedecker);
			...
			echo ($login_fabrica == 88) ? $seu_pedido : pg_fetch_result($res,0,pedido);
		}
		?>
		</b></td>
	...
	<tr>
		...
		<td class="conteudo" colspan="3" NOWRAP bgcolor='#eeeeee'>&nbsp;<U><B><? echo pg_fetch_result ($res,0,codigo_posto)."-".pg_fetch_result ($res,0,nome); ?></B></U></td>
	...
	<tr>
		...
		<td class="conteudo">&nbsp;<? echo pg_fetch_result ($res,0,tipo_frete)?></td>
	</tr>
	<tr>
...
<TR>
	...
	<?
		if($login_fabrica == 86)
		{
			echo "<td class='menu_top'>Rastreio Transporte</td>";
			echo "<td class='menu_top'>Envio Frete</td>";
		}
	?>
...
if ($login_fabrica == 14) {
	...
	if (pg_num_rows($resz) > 0) $total  = trim(pg_fetch_result ($resz,0,total_pedido));
}
...
for ($i = 0 ; $i < pg_num_rows ($resI) ; $i++) {
	...
	$peca                        = trim(pg_fetch_result ($resI,$i,peca));
	$pedido                        = trim(pg_fetch_result ($resI,$i,pedido));
			
	$peso_estimado               = $peso_estimado + $peso;
	$obs_pedido_item=pg_fetch_result($resI,$i,obs);
	if ($login_fabrica <> 14) {
		...
		$preco_unitario_item         = pg_fetch_result ($resI,$i,total);
		$preco_unitario_item_sem_ipi = pg_fetch_result ($resI,$i,preco);
	...
	if($login_fabrica == 86){
		...
		if (pg_num_rows($res)>0){
			$rastreio = pg_fetch_result($res,0,rastreio);
			$envio_frete = pg_fetch_result($res,0,envio_frete);
		} else {
			$rastreio = "";
			$envio_frete = "";
		}
	...
	if($login_fabrica==7){
		if(strlen($pedido)>0 AND strlen($peca)>0){
			...
			if (@pg_num_rows($resx)>0){
				$preco_unitario_item_sem_ipi = pg_fetch_result($resx,0,preco);
				$preco_total_item         = $preco_unitario_item_sem_ipi + ($preco_unitario_item_sem_ipi * $ipi / 100);
...
<TR>
	...
	<TD class="table_line" nowrap><? echo pg_fetch_result ($resI,$i,referencia) ?> <?php echo ($login_fabrica == 171) ? " / ".pg_fetch_result ($resI,$i,referencia_fabrica) : "";?></TD>
	...
	<?
		if($login_fabrica == 86)
		{
			echo "<td class='menu_top'>$rastreio&nbsp;</td>";
			echo "<td class='menu_top'>$envio_frete&nbsp;</td>";
		}
	?>
...
if ($login_fabrica == 50){
	...
	if (pg_numrows($resX) > 0){
		...
		$saida				= pg_fetch_result ($resX,0,saida);
		$transportadora		= pg_fetch_result ($resX,0,nome);
		$transp				= pg_fetch_result ($resX,0,transp);
