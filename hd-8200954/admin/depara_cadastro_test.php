	...
	for ($y = 0 ; $y < pg_numrows($res) ; $y++){
		...
		for($xx=0;$xx<8;$xx++){
			...
			if($xx ==2) {
				...
				$sqlx = "SELECT fn_depara_lbm(de,para,fabrica)
						FROM tbl_depara
						WHERE peca_de = $parar[0]
						AND   peca_para = $parar[2];";
				#echo $sqlx;
			...
			if($xx ==3) {
				...
				print_r($parar);
				echo "<br>";
								$sqlu = "UPDATE tbl_depara SET peca_para = $parar[3], para = tbl_peca.referencia from tbl_peca
				WHERE peca_de = $parar[0] and tbl_peca.peca = $parar[3]; ";
				#echo $sqlu;
				#$resu = pg_query($con,$sqlu);
			...
			if($xx ==4) {
				...
				$sqlu = "UPDATE tbl_depara SET peca_para = $parar[4], para = tbl_peca.referencia from tbl_peca
				WHERE peca_de = $parar[0] and tbl_peca.peca = $parar[4]; ";
				echo $sqlu;
				#$resu = pg_query($con,$sqlu);
			...
			if(!empty($peca_parax)) {
				$sql_para="SELECT peca_para,para FROM tbl_depara
						WHERE  peca_de = $peca_parax ";
