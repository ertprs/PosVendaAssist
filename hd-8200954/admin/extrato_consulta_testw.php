	...
	for ($i=0; $i < $total_postos; $i++) {
		...
		if ($km_pendente) {
			...
			if (strlen($extrato) > 0 AND strlen($msg_erro) == 0) {
				...
				elseif (in_array($login_fabrica,array(6, 7, 11, 30,14, 15, 24, 25, 35, 43, 45, 46, 50, 51, 59, 66, 74, 80,52,42))) {
					//HD 205958: Este conceito está errado, um extrato nunca pode ser aprovado na liberação. Esta linha
					//			 está aqui provisóriamente enquanto arrumamos os conceitos das fábricas
...
/*			LEFT JOIN tbl_os_extra          OE ON OE.extrato    = EX.extrato*/
	...
	if (pg_num_rows ($res) > 0) {
		...
		//	if ($login_fabrica == 14)				
				...
				if (in_array($login_fabrica,array(6,7,14, 15, 11 , 24, 25, 35, 40, 50, 43, 51,46, 47, 74, 59, 30,42)) or $login_fabrica > 51) {
					if ($recalculo_pendente == 't') {
...
//							echo " <input type='checkbox' class='frm' name='liberar_$i' id='liberar' value='$extrato'>";
				...
				echo "</td>\n";
			} elseif (in_array($login_fabrica,array(6,7,14,15,11,24,25,35,40,50,43,51,46,47,74,59,30,45,42)) or $login_fabrica > 51) {//HD 205958: Rotina antiga
				echo "<td align='center' nowrap>";
				if (strlen($liberado) == 0) {
					if($recalculo_pendente == 't'){
						echo "*Aguardando recalculo\n";
		...
		if (in_array($login_fabrica, array(6,7,11,15,20,24,25,30,35,40,50,51,46,47,59,74,52,42))) {
			echo "<td align='center'>";
