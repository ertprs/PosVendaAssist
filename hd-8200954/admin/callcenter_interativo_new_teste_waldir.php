...
if($login_fabrica == 3){
	...
	exit;
}/*else if($login_fabrica == 30){ //hd 311031 - voltando a esmaltec no mesmo arquivo
    if(strlen($callcenter)>0) header ("Location:callcenter_interativo_new_esmaltec.php?callcenter=$callcenter");
    else                      header ('Location:callcenter_interativo_new_esmaltec.php');
 exit;
} */
...
function abrir_anexo(anexo){
	window.open ("callcenter_interativo_anexo.php?anexo="+anexo, "Anexo", "status=no");
	abrir() ;
}
...
<?}?>
<?if($login_fabrica==11 and strlen($callcenter)>0) { ?>
	<tr>
		<td align='right' style='font-size: 14px; font-weight: bold; font-family: arial; color:red'>

			<a href='callcenter_upload_imagens.php?callcenter=<?=$callcenter;?>'>ANEXAR IMAGEM</a>

			<?
			$limite_anexos = 10;

			$caminho = '/www/assist/www/admin/callcenter_digitalizados/';
			...
			while (file_exists($arquivo_destino)){
				echo "  - <span style='font-size:11px;'>
						<a href=\"javascript://\" OnClick=\"abrir_anexo('$arquivo')\">Anexo ".($contador)."</a>
						</span>";
				$arquivo = $callcenter . "-$contador.jpg";
				$arquivo_destino = $caminho.$arquivo;
				if ($contador > $limite_anexos){
					break;
				}
				$contador++;
			}
