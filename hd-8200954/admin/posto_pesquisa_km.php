...
if (in_array($login_fabrica, array(30))) {
	$where_tipo_posto = ',(SELECT tbl_tipo_posto.descricao FROM tbl_tipo_posto WHERE tbl_tipo_posto.tipo_posto = tbl_posto_fabrica.tipo_posto) AS tipo_posto';
}
...
if (pg_numrows($res) == 1) {
	...
	if (in_array($login_fabrica, array(30))) {
		$campo_tipo_posto = "tipo_posto.value='$tipo_posto';";
	}

	echo "contato_endereco.value = '$contato_endereco';contato_numero.value = '$contato_numero';contato_bairro.value = '$contato_bairro';contato_cidade.value = '$contato_cidade';contato_estado.value = '$contato_estado';posto.value='$posto';$campo_tipo_posto if (window.contato_cep) { contato_cep.value = '$contato_cep'; }";
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		if (in_array($login_fabrica, array(30))) {
			...
			$campo_tipo_posto = "tipo_posto.value='$tipo_posto';";
			$status_credenciamento = trim(pg_result($res, $i, credenciamento));

			if ($status_credenciamento != "CREDENCIADO") {
		...
		echo "<td>\n";
		if ($login_fabrica == 30 && $descredenciado) {

			echo "<a href=\"javascript: alert('Posto autorizado selecionado está “Em Descredenciamento”')\"/>";

			$descredenciado = false;

		} else {

			echo "<a href=\"javascript: nome.value = '$nome'; codigo.value = '$codigo_posto';contato_endereco.value = '$contato_endereco';contato_numero.value = '$contato_numero';contato_bairro.value = '$contato_bairro';contato_cidade.value = '$contato_cidade';contato_estado.value = '$contato_estado'; posto.value='$posto';$campo_tipo_posto if (window.contato_cep) { contato_cep.value = '$contato_cep'; }";
