...
function retorno(referencia, descricao) {
	...
	f.peca_referencia<? echo $linha; ?>.value = referencia;
	f.peca_descricao<? echo $linha; ?>.value  = descricao;
	window.close();
}
// -->
</script>
...
if (pg_numrows ($res) == 1 ) {
	...
	echo "<script language=\"JavaScript\">\n";
	echo "<!--\n";
	echo "opener.window.document.$formulario.peca_referencia$linha.value = '$referencia'; \n";
	echo "opener.window.document.$formulario.peca_descricao$linha.value  = '$descricao';   \n";
	echo "window.close();\n";
	echo "// -->\n";
	echo "</script>\n";
}else{
