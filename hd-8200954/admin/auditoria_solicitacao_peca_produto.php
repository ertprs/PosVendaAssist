	...
	if (!strlen($motivo)) {
		...
		if (!pg_num_rows($result)) {
			...
			switch ($tipo_auditoria) {
				case 'troca_produto':
					$status = $troca_produto["reprovada"];
					$msg = "Troca de produto foi recusada pela Fábrica";
					break;
				
				default:
					$status = $troca_peca["reprovada"];
					$msg = "Solicitação de peça foi recusada pela Fábrica";
					break;
			}
			...
			$insert = "INSERT INTO tbl_os_status
					   ...
					   VALUES
					   ({$os}, {$status}, '{$msg}: {$motivo}', {$login_admin})";
			$result = pg_query($con, $insert);
...
if ($_POST["aprovar"] == true) {
	...
	if (empty($os)) {
		...
		if (!pg_num_rows($result)) {
			...
			switch ($tipo_auditoria) {
				case 'troca_produto':
					$status = $troca_produto["aprovada"];
					$msg = "Troca de produto aprovada pela Fábrica";
					break;
				
				default:
					$status = $troca_peca["aprovada"];
					$msg = "Solicitação de peça aprovada pela Fábrica";
					break;
			}
			...
			$insert = "INSERT INTO tbl_os_status
					   (os, status_os, observacao, admin)
					   VALUES
					   ({$os}, {$status}, '{$msg}', {$login_admin})";
			$result = pg_query($con, $insert);

			if (strlen(pg_last_error()) > 0) {
				$retorno = array("erro" => utf8_encode("Erro ao aprovar OS"));
			} else {
				if ($tipo_auditoria == "troca_produto") {
...
if ($_POST["btn_acao"] == "submit" || $_POST['btn_acao'] == "listar_todas") {
	...
	if (!count($msg_erro["msg"])) {
		...
		switch ($status) {
			case 'aprovadas':
				$cond_status = $$tipo_auditoria;
				$cond_status = $cond_status["aprovada"];
				break;

			case 'reprovadas':
				$cond_status = $$tipo_auditoria;
				$cond_status = $cond_status["reprovada"];
				break;
			
			default:
				$cond_status = $$tipo_auditoria;
				$cond_status = $cond_status["pendente"];
				break;
		}
		...
		$select = "SELECT
					...
					AND (
						...
						AND tbl_os_status.status_os IN(".implode(",", $status_os).") 
...
$title = "AUDITORIA DE SOLICITAÇÃO DE PEÇA/PRODUTO";
...
$(function() {
	...
	$(document).on("click", "button[name=button_interagir]", function() {
		...
		if (interacao.length == 0) {
			...
			$.ajax({
				url: "auditoria_solicitacao_peca_produto.php",
				type: "post",
...
<form name="frm_auditoria_os_troca_produto" method="post" action="<?=$_SERVER['PHP_SELF']?>" class="form-search form-inline tc_formulario" style="margin: 0 auto;" >
	...
	<div class='row-fluid'>
			...
			<div class='span8'>
				<label class='control-label' for='tipo_auditoria'>Tipo Auditoria</label>
				<div class='controls controls-row'>
					<div class='span4'>
						 <label class="radio">
					        <input type="radio" name="tipo_auditoria" value="troca_peca" checked>
					        Solicitação de Peça
					    </label>
					</div>
					<div class='span6'>
					    <label class="radio">
					        <input type="radio" name="tipo_auditoria" value="troca_produto" <?php if($tipo_auditoria == "troca_produto") echo "checked"; ?> />
					...
					<tr>
						...
						?>
							<td class="tac" style="background-color: <?=$cor?>;" nowrap >
								<button type="button" rel="<?=$os?>" tipo-auditoria="<?=$tipo_auditoria?>" name="interagir" class="btn btn-small btn-primary" >Interagir</button>
								<button type="button" rel="<?=$os?>" tipo-auditoria="<?=$tipo_auditoria?>" name="aprovar" class="btn btn-small btn-success" >Aprovar</button>
								<button type="button" rel="<?=$os?>" tipo-auditoria="<?=$tipo_auditoria?>" name="recusar" class="btn btn-small btn-danger" >Recusar</button>
							</td>
						<?php
						}
						?>
