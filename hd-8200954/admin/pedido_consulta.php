...
<?
    ...
    if($login_fabrica == 151){ //hd_chamado=2765193
        if($_REQUEST['familia_produto'])      $familia_produto       = $_REQUEST['familia_produto'];
    }        
    
    if ($login_fabrica == 104) {
        if($_POST['tipo_peca']) $tipoPeca = trim($_POST['tipo_peca']);
    }
...
<?
     ...
     */
        if (in_array($login_fabrica, array(40, 104, 151, 153, 24))) {
        ...
        if($login_fabrica == 151){
            $groupBy = array_merge($groupBy, array(
                "tbl_os_campo_extra.os_bloqueada", "tbl_os.finalizada", "tbl_pedido_item.qtde_faturada_distribuidor", "tbl_pedido_item.qtde_cancelada", "tbl_tabela.descricao", "tbl_pedido_item.qtde", "tbl_produto.produto", "tbl_produto.descricao", "tbl_posto.estado, tbl_linha.nome"
            ));
        }
        ...
        if ($login_fabrica == 153) {
            ...
            if($login_fabrica == 151){
                if(!empty($familia_produto)){                    
                    for($fam = 0; $fam < count($familia_produto); $fam++){
                        $id_familia[]   = $familia_produto[$fam];  
                        $familia_id     = implode(',', $id_familia);                    
                    }
                    $condFamilia = " AND tbl_produto.familia IN ($familia_id) ";
                } else {
                    $condFamilia = "";
                }
                    
                $leftJoin_x = " LEFT JOIN tbl_familia ON tbl_familia.familia = tbl_produto.familia ";
                $campoFamilia = " tbl_familia.descricao AS descricao_familia,";

                array_push($groupBy,"tbl_familia.descricao");                
            }
            ...
            switch ($tipoPeca) {
                ...
                case 1: 
                    $condPeca .= " AND tbl_peca.devolucao_obrigatoria IS true "; 
                    break;
                case 2:    
                    $condPeca .= " AND tbl_peca.item_aparencia IS true ";        
                    break;
                case 3: 
                    $condPeca .= " AND tbl_peca.acumular_kit IS true ";          
                    break;
                case 4: 
                    $condPeca .= " AND tbl_peca.retorna_conserto IS true ";      
                    break;
                case 5: 
                    $condPeca .= " AND tbl_peca.bloqueada_garantia IS true ";    
                    break;
                case 6: 
                    $condPeca .= " AND tbl_peca.acessorio IS true ";             
                    break;
                case 7: 
                    $condPeca .= " AND tbl_peca.aguarda_inspecao IS true ";      
                    break;
                case 8: 
                    $condPeca .= " AND tbl_peca.peca_critica IS true ";          
                    break;
                case 9: 
                    $condPeca .= " AND tbl_peca.produto_acabado IS true ";       
                    break;
                default: 
                    break;
            }
    ...
    if($login_fabrica == 151){
        $camposDetalhado = ",tbl_pedido_item.qtde_faturada_distribuidor,
                             tbl_pedido_item.qtde_cancelada,
                             tbl_tabela.descricao as descricao_tabela,
                             tbl_pedido_item.qtde as qtde_pecas,
                             tbl_produto.produto AS referencia_produto,
                             tbl_produto.descricao AS descricao_produto,                             
                             $campoFamilia
                             tbl_posto.estado AS uf,
                             tbl_linha.nome AS nome_linha"; 
        $leftDetalhado = "LEFT JOIN tbl_tabela      ON tbl_tabela.tabela = tbl_pedido.tabela";
    }
    ...
    $leftJoinStatus = "";
    $condFinalizado = "AND     tbl_pedido.finalizado   IS NOT NULL";
    ...
    $sql = "SELECT  DISTINCT
       ...
       LEFT JOIN    tbl_condicao        ON  tbl_condicao.condicao           = tbl_pedido.condicao
            ...
            $join_chk8
            $sql_left
        $leftJoinStatus  JOIN    tbl_status_pedido   ON  tbl_status_pedido.status_pedido = tbl_pedido.status_pedido
...
if (in_array($login_fabrica,array(151,158,169,170)) && count($estado_posto_autorizado) > 0) {    
    for($epa = 0; $epa < count($estado_posto_autorizado); $epa++){
        $id_epa[]   = $estado_posto_autorizado[$epa];  
        $epa_id     = implode("','", $id_epa);                    
	}
    if(strlen($epa_id) > 0) {	
		$sql .= " AND tbl_posto_fabrica.contato_estado IN ('$epa_id')";
	}
}
...
if (strlen($estado) > 0 AND in_array($login_fabrica, array(72))) { #HD 280384
...
if (!empty($data_inicial_01) && !empty($data_final_01)) {
    ...
    if(isset($_POST['equipe_venda'])){
        $equipe_venda = $_POST["equipe_venda"]; 
    }

    if(isset($_POST['escritorio_venda'])){
        $escritorio_venda = $_POST["escritorio_venda"]; 
    }
...
if(strlen(trim($escritorio_venda))>0) {
    $monta_sql .= " and tbl_pedido.valores_adicionais::JSON->>'escritorio_venda' = '$escritorio_venda'  ";
}

if(strlen(trim($equipe_venda))>0){
    $monta_sql .= " and tbl_pedido.valores_adicionais::JSON->>'equipe_venda' = '$equipe_venda'  ";
}
...
if($login_fabrica == 151){
    if(!empty($linha_produto)){
        for($l = 0; $l < count($linha_produto); $l++){
            $id_linha[]   = $linha_produto[$l];  
            $linha_id     = implode(',', $id_linha);                    
        }
        $monta_sql .= " AND tbl_produto.linha IN ($linha_id) ";
    } else {
        $monta_sql .= "";
    }
    
    //$camposDetalhado .=  " ,tbl_linha.nome AS nome_linha ";
    //array_push($groupBy, "tbl_linha.nome");
}
...
if (empty($msg_erro)){
	...
	echo "<br /><table>
			    ...
			    if(!$telecontrol_distrib){
			    	...
			    	echo "</TH>";
							...
							if (($login_fabrica == 160 or $replica_einhell) && $_POST['csv_detalhado']) {
                                ...
                                if($login_fabrica == 24){
                                    echo "<TH>Nome do Atendente</TH>";
                                    echo "<TH>Qtde. Parcelas</TH>";
                                }
                                if ($login_fabrica == 85) {
	    ...
	    if ($login_fabrica == 175) {
		  $aprovado_cliente = pg_fetch_result($res, $i, 'aprovado_cliente');
			...
			if (($login_fabrica == 160 or $replica_einhell) && $_POST['csv_detalhado']) {
                ...
                $qtde_faturada     = trim(pg_result ($res,$i,'qtde_faturada_distribuidor'));
	    ...
	    if(!$telecontrol_distrib){
            ...
            if($login_fabrica == 151){
                ...
                echo "  <TD>$descricao_familia</TD>\n";
                //echo "  <TD>$referencia_peca - $descricao_peca</TD>\n";
                /*echo "<td><button type='button' class='btn btn-small btn-primary' id='ver-pecas' name='ver-pecas' onclick='verPecas($('#pedido_".$i."').value())'><i class='icon-search icon-white'></i> Ver Peça(s)</button>\n<input type='text' name='pedido_".$i."' id='pedido_".$i."' value='".$pedido_aux."' /></td>\n";*/
                echo "<td><button type='button' class='btn btn-small btn-primary' id='ver-pecas' name='ver-pecas' onclick='verPecas(".$pedido_aux.")'><i class='icon-search icon-white'></i> Ver Peça(s)</button>\n<input type='hidden' name='pedido_".$i."' id='pedido_".$i."' value='".$pedido_aux."' /></td>\n";                
            }
            if($login_fabrica == 88) {
            ...
            if($login_fabrica == 24){
                ...
                if(!empty($valores_adicionais)){
                    ...
                    $nome_atendente = $valores_adicionais->nome_funcionario;
                    $qtde_parcelas = $valores_adicionais->qtde_parcelas; 
                    $valor_parcela = $total / (int)$qtde_parcelas;
                    $qtde_parcelas = $qtde_parcelas . ' x ' . number_format($valor_parcela,2,",","."); 
                }

                echo "<TD class='tac'>{$nome_atendente}</TD>";
                echo "<TD class='tac'>{$qtde_parcelas}</TD>";
            }
            ...
            if (((empty($exportado)) || (!empty($exportado) && $altera_pedido_exportado =='t') || $login_fabrica == 101) && strpos("garantia",strtolower($descricao_tipo)) === false){
                if ($login_fabrica == 14) {
					echo "  <TD>&nbsp;</TD>\n";
    ...
    ?>
        <script>
            function verPecas(pedido){
                Shadowbox.init();
                
                Shadowbox.open({
                    content: "visualizar_pecas.php?pedido="+pedido,
                    player: "iframe",
                    title: "Ver Peças",
                    width: 900,
                    height: 390,
    ...
    if ($xls_rows > 0) {
        ...
        //system("rm /home/kaique/public_html/PosVenda/admin/xls/assist/relatorio-consulta-pedido-$login_fabrica.csv");
        //$fp = fopen ("/home/kaique/public_html/PosVenda/admin/xls/relatorio-consulta-pedido-$login_fabrica.csv","w");
	...
	if(!in_array($login_fabrica, array(95, 153, 193)) && !$telecontrol_distrib){
        ...
        if (!in_array($login_fabrica, [153,193])) {
            $cabecalho[] = "Tipo";
        }
        if ($login_fabrica == 7){
        ...
        if($login_fabrica == 24){
            $cabecalho[] = "Nome do Atendente";
            $cabecalho[] = "Qtde. Parcelas";
        }
	...
	if ($telecontrol_distrib && !$_POST['csv_detalhado']) {
        ...
        for ($i = 0; $i < $xls_rows; $i++){
            ...
            if (($telecontrol_distrib || $interno_telecontrol) && $_POST['csv_detalhado']) {
                $sql_dados_exel = " SELECT tbl_faturamento.nota_fiscal,
                                           ...
                                           (tbl_pedido_item.qtde- (tbl_pedido_item.qtde_faturada_distribuidor + qtde_cancelada)) AS pendencia_pedido      
                                    FROM tbl_pedido 
                                    JOIN tbl_pedido_item ON tbl_pedido.pedido = tbl_pedido_item.pedido 
                                    JOIN tbl_peca ON tbl_pedido_item.peca = tbl_peca.peca
                ...
                if (pg_num_rows($res_dados_exel) > 0) {
                    ...
                    $qtde_faturada_tc    = pg_fetch_result($res_dados_exel, 0, 'qtde_faturada_distribuidor');
            ...
            $pedido_aux2 = (in_array($login_fabrica,[30,88]) AND (!empty($seu_pedido))) ? $seu_pedido : $pedido;
            ...
            if($login_fabrica == 24){
               $total_pedido_formatado = number_format($total, 2, ",", "");
            }
                ...
                if (pg_num_rows($resRefPeca) > 0) {
                    foreach (pg_fetch_all($resRefPeca) as $key => $rows) {
                        ...
                        if (pg_num_rows($res_dados_exel) > 0) {
                                ...
                                $nota_fiscal_tc      = pg_fetch_result($res_dados_exel, 0, 'nota_fiscal');
                                $emissao_tc          = pg_fetch_result($res_dados_exel, 0, 'emissao');
                                $transportadora_tc   = pg_fetch_result($res_dados_exel, 0, 'transportadora_nome');
                                $conhecimento_tc     = pg_fetch_result($res_dados_exel, 0, 'conhecimento');
                                $atende_parcial_tc   = pg_fetch_result($res_dados_exel, 0, 'atende_parcial');
                                $atende_parcial_tc   = ($atende_parcial_tc == 't') ? 'Sim' : 'Não';
                                $preco_unitario_tc   = pg_fetch_result($res_dados_exel, 0, 'preco_unitario');
         ...
         */
        if (in_array($login_fabrica, array(40, 151, 104, 153, 24))) {
            $xlsdata = date ("d/m/Y H:i:s");
            ...
            while ($row = pg_fetch_object($res)) {
				...
				foreach ($itens as $item_pedido => $peca) {
                    if ($login_fabrica == 153) {
                        ...
                        $produto_descricao = str_replace([".",",",";"], "", $row->produto_descricao);
                        $posto_nome        = str_replace([".",",",";"], "", $row->posto_nome);
                        $peca_descricao    = str_replace([".",",",";"], "", $row->peca_descricao);
                        $auxiliar = explode(",", $peca[0]);
                        # $auxiliar[6]
                        $linha = array(
                            $row->pedido,                   # PEDIDO                           #               
                            $row->descricao_tipo_pedido,    # TIPO DE PEDIDO                   #
                            $auxiliar[0],                   # OS                               #  
                            $auxiliar[5],                   # DATA DE ABERTURA DO PEDIDO       #
                            $row->descricao_status_pedido,  # STATUS PEDIDO                    #  
                            $auxiliar[8],                   # QUANTIDADE DAS PEÇAS SOLICITADAS #
                            $row->codigo_posto,             # CÓDIGO DO POSTO                  #
                            $posto_nome,                    # POSTO                            #
                            $auxiliar[1],                   # REFERENCIA PRODUTO"              #   
                            $auxiliar[2],                   # DESCRIÇÃO PRODUTO"               #  
                            $auxiliar[9],                   # REFERENCIA PEÇA                  #  
                            $auxiliar[10]                   # DESCRIÇÃO PEÇA                   #                    
                        );      
                        ...
                        $posto_nome = $row->codigo_posto . ' - ' .str_replace([".",",",";"], "", $row->posto_nome);
                        $auxiliar = explode(",", $peca[0]);

                        $linha = array(
                            $row->pedido,                   # PEDIDO                           #     
                            $row->pedido_cliente,           # PEDIDO CLIENTE                   #            
                            $row->descricao_tipo_pedido,    # TIPO PEDIDO                      #
                            $row->descricao_status_pedido,  # STATUS PEDIDO                    #
                            $row->data,                     # DATA DO PEDIDO                   #
                            $row->data_recebido,            # DATA RECEBIDO                    #
                            $posto_nome,                    # POSTO                            #
                            $auxiliar[0],                   # CÓDIGO PEÇA                      #  
                            $auxiliar[1],                   # DESCRIÇÃO PEÇA                   #         
                            $auxiliar[2]                    # QTDE. PEÇA                       #           
                        );     
                    }
