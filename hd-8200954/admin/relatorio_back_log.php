...
<caption>Relatório Back-Log (Sem data de Conserto)</caption>
...
if ((strlen($btn_acao)  > 0 AND strlen($msg_erro)==0) OR strlen($pedido) > 0 ) {
	...
	if (strlen($xdata_inicial) > 0 AND strlen($xdata_final) > 0) {
		$sql_data = " AND tbl_os.data_abertura BETWEEN '$xdata_inicial' AND '$xdata_final' ";
	}
...
*/		echo "</tr>";
		...
		for ($x=0; $x<pg_numrows($res);$x++){
			...
			$nome_posto					= substr(pg_result($res, $x, nome_posto), 0, 35);
			$referencia_produto			= pg_result($res, $x, referencia_produto);
			$descricao_produto			= substr(pg_result($res, $x, descricao_produto), 0, 35);
			...
			/*if(strlen($inicio_abertura)>0  and strlen($inicio_atendimento)>0){
...
*/
			...
			$inicio = date("d/m/Y H:i",strtotime($inicio_abertura));
			$fim    = date("d/m/Y H:i",strtotime($inicio_atendimento));
			...
			echo "<td align='left' style='font-size: 9px; font-family: verdana' nowrap> $referencia_produto - $descricao_produto</td>";
