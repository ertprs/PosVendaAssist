...
$sql = "SELECT posto,nome FROM tbl_posto_fabrica JOIN tbl_posto USING(posto) WHERE codigo_posto = '$codigo_posto' and tbl_posto_fabrica.fabrica = $login_fabrica";
$res = pg_exec ($con,$sql);
...
if (strlen($faturamento) > 0) {
	...
	if (pg_numrows($res) > 0) {
		...
		echo "<tr>\n";
		echo "<td class='menu_top' colspan='6'>$codigo_posto - $nome</td>\n";
