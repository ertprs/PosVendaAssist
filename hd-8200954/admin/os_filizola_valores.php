...
if ($btn_acao == "gravar") {
	...
	function pg_money_format($val) {
		if($val == '' or $val == '0')
		{ return null;
		
		}
		if (!preg_match("/^(\d{1,3}[.])*?(\d{1,3})+([.,]\d{1,2})?$/", $val, $m)) {
			print_r($m);
			return false;
		}
		return (strpos($val, ',') > strlen($val)-4) ?
				str_replace(array('.', ','), array('', '.'), $val) :
				str_replace('.', '', $val);
	}
//echo "<pre>";var_dump($_POST, true);echo "</pre>";
	...
	$taxa_visita				= pg_money_format(trim($_POST['taxa_visita']));
	$visita_por_km				= pg_money_format(trim($_POST['visita_por_km']));
	$valor_por_km				= pg_money_format(trim($_POST['valor_por_km']));
	$veiculo					= trim($_POST['veiculo']);
	$deslocamento_km			= pg_money_format(trim($_POST['deslocamento_km']));

	$hora_tecnica				= pg_money_format(trim($_POST['hora_tecnica']));
	$regulagem_peso_padrao		= pg_money_format(trim($_POST['regulagem_peso_padrao']));

	$certificado_conformidade	= pg_money_format(trim($_POST['certificado_conformidade']));
	$valor_diaria				= pg_money_format(trim($_POST['valor_diaria']));

	$cobrar_deslocamento		= trim($_POST['cobrar_deslocamento']);
	$cobrar_hora_diaria			= trim($_POST['cobrar_hora_diaria']);

	$desconto_deslocamento		= pg_money_format(trim($_POST['desconto_deslocamento']));
	$desconto_hora_tecnica		= pg_money_format(trim($_POST['desconto_hora_tecnica']));
	$desconto_diaria			= pg_money_format(trim($_POST['desconto_diaria']));
	$desconto_regulagem			= pg_money_format(trim($_POST['desconto_regulagem']));
	$desconto_certificado		= pg_money_format(trim($_POST['desconto_certificado']));

	$cobrar_regulagem			= trim($_POST['cobrar_regulagem']);
	$cobrar_certificado			= trim($_POST['cobrar_certificado']);

	$reabre_os					= trim($_POST['reabre_os']);
	...
	if ($tipo_posto == 214 or $tipo_posto == 215) {
	...
	// HD 114544 (Salvar valor de regulagem mesmo que o checkbox não esteja checkado ) [augusto]
	// HD 121493 (Salvar regulagem apenas se estiver checado) [augusto]
...
*/
	...
	if (($tipo_posto == 7  OR $tipo_posto == 224) AND $classificacao_os == 6 AND ($cobrar_hora_diaria == 'isento' OR strlen($cobrar_hora_diaria) == 0 )) {
	...
	if (pg_numrows($res) > 0) {
		# HD 47243
		$sqlFec = "SELECT data_fechamento, finalizada
				FROM tbl_os
				WHERE os = $os
				AND fabrica = $login_fabrica;";
		$resFec = pg_exec ($con,$sqlFec);

		if (pg_numrows($res) > 0) {
			$data_fechamento_tmp = pg_result($resFec,0,data_fechamento);
			$finalizada_tmp = pg_result($resFec,0,finalizada);
		}
	...
	if ($xclassificacao_os == 5) {
		...
		if (pg_numrows($res) > 0) {
			...
			if (isset($xdata_fechamento) and $xdata_fechamento <> 'null' and (strlen($msg_erro) == 0)){
				...
				if ($os_manutencao == 't') {
					...
					if ($classificacao_os == '46'){
						$sql = "SELECT count(*) AS qtde_garantia
								...
								WHERE  tbl_os.fabrica   = $login_fabrica
								AND    tbl_os.posto     = (SELECT posto FROM tbl_os WHERE fabrica = $login_fabrica AND os = $os)
	...
	if (strlen($msg_erro) == 0){
		...
		if ((isset($data_fechamento_tmp) and isset($finalizada_tmp)) and ($altera_os == "0")){
		$sql = "UPDATE tbl_os SET data_fechamento = '$data_fechamento_tmp', finalizada = '$finalizada_tmp'
					WHERE  tbl_os.os      = $os
					AND    tbl_os.fabrica = $login_fabrica
					;";
			$res = pg_exec ($con,$sql);
...
//		$cliente_contrato		= pg_result ($res,0,'cliente_contrato');
		...
		$regulagem_peso_padrao	= ( empty($regulagem_peso_padrao) ) ? pg_result ($res,0,'regulagem_peso_padrao') : $regulagem_peso_padrao ;
		...
		if ($os_manutencao == 't'){
			...
			if (pg_numrows ($res2) > 0 ) {
				...
				$deslocamento_km          = trim(pg_result($res2,0,deslocamento_km));
				...
				$regulagem_peso_padrao    = ( empty($regulagem_peso_padrao) ) ? trim(pg_result($res2,0,'regulagem_peso_padrao')) : $regulagem_peso_padrao ;
		...
		if ($os_manutencao != 't' or 1==1){
			...
			if (pg_numrows($res) > 0) {
				...
				if ($regulagem_peso_padrao == "0" or strlen($regulagem_peso_padrao)==0){
					$regulagem_peso_padrao    = ( empty($regulagem_peso_padrao) ) ? trim(pg_result($res,0,regulagem_peso_padrao)) : $regulagem_peso_padrao ;
				}
...
function fnc_pesquisa_lista_basica (produto_referencia, peca_referencia, peca_descricao, peca_preco, voltagem, tipo, peca_qtde) {
		var desconto = parseFloat(document.frm_os.desconto_peca.value);
        var url = "";
        if (tipo == "tudo") {
                url = "peca_pesquisa_lista.php?produto=" + produto_referencia + "&descricao=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>&posto=<? echo $posto; ?>&desconto="+desconto;
        }

        if (tipo == "referencia") {
                url = "peca_pesquisa_lista.php?produto=" + produto_referencia + "&peca=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>&posto=<? echo $posto; ?>&desconto="+desconto;
        }

        if (tipo == "descricao") {
                url = "peca_pesquisa_lista.php?produto=" + produto_referencia + "&descricao=" + peca_descricao.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>&posto=<? echo $posto; ?>&desconto="+desconto;
        }
...
function verificaProduto(produto,serie){
	...
	function mostraServicos(linha) {
		<? if($posto == 6359){ #liberado somente para PA de teste - HD 47695?>
		$.ajax({
			type: "GET",
...
*/?>
	...
	<tr>
		...
		<TD class="table_line2" colspan='2'>
			<input type='text' name='desconto_peca' class='frm' value='<?=$desconto_peca?>' size='6' maxlength='5'>
...
<table class="border tabela_item" width='700' align='center' border='0' cellpadding="1" cellspacing="3">
	...
	<tr>
		...
		<td class="menu_top" colspan='3'>Peça</td>
...
<?
	...
	for ($i = 0 ; $i < $qtde_item ; $i++) {
		...
		echo "<input type='hidden' name='valor_$i' value='$preco'>\n";
		echo $i+1;
		echo "</td>\n";

		echo "<td align='center'>";
		echo "<input type='text' name='referencia_$i' id='referencia_$i' size='12' maxlength='20' value='$referencia' onChange='mostraServicos($i)'>";
		echo "<img src='imagens/btn_lupa.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_lista_basica (document.frm_os.produto_referencia.value , document.frm_os.referencia_$i , document.frm_os.descricao_$i , document.frm_os.preco_$i , document.frm_os.voltagem, \"referencia\",document.frm_os.qtde_$i)' alt='Clique para efetuar a pesquisa' style='cursor:pointer;'>";
		echo "</td>\n";
		/*onclick=\"javascript: fnc_pesquisa_peca (document.frm_os.referencia_$i, document.frm_os.descricao_$i, 'referencia')\" */
		

		echo "<td align='center'>";
		echo "<input type='text' name='descricao_$i' size='20' maxlength='50' value='$descricao' onChange='mostraServicos($i)'>";
		echo "<img src='imagens/btn_lupa.gif' border='0' align='absmiddle' onclick='javascript: fnc_pesquisa_lista_basica (document.frm_os.produto_referencia.value , document.frm_os.referencia_$i , document.frm_os.descricao_$i , document.frm_os.preco_$i , document.frm_os.voltagem, \"descricao\",document.frm_os.qtde_$i)' alt='Clique para efetuar a pesquisa' style='cursor:pointer;'>";
		echo "</td>\n";
		/*onclick=\"javascript: fnc_pesquisa_peca (document.frm_os.referencia_$i, document.frm_os.descricao_$i, 'descricao')\"*/

		echo "<td align='center'><input type='text' name='preco_$i' size='20' maxlength='10' value='$preco'></td>\n";
		...
		echo "<td align='center'>";
		echo "<input type='text' name='qtde_$i' size='3' maxlength='3' value='$qtde'>";
		echo "</td>\n";

		echo "<td align='center'>";
		echo "<select size='1' name='defeito_$i'>";
		...
		echo "<td align='center'>";
		echo "<select size='1' name='servico_realizado_$i' id='servico_realizado_$i' style='width:250px' onFocus='mostraServicos($i)'>";
		echo "<option selected></option>";
...
<table class="border" width='700' align='center' border='0' cellpadding="1" cellspacing="3">
	...
	<?php # HD 44796 ?>
	<TR>
		<TD class='menu_top' colspan='2'>Assinatura:</TD>
		<TD class='menu_top' colspan='2'>Representante:</TD>
	</TR>
	<TR>
		<TD class='table_line' colspan='2'>
			<INPUT TYPE='text' NAME='assinou_os' value='<? echo $assinou_os; ?>' size='50' maxlength='40'>
		</TD>
		<TD class='table_line' colspan='2'><?php
	...
	<? //if ($tipo_posto == 214) { # 22874 foi retirado no HD 44134 a pedido do Eduardo ?>
	<tr>
	...
	<?php
		if (strlen($data_fechamento) == 0){ ?>
				<TD class='table_line' colspan='4'><INPUT TYPE='text' NAME='data_fechamento' value='<? echo $data_fechamento; ?>' rel='data' size='12' maxlength='10'></TD>
	<?php	} else { ?>
				<TD class='table_line'>&nbsp;</TD>
				<TD class='table_line'><INPUT TYPE='text' NAME='data_fechamento' value='<? echo $data_fechamento; ?>' rel='data' size='12' maxlength='10' disabled></TD>
				<TD class='table_line'><INPUT TYPE='checkbox' NAME='reabre_os' value='t' <? if ($reabre_os=='t') echo "checked" ?>>Reabrir OS</td>
				<TD class='table_line'>&nbsp;</TD>
	<?php	} ?>
