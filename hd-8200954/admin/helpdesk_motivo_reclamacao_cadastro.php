	...
	function ativa_inativa(hd_situacao,status){
		$.ajax({
            ...
            if (data.success == 'inativado') {
            	...
            	$("#label_"+hd_situacao).html('Inativo');
            }else{
...
<?php
		$sql = "SELECT hd_situacao,descricao,ativo FROM tbl_hd_situacao WHERE fabrica = $login_fabrica AND tipo_registro = 'motivo_hd_chamado' ORDER BY descricao";
		$res = pg_query($con,$sql);
	...
	?>
			<table id="motivo_situacao_cadastro" class='table table-striped table-bordered table-hover table-fixed' >
				<thead>
					...
					<th colspan='3' class='tac'><?=$nome_cadastro?></th>
					</tr>
					<tr class='titulo_coluna'>
				...
				<tbody>
					...
					?>
							<tr>
								...
								<td class='tac'>
									<p id="label_<?=$hd_situacao?>" class="label <?=$class_label?>"><?=$status?></p>
								</td>
								<td class='tac'>
