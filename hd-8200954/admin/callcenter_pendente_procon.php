...
if(strlen($supervisor)>0){
	...
	$sql = "SELECT tbl_hd_chamado.hd_chamado        ,
			...
			AND (tbl_hd_chamado.categoria = 'Procon' OR tbl_hd_chamado.categoria = 'Jec')
			AND $cond1
	...
	if(pg_numrows($res)>0){
		...
		if($login_fabrica==6)echo "<td class='menu_top' background='imagens_admin/azul.gif'><ACRONYM TITLE=\"NATUREZA\">NATUREZA</ACRONYM></TD>\n";
		...
		for($x=0;pg_numrows($res)>$x;$x++){
			...
			$categoria = pg_result($res,$x,categoria); //HD 42763
			...
			if($login_fabrica==6) echo "<TD class='linha' align='center' nowrap><a href='cadastra_callcenter.php?callcenter=$callcenter' class='linha'>$categoria</a></TD>\n";
