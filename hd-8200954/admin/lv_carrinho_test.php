					...
					while (false !== ($filename = readdir($dh))) {
						...
						if (strpos($filename,$referencia) !== false){
							...
							if(substr($filename, 0,$po)==$referencia){?>
								<a href="<? echo "$PHP_SELF?ajax=true&peca=$filename"; ?>&keepThis=trueTB_iframe=true&height=340&width=420" title="<?echo $referencia;?>" class="thickbox">
				...
				if (strlen($pedido)==0){
					...
					if(strlen($msg_erro)==0){
						...
						if (strlen($msg_erro) == 0){
							...
							if ($login_fabrica==10){								
								add_cookie($cookie_login,"cook_pedido_lu",$pedido);
...
if ($btn_acao == "fechar_pedido"){
	...
	if ($valor_total_pedido < $valor_minimo) {
		$msg_erro .= "Cada pedido deve ser superior a R$ $valor_minimo";
	}
	...
	if (strlen ($msg_erro) == 0) {
		...
		echo "<script languague='javascript'>window.location='".$PHP_SELF."?status=finalizado&p=$pedido';</script>";
	...
	echo "<div class='lcar_title'>";
		...
		if ( $qtde_itens > 0) {
			...
			for ($i=0;$i<pg_numrows($res);$i++){
				...
				$xsoma = number_format($soma, 2, ',', '');
				$xsoma = str_replace(".",",",$xsoma);
				$retorno .="Sub Total: <font color='#FF0033'>R$ $xsoma</font> <br>";
			}
			echo "</tbody>";
			echo "<tfoot>";


			if (strlen($login_unico)==0 AND $login_fabrica<>10  AND $login_fabrica <> 1 AND $login_fabrica<>3 AND $login_fabrica<>35 ){
				echo "<tr>";
				echo "<td colspan='7'>";
