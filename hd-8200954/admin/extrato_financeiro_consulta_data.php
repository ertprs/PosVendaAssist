...
if (strlen ($posto_codigo) > 0 OR (strlen ($data_inicial) > 0 and strlen ($data_final) > 0) ) {
	...
	if (strlen ($x_data_inicial) == 10 AND strlen ($x_data_final) == 10)
		$sql .= " AND tbl_extrato_financeiro.data_envio BETWEEN '$x_data_inicial 00:00:00' AND '$x_data_final 23:59:59'";
...
//if ($ip=="201.43.201.204")echo nl2br($sql);
	...
	if ($login_fabrica == 1) echo "<td align='center' nowrap>Pendência</td>\n";
			...
			$totalpeca = $totalpeca + pg_result($resT,0,total_pecas);
			$totalmaodeobra = $totalmaodeobra + pg_result($resT,0,total_maodeobra);
			$totalavulso = $totalavulso + pg_result($resT,0,total_avulso);
		...
		echo "</tr>\n";
		echo $totalgeral. "-G ". $totalpeca. "-peca ". $totalmaodeobra."-maodeobra ".$totalavulso."-avulso";
		echo "</table>\n";
		echo "</form>\n";
	}
