...
<?
	...
	echo "<tr><td>";
	echo "<div class='titulo_tabela' style='padding:0;margin:0;'>Conforme Determina a Legislação Local</div><p>";
...
echo "<br /><div class='formulario' style='width:700px;margin:auto;'>",
	 "<center><font size='+1' face='arial'>Data do Extrato $data </font>";
echo "<br>";
echo "<font size='+0' face='arial'>$codigo - $nome</font></center>";
...
if(strlen($somente_consulta)> 0){
	...
	<button type='button' onclick=\"window.location='extrato_posto_mao_obra_consulta.php?extrato=$extrato&posto=$posto&somente_consulta=$somente_consulta'\">Ver Mão-de-Obra</button>";
}else{
	echo "<td align='center' width='200px'>
	<button type='button' onclick=\"window.location='extrato_posto_mao_obra.php?extrato=$extrato&posto=$posto'\">
	Ver Mão-de-Obra</button>";
}
...
?>
	...
	<td align='left'>
		<button type='button' onclick="window.location='extrato_posto_britania.php?somente_consulta=sim'">Ver outro extrato</button>
	</td>
</tr>
</table>
</div>
...
if ($qtde_for > 0 OR 1==1) {
	...
	for ($i=0; $i < $qtde_for; $i++) {
		...
		if ($posto=='13996'){ #BRITANIA
			...
			if(strtotime(date("Y-m-d")) >= strtotime("2017-03-01")){
...
//			if ($aliq_final <> $aliq_icms) $aliq_final = -1;
		...
		echo "Nota de Devolução $nota_fiscal";
		echo ($temNota) ? "</a>" : "";
		echo "</center></h1></td>\n";
        echo ($temNota) ? "<td><center><button type='button' class='btn_remove_nota' data-tdocs='".$nota_id."'> Remover Nota Fiscal</button></center></td>" : "";
		echo "</tr>";
		echo "</table>";
