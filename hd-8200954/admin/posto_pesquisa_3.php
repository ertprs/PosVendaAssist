...
//ARQUIVO USADO PARA PESQUISA DE POSTOS PELO ARQUIVO: /admin/condicao_pagamento_alterar.php
...
if (pg_numrows($res) == 1) {
	...
	for ( $i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
		...
		//if ($_GET['forma'] == 'reload') {
			echo "<a href=\"javascript: janela = opener.document.location.href ; posicao = janela.lastIndexOf('.') ; janela = janela.substring(0,posicao+4) ; opener.document.location = janela + '?posto_codigo=$codigo_posto' ; this.close() ;\" > " ;
/*		}else{
			echo "<a href=\"javascript: alert(); nome.value = '$nome'; codigo.value = '$codigo_posto';";
