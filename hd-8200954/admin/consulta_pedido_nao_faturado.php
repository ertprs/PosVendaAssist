...
if(isset($_POST['data_inicial']) || isset($_POST['numero_pedido'])){
	...
	if(empty($numero_pedido)){
		if(empty($data_inicial)){
			$msg_erro['msg'][0] = traduz("Preencha os campos obrigatórios!");
	...
	if(count($msg_erro['msg']) == 0){
		...
		if (strlen($posto_codigo) > 0 || strlen($posto_nome) > 0){
			$sql = "
				...
				OR (TO_ASCII(UPPER(tbl_posto.nome), 'LATIN-9') = TO_ASCII(UPPER('{$posto_nome}'), 'LATIN-9')));
			";
...
$title = traduz("CONSULTA DE PEDIDO NÃO FATURADO / FATURADO PARCIALMENTE");
...
<form name="frm_consulta_pedido_faturado" method="POST" action="<?echo $PHP_SELF?>"  class="form-search form-inline tc_formulario">
	...
	<div class='row-fluid'>
		...
		<div class="span2">
			<div class='control-group' >
				<label class="control-label" for="numero_pedido"><?=traduz('Nº Pedido')?></label>
		...
		<div class="span2">
			<div class='control-group <?=(in_array('status_pedido', $msg_erro['campos'])) ? "error" : "" ?>'>
				<label class="control-label" for="status_pedido"><?=traduz('Status do Pedido')?></label>
				<div class="controls controls-row">
					<div class="span12"><h5 class='asteristico'>*</h5>
						<select name="status_pedido" id="status_pedido">
							<option value=""><?=traduz('Selecione')?></option>
							<option value="1" <?php echo $status_pedido == "1" ? "selected" : ""; ?>><?=traduz('Ambos')?></option>
							<option value="2" <?php echo $status_pedido == "2" ? "selected" : ""; ?>><?=traduz('Não Faturado')?></option>
							<option value="3" <?php echo $status_pedido == "3" ? "selected" : ""; ?>><?=traduz('Faturado Parcialmente')?></option>
						</select>
					</div>
				</div>
			</div>
	...
	<div class='row-fluid'>
		...
		<input type="hidden" id="posto" name="posto" value="<?=$posto?>" />
			...
			<div class="span4">
					<div class='control-group' >
						<label class="control-label" for="posto_nome">Região Posto</label>
						<div class="controls controls-row">
							<div class="span10 input-append">
								<select id="regiao_posto" name="regiao_posto" class="span12">
									<option value="">Todos</option>
									<?php
									foreach ($array_regioes as $regiao) {
										$selected = ($regiao == $regiao_posto) ? "selected" : "";
	...
	<p>
		<button type="submit" class='btn' data-loading-text="Pesquisando..." id="btn_pesquisa" ><?=traduz('Pesquisar')?></button>
	</p>
	<br />
</form>
<?php
...
?>
	<p class="tac">
		<button type="button" class='btn btn-success gerar_csv' data-loading-text="Gerando arquivo..."><?=traduz('Gerar Arquivo CSV')?></button>
	</p>
	...
	<table id="resultado_pedido" class='table table-striped table-bordered table-large' style="margin: 0 auto;" >
		<thead>
			<tr class='titulo_coluna'>
				...
				?>
				<th><?=traduz('CNPJ Posto')?></th>
				<th><?=traduz('Peça')?></th>
				<th><?=traduz('Quantidade Pendente')?></th>
				<th><?=traduz('Preço')?></th>
			</tr>
		</thead>
		<tbody>
