...
*/
	...
	//guarda os parametros da ultima consulta
	$sql_parametros = "
	select 
		$todas::int4 as todas, 
...
/* hd 3024 takashi 18/07/2007 inicio
...
//	$res = @pg_exec ($con,"drop table $tmp_table". "_parametros;");
	...
	echo "drop table $tmp_table". "_parametros;";

	//grava os parametros da consulta atual, para sempre estar disponivel
	$sql_parametros .= " $tmp_table". "_parametros;";

	echo $sql_parametros;

	//$res = @pg_exec ($con,$sql_parametros );
...
if($todas == 1) {
	...
	if ($todas == 2 OR $todas == 3 ) { 
		...
		if($todas == 5){
			...
			//$res = pg_exec ($con,$sql);
			echo "<br> sql dele4:$sql<br> ";
		}
	}
}

echo "<br>Gerado com Sucesso<br>";
...
if($todas == 4) {
  ...
  if (strtoupper($btnacao) == "LISTAR") {
	...
	$sql = "Select * from $tmp_table"."_parametros;";
	$res = @pg_exec ($con,$sql);
	if(@pg_numrows($res)>0){
		...
		$estado         = trim(pg_result($res,0,linha));
		$todas          = trim(pg_result($res,0,todas));
	}
