<?
//somente a tectoy esta utilizando este callcenter. Alteramos o callcenter e liberamos para os fabricantes, a tectoy comecou a utilizar e uma semana depois a Hb Flex solicitou umas alterações que acabou resultando no novo callcenter interativo. Todos os fabricantes estao utilizando o callcenter interativo, somente a tectoy deixamos pq ela comecou a utilizar este e a Paula não quis mudar por enqto.
...
if($login_fabrica<>6){
	header ("Location: callcenter_interativo.php");
...
if(strlen($btn_acao)>0){
	...
	$numero_processo           = trim($_POST['numero_processo']); //HD 42763
...
if(strlen($callcenter)==0){
	...
	/*VALIDACAO DE DADOS*/
		...
		if(strlen($defeito_reclamado)==0) {
			if($login_fabrica==6 and ($natureza_chamado == 'Reclamação' or $natureza_chamado == 'Informação' or $natureza_chamado == 'Email' or $natureza_chamado == 'Ocorrência')){//hd 6135
					$msg_erro .="<BR>Escolha o defeito";
			}else{
		...
		if(strlen($numero_processo)==0) { $xnumero_processo ="null"; }else{ $xnumero_processo = "'".$numero_processo."'";}
	...
	/*CLIENTE*/
		...
		//insere o cliente
		// MUdado para inserir no tbl_hd_chamado_extra
	/*CLIENTE*/
...
//			$msg_erro = "aaa";
	...
	if(strlen($msg_erro)==0){
		if(strlen($transferir)>0){
			$sql = "UPDATE tbl_hd_chamado set atendente = $transferir, categoria='$natureza_chamado'
		...
		$sql = "UPDATE tbl_hd_chamado set categoria='$natureza_chamado'
					WHERE  tbl_hd_chamado.fabrica_responsavel = $login_fabrica
					and    hd_chamado = $callcenter";
		$res = pg_exec($con,$sql);
		$msg_erro .= pg_errormessage($con);
	...
	//		exit;
			...
			$sql = "SELECT fn_callcenter_intervalo_interacao($callcenter,$login_fabrica);";
			$res = pg_exec($con,$sql);
			$msg_erro .= pg_errormessage($con);
		}else{
			$res = pg_exec($con,"ROLLBACK TRANSACTION");
...
if(strlen($callcenter)>0){
	...
	$sql = "SELECT	tbl_hd_chamado_extra.hd_chamado as callcenter,
					...
					ELSE tbl_cliente.nome END,
					CASE WHEN tbl_hd_chamado_extra.endereco IS NOT NULL THEN tbl_hd_chamado_extra.endereco
					ELSE tbl_cliente.endereco END,
					CASE WHEN tbl_hd_chamado_extra.numero IS NOT NULL THEN tbl_hd_chamado_extra.numero
					ELSE tbl_cliente.numero END,
					CASE WHEN tbl_hd_chamado_extra.complemento IS NOT NULL THEN tbl_hd_chamado_extra.complemento
					ELSE tbl_cliente.complemento END,
					CASE WHEN tbl_hd_chamado_extra.bairro IS NOT NULL THEN tbl_hd_chamado_extra.bairro
					ELSE tbl_cliente.bairro END,
					CASE WHEN tbl_hd_chamado_extra.cep IS NOT NULL THEN tbl_hd_chamado_extra.cep
					ELSE tbl_cliente.cep END,
					CASE WHEN tbl_hd_chamado_extra.fone IS NOT NULL THEN tbl_hd_chamado_extra.fone
					ELSE tbl_cliente.fone END,
					CASE WHEN tbl_hd_chamado_extra.email IS NOT NULL THEN tbl_hd_chamado_extra.email
					ELSE tbl_cliente.email END,
					CASE WHEN tbl_hd_chamado_extra.cpf IS NOT NULL THEN tbl_hd_chamado_extra.cpf
					ELSE tbl_cliente.cpf END,
					CASE WHEN tbl_hd_chamado_extra.rg IS NOT NULL THEN tbl_hd_chamado_extra.rg
					ELSE tbl_cliente.rg END,
					tbl_cliente.cliente ,
		...
		LEFT JOIN tbl_cidade on tbl_cidade.cidade = tbl_hd_chamado_extra.cidade /* HD 43872 */
...
function Mostra_Processo(natureza_chamado){ /* HD 42763 */
	var style2 = document.frm_callcenter.natureza_chamado.value;
	if (style2.length > 0){
		if (style2=="Procon" || style2=="Jec"){
			if ($('#div_processo').style.display=="none"){
				$('#div_processo').style.display = "block";
			}
		}else{
			$('#div_processo').style.display = "none";
		}
	}
}
...
<form name="frm_callcenter" method="post" action="<?$PHP_SELF?>">
	...
	<table width="700" border="0" align="center" cellpadding="2" cellspacing="2" style='font-size:12px'>
					...
					<table width='700' border='0' align='center' cellpadding="2" cellspacing="2" style='border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
						<tr>
							...
							<td align='left'>
								<?
								if(strlen($callcenter)>0){ ?>
									<label class="respondido" style="width:100px" id="abertura_callcenter"><? echo "$abertura_callcenter";?></label>
								<? 
								}else{ ?>
								<input name="data_abertura_callcenter" id="data_abertura_callcenter" class="input" type="text" size="13" maxlength="10" value="<?	echo date("d/m/Y");	?>">
							...
							<td align='left'>
								<?
								if(strlen($callcenter)>0 and $login_fabrica <> 6){ ?>
									<label class="respondido"  style="width:100px" id="natureza_chamado"><? echo "$natureza_chamado";?></label>
								<? 
								}else{ 
								?>
									<select name="natureza_chamado" id="natureza_chamado" onChange="javascript:Mostra_Processo(this.value);" style='width:100px; font-size:9px'   <? if(strlen($callcenter)==0){ ?> onBlur='mostraDefeitos(document.frm_callcenter.natureza_chamado.value,document.frm_callcenter.produto_referencia.value)'<?}?>>
									<?
									if($login_fabrica==6){ ?>
										...
										<option value='Procon' <? /* HD 42763 */ if($natureza_chamado == 'Procon')       echo ' selected';?>>Procon</option>
										<option value='Jec'       <? if($natureza_chamado == 'Jec')       echo ' selected';?>>Jec</option>
									<?
									}else{ ?>
						...
						<tr>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:200px" id="consumidor_nome"><? echo "$consumidor_nome";?></label>
							<? }else{ ?>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="consumidor_cpf"><? echo "$consumidor_cpf";?></label>
							<? }else{ ?>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="consumidor_rg"><? echo "$consumidor_rg";?></label>
							<? }else{ ?>
						...
						<tr>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:200px" id="consumidor_email"><? echo "$consumidor_email";?></label>
							<? }else{ ?>
							...
							<td align='left'>
								<?	if(strlen($callcenter)>0){ ?>
									<label class="respondido" style="width:100px" id="consumidor_fone"><? echo "$consumidor_fone";?></label>
								<? }else{ ?>
							...
							<td align='left'>
								<?	if(strlen($callcenter)>0){ ?>
									<label class="respondido" style="width:100px" id="consumidor_cep"><? echo "$consumidor_cep";?></label>
								<? }else{ ?>
						...
						<tr>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido"  style="width:200px" id="consumidor_endereco"><? echo "$consumidor_endereco";?></label>
							<? }else{ ?>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="consumidor_numero"><? echo "$consumidor_numero";?></label>
							<? }else{ ?>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="consumidor_complemento"><? echo "$consumidor_complemento";?></label>
							<? }else{ ?>
						...
						<tr>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:200px" id="consumidor_bairro"><? echo "$consumidor_bairro";?></label>
							<? }else{ ?>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="consumidor_cidade"><? echo "$consumidor_cidade";?></label>
							<? }else{ ?>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="consumidor_estado"><? echo "$consumidor_estado";?></label>
							<? }else{ ?>
						...
						<? if($login_fabrica == 6){?>
							<tr>
								<? if(strlen($callcenter)>0){ ?>
									<td align='left'><strong>Nº Processo:</strong></td>
									<td colspan='3'>
										<label class="respondido" style="width:200px" id="numero_processo"><? echo "$numero_processo";?></label>
									</td>
								<? }else{ ?>
									<td colspan='4'>
										<div id='div_processo' style="display: none;" >
											<strong>Nº Processo:</strong>
											<INPUT TYPE="text" NAME="numero_processo" class="input" VALUE="<? echo $numero_processo; ?>">
										<div>
									</td>
								<? } ?>
							</tr>
		...
		<tr>
			<td align='left'>
				<span style=' position:absolute;float:left;'><strong>Informações do Produto</strong></span>
				<span style='position:relative;float:right;'><a href="javascript:minimizar('dados_2');"><img src='imagens_admin/btn_fechar.gif' border='0' alt='Minimizar'></a></span>
				<div id='dados_2' style='position:relative; display:block;width:700px;'>
					<table width='700' border='0' align='center' cellpadding="2" cellspacing="2" style='border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
						<tr>
							<td align='left' width='80'><strong>Assunto:</strong></td>
							<td align='left' width='230'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:200px" id="assunto"><? echo "$assunto";?></label>
							<? }else{ ?>
								<input name="assunto" value='<?echo $assunto ;?>'  <?if(strlen($callcenter)>0)echo "DISABLED";?>  class="input" type="text" size="35" maxlength="50">
							<? } ?>
							</td>
							<td align='left' width='50'><strong>OS:</strong></td>
							<td align='left' width='130'>
							<?	if(strlen($callcenter)>0){ ?>
									<label class="respondido" style="width:100px" id="sua_os"><? echo "$sua_os";?></label>
							<? }else{ ?>
								<input name="sua_os" value='<?echo $sua_os ;?>'  <?if(strlen($callcenter)>0)echo "DISABLED";?>  class="input" type="text" <?if(strlen($callcenter)>0)echo "DISABLED";?>  size="12" maxlength="10">
								<img src='imagens/lupa.png' border='0' align='absmiddle' style='cursor: pointer' onclick="javascript: fnc_pesquisa_os (document.frm_callcenter.sua_os.value)">
							<? } ?>
							</td>
							<td align='left' width='90'><strong>Data OS:</strong></td>
							<td align='left' width='100'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:80px" id="data_abertura"><? echo "$data_abertura";?></label>
							<? }else{ ?>
								<input name="data_abertura" id="data_abertura" value='<?echo $data_abertura ;?>'  <?if(strlen($callcenter)>0)echo "DISABLED";?>  class="input" type="text" size="11" maxlength="10" >
							<? } ?>
							</td>
						</tr>
						<tr>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido"  style="width:200px" id="produto_nome"><? echo "$produto_nome";?></label>
							<? }else{ ?>
								...
								<? if(strlen($callcenter)==0){ ?>
									mostraDefeitos(document.frm_callcenter.natureza_chamado.value,document.frm_callcenter.produto_referencia.value)<?}?>" type="text" size="35" maxlength="500">
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido"  style="width:100px" id="serie"><? echo "$serie";?></label>
							<? }else{ ?>
								<input name="serie" value='<?echo $serie ;?>'  <?if(strlen($callcenter)>0)echo "DISABLED";?>  class="input" type="text" size="16" maxlength="20">
							<? } ?>
							</td>
							<td align='left'><strong>Data Compra:</strong></td>
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido"  style="width:80px" id="data_nf"><? echo "$data_nf";?></label>
							<? }else{ ?>
								<input name="data_nf" id="data_nf"  value='<?echo $data_nf ;?>'  <?if(strlen($callcenter)>0)echo "DISABLED";?>  class="input" type="text" size="11" maxlength="10">
							<? } ?>
							</td>
						</tr>
						<tr>
							...
							<td align='left'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:100px" id="nota_fiscal"><? echo "$nota_fiscal";?></label>
							<? }else{ ?>
								<input name="nota_fiscal" value='<?echo $nota_fiscal ;?>'  <?if(strlen($callcenter)>0)echo "DISABLED";?>  class="input" type="text" size="10" maxlength="10">
							<? } ?>
							</td>
							<td align='left'><strong>Revenda:</strong> </td>
							<td align='left'  colspan='3'>
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:200px" id="revenda_nome"><? echo "$revenda_nome";?></label>
							<? }else{ ?>
								...
								<img src='imagens/lupa.png' border='0' align='absmiddle' style='cursor: pointer' onclick="javascript:fnc_pesquisa_revenda(document.frm_callcenter.revenda_nome, 'nome')">
						...
						<tr>
							...
							<?	if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:200px" id="posto_nome"><? echo "$posto_nome";?></label>
							<? }else{ ?>
								...
								<img src='imagens/lupa.png' border='0' align='absmiddle' style='cursor: pointer' onclick="javascript:fnc_pesquisa_posto(document.frm_callcenter.codigo_posto,document.frm_callcenter.posto_nome, 'nome')">
		...
		<tr>
			<td align='left'>
				<span style=' position:absolute;float:left;'><strong>Ocorrência</strong></span><span style='position:relative;float:right;'><a href="javascript:minimizar('dados_3');"><img src='imagens_admin/btn_fechar.gif' border='0' alt='Minimizar'></a></span>
				<BR>
				<div id='dados_3' style='position:relative; display:block;width:700px;'>
					<table width='700' border='0' align='center' cellpadding="2" cellspacing="2" style='border:#485989 1px solid; background-color: #e6eef7;font-size:10px'>
						<tr>
							<td align='left' valign='top' width='70'><strong>
							<?	if(strlen($callcenter)==0){ ?>
								<a href='javascript:mostraDefeitos(document.frm_callcenter.natureza_chamado.value,document.frm_callcenter.produto_referencia.value)'>
							<? } ?>
								Defeito:
							<?	if(strlen($callcenter)==0){ ?>
								</a>
							...
							<td align='left' colspan='5' width='630'>
						...
						<tr>
							...
							<? if(strlen($callcenter)>0){ ?>
								<label class="respondido" style="width:600px; height:60px" id="reclamado"><? echo "$reclamado";?></label>
							<? }else{ ?>
				...
				<? if(strlen($callcenter)>0){
					...
					if(pg_numrows($res)>0){
						for($x=0;pg_numrows($res)>$x;$x++){
							...
							$xx = $xx + 1;
							?>
							<table width='700' border='0' align='center' cellpadding="5" cellspacing="1" style=' border:#485989 1px solid; background-color: #A0BFE0;font-size:10px'>
								<tr>
				...
				<table width='700' border='0' align='center' cellpadding="2" cellspacing="2" style=' border:#006633 1px solid; background-color: #DCF1DF;font-size:10px'>
					...
					<tr>
						...
						<td align='left'>
							...
							<option value='Aberto'   <? if ($status=="Aberto") echo "SELECTED";?> >Aberto</option>
