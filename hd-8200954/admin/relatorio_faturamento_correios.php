...
if (isset($_REQUEST["conhecimento"])) {
	$conhecimento = $_REQUEST["conhecimento"];
	if(strlen(trim($conhecimento)) >13) {
		$conhecimento = preg_replace("/\W/","",$conhecimento);
		$conhecimentos = explode("BR",$conhecimento);	
		$conhecimento = implode ("BR','", $conhecimentos);
	}
}
...
$sql = "SELECT * from tbl_faturamento_correio 
		WHERE fabrica in  ($fabricas) and conhecimento in ('$conhecimento') order by conhecimento, data";
						...
						<?php						
							for($i=0; $i<pg_num_rows($res); $i++){
								...
								<tr class='produto-item' data-produto='".json_encode($r)."'>".
									 ...
									 "<td class='cursor_lupa'>{$situacao} - $conhecimento</td>".
