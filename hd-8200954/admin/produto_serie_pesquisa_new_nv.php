...
if ($login_fabrica == 158) { /*HD - 6088097*/
	$cliente_admin = $_REQUEST["cliente_admin"];

	if (strlen($cliente_admin) > 0) {
		$aux_sql = "SELECT parametros_adicionais FROM tbl_cliente_admin WHERE fabrica = $login_fabrica AND cliente_admin = $cliente_admin";
		$aux_res = pg_query($con, $aux_sql);
		
		$parametros_adicionais = json_decode(pg_fetch_result($aux_res, 0, 'parametros_adicionais'), true);

		if (in_array("6400", $parametros_adicionais["unidadeNegocio"])) {
			$retornar_tabela_garantia = 'true';
		} elseif ($parametros_adicionais["unidadePrincipal"] == "6400") {
			$retornar_tabela_garantia = 'true';
		} else {
			$retornar_tabela_garantia = 'false';
		}
	}
}
?>
...
<?php
        ...
        if ($login_fabrica == 52) {
        	$campos .= " ,tbl_numero_serie.nota_fiscal AS nota_fiscal_numero_serie, tbl_numero_serie.cnpj ";
        }
		...
		for($i = 0; $i < pg_num_rows($res); $i++) {
			...
			if (in_array($login_fabrica, array(30,74,158,165))) {
				...
				if ($login_fabrica == 158) {
					...
					$onclick .= ", '$retornar_tabela_garantia'";

					$auxiliar = explode("/", $data_fabricacao);
					$onclick .= ", '" . $auxiliar[2] . "'";
				}
				$mostraDefeitos = " window.parent.mostraDefeitos('Reclamado', '".$referencia."');";
			}

			if($login_fabrica == 52 && $bloqueada_garantia == "t" and strlen(trim($motivo)) > 0){
				?>
				...
				if($login_fabrica == 161){
                	...
                	if ($login_fabrica == 52) {
                		...
                		$nota_fiscal_numero_serie = pg_fetch_result($res, $i, 'nota_fiscal_numero_serie');
                        $cnpj                     = pg_fetch_result($res, $i, 'cnpj');

                        $aux_sql = "SELECT cliente_admin, nome FROM tbl_cliente_admin WHERE fabrica = $login_fabrica AND cnpj = '$cnpj'";
                        $aux_res = pg_query($con, $aux_sql);
                        
                        $cliente_admin      = pg_fetch_result($aux_res, 0, 'cliente_admin');
                        $cliente_admin_nome = pg_fetch_result($aux_res, 0, 'nome');
                		
                		list($yi, $mi, $di) = explode("-", $data_venda);
                		$xdata_venda        = "$di/$mi/$yi";

                		$onclick .= " ,'{$xdata_venda}' ,'{$nota_fiscal_numero_serie}' ,'{$cliente_admin}', '{$cliente_admin_nome}' ";
                	}
                    echo "<tr style='background: $cor' onclick=\"window.parent.retorna_serie($onclick);$mostraDefeitos window.parent.Shadowbox.close();\">
                        <td style='text-align: center;'>$serie</td>
