...
<form name="frm_cad_item" method="post" action="<? $PHP_SELF ?>" align='center' class='form-search form-inline tc_formulario'>	
		...
		<p><br/>
			<input class='btn' type="button" value="<?=$labelBtnGravar?>" ONCLICK="javascript: if (document.frm_cad_item.btnacao.value == '' ) { document.frm_cad_item.btnacao.value='gravar' ; document.frm_cad_item.submit() } else { alert ('Aguarde submissão') }" ALT="Gravar formulário" border='0'>
			<input class='btn' type="button" value="Limpar" ONCLICK="javascript: if (document.frm_cad_item.btnacao.value == '' ) { document.frm_cad_item.btnacao.value='reset' ; window.location='<? echo $PHP_SELF ?>' } else { alert ('Aguarde submissão') }" ALT="Limpar campos" border='0' >						
		</p><br/>
</form>
<br>
