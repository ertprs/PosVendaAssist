	...
	if (strlen($fabrica) > 0) {
		...
		if ($row > 0) {
			...
			for ($i = 0; $i < $row; $i++) {
				...
				$xml .= "<admin>\n";
					$xml .= "<id>".$admin."</id>\n";
					$xml .= "<login>".htmlspecialchars($login)."</login>\n";
				$xml .= "</admin>\n";
...
if (strlen($btn_acao) > 0) {
	...
	if (strlen($titulo) == 0) {//HD 711738
		$msg_erro .= "Por favor insira um titulo!<br />";
	} else if (strlen($titulo) < 5) {//HD 711738
		$msg_erro .= "Título muito pequeno <br />";
	}
	...
	if (strlen($msg_erro) == 0) {
		...
		/* 03/07/2013 - comentado para testes, se não der problema, deve ser retirado onde utiliza "Resolvido"
			if ($status == 'Resolvido') {
...
<tr>
	...
	<td bgcolor="#E5EAED" style='border-style: solid; border-color: #6699CC; border-width=1px'>&nbsp;<input type='text' size='30' name='email' maxlength='60' value='<?= $email ?>'></td>
...
$n_fabricas = pg_numrows($res);
	...
	<?
	echo "<td bgcolor='#E5EAED' style='border-style: solid; border-color: #6699CC; border-width=1px'>";
	echo "<select class='frm' style='width: 100px;' name='status' class='caixa'>\n";
	echo "<option value='Novo' "; 
	if($status == 'Novo'){echo "SELECTED";}
	echo " >Novo</option>\n";
...
<?
	$sql = "SELECT	tipo_chamado,
			...
			WHERE tipo_chamado in (4,5,6,10) 
			ORDER BY descricao;";
	$res = pg_exec($con,$sql);
	if(pg_numrows($res)>0){
...
echo "<textarea name='comentario' cols='50' rows='6' wrap='VIRTUAL'>".stripcslashes($comentario)."</textarea><br>";
echo "</td>";
echo "</tr>";
echo "</table>";
