...
document.write('.btnX     { width: 22px; height: 22px; border: 1px solid buttonface; margin: 0; padding: 0; }\n');
...
//  ['strikethrough','subscript','superscript','separator'],
    ['justifyleft','justifycenter','justifyright','justifyjustify','separator'],
...
this.btnList = {
    ...
    "italic":         ['Italic',               'Itálico',            'editor_action(this.id)',  'ed_format_italic.gif'],
    ...
    "superscript":    ['SuperScript',          'Superescrit',        'editor_action(this.id)',  'ed_format_sup.gif'],
    "justifyleft":    ['JustifyLeft',          'Alinhado à Esquerda','editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',        'Alinhado ao Centro', 'editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',         'Alinhado à Direita', 'editor_action(this.id)',  'ed_align_right.gif'],
    "justifyjustify": ['JustifyFull',          'Justificado',        'editor_action(this.id)',  'ed_align_justify.gif'],
    ...
    "outdent":        ['Outdent',              'Diminuir Identação', 'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               'Aumentar Identação', 'editor_action(this.id)',  'ed_indent_more.gif'],
    "forecolor":      ['ForeColor',            'Cor do Texto',       'editor_action(this.id)',  'ed_color_fg.gif'],
    ...
    "createlink":     ['CreateLink',           'Inserir Link',       'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          'Inserir Imagem',     'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',          'Inserir Tabela',     'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             'Ver código HTML',    'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          'Maximizar Editor',   'editor_action(this.id)',  'fullscreen_maximize.gif'],
    //"about":          ['about',                'Sobre este editor','editor_about(\''+objname+'\')',  'ed_about.gif'],

    // Add custom buttons here:
    "custom1":           ['custom1',         'Inserir imagem da Galeria',  'editor_action(this.id)',  'ed_custom.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      toolbar += '<button title="' +btnTitle+ '" id="_' +objname+ '_' +btnCmdID+ '" class="btnX" onClick="' +btnOnClick+ '" onmouseover="if(this.className==\'btnX\'){this.className=\'btnOver\'}" onmouseout="if(this.className==\'btnOver\'){this.className=\'btnX\'}" unselectable="on"><img src="' +config.imgURL + btnImage+ '" border=0 unselectable="on"></button>';
...
function editor_action(button_id) {
  ...
  // Custom1
    ...
    showModalDialog(_editor_url + "popups/insert_image.php", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    ...
    for (var idxN in tbItems) {
      ...
      if (action == "enable")  { tbObj.disabled = false; if (isBtn) { tbObj.className = 'btnX' }}
  ...
  for (i=0; i<IDList.length; i++) {
    ...
    if (!cmdActive)  {                                  // option is OK
      if (btnObj.className != 'btnX') { btnObj.className = 'btnX'; }
