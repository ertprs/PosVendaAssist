...
if (strlen ($btn_acao) > 0) {
	...
	if (strlen($msg_erro) == 0){
		$comentario_controle = $_POST['comentario_controle'];
		$horas_controle      = $_POST['horas_controle'];
		if(strlen($comentario_controle)>0 and strlen($horas_controle) and $login_admin = 432){
			$res = @pg_exec($con,"INSERT INTO tbl_hd_chamado_controle (hd_chamado, qtde_horas, admin_analista, admin_desenvolvedor, comentario) values ($hd_chamado, $horas_controle, 432,$transfere, $comentario_controle)");
		}
		$res = pg_exec($con,"BEGIN TRANSACTION");
...
<?if($login_admin == 432){?>
	<tr>
		<td  align='center' colspan='2'  class='sub_label'>
			<textarea name='comentario_controle' cols='60' rows='6' wrap='VIRTUAL' id='comentario_controle'><?echo $comentario_controle;?></textarea><br>
			Horas previstas<input type='text' size='2' maxlength ='5' name='horas_controle' value='<?= $horas_controle ?>' class='caixa' >
		</td>
	</tr>
<?}?>
</table>
