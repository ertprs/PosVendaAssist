...
if($_POST['_POST'] == 'teste1'){
	...
	$sql = 'UPDATE  tbl_hd_chamado 
			   SET  procedimento_teste = \''.$procedimento_teste.'\',
					...
					admin_procedimento_teste = \''.$login_admin.'\'
			 WHERE  hd_chamado = '.$_POST['chamado'].';';
	$res = pg_query($con,$sql);
	$msg_erro .= pg_last_error($con);
...
if($_POST['_POST'] == 'efetivacao1'){
	...
	$sql = 'UPDATE  tbl_hd_chamado 
			   SET  procedimento_efetivacao = \''.$procedimento_efetivacao.'\',
			        comentario_efetivacao   = \''.$comentario_efetivacao.'\',
					admin_efetivacao        = \''.$login_admin.'\'
			 WHERE  hd_chamado = '.$_POST['chamado'].';';
	$res = pg_query($con,$sql);
	$msg_erro .= pg_last_error($con);
...
if($_POST['_POST'] == 'orcamento1'){
	...
	if(strlen($msg_erro) > 0){
		...
		$mensagem_orcamento = "
	<p>Estimativa de Horas<br />
	Conforme o levantamento realizado, apresentamos o orçamento para atender este chamado. Serão utilizadas as quantidades de horas a seguir:</p>
	<dl>\n";
		if (is_numeric($taxa_abertura))    $mensagem_orcamento .= "<dt>Taxa de Abertura de Chamado</dt><dd>$taxa_abertura</dd><br />\n";
		if (is_numeric($horas_suporte))    $mensagem_orcamento .= "<dt>Horas Suporte</dt><dd>$horas_suporte</dd><br />\n";
		if (is_numeric($horas_telefone))   $mensagem_orcamento .= "<dt>Horas Suporte Telefone</dt><dd>$horas_telefone</dd><br />\n";
		if (is_numeric($horas_analise))    $mensagem_orcamento .= "<dt>Horas Análise</dt><dd>$horas_analise</dd><br />\n";
		if (is_numeric($prazo_horas))      $mensagem_orcamento .= "<dt>Horas Desenvolvimento</dt><dd>$prazo_horas</dd><br />\n";
		if (is_numeric($horas_teste))      $mensagem_orcamento .= "<dt>Horas Teste</dt><dd>$horas_teste</dd><br />\n";
		if (is_numeric($horas_efetivacao)) $mensagem_orcamento .= "<dt>Horas Efetivação</dt><dd>$horas_efetivacao</dd><br />\n";
		$mensagem_orcamento .= "<dt>Total Horas</dt><dd>$total_horas</dd>
	</dl>
<p>A aprovação deste orçamento está disponível na tela do supervisor, localizada na área de Help Desk, bastando clicar no botão <APROVAR ORÇAMENTO>.</p>";
		$res = pg_begin();
		$sql = " INSERT INTO tbl_hd_chamado_item (
						hd_chamado,
						comentario,
...
#Ao enviar o formulário de Análise, faz as validações
if($_POST['_POST'] == 'check1' OR $_GET['check1']==1 ){
		...
		if (strlen ($comentario) > 0) {
			...
			if (strlen($msg_erro) == 0 and strlen($hd_chamado_item) > 0) {
				...
				if (strlen($arquivo["tmp_name"]) > 0 && $arquivo["tmp_name"] != ""){
					...
					$ext          = strtoloweR($a_att_info['extension']);
...
<? if(in_array($login_admin,array(822,398,1375,852))) {
	echo "<script type='text/javascript' src='../admin/js/fckeditor/fckeditor.js'></script>";
}
?>
<script>
<? if(in_array($login_admin,array(398,822,852,1375))) { ?>
...
<tr>
	...
	<td  class='sub_label' align='center'><?='('.$fabrica.') - ' . $fabrica_nome ?> </td>
</tr>
			...
			<li class="purpleHD">
				...
				<div style="display:none;">
					...
					<?php endif;?>
						...
						<table border='0' cellpadding='2' class="table_analise" id="tbl_correcao_<?php echo $rodadaCorrecao;?>">
							<tr>
								<th align="left">Taxa de Abertura de Chamado</th>
								<th align="left"  width="35%">
									<input type="text" size="2" maxlength="5" name="taxa_abertura" value="<?php echo ($taxa_abertura)?$taxa_abertura:'1';?>" class="caixa horas"> h
								</th>
							</tr>
							<tr>
				...
				<div style="display:none;">
					...
					<?php endif;?>
						...
						<div>
							...
							<textarea name="comentario_efetivacao" id="comentario_efetivacao" class="textareaEfetivacao"><?php echo $comentario_efetivacao;?></textarea>
						</div>
					<?php 
					#HD 351094
					if($atualizacaoDados):?>
		...
		<td valign='top'>
			<table width = '225px' align = 'center' class='tab_cabeca'  cellpadding='2' cellspacing='1' border='0' >
				...
				<tr>
					...
					<td class ='sub_label'  align = 'center' >
						<select name="status" size="1"  style='width: 150px;'>
							...
							<option value='Pré-Análise'   <? if($status=='Pré-Análise')   echo ' SELECTED '?> >Pré-Análise</option>
				...
				<? if($analista_hd == "sim"){ ?>
					...
					<?php
					if($grupo_admin == 1 or $grupo_admin == 2 or $grupo_admin == 6 or $grupo_admin == 7) {
					?>
					<tr>
						<td  class ='sub_label'><strong>Horas Franqueada Terceiros<span title='Hora interna que será pago para o analista terceirizado'>(?)</span></strong></td>
						<td  class ='sub_label'align='center'>
						<input type='text' size='2' maxlength ='5' name='horas_franq_ter' value='<?= $horas_franq_ter ?>' class='caixa' onblur="javascript:checarNumero(this);atualizaPrazo_terceiro('1','<?echo $hd_chamado;?>',this.value)"> h
						<div id='msg_result_1' style='position:absolute; display:none; border: 1px solid #949494;background-color: #F1F0E7;width:150px;'>
						</div>
						</td>
					</tr>
					...
					<tr>
						<td  class ='sub_label'><strong>Horas Faturada para Terceiros<span title='Faturar horas para terceirizado'>(?)</span></strong></td>
						<td  class ='sub_label'align='center'>
						<input type='text' size='2' maxlength ='5' name='horas_fat_ter' value='<?= $horas_fat_ter ?>' class='caixa' onblur="javascript:checarNumero(this);atualizaPrazo_terceiro('2','<?echo $hd_chamado;?>',this.value)"> h
						<div id='msg_result_2' style='position:absolute; display:none; border: 1px solid #949494;background-color: #F1F0E7;width:150px;'>
						</div>
						</td>
					</tr>
					<?php
...
/*--=== ARQUIVOS SOLICITADOS ==================================================--*/
	/* MLG - 23-08-2011 - Tirar a carga de dados fixa, deixar apenas um botão para carregar os dados dos arquivos */?>
	<br>
	<div class='exibe' id='dados' value='1' align='center' style='font-size: 9px;'>
		<button type='button' style='margin: auto' onClick="document.getElementById('dados').style.visibility = 'visible'; Exibir('dados','','','');">Arquivos Requisitados</button>
	</div>
<?
if(!in_array($login_admin,array(398,822,852,1375))){
