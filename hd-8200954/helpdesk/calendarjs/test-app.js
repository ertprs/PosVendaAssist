	var eventStoreList = new Ext.data.JsonStore({
				id: 'eventStore',
				root: 'evts',
				url: 'calendario.php?gerar=sim&consulta_admin='+consulta_admin, // defined in event-list.js
	...
	desenvolvedor.on('select',function(pesquisa,record){
		var valor = record.data.admin;
		window.location.href ='calendario.php?consulta_admin='+valor;
	});
...
App = function() {
    return {
        init : function() {
			...
			Ext.BLANK_IMAGE_URL = './calendarjs/s.gif';
		...
		// A sample event store that loads static JSON from a local file. Obviously a real
			...
			// provide custom data mappings for events, see EventRecord.js.o
			this.eventStore = eventStoreList;
			this.atualizar =  new Ext.Toolbar({
				width:'140',
				cls:'border',
				items: [
				{
				...
				},
				{
					...
					scale: 'large',
					handler:function(){
							window.location.href ='calendario.php?consulta_admin=99999';
					}
				}] 
			});
			...
			new Ext.Viewport({
				...
				items: [ {
					...
					items: [{
						...
						desenvolvedor,
						{xtype: 'tbspacer',width:10 , height:10},
						{xtype: 'tbtext', text: 'Requisitos', cls:'ext-color-1'},
						{xtype: 'tbtext', text: 'Analise', cls:'ext-color-2'},
						{xtype: 'tbtext', text: 'Aguardando Execu&ccedil;&atilde;o', cls:'ext-color-3'},
						{xtype: 'tbtext', text: 'Aguardando Admin', cls:'ext-color-9'},
						{xtype: 'tbtext', text: 'Execu&ccedil;&atilde;o ou Corre&ccedil;&atilde;o', cls:'ext-color-4'},
						{xtype: 'tbtext', text: 'Or&ccedil;amento', cls:'ext-color-5'},
						{xtype: 'tbtext', text: 'Valida&ccedil;&atilde;o', cls:'ext-color-6'},
						{xtype: 'tbtext', text: 'Efetiva&ccedil;&atilde;o', cls:'ext-color-7'},
						{xtype: 'tbtext', text: 'Outros', cls:'ext-color-8'}
						]
					},{
						xtype: 'calendarpanel',
...
//									this.clearMsg();
        ...
        updateTitle: function(startDt, endDt){
            ...
            if(startDt.clearTime().getTime() == endDt.clearTime().getTime()){
                p.setTitle(startDt.format('F j, Y') + ' '+ nome_admin);
            }
            else if(startDt.getFullYear() == endDt.getFullYear()){
                if(startDt.getMonth() == endDt.getMonth()){
                    p.setTitle(startDt.format('F j') + ' - ' + endDt.format('j, Y')+ ' '+ nome_admin);
                }
                else{
                    p.setTitle(startDt.format('F j') + ' - ' + endDt.format('F j, Y') + ' '+ nome_admin);
                }
            }
            else{
                p.setTitle(startDt.format('F j, Y') + ' - ' + endDt.format('F j, Y') + ' '+ nome_admin);
            }
        },
        ...
        showMsg: function(msg){
			Ext.mensagem.msg2(msg);
        },
        
        clearMsg: function(){
			Ext.mensagem.msg2('retirado');
        }
    }
}();
