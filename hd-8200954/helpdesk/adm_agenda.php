	...
	if (strlen ($aux_fabrica) > 0) {
		...
		if ($n_resu > 0) {
			...
			for ($i=0; $i<$n_resu; $i++){
				...
				//$resposta .= "<form name='frm_admin' method='post' action='$PHP_SELF '><center><input type='submit' name='btn_acao' value='Logar' class='botao2'><input type='hidden' value='$resu_admin'name='admin'></center></form>";  // comentado pois da erro no IE
				$resposta .= "<a href='$PHP_SELF?btn_acao=Logar&admin=$resu_admin'>Logar</a>";
				$resposta .= "</td>";
...
function retornaAgenda (http , componente ) {
	var com = document.getElementById(componente);
	if (http.readyState == 1) {
		com.innerHTML   = "<font size='1'>Carregando...<br><img src='../imagens/carregar_os.gif'>";
	}
		...
		if (http.status == 200) {
			...
			if (typeof (results[0]) != 'undefined') {
				if (results[0] == 'ok') {
					...
					com.innerHTML   = "<h4>Selecione a Fábrica</h4>";
				}
			}else{
...
function ExibirAgenda (fabrica,dados) {
	
	url = "<?=$PHP_SELF?>?fabrica=" + escape(fabrica)+"&ajax=sim" ;
	http.open("GET", url , true);
	http.onreadystatechange = function () { retornaAgenda (http , dados) ; } ;
	http.send(null);
}
...
echo "<input type='button' name='btn_acao' value='Listas Usuários' class='botao2' onclick=\"ExibirAgenda(document.frm_agenda.fabrica.value,'dados')\">";
echo "<BR><BR>";
