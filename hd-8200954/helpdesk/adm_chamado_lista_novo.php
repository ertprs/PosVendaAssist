...
<tr valign='middle' align='center'>
	<td >ATENÇÃO - TODAS AS EMPRESAS ESTÃO NO NOVO PROCESSO A PARTIR DE 09/07/2010
	</td>
</tr>
</table><BR>";
$lista_novo_processo = '(46,50,10,14,43,20,2,52,30,1,24,80,3,5,15,42,47 ,89,86,7,51,85,8,87,11,19,72,40,66,5,45,61,88,77,76,78,75,81,59,6)';
...
if(!empty($data_inicial) || !empty($data_final)) {
	...
	$data_aux = explode("/", $data_inicial);

	if(empty($data_inicial) || strlen($data_inicial) != 10 || !checkdate($data_aux[1], $data_aux[0], $data_aux[2])) {
		$msg_erro[] = "Data inicial inválida";
	}

	$data_aux = explode("/", $data_final);

	if(empty($data_final) || strlen($data_final) != 10 || !checkdate($data_aux[1], $data_aux[0], $data_aux[2])) {
		$msg_erro[] = "Data final inválida";
	}

	if(!count($msg_erro)) {
		$date_diff = date_diff(new DateTime($aux_data_inicial), new DateTime($aux_data_final));

		if($date_diff->days && $date_diff->invert) {
...
if($atendente_busca <> '') {
	...
	//Samuel 09/12/2009 - Não mostrar Gustavo quando for igual a todos
	$cond1 = " /* AND tbl_hd_chamado.atendente <> 852 */";
}
...
$sql = "SELECT  hd_chamado,
				...
				case when tbl_hd_chamado.fabrica in $lista_novo_processo then '*' else null END as novo_processo,
		...
		if(!empty($aux_data_inicial) && !count($msg_erro)) {

			if(!is_array($status_periodo[$status])) {
				$sql .= " AND    tbl_hd_chamado." . ((isset($status_periodo[$status]) && !empty($status_periodo[$status])) ? $status_periodo[$status] : "data") . " BETWEEN '$aux_data_inicial 00:00:00' AND '$aux_data_final 23:59:59'";
			} else {
				foreach ($status_periodo[$status] as $key => $value) {
					$sql .= " AND tbl_hd_chamado.$key " . ($value === false ? "IS NOT NULL" : ($value === true ? "IS NULL" : "= $value")) . "\n";
				}
			}
		}
	...
	if (!count($msg_erro) && isset($status_periodo[$status]) && empty($status_periodo[$status]) && !empty($aux_data_inicial)) {
		//$status_busca = 'r';
	...
	//Samuel 09/12/2009 - Não mostrar Gustavo quando for igual a todos
		$sql .= " /* AND tbl_hd_chamado.atendente <> 852 */";
	}
...
*/	echo "<tr >";
	...
	for ($x = 0 ; $x < pg_numrows($resfabrica) ; $x++){
		...
		echo "<option value='$nome'"; if ($fabrica_busca == $nome) echo " SELECTED "; echo ">($fabrica)$nome</option>\n";
	}
...
#===========================
	...
	/*--===============================TABELA DE CHAMADOS========================--*/
	echo "<table width='650' id='chamados_erro' align='center' cellpadding='0' cellspacing='0' border='0'>";
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS DE ERRO</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	...
	echo "	<th nowrap><strong>Previsão Interna</strong></td>";
	...
	if (@pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
			...
			$novo_processo            = pg_result($res,$i,novo_processo);
			...
			echo "$novo_processo$hd_chamado</td>";
			...
			echo "<td width='80' name='data_filtro'><font size='1'>$data</font></td>";
			...
			echo "<td nowrap><font size='1'>$xxatendente</font></td>";
...
$sql = "SELECT  hd_chamado,
				...
				tbl_hd_chamado.cobrar,
				tbl_hd_chamado.prioridade,
				to_char (tbl_hd_chamado.data_aprovacao,'DD/MM HH24:MI') AS data_aprovacao,
				tbl_hd_chamado.hora_desenvolvimento,
				tbl_hd_chamado.prazo_horas,
				case when tbl_hd_chamado.fabrica in $lista_novo_processo then '*' else null END as novo_processo,
				tbl_fabrica.nome AS fabrica_nome,
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS DE DESENVOLVIMENTO APROVADO PELO FABRICANTE - FALTA PASSAR PARA ANALISTA E PREVISÃO DE TÉRMINO</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
	...
	if (@pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
			...
			//$posto                    = pg_result($res,$i,posto);
			$data                     = pg_result($res,$i,data);
			$data_aprovacao           = pg_result($res,$i,data_aprovacao);
			$titulo                   = pg_result($res,$i,titulo);
			$status                   = pg_result($res,$i,status);
			$atendente                = pg_result($res,$i,atendente);
			$exigir_resposta          = pg_result($res,$i,exigir_resposta);
	...
	//		echo $sql2;
			...
			echo "<td width='80' name='data_filtro'><font size='1'>";
			...
			echo "</font></td>";
			echo "<td nowrap ><font size='1'>$fabrica_nome</font></td>";
			echo "<td nowrap><font size='1'>$xxatendente</font></td>";
			echo "<td nowrap align='center'><font size='1'>$horas</font></td>";
			echo "<td nowrap align='center'><font size='1'>$prazo_horas</font></td>";
			echo "<td><font size='1' nowrap>&nbsp;$previsao_termino_interna&nbsp;</font></td>";
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS DE DESENVOLVIMENTO QUE ESTÃO COM HORAS DE DESENVOLVIMENTO - FALTA PASSAR PARA ANALISTA E PREVISÃO DE TÉRMINO</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
...
$sql = "SELECT  hd_chamado,
		...
		$sql .= "
		AND    tbl_hd_chamado.atendente IN (435)
		AND    tbl_hd_chamado.data_aprovacao is null
...
//$sql .= " ORDER BY tbl_hd_chamado.atendente,tbl_hd_chamado.previsao_termino,
//		tbl_hd_franquia.hora_franqueada DESC,
//		tbl_hd_chamado.data ASC;";
$sql .= " ORDER BY tbl_hd_chamado.data ASC, tbl_hd_chamado.atendente,tbl_hd_chamado.previsao_termino,
		tbl_hd_franquia.hora_franqueada DESC;";
//echo $sql;
if(!count($msg_erro)) {
	$res = pg_exec ($con,$sql);
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS DE DESENVOLVIMENTO QUE ESTÃO SEM PRAZO NEM DE HORAS DE DESENVOLVIMENTO</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
...
$sql .= " ORDER BY tbl_hd_chamado.previsao_termino,
		hora_utilizada / (case when hora_franqueada + saldo_hora > 0 then hora_franqueada +saldo_hora else 1 end)  ASC,
		tbl_hd_franquia.hora_franqueada DESC,
		tbl_hd_chamado.data ASC;";
if(!count($msg_erro)) {
	$res = pg_exec ($con,$sql);
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS QUE ESTÃO PEDENTES PARA O FABRICANTE E SEM PRAZO</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
...
$sql = "SELECT  hd_chamado,
		...
		AND    tbl_hd_chamado.titulo <> 'Atendimento interativo'
		AND    tbl_hd_chamado.status = 'Aguard.Admin'
		";
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS PENDENTES AGUARDANDO ADMIN</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
	...
	//		echo $sql2;
		...
		echo "</tbody>";
		echo "</tfoot>";
		echo "<tr><td colspan='100%' align='right' nowrap>";
		$total_outros = pg_numrows($res);
		//echo "<font size='2'><b>Total $total_outros chamados</b></font>";
		echo "<font size='2'><b>Total $total_outros chamados - Total: $total_prazo_horas horas</b></font>";
		echo "</td></tr>";
...
$sql = "SELECT  hd_chamado,
		...
		LEFT JOIN tbl_hd_franquia ON tbl_hd_chamado.fabrica = tbl_hd_franquia.fabrica
								 ...
								 AND tbl_hd_franquia.ano = to_char(current_date,'YYYY')::integer
		WHERE  tbl_hd_chamado.fabrica_responsavel = $login_fabrica
		AND    tbl_hd_chamado.atendente NOT IN (435)
		AND    tbl_hd_chamado.titulo <> 'Atendimento interativo'
		AND    tbl_hd_chamado.status <> 'Aguard.Admin'
		";
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS PENDENTES COM ANALISTAS</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='../imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
...
echo "<BR>";
	...
	$sql = "
			...
			WHERE data  BETWEEN  (current_date||' 00:00:00')::timestamp AND (current_date||' 23:59:00')::timestamp
				...
				AND tbl_hd_chamado.titulo <> 'Atendimento interativo'
				AND tbl_hd_chamado.admin is not null
	...
	if (@pg_numrows($res) > 0) {
		...
		echo "<thead>";
		echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS ABERTOS</CENTER></th></tr>";
		echo "<tr bgcolor='#D9E8FF' >";
	...
	$sql = "
			...
			WHERE data_resolvido BETWEEN  (current_date||' 00:00:00')::timestamp AND (current_date||' 23:59:00')::timestamp
				...
				AND tbl_hd_chamado.titulo <> 'Atendimento interativo'
				AND tbl_hd_chamado.admin is not null
				AND tbl_hd_chamado.tipo_chamado IS NOT NULL
			GROUP by tbl_fabrica.nome
			ORDER BY COUNT(tbl_hd_chamado.hd_chamado) DESC";
	...
	if (@pg_numrows($res) > 0) {
		echo "<table width = '300' align = 'center' cellpadding='2' cellspacing='1' border='0' class='relatorio'>";
		echo "<thead>";
		echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS RESOLVIDOS HOJE</CENTER></th></tr>";
		echo "<tr bgcolor='#D9E8FF' >";
		echo "	<th ><strong>Fabrica</strong></th>";
		echo "	<th >QTDE</th>";
		echo "</tr>";
	...
	if (@pg_numrows($res) > 0) {
		echo "<table width = '300' align = 'center' cellpadding='2' cellspacing='1' border='0' class='relatorio'>";
		echo "<thead>";
		echo "<tr bgcolor='#D9E8FF'><th colspan=12><CENTER>CHAMADOS DE TELEFONE ABERTOS HOJE </CENTER></th></tr>";
		echo "<tr bgcolor='#D9E8FF' >";
		echo "	<th ><strong>Fabrica</strong></th>";
		echo "	<th >QTDE</th>";
		echo "</tr>";
