...
echo "<tr style='font-family: arial ; color: #666666' align='center'>";
	...
	echo "<td nowrap align='left'>";
		...
		if($sistema_lingua == 'ES') echo "&nbsp;Pendiente Telecontrol&nbsp;";
		else                        echo "&nbsp;<a href='$PHP_SELF?status=Análise&exigir_resposta=f'>Pendente Telecontrol</a>&nbsp;";
	echo "</td>";
	echo "<td nowrap align='left'";
		if($sistema_lingua == 'ES')
			{ echo "title='Aguardando su respuesta'";
		}else{
			echo "title='Aguardando sua resposta'";
		}
		echo ">";
		...
		if($sistema_lingua == 'ES')
			...
			echo "&nbsp;<a href='$PHP_SELF?status=Análise&exigir_resposta=t'>Pendente</a>&nbsp;";
		}
	echo "</td>";
	echo "<td nowrap align='left'>";
...
if($filtro==1){
	...
	echo "<tr align='left'  height ='70' valign='top'>";
		...
		echo "</tr>";
		echo "<tr>";
			echo "<td align='center' colspan=2>Título</td>";
		echo "</tr>";
		echo "<tr>";
			echo "<td align='center' colspan=2><input type='text' name='titulo' id='titulo' value=$titulo></td>";
		echo "</tr>";
	...
	if(strlen($titulo)>0){
		...
		if(strlen($status) >0 ){
			if($exigir_resposta == "f" and ($status =="Análise" OR $status =="Execução" OR $status =="Novo")) {
				$cond_status = " tbl_hd_chamado.exigir_resposta IS NOT TRUE and (tbl_hd_chamado.status = 'Análise' OR tbl_hd_chamado.status = 'Execução' OR tbl_hd_chamado.status = 'Novo' ) ";
