...
class Mult_Pag {
	...
	*/ 
	function Executar($sql, $total_sql, $conexao, $velocidade, $tipo) 
	{ 
		// variavel para o inicio das pesquisas 
		$inicio_pesq = $this->pagina * $this->num_pesq_pag; 

		// RETIRADO, POIS O SQL COM COUNT() DEVE SER PASSADO PELA FUNCAO, E NAO MAIS ALTERACAO PELA CLASSE
		if ($total_sql == ""){
	...
	{ 
		...
		for ($atual = 0; $atual < $num_mult_pag; $atual++) { 
			...
			if ((($opcao == "todos") || ($opcao == "strings")) && ($atual == 0)) { 
				if ($this->pagina != 0){ 
					$array[++$indice] = '<A HREF="' . $arquivo . '?pagina=' . ($this->pagina - 1) . $extra_vars . ' class="thickbox" title="As palavras cadastradas" ">' . $this->str_anterior . '</A>'; 
	...
	*/ 
	...
	/*
		Retorna número de total de registros
	*/
	function Retorna_Resultado()
	{
		$registros = $this->total_reg;
		return $registros;
	}
