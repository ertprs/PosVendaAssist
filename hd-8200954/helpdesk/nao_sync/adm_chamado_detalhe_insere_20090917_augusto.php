	...
	if (strlen($msg_erro) == 0){
		...
		if (strlen ($msg_erro) == 0) {
			...
			if (strlen($arquivo["tmp_name"]) > 0 && $arquivo["tmp_name"] != "none"){
				...
				if (!preg_match("/\/(pdf|msword|pjpeg|jpeg|png|gif|bmp|vnd.ms-excel|richtext|plain|zip|x-zip|x-zip-compressed|octet-stream|x-compress|x-compressed|x-zip)$/", $arquivo["type"])){
...
<tr>
	...
	<td bgcolor="#E5EAED" style='border-style: solid; border-color: #6699CC; border-width=1px' colspan='3'>&nbsp;<input type='text' size='30' maxlength='30'  name='nome' value='<?= $nome ?>'></td>
</tr>
