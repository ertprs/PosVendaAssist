...
if($_POST['_POST'] == 'correcao1'){
	...
	for($i=0;$i<$totalCorrecaos;$i++){
		...
		$descricaoCorrecaos = filter_var($descricaoCorrecaos, FILTER_SANITIZE_STRING);
		$analiseCorrecaos = $_POST['analiseCorrecaos'][$i];
		$analiseCorrecaos = filter_var($analiseCorrecaos, FILTER_SANITIZE_STRING);
...
if($_POST['_POST'] == 'check1' OR $_GET['check1']==1 ){
	...
	$sqlCheck = "SELECT hd_chamado 
				...
				AND   checklist = ".$idCheck;
	$resCheck = pg_query($con,$sqlCheck);
	$numCheck = pg_numrows($resCheck);
...
function atualizaChecklist(opcao,check){
		$.ajax({
			...
			cache: false,
				success: function(data){
					$('#checkRes_'+check).html('Admin Responsável: '+data);
				}
		});
	}
								...
								<?php if($totalCheckList > 0):?>
									...
									$sqlAdmin = 'SELECT tbl_admin.nome_completo
												...
												JOIN tbl_admin ON (tbl_admin.admin = tbl_hd_chamado_checklist.admin)
												 ...
												 AND tbl_hd_chamado_checklist.hd_chamado = '.$hd_chamado.'';
									...
									if($num_checklist):
										...
										?>
										<tr>
											<td colspan="5" align="right" class="admin" id='checkRes_<?php echo $checklist_id; ?>'>Admin Responsável: <?php echo $admin_checklist;?></td>
										</tr>
									<?php else:?>
										<tr>
											<td colspan="5" align="right" class="admin" id='checkRes_<?php echo $checklist_id; ?>'></td>
										</tr>
									<?php endif;?> 
									<?php endfor;?>
