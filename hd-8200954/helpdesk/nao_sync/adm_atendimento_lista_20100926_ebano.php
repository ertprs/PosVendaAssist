...
<? if(in_array($login_admin,array(822,1375,1553,1630))) { ?>
...
echo "<div style='text-align:left;float:left' id='tempo'></div>";
...
if ($login_admin == 1819) {
	$sql .= " ORDER BY tbl_hd_chamado.prioridade DESC, tbl_hd_chamado.tipo_chamado DESC, tbl_hd_chamado.status, tbl_hd_chamado.previsao_termino_interna ASC,tbl_hd_chamado.previsao_termino ASC,tbl_hd_chamado.data DESC ";
}
else {
...
//		$posto                    = pg_fetch_result($res,$i,posto);
		...
		$atrasou                  = trim(pg_fetch_result($res,$i,atrasou));
		$atrasou_interno          = trim(pg_fetch_result($res,$i,atrasou_interno));
		$cobrar                   = trim(pg_fetch_result($res,$i,cobrar));
		$hora_desenvolvimento     = trim(pg_fetch_result($res,$i,hora_desenvolvimento));
		$esta_agendado            = trim(pg_fetch_result($res,$i,esta_agendado));
		...
		if ($prioridade == 't'){
			$cor=($login_admin==822) ? '#CCFFCC':'#FFD5CC';
		}
		...
		if ($atrasou == '1'){
			...
			$cor=($login_admin == 822) ? '#FFFF99' : '#FF3333';
		}
...
### PÉ PAGINACAO###
	...
	if($login_admin == 852){
		...
		if (@pg_num_rows($res) > 0) {
			...
			echo "</tr>";
			$count= pg_fetch_result($res,0,count);
			$horas= pg_fetch_result($res,0,horas);
	...
	if($login_admin == 822) {
		...
		echo "<a href='adm_chamado_lista.php?KeepThis=true&TB_iframe=true&height=590&width=940' class='fisheyeItem2 thickbox' ><span id='total_chamado'></span><img src='imagem/fisheye/firefox.png' width='30' /></a>";
		echo "<a href='adm_chamado_lista_novo.php?KeepThis=true&TB_iframe=true&height=590&width=940' class='fisheyeItem2 thickbox'><span id='resolvido'></span><img src='imagem/fisheye/opera.png' width='30' /></a>";
		echo "<a href='adm_relatorio_diario.php?KeepThis=true&TB_iframe=true&height=590&width=940' class='fisheyeItem2 thickbox' ><span id='aberto'></span><img src='imagem/fisheye/ie.png' width='30' /></a>";
		echo "<a href='#' class='fisheyeItem2' ><span id='meu' style='display:inline'></span><img src='imagem/sticky_thumb.png' width='30' id='sticky'/></a>";
		echo "<a href='#' class='fisheyeItem2' id='statistic'><span></span><img src='imagem/statistic.png' width='30' /></a>";
		echo "<a href='#' class='fisheyeItem2' id='relogio'><span></span><img src='imagem/clock.png' width='30' /></a>";
		echo "</div>";
		echo "</div>";
	}
}
