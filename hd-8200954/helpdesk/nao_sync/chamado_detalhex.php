	...
	if(strlen($hd_chamado)>0){
		...
		if (pg_numrows($res) > 0) {
			...
			if (pg_numrows($res) > 0) {
				...
				$titulo_anterior_tmp     = pg_result($res,0,titulo);
				$titulo_anterior         = str_replace("'", "", $titulo_anterior_tmp);
				# se quiser reabrir o mesmo, setar a variável "REABRIR MESMO"  caso contrario
				# somente reabrir....vai abrir um erro. COmo já foi validado com o Suporte, não será
				# possivel reabrir...e se colocar reabrir...o chamado esta voltando para o admin que
				# desenvolveu...e isto nao poderá.
