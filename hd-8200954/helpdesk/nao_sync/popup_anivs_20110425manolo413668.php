<?php
/*  Funções */
include_once('/var/www/assist/www/helpdesk/mlg_funciones.php');
...
if (!$included) {
	...
	if (strpos($PHP_SELF, 'assist/')) {
		...
		if (strpos($PHP_SELF, '/admin')) {
			include_once '/var/www/assist/www/admin/autentica_admin.php';
			$e_admin = true;
		} else {
...
if (strpos($PHP_SELF, 'mlg/')) {
	...
	// Recupera o admin ID
	$sql = "SELECT admin, nome_completo FROM tbl_admin WHERE fabrica = $login_fabrica AND login = '$usr_login'";
	...
	$sql = "SELECT login_unico, nome FROM tbl_login_unico WHERE nome ~* '$usr_login' AND posto = $login_posto";
	$res = pg_query($con, $sql);
	if (pg_num_rows($res) == 1) {
...
if ($e_posto or $e_admin) {
	...
	if (!function_exists('createHTMLSelect')) {
		function createHTMLSelect($name,$id,$a_dados,$js = "",$branco = true,$usar_key = false, $default = '') {
			echo "\t\t\t\t<SELECT name='$name' id='$id' $js>\n";
			if ($branco) echo "\t\t\t\t\t<OPTION value=''></OPTION>\n";
			if (!is_array($a_dados)) {
				$_a_dados = explode(",",$a_dados);
				unset($a_dados); $a_dados = $_a_dados;
			}
			foreach ($a_dados as $key=>$value) {
			    $valor = iif($usar_key,$key,str_replace(".", "#", $value));
			    $sel   = iif($valor == $default, ' SELECTED');
			    $texto = str_replace("_", " ", iif(strpos($value, ".")>0,substr($value, strpos($value, ".")+1),$value));
	...
	if ($e_admin) {
		...
		$sql_admin_anivs = "SELECT TO_DATE(dia_nascimento||'-'||mes_nascimento||'-'|| EXTRACT('year' FROM current_date),'DD/MM/YYYY') AS data_aniv,
								   LPAD(dia_nascimento::TEXT,2,'0')||'-' ||LPAD(mes_nascimento::TEXT,2,'0')||'-'|| EXTRACT(YEAR FROM current_date) AS aniv,
	...
	$sql_lu_anivs= "SELECT TO_DATE(dia_nascimento||'-'||mes_nascimento||'-'|| EXTRACT('year' FROM current_date),'DD/MM/YYYY') AS data_aniv,
						   LPAD(dia_nascimento::TEXT,2,'0')||'-' ||LPAD(mes_nascimento::TEXT,2,'0')||'-'|| EXTRACT(YEAR FROM current_date) AS aniv,
	...
	$res_lu     = pg_query($con, $sql_lu_anivs);if ($pg_err = pg_last_error($con)) pre_echo($sql_lu_anivs, $pg_err);
	...
	if ($tem_aniv_admin or $tem_aniv_lu) {
		...
		foreach ($a_nivers as $aniv_item) {
			...
			$li_item = "<li class='aniv$hoje'>".
						"<p>Dia $aniv_aniv<br>&Eacute; o anivers&aacute;rio de <b>$usuario</b>".
...
?><!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
	<html>
	<head>
	<title>Teste Aniversários</title>
<?}?>
	<style type="text/css">
	 div#anivs {
		position: fixed;
...
<?}?>
		...
		<script src="/mlg/js/cookies.js" type="text/javascript" language="JavaScript"></script>
