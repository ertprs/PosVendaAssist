...
if(strlen($atualiza_hd)>0){
	...
	if(strlen($hd)>0 and strlen($prazo)>0){
		...
		hora_desenvolvimento = $prazo
				where hd_chamado = $hd
				and fabrica_responsavel = $login_fabrica";
		$res = pg_exec($con,$sql);
		$msg_erro = pg_errormessage($con);
		if(strlen($msg_erro)==0){
			echo "Prazo atualizado!";
		}else{
			echo "Ocorreu o seguinte erro $msg_erro";
		}
	}
	...
	if (strlen($msg_erro) == 0){
		...
		$sql =" UPDATE tbl_hd_chamado
				SET status = '$status' ,
					...
					/*takashi colocou isso 30-05-07 2495 */
		...
		if($status== "Aprovação" AND ($status_anterior <> "Cancelado" OR $status_anterior <> "Resolvido")){
			...
			if(strlen($msg_erro) ==0){
				...
				$res = pg_exec ($con,$sql);
				if (pg_numrows($res) > 0) {
					$supervisor_email  = pg_result($res,0,email);
					$supervisor_nome   = pg_result($res,0,nome_completo);
					$supervisor_adm    = pg_result($res,0,admin);
		...
		if (strlen ($comentario) > 0) {
			...
			//fecho o atendimento se tiver algum aberto
...
function resolvido(){
	if (document.frm_chamada.status.options.value=='Resolvido'||document.frm_chamada.sequencia[1].checked){
		$('#btn_acao').slideToggle();
		$('#resolvido').confirm({
...
<tr>
	<td class='titulo_cab'><strong>Email </strong></td>
...
<tr>
	...
	<td  class='sub_label'>
		...
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type='radio' name='sequencia' value='SEGUE' onClick="javascript: resolvido();">Resolvido
...
<td  align='center' colspan='1' class='sub_label'>
<center><input type='submit' name='btn_acao' id='btn_acao' value='Responder Chamado' >
<div id='resolvido'></div>
</center>
</td>
</tr>
</table>
...
<td valign='top'>
	...
	<tr>
		...
		<td class ='sub_label'  align = 'center' >
			<select name="status" size="1"  style='width: 150px;' onchange='resolvido()'>
			<!--<option value=''></option>-->
...
<? if($analista_hd == "sim"){ ?>
	<tr>
		<td  class ='sub_label'><strong>Prazo </strong></td>
		<td  class ='sub_label'align='center'>
...
if($login_admin <> 822 and $login_admin <> 398){
