		...
		if(pg_numrows($res) ==0){
			$sql="INSERT INTO tbl_hd_franquia(
					...
					saldo_hora               
						...
						valor_hora_franqueada,
						(
						SELECT (0 + hora_desenvolvimento) as hora_utilizada
						FROM tbl_hd_chamado
						WHERE tbl_hd_chamado.fabrica=tbl_hd_franquia.fabrica and hd_chamado=$hd_chamado 
						),
						'0'                  ,
						(hora_franqueada - hora_utilizada) 
						FROM tbl_hd_franquia
						WHERE fabrica = $login_fabrica
						AND   mes     = '$mes_passado'
						AND   ano     = '$ano_passado' ";
		}else{
			$sql = "UPDATE tbl_hd_franquia
...
echo "<tr style='font-family: arial ; color: #666666' align='center'>";
	echo "<td nowrap align='left'>";
		...
		else                        echo "&nbsp;<a href='chamado_lista.php?status=Aprovação'>Aguardando Aprovação</a>&nbsp;";
	echo "</td>";
	...
	echo "<td nowrap align='left'>";
		...
		else                        echo "&nbsp;<a href='chamado_lista.php?status=Análise&exigir_resposta=f'>Pendente Telecontrol</a>&nbsp;";
	echo "</td>";
	echo "<td nowrap align='left'>";
		...
		else                        echo "&nbsp;<a href='chamado_lista.php?status=Análise&exigir_resposta=t'>Aguardando sua resposta</a>&nbsp;";
	echo "</td>";
	echo "<td nowrap align='left'>";
...
if(pg_numrows($res) > 0){
	...
	$valor_faturado       = number_format($valor_faturado,2,',','.');
	...
	echo "</tr>";
	echo "<tr bgcolor='#D9E8FF' style='font-family: arial ; color: #666666'>";
	echo "<td align='center' colspan='100%'>$mes/$ano";
	echo "</td>";
	echo "</tr>";
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
		...
		if ($supervisor=='t' AND $status=='Aprovação'){
			$sql3 = "SELECT hora_utilizada
						...
						HAVING  hora_franqueada < hora_utilizada ";
			$res3 = pg_exec ($con,$sql3);	
