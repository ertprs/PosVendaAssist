...
if($aprova=='sim'){
	...
	$sql = "UPDATE tbl_hd_chamado
			SET exigir_resposta = 'f', status = 'Análise', data = CURRENT_TIMESTAMP
...
$qtd_hd_2etapa = @pg_numrows($res2etapa);
		...
		if ($supervisor=='t' AND ($status=='Aprovação' or $status=='Novo')){
			...
			if(pg_numrows($res3)>0){
				if(strlen($hora_desenvolvimento)>0){
					...
					if(strlen($qtd_hd_2etapa)>0 and $qtd_hd_2etapa>0 ) {
