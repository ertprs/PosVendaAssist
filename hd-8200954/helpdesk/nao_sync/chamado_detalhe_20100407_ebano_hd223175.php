...
function validatemail($email=""){ 
	if (eregi("^[a-z]+([\._\-]?[a-z0-9]+)+@+[a-z0-9\._-]+\.+[a-z]{2,3}$", $email)) { 
...
//HD 218848: Questionário HelpDesk, habilitando apenas para algumas fábricas
if ($login_fabrica == 10 || $login_fabrica == 46 || $login_fabrica >= 81) {
	$habilita_questionario = true;
}
else {
	$habilita_questionario = false;
}

//HD 7277 Paulo - tirar acento do arquivo upload
function retira_acentos( $texto ){
 $array1 = array("á", "à", "â", "ã", "ä", "é", "è", "ê", "ë", "í", "ì", "î", "ï", "ó", "ò", "ô", "õ", "ö", "ú", "ù", "û", "ü", "ç" , "Á", "À", "Â", "Ã", "Ä", "É", "È", "Ê", "Ë", "Í", "Ì", "Î", "Ï", "Ó", "Ò", "Ô", "Õ", "Ö", "Ú", "Ù", "Û", "Ü", "Ç","n°","N°");
	...
	//	echo "<option></option>";
		if($sistema_lingua<>"ES"){
			for($i=0;pg_numrows($res)>$i;$i++){
				...
				if($_POST["combo_tipo_chamado"] == $xtipo_chamado){echo " SELECTED ";}
...
<tr>
	<td colspan=4 bgcolor="#E5EAED" style='border-style: solid; border-color: #6699CC; border-width:1px; padding-right: 42px;' align='center'>
		http:// <input <?=$desabilita_questionario?> size=90 type="text" name='http' id='http' value='<?= $http ?>' class='questionarioCaixa' />
	</td>
</tr>
<?
}	//if ($mostra_questionario) {
