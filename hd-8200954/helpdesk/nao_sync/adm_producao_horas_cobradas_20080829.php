...
if(strlen($fecha_fabrica) >0){
	...
	$sql="UPDATE tbl_hd_franquia SET periodo_fim=CURRENT_TIMESTAMP
	...
	if(($total_desenvolvimento - $hora_utilizada) >0) {
		$hora_utilizada=$total_desenvolvimento - $hora_utilizada;
	}else{
		$hora_utilizada=0;
	}
...
if( strlen($data_de2)>0 AND strlen($data_ate2)>0){
	...
	if(strlen($fabrica_busca > 0)){
		...
		if (pg_numrows($res) > 0) {
			...
			$sqlf = "SELECT hora_utilizada, hora_franqueada, hora_faturada, saldo_hora FROM tbl_hd_franquia where fabrica = $fabrica_busca and mes=7 and ano = 2008;";
