...
$sql = "SELECT  hd_chamado,
				...
				CASE WHEN current_timestamp > tbl_hd_chamado.previsao_termino_interna
					...
					ELSE 0
				END AS atrasou_interno,
				CASE WHEN (hora_franqueada + saldo_hora) = 0
					THEN '0'
					ELSE hora_utilizada / (hora_franqueada + saldo_hora) 
...
if (@pg_numrows($res) >= 0) {
	...
	echo "<thead>";
	echo "<tr bgcolor='#D9E8FF'><th colspan=11><CENTER>CHAMADOS PENDENTES AGUARDANDO ADMIN</CENTER></th></tr>";
	echo "<tr bgcolor='#D9E8FF' >";
	echo "	<th >Nº </th>";
	echo "	<th nowrap><strong>Título</strong><img src='/assist/imagens/pixel.gif' width='5'></th>";
	echo "	<th ><strong>Status</strong></th>";
...
$sql = "SELECT  hd_chamado,
				...
				CASE WHEN current_timestamp > tbl_hd_chamado.previsao_termino_interna
					...
					ELSE 0
				END AS atrasou_interno,
				CASE WHEN (hora_franqueada + saldo_hora) > 0 THEN
						hora_utilizada / (hora_franqueada + saldo_hora)
					ELSE 0
				END as percentual_atendido
		FROM tbl_hd_chamado
		LEFT JOIN tbl_tipo_chamado on tbl_tipo_chamado.tipo_chamado = tbl_hd_chamado.tipo_chamado
