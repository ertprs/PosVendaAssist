...
/*      Não é usada no progama inteiro...
...
/*--==VALIDAÇÕES=====================================================--*/
	...
	if(strlen($hd_chamado)>0){
		...
		elseif (intval($_POST["combo_tipo_chamado"]) == 5) {
			$msg_erro = "Para chamado de erro, por favor, anexe um <i>PrintScreen</i> (imagem) da tela aonde o erro ocorreu";
		}
	}
	...
	if (!is_email($email)){
	  $msg_erro= "O e-mail informado ($email) não é válido.";
	}
	if (strlen($fone) == 0){
...
//FIM DO ANEXO DO ARQUIVO
	//ENVIA EMAIL PARA SUPERVISOR DA FÁBRICA
		...
		if($tot_sups > 0) {
/*  08/09/2010 MLG - HD 291166 - Ao invés de enviar uma mensagem por e-mail, manda um só para o total de
								 destinatários (normalmente tem 1, máx. 3)  */
...
//AND $login_fabrica == 6 tirei, agora manda email para todos supervisores
			if(is_email($email_supervisor) AND strlen($dispara_email) > 0 AND strlen($msg_erro)==0) {
