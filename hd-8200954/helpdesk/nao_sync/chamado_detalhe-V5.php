...
if (strlen ($btn_acao) > 0) {
	...
	if (strlen($msg_erro) == 0){
		...
		#Alterado por Sono, pois não estava reabrindo chamados.
		//if ($status == 'Aprovação') {
...
<tr>
	...
	<td bgcolor="#E5EAED" style='border-style: solid; border-color: #6699CC; border-width=1px' colspan='3'>
		...
		<option value='Alteracao'      <? if($categoria=='Alteracao')      echo ' SELECTED '?> >Alteração de Dados</option>
		<option value='Mudanca'        <? if($categoria=='Mudanca')        echo ' SELECTED '?> >Mudança em Tela ou Processo</option>
		<option value='Melhoria'       <? if($categoria=='Melhoria')       echo ' SELECTED '?> >Sugestão de Melhorias</option>
...
if (strlen ($hd_chamado) > 0 AND strlen($resolvido==0)) {
	if ($status == 'Resolvido') {
		...
		echo "<b><font face='arial' color='#FF0000' size='-1'>Se você não concordar com a solução, pode reabri-lo digitando uma mensagem abaixo.</font><br><font face='arial' color='#00CC00' size='-1'>Se você concorda com a solução clique no botão RESOLVIDO</font></b><br>";
	}else{
