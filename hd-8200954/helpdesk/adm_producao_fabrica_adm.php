...
if( strlen($data_de2)>0 AND strlen($data_ate2)>0){
	...
	if(strlen($atendente_busca > 0)){
		...
		if (pg_numrows($res) > 0) {
			...
			if ($login_admin == 1819) {
				echo "<td align='center' nowrap><b>At. Chamado</b></td>";
			}
			...
			for ($x = 0 ; $x < pg_numrows($res) ; $x++){
				...
				$sqlp = "SELECT horas_analisadas  as total_prazo_horas
						FROM   tbl_backlog_item
						WHERE  hd_chamado = $hd_chamado limit 1";
				...
				if ($login_admin == 1819) {
					if ($atendente_hd != $login_admin) {
						$cor = "#DD3333";
					}
					else {
						$cor = "";
					}
					echo "<td nowrap'><font color='$cor'>$atendente_hd</font></td>";
				}
		...
		if (pg_numrows($resf) > 0) {
			...
			for ($x = 0 ; $x < pg_numrows($resf) ; $x++){
				...
				$sqlq = "SELECT distinct tbl_hd_chamado.hd_chamado, case when horas_analisadas notnull then horas_analisadas else tbl_hd_chamado.prazo_horas end as prazo_horas
						INTO TEMP TABLE tmp_prazo_horas
						FROM   tbl_hd_chamado
						JOIN   tbl_hd_chamado_atendente USING(hd_chamado)
						JOIN tbl_admin   ON tbl_hd_chamado.admin    = tbl_admin.admin
						JOIN tbl_tipo_chamado on tbl_hd_chamado.tipo_chamado = tbl_tipo_chamado.tipo_chamado
						JOIN tbl_backlog_item on tbl_backlog_item.hd_chamado = tbl_hd_chamado.hd_chamado
						WHERE  data_inicio between '$data_de 00:00:00' and '$data_ate 23:59:59'
						and    tbl_backlog_item.faturado
						AND   tbl_hd_chamado.status <> 'Cancelado' ";
					if($x_status == 'Resolvido'){
						$sqlq .= "AND  tbl_hd_chamado.status = 'Resolvido' ";
					}
			...
			echo "<td align='right' >$total_prazo_horas</td>";
			echo "</tr>";
			echo "<tr style='font-size:11px' bgcolor='#D9E8FF'><td align='center' colspan='4'>(*)Este total de chamados pode <b>não bater</b> com o total de chamados do fabricante. O motivo é que um chamado pode ser trabalhado por mais de um atendente!</td></tr>";
			echo "</foot>";
			echo "</table>";
		}
	}
