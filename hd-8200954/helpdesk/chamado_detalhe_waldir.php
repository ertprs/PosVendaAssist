		...
		if (!is_resource($res) or pg_affected_rows($res)<1) {
			...
			$sql = "UPDATE tbl_hd_chamado
					...
					INSERT INTO tbl_hd_chamado_item (
						...
						$hd_chamado,
						'MENSAGEM AUTOMÁTICA - REQUISITOS APROVADOS EM $data PELO USUÁRIO $nome_completo',
						$login_admin
				)";
...
if (strlen ($btn_acao) > 0) {
	...
	if (strlen($hd_chamado) > 0) {
		...
		// 12-12-2011 - MLG - alidar o endereço do campo HTTP - Solicitação do Waldir.
		$urlOK=preg_match("#^(http://)?(ww[w2]\.telecontrol\.com\.br)?(/\w+)+(.php)(\?.*)?$#", $http, $a_url);
		//die(print_r($a_url));
		if (!$urlOK) $msg_erro .= "<br>O link ('Endereço HTTP') não é válido, confira e corrija.";

		//HD 218848: Quando for chamado de erro, é obrigatório enviar o printscreen da tela
		if (intval($_POST["combo_tipo_chamado"]) == 5) {
			if (!is_array($arquivo)) $msg_erro .= "Para chamado de erro, por favor, anexe um <i>PrintScreen</i> (imagem JPG, de preferência) da tela aonde o erro ocorreu<br>";
		}
	...
	if (strlen($msg_erro) == 0) {
		...
		if (strlen ($msg_erro) == 0) {
			...
			if (strlen($arquivo["tmp_name"]) > 0 && $arquivo["tmp_name"] != "" and !$msg_erro) {
				...
				if (strlen($msg_erro) == 0) { // Processa o arquivo
					...
					$nome_anexo   = dirname(__FILE__)."/documentos/" . $hd_chamado_item . '-' . strtolower($arquivo_nome) . '.' . $ext;
				}
		...
		if ($tot_sups > 0 and strlen($msg_erro) == 0) {
			...
			if ($email_destino != '' AND strlen($dispara_email) > 0 AND strlen($msg_erro) == 0) {
				...
				$corpo .= "<br><a href='http://www.telecontrol.com.br../helpdesk/chamado_detalhe.php?hd_chamado=$hd_chamado'>CLIQUE AQUI PARA VER O CHAMADO</a> \n\n";
		...
		if (strlen($dispara_email) > 0 AND strlen($msg_erro) == 0) {
			...
			if ($status_anterior == 'REABRIR') {
				...
				$corpo .= "<br><a href='http://www.telecontrol.com.br../helpdesk/adm_chamado_detalhe.php?hd_chamado=$hd_chamado'>CLIQUE AQUI PARA VER O CHAMADO</a> \n\n";
				$corpo .= "<br><br>Telecontrol\n";
				$corpo .= "<br>www.telecontrol.com.br\n";
				$corpo .= "<br>_______________________________________________\n";
				$corpo .= "<br>OBS: POR FAVOR NÃO RESPONDA ESTE EMAIL.";
...
<script type='text/javascript'>
	...
	$(function() {
		...
		$('#aprovaRequisitos').click(function(){
			...
			if (confirm("Confirma a aprovação dos requisitos relacionados?")) {
...
<?		} elseif ($tc_com_hoje > strtotime('12/19/2011 07:00:00') and $tc_com_hoje < strtotime('01/08/2012 17:00:00')) { ?>
			<input type='hidden' readonly  name='combo_tipo_chamado'  value='<?=$tipo_de_hd[5]?>' />
			<input type='text'   readonly value='<?=$tipo_de_hd[5]?>' id='tipo_chamado_bloqueado' />
<?		} else { ?>
...
if (@pg_numrows($res) > 0) {
	...
	if ($tot_reqs = pg_num_rows($res_req)) {
		...
		echo "<center><div id='divReqAprova'><h3 style='color:yellow'>&nbsp;Requisitos do Chamado</h3>\n<ol style='background-color:#F2F7FF;font-size: 13px;text-align:left'>\n";
		...
		if (count(array_filter($req_controle_admin))==$tot_reqs) {
			...
			echo "<p style='color:white'>Requisitos aprovados em <b>" . $data_aprovadas . "</b> por <b>" . pg_fetch_result($res_nome_admin, 0, 0) . '</b>';
		} else {
			echo "<span style='float:right;margin-top:-10px'><button type='button'
				id='aprovaRequisitos'>Aprovar Requisitos</button></span><br />\n";
		}
		echo "</div></center>\n";
	}
	echo "<BR>";
	...
	echo "<td background='../helpdesk/imagem/fundo_tabela_top_centro_azul_claro.gif' colspan='7' align = 'center' width='100%' style='font-size:14px;color:#666666'><b>";
	...
	echo "<td ><img src='../imagens/pixel.gif' width='10'></td>";
	...
	echo "</strong></td>";
	echo "<td nowrap align='center'><img src='../imagens/pixel.gif' width='10'><strong>Anexo </strong></td>";
	...
	for ($i = 0; $i < pg_numrows($res) ; $i++) {
		...
		if ($dh  = glob($dir."$hd_chamado_item-*")) {
			foreach($dh as $filename) {
				$att_icon = (preg_match("/\.(gif|jpg|jpeg|png|tif|tiff|bmp)$/", $filename)) ?
								"'$filename' width='24' height='24' " : 
								"'imagem/clips.gif'";
				echo "<!--ARQUIVO-I-->&nbsp;&nbsp;<a href=$filename target='blank'><img src=$att_icon border='0'>Baixar</a>&nbsp;&nbsp;<!--ARQUIVO-F-->";
			}
		}
	...
	echo "<td background='../helpdesk/imagem/fundo_tabela_baixo_centro.gif' colspan='7' align = 'center' width='100%'></td>";
...
if ($permissao == 'sim') {
	...
	echo "<textarea name='comentario' cols='90' rows='10' class='questionarioCaixa' wrap='VIRTUAL'>".stripcslashes($comentario)."</textarea><br>";
...
if ($status == 'Resolvido' AND strlen($resolvido) == 0) {
	...
	if ($sistema_lingua == "ES") echo "SOLUCIONADO - Estoy de acuerdo con la solución";
