...
if($btn_acao=="atualizar"){
	...
	}elseif(strlen($msg_erro) == 0){	$msg_erro = '<BR>Digite a Inscrição Estadual.';}
	
	$codigo                  = $_POST["codigo"];
	if (strlen($codigo) > 0 and strlen($msg_erro) == 0){$codigo = "'".$codigo."'";
	}elseif(strlen($msg_erro) == 0){	$msg_erro = '<BR>Digite o Código do Posto.';}

	$nome                    = $_POST["nome"];
	if (strlen($nome) > 0 and strlen($msg_erro) == 0){$nome = "'".$nome."'";
	}elseif(strlen($msg_erro) == 0){	$msg_erro = '<BR>Digite o endereço.';}
	...
	}elseif(strlen($msg_erro) == 0){	$msg_erro .= '<BR>Digite o número.';}
...
//autorizado de outros eletronicos
	...
	if($treinado_outros == 't' AND strlen($treinado_outros_quais)==0 and strlen($msg_erro) == 0){$msg_erro.="Informe a fábrica que seu posto recebeu treinamento.<BR>";}
...
if($treinado_pneu == 't' AND strlen($treinado_pneu_quais)==0 and strlen($msg_erro) == 0){$msg_erro.="Informe a fábrica que seu posto é autorizado.<BR>";}
...
if($treinado_pneu_outros == 't' AND strlen($treinado_pneu_outros_quais)==0 and strlen($msg_erro) == 0){$msg_erro.="Informe a fábrica que seu posto recebeu treinamento.<BR>";}
...
<!-- CNPJ/CPF - I.E. - FONE - FAX - CONTATO - CÓDIGO - RAZÃO SOCIAL -->
<table class='formulario' width='700' align='center' border='0' cellpadding="3" cellspacing="1">
	...
	<tr  class='subtitulo'>
		<td colspan="1"><? echo "Código";?></td>
		<td colspan="4"><? echo "Razão Social";?></td>
	</tr>
	<tr>
...
<table class="formulario" align='center' border='0' cellpadding="3" width='700px' cellspacing="1">
	<tr class='subtitulo'>
		<td width='45%'><? echo "Endereço";?></td>
		<td width="100px"><? echo "Número";?></td>
...
<table class="formulario" align='center' border='0' cellpadding="3" cellspacing="1" width='700px'>
	...
	<tr >
		<td colspan="4">&nbsp Nome dos técnicos  <input type="text" name="nome_tecnicos" size="82" maxlength="50" value="<? echo $nome_tecnicos ?>"></td>
	</tr>
	<tr  class='subtitulo' >
...
<!--questao nova -->
	...
	<tr>
		<td align='center' colspan='5'>
			...
			<input type="button" value="Gravar" style="cursor: pointer;" onclick="javascript: if (document.frm_posto.btn_acao.value == '' ) { document.frm_posto.btn_acao.value='atualizar' ; document.frm_posto.submit() } else { alert ('Aguarde submissão') }" ALT="Gravar formulário" title="Gravar Alterações">
