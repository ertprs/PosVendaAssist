...
if ($acao == 'alterar' and strlen ($btn_alterar) > 0) {
...
$senha_nova2= $_POST['senha_nova2'];
	if($senha_nova == $senha_nova2){
		...
		$res = pg_query($con,$sql);
		if (is_resource($res)) $msg = "Senha atualizada com sucesso!";
		else			  $msg_erro = "Senha não atualizada. Tente novamente ou contate com o Suporte Telecontrol.";
 	}else{
		$msg_erro = "Senhas não conferem!";
	}
}

//  Para liberar, atualizar para NULL
if ($acao == 'liberartela') {
	$sql = "UPDATE tbl_posto_fabrica SET senha_financeiro = NULL
			 WHERE posto   = $login_posto
			   AND fabrica = $login_fabrica";
	$res = pg_query($con, $sql);
	if (!is_resource($res)) $msg_erro = "Erro ao liberar a tela. Tente novamente ou contate com o Suporte Telecontrol.";
}
...
if (strlen($msg_erro) > 0) {
	echo "<span class='Erro'><img src='imagens/proibido2.jpg' align='absmiddle'>$msg_erro</span>";
}
...
<form name='frm_alterar' method='post' action='<?=$PHP_SELF?>?acao=alterar' align='center'>
	<table class='Tabela' width='300' cellspacing='0'  cellpadding='0' bgcolor='#596D9B' align='center'>
		<tr >
			<td class='Titulo'>Alteração de Senha do Usuário</td>
		</tr>
		<tr>
			<td bgcolor='#F3F8FE'>
			<table width='100%' border='0' cellspacing='1' cellpadding='2' class='table_line' bgcolor='#f3f8fe'>
				<tr class='Conteudo'>
					<td colspan='2'>
						<br>Por favor entrar com a nova senha.
					</td>
				</tr>
				<TR width='100%'>
					<td align='right' height='40'>Senha:&nbsp;</td>
					<td><input type='password' name='senha_nova' ></td>
				</tr>
				<tr width='100%'>
					<td align='right' height='40'>Repetir Senha:&nbsp;</td>
					<td><input type='password' name='senha_nova2' ></td>
				</tr>
				<tr class='Conteudo'>
				<TD colspan='2'>
					<br><input type='submit' name='btn_alterar' value='Alterar Senha'>
				</TD>
				</tr>
			</table>
		</td>
...
elseif ($acao=='liberartela' and $msg_erro == '') {
//alterar senha
echo "<script>alert('Acesso liberado!'); window.location.replace(location.pathname);</script>";
