...
?>
		<br />
		<table width='700' align='center' class='table_line3'>
			<caption class='menu_top4'>ATENÇÃO</caption>
			<tr>
				<td>
					Não será mais feita a devolução de produtos ou peças trocadas.
				</td>
			</tr>

			<tr >
				<td>
					O posto deverá armazená-los pelo período de 90 dias para uma possível auditoria, após este prazo,<br> está autorizado que o posto desmonte os produtos e utilize as peças para conserto de produtos em garantia.
				</td>
			</tr>

			<tr>
				<td>
					<b>Os produtos não deverão ser descartados em lixo comum.</b>
				</td>
			</tr>
		</table>
...
//if ($ip=='201.42.44.145') echo $sql; exit;
	...
	if ($totalRegistros > 0){
		...
		echo "<TD colspan='$colspan' align = 'center'>";
		echo "Exibir peças com o Serviço Realizado ";
...
//if ($ip=='201.42.44.145') echo "1"; echo $sql;
	...
	if ($agrupar == "false") {
		...
		$sql = "SELECT	tbl_os.os                                                      ,
				...
				//hd 14981 20041
				if ($login_fabrica <> 35 AND $login_fabrica <>14 AND $login_fabrica <>51 and $login_fabrica <> 81) {
				...
				if($login_fabrica == 35 or $login_fabrica == 81){ /*HD 48868*/
				...
				if($login_fabrica == 81){
					$sql .=" AND tbl_os.consumidor_revenda = 'R'";
				}

				$sql .= "ORDER BY lpad(tbl_os.sua_os,10,' '), tbl_os_item.preco;";
	...
	*/
	...
	//agrupado
		...
		$sql = "SELECT	tbl_produto.descricao      AS produto_nome                         ,
				...
				//hd 14981 20041
				if (!in_array($login_fabrica,array(35,14,51,40,81))) {
					$sql .= "AND     tbl_os_item.liberacao_pedido_analisado  IS TRUE ";
				}elseif ($login_fabrica ==14) {
					$sql .= "AND     tbl_os_item.liberacao_pedido IS TRUE ";
				}
...
//if ($ip=='200.228.76.93') echo "2"; echo $sql;
	...
	if ($totalRegistros > 0){
		...
		if ($sistema_lingua=='ES')
			echo "<TD colspan='$colspan' align = 'center'>Repuesto del estoq que generan crédito pero no necesitan ser devolvidas fisicamiente a la planta.</TD>";
		else
			//hd 35590
			if(!in_array($login_fabrica,array(35,14,81))){
				...
				echo "<TD colspan='$colspan' align = 'center'>Peças Retornáveis.</TD>";
		}
		echo "</TR>\n";
		echo "<TR class='menu_top'>\n";
...
// 	echo $sql;
	...
	if ($totalRegistros > 0 and ($login_fabrica <> 14 and $login_fabrica <> 35)){
		...
		if ($login_fabrica <> 14) {
			echo "<TR class='menu_top'>\n";
			echo "<TD colspan='$colspan' align = 'center'>Peças que geram Crédito e que devem <br> ser devolvidas fisicamente para a Fábrica.</TD>";
			echo "</TR>\n";
		}
	...
	if ($agrupar == "false") {
		$sql = "SELECT	tbl_os.os                                                      ,
				...
				//hd 14981 20041
				if (!in_array($login_fabrica,array(35,14,51,40))) { 
					$sql .= "AND     tbl_os_item.liberacao_pedido    IS NOT FALSE ";
				}
