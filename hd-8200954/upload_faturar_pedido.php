        ...
        if (!count($pedidos)) {
            ...
            foreach ($pedidos as $key => $pedido) {
                if (empty($pedido["referencia_peca"])) {
                    ...
                    $sql = "SELECT peca FROM tbl_peca WHERE fabrica = {$login_fabrica} AND LOWER(referencia) = LOWER('{$pedido['referencia_peca']}')";
                    $res = pg_query($con, $sql);
                    if (!pg_num_rows($res)) {
            ...
            foreach ($pedidos as $pedido) {
                ...
                if(pg_num_rows($res_fat_item)> 0){
                    ...
                    if (!empty($os_troca) && empty($data_conserto)) {
            ...
            if ($login_fabrica == 152 && count($array_comunicado)) {
                $html = "<ul>";
                ...
                foreach ($array_comunicado as $pedido) {
                    ...
                    $html .= "
                        <li>Pedido: {$pedido['pedido']}, Peça: {$pedido['peca']}, Previsão de Chegada: {$previsao_chegada}</li>
                    ";
                }
...
<?php
if(count($linhas) > 0){
    if(in_array($login_fabrica, array(152)) && count($pedido) > 0){
    ?>
    <table class='pedido_sem_nota table table-striped table-bordered table-large' style="margin: 0 auto;" >
        <tr class="warning" >
            <td class="tac" colspan="5" style="font-weight: bold;" >Atualização da Ordem/Previsão de Faturamento</td>
        </tr>
        <tr>
            <td class="tac" colspan="5" >
                <button type="button" data-loading-text="Processando..." class="btn btn-warning btn_previsao">Gravar Ordem/Previsão de Faturamento</button>
            </td>
        </tr>
    ...
    <table class='fatura_pedido table table-striped table-bordered table-large' style="margin: 0 auto;" >
        ...
        <table class="table table-striped table-bordered table-large" style="margin: 0 auto;" >
            <tr class="success" >
                <td class="tac" style="font-weight: bold;" >Faturamento de Pedidos</td>
            </tr>
            <td class="tac" >
                <button type="button" data-loading-text="Processando..." class="btn btn-success btn_grava_nota_fiscal" >Gravar Notas Fiscais</button>
            </td>
        </table>
        ...
        foreach ($pedido_nf as $nota_fiscal => $dados) {
            if (in_array($login_fabrica,array(152,162,171))) {
                $colspan = 5;
                $colspan2 = 2;
            } else {
                $colspan = 4;
            ...
            <table class="table table-striped table-bordered table-large nota_fiscal_faturar" style="margin: 0 auto;" >
                ...
                <tr class='titulo_coluna'>
                    ...
                    ?>
                    <th <?=($login_fabrica == 162) ? $colspan2 : ""?>>Posto</th>
