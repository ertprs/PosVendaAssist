...
//echo "$conta_ativo!=$qtde_os";
		...
		if (strlen($msg_erro) == 0){
			...
			for ($i = 0 ; $i < $qtde_os ; $i++) {
				...
				if($login_fabrica == 94 && $ativo == 't') { // HD 414797
					...
					if ( pg_num_rows($res) == 0 ) {
						...
						$msg_erro = "Para fechar a OS é necessário lançar peça e o serviço realizado.";
					...
					if(!empty($nota_fiscal_verif) and $ativo == 't'){
						if(!empty($data_nf_saida) and !empty($nota_fiscal_saida)){
							...
							if(!checkdate($mnf,$dnf,$ynf)){
								$erro .= "Data de Saída de NF da OS : ".$sua_os." está incorreta";
							}else{
								...
								if(strtotime($aux_data_nf_saida) < strtotime($aux_data_fechamento)){
									$erro .= "Data de Saída de NF da OS : ".$sua_os." não pode ser inferior a Data de Fechamento";
								}
							}
						}
						else{
							$erro .= "O número e data da NF devem ser informados na OS : ".$sua_os."<br>";
						}
					}
				}
				...
				if ( (in_array($login_fabrica,array(1,3,6,11,51,81))) AND ($ativo == 't' or $ativo_revenda=='t') and strlen($erro) == 0 ) {
					...
					if (strlen ($erro) == 0) {
						...
						if (strlen ($erro) == 0) {
							if ($login_fabrica==1){
								...
								if(($login_fabrica == 11 OR $login_fabrica == 96) and strlen($erro) == 0){ #HD 96191
...
//							echo $sql="INSERT INTO tbl_os_interacao (os,comentario) values ($os,'$observacao')";
			...
			if($login_fabrica == 11 OR $login_fabrica == 96){ #HD 96191
...
if (strlen ($msg_erro) > 0 || strlen($nao_fechou) > 0) {
	...
	if (strpos($msg_erro,"ERROR: ") !== false) {
		...
		#HD 425978
		if ($login_fabrica != 95 and $login_fabrica != 98) {
...
if ((strlen($sua_os) > 0 AND $btn_acao_pesquisa == 'continuar') OR strlen($listar) > 0 OR strlen ($codigo_posto) > 0 OR (strlen($prateleira_box) > 0 AND $btn_acao_pesquisa == 'continuar' ) ){
		...
		<table width="700px" border="0" cellspacing="1" cellpadding="4" align="center" style='font-family: verdana; font-size: 10px' class='tabela_resultado Relatorio'>
		...
		<tr height="20px">
			...
			<th ><b><? fecho ("os", $con, $cook_idioma); /*OS*/
			...
			/*Box/Prateleira*/ ?></b></th><?}?>
			<th width="100px"><b><? fecho ("data.abertura", $con, $cook_idioma); /*if($sistema_lingua == 'ES') echo "Fecha Abertura";else echo "Data Abertura";*/?></b></th>
			<th width="100px"><b><? fecho ("consumidor", $con, $cook_idioma);/*if($sistema_lingua == 'ES') echo "Usuário";else echo "Consumidor";*/?></b></th>
			<th width="100px"><b><? fecho ("produto", $con, $cook_idioma); /*if($sistema_lingua == 'ES') echo "Producto";else echo "Produto";*/?></b></th>
			...
			<? if ($login_fabrica <> 2 AND $login_fabrica <> 1 AND $login_fabrica<>20){ ?>
				<th width="100px"><b><? fecho ("nf.saida", $con, $cook_idioma); /*NF de Saída*/ ?></b></th>
				<th width="100px"><b><? fecho ("data.nf.saida", $con, $cook_idioma); /*Data NF de Saída*/ ?></b></th>
			<? } ?>
			<?if($login_fabrica==20){?>
			<th width="100px"><b><? fecho ("valor.das.pecas", $con, $cook_idioma); /*if($sistema_lingua=='ES')echo "Valor de Piezas";else echo "Valor das Peças";*/?>
			</b></th>
			<th width="100px"><b><? fecho ("mao.de.obra", $con, $cook_idioma); /*if($sistema_lingua=='ES')echo "Mano de Obra";else echo "Mão-de-Obra";*/?>
			</b></th>
			<? } ?>
			<?
...
<?
		...
		for ($i = 0 ; $i < $total_os ; $i++) {
			if(($login_fabrica == 11 OR $login_fabrica == 96) AND strlen ($msg_ok) == 0){
		...
		<tr bgcolor="<?php echo $cor;?>" <? echo " rel='status_$status_checkpoint' ";?> <? if (($erros == $os or strpos($erros,$os) > 0) and strlen ($msg_erro) > 0 )?>>
			...
			<td nowrap>
				<? echo $cor_status_os;?>
				<a href='<? if ($cor == "#FFCC66" or ($login_fabrica==11 and $login_posto==14254)) echo "os_item"; else echo "os_press"; ?>.php?os=<? echo $os ?>' target='_blank'><? if ($login_fabrica == 1)echo $login_codigo_posto; echo $sua_os; ?></a>
			</td>
			<? if($login_fabrica == 11 and $login_posto == 14301) 
				echo "<td>$prateleira_box</td>"; ?>
			<td><? echo pg_fetch_result ($res,$i,data_abertura); ?></td>
			<td ><? echo substr (pg_fetch_result ($res,$i,consumidor_nome),0,10); ?></td>
			<? if($login_fabrica == 30 or $login_fabrica == 85){
				 $serie = pg_fetch_result ($res,$i,serie); ?>
				<td><? echo substr ($descricao,0,15); ?></td>
			<? }else{ ?>
				<td><? echo ($login_fabrica== 11) ? pg_fetch_result ($res,$i,serie)." - ".substr($referencia,0,15) : pg_fetch_result ($res,$i,serie) . " - " . substr ($descricao,0,15); ?></td>
			<? } 
			
			if ($login_fabrica <> 2 AND $login_fabrica <> 1 AND $login_fabrica<>20){ 
				if (($consumidor_revenda == 'R' OR (1==2 AND $login_fabrica == 19 AND strlen ($admin) > 0 AND $tipo_atendimento == 2)) OR $login_fabrica == 96){
					if($login_fabrica == 15){
					...
					echo "<td>
							<input class='frm' type='text' name='nota_fiscal_saida_$i' size='8' maxlength='10' value='$nota_fiscal_saida'>
...
<?//HD 9013
	if($login_fabrica=='1' or $login_fabrica ==7){ ?>
		<table width="700px" border="0" cellspacing="1" cellpadding="4" align="center" style='font-family: verdana; font-size: 10px' class='tabela_resultado sample'>
