...
$rphp_all_evald_posts = array();
define('rphp_metakey', 'runphp');
...
class runPHP {
   ...
   */
   function DetermineAllEvaldPosts() {
      global $wpdb, $rphp_all_evald_posts;

      $qry = "SELECT {$wpdb->posts}.ID
              FROM {$wpdb->posts}, {$wpdb->postmeta}
              WHERE {$wpdb->posts}.ID = {$wpdb->postmeta}.Post_ID
               AND meta_key = '" . rphp_metakey . "'
               AND meta_value = '1'";

      $rphp_all_evald_posts = $wpdb->get_col($qry);
   ...
   */
   function InjectJSToMoveUI() {
      ...
      if (preg_match('/(post|page)(-new)?.php/i', $_SERVER['SCRIPT_NAME'])) {
         ...
         function moveRunPHPCheckbox() {
            var placement = document.getElementById("commentstatusdiv");
    ...
    *
    * If $id is not an integer, then this function will return early and not
    * take any action. This was done as an additional safety measure against
    * SQL injection attacks.
    *
    * @param integer
    *   Primary key of the post to update.
    * @thanks Benajmin Flesch of mybeNi websecurity
   */
   function UpdateEvaldState($id) {
      ...
      // sanity check - id should _always_ be a number
      // todo: notify site owner?
      if (!is_numeric($id)) {
         die ("error: id is $id");
         $id = 0;
         return;
   ...
   // eval() the code in this post if it is marked in our $rphp_all_evald_posts array
   function EvalContent($content) {
      global $post;
      global $rphp_all_evald_posts;

      if (is_array($rphp_all_evald_posts) &&
          in_array($post->ID, $rphp_all_evald_posts)) {
         //$content = "\n<h3>eval'ing post {$post->ID}</h3>\n" . $content;
   ...
   /**
    * Determine whether the current post should be eval()'d or not.
    *
    */
   function IsPostEvald($postID) {
      ...
      if (!is_numeric($postID)) {
         //return false;
         die("Error from runPHP: postID ($postID) must be an integer. Aborting.");
      }

      return $wpdb->get_var("SELECT meta_value
                             FROM {$wpdb->postmeta}
                             WHERE Post_ID = " . $wpdb->escape($postID) . "
                             AND meta_key = '" . rphp_metakey . "';");
   }
