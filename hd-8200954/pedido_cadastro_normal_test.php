...
if(strlen($msg_erro)==0){
	...
	if (strlen ($msg_erro) == 0) {
		...
		$sql = "SELECT fn_pedido_finaliza_test ($pedido,$login_fabrica)";
		$res = @pg_exec ($con,$sql);
		$msg_erro = pg_errormessage($con);
	}
		...
		<DIV class='content'>
			<script type='text/javascript'>
				function limiteMinimo(){
					var valor = $("#condicao option:selected").text();
					valor = valor.match(/R\$\s?\d+/i);
					document.getElementById("valor_minimo").innerHTML = valor+",00";
				}
			</script>
			<H1 style='font-size: 1em;color:#B00'>
				<?
					$sql = "SELECT limite_minimo from tbl_condicao
								WHERE fabrica = 30 ORDER BY limite_minimo limit 1";
					$res = pg_exec($con,$sql);
					$limite_minimo = pg_result($res,0,limite_minimo);
				?>
				<div><b>Atenção:</b> O pedido só será incluído se tiver no mínimo <B><span id='valor_minimo' style='width:20px;'>R$&nbsp;30,00</span></B></div>
			</H1>
		</DIV>
		<?}?>
	</div>
...
<div>
	...
	<p><span class='coluna1'>Condição Pagamento</span>
		<select size='1' name='condicao'  id='condicao'class='frm' onchange='limiteMinimo();'>
		<?
			//hd 17625
			if ($login_fabrica == 24) {
			...
			for ($i = 0 ; $i < pg_numrows ($res) ; $i++ ) {
				...
				if($login_fabrica==24){
					...
					echo "<option value='" . pg_result ($res,$i,condicao) . "'";
					if (pg_result ($res,$i,condicao) == $condicao) echo " selected";
					echo ">" . pg_result ($res,$i,descricao) . " - Pedido Mínimo R$".pg_result ($res,$i,limite_minimo)."</option>";
				}
			}
		?>
		</select>
...
//			}
		...
		<? if($login_fabrica<>24 and $login_fabrica<>30 and $login_fabrica<>42){ //HD 70768-Retirar estes campos para a Esmaltec ?>
