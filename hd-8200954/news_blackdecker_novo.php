...
$sql =	"SELECT tbl_posto_fabrica.codigo_posto ,
				...
				TO_CHAR(tbl_os.data_digitacao,'DD/MM/YYYY') AS data_digitacao,
				TO_CHAR(tbl_os.data_abertura,'DD/MM/YYYY') AS data_abertura ,
				(SELECT status_os FROM tbl_os_status WHERE tbl_os_status.os = tbl_os.os ORDER BY tbl_os_status.status_os DESC LIMIT 1) AS status_os ,
				(SELECT observacao FROM tbl_os_status WHERE tbl_os_status.os = tbl_os.os ORDER BY tbl_os_status.status_os DESC LIMIT 1) AS observacao ,
				(SELECT tbl_status_os.descricao FROM tbl_os_status JOIN tbl_status_os USING (status_os) WHERE tbl_os_status.os = tbl_os.os ORDER BY tbl_os_status.status_os DESC LIMIT 1) AS status_descricao
			FROM tbl_os
			JOIN tbl_os_extra USING (os)
			JOIN tbl_posto USING (posto)
...
echo "<table width='600' cellpadding='0' cellspacing='0' border='1' bordercolor='DDDDDD' >";
echo "<tr>";
echo "<td>";
echo "<table width='200'  border='0' cellpadding='0' class='contentBlockLeft'>
	<tr>
		<td><img border='0' src='imagens/esclamachion1.gif'></td>
		<td align='center' class'tb_fim' width='200'>
			<a href='http://www.telecontrol.com.br/bd/index2.php' 'target='_blank'><font color='#F7F5F0'><b>Sistema Antigo</b></font></A>
		</td>
	</tr>
	<tr class='contentBlockLeft'>
		<td colspan='2' style='font-family: arial ; color:#F7F5F0 size='10px'><center><font color='#F7F5F0'><a href='http://www.telecontrol.com.br/bd/index2.php' class='menu' target='_blank'>Clique aqui para consultar seus extratos gerados no sistema antigo.<!-- Para lançar suas OSs pendentes e consultas no sistema antigo, clique aqui.</font><br>(Disponível somente no mês de setembro para finalizar todas as pendências anteriores) --></a></font>
	</center></td>
	</tr>
      </table>
...
?>
	...
	<div id="leftCol" bgcolor='#FFCC66'>
		<div class="contentBlockRight" align='left'>
<style>
