...
$cons_status_checkpoint = "0,1,2,3,4,8,9,14,28,52";
                    ...
                    if (strlen($admin) == 0 && strlen($data_conserto) == 0) {
                        $botao_excluir_os = '
                                            <li>
                                                ...
                                                <a href="javascript: if(confirm(\''.traduz("deseja.realmente.excluir.a.os",$con).' (OS nÂº '.$os.')!\') == true) { excluirOs('.$os.','.$posicao.');}" title="'.traduz("Excluir O.S.").'"><i class="icon-remove"></i>  '.traduz("Excluir O.S.").'</a>
...
function retorna_botao_consertado_os($con, $os, $data_conserto, $finalizada, $posicao)
{
    global $cook_idioma, $areaAdmin;
          


    $botao_consertado =  "";

    if (strlen($data_conserto) == 0  && empty($finalizada) && $areaAdmin != true) {
        $botao_consertado =  '
                            <li>
                                <a href="javascript: if(confirm(\''.traduz("apenas.clicar.ok.se.tiver.certeza.que.a.data.de.conserto.do.produto.da.%.seja.hoje",$con,$cook_idioma,array($os)).'!\') == true) { consertadoOS('.$os.','.$posicao.');}" title="'.traduz("Consertado").'" target="_blank"><i class="icon-check"></i> '.traduz("Consertado").'</a>
                            </li>
                            <li class="divider"></li>';
...
function retorna_botao_cancelar_reabrir_os($con, $os, $extrato, $finalizada, $posicao, $sua_os = '',$excluida)
{
	...
	if (empty($finalizada) AND $excluida == "t") {
		$botao_reabrir =  '
							<li>
								<a href="javascript: if(confirm(\''.traduz("deseja.realmente.reabrir.a.ordem.de.servico.%",$con,$cook_idioma,array($numero_os)).'!\') == true) { reabrir_os('.$os.',$(this));}" title="'.traduz("Reabrir").'" >'.traduz("Reabrir").'</a>
							</li>
		                    <li class="divider"></li>';

	}else if (empty($finalizada) AND $excluida != "t") {
        $botao_reabrir =  '
                            <li>
                                <a href="javascript: if(confirm(\''.traduz("deseja.realmente.cancelar.a.ordem.de.servico.%",$con,$cook_idioma,array($numero_os)).'!\') == true) { cancelar_os('.$os.',$(this));}" title="'.traduz("Cancelar").'" >'.traduz("Cancelar").'</a>
                            </li>
                            <li class="divider"></li>';
    }

	return $botao_reabrir;
} 
