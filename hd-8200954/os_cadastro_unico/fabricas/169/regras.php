    ...
    if (!$areaAdmin && empty($campos["os"]["os_numero"])) {
	    ...
	    if (!empty($agendamento['data_agendamento'])) {
    		if (strtotime(date("Y-m-d")) < strtotime($agendamento['data_agendamento'])) {
            	    throw new Exception("Não é possível gravar os dados do atendimento com data de agendamento futura. Para gravação ajustar a data de agendamento");
    		}
    	    }
            ...
            if ($pecasExcedenteLB == true) {
                if ($campos_peca["tem_obs"] == "t" && empty($campos_peca['causa_defeito']) && $peca_qtde <= $lista_basica_qtde) {
                    ...
                    if (pg_fetch_result($res_servico, 0, 'gera_pedido') == 't') {
                        $msg_erro["msg"]["lista_basica_qtde"] = traduz("Para solicitação excedente da peça % um motivo deve ser informado", null, null, $referencia." - ".$descricao);
                        $msg_erro["campos"][] = "{$nome}[{$posicao}]";
                    }
                } else {
		    if ($pecaQtdeOk === false) {
...
function auditoria_troca_obrigatoria_midea_carrier()
{
    ...
    if (pg_num_rows($res) > 0 && verifica_auditoria_unica(" tbl_auditoria_status.produto = 't' AND tbl_auditoria_os.observacao ILIKE '%troca obrigatória%'", $os) === true) {
        ...
        $sql = "
            ...
            VALUES ({$os}, {$auditoria_status}, 'OS em intervenção da fábrica por Produto de troca obrigatória', 't');
        ";
        $res = pg_query($con, $sql);
            ...
            if ($qtde_pecas > $qtde_pecas_intervencao) {
                ...
                if (verifica_auditoria_unica("tbl_auditoria_status.peca = 't' AND tbl_auditoria_os.observacao ILIKE '%peças excedentes%'", $os) === true || aprovadoAuditoria("tbl_auditoria_status.peca = 't' AND tbl_auditoria_os.observacao ILIKE '%peças excedentes%'")) {
                    $sql = "INSERT INTO tbl_auditoria_os (os, auditoria_status, observacao, bloqueio_pedido) VALUES
                        ({$os}, {$auditoria_status}, 'OS em auditoria de peças excedentes', 't')";
                    $res = pg_query($con, $sql);
                ...
                if (verifica_auditoria_unica("tbl_auditoria_status.peca = 't' AND tbl_auditoria_os.observacao ILIKE '%OS em auditoria a pe%'", $os) === true || aprovadoAuditoria("tbl_auditoria_status.peca = 't' AND tbl_auditoria_os.observacao ILIKE '%OS em auditoria a pe%'")) {
                    ...
                    foreach ($campos['produto_pecas'] as $xkey => $xvalue) {
                        ...
                        if ($servico_gera_pedido) {
                            $sql = "INSERT INTO tbl_auditoria_os (os, auditoria_status, observacao, bloqueio_pedido) VALUES
                                ({$os}, {$auditoria_status}, 'OS em auditoria a peça excedente à Lista Básica', 't')";
                            $res = pg_query($con, $sql);
                            if (strlen(pg_last_error()) > 0) {
...
function auditoria_numero_serie_coringa_midea_carrier()
{
    ...
    if (pg_num_rows($res) > 0 && verifica_auditoria_unica("tbl_auditoria_status.numero_serie = 't' AND tbl_auditoria_os.observacao ILIKE '%número de série%'", $os) === true) {
        ...
        $sql = "
            INSERT INTO tbl_auditoria_os (os, auditoria_status, observacao, bloqueio_pedido)
            VALUES ({$os}, {$auditoria_status}, 'OS em auditoria de número de série', 't');
        ";
        $res = pg_query($con, $sql);
...
function auditoria_os_reincidente_midea_carrier()
{
    ...
    if (pg_num_rows($res) > 0) {
        ...
        if (pg_num_rows($resVerificaOSAberta) > 0) {
            throw new Exception("Já existe uma OS em aberto com o número de série informado");
        }

        $descricao_atendimento_atual = pg_fetch_result($res, 0, "descricao_atendimento");

        if (in_array($descricao_atendimento_atual, ['Triagem'])) {
            $condTriagemReop = " AND tbl_tipo_atendimento.descricao != 'Reoperação'";
        }

        if (in_array($descricao_atendimento_atual, ['Reoperação'])) {
...
function auditoria_km_midea_carrier()
{
    ...
    if (pg_num_rows($res) > 0 && verifica_auditoria_unica("tbl_auditoria_status.km = 't' AND tbl_auditoria_os.observacao ILIKE '%auditoria de KM%'", $os) === true) {
		...
		UPDATE tbl_auditoria_os SET
                    cancelada = CURRENT_TIMESTAMP,
                    justificativa = 'Cancelamento automático'
            	WHERE os = {$os}
		...
		AND observacao !~ 'KM alterado manualmente'
            ...
            if (strlen(pg_last_error()) > 0) {
            	throw new Exception("Erro ao lançar ordem de serviço #AUD013");
            }
