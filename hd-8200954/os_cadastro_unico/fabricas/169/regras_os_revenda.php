...
$regras["revenda_contato"] = [
    "function"    => array("valida_obrigatoriedade_contato")
];
...
$auditorias_os_revenda = [
    "auditoria_tipo_atendimento_midea"
];

function auditoria_tipo_atendimento_midea() {
    global $con, $login_fabrica, $campos, $os_revenda;

    $dadosProduto = $campos["produtos"];

    unset($dadosProduto["__modelo__"]);
    ...
    if(verifica_tipo_atendimento_triagem() && (count($dadosProduto) > 1 || $entraAud)) {
        if (verifica_auditoria_unica_revenda("tbl_auditoria_status.fabricante = 't' AND tbl_auditoria_os_revenda.observacao ILIKE '%OS em auditoria de Triagem e Reoperação%'", $os_revenda) === true) {
            ...
            $sql = "INSERT INTO tbl_auditoria_os_revenda (os_revenda, auditoria_status, observacao, bloqueio_pedido) VALUES ({$os_revenda}, {$auditoria_status}, 'OS em auditoria de Triagem e Reoperação em lote', true)";
...
function valida_obrigatoriedade_contato() {
    global $campos, $regras, $msg_erro;

    if (verifica_tipo_atendimento_triagem() && empty(trim($campos["revenda_contato"]))) {
        $msg_erro["campos"][] = "revenda_contato";
        throw new Exception("Informe o contato da revenda");
    }

}

function verifica_tipo_atendimento_triagem() {
    global $campos;

    $triagem = false;
    foreach ($campos["produtos"] as $key => $val) {

        if (!empty($val["id"]) && ($val["tipo_atendimento"] == 304 || $val["tipo_atendimento"] == 305)) {
            $triagem = true;
        }
