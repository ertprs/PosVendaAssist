...
function retorna_botao_consertado_os($con, $os, $data_conserto, $finalizada, $posicao)
{
    ...
    if (strlen($data_conserto) == 0  && empty($finalizada)) {
        $botao_consertado =  '
                            <li>
                                <a href="fechamento_os.php?sua_os='.$os.'&btn_acao=submit"><i class="icon-check"></i> '.traduz("Consertado").'</a>
                            </li>
                            <li class="divider"></li>';
...
function retorna_botao_cancelar_reabrir_os($con, $os, $extrato, $finalizada, $posicao, $sua_os = '',$excluida)
{
    ...
    if (empty($finalizada) AND $excluida == "t") {
        $botao_reabrir =  '
                            <li>
                                <a href="javascript: if(confirm(\''.traduz("deseja.realmente.reabrir.a.ordem.de.servico.%",$con,$cook_idioma,array($numero_os)).'!\') == true) { reabrir_os('.$os.',$(this));}" title="'.traduz("Reabrir").'" ><i class="icon-ok"></i> '.traduz("Reabrir").'</a>
                            </li>
                            <li class="divider"></li>';
        ...
        $botao_reabrir =  '
                            <li>
                                <a href="javascript: if(confirm(\''.traduz("deseja.realmente.cancelar.a.ordem.de.servico.%",$con,$cook_idioma,array($numero_os)).'!\') == true) { cancelar_os('.$os.',$(this));}" title="'.traduz("Cancelar").'" ><i class="icon-remove"></i> '.traduz("Cancelar").'</a>
                            </li>
                            <li class="divider"></li>';
    }
