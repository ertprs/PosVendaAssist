...
$cons_status_checkpoint = "0,1,2,3,4,9";
...
if (empty($login_posto)) {
	...
	$cons_lista_de_legendas = [
							    ['descricao' => 'Reincidências', 'cor' => '#D7FFE1'],
							    ['descricao' => 'OSs Abertas há mais de 25 dias', 'cor' => '#91C8FF'],
							    ['descricao' => 'OS com Intervenção da Fábrica. Ag. Liberação', 'cor' => '#FFCCCC'],
							    ['descricao' => 'OS com Intervenção da Fábrica. Reparo na Fábrica', 'cor' => '#FFFF99'],
							    ['descricao' => 'OS Liberada Pela Fábrica', 'cor' => '#CCFFFF'],
							    ['descricao' => 'Os Reincidente e Aberta a mais de 25 dias', 'cor' => '#CC9900'],
							    ['descricao' => 'OS com Ligação no CallCenter da Fábrica', 'cor' => '#FFAA33']
	];
}
...
function retorna_botao_excluir_os($con, $os, $reparoNaFabrica = '', $posicao, $sua_os = '')
{	
	                ...
	                if (strlen($data_conserto) == 0) {
	                	$botao_excluir_os = '
	                						<li>
												...
												<a href="javascript: if(confirm(\''.traduz("deseja.realmente.excluir.a.os",$con).' (OS nº '.$numero_os.')!\') == true) { excluirOs('.$os.','.$posicao.');}" title="'.traduz("Excluir O.S.").'" ><i class="icon-remove"></i>  '.traduz("Excluir O.S.").'</a>
...
{
	global $cook_idioma, $login_fabrica, $login_posto; 
	...
	$defeito_constatado = false;

	$sql = " SELECT os FROM tbl_os WHERE fabrica = $login_fabrica AND os = $numero_os AND defeito_constatado NOTNULL";
	$res = pg_query($con, $res);
	if (pg_num_rows($res) > 0) {
		$defeito_constatado = true;
	}

	$botao_consertado =  "";

	if (empty($data_conserto)  && empty($finalizada) && !empty($login_posto) && $defeito_constatado) {
		$botao_consertado =  '
							<li>
