...
$ob = valida_obrigatoriedade_celular();
if ($ob == 'ok') {
...
$regras["produto|fora_garantia"] = array(
...
$regras["produto|versao"] = array(
    "obrigatorio" => true
);

$auditorias = array(
  "auditoria_fora_garantia",
...
$pre_funcoes_fabrica = array("valida_anexo_termo");

$valida_anexo_boxuploader = "valida_anexo_boxuploader";

$valida_anexo = "valida_anexo_osTermo";

$funcoes_fabrica = array("verifica_defeito_constatado");

$antes_valida_campos = "verifica_conserto_os_item";

function verifica_conserto_os_item() {
  
  global $con, $os, $login_fabrica, $campos, $historico_alteracao;
  ...
  foreach ($campos["produto_pecas"] as $peca) {
    ...
    if (strlen($peca["id"]) > 0) { 
      ...
      if(empty($peca["os_item"])) {
        ...
        if (pg_num_rows($res) > 0) {
           $servico_banco = "";
          $sql = "SELECT servico_realizado FROM tbl_os_item WHERE os_item = {$peca["os_item"]}";

          $r = pg_query($con,$sql);
          $servico_banco = pg_fetch_result($r, 0, servico_realizado);

          if ($servico_banco != $peca["servico_realizado"]) {
            $geraPedido = True;
          } 
        }
  ...
  if ($geraPedido) {
    $query = "SELECT data_conserto FROM tbl_os WHERE os = $os";
    $res = pg_query($con, $query);
    $dataConserto = pg_fetch_result($res, 0, data_conserto);

    if (!empty($dataConserto)) {
      $query = "UPDATE tbl_os SET data_conserto = Null WHERE os = $os";
      $res   = pg_query($con, $query);
    } 
  }
}
...
function valida_obrigatoriedade_celular() {
  global $os, $con;
  if (empty($os)) {
    return 'ok';
  } else {
    $sql = "SELECT data_abertura FROM tbl_os WHERE fabrica = 160 AND os = $os";
    $res_sql = pg_query($con, $sql);
    $data_abertura = pg_fetch_result($res_sql, 0, 'data_abertura');
    $data_corte = '2018-07-01';
    if(strtotime($data_abertura) > strtotime($data_corte)) {
      return 'ok';
    } else {
      return 'erro';
    } 
...
function tira_valida_garantia(){
  global $login_fabrica, $campos, $os, $con, $login_admin, $valida_garantia, $valida_anexo;

  if($campos['produto']['fora_garantia'] == 17){
      $valida_garantia = "";
      $valida_anexo = "valida_anexo_fora_garantia";
...
function grava_os_fabrica(){
      ...
      global $campos, $os, $anexos_obrigatorios;
