function itens_os_adicionar() {
	...
	item += $('<div>').append($("#div_peca_preco__modelo__").clone()).remove().html().replace(/__modelo__/g, sequencia);
	item += $('<div>').append($("#div_peca_ref__modelo__").clone()).remove().html().replace(/__modelo__/g, sequencia);
...
function analise_adicionar() {
		//$("div[id^=div_solucao_os]").css('width', '285px');
	$("select[id^=solucao_os]").css('width', '285px');

	$("div[id^=div_defeito_constatado]").css('width', '285px');
	$("input[id^=defeito_constatado]").css('width', '285px');
...
function mostra_anexo_solucao(posicao){
	//$("div[id^=div_solucao_os]").css('width', '285px');
	$("select[id^=solucao_os]").css('width', '285px');
	...
	var defeito_constatado = $("#defeito_constatado"+posicao+"_id").val();
	var solucao_os = $("#solucao_os"+posicao).val();
	var produto_id = $("#produto_id").val();

	$('#anexo_'+posicao).remove();

	//var solucao_os_analise = $("#solucao_os_analise").val();
	//$("#analise_os_header_labels").append('<label id="analise_os_header_label" class="header_label">Anexo</label>');

//	$("#analise_os_corpo").append('<div style="width:100px; height:20px; float:right; margin-left:5px; font-size:12px ">teste'+posicao+'</div>');
	$.ajax({
		type: "GET",
		url: "os_cadastro_unico_ajax.php",
		data: "diagnostico=true&solucao_os="+solucao_os+"&defeito_constatado=" + defeito_constatado+"&produto_id="+produto_id,
		dataType: "html",
		success: function(html){
			//$("#solucao_os"+seq).html(html);
			if(html.length > 0){
				$("#div_solucao_os"+posicao).append('<div id="anexo_'+posicao+'" style="width:100px; height:20px; float:right; margin-left:5px; font-size:12px; text-align:center ">'+html+'</div>');	
			}			
		}
	})
}
...
$(document).delegate('.solucao_os_analise', 'blur',function(){
	var posicao = $(this).attr("id").replace(/\D/g,'');
	mostra_anexo_solucao(posicao);
});

/*$(document).delegate('.solucao_os_analise', 'change',function(){
	var posicao = $(this).attr("id").replace(/\D/g,'');
	alert(posicao);
		...
		if ($("#produto_id").val().length) {
			...
			for (; ;) {
				if ($("#peca"+i).length) {
					...
					$('#itens_os_adicionar_linha').trigger('click');
					i = -1;
				}
				i++;
			}

			var peca_aux = document.getElementById('peca'+posicao+'_id');
			var referencia_aux = document.getElementById('peca_ref'+posicao);
			var descricao_aux = document.getElementById('peca'+posicao);
			var preco_aux = document.getElementById('peca_preco'+posicao);
			var descricao_last_aux = document.getElementById('peca'+posicao+'_last');


			var janela = null;
			var url = "peca_pesquisa_lista.php?produto=" + $("#produto_referencia").val()+"&exibe=os_cadastro.php";

			if (janela == null || janela.closed) {
				janela = window.open(url, "janela", "toolbar=no, location=yes, status=yes, scrollbars=yes, directories=no, width=560, height=400, top=18, left=5");
				peca = peca_aux; //Codigo
				referencia = referencia_aux;
				descricao = descricao_aux;
				preco = preco_aux;
				descricao_last = descricao_last_aux;
			}else{
				janela.focus();
			}
		}
