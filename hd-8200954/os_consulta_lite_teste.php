...
<style type="text/css" title="Diálogo motivo exclusão">
	#dlg_motivo {
	    display: none;
	    position: fixed;
	    opacity: 0.85;
...
$(function()
{
	...
	$('#produto_referencia').tooltip();
	$('#produto_descricao').tooltip();
...
if (strlen($msg_erro) > 0) {
	echo "<p class='error'><b>$msg_erro</b></p>";
}
echo "<br/>";
...
if ((strlen($btn_acao) > 0 AND strlen($msg) == 0) OR strlen($btn_acao_pre_os) > 0) {
	if (strlen($btn_acao_pre_os) > 0){
		if ($login_fabrica <> 52 and $login_fabrica <> 30) {
		...
		$sql .= " LEFT JOIN tbl_hd_chamado_item ON tbl_hd_chamado_item.os = tbl_os.os" ;

		$sql .= " LEFT JOIN tbl_hd_chamado_extra ON tbl_hd_chamado_extra.os = tbl_os.os" ;
		...
		if(strlen($rg_produto)>0){
			if($login_fabrica== 6 or $login_fabrica == 11) {
				if($login_posto== 4262 or $login_fabrica == 11) {
					$sql .= " AND tbl_os.rg_produto = '$rg_produto'";
				}
			} else {
...
/*		02/12/2009 - MLG HD 180918 - Se faz um strtoupper no $_POST, no WHERE fazer também um UPPER!
		...
		if (strlen($nf_compra) > 0) {
		    if (is_numeric($nf_compra)) {
				...
				$sql .= " AND UPPER(tbl_os.nota_fiscal) = '$nf_compra'";
			}
		}
		...
		if($login_fabrica == 43) {
			if (strlen($_POST['ordem_montagem'])>0) {
				...
				$sql .= " AND tbl_hd_chamado_extra.ordem_montagem = '$ordem_montagem' ";
			}
		}
	...
	if (pg_num_rows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_num_rows($res) ; $i++) {
			...
			if ($i % 50 == 0) {
				...
				if ($login_fabrica == 52 or $login_fabrica == 80 or $login_fabrica == 30 or $login_fabrica == 43 or $login_fabrica == 14) {
			...
			elseif (strlen($fechamento) == 0 AND $status_os!="62"  && $status_os!="65" && $status_os!="72" && $status_os!="87" && $status_os!="116" && $status_os!="120" && $status_os!="122" && $status_os!="126" && $status_os!="140" && $status_os!="141" && $status_os!="143" && $status_os != "98" && $status_os != "171") {
				if ($excluida == "f" || strlen($excluida) == 0) {
			...
			if (usaDataConserto($login_posto, $login_fabrica)) { //HD 13239
				...
				elseif($login_fabrica == 3 AND ($status_os == "120" || $status_os == "122" || $status_os == "126" || $status_os == "140" || $status_os == "141" || $status_os == "143")){
					echo "&nbsp;";
				}else{
					$os_troca = false;
...
<table width="400" align="center" border="0" cellspacing="0" cellpadding="2">
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td><input type="text" name="consumidor_cpf" size="17" value="<?echo $consumidor_cpf?>" class="frm" id='consumidor_cpf'></td>
		<? if($login_fabrica==19 OR $login_fabrica==10 OR ($login_fabrica==20 AND $login_pais=='AR')){ ?>
		<td></td>
		<? } ?>
		<td>
...
<table width="400" align="center" border="0" cellspacing="0" cellpadding="2">
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td>
		<input class="frm" type="text" name="produto_referencia" size="15" maxlength="20" value="<? echo $produto_referencia ?>" id='produto_referencia' title='<?fecho ('informe.parte.da.descricao.ou.da.referencia.e.clique.na.lupa.ao.lado.do.campo.para.pesquisar',$con,$cook_idioma)?>'>
		&nbsp;
		...
		<td>
		<input class="frm" type="text" name="produto_descricao" size="30" value="<? echo $produto_descricao ?>" id='produto_descricao' title='<?fecho ('informe.parte.da.descricao.ou.da.referencia.e.clique.na.lupa.ao.lado.do.campo.para.pesquisar',$con,$cook_idioma)?>'>
		&nbsp;
...
if(($login_fabrica == 3 and $linhainf ==528) or ($login_fabrica == 80 and $login_posto == 6359) or ($login_fabrica == 91 and $login_posto == 6359) or in_array($login_fabrica,array(59,14,24,43,66,52,30,81,46,89)) ) {
?>
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td colspan='2'> <hr> </td>
	</tr>
...
?>
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td colspan='2'> <?
			...
			?>
		<input class="frm" type="text" name="revenda_cnpj" id='revenda_cnpj' size="8" value="<? echo $revenda_cnpj ?>" >
	<? if ($sistema_lingua<>'ES'){?>
		 /0001-00
<? } ?>
		</td>
