...
$vet_email_consumidor = array(14,43,59,66);
...
$historico_intervencao = (in_array($login_fabrica, array(1,2,3,6,11,14,25,35,43,45,50)) or $login_fabrica > 84);
...
//	include "ajax_cabecalho.php";
	...
	if($login_fabrica == 3){
		...
		$res = pg_query ($con,$sql);
		if (pg_num_rows ($res) > 0) {
			$sql = "UPDATE tbl_os SET defeito_constado = 0 WHERE tbl_os.os = $os";
			$res = pg_query ($con,$sql);
		}
	...
	if (strlen ($msg_erro) == 0) {
		...
		$res = pg_query ($con,$sql);
		$msg_erro = pg_errormessage($con) ;
		if (strlen ($msg_erro) == 0 and ($login_fabrica==1 or $login_fabrica==24 OR ($login_fabrica == 3 and $login_posto == 6359))) {
...
if (strlen($btn_acao) > 0 ) {
	...
	if (strlen($sua_os)==0 && strlen($rg_produto)==0 && strlen($serie)==0 && strlen($nf_compra)==0 && strlen($consumidor_cpf)==0 && strlen($os_posto)==0 && strlen($consumidor_nome)==0 && strlen($produto_referencia)==0 && strlen($os_aberta)==0){
		...
		if($data_inicial && $data_final){
			...
			if(strlen($msg)==0){
				if (strtotime($aux_data_inicial) < strtotime($aux_data_final . ' -30 day')) {
					$msg = traduz("Período não pode ser maior de 31 dias",$con,$cook_idioma);
				}
			}
...
<style type="text/css">
	#dlg_motivo {
		...
		box-shadow: 3px 3px 3px WindowFrame;
        border-radius: 8px;
	...
	#dlg_motivo #motivo_header {
	    ...
	    padding: 2px 1em;
	    color: inherit;
	    font: normal bold 12px Segoe UI, Verdana, MS Sans Serif, Arial, Helvetica, sans-serif;
	}
	...
	#dlg_motivo #dlg_fechar {
        ...
        filter:progid:DXImageTransform.Microsoft.DropShadow(color='#555555', offX=3px, offY=3px,enabled=true,positive='false');
	}
	#dlg_motivo button {
		float: right;
		margin: 5px;
...
var exigir_motivo = (<?=$login_fabrica?> != 1);
$().ready(function() {
	$('#dlg_motivo').draggable({cursor: 'move', handle : 'div#motivo_header'});
	$('p[id^=excluir],img[id^=excluir]').css('cursor','pointer').click(function () {
		...
		if (confirm('<?=fecho("deseja.realmente.excluir.a.os",$con,$cook_idioma)?> (OS nº '+sua_os+')') == false) return false;
		if (exigir_motivo) {
...
//						alert ('OS em intervenção por Número de Série');
					}else if (http.responseText.indexOf ('informe o motivo de atraso de fechamento na aba')>0){
						alert ('Favor informar o motivo de atraso de fechamento na aba O.Serviço');
					}else if(http.responseText.indexOf('OS não pode ser finalizada pois esta OS encontra-se em auditoria') > 0){
						alert ('OS não pode ser finalizada pois esta OS encontra-se em auditoria');	
...
<?
if (strlen($msg) > 0) {
	echo "<div align='center'><div width='700' style='width:700px' class='error'>$msg</div></div>";
}
...
if ((strlen($btn_acao) > 0 AND strlen($msg) == 0) OR strlen($btn_acao_pre_os) > 0) {
	if (strlen($btn_acao_pre_os) > 0){
		...
		if (strlen($sua_os) > 0) {
			...
			if ($pos === false) {
				if(!ctype_digit($sua_os)){
					...
					$cond_os .= " AND tbl_os.os_numero = '$sua_os' ";
				}
			}else{
		...
		if (strlen($os_aberta) > 0) {
			$cond_os .= " AND tbl_os.os_fechada IS FALSE";
		}
		...
		if ($login_fabrica == 96) {//HD391024
			$campo_check = ", tbl_os.status_checkpoint ";
		}
		// OS não excluída
		...
		$sql2  .= " 	JOIN      tbl_posto_fabrica ON  tbl_posto_fabrica.posto   = tbl_posto.posto AND tbl_posto_fabrica.fabrica = $login_fabrica
					LEFT JOIN tbl_produto       ON  tbl_produto.produto       = tmp_consulta.produto
					LEFT JOIN tbl_hd_chamado_extra ON tbl_hd_chamado_extra.os = tmp_consulta.os
					JOIN      tbl_os_extra      ON  tbl_os_extra.os           = tmp_consulta.os";
		...
		$sql2 .= " 	LEFT JOIN tbl_posto_linha           ON tbl_posto_linha.linha         = tbl_produto.linha AND tbl_posto_linha.posto = tmp_consulta.posto
					...
					WHERE (tbl_os_extra.status_os NOT IN (13,15) OR tbl_os_extra.status_os IS NULL)";
		...
		if ($login_fabrica == 7) {
			...
			$sql2 .= " ORDER BY tmp_consulta.sua_os DESC";
		}
		
		$sqlT = preg_replace ("/\n|\t/"," ",$sql.$sql2) ;

		#$sql_log = "INSERT INTO log_os_consulta_lite(sql) VALUES('" . str_replace("'", "\'", $sql.$sql2) . "')";
	...
	if (pg_num_rows($res) > 0) {
		...
		echo "<tr height='18'>";
			...
			echo "<td align='left'><font size='1'><b>&nbsp; ";
			echo "Os reincidente e aberta a mais de 25 dias ";
			echo "</b></font></td>";
		echo "</tr>";
		...
		echo "<tr height='18'>";
			echo "<td width='18' bgcolor='#FFAA33'>&nbsp;</td>";
			echo "<td align='left'><font size='1'><b>&nbsp; ";
			echo "OS com ligação no CallCenter da fábrica ";
			echo "</b></font></td>";
		echo "</tr>";
		echo "<tr height='3'><td colspan='2'></td></tr>";
		...
		if ($login_fabrica == 96 && strlen($btn_acao_pre_os) == 0) { //HD391024
			echo "<tr height='18'>";
				echo "<td width='18' bgcolor='#CCCCFF'>&nbsp;</td>";
				echo "<td align='left'><font size='1'><b>&nbsp; Aguardando Analise </b></font></td>";
			echo "</tr>";
			echo "<tr height='3'><td colspan='2'></td></tr>";
		...
		<div id='dlg_motivo'>
			<div id='motivo_header'>Informe o motivo da exclusão</div>
			...
			<div id='motivo_container'>
	            ...
	            <br>
	            <button type="button" id="dlg_btn_excluir">Excluir</button>
	            <button type="button" id="dlg_btn_cancel">Cancelar</button>
            </div>
		</div><?php
		...
		for ($i = 0 ; $i < pg_num_rows($res) ; $i++) {
			...
			if ($i % 50 == 0) {
				...
				if (in_array($login_fabrica,array(52,80,30,43,14,96))) {
					echo "<td>Atendimento nº</td>";
				}
				
				if ($login_fabrica == 14 or $login_fabrica == 43) {
					echo "<td>Ordem de Montagem</td>";
					echo "<td>Codigo de Postagem</td>";
				}
				...
				#SISTEMA RG
				if($login_posto==6359 OR $login_posto==4311 or ($login_posto== 4262 and $login_fabrica==6) or $login_fabrica == 11){
				...
				if ($login_fabrica == 1) {
					...
					$colspan = "8";
				}elseif(usaDataConserto($login_posto, $login_fabrica)){
					$colspan = "6";
				}else{
					$colspan = "5";
				}
			...
			if ($login_fabrica == 14 || $login_fabrica == 43) {
				...
				echo "<td style='background:$cor_status' align='center' nowrap onclick='alert(\"$justificativa\");'><acronym style='cursor:help' title='Clique aqui para ver a justificativa'>$legenda_status</acronym></td>";
			}
			//HD 214236::: FIM :::
			...
			if ($sinalizador == 18) {
				...
				echo "$aux_fechamento - FECHAMENTO AUTOMÁTICO' style='cursor: help; color:#FF0000; font-weight: bold;'>F. AUT</acronym></td>";
			}
			else {
				echo "<td nowrap><acronym title='".traduz("data.fechamento",$con,$cook_idioma).": ";
			...
			if(strlen($btn_acao_pre_os)>0){
				...
				if (!in_array($login_fabrica, array(14,43,66,88))) {
					echo "<a href='os_cadastro.php?pre_os=t&serie=$serie&hd_chamado=$hd_chamado&hd_chamado_item=$hd_chamado_item'>";
				}
					elseif ($login_fabrica == 43) {
				echo "<a href='os_cadastro.php?pre_os=t&serie=$serie&hd_chamado=$hd_chamado'>";
				} else {
					echo "<a href='os_cadastro_intelbras_ajax.php?pre_os=t&serie=$serie&hd_chamado=$hd_chamado'>";
				}
				echo traduz("abrir.pre-os",$con,$cook_idioma)."</a></td>\n";
	}else{
			...
			elseif ($troca_garantia == "t" OR (($status_os=="62" and $produto_critico <> 't') || $status_os=="65" || $status_os=="72" || $status_os=="87" || $status_os=="116" || $status_os=="120" || $status_os=="122" || $status_os=="126" || $status_os=="140" || $status_os=="141" || $status_os=="143")) {
						...
						if($login_fabrica==19){
							...
							if($sistema_lingua == "ES"){
								...
								if($login_fabrica==11 AND strlen($data_conserto)>0 AND $data_anterior == 't'){
									...
									if ($login_fabrica == 81 and strlen($os_troca)>0) {
										echo "<img id='lancar_$i' border='0' src='imagens/pixel.gif'>";
									} else {
			...
			elseif (strlen($fechamento) == 0 && strlen($pedido) == 0) {
					...
					if ($excluida == "f" || strlen($excluida) == 0) {
						if (strlen ($admin) == 0) {
							if($login_fabrica == 1 AND ($tipo_atendimento == 17 OR $tipo_atendimento == 18) AND $valores_adicionais > 0){
								echo "<p id='excluir_$i' border='0' alt='$os,$sua_os' title='Excluir $sua_os'></p>";
							}else{
								# HD 311411 - Condição para não exibir botão excluir ao posto quando ele estiver com data de conserto
								if($login_fabrica != 6 || strlen($data_conserto) == 0){
									echo "<img id='excluir_$i' border='0' src='imagens/btn_excluir.gif' alt='$os,$sua_os' title='Excluir $sua_os'>";
								}
							}
			...
			elseif (strlen($fechamento) == 0 AND $status_os!="62"  && $status_os!="65" && $status_os!="72" && $status_os!="87" && $status_os!="116" && $status_os!="120" && $status_os!="122" && $status_os!="126" && $status_os!="140" && $status_os!="141" && $status_os!="143" && $status_os != "98") {
				if ($excluida == "f" || strlen($excluida) == 0) {
				...
				if ($login_fabrica == 51 AND $status_os =='62') {
					...
					if ($sinalizador == 18) {
						echo "<img border='0' src='/assist/imagens/btn_fecha.gif' onclick='alert(\"Esta OS foi fechada automaticamente pelo sistema. Utilizar a tela de Fechamento de OS para informar a entrega do produto ao consumidor e enviar a nota fiscal para a fábrica para pagamento da mão de obra\")' style='cursor:pointer;'>";
					}
					else {
						echo "&nbsp;";
					}
			...
			if (usaDataConserto($login_posto, $login_fabrica)) { //HD 13239
				echo "<td width='60' align='center'>";
				//HD:44202
				//HD 214236: Travar as opções quando a OS estiver em auditoria
				if (($login_fabrica == 14 || $login_fabrica == 43) && ($auditoria_travar_opcoes)) {
				}
				elseif($login_fabrica == 3 && ($status_os == "120" || $status_os == "122" || $status_os == "126" || $status_os == "140" || $status_os == "141" || $status_os == "143") || ($status_os == "62" && $login_fabrica == 6)){
					echo "&nbsp;";
				}else{
					$os_troca = false;
					...
					if ( (strlen($data_conserto) ==0) ) {
						...
						$botao_consertado =  "<a href=\"javascript: if (confirm('".traduz("apenas.clicar.ok.se.tiver.certeza.que.a.data.de.conserto.do.produto.da.%.seja.hoje",$con,$cook_idioma,array($sua_os))."!') == true) { consertadoOS ($os,consertado_$i,$i);}\"><img id='consertado_$i' border='0' src='/assist/imagens/btn_consertado.gif'></a>";
		...
		if ($login_fabrica == 20) { //hd 88308 waldir HD 196744
			...
			echo "<tr>";
				...
				?>
					...
					<input type='text' size='18' maxlength="16" name='data_fechamento_<? echo $i;?>' id='data_fechamento_<? echo $i?>' onKeyPress='DataHora(event, this)'> <input type='button' value='<? fecho("fechar.os",$con,$cook_idioma); ?>' onclick="javascript: if (data_fechamento_<? echo $i;?>.value.length<10){ alert('digite uma data no formato dd/mm/aaaa!');} else {fechaOSnovo2(<?php echo $os; ?>,data_fechamento_<? echo $i;?>.value,sinal_<? echo $i;?>,excluir_<?echo $i;?>,lancar_<? echo $i ?>,<?echo $i?>); }">
					</div>
				<?
				echo "<td>";
			echo "</tr>";
...
<table align="center" class="formulario" width="700" border="0">
	...
	<tr align='left'>
		<td class="espaco"> Tipo de OS</td>
		<?if ($login_fabrica == 43) {?>
		<td>Ordem de Montagem</td>
		<?}?>
		<td colspan='2'></td>
	</tr>
	<tr align='left'>
...
<table align="center" class="formulario" width="700" border="0">
	...
	<tr align='left'>
		...
		<td><input type="text" name="consumidor_nome" size="30" onkeyup="somenteMaiusculaSemAcento(this)" value="<? echo $consumidor_nome; ?>" class="frm"> <img src='imagens/help.png' title='Clique aqui para ajuda na busca deste campo' onclick='mostrarMensagemBuscaNomes()'></td>
	</tr>
	...
	<tr align='left'>
		<td class="espaco">
			...
			&nbsp;
			<img src='imagens/lupa.png' border='0' align='absmiddle' style="cursor:pointer" onclick="javascript: fnc_pesquisa_produto (document.frm_consulta.produto_referencia, document.frm_consulta.produto_descricao,'referencia',document.frm_consulta.voltagem)">
		</td>

		<td>
			<input class="frm" type="text" name="produto_descricao" size="30" value="<? echo $produto_descricao ?>" id='produto_descricao'>
			&nbsp;
			<img src='imagens/lupa.png'  style="cursor:pointer" border='0' align='absmiddle' onclick="javascript: fnc_pesquisa_produto (document.frm_consulta.produto_referencia, document.frm_consulta.produto_descricao,'descricao',document.frm_consulta.voltagem)">
		</td>
	</tr>
...
?>
	...
	<tr align='left'>
		...
		<td>
			<input class="frm" type="text" name="revenda_cnpj" id='revenda_cnpj' size="12" value="<? echo $revenda_cnpj ?>" >
			<? if ($sistema_lingua<>'ES'){?>
				 /0001-00
			<? } ?>
...
<?
if(($login_fabrica == 3 and $linhainf ==528) or ($login_fabrica == 80 and $login_posto == 6359) or ($login_fabrica == 91 and $login_posto == 6359) or in_array($login_fabrica,array(59,14,24,43,66,52,30,81,46,89,96))) {
?>
	<table align="center" class="formulario" width="700" border="0">
		<tr align='left'>
			<td colspan='2'>&nbsp;</td>
