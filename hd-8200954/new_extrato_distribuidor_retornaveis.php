...
if ($login_e_distribuidor <> 't' && $login_fabrica <> 140) {
...
function fnc_consulta_os(data_extrato,peca,os) {
	window.open("new_extrato_distribuidor_retornaveis_os.php?data="+data_extrato+"&peca="+peca+"&os="+os, "Pesquisa", "toolbar=no, location=no, status=yes, scrollbars=yes, directories=no, width=637, height=400, top=50, left=50");
}
</script>
...
<tr>
	<?php
		$link_outro_extrato = ($login_fabrica == 140) ? "os_extrato_novo_lgr.php" : "new_extrato_distribuidor.php";
	?>
<? if($login_fabrica <> 140){?>
<td align='center' width='50%'><a href='new_extrato_distribuidor.php?periodo=<? echo $data ?>'>Ver extrato total</a></td>
<? } ?>
<td align='center' width='50%'><a href='<?=$link_outro_extrato?>'>Ver outro extrato</a></td>
</tr>
</table>
...
if($login_fabrica == 140){
	$sql_extrato_lavor = " AND tbl_os_extra.extrato IN(SELECT tbl_extrato.extrato
					FROM   tbl_extrato
					WHERE  tbl_extrato.fabrica = $login_fabrica
					ORDER BY tbl_extrato.extrato DESC LIMIT 6) ";
}else{
	$sql_extrato = "JOIN (
...
if (strlen ($data) > 0 OR $login_fabrica == 140) {
	$sql = "SELECT  tbl_os_extra.linha           ,
			...
			$sql_extrato_lavor
			GROUP BY tbl_os_extra.linha, tbl_os.os , tbl_linha.nome, tbl_peca.peca, tbl_peca.referencia, tbl_peca.descricao 
...
#exit;
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
		...
		echo "<td><a href=\"javascript: fnc_consulta_os ('$data','".pg_result ($res,$i,peca)."', '{$os}')\">" . pg_result ($res,$i,referencia) . "</a></td>\n";
