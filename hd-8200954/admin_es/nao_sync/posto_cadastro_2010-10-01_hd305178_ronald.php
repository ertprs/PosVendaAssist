...
if ($btn_acao == "gravar") {
	...
	if(strlen($xcnpj) == 0)
		$msg_erro = "Digite la Identificación del servicio.";
	else
		$cnpj = $xcnpj;
	...
	if (strlen($msg_erro) == 0){
		...
		if (strlen($msg_erro) == 0) {
			...
			if (strlen($msg_erro) == 0){
				...
				if (strlen($msg_erro) == 0 AND strlen($posto) > 0 AND strlen($xcodigo_posto) > 0 AND $login_fabrica == 11) {
					...
					if (pg_numrows($resx) > 0) $msg_erro = "Ya existe un servício catastrado con el código $xcodigo_posto";
				}
		...
		if (strlen($msg_erro) == 0 && $login_fabrica == 1) {
			...
			for ($i = 0 ; $i < pg_numrows($res1) ; $i++) {
				...
				$msg_erro = pg_errormessage($con);
				if (strlen($msg_erro) > 0) {
					$msg_erro = " No fue posible catastrar la condición de pagamiento p/ esto servicio. ";
					break;
				}
			}
			if($login_fabrica==1){
...
if (strlen($posto) > 0 and strlen ($msg_erro) == 0 ) {
	...
	if (pg_numrows ($res) > 0) {
		...
		$prestacao_servico_sem_mo= trim(pg_result($res,0,prestacao_servico_sem_mo));
...
<table class="border" width='650' align='center' border='0' cellpadding="1" cellspacing="3">
	...
	<tr class='menu_top'>
		<td colspan='5'>Contraseña do financeiro</td>
	</tr>
	<tr class='table_line'>
...
<table class="border" width='650' align='center' border='0' cellpadding="1" cellspacing="3">
	...
	<tr class="menu_top">
		<td width = '33%'>IDENTIFICACIÓN 01 - FAVORECIDO</td>
...
<TR>
	...
	<TD align='left'>PRESTACIÓN DE SERVICIO<br><font size='-2'>&nbsp;Servicio solo recibe mano de obra.</font></TD>
</TR>
<TR>
	<TD align='center'>
...
<TR>
	...
	<TD align='left'>PRESTACION DE SERVICIO SIN MANO DE OBRA<br><font size='-2'>&nbsp;Servicio solamente recibe el valor de piezas. Mano–de-obra no es cobrada.</font></TD>
</TR>
<? } ?>
...
<br>
<center>
<img src="imagens_admin/btn_gravar.gif" style="cursor: pointer;" onclick="javascript: if (document.frm_posto.btn_acao.value == '' ) { document.frm_posto.btn_acao.value='gravar' ; document.frm_posto.submit() } else { alert ('Aguarde submisión') }" ALT="Guardas formulário" border='0'>
<img src="imagens_admin/btn_limpar.gif" style="cursor: pointer;" onclick="javascript: if (document.frm_posto.btn_acao.value == '' ) { document.frm_posto.btn_acao.value='reset' ; window.location='<? echo $PHP_SELF ?>' } else { alert ('Aguarde submisión') }" ALT="Borrar campos" border='0'>
</center>
<br>
<!-- ============================ Botoes de Acao ========================= -->
	...
	if (pg_numrows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
			if ($i % 20 == 0) {
				...
				if($login_fabrica==20){ echo "<td>PRESTACION DE SERVICIO SIN MANO DE OBRA</td>";}
				echo "</tr>";
			}
			...
			if($login_fabrica == 20) {
				echo "<td>";
				if (pg_result($res,$i,prestacao_servico_sem_mo) == "t") echo "<img border='0' src='imagens/img_ok.gif'>";
				echo "</td>";
			}
			echo "</tr>";
		}
