...
$title = "Reporte de cantidad de OS aprobadas por línea";
...
echo "<td rowspan='2'><b>Nombre</td>";
if($login_fabrica==20) echo "<td rowspan='2'><b>Pais</td>";

$sql = "SELECT nome FROM tbl_linha WHERE fabrica = $login_fabrica ORDER BY nome";
...
$sql = "SELECT  tbl_posto_fabrica.codigo_posto ,
		...
		LEFT JOIN (
			SELECT COUNT(*) AS qtde , tbl_os.posto, tbl_linha.nome AS linha_descricao , TO_CHAR (tbl_extrato.aprovado,'MM') AS mes
			FROM tbl_os
			...
			GROUP BY tbl_os.posto, tbl_linha.nome, TO_CHAR (tbl_extrato.aprovado,'MM')
		) os ON tbl_posto.posto = os.posto
		WHERE tbl_posto.pais = '$login_pais'
		ORDER BY nome_posto, mes, linha_descricao ";
