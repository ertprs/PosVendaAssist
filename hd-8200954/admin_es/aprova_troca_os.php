...
if(strlen($btn_acao)>0 AND strlen($select_acao)>0){
	...
	$observacao = (strlen($observacao) > 0) ? "' Observación: $observacao '" : " NULL ";
	...
	for ($x=0;$x<$qtde_os;$x++){
		...
		if (strlen($xxos) > 0 AND strlen($msg_erro) == 0){
			...
			$sql = "SELECT contato_email,pais,tbl_os.sua_os, tbl_os.posto, tbl_os.tipo_atendimento
			...
			$posto_pais  = pg_result($res_x,0,pais);
	...
	$cabecalho .= "from: helpdesk@telecontrol.com.br";
									...
									@mail($destinatario, utf8_encode($assunto), utf8_encode($mensagem), $cabecalho);
								}
							}
						}
						...
						$corpo ="\n Su OS($sua_os) fue aprobada. Imprima una copia y envíe junto al producto\n\n";
						$corpo.="\nResponsable que  concedió la aprobación: $promotor\n\n";
						$corpo.="\n\nRazón del Cambio: $motivo ";
						if(strlen($m_observacao) > 0 ) {
							$corpo.="\nObservación: $m_observacao";
						}
						$corpo.="\n Identificación del defecto y Defecto: ";
						$cd_descricao = str_replace("'",'"',$cd_descricao);
						$corpo.="\n\n$s_descricao &nbsp;&nbsp; $cd_codigo - $cd_descricao ";
						$corpo_comunicado = $corpo;
						$corpo.="\n_______________________________________________\n";
						$corpo.="\nPor favor no contestar ese correo .";
						$descricao_comunicado = "Aprobacion de Cambio da OS";
						$cabecalho  = "mime-version: 1.0 \r";
						$cabecalho .= "content-type: text/html; charset=iso-8859-1 \r";
						$cabecalho .= "from: helpdesk@telecontrol.com.br";

						if ( @mail($email_destino, stripslashes($assunto), $corpo, $cabecalho) ){
						}
					}
					//Recusada
					...
					if($select_acao == "94"){
						...
						$descricao_comunicado = "Cambio de OS Reprovada";
...
if (strlen($btn_acao)  > 0 AND strlen($msg_erro)==0) {
	...
	$sql =  "SELECT interv.os
			...
			SELECT
			ultima.os,
			(SELECT status_os FROM tbl_os_status WHERE fabrica_status = $login_fabrica AND status_os IN (92,93,94) AND tbl_os_status.os = ultima.os ORDER BY data DESC LIMIT 1) AS ultimo_status
			FROM (SELECT DISTINCT os FROM tbl_os_status WHERE fabrica_status = $login_fabrica AND status_os IN (92,93,94) ) ultima
			) interv
			...
			SELECT	tbl_os.os                                                   ,
					...
					(SELECT status_os FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND fabrica_status = $login_fabrica AND status_os IN (92,93,94) ORDER BY data DESC LIMIT 1) AS status_os         ,
					(SELECT observacao FROM tbl_os_status WHERE tbl_os.os = tbl_os_status.os AND fabrica_status = $login_fabrica AND status_os IN (92,93,94) ORDER BY data DESC LIMIT 1) AS status_observacao,
					(SELECT tbl_status_os.descricao FROM tbl_os_status JOIN tbl_status_os USING(status_os) WHERE tbl_os.os = tbl_os_status.os AND fabrica_status = $login_fabrica AND status_os IN (92,93,94) ORDER BY data DESC LIMIT 1) AS status_descricao
