...
if(strlen($data_inicial)>0 AND strlen($data_final)>0 AND $agrupar<>'sim'){
	...
	if (pg_numrows($res) > 0) {
		...
		fputs ($fp,"<td >GENERACIÓN</td>");
		fputs ($fp,"<td align='center'>M.O</td>");
		fputs ($fp,"<td >REPUESTO</td>");
		fputs ($fp,"<td >AVULSO</td>");
		//hd 39502
		if ($login_fabrica==20) {
			fputs ($fp,"<td >TOTAL CORTESIA</td>");
			fputs ($fp,"<td >TOTAL GERAL</td>");
		} else {
		...
		for ($i=0; $i<pg_numrows($res); $i++){
			...
			//hd 39502
			if ($login_fabrica==20) {
				fputs ($fp,"<td bgcolor='$cor' align='right'>$ $total_cortesia</td>");
			}
