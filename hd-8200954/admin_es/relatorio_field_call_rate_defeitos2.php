...
#Está gerando duplicidade das OS, número superiores ao real - HD 17679
...
//AND    (tbl_os_status.status_os NOT IN (13,15) OR tbl_os_status.status_os IS NULL) - HD 17679
$sql .= "  
AND     tbl_os.fabrica = $login_fabrica
AND     tbl_os_item.peca = $peca
AND     $cond
...
if (pg_numrows($res) > 0) {
	...
	echo "<TR>";
	echo "<TD class='titChamada10'>FALLA</TD>";
	echo "<TD class='titChamada10'>OCURRENCIA</TD>";
...
echo "</form>";
	...
	for ($i = 0;$i < pg_numrows($res);$i++){
		...
		echo "	<TD class='conteudo10' align='left'>".$descricao_defeito."</a></TD>";
	...
	echo "<br>";
	echo "Ningún resultado encuentrado entre $data_inicial e $data_final $mostraMsgLinha $mostraMsgEstado</b>";
}
?>
<br>
...
if($login_fabrica == 20 and $pais !='BR'){
	...
	$join_pais = " JOIN tbl_posto ON tbl_posto.posto = tbl_os.posto AND tbl_posto.pais = '$pais'";
}else{
...
$sql = "SELECT tbl_produto.referencia,
		...
		FROM (
			...
			$sql .="WHERE   tbl_os_item.peca = $peca
					AND     tbl_os.fabrica   = $login_fabrica
					AND     $cond ";
			if(strlen($posto) >0 ) {
...
if (pg_numrows($res) > 0) {
	...
	echo "<TR>";
	echo "<TD class='titChamada10'>REFERENCIA</TD>";
	echo "<TD class='titChamada10'>PRODUCTO</TD>";
	echo "<TD class='titChamada10'>OCURRENCIA</TD>";
	echo "<TD class='titChamada10'>%</TD>";
	echo "</TR>";
