...
<?
	if ($login_fabrica <> 1) $url = "peca_pesquisa_lista";
	else                     $url = "peca_pesquisa_blackedecker";
?>

function fnc_pesquisa_peca_lista (produto_referencia, peca_referencia, peca_descricao, peca_preco, voltagem, tipo, peca_qtde, posto) {
	var url = "";
	if (tipo == "tudo") {
		url = "<? echo $url; ?>.php?produto=" + produto_referencia + "&descricao=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&posto=" + posto + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}

	if (tipo == "referencia") {
		url = "<? echo $url; ?>.php?produto=" + produto_referencia + "&peca=" + peca_referencia.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&posto=" + posto + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}

	if (tipo == "descricao") {
		url = "<? echo $url; ?>.php?produto=" + produto_referencia + "&descricao=" + peca_descricao.value + "&tipo=" + tipo + "&voltagem=" + voltagem.value + "&posto=" + posto + "&exibe=<? echo $_SERVER['REQUEST_URI']; ?>";
	}
<? if ($login_fabrica <> 2) { ?>
...
<? if ($login_fabrica <> 2) { ?>
		...
		alert("Digite al minus 3 caracteres!");
	}
<? } ?>
}
/* ####################################################################### */
