...
if (strlen($_GET["exportar"])>0){
	...
	echo "ok|Exportación concluida con éxito! En unos minutos los archivos de exportación estarán disponibles en el sistema.";
	exit;
}
...
function gerarExportacao(but){
	 if (but.value == 'Exportar Extratos' || but.value == 'Exportar extractos') {
		if (confirm('¿Realmente desea continuar con la exportación? Será solamente de extractos libres Aprobados y Liberados exportados.')){
			but.value='Exportación...';
			exportar();
		}
	} else {
		 alert ('Espere mientras se procesa la petición anterior...');
	}
		...
		for ($i = 0 ; $i < pg_num_rows ($res) ; $i++) {
			...
			if ($i == 0) {
				...
				if ($login_fabrica == 1 ) {
					...
					//hd 39502
					if ($login_fabrica==20) {
						echo "<td align='center'>Total cortesía</td>\n";
						echo "<td align='center'>Total geral</td>\n";
					} else {
...
// 			$x_data_final = substr ($data_final,6,4) . "-" . substr ($data_final,3,2) . "-" . substr ($data_final,0,2);
		...
		if (pg_num_rows ($res) == 0) {
			echo "<center><h2>Ningún extracto encontrado</h2></center>";
		}
	
		if (pg_num_rows ($res) > 0) {
			for ($i = 0 ; $i < pg_num_rows ($res) ; $i++) {
				$row  = trim(pg_fetch_assoc($res,$i);   // Copia o registro num array com key=nome campo e o valor como valor
                foreach ($row as $campo => $valor) {    // Coloca cada valor na variável com o nome do campo
                    $$campo = trim($valor);
                }
// 				$total = number_format ($total,2,',','.');   // Está no SELECT a formatação
