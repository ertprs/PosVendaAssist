...
echo "<table border='0' cellspacing='0' cellpadding='6' align='center' bgcolor='#596D9B' style='font-family: verdana; font-size: 12px'>";
echo "<tr>";
...
if (strlen($btn_acao)>0){
	...
	if (strlen($mes)==0){
		$msg_erro = "<script>alert('Por favor elija el mês');</script> ";
	}
	if (strlen($ano)==0){
		$msg_erro = "<script>alert('Por favor elija el ano');</script> ";
	}
	...
	if(strlen($codigo_posto)>0) {
		...
		if (pg_numrows ($res)>0){
			...
			$msg_erro = "Servicio $codigo_posto no encuentrada";
		}
	}
	...
	if(strlen($msg_erro)==0){
		$sql = "SELECT tbl_posto_fabrica.codigo_posto                   ,
				...
				JOIN (
					...
					JOIN tbl_posto         ON tbl_os.posto = tbl_posto.posto AND tbl_posto.pais = '$login_pais'
		...
		echo "</table>";
			...
			echo "<center>Ningun resultado encuentrado</center>";
		}
	}
