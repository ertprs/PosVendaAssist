...
//takashi alterei novamente conforme Tulio e Samuel falaram
	if((strlen($sua_os)>0) and (strlen($sua_os)<4))$msg="Digite el mínimo 3 caracteres para efetuar la búsqueda";
	...
	if (strlen ($consumidor_nome) > 0 AND strlen ($codigo_posto) == 0 AND strlen ($produto_referencia) == 0) {
		$msg = " Especifique el Servicio Técnico  o el Producto";
	}
	...
	if (strlen ($revenda_cnpj) <> 8 AND strlen ($revenda_cnpj) > 0) {
		$msg = "Escriba los 8 primeros dígitos de la Identificación";
	}
	...
	if ( (strlen ($codigo_posto) > 0 OR strlen ($posto_nome) > 0 OR strlen ($consumidor_nome) > 0 OR strlen ($produto_referencia) > 0 ) AND ( strlen ($mes) == 0 OR strlen ($ano) == 0) )  {
		$msg = "Escriba el mes y el año para hacer la búsqueda";
	}
	...
	if ( strlen ($posto_nome) > 0 AND strlen ($posto_nome) < 5 ) {
		$msg = "Teclee al menos 5 letras para el nombre del servicio";
	}

	if ( strlen ($consumidor_nome) > 0 AND strlen ($consumidor_nome) < 5) {
		$msg = "Teclee al menos 5 letras para el nombre del consumidor";
	}

	if ( strlen ($serie) > 0 AND strlen ($serie) < 5) {
		$msg = "Teclee al menos 5 letras para el número de série";
	}
...
$title = "Seleción de Parámetros para Relación de Órdenes de Servicio Registradas";
...
if (strlen ($os_off) == 0 AND 
	...
	strlen ($produto_referencia) == 0 and strlen($_POST['btn_acao']) > 0) {
		$msg = "Necesario especificar más campos para la búsqueda";
}
#--------------------------------------------------------------------
...
/*	##### PAGINAÇÃO - INÍCIO #####
	...
	echo "<br><h1>Resultado de la búsqueda: $resultados</h1>";
}
?>
...
<table width="400" align="center" border="0" cellspacing="0" cellpadding="2">
	...
	<tr class="Conteudo" bgcolor="#D9E2EF" align='left'>
		<td>
			...
			<img border="0" src="imagens_admin/btn_lupa.gif" style="cursor: hand;" align="absmiddle" alt="Pulse aquí para localizar Servicios Técnicos por el Código" onclick="javascript: fnc_pesquisa_posto (document.frm_consulta.codigo_posto, document.frm_consulta.posto_nome, 'codigo')">
		</td>
		<td>
			...
			<img border="0" src="imagens_admin/btn_lupa.gif" style="cursor: hand;" align="absmiddle" alt="Pulse aquí para localizar Servicios Técnicos por el Nombre" onclick="javascript: fnc_pesquisa_posto (document.frm_consulta.codigo_posto, document.frm_consulta.posto_nome, 'nome')">
		</td>
	</tr>
