...
#	echo $sql . "<br>- ". pg_numrows ($res);
	...
	if (pg_numrows ($res) > 0) {
		...
		$codigo_post   = trim(pg_result($res,0,posto));

		$tipo_troca    = trim(pg_result($res,0,tipo_troca)); //HD 81632
...
if (strlen($os_reincidente) > 0) {
	...
	$serie_reincidente = trim(pg_result($res1,0,serie));
	...
	echo "</tr>";
	echo "<tr>";
	echo "<td class='titulo'>ORDEN DE SERVICIO CON NÚMERO DE SERIE: $serie_reincidente REINCIDENTE. ORDEN DE SERVICIO ANTERIOR: $sos</td>";
	echo "</tr>";
	echo "</table>";
	echo "<br>";
}
...
if($login_fabrica==19 OR $login_fabrica==20){//HD 81632
...
<TABLE width="700px" border="0" cellspacing="1" cellpadding="0" align='center' class='Tabela'>
	...
	<?php if ($login_fabrica == 20) { ?>
		<TR>
			<TD class="titulo" width='90' height='15'>CELULAR&nbsp;</TD>
			<TD class="conteudo" height='15' width='300'>&nbsp;<? echo $consumidor_celular ?></TD>
			<TD class="titulo" width='80'>EMAIL&nbsp;</TD>
			<TD class="conteudo" height='15'>&nbsp;<? echo $consumidor_email ?></TD>
		</TR>
	<?php } ?>
	<TR>
...
<?
	...
	if(pg_num_rows($res)==1){
		...
		<div>
			<div>
				<h2>Informaciones complementares 
				<div id="contentcenter" style="width: 650px;">
...
if (in_array($login_fabrica, array(20))) {
	...
	$arquivo = temNF($os, 'array', '', false, false, 0);
	if (count($arquivo)){
		echo "<br>";
		echo '<table class="Tabela" border="0" cellpadding="0" cellspacing="1" align="center" width="700px">';
		echo "<tbody><tr>";
		echo '<td class="inicio" style="text-align: center" colspan="4" height="15">Anexo(s)</td>';
		echo "</tr>";
		echo "<tr>";
		foreach ($arquivo["arquivos"] as $arq) {
			echo '<td class="conteudo" height="15" width="300" style="text-align: center">';
			echo '<a href="'.$arq.'" target="_blank">';
			if (strpos($arq, ".pdf") === false) {
				echo '<img src="'.$arq.'" title="Para ver a imagem completa, clique com o botão direito e selecione &quot;Abrir link em uma nova janela&quot; ou &quot;Mostrar Imagem&quot;" style="_display:block;_zoom:1;max-height:150px;max-width:150px;_height:150px;*height:150px;margin-top: 10px"></a>';
			}else{
				echo '<img src="../imagens/icone_PDF.png" title="Para ver a imagem completa, clique com o botão direito e selecione &quot;Abrir link em uma nova janela&quot; ou &quot;Mostrar Imagem&quot;" style="_display:block;_zoom:1;max-height:150px;max-width:150px;_height:150px;*height:150px;margin-top: 10px"></a>';
			}
			echo '</td>';
		}
		echo "</tr>";
	...
	//var_dump(temNF($os, 'array', '', false, false, 0));
	//echo '<td>' . temNF($os, 'linkEx', '', false, false, 0) .  '</td>';
}
?>
