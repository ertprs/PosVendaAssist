...
//Quando selecionando o 'Aviso Posto Unico' faz a validacão para saber se entrou com os dados do posto
	...
	if(strlen($codigo_posto) > 0) {
		...
		$res = pg_exec($con,$sql);
		if (pg_numrows ($res) <> 1) {
			$msg_erro = "Código del servicio ($codigo_posto) no encuentrado";
		}else{
		...
		if (strlen($arquivo["tmp_name"]) > 0 && $arquivo["tmp_name"] != "none"){
			...
			if (!preg_match("/\/(pdf|msword|pjpeg|jpeg|png|gif|bmp|vnd.ms-excel|richtext|plain|vnd.ms-powerpoint|zip|x-zip-compressed)$/", $arquivo["type"])){
				...
				if ($arquivo["size"] > $config["tamanho"]) 
					$msg_erro = "Archivo en tamaño muy grande! Debe tener el máximo 4MB. Envie otro archivo.";
			}
...
<tr>
	...
	<td class='table_line2'>
		<select class='frm' name='tipo'>
			...
			<option value='Com. Unico Posto'            <? if ($tipo == 'Com. Unico Posto')       echo " SELECTED ";?>>Com. Único Servicio</option>
			<option value='Boletim'                     <? if ($tipo == "Boletim")                echo " SELECTED ";?>>Boletín</option>
			...
			<option value='Descritivo técnico'          <? if ($tipo == "Descritivo técnico")     echo " SELECTED ";?>>Descriptivo tecnico</option>
			...
			<option value='Orientação de Serviço'       <? if ($tipo == "Orientação Serviço")     echo " SELECTED ";?>>Orientación de servicio</option>
			<option value='Lançamentos'                 <? if ($tipo == "Lançamentos")            echo " SELECTED ";?>>Lanzamientos</option>
			<option value='Procedimentos'               <? if ($tipo == "Procedimentos")          echo " SELECTED ";?>>Procedimientos</option>
			<option value='Promocao'                    <? if ($tipo == "Promocao")               echo " SELECTED ";?>>Promoción</option>
			<option value='Estrutura do Produto'        <? if ($tipo == "Estrutura do Produto")   echo " SELECTED ";?>>Estructura del producto</option>
			<option value='Capacitação Vídeo'           <? if ($tipo == "Capacitação Vídeo")      echo " SELECTED ";?>>Capacitación Video</option>
			<option value='Capacitação Manual'          <? if ($tipo == "Capacitação Manual")     echo " SELECTED ";?>>Capacitación Manual</option>
		</select> 
	</td>
	<td class='table_line2'><FONT COLOR="#6B6B6B">*<B>Com. Único Servicio:</B> utilizando esa opción, es necesario definir el servicio.</FONT></td>
</tr>
<tr>
...
<tr>
	...
	<td class="table_line2" colspan='2' nowrap>Cod.:<input class="frm" type="text" name="codigo_posto" size="10" value="<? echo $codigo_posto ?>" <? if ($login_fabrica == 5) { ?> onblur="fnc_pesquisa_posto2 (document.frm_comunicado.codigo_posto,document.frm_comunicado.posto_nome,'codigo')" <? } ?>>&nbsp;<img src='imagens/btn_buscar5.gif' border='0' align='absmiddle' style="cursor:pointer" onclick="javascript: fnc_pesquisa_posto2 (document.frm_comunicado.codigo_posto,document.frm_comunicado.posto_nome,'codigo')"></A>
		...
		Email confirmación lectura:&nbsp;<INPUT TYPE="text" size='40' value='<?echo $remetente_email?>' class='frm' NAME="remetente_email"></A></td>
</tr>
...
<tr>
	...
	<td class='table_line2'><select name='tipo_posto' class='frm' size='1'>
		...
		?></select> </td>
	<td class='table_line2'><FONT COLOR="#6B6B6B">* Solo los postos del tipo elejidos receberán el comunicado.</FONT></td>
</tr>
...
<tr>
	...
	<td class='table_line2'><FONT COLOR="#6B6B6B">*Apenas os postos do estado selecionado receberão o comunicado.</FONT></td>
</tr>
*/?>
...
<tr>
	...
	<td class='table_line2' colspan='2'>
		...
		if (is_object($s3)) {
			...
			echo "Click aquí para abrir el archivo";
			echo "</a>";
		} else {
	...
	if (strlen ($produto_referencia) > 0) {
		...
		if (pg_numrows ($res) > 0) {
			...
			for ($i = 0; $i < pg_numrows ($res); $i++){
				...
				echo "<td align='center'><a href='$PHP_SELF?comunicado=$comunicado'><img src='imagens_admin/btn_listar.gif' alt='Haga un click aquí para borrar.'></a>";
				echo "</tr>";
			}
			echo "</table>";
		}
...
<tr>
	<td style='color: B1B1B1; font-size: 10px; '>*Caso no seleccione el servicio, todos los servicios recibirán el comunicado.<br>
...
<tr>
	<td align='center'>
		<select class='frm' name='psq_tipo'>
			<option value=''></option>
			<option value='Boletim'                            <? if ($psq_tipo == "Boletim")                           echo "SELECTED ";?>>Boletín</option>
			<option value='Comunicado'                         <? if ($psq_tipo == "Comunicado")                        echo "SELECTED ";?>>Comunicado</option>
			<option value='Com. Unico Posto'                   <? if ($psq_tipo == "Com. Unico Posto")                  echo "SELECTED ";?>>Com. Único Servicio</option>
			...
			<option value='Descritivo técnico'                 <? if ($psq_tipo == "Descritivo técnico")                echo "SELECTED ";?>>Descriptivo técnico</option>
			<option value='Informativo tecnico'                <? if ($tipo == "Informativo tecnico")                   echo "SELECTED ";?>>Informativo tecnico</option> 
			<option value='Manual'                             <? if ($psq_tipo == "Manual")                            echo "SELECTED ";?>>Manual</option>
			<option value='Orientação de Serviço'              <? if ($psq_tipo == "Orientação de Serviço")             echo "SELECTED ";?>>Orientación de servicio</option>
			<option value='Lançamentos'                        <? if ($psq_tipo == "Lançamentos")                       echo "SELECTED ";?>>Lanzamientos</option>
...
<tr>
	...
	<td align='center' class='menu_top'>Producto - Descripción</td>
</tr>
<tr>
