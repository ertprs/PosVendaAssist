...
$title = "REPORTES DE FALLAS: PIECAS DE LOS ULTIMOS 12 MESES";
...
$sql = "SELECT tbl_peca.peca,tbl_peca.descricao,to_char(fcr.data_geracao,'YYYY-MM')As data_geracao, COUNT(*) AS qtde
		FROM tbl_os
		JOIN (
			...
			JOIN tbl_posto   ON tbl_os.posto = tbl_posto.posto AND tbl_extrato.posto = tbl_posto.posto
		...
		JOIN tbl_os_item      ON tbl_os_produto.os = tbl_os_item.os_produto
...
if (pg_numrows($res) > 0) {
	...
	for ($i=0; $i<pg_numrows($res); $i++){
		...
		if($peca_anterior<>$peca OR pg_numrows($res)==1){
