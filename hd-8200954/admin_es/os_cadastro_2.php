...
//Email da Samel para não validar o campo em 15/06/2007	
//Eu já havia solicitado anteriormente que para toda a América latina ( exceção Brasil)esse campo //deveria se aberto, pelo menos agora no início, porque cada país trabalha com qtd de caracteres //diferentes. Eu pensei que já estivesse ok! Você poderia alterar com urgência. 
// 
//Grata,
//Best regards/Saludos/Mit freundlichen Grüßen/Saudações
//
//Samel Silva 
//
//
//		if ($xcpf <> "null" and strlen($xcpf) <> 11 and strlen ($xcpf) <> 14) {
//			$msg_erro = 'Tamaño de la Identificación del consumidor inválido';
//		}
...
#	if ($login_fabrica == 1 AND strlen($consumidor_cpf) == 0) $msg_erro .= " Tamanho do CPF/CNPJ do cliente inv?lido.";
	...
	if ($fOsRevenda == 1){
		...
		if (strlen ($nota_fiscal) == 0){ 
			//$nota_fiscal = "null";
			$msg_erro = "Entre con el número de la Factura Comercial";
		}
		else
			$nota_fiscal = "'" . $nota_fiscal . "'" ;
	...
	if ($fOsRevenda == 0){
		...
		if (@pg_numrows ($res) == 0) {
			$msg_erro = "Producto $produto_referencia sin garantía";
		}
		...
		if ($login_fabrica <> 3 and $login_fabrica <> 11 AND $login_fabrica <> 24) {
			if ($data_final_garantia < $cdata_abertura) {
				$msg_erro = "[ $data_nf ] - [ $data_final_garantia ] = [ $cdata_abertura ] Producto $produto_referencia fuera de la garantía, vencida en ". substr($data_final_garantia,8,2) ."/". substr($data_final_garantia,5,2) ."/". substr($data_final_garantia,0,4);
			}
		}
	}
...
//echo $sql;
	...
	if (strlen ($msg_erro) > 0) {
		...
		$msg_erro = " Fecha de abertura debe ser inferior o igual a fecha de digitación de la OS en el sistema (fecha de hoy).";
...
$title = "Catastro de órdene de servicio  - ADMIN"; 
...
*/
			...
			if($sistema_lingua) echo "En caso de garantía de piezas o accesorios no es necesario insertar el producto en la OS";else echo "Nos casos de Garantia de Peças ou  Acessórios não é necessário lançar o Produto na OS.";
			echo "</FONT></b><br>";	
		}
		echo "</div>";
		?>
		...
		<tr>
			...
			<td nowrap>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">Código</font>
				<br>
		...
		<tr valign="top">
			...
			<td nowrap>
				...
				if ($login_fabrica == 3) {
					...
					echo "<font size='1' face='Geneva, Arial, Helvetica, san-serif'>Descripción del producto</font>";
				}
				?>
				<br>
				<?	if (strlen($pedido) > 0) { ?>
...
?>
		...
		<tr>
			...
			<td> 
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">Identificación del consumidor</font>
				<br>
			...
			<td>
				<font size="1" face="Geneva, Arial, Helvetica, san-serif">Teléfono</font>
				<br>
...
<tr class="top">
	<td><font size="1" face="Geneva, Arial, Helvetica, san-serif">Dirección</font></td>
...
if (strlen ($os) > 0) {
?>
		<img src='imagens_admin/btn_alterar_azul_es.gif' style="cursor:pointer" onclick="javascript: if (document.frm_os.btn_acao.value == '' ) { document.frm_os.btn_acao.value='continuar' ;  document.frm_os.submit() } else { alert ('Espere la submisión') }" ALT="Cambiar los itens de la orden de servicio" border='0'>
...
?>
		<img src='imagens/btn_continuar.gif' style="cursor:pointer" onclick="javascript: if (document.frm_os.btn_acao.value == '' ) { document.frm_os.btn_acao.value='continuar' ; document.frm_os.submit() } else { alert ('Espere la submisión') }" ALT="Continuar com Ordem de Serviço" border='0'>
<?
}
?>
	</td>
