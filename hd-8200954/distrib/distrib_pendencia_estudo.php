...
AND strlen($data_corte_faturada_distrib) > 0 ){
	...
	if(!in_array($fabrica, array(51,81))) {
		$sql .= " AND tbl_tipo_pedido.pedido_em_garantia ";
	}
	$sql .= "
	...
	if($script == 'gerar'){
		...
		if(!in_array($fabrica, array(51,81))) {
			$sql_gera_script .= " AND tbl_tipo_pedido.pedido_em_garantia ";
		}
	...
	if(!in_array($fabrica, array(51,81))) {
		$sql .= " AND tbl_tipo_pedido.pedido_faturado ";
	}
	...
	if($script == 'gerar'){
		$sql_gera_script = "SELECT x.distribuidor, x.fabrica, x.posto, x.pedido, x.finalizado, x.peca, x.qtde_cancelar,
									case when x.peca_critica = 't' or x.troca_obrigatoria = 't' then
							...
							SUM (tbl_pedido_item.qtde - tbl_pedido_item.qtde_cancelada - tbl_pedido_item.qtde_faturada_distribuidor) AS qtde_cancelar, tbl_peca.peca_critica, tbl_peca.troca_obrigatoria
							FROM tbl_pedido
							JOIN tbl_tipo_pedido USING(tipo_pedido)
							JOIN tbl_pedido_item USING (pedido)
							JOIN tbl_peca USING (peca)
							WHERE tbl_pedido.distribuidor = 4311
							and   (tbl_pedido.data <= '$aux_data_corte_faturada_posto 23:59:59' or tbl_peca.peca_critica = 't' or tbl_peca.troca_obrigatoria = 't')
							and   tbl_pedido_item.qtde > ( tbl_pedido_item.qtde_cancelada + tbl_pedido_item.qtde_faturada_distribuidor )
							AND   tbl_pedido.fabrica = $fabrica ";
		if($fabrica==51){
		...
		if(!in_array($fabrica, array(51,81))) {
			$sql_gera_script .= " AND tbl_tipo_pedido.pedido_faturado ";
		}

		$sql_gera_script .= "
							GROUP BY tbl_pedido.distribuidor, tbl_pedido.fabrica, tbl_pedido.posto, tbl_pedido.pedido, finalizado, tbl_pedido_item.peca,  tbl_peca.peca_critica, tbl_peca.troca_obrigatoria
							) x  ";
		$res_gera_script = pg_exec($con,$sql_gera_script);
		...
		for ($i=0;$i<pg_numrows($res_gera_script);$i++){
			...
			//fputs ($file_script,"INSERT INTO tbl_comunicado (mensagem,tipo,fabrica,posto,obrigatorio_site,ativo)	 VALUES ('$mensagem','Comunicado',$fabrica,$s_posto,'t','t'); \n");
			$sql_can = "INSERT INTO tbl_comunicado (mensagem,tipo,fabrica,posto,obrigatorio_site,ativo)	 VALUES ('$mensagem','Comunicado',$fabrica,$s_posto,'t','t');";
			$res_can = pg_exec($con,$sql_can);
		}
	}
	...
	$sql .= " UPDATE tmp_distrib_pendencia_estudo set qtde_pendente_garantia_distrib = x.qtde_pendente
			FROM (
				...
				JOIN tbl_pedido_item USING (pedido)
				JOIN tbl_tipo_pedido USING(tipo_pedido)
				WHERE tbl_pedido.posto in (4311,20682)
	...
	if($script == 'gerar'){
		$sql_gera_script = "SELECT x.distribuidor, x.fabrica, x.pedido, x.peca, x.qtde_cancelar,
							...
							FROM (
							SELECT tbl_pedido.distribuidor, tbl_pedido.fabrica, tbl_pedido.pedido, tbl_pedido_item.peca,
							SUM (tbl_pedido_item.qtde - tbl_pedido_item.qtde_cancelada - tbl_pedido_item.qtde_faturada) AS qtde_cancelar,  tbl_peca.peca_critica, tbl_peca.troca_obrigatoria
							FROM tbl_pedido
							JOIN tbl_pedido_item USING (pedido)
							JOIN tbl_tipo_pedido USING(tipo_pedido)
							JOIN tbl_peca USING (peca)
							WHERE tbl_pedido.posto in (4311,20682)
							and   ( tbl_pedido.data <= '$aux_data_corte_garantia_distrib 23:59:59' or tbl_peca.peca_critica = 't' or tbl_peca.troca_obrigatoria = 't')
							and   tbl_pedido_item.qtde > ( tbl_pedido_item.qtde_cancelada + tbl_pedido_item.qtde_faturada )
							AND   tbl_pedido.fabrica = $fabrica ";
		if($fabrica==51){
			$sql_gera_script .= " AND tbl_pedido.tipo_pedido = 132 ";
		...
		$sql_gera_script .= "
							GROUP BY tbl_pedido.distribuidor, tbl_pedido.fabrica, tbl_pedido.pedido, tbl_pedido_item.peca,  tbl_peca.peca_critica, tbl_peca.troca_obrigatoria
							) x  ";
		$res_gera_script = pg_exec($con,$sql_gera_script);
	...
	if($script == 'gerar'){
		$sql_gera_script = "SELECT x.distribuidor, x.fabrica, x.pedido, x.peca, x.qtde_cancelar,
							...
							JOIN tbl_peca USING (peca)
							WHERE tbl_pedido.posto in (4311,20682)
							and   ( tbl_pedido.data <= '$aux_data_corte_faturada_distrib 23:59:59' or tbl_peca.peca_critica = 't' or tbl_peca.troca_obrigatoria = 't')
							and   tbl_pedido_item.qtde > ( tbl_pedido_item.qtde_cancelada + tbl_pedido_item.qtde_faturada )
							AND   tbl_pedido.fabrica = $fabrica ";
		if($fabrica==51){
			$sql_gera_script .= " AND tbl_pedido.tipo_pedido = 131 ";
	...
	$sql .= " UPDATE tmp_distrib_pendencia_estudo set qtde_atendido_gar_nf_posto = x.qtde_atendido_gar_nf_posto
			FROM (
				...
				JOIN tbl_tipo_pedido USING(tipo_pedido)
				WHERE (tbl_faturamento.fabrica = $fabrica or tbl_faturamento.fabrica = 10)
	...
	$sql .= " GROUP by tbl_faturamento_item.peca
			...
			FROM (
				...
				AND (
					...
					OR
					tbl_faturamento.fabrica in ($fabrica,10)
	...
	$sql .= " GROUP by tbl_faturamento_item.peca
			...
			where  tbl_posto_estoque.posto in (4311,20682)
...
?>
	...
	logo, alguns casos teremos mais faturamento para posto do que recebemos. <br>
				...
				<table>
					...
					<tr>
						...
						<td align="left" nowrap width='230px' title='Desconsiderando todos os pedidos até esta data!'>
							...
							<input type="text" name="data_corte_garantia_posto" id="data_corte_garantia_posto" size="12" maxlength="10" class='frm' value="<?  echo $data_corte_garantia_posto;  ?>" >
						</td>
						<td nowrap align="left" width='230px' title='Desconsiderando todos os pedidos até esta data!'>
							Data Corte Pedido faturado (?)<br />
							<input type="text" name="data_corte_faturada_posto" id="data_corte_faturada_posto" size="12" maxlength="10" class='frm' value="<?  echo $data_corte_faturada_posto; ; ?>" >
						</td>
					</tr>
					<tr>
						<td>&nbsp;&nbsp;</td>
					...
					<tr>
						...
						<td align="left" nowrap width='230px' title='Desconsiderando todos os pedidos até esta data!'>
							Data Corte Pedido em Garantia (?)<br />
							<input type="text" name="data_corte_garantia_distrib" id="data_corte_garantia_distrib" size="12" maxlength="10" class='frm' value="<?  echo $data_corte_garantia_distrib; ?>">
						</td>
						<td align="left" nowrap width='230px' title='Desconsiderando todos os pedidos até esta data!'>
							Data Corte Pedido faturado (?)<br />
							<input type="text" name="data_corte_faturada_distrib" id="data_corte_faturada_distrib" size="12" maxlength="10" class='frm' value="<?  echo $data_corte_faturada_distrib; ?>">
						</td>
					</tr>
					<tr>
