...
define('re_match_YMD', '/(\d{4})\W?(\d{2})\W?(\d{2})/');
define('re_match_DMY', '/(\d{2})\W?(\d{2})\W?(\d{4})/');
define('re_format_YMD', '$3-$2-$1');
define('re_format_DMY', '$3/$2/$1');
...
echo "Inicio em ".date('H:m:i').'<br>';
	if(strlen($erro_msg) == 0){
		...
		if(strlen($erro_msg) > 0){
			...
			print $erro_msg="<br>Erro ao inserir a NF: $nota_fiscal<br>$erro_msg";
		}else{
...
echo date('H:m:i')."<p>Procura a peça $referencia</p>";flush();
...
echo "<p>Não há peça, procurando produto</p>";flush();
						$sql = "SELECT  produto   ,
								referencia,
								descricao ,
								ipi       ,
...
echo "<p>Inserindo peça.</p><pre>$sql</pre>";flush();
							$res = @pg_query($con,$sql);
...
echo date('H:m:i')."<p>Inserindo linha $i da fra.</p>";flush();
					if(strlen($erro_item)==0){
...
echo '<br>'.date('H:m:i')."<p>Fim processo linha $i</p>";
			}
				$res = pg_query ($con,"ROLLBACK TRANSACTION");
exit;
...
echo '<br>'.date('H:m:i')."<p>Fim processo NF!</p>";
...
if ($btn_acao == "Alterar") {
	if(strlen($faturamento)==0){
		header ("Location: nf_cadastro.php?faturamento=$faturamento&erro_msg=FATURAMENTO VAZIO!");
		exit;
	}
	...
	if(pg_num_rows($res)==0){
		header ("Location: nf_cadastro.php?faturamento=$faturamento&erro_msg=NÃO FOI ENCONTRADO O FATURAMENTO: $faturamento");
		exit;
	}
		...
		<tr>
		    ...
		    <td width='300'><br><br>
		            ...
		            <button type="submit"	id='btn_acao'	name='btn_acao' value="NFe"
						   title='Processar NF-e' class='frm' style='cursor:pointer'>NFe</button>
			</td>
			<td>&nbsp;</td>
		</tr>
	</table>
