...
JOIN tbl_compra_manual ON tbl_pedido_item.peca = tbl_compra_manual.peca AND tbl_compra_manual.posto = 4311
...
SELECT tbl_pedido_item.peca, SUM (qtde - qtde_cancelada - qtde_faturada) AS qtde_fabrica
INTO TEMP TABLE x_pedido_fabrica
FROM tbl_pedido 
JOIN tbl_pedido_item USING (pedido)
WHERE tbl_pedido.fabrica = 3 
AND  (tbl_pedido.status_pedido IN (SELECT status_pedido FROM tbl_status_pedido WHERE status_pedido NOT IN (3,4,6,13)) OR tbl_pedido.status_pedido IS NULL)
...
DELETE FROM x_pedido WHERE peca IN (SELECT peca FROM tbl_peca WHERE fabrica = 3 AND produto_acabado) ;
DELETE FROM x_pedido WHERE peca IN (SELECT peca FROM tbl_peca WHERE fabrica = 3 AND referencia LIKE '7%') ;


/*       30 dias           */
UPDATE x_pedido SET dias30 = (
SELECT SUM (qtde) 
FROM tbl_pedido 
JOIN tbl_pedido_item USING (pedido)
WHERE tbl_pedido.fabrica = 3
