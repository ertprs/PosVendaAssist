    ...
    function __construct() {
        ...
        if ( is_file( $P_ROOT . "config/config.php") ){
            ...
            if ( !is_dir($this->arqDir . "_$sAmb") ){
                mkdir($this->arqDir . "_$sAmb", 0777, true);
            }
            if ( !is_dir($this->entDir) ){
                mkdir($this->entDir, 0777, true);
            }
            if ( !is_dir($this->valDir) ){
                mkdir($this->valDir, 0777, true);
            }
            if ( !is_dir($this->repDir) ){
                mkdir($this->repDir, 0777, true);
            }
            if ( !is_dir($this->assDir) ){
                mkdir($this->assDir, 0777,true);
            }
            if ( !is_dir($this->envDir) ){
                mkdir($this->envDir, 0777,true);
            }
            if ( !is_dir($this->aprDir) ){
                mkdir($this->aprDir, 0777,true);
            }
            if ( !is_dir($this->denDir) ){
                mkdir($this->denDir, 0777, true);
            }
            if ( !is_dir($this->rejDir) ){
                mkdir($this->rejDir, 0777, true);
            }
            if ( !is_dir($this->canDir) ){
                mkdir($this->canDir, 0777, true);
            }
            if ( !is_dir($this->inuDir) ){
                mkdir($this->inuDir, 0777, true);
            }
            if ( !is_dir($this->temDir) ){
                mkdir($this->temDir, 0777, true);
            }
            if ( !is_dir($this->recDir) ){
                mkdir($this->recDir, 0777, true);
            }
            if ( !is_dir($this->recDir) ){
                mkdir($this->conDir, 0777, true);
            }
