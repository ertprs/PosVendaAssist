...
if ($Faturar=='1' AND strlen($embarque)>0) {
	...
	if (pg_numrows ($res) == 1) {
		...
		$fabrica_pedido = pg_result ($res,0,fabrica);

		$tipos_pedidos[0] = "76, 77, 2, 116, 131, 153,236,234,246,261";   // tipo_pedido de venda
		$tipos_pedidos[1] = "3, 115, 132, 154,237,235,247,260";           // tipo_pedido garantia

		if ($fabrica_pedido == 168) {

			//Código que verifica o campo parametros_adicionais na tbl_fábrica e que monta as variáveis de verificação
			$sql = "SELECT parametros_adicionais FROM tbl_fabrica WHERE fabrica = $fabrica_pedido ;";
			$res = pg_query($con,$sql);
			if (pg_num_rows($res) > 0) {
			    $param_adc = json_decode(pg_fetch_result($res, 0, 'parametros_adicionais'), true); // Array			    
			...
			$sql = "SELECT 	tbl_embarque_item.qtde,
							...
							tbl_pedido.pedido
			-- tbl_embarque_item.qtde , tbl_tabela_item.preco, tbl_peca.ipi, tbl_condicao.acrescimo_financeiro, tbl_pedido.pedido
						...
						WHERE tbl_embarque_item.embarque = $embarque
							AND   (tbl_pedido.tipo_pedido IN (" . $tipos_pedidos[0] . ") OR tbl_tipo_pedido.pedido_faturado)  ";
			$res = pg_query($con,$sql);
			...
			if (pg_num_rows($res) > 0 ) {				
				for ($i=0; $i < pg_num_rows($res) ; $i++) { 
					$qtde_item_cond       = pg_fetch_result($res, $i, qtde);
					$preco_item_cond      = pg_fetch_result($res, $i, preco);
					$ipi_item_cond        = pg_fetch_result($res, $i, ipi);
					$parcelas_item_cond   = pg_fetch_result($res, $i, parcelas);
					$acrescimo_financeiro = pg_fetch_result($res, $i, acrescimo_financeiro);
					$desconto_financeiro  = pg_fetch_result($res, $i, desconto_financeiro);
					$pedido_item_cond     = pg_fetch_result($res, $i, pedido);

					$total_faturado = $qtde_item_cond * $preco_item_cond;
					...
					if ($acrescimo_financeiro > 0.00) {
						...
						$total_faturado = $total_faturado + ( $preco_total_item_cond * (pow((1 + $taxa), $parcelas_item_cond)) );
						
					} elseif ($desconto_financeiro > 0.00) {						
						$total_faturado = $total_faturado + $preco_total_item_cond;
						$porcento = ($desconto_financeiro / 100) * $preco_total_item_cond;
      					$total_faturado = $total_faturado - $porcento;
					} else {
						$total_faturado = $total_faturado + $preco_total_item_cond;
					}
				}
			...
			-- tbl_embarque_item.qtde , tbl_tabela_item.preco, tbl_peca.ipi, tbl_condicao.acrescimo_financeiro, tbl_pedido.pedido
						FROM tbl_embarque_item
							...
							LEFT JOIN tbl_tabela_item ON tbl_tabela_item.tabela = tbl_pedido.tabela AND tbl_tabela_item.peca = tbl_embarque_item.peca
						WHERE tbl_embarque_item.embarque = $embarque
							AND   (tbl_pedido.tipo_pedido IN (" . $tipos_pedidos[1] . ") OR tbl_tipo_pedido.pedido_em_garantia)  ";
			$res = pg_query($con,$sql);
			$msg_erro .= pg_errormessage($con);
			...
			if (pg_num_rows($res) > 0 ) {
				for ($i=0; $i < pg_num_rows($res) ; $i++) { 
					...
					$total_garantia = $qtde_item_cond * $preco_item_cond;
					...
					if ($acrescimo_financeiro > 0.00) {
						...
						$taxa = $acrescimo_financeiro / 100;
						$total_garantia = $total_garantia + ( $preco_total_item_cond * (pow((1 + $taxa), $parcelas_item_cond)) );
						
					} elseif ($desconto_financeiro > 0.00) {						
						$total_garantia = $total_garantia + $preco_total_item_cond;
						$porcento = ($desconto_financeiro / 100) * $preco_total_item_cond;
      					$total_garantia = $total_garantia - $porcento;
					} else {
						$total_garantia = $total_garantia + $preco_total_item_cond;
					}
				}
			...
			#------------ Totaliza Embarque ----------------- #
			$sql = "SELECT SUM (tbl_embarque_item.qtde * tbl_pedido_item.preco * (1 + (tbl_peca.ipi / 100)))
					FROM tbl_embarque_item
					JOIN tbl_peca ON tbl_embarque_item.peca = tbl_peca.peca
					JOIN tbl_pedido_item ON tbl_embarque_item.pedido_item = tbl_pedido_item.pedido_item
					JOIN tbl_pedido ON tbl_pedido_item.pedido = tbl_pedido.pedido
		...
		*/
		...
		for ($t = 0 ; $t < 2 ; $t++) {
			...
			if ($t == 0) {
				...
				$tipo_pedido_nf = "76";
				$cond_tipo_pedido = " or tbl_tipo_pedido.pedido_faturado "; 
			}else{
				$tipo_pedido_descricao = "GARANTIA";
				$tipo_pedido_nf = "158";
				$cond_tipo_pedido = " or tbl_tipo_pedido.pedido_em_garantia "; 
			}
			...
			/**
			 * XXX - Se um dia a aliquota mudar lembrar se a regra irá se aplicar aos produtos de origem IMP
			 */
			...
			if ($tipo_pedido_descricao == 'GARANTIA') {
				...
				/* CFOP e ICMS PADRAO para TODOS os estados MENOS SP,MG,PR,RJ,RS,SC */
				$cfop = "6949";
				$natureza = "OUTRAS SAÍDAS - REMESSA EM GARANTIA";
				$aliq_icms = 7;
				$aliq_ipi  = 0;
				$aliq_reducao = 0;
			...
			if ($fabrica_pedido == 168) {
				if ($tipo_pedido_descricao == "VENDA") {
					...
					$total_nota = $total_faturado;
				} elseif ($tipo_pedido_descricao == 'GARANTIA') {
					if ($total_garantia == 0){
						continue;
					}
					$total_nota = $total_garantia;
				}
			} else {

				if ($fabrica_pedido == 147) {
					$calculo_desconto = " * (1 - (case when tbl_pedido.desconto notnull then tbl_pedido.desconto else 0 end / 100))";
				}
				$campo_ipi = "tbl_peca.ipi" ; 
				if ($fabrica_pedido == 160 AND $tipo_pedido_descricao != 'GARANTIA') {
					$calculo_desconto = " * (1 - (case when tbl_pedido.desconto notnull then tbl_pedido.desconto else 0 end / 100))";
					$campo_ipi = "0"; 
				}
				...
				#------------ Totaliza Nota ----------------- #
				$sql = "SELECT 
				SUM ((tbl_embarque_item.qtde * case when tbl_tabela_item.preco notnull then tbl_tabela_item.preco else tbl_pedido_item.preco end) * (1 + ($campo_ipi / 100))$calculo_desconto)
						FROM tbl_embarque_item
						JOIN tbl_peca ON tbl_embarque_item.peca = tbl_peca.peca
						JOIN tbl_pedido_item ON tbl_embarque_item.pedido_item = tbl_pedido_item.pedido_item
						JOIN tbl_pedido ON tbl_pedido_item.pedido = tbl_pedido.pedido
						...
						LEFT JOIN tbl_tabela_item ON tbl_tabela_item.tabela = tbl_pedido.tabela AND tbl_tabela_item.peca = tbl_embarque_item.peca
						WHERE tbl_embarque_item.embarque = $embarque
						AND   (tbl_pedido.tipo_pedido IN (" . $tipo_pedido . ") $cond_tipo_pedido )";
				$res = pg_exec ($con,$sql);
				$msg_erro .= pg_errormessage($con);
			...
			/**
			 * Acácia é do regime do Simples Nacional, portanto não destaca ICMS
			 */
			$base_icms  = 0;
			$valor_icms = 0;
			...
			*/
			...
			$sql = "SELECT MAX (nota_fiscal) AS nota_fiscal FROM tbl_faturamento WHERE distribuidor = $login_posto AND empresa = 'ACAC' and serie = '1'";
			$res = pg_exec ($con,$sql);
			$msg_erro .= pg_errormessage($con);
		    ...
		    $nota_fiscal = "000000";//SIGE que vai fornecer numero da nota
			...
			if (strlen (pg_errormessage ($con)) > 0) {
				...
				$email = "francisco.ambrozio@telecontrol.com.br";

				$assunto = "ERRO NO FATURAMENTO DO EMBARQUE";

		        $mensagem = "ERRO NO FATURAMENTO DO EMBARQUE: {$embarque} ---- {$msg_erro} ---- {$sql}";

		        $headers = 'From: Telecontrol <noreply@telecontrol.com.br>';
		        if (mail($email, utf8_encode($assunto), utf8_encode($mensagem), $headers)) {
		            $enviou_email = 'e-mail ';
		            $enviou = true;
		        }
			...
			$sql = "SELECT tbl_peca.peca,
							...
							tbl_peca.ipi,
							tbl_peca.ncm,
							( CASE WHEN tbl_pedido_item.preco IS NOT NULL THEN tbl_pedido_item.preco ELSE tbl_tabela_item.preco END ) AS preco,
							tbl_embarque_item.qtde,
							tbl_pedido.pedido,
							tbl_embarque_item.pedido_item,
							tbl_embarque_item.os_item,
							tbl_os.os,
							tbl_pedido_item.preco AS preco_pedido
					FROM   tbl_peca
					...
					WHERE ( tbl_pedido.tipo_pedido    IN ($tipo_pedido) $cond_tipo_pedido )
			...
			for ($p = 0 ; $p < pg_numrows ($resPeca) ; $p++) {
				...
				$origem      = trim(pg_fetch_result($resPeca, $p, 'origem'));
				$ncm = trim(pg_fetch_result($resPeca, $p, 'ncm'));

				/**
				 * @since HD 1051991 - alteração da alíquota de ICMS para produtos importados
				 *    conforme regulamentação da Resolução do Senado 13/2012
				 */
				if ($origem == "IMP" and $aliq_icms <> 18) {
					$aliq_icms = $aliq_icms_imp;
				} else {
					$aliq_icms = $aliq_icms_nac;
				}
				...
				if (($fabrica_pedido == 168 and $calcula_ipi == true) || !in_array($fabrica_pedido,[160,168])) {
			...
			/** Comentado pois estava zerando estes valores quando era garantia
			 * Conversei com a Roseli do Delta Contabilidade e ela informou que
			 * Isto está incorreto - Andreus Timm
			if ($tipo_pedido_descricao == 'GARANTIA') {
				$base_icms   = 0;
				$valor_icms  = 0;
				$base_ipi    = 0;
		...
		if (strlen ($msg_erro) == 0) {
            $sqlFabrica = "SELECT DISTINCT tbl_os.os
                           ...
                           WHERE tbl_faturamento.embarque = {$embarque}
                           ";
            $resFabrica = pg_query($con, $sqlFabrica);
            if (pg_num_rows($resFabrica) > 0) {
                while ($dados = pg_fetch_object($resFabrica)) {
                    atualiza_status_checkpoint($dados->os, 'Aguardando Peças');
                    
                }
            }

            $sqlEmbarque = "SELECT DISTINCT tbl_pedido_item.pedido 
            			FROM Tbl_embarque_item 
            			join tbl_pedido_item on tbl_pedido_item.pedido_item = tbl_embarque_item.pedido_item 
            			where tbl_embarque_item.embarque = $embarque";
            $resEmbarque = pg_query($con, $sqlEmbarque);

            for($k = 0; $k < pg_num_rows($resEmbarque); $k++){ 
            	$pedidoEmbaque = pg_fetch_result($resEmbarque, $k, 'pedido');
            	$sql = "SELECT pedido_item FROM tbl_pedido_item WHERE pedido = $pedidoEmbaque AND qtde - qtde_faturada_distribuidor - qtde_cancelada > 0";
        		$resPedido = pg_query($con, $sql);
        		if(pg_num_rows($resPedido)==0){
        			//total
        			$sql_upd = "UPDATE tbl_pedido set status_pedido = 4 where pedido = $pedidoEmbaque";
        			$res_upd = pg_query($con, $sql_upd);
        			$msg_erro .= pg_last_error($con);

        			$aux_sql = " INSERT INTO tbl_pedido_status(pedido, data, status,  observacao) VALUES ($pedidoEmbaque, current_timestamp, 4, 'Total')";
                	$aux_res = pg_query($con, $aux_sql);
                	$msg_erro .= pg_last_error($con);
        		}else{
        			//parcial
        			$sql_upd = "UPDATE tbl_pedido set status_pedido = 5 where pedido = $pedidoEmbaque";
        			$res_upd = pg_query($con, $sql_upd);
        			$msg_erro .= pg_last_error($con);

        			$aux_sql = " INSERT INTO tbl_pedido_status(pedido, data, status,  observacao) VALUES ($pedidoEmbaque, current_timestamp, 5, 'Parcial')";
                	$aux_res = pg_query($con, $aux_sql);
                	$msg_erro .= pg_last_error($con);
        		}        		
            }

            $sql_sms = "SELECT  tbl_os.os,
						        tbl_os.fabrica,
						...
						JOIN tbl_produto ON tbl_os.produto = tbl_produto.produto
						WHERE tbl_embarque_item.embarque = $embarque
						AND tbl_os.status_checkpoint = 3";
			$res_sms = pg_query($con, $sql_sms);
			if (pg_num_rows($res_sms) > 0) {
				$res_value = pg_fetch_all($res_sms);
				foreach ($res_value as $key => $value) {
					if ($value['fabrica'] != 123) {
						continue;
					}

					if (!empty($value['consumidor_celular'])) {
						$xos = $value['os'];
						$ref = $value['referencia'];
						$fab = $value['fabrica'];
						$cel = trim($value['consumidor_celular']);

			            $xsql_sms = "SELECT campos_adicionais FROM tbl_os_campo_extra WHERE os = $xos";
			            $qry_sms = pg_query($con, $xsql_sms);
			            ...
			            if (!array_key_exists("enviou_msg_consertado", $campos_adicionais) or $campos_adicionais["enviou_msg_consertado"] <> "t") {
		                    ...
		                    $msg_conserto_os = "Informamos que as peças para reparo de seu equipamento $ref já chegaram na Assistência. Favor aguardar o reparo e contato da Assistência Técnica";

                			$sms = new SMS($fab);
                			$enviar_sms = $sms->enviarMensagem($cel, $xos, '', $msg_conserto_os);
                
			                if (!$enviar_sms) {
			                    $erro_sms = "Erro ao enviar SMS ao consumidor";
			                }
				                ...
				                if (true === $insert_campo_extra) {
				                    $sql_msg_consertado = "
				                        INSERT INTO tbl_os_campo_extra (os, fabrica, campos_adicionais)
				                            VALUES ({$xos}, {$fab}, '{$json_campos_adicionais}')";
				                } else {
				                    $sql_msg_consertado = "
	...
	if (strlen ($msg_erro) == 0) {
        ...
        $mensagem = "ERRO NO FATURAMENTO DO EMBARQUE: {$embarque} ---- {$msg_erro}";
...
if ($Importar=='1' AND (strlen($embarques_importar)>0 OR strlen ($copia_nota_fiscal) > 0)){
	...
	if (strlen ($copia_nota_fiscal) > 0) {
		$sql = "SELECT * FROM tbl_faturamento WHERE nota_fiscal  = '$copia_nota_fiscal' AND distribuidor = $login_posto AND fabrica IN ($telecontrol_distrib)";
