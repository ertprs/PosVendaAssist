...
$faturamento_fabrica = $_POST['faturamento_fabrica'];
if (strlen($faturamento_fabrica)==0){
	$faturamento_fabrica = $_GET['faturamento_fabrica'];
}

if (strlen($faturamento_fabrica)==0){
	echo "Selecione a fábrica! ";
}
...
if ($Importar=='1' AND strlen ($copia_nota_fiscal) > 0 AND strlen($faturamento_fabrica)>0) {
	...
	$sql = "SELECT * ,
			...
			WHERE nota_fiscal  = '$copia_nota_fiscal' 
				...
				AND fabrica = $faturamento_fabrica";
	...
	for ($nf = 0 ; $nf < pg_numrows ($resNF) ; $nf++) {
		...
		$razao    = trim (pg_result ($resPosto,0,nome));
		$endereco = trim (pg_result ($resPosto,0,endereco)). ", " .trim (pg_result ($resPosto,0,numero))." ".trim (pg_result ($resPosto,0,complemento));
		$bairro   = trim (pg_result ($resPosto,0,bairro));
		$cidade   = trim (pg_result ($resPosto,0,cidade));
		$estado   = trim (pg_result ($resPosto,0,estado));
		$cep      = trim (pg_result ($resPosto,0,cep));
		$fone     = trim (pg_result ($resPosto,0,fone));
		$cnpj     = trim (pg_result ($resPosto,0,cnpj));
		$ie       = trim (pg_result ($resPosto,0,ie));

		

		#------------------ Gera arquivo texto -----------------#

		$tipo_pediido = pg_result ($resNF,$nf,tipo_pedido);
		...
		for ($p = 0 ; $p < pg_numrows ($resPeca) ; $p++) {
			$arquivo_nf .= substr (sprintf ("%-10s" ,pg_result ($resPeca,$p,referencia)),0,10);
	...
	#	echo "SP";                             #Estado
		...
		if(strlen($transportadora )>0){
			...
			$arquivo_nf .= substr (sprintf ("%-30s",trim (" ")),0,30);
			$arquivo_nf .= substr (sprintf ("%-15s",trim (" ")),0,15);
			$arquivo_nf .= "                    ";           #I.E.
			$arquivo_nf .= substr (sprintf ("%-30s",trim (" ")),0,30);
			$arquivo_nf .= substr (sprintf ("%-30s",trim (" ")),0,30);
			$arquivo_nf .= substr (sprintf ("%-2s", trim (" ")),0,2);
	...
	#	if ($tipo_pedido == "3") echo "CIF";
		...
		$sql = "SELECT rpad(nome, 20,' ' )
				FROM tbl_fabrica
				WHERE fabrica = $faturamento_fabrica";

		$resFab = pg_exec ($con,$sql);

		$nome_fabrica = pg_result ($resFab,0,0);

		$arquivo_nf .= strtoupper($nome_fabrica);
		$arquivo_nf .= "S";
...
/*		header('Pragma: public');
		...
		echo "<a href='nf_saida_download.php?arquivo=nota_fiscal.telecontrol' style='font-size:16px'>Clique aqui para importar</a>";
	}else{
