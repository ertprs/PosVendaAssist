...
if (isset($_GET["q"])){
	if($tipo == 'fornecedor') {
		$sql = "SELECT posto,nome, tbl_posto_extra.fornecedor_distrib_fabrica 
						FROM tbl_posto
		...
		if(pg_num_rows($res)>0) {
			for ( $i = 0 ; $i < @pg_num_rows ($res) ; $i++ ) {
				...
				$fornecedor_distrib_fabrica = trim(pg_fetch_result($res,$i,fornecedor_distrib_fabrica));	
				echo "$nome|$posto|$fornecedor_distrib_fabrica";
				echo "\n";
			}
		}else{
