...
<script>
            ...
            success: function(data){
                ...
                if (data.erro_correios != undefined || data.erro_offline != undefined) {
                    ...
                    if (data.erro_correios != "") {
                        msg_webservice_correios += "Atenção: Realizaremos o cálculo de frete offline, pois não foi possível realizar o cálculo pelo webservice dos correios. Os erros retornados foram: <br /><br />"+data.erro_correios;
                    }
                ...
                success: function(data){
                    ...
                    $.each(data,function(key, value){
                        if(value.resultado == "true"){
                            ...
                            etiquetas += "<td colspan='4'><b>Serviço selecionado:</b>Transp. Rodoviário</td><td colspan='2'><b>Etiqueta: </b><input type='text' readOnly name='estiqueta"+button+"' value='"+value.etiqueta+"'></td>";
    ...
    function excluirItem(url,multiplos = 0){
        if (multiplos == 0) {
            ...
            if(confirm('Ao remover todas as peças do embarque, o mesmo será excluído. Deseja continuar?')){
                window.location=url;
            }
        }
    }
...
<div style="padding-left: 35% !important;">
    <table>
        <tr height="18">
            ...
            <td align="left">
                <font size="1"><b>Ordem de Serviço Reincidente</b></font>
            </td>
        </tr>
    </table>
</div>
