...
if(strlen($nf)>0 and strlen($faturamento_fabrica)>0 AND strlen($posto_codigo)>0) {
	...
	if(pg_numrows($res)>0){
		...
		if(pg_numrows($res)>0){
			...
			header ("Location: nf_saida.php?faturamento_fabrica=$faturamento_fabrica&posto_codigo=$posto_codigo&erro_msg=Não encontrou Faturamento para esse cliente.");

		}
	}else{
		echo "Cliente/Posto não encontrado!";
	}
}elseif(strlen($nf)>0 and strlen($faturamento_fabrica)>0){ //HD-2416428
	  $sql = "SELECT
            faturamento
        FROM  tbl_faturamento
        WHERE fabrica = 10
    ...
    if(pg_numrows($res)>0){
      ...
      header ("Location: nf_saida.php?faturamento_fabrica=$faturamento_fabrica&erro_msg=Não encontrou Faturamento para esse cliente.");
    }
}else{
	$faturamento_fabrica= trim($_GET['faturamento_fabrica']) ;
	$nf                 = trim($_GET['nf']);
...
if($btn_acao == "Enviar CSV"){
	$faturamento_fabrica = $_POST["fabrica_i"];

	if(strlen($faturamento_fabrica) > 0){
		$numeroDeCamposDaTabela = 50;
	  $file_csv = $_FILES['file_csv'];
	 	$types = 'csv';
	  $type  = strtolower(preg_replace("/.+\//", "", $types));

	  if(strlen($type) > 0){
		  if($type == $types){
		  	if($file_csv["size"] == 0){
			    $erro_msg = "O arquivo importado esta Vazio" ;
			  }else{
			    $file_csv = file_get_contents($file_csv['tmp_name']);
			    $file_csv = explode("\n", $file_csv);
			  }
			}else{
				$erro_msg = "Arquivo com Formato inválido" ;
			}
		}else{
			$erro_msg = "Selecionar o Arquivo";
		}

	}else{
		$erro_msg = "Selecione a Fábrica" ;
	}
}
## FIM HD-2416428 ##
...
if ($btn_acao == "Gravar") {
	...
	# hd_chamado=2416428 $condicao    = trim($_POST['condicao'])   ;
	$cfop        = trim($_POST['cfop'])       ;
	# hd_chamado=2416428 $serie       = trim($_POST['serie'])      ;
	...
	# hd_chamado=2416428 $transportadora = trim($_POST['transportadora']);
	$valor_frete= trim($_POST['valor_frete']) ;
	# hd_chamado=2416428 $tipo_frete= trim($_POST['tipo_frete']) ;
	...
	if(strlen($posto_codigo)>0){
		...
		if(pg_numrows($res)>0){
			...
			header ("Location: nf_saida.php?erro_msg=É necessário selecionar o Cliente/Posto.");
	}
	...
	# hd_chamado=2416428 if(strlen($serie)==0)       $erro_msg .= "Digite o Número da Série<br>" ;
	if(strlen($natureza)==0)    $erro_msg .= "Digite a natureza da operação<br>" ;
	# hd_chamado=2416428 if(strlen($transportadora)==0) $erro_msg .= "Selecione a Transportadora<br>" ;
	# hd_chamado=2416428 if(strlen($condicao)==0)       $erro_msg .= "Selecione a Condição<br>" ;
	if(strlen($valor_frete)==0)    $erro_msg .= "Digite o valor do Frete.<br>" ;
	# hd_chamado=2416428 if(strlen($tipo_frete)==0)     $erro_msg .= "Digite o tipo do Frete.<br>" ;
	...
	if(strlen($erro_msg)==0 ){
		...
		$fabrica = 10;
		// HD-2416428 - Retirado campo Serie, condição, transportadora, tipo de frete
					...
					if(strlen($referencia)>0){
						...
						if(strlen($erro_item)==0){
							...
							if(strlen(pg_last_error($con))>0){
								...
								$erro_msg .=$erro_item . "<br>Erro ao inserir peça: $referencia - ". pg_last_error($con);
							}

							$sql = "SELECT fn_estoque_nf_saida($peca,$qtde);";
							$res = pg_query($con,$sql);
							if(strlen(pg_last_error($con))>0){
								#echo "<br>Erro ao inserir peça: $referencia";
								$erro_msg .=$erro_item . "<br>Erro ao dar baixa no estoque: $referencia - ". pg_last_error($con);
							}
						}else{
							$erro_msg .= $erro_item ;
...
*/
		// HD-2416428 Retirado o campo Serie
		...
		if(strlen(pg_last_error($con))>0) {
			$erro_msg.="<br>Erro ao atualizar o faturamento." . pg_last_error($con);
		}
...
<?php if($btn_acao == "Enviar CSV" AND strlen($faturamento) == 0 AND strlen($erro_msg) == 0){ ?>
$(document).ready(function(){
	for (var i = 0; i <= <?php echo count($file_csv); ?>; i++) {
		calc_base_icms(i);
	};
});
<?php } ?>
...
<p>
<form name='nf_saida' method="POST" action='<? echo $PHP_SELF?>' enctype="multipart/form-data">
<input type="hidden" name="fabrica_i" value="<?php echo $_GET['faturamento_fabrica']; ?>" />
<?php
if($btn_acao == "Enviar CSV"){
	$faturamento_fabrica = $_POST["fabrica_i"];
}
...
if(strlen($faturamento) > 0 ){
	...
	if(pg_numrows($res)>0){
		...
		# hd_chamado=2416428 $serie            = trim(pg_result($res,0,serie));
		# hd_chamado=2416428 $condicao         = trim(pg_result($res,0,condicao));
		# hd_chamado=2416428 $transportadora   = trim(pg_result($res,0,transportadora));
		...
		# hd_chamado=2416428 $tipo_frete       = trim(pg_result($res,0,tipo_frete));
	...
	$cfop        = $_POST['cfop']             ;
	# hd_chamado=2416428 $serie       = $_POST['serie']            ;
	# hd_chamado=2416428 $transportadora= $_POST['transportadora'];
	# hd_chamado=2416428 $condicao    = $_POST['condicao']        ;
	$valor_frete    = $_POST['valor_frete']  ;
	# hd_chamado=2416428 $tipo_frete    = $_POST['tipo_frete']    ;
...
echo "<tr bgcolor='#596D9B' style='color:#ffffff ; font-weight:bold' >";
echo "<td colspan='2' align='center'>ID Faturamento</td>";
...
echo "<tr>";
//hd_chamado=2416428 echo "<td><input type='text' name='nota_fiscal' id='nota_fiscal' value='$nota_fiscal' size='6'  maxlength='8' onBlur=\"exibirFat('dados','','','alterar')\" class='frm'><br><div name='f2' id='f2' class='carregar' class='frm'></div></td>\n";
...
//hd_chamado=2416428 echo "<td><input type='text' name='serie'    id='serie'    value='$serie'    size='10' maxlength='10' class='frm'></td>\n";
...
echo "<td align='center' nowrap colspan='1'><input type='text' name='total_nota' id='total_nota' value='$total_nota'  size='10'  maxlength='12' onblur=\"checarNumero(this);\" class='frm'></td>\n";
echo "</tr>";
...
<!-- HD-2416428 -->
<?php if(strlen($_GET['faturamento']) == 0){ ?>
	<tr bgcolor='#596D9B' style='color:#ffffff ; font-weight:bold'>
		<td colspan='14' align="center">Upload Arquivo CSV</td>
	</tr>
	<tr align="center">
		<td colspan="6">
			<input id='file_csv' type="file" name="file_csv" />
			<input type='submit' id='enviar_csv' name='btn_acao' value="Enviar CSV">
		</td>
	</tr>
	<tr>
		<td colspan='6'>&nbsp;</td>
...
<tr>
	<td colspan='6'>
		<?php
			#HD-2416428
			if($btn_acao == "Enviar CSV" AND strlen($faturamento) == 0 AND strlen($erro_msg) == 0){
		?>
        		...
        		foreach ($file_csv as $key => $value) {
          		...
          		list($referencia, $qtde, $preco) = explode(";", $value);

		          if(strlen(trim($referencia)) > 0){
		            $sql_desc = "SELECT descricao FROM tbl_peca WHERE (referencia ilike '%$referencia%' OR referencia_pesquisa ilike '%$referencia%')  AND fabrica = $fabrica_i";
		            $res_desc = pg_query($con, $sql_desc);
		            if(pg_num_rows($res_desc) > 0){
		              $descricao = pg_fetch_result($res_desc, 0, "descricao");
		            }
		          }
		          $cor = "#ffffff";
		          if ($i % 2 == 0) $cor = "#FFEECC";
...
if(strlen($faturamento)==0){
	...
	#	$preco = number_format ($preco,2,',','.');
		...
		echo "<td align='right' nowrap><input class='frm' type='text' name='base_icms_$i' id='base_icms_$i' value='$base_icms' size='5' maxlength='10' style='background-color: $cor; border: none;' readonly></td>\n";
