...
if ($btn_acao == 'faturar') {
	...
	for ($i = 0 ; $i < $qtde_item ; $i++) {
		...
		if (strlen ($qtde_volume) > 0) {
			...
			$valor_frete = (empty($valor_frete))? 0 : $valor_frete;
...
$title = "Faturamento de Embarques";
#$layout_menu = 'pedido';
...
<script type="text/javascript">
	$(document).ready(
		function(){
			 $(".peso_real").maskMoney({symbol:"", decimal:".", thousands:'', precision:3, maxlength: 8});
			 $(".peso_cubado").maskMoney({symbol:"", decimal:".", thousands:'', precision:3, maxlength: 8});
			 $(".frete").maskMoney({symbol:"", decimal:".", thousands:'', precision:2, maxlength: 8});
		}
	);
</script>
...
if ($hora > 12) {
	echo "<center><h2><font color='#663366'>Atenção, faturamento apenas até às 15 horas </font> </h2> </center>";
}
...
$sql = "SELECT tbl_embarque.embarque, tbl_posto.nome, tbl_posto.posto, tbl_posto.cep as cep_destino, 17519255 as cep_origem, 
			  ...
			  COALESCE(tbl_etiqueta_servico.preco, tbl_frete_transportadora.preco) AS preco_frete
	FROM tbl_embarque JOIN tbl_posto USING (posto)
	left join tbl_etiqueta_servico USING(embarque) 
	LEFT JOIN tbl_frete_transportadora ON tbl_embarque.embarque = tbl_frete_transportadora.embarque
	WHERE tbl_embarque.faturar IS NULL
	AND tbl_embarque.embarque <> 0
	AND tbl_embarque.distribuidor = $login_posto
	ORDER BY tbl_embarque.fabrica,tbl_embarque.embarque";
$res = pg_exec ($con,$sql);

echo "<table cellpadding='2' cellspacing='1' align='center' class='formulario' border='0' style='width: 950px'>";
	echo "<tr class='titulo_coluna'>";
		echo "<th>Embarque</th>";
		echo "<th>Posto</th>";
		echo "<th>CEP origem</h>";
		echo "<th>CEP destino</th>";
		echo "<th>Frete SEDEX</th>";
		echo "<th>Peso Real</th>";
		echo "<th>Peso Cúbico</th>";
		echo "<th>Volumes</th>";
		echo "<th>Frete R$</th>";
		echo "<th>Transp.</th>";
	echo "</tr>";
...
for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
	...
	$peso_real = pg_fetch_result($res,$i,'peso');
	...
	$valor_frete = pg_fetch_result($res, $i, 'preco_frete');

		$posto_liberar = pg_fetch_result($res, $i, 'posto');
		if (in_array($posto_liberar,array(4311,20682,376542))){
				echo "<td colspan='8'><a href='$PHP_SELF?embarque_local=$embarque'><font size='+1'>Liberar</font></a></td>";
		}else{
			// rotina de calculo do frete sedex automatico pelo correio
			$cep_origem  = pg_result ($res,$i,cep_origem);
			echo "<td><input type='text' size='8' class='frm' name='cep_origem_$i' value='$cep_origem' readonly></td>";

			$cep_destino  = pg_result ($res,$i,cep_destino);
			echo "<td><input type='text' size='8' class='frm' name='cep_destino_$i' value='$cep_destino' readonly></td>";
...
# não é mais necessário, e estava atrasando o processamento. TULIO 23/10/2009
			$frete_sedex = 0;
			echo "<td><input type='text' size='5' class='frm' name='frete_sedex_$i' value='$frete_sedex' readonly></td>";
			// final da rotina de calculo sedex

			echo "<td nowrap><input type='text' size='5' class='frm peso_real' maxlength='7' name='peso_real_$i' id='peso_real_$i' style='text-align:right;' value='$peso_real' >kg</td>";

			echo "<td nowrap><input type='text' size='5' class='frm peso_cubado' maxlength='7' name='peso_cubado_$i' id='peso_cubado_$i' style='text-align:right;' value='$peso_cubado' >kg</td>";

			echo "<td><input type='text' size='5' class='frm' name='qtde_volume_$i' value='$qtde_volume'></td>";

			echo "<td><input type='text' size='10' class='frm frete' name='valor_frete_$i' value='$valor_frete' autocomplete='off'><br></td>";

			echo "<td>";
			echo "<select name='transportadora_$i' size='1' class='frm'>";
	...
	#		Adicionei e-sedex. Fabio - 05/08/2007 - Solicitado por Ronaldo
			...
			echo "<option value='4773' >MAIS TRANSPORTE</option>";
...
<script type="text/javascript">
		...
		function fnZebraItens(){
			$('table tr:even td').css('background-color','#98C7D3');
