...
$res = pg_exec ($con,"DROP TABLE x_pedido_ebano");
$res = pg_exec ($con,"DROP TABLE x_pedido_fabrica_ebano");
$res = pg_exec ($con,"DROP TABLE x_pedido_transp_ebano");
...
DELETE FROM x_pedido_ebano WHERE qtde <= 0 ;
DELETE FROM x_pedido_ebano WHERE qtde IS NULL ;

ALTER TABLE x_pedido_ebano ADD COLUMN estoque INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN fabrica INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN transp  INT4 ;

ALTER TABLE x_pedido_ebano ADD COLUMN dias30  INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN dias60  INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN dias90  INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN maior   INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN media   INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN desvio  INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN qtdex   INT4 ;
ALTER TABLE x_pedido_ebano ADD COLUMN media_ok INT4 ;


/*            Desconsidera pedidos de postos em atraso                  */

ALTER TABLE x_pedido_ebano ADD COLUMN qtde_atraso INT4 ;

UPDATE x_pedido_ebano SET qtde = (
...
AND   tbl_pedido_item.peca = x_pedido_ebano.peca
)
;

UPDATE tmp_x_pedido_ebano SET qtde_atraso = 0 WHERE qtde_atraso IS NULL ;

UPDATE tmp_x_pedido_ebano SET qtde = qtde - qtde_atraso ;



/*                Estoque                   */

UPDATE x_pedido_ebano SET estoque = (SELECT qtde FROM tbl_posto_estoque WHERE peca = x_pedido_ebano.peca AND posto = 4311);
UPDATE x_pedido_ebano SET estoque = 0 WHERE estoque IS NULL ;
...
INTO TEMP TABLE x_pedido_fabrica_ebano
FROM tbl_pedido
JOIN tbl_pedido_item USING (pedido)
...
UPDATE x_pedido_ebano SET fabrica = x_pedido_fabrica_ebano.qtde_fabrica FROM x_pedido_fabrica_ebano WHERE x_pedido_ebano.peca = x_pedido_fabrica_ebano.peca ;
UPDATE x_pedido_ebano SET fabrica = 0 WHERE fabrica IS NULL ;
...
INTO TEMP TABLE x_pedido_transp_ebano
...
UPDATE x_pedido_ebano SET transp = x_pedido_transp_ebano.qtde_transp FROM x_pedido_transp_ebano WHERE x_pedido_ebano.peca = x_pedido_transp_ebano.peca ;
UPDATE x_pedido_ebano SET transp = 0 WHERE transp IS NULL ;


/*            Saldo para Comprar           */

ALTER TABLE x_pedido_ebano ADD COLUMN qtde_comprar INT4 ;
UPDATE x_pedido_ebano SET qtde_comprar = qtde - estoque - fabrica - transp ;



DELETE FROM x_pedido_ebano WHERE peca IN (SELECT peca FROM tbl_peca WHERE fabrica = 51 AND produto_acabado) ;

/*DELETE FROM x_pedido_ebano WHERE peca IN (SELECT peca FROM tbl_peca WHERE fabrica = 51 AND referencia LIKE '7%') ;*/
...
UPDATE x_pedido_ebano SET dias30 = x_pedido_30.qtde
FROM x_pedido_30
WHERE x_pedido_ebano.peca = x_pedido_30.peca;


UPDATE x_pedido_ebano SET dias30 = 0 WHERE dias30 IS NULL;
...
UPDATE x_pedido_ebano SET dias60 = x_pedido_60.qtde
FROM x_pedido_60
WHERE x_pedido_ebano.peca = x_pedido_60.peca;

UPDATE x_pedido_ebano SET dias60 = 0 WHERE dias60 IS NULL;
...
UPDATE x_pedido_ebano SET dias90 = x_pedido_90.qtde
FROM x_pedido_90
WHERE x_pedido_ebano.peca = x_pedido_90.peca;

UPDATE x_pedido_ebano SET dias90 = 0 WHERE dias90 IS NULL;


/*       maior comprador           */
UPDATE x_pedido_ebano SET maior = (
SELECT MAX (qtde)
FROM (
...
UPDATE x_pedido_ebano SET maior = 0 WHERE maior IS NULL;


/*       media de compra         */
UPDATE x_pedido_ebano SET media = (
SELECT AVG (qtde)
FROM (
	SELECT posto, SUM (qtde) AS qtde
	FROM tbl_pedido
...
UPDATE x_pedido_ebano SET media = 0 WHERE media IS NULL;



/*       desvio padrao       */
UPDATE x_pedido_ebano SET desvio = (
SELECT stddev (qtde)
FROM (
	SELECT posto, SUM (qtde) AS qtde
	FROM tbl_pedido
...
UPDATE x_pedido_ebano SET desvio = 0 WHERE desvio IS NULL;


/*       quantidade de postos que pedem a peca         */
UPDATE x_pedido_ebano SET qtdex = (
SELECT COUNT (*)
FROM (
	SELECT posto, SUM (qtde) AS qtde
	FROM tbl_pedido
...
UPDATE x_pedido_ebano SET qtdex = 0 WHERE qtdex IS NULL;



/*    media sem o desvio padrao       */
UPDATE x_pedido_ebano SET media_ok = (
SELECT AVG (qtde)
FROM (
	SELECT posto, SUM (qtde) AS qtde
	FROM tbl_pedido
	...
	WHERE maior.qtde < x_pedido_ebano.media + x_pedido_ebano.desvio
) ;
UPDATE x_pedido_ebano SET media_ok = 0 WHERE media_ok IS NULL;




SELECT tbl_peca.referencia, tbl_peca.descricao, x_pedido_ebano.* FROM x_pedido_ebano JOIN tbl_peca USING (peca) ORDER BY x_pedido_ebano.dias30 DESC ;

";
//echo nl2br($sql); exit;
$res = pg_exec ($con,$sql);
echo "<h1>Dados gerados em 16/07/2009 - Fazer apenas 1 compra </h1>";
$sql = "SELECT tbl_peca.referencia, tbl_peca.descricao, tmp_x_pedido_ebano.* FROM tmp_x_pedido_ebano JOIN tbl_peca USING (peca) ORDER BY tmp_x_pedido_ebano.dias30 DESC";
