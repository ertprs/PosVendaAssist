...
if (strlen ($btn_acao) == 0) {
	...
	arquivo no Editplus, salve e envie para roseli@deltacontabil.com.br.<br>
	6)<b>Enviar somente o arquivo de ENTRADA para Roseli</b><br>
	7)<b>O arquivo de saída deverá ser feito da seguinte forma:<br>
	- Compactar com zip os arquivos do diretorio <br>
	SAÍDAS http://www.telecontrol.com.br/assist/nfephp2/arquivos/producao/enviadas/aprovadas/yyyymm <br>
	CANCELADAS: http://www.telecontrol.com.br/assist/nfephp2/arquivos/producao/canceladas/yyyymm<br>
	e enviar o link no email para a Roseli do Delta</b><br>
	8)<b>No banco de dados tbl_faturamento_cancelado, listar o número de NF e ChaveNFE e enviar para a<br>
	Roseli, com o seguinte título: NF canceladas/inutilizadas<br>
	Ex:	telecontrol=> select tbl_faturamento_cancelado.nota_fiscal, to_char(tbl_faturamento.emissao,'dd-mm-yyyy') as emissao, <br>
	tbl_faturamento_cancelado.chave_nfe, tbl_faturamento_cancelado.motivo <br>
	from tbl_faturamento_cancelado <br>
	join tbl_faturamento using(faturamento) <br>
	where emissao > '2011-09-01 00:00:00';<br>
 nota_fiscal |  emissao   |                  chave_nfe                   |                      motivo<br>
-------------+------------+----------------------------------------------+--------------------------------------------------<br>
 054392      | 23-09-2011 | 35110904716427000141550000000543921029313967 | Foi emitida a NF 054434 26/9/2011 no lugar desta	<br>
(1 row)
";
...
*/
			...
			echo str_pad("",                             12, "0", STR_PAD_RIGHT); //Controle Interno - preencher com zeros
			...
			echo "000000" ;            # Controle Interno - preencher com zeros
			...
			echo str_pad("", 730, " ", STR_PAD_LEFT); //Campos 30 ao 86 não utilizados 
			echo str_pad($nota_fiscal, 9, "0", STR_PAD_LEFT); //N Documento Inicial e Final: posição  1024/1041
			echo "000000000";

			echo "         "; //N Nota Devolução
			echo "0"; //Cliente do Simples Nacional: 0 = não; 1 = sim
			echo str_pad("", 70, " ", STR_PAD_LEFT); //Campos 90 ao 97: não utilizados
			...
			echo "  ";
			echo "00000000000"; //Controle Interno
			...
			echo "0"; //Exportar DNF
			echo str_pad($nota_fiscal, 9, "0", STR_PAD_LEFT); //N Documento Mov Principal
			echo "000000000";
			echo str_pad("", 84, " ", STR_PAD_LEFT); //Controle Interno: campos 26 ao 33
			echo $tipo_frete; //Modalidade do Frete
			echo str_pad("", 324, " ", STR_PAD_LEFT); //Controle Interno: campos 35 ao 47
			...
			for ($j = 1; $j <= pg_num_rows($res_itens); $j++) {
				...
				echo str_pad($peca,				14	,	"0", STR_PAD_LEFT); //Código Produto Empresa
				echo str_pad($ncm,				8	,	" ", STR_PAD_RIGHT); //NCM
				...
				echo "5"; //Indicador de Tributação do IPI
...
#				WHERE tbl_faturamento.faturamento IN (1086471,1086472,1086473,1086474,1086475,1086476,1086477,1086478,1086479,1086480,1086482,1086483,1086484,1086485,1086486,1086487,1086488,1086489,1086490,1086491,1086492,1086493,1086494,1086495,1086496,1086497,1086498,1086499,1086500,1086501,1086502,1086503,1086504,1086505,1086506,1086507,1086508,1086509,1086481,1064809,1075527,1099322,1099639,1100214,1100961) OR (
		...
		for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
			...
			if ($fabrica == 81) {
				$fabrica_nome   = 'BEST WAY DISTRIBUIDORA DE BENS DE CONSUMO LTDA.';
				$fabrica_cnpj   = '11.195.362/0001-00';
			...
			#--------- Mudanca feita a partir de agosto 2010 ------------------
			if ($fabrica == 10) {
				if ($distribuidor == '58810') {
					$fabrica_nome   = 'BEST WAY DISTRIBUIDORA DE BENS DE CONSUMO LTDA.';
					$fabrica_cnpj   = '11.195.362/0001-00';
					$fabrica_ie     = '148.935.793-115     ';
					$fabrica_ibge   = '35.503.08';
					$fabrica_icms   = 18 ;
					$aliq_icms      = "18,0000";
					$fabrica_estado = 'SP';
			...
			#--------------- Acertos para Fevereiro, Marco, Abril 2011 (tem que fazer em 2 lugares) ----------------------------------------
			if ($faturamento == '2212323' OR $faturamento == '2324092' OR $faturamento == '2386551' OR $faturamento == '2324093') {
				$total_nota   = pg_result ($res,$i,total_nota) ;
				$total_nota   = round ($total_nota * 3,2);
				$total_nota   = number_format ($total_nota ,2,",","");
				$total_nota   = str_pad($total_nota,12, "0", STR_PAD_LEFT);
				$qtde_acertos = $qtde_acertos + 1;
			}
			...
			echo str_pad("",				6	,	"0", STR_PAD_RIGHT); //Controle Interno
			echo $emissao;
			...
			echo "1";                                        # A prazo
			echo "000000000000";                             # Controle Interno
			echo "     "  ;                                  # Especie NF Devolucao
			if ($mes_ano >= '2006-01') {
				echo "   "    ;                              # Serie    NF Devolucao
				echo "  "     ;                              # Subserie NF Devolucao
			...
			echo str_pad("", 590, " ", STR_PAD_LEFT);        # Campos 205 ao 794: não utilizados
			
			echo str_pad($nota_fiscal, 9, "0", STR_PAD_LEFT);          # NF - Número do Documento
			
			echo str_pad("", 90, " ", STR_PAD_LEFT);         # Campos 804 ao 822: não utilizados
			...
			if ($faturamento == '2212323' OR $faturamento == '2324092' OR $faturamento == '2386551' OR $faturamento == '2324093') {
				...
				$total_nota   = round ($total_nota * 3,2);
				$base_icms    = round ($base_icms * 3,2);
				$valor_icms   = round ($valor_icms * 3,2);
				$outras_icms  = round ($outras_icms * 3,2);
			...
			echo "      "; //Controle interno - preencher com espaços
			echo "                   "; //Controle interno - preencher com espaços
			echo "0000000000000,00"; //Controle interno - preencher com zeros (consta duas casas decimais no layout)
