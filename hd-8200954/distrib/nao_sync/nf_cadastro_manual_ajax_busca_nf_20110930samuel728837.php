...
if($tipo == 'codigo') {
	...
	if(pg_num_rows($res)>0) {
		for ( $i = 0 ; $i < @pg_num_rows ($res) ; $i++ ) {
			...
			echo "$sua_os|$consumidor_nome|$consumidor_cpf|$consumidor_cep|$consumidor_numero|$consumidor_complemento|$consumidor_fone|$os|$consumidor_endereco|$consumidor_bairro|$consumidor_cidade|$consumidor_estado";
			echo "\n";
		}
	}else{
