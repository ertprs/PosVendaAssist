...
/*
	...
	JOIN tbl_distrib_lote_os ON tbl_distrib_lote_os.nota_fiscal_mo = lote.nota_fiscal_mo
	JOIN tbl_os_extra on tbl_os_extra.os = tbl_distrib_lote_os.os
	ORDER BY tbl_posto.nome, tbl_os_extra.extrato, lote.nota_fiscal_mo;
	"; */
...
$sql = "SELECT tbl_os.posto, 
	...
	for ($i = 0 ; $i < @pg_numrows ($res) ; $i++) {
		if ($i == pg_numrows ($res) ) $codigo_posto = "*";
			...
			echo "<td align='right'><b>";
			//echo " $nota_fiscal_mo_ant <> $nota_fiscal_mo and $codigo_posto_ant == $codigo_posto ";
