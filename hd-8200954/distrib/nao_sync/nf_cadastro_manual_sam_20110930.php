...
if ($btn_acao2 == "Consultar") {
	...
	$sqlf= "SELECT faturamento, TO_CHAR(emissao,'DD/MM/YYYY') as emissao           ,conferencia       ,TO_CHAR(saida,'DD/MM/YYYY') as saida             ,tbl_posto.nome as posto_nome, posto          ,distribuidor      ,
			cfop              ,nota_fiscal       ,serie             ,transp            ,qtde_volume       ,natureza          ,
			obs               ,total_nota        ,tipo_nf           ,base_icms_substtituicao,valor_icms_substtituicao,
			valor_seguro,valor_desconto,valor_outros,valor_frete,tipo_frete, chave_nfe, recibo_nfe, status_nfe
			FROM tbl_faturamento
			JOIN tbl_faturamento_destinatario using(faturamento)
			LEFT JOIN tbl_posto using(posto)
			WHERE nota_fiscal like '%$nota_fiscal%'
			AND  fabrica in (10,51,81)
			LIMIT 1";
	...
	if(pg_num_rows($resf)>0){
		...
		$sqlff = "SELECT faturamento,nome,cpf_cnpj,ie,logradouro,numero,bairro,complemento,cep,municipio,uf,fone
	...
	$("#lupa_os").click(function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#referencia_0').val(retorno[12]);
				$('#descricao_0').val(retorno[13]);
				$('#qtde_0').val(retorno[14]);
				$('#preco_0').value=0;
				$('#valida_nf_manual').value='1';
				$('#valida_referencia').val(retorno[12]);
...
<tr class="titulo_coluna">
	...
	<input type='hidden' name='valida_nf_manual'  id='valida_nf_manual' value=''>
	<input type='hidden' name='valida_referencia' id='valida_referencia' value=''>
</tr>
