...
//		echo "Programa em manutenção";exit;
	...
	<script type="text/javascript" src='../../distrib/js/tinybox2/tinybox.js'></script>
...
if (strlen($msg_erro) == 0 && strlen($data_inicial) > 0 && strlen($data_final) > 0 && strlen($fabrica) > 0) {
	...
	$sql = "SELECT tbl_peca.peca, tbl_peca.referencia, tbl_peca.descricao, (
					SELECT preco
					 ...
					 WHERE fabrica = $fabrica
					   AND peca    = tbl_peca.peca) AS preco, tbl_posto_estoque_localizacao.localizacao
			  FROM tbl_faturamento
			  JOIN tbl_faturamento_item
				ON tbl_faturamento.faturamento         = tbl_faturamento_item.faturamento
			  JOIN tbl_peca
				ON tbl_faturamento_item.peca           = tbl_peca.peca
			  JOIN tbl_posto_estoque_localizacao
				ON tbl_peca.peca                       = tbl_posto_estoque_localizacao.peca
			   AND tbl_posto_estoque_localizacao.posto = $login_posto /*Os depara desta data em específicos foram feitos para a BestWay para a migração das referências da JM para
			   referências da Telecontrol*/
			  LEFT JOIN tbl_depara
			 ...
			 WHERE tbl_peca.fabrica                    = $fabrica
			   ...
			   AND tbl_faturamento.emissao BETWEEN '$data_inicial 00:00:00'
			   AND '$data_final 23:59:59'
			   AND tbl_depara.peca_de IS NULL $sql_peca $sql_produto_acabado
			 GROUP BY tbl_peca.peca, tbl_peca.referencia, tbl_peca.descricao, tbl_posto_estoque_localizacao.localizacao
	...
	for ($i = 0 ; $i < pg_numrows($res_pecas) ; $i++) {
		...
		$sql = "SELECT SUM(tbl_faturamento_item.qtde) AS saida_total, tbl_tipo_pedido.codigo
				 ...
				 USING (faturamento)
				  JOIN tbl_peca
					ON tbl_peca.peca                = tbl_faturamento_item.peca
				  JOIN tbl_tipo_pedido
					ON tbl_faturamento.tipo_pedido  = tbl_tipo_pedido.tipo_pedido
				 WHERE tbl_faturamento.distribuidor = $login_posto
				   AND tbl_peca.peca IN (
						SELECT $peca_atual $select_depara_acerto_salton)
				   AND tbl_faturamento.emissao BETWEEN '$data_inicial 00:00:00'::timestamp
				   AND '$data_final 23:59:59'::timestamp /*HD 365973 - Separar garantia de faturado*/
		...
		$saidas = pg_num_rows($res);
		$tot_gar = 0; $tot_fat = 0;
		//pre_echo(pg_fetch_all($res),'Saidas');

		for ($i=0; $i < $saidas; $i++) {
			$st = pg_fetch_result($res, $i, 'saida_total');
			$tp = pg_fetch_result($res, $i, 'codigo');
		...
		for($m = 0; $m < $media; $m++) {
			# HD 285329
            $sqlm = "select to_char('$data_inicial_meses'::date + interval '$m months','YYYY-MM'), to_char('$data_inicial_meses'::date + interval '$m
			          months','YYYY-MM-DD 00:00:00');";
			...
			$sql = "SELECT SUM(tbl_faturamento_item.qtde) AS saida_total_mensal, TO_CHAR(emissao, 'YYYY-MM') AS mes
					 ...
					 WHERE tbl_faturamento.distribuidor = $login_posto
					   ...
					   AND tbl_faturamento.emissao BETWEEN '$data_inicial_v'::timestamp AND '$data_final_v'::timestamp - INTERVAL '1 SECOND'
	...
	$t_colspan = ($fabrica == 81) ? 6 : 5;
	echo "<td colspan='$t_colspan'></td>";
