...
if ($_GET["formato"] == "xls" && strlen($msg_erro) == 0 && strlen($data_inicial) > 0 && strlen($data_final) > 0 && strlen($fabrica) > 0) {
	...
	<table>
		...
		<tr>
			...
			<td><input type='text' size='3' name='media' rel='numero' value='<?=$media?>'></td>
...
if (strlen($msg_erro) == 0 && strlen($data_inicial) > 0 && strlen($data_final) > 0 && strlen($fabrica) > 0) {
	...
	/*Os depara desta data em específicos foram feitos para a Salton para a migração das referências da JM para referências da Telecontrol*/
	...
	if ($fabrica == 81) {
		echo "<td style='width:70px;'>Entrada Total Salton</td>";
	...
	for($m=$media-1;$m>=0;$m--) {
		...
		echo "<td style='width:70px;' class='mostra_esconde'>Saída $mes</td>";
	...
	if ($fabrica == 81) {
		echo "<td style='width:70px;'>Devolucao Salton</td>";
		echo "<td style='width:70px;'>Valor Devolucao Salton</td>";
	...
	for ($i = 0 ; $i < pg_numrows($res_pecas) ; $i++) {
		...
		switch($fabrica) {
			case 81:
				...
				AND tbl_peca.peca IN (SELECT $peca_atual)
