...
*/
	...
	var item = 0;
	var total_item = (preco*qtde) + parseFloat(valor_icms) + parseFloat(valor_ipi);
...
function pesquisaPostoAjax(){
	...
	$.ajax({
			url: "nf_cadastro_manual_ajax_busca.php?tipo=posto&q="+q, 
...
$().ready(function() {
	...
	$("#posto_nome").autocomplete("nf_cadastro_manual_ajax_busca.php?tipo=posto", {
		minChars: 2,
		delay: 0,
		width: 350,
		max:50,
...
/*^^^^ PESQUISA POSTO E TRANSPORTADORA - AUTOCOMPLETE ^^^^*/
	...
	$("#lupa_os").click(function(){
		...
		$.ajax({
			url: "nf_cadastro_manual_ajax_busca.php?tipo=codigo&os="+os,
			success: function(data){
				var retorno = data.split("|");
	...
	<tr>
		<td>
			...
			<table class="formulario" cellpadding="0" cellspacing="2" width="600px" align="center" border="0">
				...
				<tr>
					<td>
						<input type='text' class='frm' name='nota_fiscal' id='nota_fiscal' value='<?=$nota_fiscal?>' style='width:100%' size='8'  maxlength='8' onBlur="exibirFat('dados','','','alterar')"><div name='f2' id='f2' class='carregar'></div>
					</td>
