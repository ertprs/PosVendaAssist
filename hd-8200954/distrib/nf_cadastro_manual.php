...
$naturezas = array (
					"ENTRADA DE MERCADORIA OU BEM RECEBIDO P/ CONSERTO OU REPARO"  => array (1915, 2915,294),
					//"ENTRADA DE MATERIAIS DE USO E CONSUMO"                        => array (1556),
                    "OUTRAS ENTRADAS - ENTRADA EM GARANTIA"                        => array (1949, 2949,158),
					"VENDA DE MERCADORIA ADQUIRIDA OU RECEBIDA DE TERCEIROS"       => array (5102, 6102,76),
					"VENDA DE MERCADORIA ADQUIRIDA OU RECEBIDA DE TERCEIROS C/ ST" => array (5403, 6403,76),
					"VENDA DE MERCADORIA REMETIDA EM CONSIGNAÇÃO"                  => array (5114, 6114,76),
					"VENDA DE MERCADORIA RECEBIDA EM CONSIGNAÇÃO"                  => array (5115, 6115,76),
					//"RETORNO DE BEM RECEB. POR CONTA DE CONTR. DE COMODATO"        => array (5909, 6909),
					"RETORNO DE MERCADORIA OU BEM PARA CONSERTO OU REPARO"         => array (5916, 6916,295),
					//"REMESSA EM CONSIGNAÇÃO MERCANTIL"                             => array (5917, 6917),
					"OUTRAS SAÍDAS - REMESSA EM GARANTIA"                          => array (5949, 6949,158),
					//"DISTRIBUIÇÃO DE CESTA BÁSICA"                                 => array (5949, 6949),
					//"DEVOLUÇÃO DE VENDA MERC.ADQ.RECEB.TERCEIROS"                  => array (5403),
					"DEVOLUÇÃO DE REMESSA EM GARANTIA"                             => array (5949,"",158),
                    //"DEV. DE COMPRA P/ COM. EM OP. COM MERC. SUJ. AO REG. DE ST"   => array (5411, 6411),
                    "REMESSA PARA TROCA EM GARANTIA"                               => array (5949, 6949,158),
                    "TRANSFERENCIA DE MERCADORIA"								   => array (5152, 6152,293),
                    "OUTRAS SAÍDAS - DEV. DE REM EM GARANTIA"					   => array (5949, 6949,99),
                    "DEVOLUÇÃO DE COMPRA COM ST"								   => array (5411, 6411,99),
                    "DEVOLUÇÃO DE COMPRA"    									   => array (5202, 6202,99),
			);
...
*/
	...
	$sql = "SELECT trim(TO_CHAR(MAX (nota_fiscal::integer) + 1, '000000')) AS nota_fiscal FROM tbl_faturamento WHERE distribuidor = 4311 AND empresa = '$empresa' and serie = '1'";
	$res = pg_query($con,$sql);
    ...
    $nota_fiscal_new = "000000";//SIGE que vai fornecer numero da nota
	...
	if(strlen($erro_msg) == 0){
		...
		if(strlen($faturamento)>0){
            ...
            $update_tipo_pedido = '';
            if (!empty($categoria)) {
                $update_tipo_pedido = ", tipo_pedido = $categoria";
            }
            ...
            $tipo_pedido_fld = ", tipo_pedido";
            $tipo_pedido_val = ", $categoria";
		...
		$somatoria_nota = 0;
		$bairro_consumidor = pg_escape_string($bairro_consumidor);
		if(strlen($erro_msg) > 0){
			$res = pg_query ($con,"ROLLBACK TRANSACTION");
			...
			$res = pg_query ($con,$sql);
			if (!is_resource($res)) $erro_msg.= "<br> 1Erro ao INSERIR nova NF.";
			$t_item = false;
			...
			for($i=0; $i< $total_qtde_item; $i++){
				...
				if(strlen($referencia)>0){
					...
					if(strlen($erro_item)==0){
						...
						if(!empty($pedido_item) and empty($faturamento_item)){
							$sqlfi = "SELECT faturamento_item FROM
										tbl_faturamento_item
										WHERE faturamento = $faturamento
										AND   peca        = $peca
						...
						if(strlen($faturamento_item)>0){
							...
							if ($pedido == 'null' || $pedido_item == 'null') {
								$erroInsert = 1;
				...
				if(is_numeric($pedido_item) and is_numeric($faturamento)) {
					$sqle = "UPDATE tbl_faturamento set embarque = tbl_embarque_item.embarque from tbl_embarque_item  join tbl_pedido_item using(pedido_item) where tbl_pedido_item.pedido_item = $pedido_item and tbl_faturamento.faturamento = $faturamento and tbl_faturamento.embarque isnull;";
			...
			if(strlen($erro_msg)==0){
				...
				header('Location: http://posvenda.telecontrol.com.br/assist/distrib/nf_cadastro_manual.php?s=1');
...
function atualizaCFOP(sNatCfop) {
	...
	//Inicio Categorias
	document.getElementById('categorias').value = categ;
	//window.alert("alerta "+categ);
...
function somaValores(){
	...
	$("#total_nota").val(total_geral.replace(",","."));
	$("#total_texto").html(total_geral.replace(",","."));
...
//Deixar campro preço unitario com 4 casas decimais
...
function limpaCampos(){
	...
	$('#qtde_volumes').val('');
	$('#transp').val('');
	$('#fornecedor_distrib_posto').val('');
	$('#base_icms_substtituicao').val('');
	$('#valor_icms_substtituicao').val('');
	$('#valor_frete').val('');
	$('#valor_desconto').val('');
	$('#outros_valores').val('');
	$('#valor_seguro').val('');
	...
	$('#tipo_frete').val('');
	$('#total_nota').val('');
	$('#email_consumidor').val('');
	$('#transportadora_ option')[0].selected = true;
	...
	$.ajax({
			...
			success: function(data){
				...
				if (retorno[0] == "F"){
					...
					var numItens;
					var lcItens;
					var linhasItens = (itens.length-1)/6;
					...
					for( contItens = 0, lcItens=1; contItens<linhasItens; contItens++){
						...
						var pedidoItem = itens[(lcItens+5)];
						$("#pedido_item_"+contItens).val(pedidoItem);

						var pedido = parseInt(itens[(lcItens+6)]);
						$("#pedido_"+contItens).val(pedido);
...
$().ready(function() {
	...
	// descomentar somente essa linha 	pesquisaOsAjax();
...
if ($erro_msg) {
?>
<table width="707px" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td class="msg_erro">
			<?=$erro_msg?>
		</td>
		...
		<td>
			<table class="formulario" cellpadding="0" cellspacing="2" style="width:600px;" align="center" border="0">
					...
					<td colspan='4'>
						...
						echo '<select name="natureza" id="natureza" onChange="atualizaCFOP(this.value)" style="width: 320px;" />';
							...
							foreach ($naturezas as $n => $cfops) {
								$class = (in_array($cfops[0],array(1915,1556,1949))) ? ' class="entrada" ' : ' class="saida" ' ;
								echo '<option value="' , $n , '#' , $cfops[0] , '#' , $cfops[2], '"' , $class , '>' , $n , '</option>';
							}
						echo '</select>';
						?>
					</td>
				...
				<tr>
					...
					<td nowrap>
						Distribuidor<br/>
						<input type="radio" name="distribuidor_cod" value="4311" />Matriz
						<input type="radio" name="distribuidor_cod" value="376542" />Filial
                ...
                <tr>
                    <td>
                        ...
                        <select name="categorias" id="categorias" disabled="disabled" >
                        <option value="">---</option>
                            <?php
                                $sql_cat = "SELECT tipo_pedido, descricao 
                                            FROM tbl_tipo_pedido 
                                            WHERE fabrica = 10 
                                            AND codigo IN ('TRANSF', 'REMC', 'RETC', 'GAR', 'FAT', 'DEVOL')
                                            ORDER BY descricao";
                                $qry_cat = pg_query($con, $sql_cat);

                                while ($res = pg_fetch_assoc($qry_cat)) {
                                    echo '<option value="' . $res['tipo_pedido'] . '">' . ucfirst(strtolower($res['descricao'])). '</option>';
                                }
			...
			<table cellpadding="0" cellspacing="2" align="center" style="width:600px;" class="formulario" border="0">
				...
				<tr>
					<td>
						...
						<select id="transportadora_" name='transportadora' style="width:80%" class='frm'>
...
<table class="formulario" align='center' id="tbl_itens_nf" name="tbl_itens_nf" cellpadding='0' cellspacing='1' style='border-top: none; border-bottom: none;'>
	<tr>
		<th class="titulo_tabela" align="center" colspan="100%">Itens da Nota Fiscal</th>
	</tr>
<tr class="titulo_tabela">
	...
	<td style="width:70px" title='Adicionada coluna de CFOP por ítem, novo padrão para NF-e'>CFOP</td>
...
<?
	for ($i = 0 ; $i < $total_qtde_item ; $i++) {
		...
		<tr style='font-size: 12px' id='<?=$qtde_linha?>'>
			...
			<td align='center' nowrap>
				<input class='frm qtde_prod' style="width:40px;" type='text' maxlength='10' name='qtde_<?=$i?>'	id='qtde_<?=$i?>'	value='<?=$qtde?>'	onKeyUp='calc_base_icms(<?=$i?>);' onblur="checarNumero(this);">
			</td>
			<td align='center' nowrap>
				<input class='frm' style="width:55px;" type='text' maxlength='12' name='preco_<?=$i?>'	id='preco_<?=$i?>'	value='<?=$preco?>'	onKeyUp='calc_base_icms(<?=$i?>);' onblur="checarPreco(this);">
			</td>
			<td align='center' nowrap>
				<input class='frm' style="width:55px;" type='text' maxlength='12' name='cfop_<?=$i?>'	id='cfop_<?=$i?>'	value='<? if ( strlen($_POST["cfop_$i"])>0 ) echo $cfop_item ;?>'>
			</td>
			<td align='center' nowrap>
				<input class='frm' style="width:45px;" type='text' maxlength='10' name='aliq_icms_<?=$i?>'	id='aliq_icms_<?=$i?>'	value='<?=$aliq_icms?>'	onKeyUp='calc_base_icms(<?=$i?>);' onblur="checarNumero(this);">
			</td>
			<td align='center' nowrap>
				<input class='frm' style="width:45px;" type='text' maxlength='10' name='aliq_ipi_<?=$i?>'	id='aliq_ipi_<?=$i?>'	value='<?=$aliq_ipi?>'	onKeyUp='calc_base_icms(<?=$i?>);' onblur="checarNumero(this);" >
			</td>
			<td align='center' nowrap>
			...
			<input type="hidden" name="pedido_item_<?php echo $i ?>" id="pedido_item_<?php echo $i ?>" value="" />
			<input type="hidden" name="pedido_<?php echo $i ?>" id="pedido_<?php echo $i ?>" value="" />
		</tr>
		<?
	}
	echo "</tbody>";
...
<table align="center" class="formulario"  cellpadding='0' cellspacing='1'>
	<tr>
		...
		<td>
			<table cellpadding='0' cellspacing='2' align='center' class='formulario' border='0' align="center">
				...
				<tr>
					...
					<td width="233" align="left"><input onKeyUp="somaValores()" type="text" class="frm money" style='width:150px;text-align:center' name="valor_frete" id="valor_frete" maxlength="12"  value="<?=$valor_frete?>" /></td>
				</tr>
				<tr>
				...
				<tr height="50px">
					...
					?>
					<td><input type="hidden" value="<?=$total_nota?>" id="total_nota" name="total_nota"></td>
...
if ($btn_acao2 == "Consultar" and (strlen($chave_nfe)>0 or strlen($recibo_nfe)>0 or strlen($status_nfe)>0)) {
	?>
	<INPUT TYPE="BUTTON" VALUE="Nova Consulta" ONCLICK="window.location.href='nf_cadastro_manual.php'">
	<?
}else{
	echo "<input type='hidden' name='btn_acao' value=''>";
	echo "<input type='button' name='btn_grava' value='Gravar' onclick='javascript: document.form_nf.btn_acao.value=\"Gravar\"; $(\"#categoria\").val($(\"#categorias\").val()); document.form_nf.submit()'>";
}
		...
		function geraLinha(){
			...
			html += "<tr>";
				html += "<td align='center' nowrap><input type='text' class='frm' style='width: 70px;' name='referencia_"+indice+"' id='referencia_"+indice+"' maxlength='20' rel='referencia' alt='descricao_"+indice+"'></td>";
				 ...
				 html += "<td align='center' nowrap><input class='frm' style='width:55px;' type='text' maxlength='12' name='preco_"+indice+"'	id='preco_"+indice+"'	onKeyUp='calc_base_icms("+indice+");' onblur='checarPreco(this);'></td>";
				 ...
				 html += "<input type='hidden'  name='total_item_nf_<?=$i?>' id='total_item_nf_<?=$i?>' value='0' class='total_item_nf'>";
				 html += "<input type='hidden' name='os_item_"+indice+"' id='os_item_"+indice+"' value='' />";
				 html += "<input type='hidden' name='pedido_item_"+indice+"' id='pedido_item_"+indice+"' value='' />";
				 html += "<input type='hidden' name='pedido_"+indice+"' id='pedido_"+indice+"' value='' />";
			html += "</tr>";
