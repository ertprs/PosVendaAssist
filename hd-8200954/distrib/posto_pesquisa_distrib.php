...
if ($tipo == "nome") {
	...
	if (@pg_num_rows($res) == 0) {
		...
		echo "<h2>Se você não conseguir encontrar o fornecedor, avise o Ger. Ronaldo para cadastrar na Fábrica Telecontrol o posto para que sirva de Fornecedor (não precisa credenciar como posto, apenas cadastrar).</h2>";
		echo "<script language='javascript'>";
		echo "setTimeout('window.close()',7000);";
		echo "</script>";
		exit;
	}
}
...
if (pg_num_rows($res) == 1) {
	echo "<script language='javascript'>";
	echo "nome.value   = '".str_replace ('"','',trim(pg_fetch_result($res, 0, 'nome')))."';";
	echo "codigo.value = '".trim(pg_fetch_result($res, 0, 'codigo_posto'))."';";
