...
class CartaoDePostagem
{
    ...
    private function _render ($dest='', $fileName= '')
    {
        ...
        while (count($objetosPostais)) {
            ...
            if ($this->logoFile) {
                $this->pdf->Image($this->logoFile, 5, ($this->pdf->GetY() + 8), 30, 15);
            }
            ...
            $this->t(105, $etiquetaFormatada, 2, 'C');
            ...
            if(count($_siglaAdicinal) > 0){
                $paddingLeftBarCode = 5;
            } else {
                $paddingLeftBarCode = 10;
            }
            ...
            // Código de Barra Etiqueta do CEP Destianatário
            $hCepBarCode = 18;
            $wCepBarCode = 40;
            $this->setFillColor(0, 0, 0);
            $code128 = new \PhpSigep\Pdf\Script\BarCode128();
    ...
    private function writeDestinatario ($l, $t, $w, $objetoPostal)
    {
        ...
        $titulo           = 'DESTINATÁRIO';
        $nomeDestinatario = $this->retira_acentos($objetoPostal->getDestinatario()->getNome());
        $logradouro       = $objetoPostal->getDestinatario()->getLogradouro();
        $numero           = $this->retira_acentos($objetoPostal->getDestinatario()->getNumero());
        $complemento      = $this->retira_acentos($objetoPostal->getDestinatario()->getComplemento());
        $bairro           = '';
        $cidade           = '';
        $uf               = '';
        $cep              = '';
		...
		$logradouro = mb_detect_encoding($logradouro,'UTF-8',true) ? $this->retira_acentos($logradouro) : $this->retira_acentos(utf8_encode($logradouro));
        if ($destino instanceof \PhpSigep\Model\DestinoNacional) {
            $bairro = $this->retira_acentos($destino->getBairro());
    ...
    {
        $titulo           = 'Remetente:';
        $nomeDestinatario = $this->retira_acentos($remetente->getNome());
        $logradouro       = strtoupper($this->retira_acentos($remetente->getLogradouro()));
        $numero           = $remetente->getNumero();
        $complemento      = $this->retira_acentos($remetente->getComplemento());
        $bairro           = $this->retira_acentos($remetente->getBairro());
        $cidade           = $this->retira_acentos($remetente->getCidade());
        $uf               = $remetente->getUf();
    ...
    private function writeEndereco (
        ...
        if ($destinatario === true) {
            ...
            $this->multiLines($w, substr($nomeDestinatario, 0, 48), 'L');
            ...
            $this->setFillColor(190, 190, 190);
            $this->pdf->SetXY(22, $t);
            $this->multiLines($w, trim(substr($nomeDestinatario, 0, 48)), 'L');
        }
    ...
    private function gerarJson($dados){
        if(isset($dados["dados"])){
            $dados['dados'] = utf8_decode($dados['dados']);
            $dados = str_replace("\\", "", $dados['dados']);
        ...
        $dados_remetente = $dados['remetente'];
        // var_dump($dados['remetente']); exit;
        ...
        if(isset($dados['embarque'])){
            $dados = $dados['embarque'];

        } else {
            $dados = $dados['dados_coleta'];
        }
        for($i=0; isset($dados[$i]); $i++){
            
            $etiqueta = new \PhpSigep\Model\Etiqueta();
            $etiqueta->setEtiquetaComDv($dados[$i]['etiqueta']);
            $etiqueta->setEtiquetaSemDv(NULL);
            $etiqueta->setDv(NULL);

            $servicoDePostagem = new ServicoDePostagem($dados[$i]['codigo']);
            $servicoDePostagem->setCodigo($dados[$i]['codigo']);
            $servicoDePostagem->setIdServico($dados[$i]['chave_servico']);
            $servicoDePostagem->setNome($dados[$i]['descricao']);

            $destinatario = new \PhpSigep\Model\Destinatario();
            $destinatario->setNome($dados[$i]['nome']);
            if(isset($dados[$i]['embarque'])){
                $destinatario->setEmbarque($dados[$i]['embarque']);
            ...
            $destinatario->setLogradouro($dados[$i]['endereco']);
            $destinatario->setComplemento($dados[$i]['complemento']);
            $destinatario->setNumero($dados[$i]['numero']);

            $destino = new \PhpSigep\Model\DestinoNacional();
            $destino->setBairro($dados[$i]['bairro']);
            $destino->setCep($dados[$i]['cep']);
            $destino->setCidade($dados[$i]['cidade']);
            $destino->setUf($dados[$i]['estado']);
	    $destino->setNumeroNotaFiscal($dados[$i]['nota_fiscal']);
	    $destino->setNumeroPedido($dados[$i]['pedido']);
            ...
            if ($dados[$i]['preco'] > 50) {
                ...
                $servicosAdicionais[0]->setValorDeclarado($dados[$i]['preco']);
            }

            $dimensao = new \PhpSigep\Model\Dimensao();
            $dimensao->setTipo(2);
            $dimensao->setAltura($dados[$i]['altura']);
            $dimensao->setLargura($dados[$i]['largura']);
            $dimensao->setComprimento($dados[$i]['comprimento']);
            ...
            $encomendas[$i]->setCubagem($dados[$i]['cubagem']);
            $encomendas[$i]->setPeso($dados[$i]['peso']);
