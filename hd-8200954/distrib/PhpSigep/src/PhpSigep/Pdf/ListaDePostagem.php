...
// Cria o cabeçalho
        ...
        $pdf->x += $wHeaderCols;
        $this->labeledText($pdf, 'Endereço:', utf8_encode($remeAddress), $wHeaderCols * 2, 1, 16 / $k);
        $yAboveAddress = $pdf->y;
    ...
    private function writeListBody($pdf, $k, $y1, $y2, $xCol1, $wCol1, $xCol2, $wCol2, $xCol8, $wCol8, $xCol3, $wCol3, $xCol4, $wCol4, $xCol5, $wCol5, $xCol6, $wCol6, $xCol7, $wCol7, $xCol9, $wCol9, $xCol10, $wCol10)
    {
        ...
        $objetos_envio = array($plp->getEncomendas());
        $objetos_envio = $objetos_envio[0];
        $objetos_envio = $objetos_envio[0];
        $i = 0;
        foreach ($objetoPostais[$i] as $objetoPostal) {
        // foreach ($objetoPostais as $objetoPostal) {
            ...
            $pdf->SetX($xCol10);
            $pdf->CellXp($wCol10, substr($objetoPostal->getDestinatario()->getNome(), 0, 38).".", 'L');
        }
    }
