 ...
 * @author: Stavarengo
 */
class FecharPreListaDePostagemImplantacao
{
    ...
    public function execute(\PhpSigep\Model\PreListaDePostagem $params)
    {
        ...
        $objetos_envio = array($params->getEncomendas());
        $objetos_envio = $objetos_envio[0];
        $objetos_envio = $objetos_envio[0];

        for($i=0; isset($objetos_envio[$i]); $i++) {
            $listaEtiquetas[] = $objetos_envio[$i]->getEtiqueta()->getEtiquetaSemDv();
        }

	$listaEtiquetas = array_unique($listaEtiquetas);
...
//		$xml = iconv('UTF-8', 'ISO-8859-1', $xml);
        ...
        $soapArgs = array(
         ...
         $result = new Result();
        // $result->setListaEtiqueta($listaEtiquetas);
         try {
    ...
    private function getPlpXml(PreListaDePostagem $data)
    {
        ...
        $objetos_envio = array($data->getEncomendas());
        $objetos_envio = $objetos_envio[0];
        $objetos_envio = $objetos_envio[0];
        for($j=0; isset($objetos_envio[$j]); $j++) {
            $this->writeObjetoPostal($writer, $objetos_envio[$j]);
        }
