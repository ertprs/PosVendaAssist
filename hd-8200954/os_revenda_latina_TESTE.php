...
$referencia               = trim($_POST['produto_referencia_0']);
$serie                    = trim($_POST['produto_serie_0']);
$capacidade               = trim($_POST['produto_capacidade_0']);
$type                     = trim($_POST['type_0']);
$embalagem_original       = trim($_POST['embalagem_original_0']);
$sinal_de_uso             = trim($_POST['sinal_de_uso_0']);
$aux_nota_fiscal          = trim($_POST['aux_nota_fiscal_0']);
$aux_qtde                 = trim($_POST['aux_qtde_0']);
$revenda_cnpj  = $_POST['revenda_cnpj'];

echo "<br>1 revena: $revenda_cnpj - $referencia - $serie - * - $aux_nota_fiscal - ";

$referencia               = trim($_POST['produto_referencia_1']);
$serie                    = trim($_POST['produto_serie_1']);
$capacidade               = trim($_POST['produto_capacidade_1']);
$type                     = trim($_POST['type_1']);
$embalagem_original       = trim($_POST['embalagem_original_1']);
$sinal_de_uso             = trim($_POST['sinal_de_uso_1']);
$aux_nota_fiscal          = trim($_POST['aux_nota_fiscal_1']);
$aux_qtde                 = trim($_POST['aux_qtde_1']);

echo "<br>2 revena: $revenda_cnpj -  $referencia - $serie - * - $aux_nota_fiscal - ";
		...
		if ($login_posto == 14254 and $login_fabrica == 11) {
			...
			if($login_fabrica==19){
				if(!is_numeric($nota_fiscal)){
					 $msg_erro = "Número da nota fiscal inválido";
				}
			}
			$xnota_fiscal = "'" . $nota_fiscal . "'" ;
		}
...
function addRowToTable(){
	...
	var celula = criaCelula(document.frm_os.produto_descricao2.value);
...
/*	var cellRight1 = row.insertCell(1);
	...
	cellRight1.appendChild(textNode);
*/
	celula = criaCelula(document.frm_os.produto_referencia2.value);
	var el = document.createElement('input');
	el.setAttribute('type', 'hidden');
...
function adicionaLinha(linha){
	...
	for (i=1;i<=linha;i++) {
		if(tbl.rows.length < 300){
			...
			alert('Limite de campos é de 20!');
			return false;
		}
	}
}
...
<table width="650" border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#ffffff" class="table">
	<tr>
		<td nowrap style='font-size: 10px; font-family: arial;' bgcolor='#FFFFCC'><b>ESTA PÁGINA NÃO FUNCIONA CORRETAMENTE COM MOZILA E OPERA.<br>CASO UTILIZE UM DESTES 2 NAVEGADORES <a href='os_revenda.php'>CLIQUE AQUI</a> PARA ACESSAR A TELA ANTIGA</b></td>
	</tr>
</table>
...
//						$sql = "SELECT * FROM tbl_tipo_atendimento WHERE fabrica = 19 ORDER BY tipo_atendimento";
					...
					<td nowrap align='center' colspan='2'><font size="2" face="Geneva, Arial, Helvetica, san-serif"><INPUT TYPE="radio" NAME="motivo" value='12' <? if ($motivo==12)echo "checked";?>>&nbsp;Inclusão &nbsp;&nbsp;&nbsp;<INPUT TYPE="radio" NAME="motivo" value='11' <? if ($motivo==11)echo "checked";?>>&nbsp;Solicitação</font></td>
				</tr>
				<? } ?>
				<tr class="menu_top">
					<? if ($pedir_sua_os == 't') { ?>
				...
				<tr>
					...
					<td nowrap align='center'>
						<? if ($login_fabrica==5) { ?>
							<input name="sua_os" class="frm" type="text" size="10" maxlength="10" value="<? echo $sua_os ?>" onblur="this.className='frm'; displayText('&nbsp;');" onfocus ="this.className='frm-on';displayText('&nbsp;Digite aqui o número da OS do Fabricante.');" onkeyup="checarNumero(this)">
						<? } else {?>
...
if($login_fabrica == 15 AND $os_revenda == 0 AND $btn_acao <> 'gravar'){
	...
	if($login_fabrica == 15 AND (strlen($os_revenda) <> 0 OR $btn_acao == 'gravar')){
	for ($i=0; $i<$qtde_item; $i++) {
