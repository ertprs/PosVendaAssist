...
*/
	...
	if ($dbport_dynacom == 0 OR $dbport_dynacom == NULL) {
		$dbport 	= 5432;
	}

	if (strlen ($dbbanco_dynacom) == 0) {
		$dbbanco 	= "postgres";
		$dbport         = 5432;
	}
	#-------------------- PostgreSQL ----------------
	if (strlen ($dbhost_dynacom) == 0) $dbhost_dynacom = "200.212.63.68";
	...
	if ($dbbanco_dynacom == "postgres") {
		$parametros_dynacom = "host=$dbhost_dynacom dbname=$dbnome_dynacom port=$dbport_dynacom user=$dbusuario_dynacom password=$dbsenha_dynacom";
		//echo $parametros;
		
		$erro_conexao = true ;
		for ($i == 0 ; $i < 10 ; $i++) {
			if ($conn = pg_connect($parametros_dynacom)) {
				$erro_conexao = false ;
				break ;
			}
			sleep (5);
		...
		if ($erro_conexao == true) {	
			...
			if (1 == 1 and $HTTP_COOKIE_VARS['cook_sistema_lingua'] =="ES") {
				...
				echo "<br><br><br>";
				echo "<center><h2><strong><img src='/assist/logos/telecontrolmini.gif'>
				<br><br>
