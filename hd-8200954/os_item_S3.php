...
$fabricas_padrao_itens = array(81,94,98,99);

#HD 670818 -  FIM

//HD 56418 Candece
if (in_array($login_fabrica, array(35,40,50))) {
...
if ($ajax == "numero_serie") {
    ...
    if (pg_num_rows($res) > 0) {
        ...
        if (pg_num_rows($res) == 1){
            ...
            $sql = "SELECT numero_serie, serie FROM tbl_numero_serie WHERE produto = $produto AND serie = '$numero_serie';";
            $res = pg_query($con,$sql);
...
# HD 13618 - Fabrica 45
# HD 12657 - Fabrica 2
if (in_array($login_fabrica,array(2,3,6,11,25,45,50,51,81,98,106,108,111))) {
...
if (isset($_REQUEST['n_itens']) AND strlen($_REQUEST['n_itens']) > 0) {
    ...
    if ($login_fabrica <> 15) {
		...
		if ($login_fabrica == 86) {
			...
			if ($qtde_itens_mostrar <= 10)
				$qtde_itens_mostrar = 10;
			elseif ($qtde_itens_mostrar <= 20)
				$qtde_itens_mostrar = 20;
			elseif ($qtde_itens_mostrar <= 30)
...
if ($btn_acao == "gravar") {
    ...
    if ( in_array($login_fabrica,array(3,6,24,42,45,80,81,91)) ) {
	
        if (strlen($defeito_constatado)==0) {
            $msg_erro .= "Por favor preencher o campo defeito constatado.<br />";
        }

        if (!in_array($login_fabrica,array(45,81,42,74,94,86,95,52,104,105))) {
	...
	if ($login_fabrica == 94) {
		...
		if ($km == 1 AND (strlen($qtde_km) == 0 OR $qtde_km == 0)) {
			...
			if ($qtde_km > 100) {
				...
				$qtde_km -= 100; // HD 415106 - Foi solicitado para pagar somente o que exceder de 100km. Brayan.
	...
	if (in_array($login_fabrica, array(15,19,24,30,52,74,85))) {
    ...
    if (strlen($msg_erro) == 0) {
        ...
        if (in_array($login_fabrica,array(30,43,59,2,85,95,94))) {
    ...
    if (($pedir_defeito_reclamado_descricao == 't') AND ( $defeito_reclamado == 'null' OR in_array($login_fabrica,array(94)) ) ) {
    ...
    } else if($login_fabrica <> 42 and $login_fabrica <> 96 and $login_fabrica <> 74) {//PARA OUTRAS FÁBRICAS SETA NULL
    ...
    if (strlen($msg_erro) == 0) {
        ...
        for ($i = 0 ; $i < $qtde_item ; $i++) {
			...
			if (!empty($xservico)) {//HD 384011
				...
				$sql_servico  = "SELECT peca_estoque FROM tbl_servico_realizado WHERE servico_realizado = $xservico AND fabrica = $login_fabrica AND peca_estoque IS TRUE;";
				$res_servico  = @pg_query($sql_servico);
				$serv_estoque = @pg_num_rows($res_servico) > 0 ? pg_result($res_servico, 0, 'peca_estoque') : 'f';
			...
			if (in_array($login_fabrica,array(3,30,74)) && empty($msg_erro) && strlen($xpeca) > 0 && $controla_estoque == 't' && $serv_estoque == 't') {
                ...
                if (strlen($xpeca) > 0 and strlen($msg_erro) == 0) {
                    ...
                    if (strlen($msg_erro) == 0) {
                        ...
                        if (($login_fabrica==3 or $login_fabrica == 45 or $login_fabrica == 24) AND ($xservico == 643 OR $xservico == 644 OR $xservico == 4247 or $xservico == 4289)) {
                            ...
                            if (strlen($xdefeito) == 0 and !in_array($login_fabrica, array(95,96,81,98))) {
                                    ...
                                    if (strlen($xpeca) > 0) {
                                        ...
                                        if (strlen($msg_erro) == 0) {
                                            ...
                                            if (strlen($xos_item) == 0) {
												...
												if(in_array($login_fabrica,array(96,81,98))){
													$xdefeito = 'null';
												}
					    ...
					    else if (in_array($login_fabrica,array(98,106,108,111)) AND $bloqueada_peca_critica == 't') { //HD 418875 Ederson Sandre
                                                // envia email teste para avisar
                                                $os_com_intervencao_carteira ='t';
                                                $gravou_peca="sim";
                                            }
...
if (strlen($x_solucao_os) > 0 && $x_solucao_os <> 'null' && $login_fabrica != 47 && $login_fabrica != 59 && empty($msg_erro)) {
...
} else if ($login_fabrica == 59) { //HD 232039: Validar defeito constatado e solução para fábricas que lançam mais de um defeito x solucao
    ...
    if (strlen($msg_erro) == 0) {
		...
		if ($login_fabrica == 74 and empty($_POST['produto_serie']) and empty($msg_erro) and empty($os_aprovada)) {
    ...
    if (strlen($msg_erro) == 0 and $gravou_peca == "sim" and (in_array($login_fabrica, array(2,3,6,11,51,81,98,106,108,111)))) {
        ...
        } else if ($login_fabrica == 11 AND $intervencao_previsao == 't' and strlen($msg_erro) == 0) {
	...
	if ( ($login_fabrica == 43 || $login_fabrica == 94 || $qtde_pecas_intervencao > 0) && strlen($msg_erro) == 0) {
	...
	if ($login_fabrica == 86 AND strlen($msg_erro) == 0) {
	...
	if (strlen($msg_erro) == 0) {
		...
		if( $tipo_atendimento <> 93) {
			...
			$sqlB = "UPDATE tbl_os SET status_checkpoint=fn_os_status_checkpoint(os)
					WHERE os = $os
					AND	status_checkpoint<>fn_os_status_checkpoint(os)";
        ...
        if (!in_array($login_fabrica,array(2,51,106,108,111)) AND ($os_bloqueada_garantia == 't' OR $peca_mais_30_dias == 't' OR $os_com_intervencao == 't' OR $intervencao_previsao == 't' OR $os_com_intervencao_carteira == 't')) {
            ...
            if ($login_fabrica == 3 AND $btn_imprimir == 'imprimir') {
                ...
                if ($login_fabrica <> 11) {
					...
					if(in_array($login_fabrica,array(106,108,111))){
						header("Location: os_press.php?os=$os");
					} else {
						header("Location: os_finalizada.php?os=$os");
					}
...
<script type="text/javascript">
    $().ready(function() {
        ...
        $("select[rel=servicos_realizados]").change(function(){
            var campo = $(this);
            if ( $("input[name='peca_"+campo.attr("alt")+"']").val() == '073894' || $("input[name='peca_"+campo.attr("alt")+"']").val() == '073373' || $("input[name='peca_"+campo.attr("alt")+"']").val() == '771889' || $("input[name='peca_"+campo.attr("alt")+"']").val() == '074957' || $("input[name='peca_"+campo.attr("alt")+"']").val() == '773936') {
...
$().ready(function() {
	...
	$("input[rel='data']").maskedinput("99/99/9999");//HD 785254
...
if($login_fabrica == 30 or $login_fabrica == 59 or $login_fabrica == 2) { // HD 46493
	...
	if(in_array($login_fabrica,$fabricas_padrao_itens) || $login_fabrica == 95 || $login_fabrica >= 99){
...
<!-----------HD 341319 FIM---------->
		...
		GeraComboType::makeComboType($parametrosAdicionaisObject, $type, null, array("class"=>"frm"));
            ...
            <? if ($login_fabrica==24 OR (($login_fabrica==30 OR $login_fabrica==52) AND strlen($produto_serie_db)==0) or $login_fabrica ==15 or $login_fabrica ==85 or $login_fabrica == 74 OR ($login_fabrica==19 and $produto_linha == 265 ) ){ # HD 708727 - ADicionado Lorenzetti somente para a linha 265 ?>
				<?
				#HD 345334
			...
			<td>
				<a style='font-size:9pt' href='os_upload_nf_produto.php?os=<?=$sua_os?>' title='Clique para anexar a imagem da Nota Fiscal' target='_blank'> <img src="imagens/add_nf.gif"> </a>
			</td>
            <?}*/?>
            ...
            <td nowrap>
				...
				<input type="hidden" name="produto_serie" id="produto_serie" value="<?=$produto_serie ?>">
            </td>
            <? } ?>
        </tr>
	</table><?php
...
//HD 351696 Início
if ($login_fabrica == 42 || $login_fabrica == 74 || (in_array($login_fabrica,$fabricas_padrao_itens) || $login_fabrica > 99)) {
	...
	<table width='100%' class='formulario'>
		<tr><?php
			...
			if (!in_array($login_fabrica, array(81,94,98,99,101)) && $login_fabrica < 104) {?>

				<td align='left'>
					Defeito Reclamado <br /><?php
			...
			if (($login_fabrica == 98 OR $login_fabrica == 81) AND $consumidor_revenda == "R" AND empty($defeito_reclamado_descricao_os)) {?>
			...
			<td><?php
				if (!empty($defeito_reclamado_descricao_os) and $login_fabrica != 74 && in_array($login_fabrica,$fabricas_padrao_itens) && $login_fabrica < 99 && $login_fabrica != 94) {
					...
					if (pg_numrows($resDesc) > 0) {
						...
						if ($login_fabrica == 98 or $login_fabrica == 81) {
							echo "readonly";
						}
			...
			<?php if ($login_fabrica != 94 && $login_fabrica !=101 && $login_fabrica < 104) { ?>
			<td>
				Defeito Constatado<br />
				<select name='defeito_constatado' id='defeito_constatado' class='frm'>
					<option value=''>Selecione Defeito Constatado</option><?php
...
#HD 670814 - INICIO
$fabricas_defeitos = array(2,3,5,6,11,19,24,25,26);
if ((in_array($login_fabrica,$fabricas_defeitos) && strlen($defeito_reclamado) > 0) || (($login_fabrica>28 && $login_fabrica <> 42 && $login_fabrica <> 74 && !in_array($login_fabrica,$fabricas_padrao_itens) && $login_fabrica < 99 ) && strlen(trim($defeito_reclamado)) > 0 ) || $login_fabrica == 96){
#HD 670814 - FIM
...
//validações da Lennox
    ...
    if ($login_fabrica <> 19) {
        ...
        echo "<select name='defeito_constatado' id='defeito_constatado' size='1' class='frm'  style='width:140px;' ";
...
if (strlen($defeito_reclamado) == 0 && $login_fabrica <> 42 && $login_fabrica <> 74 && $login_fabrica <> 96) {
    echo "<table width='100%' border='0' cellspacing='5' cellpadding='0'>";
	...
	//validacoes lennox
	if ($login_fabrica != 98 AND $login_fabrica != 81) {
		...
		if ($tipo_atendimento == 22) {
			...
			//CONSTATADO
			if ($login_fabrica <> 19 && $login_fabrica != 99) {
				...
				if ($pedir_defeito_constatado_os_item <> 'f' OR $login_fabrica <> 5) {
					...
					if ($login_fabrica<>30 AND $login_fabrica <> 43 and $login_fabrica <> 85 and $login_fabrica <> 95 and $login_fabrica <> 94) {
					...
					if ($login_fabrica == 59 or $login_fabrica == 2 or $login_fabrica ==45 ) {
						...
						#HD 670814
						if(in_array($login_fabrica,$fabricas_padrao_itens) || $login_fabrica > 99 && $login_fabrica != 94){
						...
						<select name="defeito_constatado" id="defeito_constatado" class="frm"  onfocus="<?echo $onfocus_def_cons?>" onchange="<?echo $onchange_def_cons?>" style='width: 300px;'>
							<!-- <option value="">Selecione o Defeito Constatado</option> //--> <?php
						...
						if(in_array($login_fabrica,$fabricas_padrao_itens) || $login_fabrica > 99){
							...
							if (pg_num_rows($res_cons) > 0) {
								...
								for ($y = 0; $y < pg_num_rows($res_cons) ; $y++){
									...
									echo "<option $selected id='opcoes2_$y' value='$defeito_cons'>$defeito_constatado_desc</option>";
								}
							} else {
						...
						}elseif ($pedir_defeito_reclamado_descricao == 't' AND strlen($defeito_constatado) AND ($login_fabrica == 45 OR $login_fabrica == 15 OR $login_fabrica == 35 OR $login_fabrica == 2 OR $login_fabrica == 43 OR $login_fabrica == 46 OR $login_fabrica == 51 OR $login_fabrica == 30 OR $login_fabrica == 56 OR $login_fabrica == 40 OR $login_fabrica == 50 OR $login_fabrica > 56)) {
						...
						if ($login_fabrica != 95 && $login_fabrica != 94) {
							...
							if ($login_fabrica == 94) {
								...
								$sql_cons = "SELECT defeito_constatado, descricao
											FROM tbl_defeito_constatado
											WHERE fabrica = $login_fabrica $cond  order by descricao; ";
							$res_cons = pg_query($con, $sql_cons);
							...
							echo "</select>";
							echo "&nbsp;<input type='button' onclick=\"javascript: adicionaIntegridade3()\" value='Adicionar' name='btn_adicionar' style='width:70px; font:bold 9px;'><br>";
						}
					}
					echo "</td>";
				}
			...
			if ($login_fabrica == 19 || (in_array($login_fabrica,$fabricas_padrao_itens) || $login_fabrica > 99)) {
				...
				if ($pedir_solucao_os_item <> 'f' or (!in_array($login_fabrica,array(5,42,86,95,99)))) {
					echo "<font size='1' face='Geneva, Arial, Helvetica, san-serif'>Solução</font><BR>";
					...
					}elseif($pedir_defeito_reclamado_descricao == 't' AND strlen($solucao_os) > 0 AND ($login_fabrica == 45 OR $login_fabrica == 43 OR $login_fabrica == 15 OR $login_fabrica == 35 OR $login_fabrica == 2 OR $login_fabrica == 46 OR $login_fabrica == 51 OR $login_fabrica==30 OR $login_fabrica==50 OR $login_fabrica == 40 OR $login_fabrica==56 OR $login_fabrica>56)){
		...
		if (in_array($login_fabrica,array(30,94))) {
			...
			echo "<td width='10px;' align='right' valign='top'>
					Km<br>
					<input type='checkbox' name='km' id='km' value='1' onclick='javascript: verificaKM();' $check_km />
				</td>";
			echo "<td width='70px'>
					<div id='qtde_km_div' $style_km>
						Distância<br>
						<input type='text' name='qtde_km' id='qtde_km' value='$qtde_km' maxlength='5' style='width: 60px;' />
					</div>
				</td>";
...
if (in_array($login_fabrica, array(2,30,43,59,85,94,95))) {

    if ($login_fabrica != 95 && $login_fabrica != 94) {
		echo "<input type='button' onclick=\"adicionaIntegridade()\" value='Adicionar Defeito' name='btn_adicionar'><br>";
	}

    echo "<table style='font:bold 14px; display:none;float:left' align='center' width='100%' border='0' id='tbl_integridade' cellspacing='3' cellpadding='3'>
		    <thead>";
	...
	if (in_array($login_fabrica, array(2,43,59,85))) {
    ...
    if (pg_num_rows($res_dc) > 0) {
        ...
        for ($f = 0; $f < $qtde_integridade; $f++) {
            ...
            echo "<tr>";
				...
				<input type='hidden' name='integridade_defeito_constatado_$inc' id='integridade_defeito_constatado_$inc' value='$itens_integridade[$f]'></td>";
...
if ($login_fabrica == 19) {
    ...
    if (pg_num_rows($res_dr) > 0) {
        ...
        for ($x = 0; $x < pg_num_rows($res_dr); $x++) {
            ...
            echo "</td>";
            echo "<td valign='top'>";
            echo "<input type='button' onclick=\"adicionaIntegridade2('$aa','tab_defeitos_$aa',document.frm_os.defeito_reclamado_$aa,document.frm_os.defeito_reclamado_descricao_$aa,document.frm_os.defeito_constatado_$aa)\" value='Adicionar Defeito' name='btn_adicionar'>";
            echo "</td>";
				...
				<td nowrap>
					...
					if ($defeito_constatado_por_familia == 't') {
						...
						if ($login_fabrica == 1) {
							...
							if ($linha == 198) {
								$sql .= "LEFT JOIN tbl_produto_defeito_constatado ON tbl_produto_defeito_constatado.defeito_constatado =  tbl_defeito_constatado.defeito_constatado AND tbl_produto_defeito_constatado.produto = $produto_os ";
							}
            ...
            <td align="left" nowrap>
                ...
                <select width="650" name="solucao_os" size="1" class="frm"  style='width: 220px;'>
                    ...
                    if (pg_num_rows($res) == 0) {
                        ...
                        $sql .= " AND tbl_servico_realizado.linha IS NULL AND tbl_servico_realizado.ativo IS TRUE ORDER BY descricao ";
                        $res = pg_query($con,$sql) ;
...
if ($tipo_atendimento == 22) {
	...
	if (in_array($login_fabrica, array(15,74,86))) {
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0">
			<tr>
				<td align="right" nowrap>
					<font size="1" face="Geneva, Arial, Helvetica, san-serif">Qtde. Itens:</font><br>
					<select name="n_itens" size="1" class="frm" onchange='mostrarLinhas(this.value);'><?php
	...
	if (in_array($login_fabrica, array(3,45,51))) {
	...
	if (in_array($login_fabrica, array(19,30,59,3)) or ( $login_fabrica == 94 && $posto_interno === true)) {?>
			<table width="100%" border="0" cellspacing="5" cellpadding="0">
			<tr>
				<td align="left" nowrap>
					<font size="1" face="Geneva, Arial, Helvetica, san-serif">Nome do Técnico</font>
					<br /><?php
					if ((in_array($login_fabrica, array(59)) && strlen($tecnico_nome) == 0) OR (in_array($login_fabrica, array(3)))) {
						...
						if (pg_num_rows($res_tecnico) == 0) {
							echo "<a href='tecnico_cadastro.php' title='É necessário cadastrar o técnico do Cadastro de Técnicos para continuar' style='font-size:9pt;' target='_blank'>Cadastre um técnico para continuar</a>";
						} else {
						...
						<input type='text' name='tecnico_nome' size='20' maxlength='20' value='<? echo $tecnico_nome ?>' /><?php
...
?>
			...
			<tr>
				<td align="right" nowrap>
					...
					<select name="n_itens" size="1" class="frm" onchange='javascript:mostrarLinhas(this.value);'><?php
						for ($val = 50; $val <= $qtde_item; $val+=50) {
		...
		if ($login_fabrica == 15 and strlen($os) > 0) {
        ...
        if ($login_fabrica == 15 || $login_fabrica == 24) {
            ...
            echo "<td align='center' style='width:150px'><b>&nbsp; Código &nbsp;</b></td>";
        }
                /*
        ...
        if ($login_fabrica == 15 || $login_fabrica == 24) {
            ...
            echo "<td align='center' width='60'><b>Descrição</b></td>";
        }
        ...
        if ($pergunta_qtde_os_item == 't') {
            echo "<td align='center' style='width:100px'><b>Qtde</b></td>";
        }
	...
	if(!in_array($login_fabrica,array(96,95)) and !in_array($login_fabrica,$fabricas_padrao_itens) && $login_fabrica < 99){
...
//takashi 04-04-07 hd 1819
                ...
                if ($login_fabrica == 15) {//HD 258901
                    ...
                    echo "<td align='left' bgcolor='$cor_coluna'><input class='frm' type='text' name='descricao_$i' size='45' value='$descricao[$i]'>&nbsp;<img src='imagens/lupa.png' border='0' align='absmiddle'";
                     echo " onclick='javascript: fnc_pesquisa_lista_basica_suggar (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.posicao_$i , document.frm_os.preco_$i, document.frm_os.voltagem, \"descricao\", document.frm_os.kit_peca_$i)'";
                    echo " alt='Clique para efetuar a pesquisa' style='cursor:pointer;'></td>\n";
                    ...
                    <input class='frm' type='text' name='peca_$i' $block_item id='peca_$i'size='15' value='$peca[$i]'"; echo " "; if($login_fabrica==30 or $login_fabrica ==43) echo " onfocus=\"pega_peca('$os','peca_$i','descricao_$i','$i');\" onblur=\"pega_peca('$os','peca_$i','descricao_$i','$i'); atualizaQtde(document.frm_os.peca_$i,document.frm_os.qtde_$i);\" ";
					...
					if ($login_fabrica == 14) echo " onclick='javascript: fnc_pesquisa_peca_lista_intel (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.posicao_$i , \"referencia\")'";
						...
						echo " onclick='javascript: fnc_pesquisa_peca_lista (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.preco_$i , document.frm_os.voltagem,\"referencia\", document.frm_os.qtde_$i)'";
                ...
                if ($login_fabrica == 15) {//HD 258901
					...
					echo " >&nbsp;<img src='imagens/lupa.png' border='0' align='absmiddle'";
                    echo " onclick='javascript: fnc_pesquisa_lista_basica_suggar (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.posicao_$i , document.frm_os.preco , document.frm_os.voltagem, \"referencia\", document.frm_os.kit_peca_$i)'";
                    echo " alt='Clique para efetuar a pesquisa' style='cursor:pointer;'>";

				} else if ($login_fabrica == 95) {

					echo "<td  align='center' bgcolor='$cor_coluna' nowrap> <input class='frm' type='text' name='descricao$i' $block_item id='descricao_$i'size='40' value='$peca[$i]'>&nbsp;<img src='imagens/lupa.png' border='0' align='absmiddle'
					onclick='fnc_pesquisa_lista_serie(document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.preco , document.frm_os.voltagem, \"descricao\", document.frm_os.qtde_$i)' alt='LISTA BÁSICA' style='cursor:pointer;'></TD>";

				} else {

                    echo "<td align='center' nowrap bgcolor='$cor_coluna'><input class='frm' type='text' name='descricao_$i' id='descricao_$i' $block_item size='25' value='$descricao[$i]' ";
                    ...
                    if ($login_fabrica == 14) echo " onclick='fnc_pesquisa_peca_lista_intel (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.posicao_$i , \"descricao\")'";
						...
						if ($login_fabrica == 40) {
							...
							echo " onclick='fnc_pesquisa_peca_lista (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.preco_$i , document.frm_os.voltagem,\"descricao\", document.frm_os.qtde_$i, $i)'";
						}

					} else {
						echo " onclick='fnc_pesquisa_peca_lista (document.frm_os.produto_$i.value , document.frm_os.peca_$i , document.frm_os.descricao_$i , document.frm_os.preco_$i , document.frm_os.voltagem,\"descricao\", document.frm_os.qtde_$i, $i);mudaFocus($i)'";
					}
                ...
                if ($pergunta_qtde_os_item == 't') {
                    ...
                    echo "<td align='center' bgcolor='$cor_coluna'>";
						echo "<input class='frm' type='text' id='qtde_$i' name='qtde_$i' size='3' value='".$qtde[$i]."' $block_item>";
					echo "</td>\n";
			...
			if (!in_array($login_fabrica,array(96,95)) and !in_array($login_fabrica,$fabricas_padrao_itens) && $login_fabrica < 99) {
				...
				if ($login_fabrica == 24 or $login_fabrica == 6 or $login_fabrica == 50 or $login_fabrica == 74) {
            ...
            if ($login_fabrica == 6) {
                ...
                echo ">";
                echo "<option value='' selected>Selecione Serviço</option>";

                $sql = "SELECT *
                        FROM   tbl_servico_realizado
                        WHERE  tbl_servico_realizado.fabrica = $login_fabrica ";

                if ($login_pede_peca_garantia == 't' AND $login_fabrica <> 1 AND $login_fabrica <> 24 and $login_fabrica<>15 and $login_fabrica <> 52 and $login_fabrica <> 85 and $login_fabrica <> 42 and $login_fabrica <> 91 and $login_fabrica <> 72 and $login_fabrica <> 95  and $login_fabrica <> 104 and $login_fabrica <> 105) {
                    $sql .= "AND tbl_servico_realizado.descricao NOT ILIKE 'troca%' ";
                }
				/* HD 415550 */
				if($login_fabrica == 94) {
				...
				/*
                if ($login_fabrica == 3 and $controla_estoque == 't') {
                    ...
                    if(pg_num_rows($res) > 0){
                        ...
                        for ($k = 0; $k < pg_num_rows($res); $k++) {
                            ...
                            echo "<tr style='font-size: 11px'>";
                                echo "<td>";
                                    echo "<input type='".($login_fabrica == 15 ? 'hidden' : 'checkbox')."' name='kit_peca_$kit_peca_peca' $checked value='$kit_peca_peca'>";
...
if (($login_fabrica == 11 OR $login_fabrica == 10) AND $login_posto == 6359) {?>
    ...
    <tr>
        <td height="27" valign="middle" align="center"  <?php echo ($anexaNotaFiscal) ?"colspan='4'":"colspan='3'"?> bgcolor="#FFFFFF">
			...
			<?php }else{?>
				<!-- HD 342629 -->
				<input type='button' value='GRAVAR' class="verifica_servidor" rel="frm_os"  style='cursos:pointer;'  ALT="Gravar itens da Ordem de Serviço" title='Gravar itens da Ordem de Serviço' ONCLICK="javascript: if (document.frm_os.btn_acao.value == '' ) { document.frm_os.btn_acao.value='gravar' ; }" >
			<?php }?>
			<?php
