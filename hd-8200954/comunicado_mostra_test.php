...
$acao = $_GET['acao'];
if($acao){
	if(!$data_inicial_01 OR !$data_final_01)
		$erro = "Data Inválida";
}
if(strlen($erro==0)){
...
$title = "COMUNICADOS ".strtoupper($login_fabrica_nome);
$layout_menu = "tecnica";
...
function fnc_pesquisa_produto2 (campo, campo2, tipo, voltagem, referencia_pai, descricao_pai, referencia_avo, descricao_avo) {
...
?>
<form name="frm_comunicado" method="get" action="comunicado_mostra_pesquisa_test.php">

<input type="hidden" name="acao">

<table width="700" align="center" border="0" cellspacing="0" cellpadding="2">
	<tr class="titulo_tabela">
		<td colspan="5" align="center">Parâmetros de Pesquisa</td>
	</tr>
	...
	if(pg_num_rows($res)>0){
		for($x=0;pg_num_rows($res)>$x;$x++){
			$tipo_comunicado = pg_fetch_result($res,$x,tipo);
	...
	<tr>
		...
		<img src="imagens/btn_lupa.gif" border="0" align="absmiddle" onclick="javascript: fnc_pesquisa_produto2 (document.frm_comunicado.produto_referencia, document.frm_comunicado.produto_descricao, 'referencia', document.frm_comunicado.produto_voltagem)" alt="<? fecho("clique.aqui.para.pesquisar.pela.referencia.do.produto",$con,$cook_idioma);?>" style="cursor: hand;">
		</td>
		<td align="left" nowrap>
			...
			<img src="imagens/btn_lupa.gif" border="0" align="absmiddle" onclick="javascript: fnc_pesquisa_produto2 (document.frm_comunicado.produto_referencia, document.frm_comunicado.produto_descricao, 'descricao', document.frm_comunicado.produto_voltagem)" alt="<? fecho("clique.aqui.para.pesquisar.pela.descricao.do.produto",$con,$cook_idioma);?>" style="cursor: hand;">
		</td>
		<td>&nbsp;</td>
	</tr>
	<tr>
	...
	<tr align="center">
		<td colspan="6"><input type='image' src="<? if ($sistema_lingua == "ES") echo "admin_es/imagens_admin/btn_pesquisar_400.gif";else  echo "imagens/btn_pesquisar_400.gif"?>" onClick="document.frm_comunicado.acao.value='PESQUISAR'; document.frm_comunicado.submit();" style="cursor: hand;" alt="<?fecho("preencha.as.opcoes.e.clique.aqui.para.pesquisar",$con,$cook_idioma);?>"></td>
	</tr>
	</tbody>
</table>
...
if (strlen ($tipo) > 0 AND strlen ($comunicado) == 0) {
	...
	if($login_fabrica==19){
		...
		echo "</td>";
		echo "<td>";
		fecho ("Titulo/Descrição",$con,$cook_idioma);
		echo "</td>";
		if ($opcao == "1"){
	...
	for ($i=0; $i<$total; $i++) {
		...
		if($login_fabrica<>19){
			...
			if (strlen($mensagem) > 0)
				echo "<div style='position: relative; float: right; width: 5; height: 5;'><acronym title='".
					  traduz("mensagem",$con,$cook_idioma).": $mensagem'><img border='0' src='imagens/comentario.gif' align='top' style='cursor: hand;'></acronym></div>";
		...
		if($login_fabrica==19){
			...
			echo "<tr bgcolor='$cor'>";
			if ($mensagem =='') {
				echo "<td colspan='5' align='center' style='color:#B7B7B7;'><br>".
				traduz("mensagem.nao.cadastrada",$con,$cook_idioma)."<br>&nbsp;</td>";
			} else {
...
if ((strlen($comunicado) > 0) && (pg_num_rows($res) > 0)) {
...
// 																"'Apresentação do Produto','Manual Técnico')";
	...
	$sql = "SELECT	tbl_comunicado.tipo,
			...
			AND    ((tbl_comunicado.posto     = $login_posto) OR (tbl_comunicado.posto           IS NULL))
			AND    tbl_comunicado.ativo IS TRUE";
// 	if ($login_fabrica==14) $sql .= $sql_so_com_admin;
	if($login_fabrica == 20) $sql .= " AND tbl_comunicado.pais = '$login_pais' ";
	...
	if (pg_num_rows($res) > 0) {
		...
		echo "<tr class='titulo_tabela'>";
		echo "<td colspan='2'>Tipos de Comunicados Disponíveis</td>";
		echo "</tr>";

		echo "<tr class='titulo_coluna'>";
		echo "<td>Tipo</td>";
		echo "<td>".traduz("qtde",$con,$cook_idioma)."</td>";
		echo "</tr>";
		...
		for ($i = 0 ; $i < pg_num_rows($res) ; $i++) {
			...
			if(pg_fetch_result ($res,$i,tipo) == ''){
				echo "<td nowrap >";
				echo "<a href='comunicado_mostra_pesquisa_test.php?acao=PESQUISAR&tipo=zero'> ".traduz("sem.titulo",$con,$cook_idioma)." </a>";
				echo "</td>";
				echo "<td align='right'>";
				...
				if(pg_fetch_result ($res,$i,tipo) =='Esquema Elétrico'){
					...
					if(pg_fetch_result($res,$i,tipo)=='Esquema Elétrico Atual' AND $login_posto == 6359){
						...
						echo "<a href='comunicado_mostra_pesquisa_test.php?acao=PESQUISAR&tipo=" . pg_fetch_result ($res,$i,tipo) . "'>";
		...
		if($login_fabrica ==14) {?>
			<tr bgcolor='#ffeecc'>
				<td nowrap>
					<a href='comunicado_mostra_pesquisa_test.php?acao=PESQUISAR&tipo=todos'
...
if($login_fabrica <> 3) { // HD 56703
?>
<form name="frm_comunicado" action="comunicado_mostra_pesquisa_test.php" method="get"> 

<input type="hidden" name="acao">

<table width="700" align="center" border="0" cellspacing="0" cellpadding="2">
	<? if(strlen($erro)>0){ ?>
		<tr class='msg_erro'><td colspan='4'><? echo $erro; ?>
	<? } ?>
	<tr class="titulo_tabela">
		<td colspan="5" align="center">Parâmetros de Pesquisa</td>
	</tr>
	<tr class="Conteudo" bgcolor="#D9E2EF" >
		...
		<td align="left">
			<? fecho ("data.inicial",$con,$cook_idioma);?><br>
			<input size="13" maxlength="10" type="text" name="data_inicial" id="data_inicial" value="" onclick="javascript: if (this.value == 'dd/mm/aaaa') { this.value=''; }">
		</td>
		<td align="left">
			<? fecho ("data.final",$con,$cook_idioma);?><br>
			<input size="13" maxlength="10" type="text" name="data_final" id="data_final" value="" onclick="javascript: if (this.value == 'dd/mm/aaaa') { this.value=''; }">
		</td>
		<td width="40">&nbsp;</td>
	</tr>
	<tr class="Conteudo" bgcolor="#D9E2EF">
	...
	<tr class="Conteudo" bgcolor="#D9E2EF">
		...
		<td align="left" nowrap>
			<? fecho("referencia",$con,$cook_idioma);?><br>
			<input type="text" name="produto_referencia" size="20" class="frm" value="<? echo $produto_referencia ?>" >
		<img src="imagens/btn_lupa.gif" border="0" align="absmiddle" onclick="javascript: fnc_pesquisa_produto2 (document.frm_comunicado.produto_referencia, document.frm_comunicado.produto_descricao, 'referencia', document.frm_comunicado.produto_voltagem);" style="cursor: hand;">
		</td>
		<td align="left" nowrap>
			<? fecho("descricao",$con,$cook_idioma);?><br>
			<input type="text" name="produto_descricao" size="40" class="frm" value="<? echo $produto_descricao ?>">
			<input type="hidden" name="produto_voltagem">
			<img src="imagens/btn_lupa.gif" border="0" align="absmiddle" onclick="javascript: fnc_pesquisa_produto2 (document.frm_comunicado.produto_referencia, document.frm_comunicado.produto_descricao, 'descricao', document.frm_comunicado.produto_voltagem);" style="cursor: hand;">
		</td>
		<td>&nbsp;</td>
	</tr>
	<tr bgcolor="#D9E2EF">
...
if (strlen($comunicado) == 0 and strlen($tipo) == 0){
	...
	if (pg_num_rows($res) > 0) {
		...
		for ($i = 0 ; $i < pg_num_rows($res) ; $i++) {
			...
			echo "<td nowrap>";
				if($login_fabrica==50 and trim(pg_fetch_result($res,$i,video)<>'')){
					echo "<A href=\"javascript:window.open('/assist/video.php?video=".trim(pg_fetch_result($res,$i,video))."','_blank'," .
						 "'toolbar=no, status=no, scrollbars=no, resizable=yes, width=460, height=380');void(0);\">" .
						 "Abrir vídeo</A>";
				}else{
