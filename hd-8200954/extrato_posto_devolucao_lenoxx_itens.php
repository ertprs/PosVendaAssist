...
<br><br>
<b style='font-size:14px;font-weight:normal'>Emitir as NF de devolução nos mesmos valores e impostos, referenciando NF de origem Lenoxx, e postagem da NF para a Lenoxx</b>
	</TD>
</TR>
</table>
...
if ($qtde_for > 0) {
	...
	for ($i=0; $i < $qtde_for; $i++) {
		...
		$sqlq  = "SELECT extrato
					FROM tbl_extrato
					WHERE extrato= $extrato
					AND  data_geracao::date='2010-10-16'";
		$resq = pg_query($con,$sqlq);
		if(pg_num_rows($resq) > 0){
			if ($movimento!='RETORNAVEL') $devolucao = "ENVIADA EM DUPLICIDADE";
		}
