<?php
	...
	if ($_POST["publicar"]){
		...
		if(strlen($titulo) == 0 and strlen($mensagem) == 0){
			...
			$msg_erro = traduz("<b> Preencha os campos obrigatórios. </b>");
...
?>
	<br>
		...
		<div class="alert alert-info tac">
				...
				<div class="tac">
					<?=traduz('Sua dúvida ou resposta deverá ser postada no <strong>Fórum do Portal de Treinamentos Britania</strong>, acesse o site: ')?><a href='http://treinamento.britania.com.br' target='_blank'><u>http://treinamento.britania.com.br</u></a> <?=traduz('e poste sua mensagem.')?>
				</div>
...
?>
	<div class="alert alert-info tac">
		...
		<div class="tac">
			...
			<?=traduz('<strong> * Atenção: </strong>O tópico será disponibilizado após <u>aprovação</u> do fabricante. Por gentileza, aguarde.')?>
...
?>
		...
		<div class="tac">
			...
			<?=traduz('<strong> * Atenção: </strong>A mensagem será gravada no tópico após <u>aprovação</u> do fabricante. Por gentileza, aguarde.')?>
...
<script>
	$(function(){
    	...
    	$(".btn-submit").click(function(){
    		if ($("#mensagem").val() == "") {
    			$("#msg").addClass("error");
    			alert('<?=traduz("Preencha a mensagem")?>');
    		} else {
    			$("#form_topico").submit();
    		}
    	});
