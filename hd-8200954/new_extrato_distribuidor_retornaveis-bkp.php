...
if (strlen ($data) > 0) {
	$sql = "SELECT  tbl_os_extra.linha           ,
			...
			JOIN (SELECT tbl_peca.peca, tbl_peca.referencia, tbl_peca.descricao FROM tbl_peca WHERE tbl_peca.fabrica = $login_fabrica AND devolucao_obrigatoria) tbl_peca ON tbl_os_item.peca = tbl_peca.peca
...
#exit;
	...
	for ($i = 0 ; $i < pg_numrows ($res) ; $i++) {
		if ($linha_ant <> pg_result ($res,$i,linha) ) {
			...
			echo "<td colspan='3' align='center'><font size='+1' color='#ffffff'>Peças da Linha: " . pg_result ($res,$i,linha_nome) . "</font></td>\n";
			echo "</tr>\n";
