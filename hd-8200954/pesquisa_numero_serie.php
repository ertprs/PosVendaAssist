...
function fn_envia_serie(tipo,referencia,descricao,voltagem){
	...
	if (confirm('Atenção! Mais de um produto com o mesmo número de série. Verifique se o produto selecionado está correto.')) {
...
if (strlen($HTTP_GET_VARS["produto_serie"]) > 5) {
	...
	if (pg_numrows ($resx) > 0 and $login_fabrica <> 50) {
		echo "<script language='javascript'>";
		echo "alert('Atenção! Mais de um produto com o mesmo número de série. Verifique se o produto selecionado está correto.')";
		echo "</script>";
	}
