...
//--====================LISTA DE PREÇOS ====================--\\
$sql = "SELECT count(comunicado)AS total_lancamentos 
		FROM tbl_comunicado 
		WHERE ativo IS TRUE 
		AND tipo = 'Lançamentos'
		AND (tbl_comunicado.tipo_posto = $tipo_posto  OR  tbl_comunicado.tipo_posto IS NULL)
		AND fabrica    = $login_fabrica";
$res = pg_exec ($con,$sql);
if (pg_numrows($res) > 0) {
	$total_lancamentos  = trim(pg_result ($res,0,total_lancamentos));
}
...
<table width="700" border="0" cellspacing="0" cellpadding="0" align='center'>
	<tr bgcolor = '#efefef'>
		...
		<td  class="chapeu" colspan='2' >Lançamentos - Novos Produtos</td>
	</tr>
...
$sql = "SELECT DISTINCT tbl_familia.familia                                  ,
		...
		AND     tbl_comunicado.tipo = 'Lançamentos'
...
if (pg_numrows($res) > 0) {
	...
	for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
		...
		if($linha_anterior <> $linha) {
						echo "<br><dt>&nbsp;&nbsp;<b>»</b> <a href='info_tecnica_visualiza.php?tipo=Lançamentos&linha=$linha'>$nome </a><br></dt>";
		}
		if($login_fabrica<>19){
			echo "<dd>&nbsp;&nbsp;<b>»</b> <a href='info_tecnica_visualiza.php?tipo=Lançamentos&linha=$linha&familia=$familia'>$descricao</a><br></dd>";
		}
...
<br>
		...
		<td rowspan='2'class='detalhes' width='350'>Clique no produtos para ver os Lançamentos.</td>
	</tr>
</table>
<br>
