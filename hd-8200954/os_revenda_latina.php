		...
		if ($tipo_busca == 'revenda') {
			...
			if ($busca == 'nome') {
				...
				$sql = "SELECT tbl_revenda_fabrica.revenda,
						...
						FROM tbl_revenda_fabrica
						WHERE tbl_revenda_fabrica.fabrica = $login_fabrica
						AND  tbl_revenda_fabrica.cnpj LIKE '$q%' ";
	...
	if (strlen($msg_erro) == 0) {
		...
		if (strlen($msg_erro) == 0) {
			...
			/* ===========INÍCIO DO PROCESSO DA IMAGEM =============== */
				...
				if($login_posto <> 343192 and strlen($msg_erro) == 0){
					if ($anexaNotaFiscal) {

						if($_FILES['foto_nf']['name'] == ''){
							...
							if($dados_anexo['name'] == ''){
								...
								$anexou = anexaNF("r$os_revenda", $dados_anexo); // Para anexar uma imagem de NF de os revenda, colocar um 'r' antes do nº
...
//	if (getenv("REMOTE_ADDR") == "201.0.9.216") echo nl2br($sql)."<br>".pg_numrows($res); exit;
	...
	if (pg_numrows($res) > 0){
		...
		$args_replace_cnpj = array('/','.','-',' ');
		$revenda_cnpj_raiz = str_replace($args_replace_cnpj,'', $revenda_cnpj);
		$revenda_cnpj_raiz = substr($revenda_cnpj_raiz, 0,8);
...
function adicionaLinha(linha){
	...
	$("input[name^=aux_nota_fiscal_]").numeric();
}
...
<!-- 						<input name="data_abertura" size="12" maxlength="10" value="<? if (strlen($data_abertura) == 0) $data_abertura = date("d/m/Y"); echo $data_abertura; ?>" type="text" class="frm" tabindex="0" > <font face='arial' size='1'> Ex.: <? echo date("d/m/Y"); ?></font> -->
						<input name="data_abertura" <?php if($os_revenda > 0 and $login_fabrica == 15){ echo "readonly='true' ";} ?> size="11" maxlength="10" value="
<?
...
?>" type="text" ><br><font face='arial' size='1'>Ex.: <? echo date("d/m/Y"); ?></font>
					...
					<td nowrap align='center'>
						<input name="data_nf" size="11" maxlength="10"value="<? echo $data_nf ?>" type="text" class="frm" tabindex="0" > <font face='arial' size='1'> Ex.: <? echo date("d/m/Y"); ?></font>
					</td>
				</tr>
			...
			<table width="100%" border="0" cellspacing="3" cellpadding="2">
				<?if ($anexaNotaFiscal and (!empty($qtde_item))) {
			...
			<?	if($temImg) {
				...
				if (($anexa_duas_fotos and $temImg < LIMITE_ANEXOS) or $temImg == 0) { ?>
					...
					<td nowrap align='center' size='11' id='nf_table_item' colspan='<?=$inputAnexoColSpan?>'>
						<?=$inputNotaFiscal?>
				<?} ?>
					</td>
				</tr>
			...
			<table width="100%" border="0" cellspacing="3" cellpadding="2">
				<tr class="menu_top">
					...
					<td>
						<font size="1" face="Geneva, Arial, Helvetica, san-serif">CNPJ Raiz <?=$revenda_aux;?></font>
					</td>
					<!--
					<td>
			...
			<?/* Foi modificado por Fernando. Foi colcoado o readonly nos campos Fone e e-mail
				...
				<tr>
					<td align='center'>
						...
						<img src='imagens/lupa.png' border='0' align='absmiddle' onclick='pesquisaRevenda(document.frm_os.revenda_nome, "nome")' style='cursor:pointer;'>

					</td>

					<td>
						<input type="text" name="revenda_cnpj_raiz" class="frm" id="revenda_cnpj_raiz" value="<?=$revenda_cnpj_raiz?>" maxlength="8" >
                    <img src="imagens/lupa.png" border="0" align="absmiddle" onclick="pesquisaRevenda(document.frm_os.revenda_cnpj_raiz, 'cnpj' ) " style="cursor: pointer">
					</td>
				...
				<tr>
					<td colspan="2">
						<p>
							<b> * Caso não encontre o cadastro da revenda, favor enviar um e-mail para cadastro@latinatec .com.br com <br /> o CNPJ e Razão social da loja.</b>
						</p>
					</td>
				</tr>
			</table>
...
if(strlen($os_revenda) <> 0 OR $btn_acao == 'gravar'){
	...
	//echo $os_revenda;
		...
		$size_qtde = ($login_fabrica == 15) ? "size='20'" : "size='8'";
		...
		echo "<td align='center'><input class='frm' type='text' name='produto_serie_$i'  $size_qtde  maxlength='20'  value='$serie'>&nbsp;";
