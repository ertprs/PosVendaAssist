		...
		for ($i = 0 ; $i < pg_numrows($res) ; $i++) {
			...
			if(pg_result ($res,$i,tipo) == ''){
				...
				if(pg_result ($res,$i,tipo) =='Esquema Elétrico'){
					...
					echo "</td>";
					echo "</tr>";
				}elseif($login_fabrica == 14 && pg_result ($res,$i,tipo) =='Esquema Elétrico Atual'){
					echo "<td nowrap>";
					echo "<a href='comunicado_mostra_pesquisa_agrupado.php?acao=PESQUISAR&tipo=" . pg_result ($res,$i,tipo) . "'>";
					echo pg_result ($res,$i,tipo);
					echo "</a>";
					echo "</td>";
					echo "<td align='right'>";
	...
	if($login_fabrica == 14){ // HD 44360
		...
		<table align='center'>
			<tr bgcolor='#f0f0f0'>
				...
				<td nowrap width='260'>
				<a href='http://ecenter.intelbras.com.br/outpublisher/suporte/pt/restrito.php' class='menu' target='_blank'><? fecho("esquemas.eletricos.descritivo.tecnico.manual.boletim.informativo.tecnico",$con,$cook_idioma);?></a>
				</td>
				<td nowrap class='descricao'><? fecho("exibicao.dos.esquemas.eletricos.dos.produtos",$con,$cook_idioma);?></td>
			</tr>
		</table>
		<?
		include "rodape.php";
