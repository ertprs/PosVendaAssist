...
if($login_fabrica <> 7 AND $login_fabrica <> 16){
...
/*
	if(strlen($valor_diaria) > 0)
		$xvalor_diaria = "'".str_replace(",",".",$valor_diaria)."'";
	else
		$xvalor_diaria = 'null';
*/
	...
	if(strlen($valor_total_hora_tecnica) > 0)
		$xvalor_total_hora_tecnica = "'".str_replace(",",".",$valor_total_hora_tecnica)."'";
	else
		$xvalor_total_hora_tecnica = '0';
...
<TR>
	<TD class="line_lst">Razão Social</TD>
...
if ($mao_de_obra_por_hora == 't') {
	...
	$valor_total_horas = $hora_tecnica;
}
...
$sub_total_mobra = $regulagem_peso_padrao + $certificado_conformidade + $valor_total_horas + $total_por_deslocamento_km + $total_diaria + $taxa_visita;
