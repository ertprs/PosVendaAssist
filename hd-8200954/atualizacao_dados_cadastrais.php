		...
		for ($i=0; $i < $qtde_perg; $i++) {
			...
			if(strlen(trim($txt_resposta)) == 0){
				if($pergunta != 580){
					if(count($erro) > 0){
						...
						$erro[] = "Favor preencher todas as respostas obrigatórias";
					}
				}
			}
			...
			if($pergunta == 578 AND strlen(trim($txt_resposta)) > 0){
				if (validaEmail($txt_resposta) === false) {
					$erro[] = "O e-mail inserido é invalido!";
				}
			}
	...
	if (pg_num_rows($res)>0) {
		...
		foreach (pg_fetch_all($res) as $key) {
			...
			if((empty($tipo_pergunta) or $tipo_pergunta <> $key['tipo_pergunta']) and !empty($key['tipo_pergunta'])){
				$html_pesquisa .="  <tr bgcolor='$cor' style='display:none;'>
									...
									<tr>
										...
										Prezado parceiro,<br/>
											Para mantermos seu cadastro atualizado e melhorar nosso meio de comunicação,
											pedimos que reserve 1 minuto para preencher os dados abaixo.<br/><br/>
											Obs: Campos obrigatórios em vermelho.
										</td>
									</tr>
									";
			}
			//Obs: (Perguntas com <span style='color:red;'>*</span> preenchimento obrigatório).
			...
			$html_pesquisa .= 			$key['descricao'];
										// if($key['pergunta'] == 562){
										// 	$html_pesquisa .= " (Pessoa responsável)";
										// }
			$html_pesquisa .= "		</td>";
					...
					for ($x=0; $x < pg_num_rows($res); $x++) {
						...
						switch ($item_tipo_resposta_tipo) {
							case 'text':
								unset($place);
								if($key['pergunta'] == 581){
									$place = 'placeholder=" Pessoa responsável"';
								}
								if($key['pergunta'] == 580){
